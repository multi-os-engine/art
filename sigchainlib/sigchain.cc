...
class SignalAction {
 public:
  SignalAction() : claimed_(false), uses_old_style_(false), managed_handler_(nullptr) {
  }
  ...
  void SetManagedHandler(SpecialSignalHandlerFn fn) {
    managed_handler_ = fn;
  }

  SpecialSignalHandlerFn GetManagedHandler() {
    return managed_handler_;
  }
 ...
 private:
  ...
  SpecialSignalHandlerFn managed_handler_;  // A special handler executed before user handlers.
};
...
extern "C" void InvokeUserSignalHandler(int sig, siginfo_t* info, void* context) {
  ...
  SpecialSignalHandlerFn managed = user_sigactions[sig].GetManagedHandler();
  if (managed != nullptr) {
...
extern "C" void SetSpecialSignalHandlerFn(int signal, SpecialSignalHandlerFn fn) {
  ...
  user_sigactions[signal].SetManagedHandler(fn);
