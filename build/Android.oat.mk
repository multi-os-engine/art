...
define create-core-oat-host-rules
  ...
  ifeq ($(1),optimizing)
    ...
    core_image_name := $($(2)HOST_CORE_IMG_OUT_BASE)-optimizing$(CORE_IMG_SUFFIX)
    core_oat_name := $($(2)HOST_CORE_OAT_OUT_BASE)-optimizing$(CORE_OAT_SUFFIX)
  else
    ifeq ($(1),interpreter)
      core_compile_options += --compiler-filter=interpret-only
      core_image_name := $($(2)HOST_CORE_IMG_OUT_BASE)-interpreter$(CORE_IMG_SUFFIX)
      core_oat_name := $($(2)HOST_CORE_OAT_OUT_BASE)-interpreter$(CORE_OAT_SUFFIX)
    else
      ifeq ($(1),default)
        core_image_name := $($(2)HOST_CORE_IMG_OUT_BASE)$(CORE_IMG_SUFFIX)
        core_oat_name := $($(2)HOST_CORE_OAT_OUT_BASE)$(CORE_OAT_SUFFIX)
      else
...
define create-core-oat-target-rules
  ...
  ifeq ($(1),optimizing)
    core_compile_options += --compiler-backend=optimizing
    core_image_name := $($(2)TARGET_CORE_IMG_OUT_BASE)-optimizing$(CORE_IMG_SUFFIX)
    core_oat_name := $($(2)TARGET_CORE_OAT_OUT_BASE)-optimizing$(CORE_OAT_SUFFIX)
  else
    ifeq ($(1),interpreter)
      core_compile_options += --compiler-filter=interpret-only
      core_image_name := $($(2)TARGET_CORE_IMG_OUT_BASE)-interpreter$(CORE_IMG_SUFFIX)
      core_oat_name := $($(2)TARGET_CORE_OAT_OUT_BASE)-interpreter$(CORE_OAT_SUFFIX)
    else
      ifeq ($(1),default)
        core_image_name := $($(2)TARGET_CORE_IMG_OUT_BASE)$(CORE_IMG_SUFFIX)
        core_oat_name := $($(2)TARGET_CORE_OAT_OUT_BASE)$(CORE_OAT_SUFFIX)
      else
        $$(error found $(1) expected default, interpreter or optimizing)
      endif
    endif
...
LOCAL_ADDITIONAL_DEPENDENCIES += $(HOST_CORE_IMG_OUTS)
...
LOCAL_ADDITIONAL_DEPENDENCIES := art/build/Android.common.mk
LOCAL_ADDITIONAL_DEPENDENCIES += art/build/Android.oat.mk
LOCAL_ADDITIONAL_DEPENDENCIES += $(TARGET_CORE_IMG_OUTS)
include $(BUILD_PHONY_PACKAGE)
