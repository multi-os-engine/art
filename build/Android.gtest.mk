...
.PHONY: $$(gtest_rule)
$$(gtest_rule): $$(ART_GTEST_$(1)_DEPS) $$(ART_NATIVETEST_OUT)/$$(TARGET_$(2)ARCH)/$(1) test-art-target-sync
	$(hide) adb shell touch $(ART_TARGET_TEST_DIR)/$(TARGET_$(2)ARCH)/$$@-$$$$PPID
	$(hide) adb shell rm $(ART_TARGET_TEST_DIR)/$(TARGET_$(2)ARCH)/$$@-$$$$PPID
	$(hide) adb shell chmod 755 $(ART_TARGET_NATIVETEST_DIR)/$(TARGET_$(2)ARCH)/$$(notdir $$<)
	$(hide) ($$(call ART_TEST_SKIP,$$@) && \
	  (adb shell sh -c "$(ART_TARGET_NATIVETEST_DIR)/$(TARGET_$(2)ARCH)/$$(notdir $$<) \
	    && touch $(ART_TARGET_TEST_DIR)/$(TARGET_$(2)ARCH)/$$@-$$$$PPID" &&
...
.PHONY: $$(gtest_rule)
$$(gtest_rule): $$(gtest_exe) test-art-host-dependencies $$(ART_GTEST_$(1)_DEPS)
