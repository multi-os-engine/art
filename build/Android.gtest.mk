...
.PHONY: art-valgrind-target
art-valgrind-target: $(ART_VALGRIND_TARGET_DEPENDENCIES)
	$(hide) adb push art/test/valgrind-target-suppressions.txt $(ART_TARGET_TEST_DIR)
...
valgrind-$$(gtest_rule): art-valgrind-target test-art-target-sync
	$(hide) adb shell touch $(ART_TARGET_TEST_DIR)/$(TARGET_$(2)ARCH)/$$@-$$$$PPID
	$(hide) adb shell rm $(ART_TARGET_TEST_DIR)/$(TARGET_$(2)ARCH)/$$@-$$$$PPID
	$(hide) adb shell chmod 755 $(ART_TARGET_NATIVETEST_DIR)/$(TARGET_$(2)ARCH)/$(1)
	$(hide) $$(call ART_TEST_SKIP,$$@) && \
