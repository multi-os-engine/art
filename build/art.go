...
func (a *artCCBinary) WholeStaticLibraryDeps(ctx common.AndroidBaseContext) []string {
	ret := a.CCBinary.WholeStaticLibraryDeps(ctx)
	ret = append(ret, "libsigchain")
	return ret
}

func (a *artCCBinary) SharedLibraryDeps(ctx common.AndroidBaseContext) []string {
	ret := a.CCBinary.SharedLibraryDeps(ctx)
	if ctx.Device() {
		ret = append(ret, "libdl")
	}

	if ctx.Debug() {
		ret = append(ret, "libartd")
	} else {
		ret = append(ret, "libart")
	}
