 ...
 public:
  ...
  void Write(const ElfCompilationUnit& compilation_unit) {
    ...
    for (auto mi : compilation_unit.methods) {
      ...
      // It is expensive, so do it just once and reuse the result.
      ...
      if (mi->IsFromOptimizingCompiler()) {
        ...
        for (size_t s = 0; s < code_info.GetNumberOfStackMaps(); ++s) {
          ...
          size_t reg_num = code_info.GetNumberOfDexRegistersOf(stack_map, false);
          dex_reg_maps.push_back(code_info.GetDexRegisterMapOf(stack_map, reg_num, false));
        }
      }
      ...
      if (dex->DecodeDebugLocalInfo(dex_code,
        ...
        for (const DexFile::LocalInfo& var : local_infos) {
          if (var.reg_ < dex_code->registers_size_ - dex_code->ins_size_) {
            ...
            WriteRegLocation(mi, dex_reg_maps, var.reg_, is64bitValue, compilation_unit.low_pc,
  ...
  void WriteRegLocation(const MethodDebugInfo* method_info,
                        std::vector<DexRegisterMap>& dex_register_maps,
                        uint16_t vreg,
                        bool is64bitValue,
