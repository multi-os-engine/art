...
static ArrayRef<const uint8_t> WriteDebugElfFileForMethodInternal(
    const MethodDebugInfo& method_info, const InstructionSetFeatures* features) {
...
ArrayRef<const uint8_t> WriteDebugElfFileForMethod(const MethodDebugInfo& method_info,
  ...
  const InstructionSet isa = method_info.compiled_method->GetInstructionSet();
  if (Is64BitInstructionSet(isa)) {
    return WriteDebugElfFileForMethodInternal<ElfTypes64>(method_info, features);
  } else {
    return WriteDebugElfFileForMethodInternal<ElfTypes32>(method_info, features);
  }
}
