...
void QuickBackend::Init(CompilerDriver& driver) const {
  ArtInitQuickCompilerContext(driver);
}

void QuickBackend::UnInit(CompilerDriver& driver) const {
  ArtUnInitQuickCompilerContext(driver);
}

CompiledMethod* QuickBackend::Compile(CompilerDriver& compiler,
...
CompiledMethod* QuickBackend::JniCompile(CompilerDriver& driver,
                                         uint32_t access_flags,
                                         uint32_t method_idx,
                                         const DexFile& dex_file) const {
...
uintptr_t QuickBackend::GetEntryPointOf(mirror::ArtMethod* method) const {
  return reinterpret_cast<uintptr_t>(method->GetEntryPointFromQuickCompiledCode());
}

bool QuickBackend::WriteElf(art::File* file,
...
Backend* QuickBackend::GetCodeGenerator(CompilationUnit* cu, void* compilation_unit) const {
...
std::vector<uint8_t>* QuickBackend::GetCallFrameInformationInitialization(
...
CompiledMethod* OptimizingBackend::Compile(CompilerDriver& compiler,
                                           const DexFile::CodeItem* code_item,
                                           uint32_t access_flags,
                                           InvokeType invoke_type,
                                           uint16_t class_def_idx,
  ...
  CompiledMethod* method = TryCompile(
      compiler, code_item, access_flags, invoke_type, class_def_idx, method_idx,
      class_loader, dex_file);
  if (method != nullptr) return method;

  return QuickBackend::Compile(
      compiler, code_item, access_flags, invoke_type, class_def_idx, method_idx,
      class_loader, dex_file);
}
