...
void X86RelativePatcher::PatchDexCacheReference(std::vector<uint8_t>* code,
  ...
  // Check that the patched data contains 256, i.e. kDummy32BitOffset.
  DCHECK_LE(literal_offset, code->size());
  DCHECK_EQ((*code)[literal_offset + 0u], 0x00u);
  DCHECK_EQ((*code)[literal_offset + 1u], 0x01u);
  DCHECK_EQ((*code)[literal_offset + 2u], 0x00u);
  DCHECK_EQ((*code)[literal_offset + 3u], 0x00u);
