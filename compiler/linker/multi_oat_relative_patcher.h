...
class MultiOatRelativePatcher FINAL : private linker::RelativePatcherTargetProvider {
 public:
  ...
  // aligned as code, so that it does not skew alignment calculations.
  ...
  uint32_t GetOffset(MethodReference method_ref) {
    auto it = map_.find(method_ref);
    return (it != map_.end()) ? it->second - adjustment_ : 0u;
  }

  // Set the offset.
  void SetOffset(MethodReference method_ref, uint32_t offset) {
    map_.Put(method_ref, offset + adjustment_);
  }
 ...
 private:
  ...
  SafeMap<MethodReference, uint32_t, MethodReferenceComparator> map_;
