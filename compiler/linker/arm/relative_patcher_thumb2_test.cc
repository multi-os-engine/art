 ...
 protected:
  ...
  bool Create2MethodsWithGap(const ArrayRef<const uint8_t>& method1_code,
    ...
    uint32_t method1_offset =
        kTrampolineSize + CodeAlignmentSize(kTrampolineSize, kThumb2) +
    ...
    const uint32_t method2_offset =
        method1_offset + method1_code.size() +
        CodeAlignmentSize(method1_offset + method1_code.size(), kThumb2) +
        sizeof(OatQuickMethodHeader);
    ...
    if (result3.second == method3_offset + 1 /* thumb mode */) {
      ...
      uint32_t thunk_end =
          ...
          ThunkSize();
      uint32_t header_offset = thunk_end + CodeAlignmentSize(thunk_end, kThumb2);
...
TEST_F(Thumb2RelativePatcherTest, CallOtherJustTooFarAfter) {
  ...
  DCHECK_EQ(thunk_offset + ThunkSize() + CodeAlignmentSize(thunk_offset + ThunkSize(), kThumb2),
