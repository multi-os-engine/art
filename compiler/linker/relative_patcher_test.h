...
class RelativePatcherTest : public testing::Test {
 protected:
  ...
  uint32_t CodeAlignmentSize(uint32_t header_offset_to_align, InstructionSet instruction_set) {
    ...
    uint32_t aligned_code_offset =
        CompiledMethod::AlignCode(unaligned_code_offset, instruction_set);
  ...
  void Link() {
    ...
    for (auto& compiled_method : compiled_methods_) {
      ...
      uint32_t alignment_size = CodeAlignmentSize(offset, compiled_method->GetInstructionSet());
