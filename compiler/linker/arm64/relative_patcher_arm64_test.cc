 ...
 protected:
  ...
  uint32_t Create2MethodsWithGap(const ArrayRef<const uint8_t>& method1_code,
    ...
    uint32_t method1_offset =
        kTrampolineSize + CodeAlignmentSize(kTrampolineSize, kArm64) + sizeof(OatQuickMethodHeader);
    ...
    for (uint32_t i = 0; i <= num_small_chunks; ++i) {  // num_small_chunks+1 iterations.
      uint32_t chunk_code_size =
          chunk_size - CodeAlignmentSize(chunk_start, kArm64) - sizeof(OatQuickMethodHeader);
      ...
      DCHECK_EQ(CodeAlignmentSize(gap_end, kArm64), CodeAlignmentSize(chunk_start, kArm64));
    }
    ...
    if (last_result.second != last_method_offset) {
      ...
      uint32_t header_offset = thunk_end + CodeAlignmentSize(thunk_end, kArm64);
...
TEST_F(Arm64RelativePatcherTestDefault, CallOtherJustTooFarAfter) {
  ...
  DCHECK_EQ(thunk_offset + ThunkSize() + CodeAlignmentSize(thunk_offset + ThunkSize(), kArm64),
