...
MultiOatRelativePatcher::MultiOatRelativePatcher(InstructionSet instruction_set,
    ...
    : map_(),
      relative_patcher_(linker::RelativePatcher::Create(instruction_set, features, &map_)),
      adjustment_(0u),
...
void MultiOatRelativePatcher::StartOatFile(uint32_t adjustment) {
  DCHECK_EQ(CompiledMethod::AlignCode(adjustment, instruction_set_), adjustment);
