...
CompiledCode::CompiledCode(CompilerDriver* compiler_driver, InstructionSet instruction_set,
                           const ArrayRef<const uint8_t>& quick_code, bool own_code_array)
    : compiler_driver_(compiler_driver), instruction_set_(instruction_set),
      own_code_array_(own_code_array), quick_code_(nullptr) {
  SetCode(&quick_code);
}
...
CompiledMethod::CompiledMethod(CompilerDriver* driver,
  ...
  if (owns_arrays_) {
    ...
    vmap_table_ = new SwapVector<uint8_t>(vmap_table.begin(), vmap_table.end(),
                                          driver->GetSwapSpaceAllocator());
    gc_map_ = native_gc_map.data() == nullptr ? nullptr :
        new SwapVector<uint8_t>(native_gc_map.begin(), native_gc_map.end(),
                                driver->GetSwapSpaceAllocator());
    cfi_info_ = cfi_info.data() == nullptr ? nullptr :
