...
void CompiledCode::ShowDebug() {
  if (final_relocations_ != nullptr) {
    for (auto& set : *final_relocations_) {
      set->ShowDebug();
    }
  } else {
    LOG(INFO) << "no relocations";
  }
}

void FinalEntrypointRelocationSet::ShowDebug() const {
  LOG(INFO) << "FinalEntrypointRelocationSet relocations:";

  for (auto& reloc : relocations_) {
    LOG(INFO) << "  type: " << reloc.type_ << ", code_offset: " << reloc.code_offset_;
  }
}

void FinalEntrypointRelocationSet::Add(uint32_t offset, uint32_t entrypoint_offset) {
  relocations_.push_back(Relocation(REL_CALL, offset, entrypoint_offset));
}
}  // namespace art
