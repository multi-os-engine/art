...
class DebugFrameOpCodeWriterForAssembler FINAL
 ...
 public:
  ...
  // The assembler must explicitly process the all delayed advances.
  ...
  std::pair<std::vector<uint8_t>, std::vector<DelayedAdvancePC>> ReleaseData() {
    DCHECK(delay_emitting_advance_pc_);
    delay_emitting_advance_pc_ = false;
  ...
  // Reserve space for CFI stream.
  void ReserveCFIStream(size_t capacity) {
    opcodes_.reserve(capacity);
  }

  // Append raw data to the CFI stream.
  void AppendRawData(const std::vector<uint8_t>& data, size_t first, size_t last) {
    DCHECK_LE(0u, first);
    DCHECK_LE(first, last);
    DCHECK_LE(last, data.size());
    opcodes_.insert(opcodes_.end(), data.begin() + first, data.begin() + last);
  }
