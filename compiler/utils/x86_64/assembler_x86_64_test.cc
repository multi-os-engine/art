...
TEST(AssemblerX86_64, SignExtension) {
  ...
  for (int32_t i = 0; i < 128; i++) {
    EXPECT_TRUE(x86_64::Is8BSignExtensionOf<int32_t>(i)) << i;
  }
  for (int32_t i = 128; i < 255; i++) {
    EXPECT_FALSE(x86_64::Is8BSignExtensionOf<int32_t>(i)) << i;
  }
  ...
  for (size_t i = 0; i < kRandomIterations; i++) {
    ...
    EXPECT_FALSE(x86_64::Is8BSignExtensionOf<int32_t>(value)) << value;
  }
  ...
  for (int64_t i = 0; i < 128; i++) {
    EXPECT_TRUE(x86_64::Is8BSignExtensionOf<int64_t>(i)) << i;
  }
  for (int32_t i = 128; i < 255; i++) {
    EXPECT_FALSE(x86_64::Is8BSignExtensionOf<int64_t>(i)) << i;
  }
  // Do some higher ones randomly.
  std::uniform_int_distribution<int64_t> uniform_dist3(256, INT64_MAX);
  for (size_t i = 0; i < 100; i++) {
    int64_t value = uniform_dist3(e1);
    EXPECT_FALSE(x86_64::Is8BSignExtensionOf<int64_t>(value)) << value;
  }
...
typedef AssemblerTest<x86_64::X86_64Assembler, x86_64::CpuRegister,
                      x86_64::XmmRegister, x86_64::Immediate> InstantiatedAssemblerTest;

class AssemblerX86_64Test : public InstantiatedAssemblerTest {
 protected:
  ...
  bool DeleteIntermediatesOnSizeMismatch() OVERRIDE {
    return true;
  }
...
std::string shll_fn(InstantiatedAssemblerTest* assembler_test, x86_64::X86_64Assembler* assembler) {
...
std::string shrl_fn(InstantiatedAssemblerTest* assembler_test, x86_64::X86_64Assembler* assembler) {
  std::ostringstream str;
...
std::string sarl_fn(InstantiatedAssemblerTest* assembler_test, x86_64::X86_64Assembler* assembler) {
  std::ostringstream str;
...
std::string x87_fn(InstantiatedAssemblerTest* assembler_test ATTRIBUTE_UNUSED,
...
std::string ret_and_leave_fn(InstantiatedAssemblerTest* assembler_test ATTRIBUTE_UNUSED,
                             x86_64::X86_64Assembler* assembler) {
  std::ostringstream str;
...
std::string setcc_test_fn(InstantiatedAssemblerTest* assembler_test,
                          x86_64::X86_64Assembler* assembler) {
  // From Condition
  /*
  kOverflow     =  0,
...
std::string buildframe_test_fn(InstantiatedAssemblerTest* assembler_test ATTRIBUTE_UNUSED,
                               x86_64::X86_64Assembler* assembler) {
...
std::string removeframe_test_fn(InstantiatedAssemblerTest* assembler_test ATTRIBUTE_UNUSED,
                                x86_64::X86_64Assembler* assembler) {
  // TODO: more interesting spill registers / entry spills.
...
std::string increaseframe_test_fn(InstantiatedAssemblerTest* assembler_test ATTRIBUTE_UNUSED,
                                  x86_64::X86_64Assembler* assembler) {
...
std::string decreaseframe_test_fn(InstantiatedAssemblerTest* assembler_test ATTRIBUTE_UNUSED,
                                  x86_64::X86_64Assembler* assembler) {
