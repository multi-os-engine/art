...
void X86_64Assembler::pushl(Register reg) {
  AssemblerBuffer::EnsureCapacity ensured(&buffer_);
  EmitUint8(0x50 + reg);
}


void X86_64Assembler::pushl(const Address& address) {
  AssemblerBuffer::EnsureCapacity ensured(&buffer_);
  EmitUint8(0xFF);
...
void X86_64Assembler::pushl(const Immediate& imm) {
  AssemblerBuffer::EnsureCapacity ensured(&buffer_);
  if (imm.is_int8()) {
    EmitUint8(0x6A);
...
void X86_64Assembler::popl(Register reg) {
  AssemblerBuffer::EnsureCapacity ensured(&buffer_);
  EmitUint8(0x58 + reg);
}


void X86_64Assembler::popl(const Address& address) {
  AssemblerBuffer::EnsureCapacity ensured(&buffer_);
  EmitUint8(0x8F);
...
void X86_64Assembler::leal(Register dst, const Address& src) {
  AssemblerBuffer::EnsureCapacity ensured(&buffer_);
  EmitUint8(0x8D);
  EmitOperand(dst, src);
}
...
X86_64Assembler* X86_64Assembler::fs() {
...
void X86_64Assembler::LoadDoubleConstant(XmmRegister dst, double value) {
  ...
  addl(RSP, Immediate(2 * kWordSize));
}
...
void X86_64Assembler::BuildFrame(size_t frame_size, ManagedRegister method_reg,
  ...
  for (int i = spill_regs.size() - 1; i >= 0; --i) {
    pushl(spill_regs.at(i).AsX86_64().AsCpuRegister());
  }
  // return address then method on stack
  addl(RSP, Immediate(-frame_size + (spill_regs.size() * kPointerSize) +
                      kPointerSize /*method*/ + kPointerSize /*return address*/));
  pushl(method_reg.AsX86_64().AsCpuRegister());
  for (size_t i = 0; i < entry_spills.size(); ++i) {
    movl(Address(RSP, frame_size + kPointerSize + (i * kPointerSize)),
         entry_spills.at(i).AsX86_64().AsCpuRegister());
  }
}
...
void X86_64Assembler::RemoveFrame(size_t frame_size,
                            const std::vector<ManagedRegister>& spill_regs) {
  CHECK_ALIGNED(frame_size, kStackAlignment);
  addl(RSP, Immediate(frame_size - (spill_regs.size() * kPointerSize) - kPointerSize));
  for (size_t i = 0; i < spill_regs.size(); ++i) {
    popl(spill_regs.at(i).AsX86_64().AsCpuRegister());
  }
  ret();
}
...
void X86_64Assembler::Store(FrameOffset offs, ManagedRegister msrc, size_t size) {
  ...
  if (src.IsNoRegister()) {
    ...
    CHECK_EQ(8u, size);
    movl(Address(RSP, offs), src.AsRegisterPairLow());
    movl(Address(RSP, FrameOffset(offs.Int32Value()+4)),
...
void X86_64Assembler::StoreRef(FrameOffset dest, ManagedRegister msrc) {
  ...
  movl(Address(RSP, dest), src.AsCpuRegister());
}
...
void X86_64Assembler::StoreImmediateToFrame(FrameOffset dest, uint32_t imm,
                                         ManagedRegister) {
  movl(Address(RSP, dest), Immediate(imm));
}
...
void X86_64Assembler::StoreStackOffsetToThread(ThreadOffset thr_offs,
  ...
  leal(scratch.AsCpuRegister(), Address(RSP, fr_offs));
...
void X86_64Assembler::StoreStackPointerToThread(ThreadOffset thr_offs) {
  fs()->movl(Address::Absolute(thr_offs), RSP);
}
...
void X86_64Assembler::Load(ManagedRegister mdest, FrameOffset src, size_t size) {
  ...
  if (dest.IsNoRegister()) {
    ...
    CHECK_EQ(8u, size);
    movl(dest.AsRegisterPairLow(), Address(RSP, src));
    movl(dest.AsRegisterPairHigh(), Address(RSP, FrameOffset(src.Int32Value()+4)));
...
void X86_64Assembler::Copy(ManagedRegister dest_base, Offset dest_offset, FrameOffset src,
  ...
  CHECK_EQ(size, 4u);
  pushl(Address(RSP, src));
  popl(Address(dest_base.AsX86_64().AsCpuRegister(), dest_offset));
}
...
void X86_64Assembler::Copy(FrameOffset dest, FrameOffset src_base, Offset src_offset,
  ...
  CHECK_EQ(size, 4u);
  movl(scratch, Address(RSP, src_base));
  movl(scratch, Address(scratch, src_offset));
  movl(Address(RSP, dest), scratch);
}
...
void X86_64Assembler::Copy(ManagedRegister dest, Offset dest_offset,
  ...
  CHECK_EQ(size, 4u);
  CHECK(scratch.IsNoRegister());
  pushl(Address(src.AsX86_64().AsCpuRegister(), src_offset));
  popl(Address(dest.AsX86_64().AsCpuRegister(), dest_offset));
}
...
void X86_64Assembler::CreateSirtEntry(ManagedRegister mout_reg,
  ...
  if (null_allowed) {
    ...
    j(kZero, &null_arg);
    leal(out_reg.AsCpuRegister(), Address(RSP, sirt_offset));
    Bind(&null_arg);
  } else {
    leal(out_reg.AsCpuRegister(), Address(RSP, sirt_offset));
  }
...
void X86_64Assembler::CreateSirtEntry(FrameOffset out_off,
  ...
  if (null_allowed) {
    ...
    j(kZero, &null_arg);
    leal(scratch.AsCpuRegister(), Address(RSP, sirt_offset));
    Bind(&null_arg);
  } else {
    leal(scratch.AsCpuRegister(), Address(RSP, sirt_offset));
  }
...
void X86_64Assembler::GetCurrentThread(ManagedRegister tr) {
  fs()->movl(tr.AsX86_64().AsCpuRegister(),
...
void X86_64Assembler::GetCurrentThread(FrameOffset offset,
  ...
  movl(Address(RSP, offset), scratch.AsCpuRegister());
}
...
#define __ sp_asm->
  ...
  __ fs()->movl(RAX, Address::Absolute(Thread::ExceptionOffset()));
