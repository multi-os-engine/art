...
void X86_64Assembler::CreateHandleScopeEntry(ManagedRegister mout_reg,
  ...
  if (in_reg.IsNoRegister()) {  // TODO(64): && null_allowed
    ...
    in_reg = out_reg;
    // TODO: movzwl
    movl(in_reg.AsCpuRegister(), Address(CpuRegister(RSP), handlescope_offset));
  }
  ...
  if (null_allowed) {
    ...
    j(kZero, &null_arg);
    leaq(out_reg.AsCpuRegister(), Address(CpuRegister(RSP), handlescope_offset));
    Bind(&null_arg);
  } else {
    leaq(out_reg.AsCpuRegister(), Address(CpuRegister(RSP), handlescope_offset));
  }
...
void X86_64Assembler::CreateHandleScopeEntry(FrameOffset out_off,
  ...
  if (null_allowed) {
    Label null_arg;
    movl(scratch.AsCpuRegister(), Address(CpuRegister(RSP), handlescope_offset));
    testl(scratch.AsCpuRegister(), scratch.AsCpuRegister());
    j(kZero, &null_arg);
    leaq(scratch.AsCpuRegister(), Address(CpuRegister(RSP), handlescope_offset));
    Bind(&null_arg);
  } else {
    leaq(scratch.AsCpuRegister(), Address(CpuRegister(RSP), handlescope_offset));
  }
