...
// Sign-extend a kFromBits-number (encoded in type T) to full type T.
template <typename T, size_t kFromBits>
inline T SignExtendValue(const T x) {
  struct {
    T x:kFromBits;
  } s;
  return s.x = x;
...
// Checks whether the given value can be used through sign-extension.
template <typename T, size_t kFromBits>
inline bool IsSignExtensionOf(const T value) {
  static_assert(kFromBits < sizeof(T) * 8, "Shifting through sign is undefined behavior.");
  return value == SignExtendValue<T, kFromBits>(value & ((static_cast<T>(1) << kFromBits) - 1));
}
...
void X86_64Assembler::imull(CpuRegister reg, const Immediate& imm) {
  ...
  if (IsSignExtensionOf<int32_t, 8>(v32)) {
...
void X86_64Assembler::imulq(CpuRegister reg, const Immediate& imm) {
  ...
  if (IsSignExtensionOf<int64_t, 8>(v64)) {
    // Sign-extension works.
    EmitUint8(0x6B);
    EmitOperand(reg.LowBits(), Operand(reg));
