...
void X86Assembler::CreateHandleScopeEntry(ManagedRegister mout_reg,
  ...
  if (null_allowed) {
    ...
    j(kZero, &null_arg);
    leal(out_reg.AsCpuRegister(), Address(ESP, handlescope_offset));
    Bind(&null_arg);
  } else {
    leal(out_reg.AsCpuRegister(), Address(ESP, handlescope_offset));
  }
...
void X86Assembler::CreateHandleScopeEntry(FrameOffset out_off,
  ...
  if (null_allowed) {
    Label null_arg;
    movl(scratch.AsCpuRegister(), Address(ESP, handlescope_offset));
    testl(scratch.AsCpuRegister(), scratch.AsCpuRegister());
    j(kZero, &null_arg);
    leal(scratch.AsCpuRegister(), Address(ESP, handlescope_offset));
    Bind(&null_arg);
  } else {
    leal(scratch.AsCpuRegister(), Address(ESP, handlescope_offset));
  }
