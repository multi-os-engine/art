...
void X86Assembler::StoreImmediateToThread(ThreadOffset<4> dest, uint32_t imm,
                                          ManagedRegister) {
  fs()->movl(Address::Absolute(dest), Immediate(imm));
}

void X86Assembler::StoreStackOffsetToThread(ThreadOffset<4> thr_offs,
                                            FrameOffset fr_offs,
...
void X86Assembler::StoreStackPointerToThread(ThreadOffset<4> thr_offs) {
  fs()->movl(Address::Absolute(thr_offs), ESP);
}

void X86Assembler::StoreLabelToThread(ThreadOffset<4> thr_offs, Label* lbl) {
...
void X86Assembler::Load(ManagedRegister mdest, ThreadOffset<4> src, size_t size) {
  X86ManagedRegister dest = mdest.AsX86();
  if (dest.IsNoRegister()) {
    CHECK_EQ(0u, size);
  } else if (dest.IsCpuRegister()) {
...
void X86Assembler::CopyRawPtrToThread(ThreadOffset<4> thr_offs,
                                      FrameOffset fr_offs,
                                      ManagedRegister mscratch) {
  X86ManagedRegister scratch = mscratch.AsX86();
  CHECK(scratch.IsCpuRegister());
...
void X86Assembler::Call(ThreadOffset<4> offset, ManagedRegister /*mscratch*/) {
