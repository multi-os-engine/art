...
class AssemblerTest : public testing::Test {
 ...
 protected:
  ...
  virtual std::vector<int64_t> CreateImmediateValuesBits(int imm_bits, bool as_uint = false) {
    CHECK_GT(imm_bits, 0);
    ...
    // Enter min/max values for unsigned/signed immediate values which are
    // imm_bits in size.
    if (as_uint) {
      res.push_back(0);
      res.push_back(0xFFFFFFFFFFFFFFFF >> (64 - imm_bits));
      if (imm_bits > 2) {
        // Add more random integers bounded by the min/max values for an imm_bit unsigned integer.
        std::uniform_int_distribution<uint64_t> u(static_cast<uint64_t>(res[0]),
                                                  static_cast<uint64_t>(res[1]));
        for (size_t tmp = imm_bits; tmp; tmp >>= 1)
          res.push_back(static_cast<int64_t>(u(e)));
      }
    } else {
      res.push_back(static_cast<int64_t>(0xFFFFFFFFFFFFFFFF << (imm_bits-1)));
      res.push_back(0);  // Add zero because it's neither positive nor negative
      res.push_back(static_cast<int64_t>(((1ull << (imm_bits-1)) - 1)));
      if (imm_bits > 2) {
        // Add more random integers bounded by the min/max values for an imm_bit signed integer.
        std::uniform_int_distribution<int64_t> u(res[0], res[2]);
        for (size_t tmp = imm_bits; tmp; tmp >>= 1)
          res.push_back(u(e));
      }
    }
