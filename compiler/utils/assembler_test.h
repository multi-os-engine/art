...
class AssemblerTest : public testing::Test {
 ...
 protected:
  ...
  virtual bool DeleteIntermediatesOnSizeMismatch() {
    return true;  // By default, keep the tmp directory clean. Override this in the specific
                  // assembler test that you're working on.
  }
 ...
 private:
  ...
  bool DisassembleBinaries(std::vector<uint8_t>& data, std::vector<uint8_t>& as,
    ...
    if (result && DeleteIntermediatesOnSizeMismatch()) {
      std::remove(data_name.c_str());
      std::remove(as_name.c_str());
      std::remove((data_name + ".dis").c_str());
