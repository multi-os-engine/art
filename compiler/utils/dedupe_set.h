 ...
 private:
  ...
  class HashedKey {
   public:
    ...
    explicit HashedKey(HashedKey&& src)
        : hash_(src.hash_),
          key_(src.key_) {
      src.key_ = nullptr;
    }

    HashedKey& operator=(HashedKey&& src) {
      DCHECK(key_ == nullptr);
      hash_ = src.hash_;
      key_ = src.key_;
      src.key_ = nullptr;
   ...
   private:
    ...
    DISALLOW_COPY_AND_ASSIGN(HashedKey);
  };

  class ShardEmptyFn {
   public:
    explicit ShardEmptyFn(Alloc* alloc) : alloc_(alloc) { }
    ...
    void MakeEmpty(HashedKey<StoreKey>& key) {
      const StoreKey* store_key = key.ReleaseKey();
      if (store_key != nullptr) {
        alloc_->Destroy(store_key);
      }
    }
  ...
  struct ShardPred {
    ...
    bool operator()(const HashedKey<LeftT>& lhs, const HashedKey<RightT>& rhs) const {
      ...
      return lhs.Key()->size() == rhs.Key()->size() &&
          std::equal(lhs.Key()->begin(), lhs.Key()->end(), rhs.Key()->end());
    }
  };
   ...
   public:
    Shard(Alloc* alloc, const std::string& lock_name)
        : alloc_(alloc),
          ...
          keys_(ShardEmptyFn(alloc), ShardHashFn(), ShardPred()) {
    }
