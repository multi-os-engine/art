...
void MipsAssembler::EmitI26(int opcode, uint32_t imm26) {
  uint32_t encoding = static_cast<uint32_t>(opcode) << kOpcodeShift | (imm26 & 0x3FFFFFF);
  Emit(encoding);
}
...
void MipsAssembler::Addiupc(Register rs, uint32_t imm19) {
  EmitI21(0x3B, rs, imm19 & 0x7FFFF);
}

void MipsAssembler::Bc(uint32_t imm26) {
  EmitI26(0x32, imm26 & 0x3FFFFFF);
}
...
void MipsAssembler::EmitBcond(BranchCondition cond, Register rs, Register rt, uint16_t imm16) {
  switch (cond) {
    ...
    case kUncond:
      ...
      DCHECK(false) << "Unexpected branch condition";
      UNREACHABLE();
  }
}
...
void MipsAssembler::LoadConst64(Register reg_hi, Register reg_lo, int64_t value) {
  LoadConst32(reg_lo, static_cast<int32_t>(value));
  LoadConst32(reg_hi, static_cast<int32_t>(value >> 32));
}
...
void MipsAssembler::StoreConst32ToOffset(int32_t value,
                                         ...
                                         Register temp) {
  if (!IsInt<16>(offset)) {
    DCHECK_NE(temp, AT);  //  Must not use AT as temp, as not to overwrite the loaded value.
...
void MipsAssembler::StoreConst64ToOffset(int64_t value,
  ...
  LoadConst32(temp, static_cast<int32_t>(value));
  Sw(temp, base, offset);
  LoadConst32(temp, static_cast<int32_t>(value >> 32));
  Sw(temp, base, offset + 4);
}
...
void MipsAssembler::Addiu32(Register rt, Register rs, int32_t value, Register rtmp) {
  if (IsInt<16>(value)) {
    Addiu(rt, rs, value);
  } else {
...
void MipsAssembler::Branch::InitShortOrLong(MipsAssembler::Branch::OfsBits ofs_size,
                                            MipsAssembler::Branch::Type short_type,
                                            MipsAssembler::Branch::Type long_type) {
  type_ = (ofs_size <= branch_info_[short_type].OfsSize) ? short_type : long_type;
}

void MipsAssembler::Branch::InitializeType(bool is_call, bool is_r6) {
  OfsBits ofs_size = GetOffsetSizeNeeded(location_, target_);
  if (is_r6) {
    // R6
    if (is_call) {
      InitShortOrLong(ofs_size, kR6Call, kR6LongCall);
    } else if (condition_ == kUncond) {
      InitShortOrLong(ofs_size, kR6UncondBranch, kR6LongUncondBranch);
    } else {
      if (condition_ == kCondEQZ || condition_ == kCondNEZ) {
        // Special case for beqzc/bnezc with longer offset than in other b<cond>c instructions.
        type_ = (ofs_size <= kOfs23) ? kR6CondBranch : kR6LongCondBranch;
      } else {
        InitShortOrLong(ofs_size, kR6CondBranch, kR6LongCondBranch);
      }
    }
  } else {
    // R2
    if (is_call) {
      InitShortOrLong(ofs_size, kCall, kLongCall);
    } else if (condition_ == kUncond) {
      InitShortOrLong(ofs_size, kUncondBranch, kLongUncondBranch);
    } else {
      InitShortOrLong(ofs_size, kCondBranch, kLongCondBranch);
    }
  }
  old_type_ = type_;
}
...
MipsAssembler::Branch::Branch(bool is_r6,
  ...
  switch (condition) {
    ...
    case kCondNE:
      ...
      DCHECK(lhs_reg != ZERO && rhs_reg != ZERO);
      break;
    case kCondLTZ:
    case kCondGEZ:
    case kCondLEZ:
...
MipsAssembler::Branch::Branch(bool is_r6, uint32_t location, uint32_t target, Register indirect_reg)
    : old_location_(location),
      ...
      rhs_reg_(0),
      condition_(kUncond) {
  DCHECK(indirect_reg != ZERO && indirect_reg != AT);
...
MipsAssembler::Branch::OfsBits MipsAssembler::Branch::GetOffsetSize() const {
  OfsBits ofs_size =
      (type_ == kR6CondBranch && (condition_ == kCondEQZ || condition_ == kCondNEZ))
          ? kOfs23
          : branch_info_[type_].OfsSize;
  return ofs_size;
}

MipsAssembler::Branch::OfsBits MipsAssembler::Branch::GetOffsetSizeNeeded(uint32_t location,
                                                                          uint32_t target) {
  ...
  if (IsInt<kOfs16>(distance))
    return kOfs16;
  else if (IsInt<kOfs18>(distance))
    return kOfs18;
  else if (IsInt<kOfs21>(distance))
    return kOfs21;
  else if (IsInt<kOfs23>(distance))
    return kOfs23;
  else if (IsInt<kOfs28>(distance))
    return kOfs28;
  return kOfs32;
}
...
uint32_t MipsAssembler::Branch::PromoteIfNeeded(uint32_t max_short_distance) {
  ...
  if (max_short_distance != UINT32_MAX) {
...
uint32_t MipsAssembler::Branch::GetOffsetLocation() const {
  return location_ + branch_info_[type_].InstrOfs * sizeof(uint32_t);
}
...
uint32_t MipsAssembler::Branch::GetOffset() const {
  ...
  uint32_t offset = target_ - GetOffsetLocation() - branch_info_[type_].PcOrg * sizeof(uint32_t);
  // Prepare the offset for encoding into the instruction(s).
  offset = (offset & ofs_mask) >> branch_info_[type_].OfsShift;
  return offset;
}

MipsAssembler::Branch* MipsAssembler::GetBranch(uint32_t branch_id) const {
  if (branch_id >= branches_.size()) {
    return nullptr;
  }
...
void MipsAssembler::Bind(MipsLabel* label) {
  ...
  // Walk the list of the branches referring to and preceding this label.
...
uint32_t MipsAssembler::GetAdjustedPosition(uint32_t old_position) {
  ...
  while (last_branch_id_ != branches_.size()) {
    Branch* branch = GetBranch(last_branch_id_);
...
void MipsAssembler::Buncond(MipsLabel* label) {
  ...
  Branch* branch = new Branch(IsR6(), buffer_.Size(), target);
  branches_.push_back(branch);
  uint32_t length = branch->GetLength();
  if (!label->IsBound()) {
...
void MipsAssembler::Bcond(MipsLabel* label, BranchCondition condition, Register lhs, Register rhs) {
  ...
  uint32_t target = label->IsBound() ? GetLabelLocation(label) : Branch::kUnresolved;
  Branch* branch = new Branch(IsR6(), buffer_.Size(), target, condition, lhs, rhs);
  branches_.push_back(branch);
  uint32_t length = branch->GetLength();
  if (!label->IsBound()) {
    // Branch forward (to a following label), distance is unknown.
...
void MipsAssembler::Call(MipsLabel* label, Register indirect_reg) {
  uint32_t target = label->IsBound() ? GetLabelLocation(label) : Branch::kUnresolved;
  Branch* branch = new Branch(IsR6(), buffer_.Size(), target, indirect_reg);
  branches_.push_back(branch);
  uint32_t length = branch->GetLength();
  if (!label->IsBound()) {
    // Branch forward (to a following label), distance is unknown.
...
// Keep branch_info_[] synchronized with EmitBranch().
const MipsAssembler::Branch::BranchInfo MipsAssembler::Branch::branch_info_[] = {
  // R2 short branches.
  //   kUncondBranch
  {  2, 0, 1, MipsAssembler::Branch::kOfs18, 2 },
  //   kCondBranch
  {  2, 0, 1, MipsAssembler::Branch::kOfs18, 2 },
  //   kCall
  {  5, 2, 0, MipsAssembler::Branch::kOfs16, 0 },
  // R2 long branches.
  //   kLongUncondBranch
  {  9, 3, 1, MipsAssembler::Branch::kOfs32, 0 },
  //   kLongCondBranch
  { 10, 4, 1, MipsAssembler::Branch::kOfs32, 0 },
  //   kLongCall
  {  6, 1, 1, MipsAssembler::Branch::kOfs32, 0 },
  // R6 short branches.
  //   kR6UncondBranch
  {  1, 0, 1, MipsAssembler::Branch::kOfs28, 2 },
  //   kR6CondBranch
  {  2, 0, 1, MipsAssembler::Branch::kOfs18, 2 },  // Exception: kOfs23 for beqzc/bnezc.
  //   kR6Call
  {  2, 0, 0, MipsAssembler::Branch::kOfs21, 2 },
  // R6 long branches.
  //   kR6LongUncondBranch
  {  2, 0, 0, MipsAssembler::Branch::kOfs32, 0 },
  //   kR6LongCondBranch
  {  3, 1, 0, MipsAssembler::Branch::kOfs32, 0 },
  //   kR6LongCall
  {  3, 0, 0, MipsAssembler::Branch::kOfs32, 0 },
};

// Keep branch_info_[] synchronized with EmitBranch().
void MipsAssembler::EmitBranch(MipsAssembler::Branch* branch) {
  DCHECK_EQ(overwriting_, true);
  ...
  switch (branch->GetType()) {
    ...
    case Branch::kUncondBranch:
      DCHECK(overwrite_location_ == branch->GetOffsetLocation());
      B(offset);
    ...
    case Branch::kLongCondBranch:
      // The large comment above applies here as well.
    ...
    case Branch::kR6LongCall:
      ...
      DCHECK(overwrite_location_ == branch->GetOffsetLocation());
      Auipc(lhs, offset >> 16);
      Addiu(lhs, lhs, offset);
      Jialc(lhs, 0);
      break;
  }
  DCHECK(overwrite_location_ == branch->GetEndLocation());
  DCHECK(branch->GetSize() < Branch::kMaxBranchSize);
}
...
void MipsAssembler::LoadFromOffset(LoadOperandType type, Register reg, Register base,
                                   int32_t offset) {
  if (!IsInt<16>(offset) ||
      (type == kLoadDoubleword && !IsInt<16>(offset + 4))) {
    LoadConst32(AT, offset);
    Addu(AT, AT, base);
    base = AT;
    offset = 0;
  ...
  switch (type) {
    ...
    case kLoadDoubleword:
      if (reg == base) {
        ...
        Lw(static_cast<Register>(reg + 1), base, offset + 4);
        Lw(reg, base, offset);
      } else {
        Lw(reg, base, offset);
        Lw(static_cast<Register>(reg + 1), base, offset + 4);
...
void MipsAssembler::LoadDFromOffset(FRegister reg, Register base, int32_t offset) {
  if (!IsInt<16>(offset) || ((offset & 0x7) && !IsInt<16>(offset + 4))) {
    LoadConst32(AT, offset);
    Addu(AT, AT, base);
    base = AT;
    offset = 0;
  ...
  if (offset & 0x7) {
    if (!Is32BitFPU()) {
      ...
      Lwc1(reg, base, offset);
      Lwc1(static_cast<FRegister>(reg + 1), base, offset + 4);
    }
  } else {
...
void MipsAssembler::StoreToOffset(StoreOperandType type, Register reg, Register base,
                                  int32_t offset) {
  if (!IsInt<16>(offset) ||
      (type == kStoreDoubleword && !IsInt<16>(offset + 4))) {
    LoadConst32(AT, offset);
    Addu(AT, AT, base);
    base = AT;
    offset = 0;
  ...
  switch (type) {
    ...
    case kStoreDoubleword:
      Sw(reg, base, offset);
      Sw(static_cast<Register>(reg + 1), base, offset + 4);
      break;
    default:
      LOG(FATAL) << "UNREACHABLE";
  }
...
void MipsAssembler::StoreDToOffset(FRegister reg, Register base, int32_t offset) {
  ...
  if (offset & 0x7) {
    if (!Is32BitFPU()) {
      ...
      Swc1(reg, base, offset);
      Swc1(static_cast<FRegister>(reg + 1), base, offset + 4);
    }
  } else {
...
void MipsAssembler::LoadRawPtrFromThread32(ManagedRegister mdest, ThreadOffset<4> offs) {
  MipsManagedRegister dest = mdest.AsMips();
  CHECK(dest.IsCoreRegister());
...
void MipsAssembler::Copy(FrameOffset /*dest*/, Offset /*dest_offset*/, FrameOffset /*src*/,
                         Offset /*src_offset*/, ManagedRegister /*mscratch*/, size_t /*size*/) {
  UNIMPLEMENTED(FATAL) << "no mips implementation";
}
...
void MipsAssembler::GetCurrentThread(FrameOffset offset, ManagedRegister /*mscratch*/) {
...
void MipsAssembler::ExceptionPoll(ManagedRegister mscratch, size_t stack_adjust) {
  ...
  exception_blocks_.push_back(slow);
  LoadFromOffset(kLoadWord, scratch.AsCoreRegister(),
  ...
  Bnez(scratch.AsCoreRegister(), slow->Entry());
}
...
void MipsAssembler::EmitExceptionPoll(MipsExceptionSlowPath* exception) {
  ...
  LoadFromOffset(kLoadWord, T9, S1, QUICK_ENTRYPOINT_OFFSET(4, pDeliverException).Int32Value());
  Jr(T9);
  Nop();
