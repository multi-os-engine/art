...
void MipsAssembler::MovfS(FRegister fd, FRegister fs, int N) {
  // CHECK(IsUint<3>(N)) << N;
  EmitFR(0x11, 0x10, static_cast<FRegister>(N << 2), fs, fd, 0x11);
}

void MipsAssembler::MovfD(FRegister fd, FRegister fs, int N) {
  // CHECK(IsUint<3>(N)) << N;
  EmitFR(0x11, 0x11, static_cast<FRegister>(N << 2), fs, fd, 0x11);
}

void MipsAssembler::MovtS(FRegister fd, FRegister fs, int N) {
  // CHECK(IsUint<3>(N)) << N;
  EmitFR(0x11, 0x10, static_cast<FRegister>((N << 2) | 1), fs, fd, 0x11);
}

void MipsAssembler::MovtD(FRegister fd, FRegister fs, int N) {
  // CHECK(IsUint<3>(N)) << N;
  EmitFR(0x11, 0x11, static_cast<FRegister>((N << 2) | 1), fs, fd, 0x11);
}
