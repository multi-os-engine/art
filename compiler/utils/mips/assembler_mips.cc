...
void MipsAssembler::StoreConst64ToOffset(int64_t value,
                                         ...
                                         int32_t offset,
                                         Register temp) {
  if (!IsInt<16>(offset) || !IsInt<16>(offset + kMipsWordSizeOffset)) {
    CHECK_NE(temp, AT);  //  Must not use AT as temp, as not to overwrite the loaded value.
    LoadConst32(AT, offset);
    Addu(AT, AT, base);
    base = AT;
...
void MipsAssembler::EmitBranch(MipsAssembler::Branch* branch) {
  ...
  switch (branch->GetType()) {
    ...
    case Branch::kLongCondBranch:
      ...
      // number of instuctions skipped (PUSH(2) + NAL + LUI + ORI + ADDU + LW + JR).
...
void MipsAssembler::LoadFromOffset(LoadOperandType type, Register reg, Register base,
                                   int32_t offset) {
  if (!IsInt<16>(offset) ||
      (type == kLoadDoubleword && !IsInt<16>(offset + kMipsWordSizeOffset))) {
    LoadConst32(AT, offset);
    Addu(AT, AT, base);
    base = AT;
    offset = 0;
...
void MipsAssembler::LoadDFromOffset(FRegister reg, Register base, int32_t offset) {
  if (!IsInt<16>(offset) || ((offset & 0x7) && !IsInt<16>(offset + kMipsWordSizeOffset))) {
    LoadConst32(AT, offset);
    Addu(AT, AT, base);
    base = AT;
    offset = 0;
...
void MipsAssembler::StoreToOffset(StoreOperandType type, Register reg, Register base,
                                  int32_t offset) {
  if (!IsInt<16>(offset) ||
      (type == kStoreDoubleword && !IsInt<16>(offset + kMipsWordSizeOffset))) {
    LoadConst32(AT, offset);
    Addu(AT, AT, base);
    base = AT;
    offset = 0;
...
void MipsAssembler::LoadFromThread32(ManagedRegister mdest, ThreadOffset<kMipsWordSize> src, size_t size) {
...
void MipsAssembler::LoadRawPtrFromThread32(ManagedRegister mdest, ThreadOffset<kMipsWordSize> offs) {
  MipsManagedRegister dest = mdest.AsMips();
  CHECK(dest.IsCoreRegister());
