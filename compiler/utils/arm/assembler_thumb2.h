...
class Thumb2Assembler FINAL : public ArmAssembler {
 public:
  ...
  uint32_t AdjustedPosition(uint32_t old_position) OVERRIDE;

  // Macros.

  using ArmAssembler::NewLiteral;  // Make the helper template visible.

  Literal* NewLiteral(size_t size, const void* data) OVERRIDE;
 ...
 private:
  ...
  // cbz and cbnz insns may also need to be replaced with separate 16-bit compare insn and
  // a 16- or 32-bit branch insn. Load from a literal pool can also be 16-bit or 32-bit insn
  // and, if the method is large, we may need to use a sequence of instructions to make up
  // for the limited range of load literal insns (up to 4KiB for the 32-bit variant). When
  // generating code for these insns we don't know the size before hand, so we assume it is
  // the smallest available size and determine the final code offsets and sizes and emit
  // code in FinalizeCode().
  //
  ...
  // every insn. When we bind a label to a branch we don't know the final location yet as
  // some preceding instructions may need to be expanded, so we record a non-final offset.
  // In FinalizeCode(), we expand the sizes of branches and literal loads that are out of
  // range. With each expansion, we need to update dependent Fixups, i.e. insns with target
  // on the other side of the expanded insn, as their offsets change and this may trigger
  // further expansion.
  //
  ...
  class Fixup {
   public:
    ...
    Fixup(uint32_t location, Type type, Size size = kBranch16Bit, Condition cond = AL)
        : rn_(kNoRegister), cond_(cond), type_(type),
          original_size_(size), size_(size), location_(location),
          target_(kUnresolved), adjustment_(0u), dependents_() {
      rt2_ = kNoRegister;
    ...
    Fixup(uint32_t location, Register rn, Condition cond)
        : rn_(rn), cond_(cond), type_(kCompareAndBranchXZero),
          original_size_(kCbxz16Bit), size_(kCbxz16Bit), location_(location),
          target_(kUnresolved), adjustment_(0u), dependents_() {
      rt2_ = kNoRegister;
    ...
    // Load narrow literal.
    Fixup(uint32_t location, Register rt, Size size = kLiteral1KiB)
        : rn_(rt), cond_(AL), type_(kLoadLiteralNarrow),
          original_size_(size), size_(size), location_(location),
          target_(kUnresolved), adjustment_(0u), dependents_() {
      rt2_ = kNoRegister;
    ...
    // Load wide literal.
    Fixup(uint32_t location, Register rt, Register rt2, Size size = kLongOrFPLiteral1KiB)
        : rn_(rt), cond_(AL), type_(kLoadLiteralWide),
          original_size_(size), size_(size), location_(location),
          target_(kUnresolved), adjustment_(0u), dependents_() {
      rt2_ = rt2;
    ...
    Fixup(uint32_t location, SRegister sd, Size size = kLongOrFPLiteral1KiB)
        : rn_(kNoRegister), cond_(AL), type_(kLoadFPLiteralSingle),
          original_size_(size), size_(size), location_(location),
          target_(kUnresolved), adjustment_(0u), dependents_() {
      sd_ = sd;
      DCHECK(size == kLongOrFPLiteral1KiB || size == kLongOrFPLiteral256KiB ||
    ...
    Fixup(uint32_t location, DRegister dd, Size size = kLongOrFPLiteral1KiB)
        : rn_(kNoRegister), cond_(AL), type_(kLoadFPLiteralDouble),
          original_size_(size), size_(size), location_(location),
          target_(kUnresolved), adjustment_(0u), dependents_() {
      dd_ = dd;
      DCHECK(size == kLongOrFPLiteral1KiB || size == kLongOrFPLiteral256KiB ||
    ...
    // Finalize the branch with an adjustment to the location. Both location and target are udated.
  ...
  FixupId AddFixup(Args&&... args) {
    FixupId fixup_id = static_cast<FixupId>(fixups_.size());
    DCHECK_EQ(fixup_id, fixups_.size());  // No truncation.
    fixups_.emplace_back(args...);
    return fixup_id;
  }
