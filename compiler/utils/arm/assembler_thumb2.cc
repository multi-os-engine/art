...
inline void Thumb2Assembler::AdjustFixupIfNeeded(Fixup* fixup, uint32_t* current_code_size,
...
uint32_t Thumb2Assembler::AdjustFixups() {
  ...
  std::deque<FixupId> fixups_to_recalculate;
  if (kIsDebugBuild) {
    // We will use the placeholders in the buffer_ to mark whether the a fixup has
...
uint32_t Thumb2Assembler::AdjustedPosition(uint32_t old_position) {
  // TODO
  return old_position;
}

Literal* Thumb2Assembler::NewLiteral(size_t size, const void* data)  {
...
void Thumb2Assembler::LoadLiteral(Register rt, Literal* literal)  {
  ...
  uint32_t location = buffer_.Size();
  FixupId fixup_id = AddFixup(location, rt, use32bit ? Fixup::kLiteral4KiB : Fixup::kLiteral1KiB);
...
void Thumb2Assembler::LoadLiteral(Register rt, Register rt2, Literal* literal)  {
  ...
  DCHECK(!literal->GetLabel()->IsBound());
  uint32_t location = buffer_.Size();
  FixupId fixup_id = AddFixup(location, rt, rt2, Fixup::kLongOrFPLiteral1KiB);
  Emit16(static_cast<uint16_t>(literal->GetLabel()->position_));
  literal->GetLabel()->LinkTo(fixup_id);
  Emit16(0);
  DCHECK_EQ(location + GetFixup(fixup_id)->GetSizeInBytes(), buffer_.Size());
...
void Thumb2Assembler::LoadLiteral(SRegister sd, Literal* literal)  {
  ...
  DCHECK(!literal->GetLabel()->IsBound());
  uint32_t location = buffer_.Size();
  FixupId fixup_id = AddFixup(location, sd, Fixup::kLongOrFPLiteral1KiB);
  Emit16(static_cast<uint16_t>(literal->GetLabel()->position_));
  literal->GetLabel()->LinkTo(fixup_id);
  Emit16(0);
  DCHECK_EQ(location + GetFixup(fixup_id)->GetSizeInBytes(), buffer_.Size());
...
void Thumb2Assembler::LoadLiteral(DRegister dd, Literal* literal) {
  ...
  DCHECK(!literal->GetLabel()->IsBound());
  uint32_t location = buffer_.Size();
  FixupId fixup_id = AddFixup(location, dd, Fixup::kLongOrFPLiteral1KiB);
  Emit16(static_cast<uint16_t>(literal->GetLabel()->position_));
  literal->GetLabel()->LinkTo(fixup_id);
  Emit16(0);
  DCHECK_EQ(location + GetFixup(fixup_id)->GetSizeInBytes(), buffer_.Size());
