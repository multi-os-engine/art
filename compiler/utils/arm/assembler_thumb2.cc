...
bool Thumb2Assembler::Is32BitDataProcessing(Condition cond,
  ...
  if (so.IsImmediate()) {
    if (opcode == RSB) {
      ...
      // The only thumb1 instruction with a register and an immediate are ADD and SUB
      // with a 3-bit immediate, and a RSB with zero immediate.
    ...
    if (so.IsShift()) {
      // Shift operand - check if it's a MOV convertible to a 16-bit shift instruction.
      if (opcode != MOV) {
        return true;
      }
      // Check for MOV with an ROR/RRX. There's no 16-bit ROR immediate and no 16-bit RRX.
      if (so.GetShift() == ROR || so.GetShift() == RRX) {
        return true;
      }
      // 16-bit shifts set condition codes iff outside IT block, i.e. iff cond == AL.
      ...
      switch (opcode) {
        case ADD:
          // The 16-bit ADD that cannot contain high registers
          // can set condition codes iff outside IT block, i.e. iff cond == AL.
          if (!can_contain_high_register &&
        ...
        case SUB:
        case SBC:
          // These 16-bit opcodes set condition codes iff outside IT block, i.e. iff cond == AL.
          if ((cond == AL) ? set_cc == kCcKeep : set_cc == kCcSet) {
            return true;
          }
          break;
  ...
  if (opcode == MOV && so.IsRegister() && so.IsShift()) {
    ...
    if (so.IsImmediate()) {
      ...
      switch (opcode) {
        ...
        case SBC:
        case BIC: {
          // Sets condition codes iff outside IT block, check that it complies with set_cc.
...
void Thumb2Assembler::EmitShift(Register rd, Register rm, Shift shift, uint8_t amount,
                                Condition cond, SetCc set_cc) {
  CHECK_LT(amount, (1 << 5));
...
void Thumb2Assembler::EmitShift(Register rd, Register rn, Shift shift, Register rm,
                                Condition cond, SetCc set_cc) {
  CHECK_NE(shift, RRX);
