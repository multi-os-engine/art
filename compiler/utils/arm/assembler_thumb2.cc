...
void Thumb2Assembler::EmitDataProcessing(Condition cond,
  ...
  bool must_be_thumb2 = false;
  if (IsHighRegister(rd) || IsHighRegister(rn)) {
    if (opcode != MOV) {
      // There is a high register MOV instruction in thumb1!
      must_be_thumb2 = true;
    }
  }

  if (!must_be_thumb2) {
    if (so.IsRegister() && IsHighRegister(so.GetRegister()) && opcode != MOV) {
      must_be_thumb2 = true;
     }
  }

  if (!must_be_thumb2) {
    // Check for single operand instructions.
    switch (opcode) {
      case CMP:
      case MOV:
      case TST:
      case MVN:
        break;
      case TEQ:
        must_be_thumb2 = true;    // No TEQ in thumb1.
        break;
      default:
    ...
    if (so.IsImmediate()) {
      if (rn != rd) {
        ...
        if (opcode != ADD && opcode != SUB) {
          ...
          // Check that the immediate is 3 bits.
