...
void Thumb2Assembler::Fixup::PrepareFixupDependents(Thumb2Assembler* assembler) {
  ...
  uint32_t num_deps = 0u;
  for (FixupId fixup_id = 0u; fixup_id != end_id; ++fixup_id) {
    num_deps += fixups[fixup_id].dependents_count_;
    fixups[fixup_id].dependents_start_ = num_deps;
  }
  if (num_deps == 0u) {
    return;
  }
  assembler->fixup_dependents_.reset(new FixupId[num_deps]);
...
uint32_t Thumb2Assembler::AdjustFixups() {
  Fixup::PrepareFixupDependents(this);
