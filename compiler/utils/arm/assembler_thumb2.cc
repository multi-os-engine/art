...
void Thumb2Assembler::EmitJumpTableDispatch(JumpTable* jump_table, Register displacement_reg) {
  ...
  Bind(jump_table->GetAnchorLabel());
  add(PC, PC, ShifterOperand(displacement_reg));

  // The anchor's position needs to be fixed up before we can compute offsets, so track it.
  AddTrackedLabel(jump_table->GetAnchorLabel());
}
