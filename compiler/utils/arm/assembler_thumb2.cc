...
void Thumb2Assembler::Branch::Emit(AssemblerBuffer* buffer) const {
  ...
  if (size_ == k32Bit) {
    ...
    if (type_ == kCompareAndBranchZero || type_ == kCompareAndBranchNonZero) {
      offset -= 4;
...
void Thumb2Assembler::EmitBranch(Condition cond, Label* label, bool link, bool x) {
  ...
  if (cond == AL) {
    if (link) {
      if (x) {
        branch_type = Branch::kUnconditionalLinkX;
      } else {
        branch_type = Branch::kUnconditionalLink;
      }
    } else {
      branch_type = Branch::kUnconditional;
    }
  } else {
    branch_type = Branch::kConditional;
  }
  ...
  if (label->IsBound()) {
    ...
    if (size == Branch::k16Bit) {
      ...
      Emit(0);            // Space for a 32 bit branch.
    }
  } else {
