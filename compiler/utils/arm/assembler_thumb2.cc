...
void Thumb2Assembler::EmitShift(Register rd, Register rn, Shift shift, Register rm, bool setcc) {
  ...
  if (IsHighRegister(rd) || IsHighRegister(rm) || IsHighRegister(rn)) {
    must_be_32bit = true;
  } else if (rd != rn) {
    must_be_32bit = true;
  }
  ...
  if (ad.IsImmediate()) {
    ...
    if (ad.GetRegister() == PC) {
      ...
      if (must_be_32bit || offset < 0) {
        ...
        int32_t encoding = 0x1f << 27 | 0xf << 16 | B22 | B20 |
            offset | up |
            static_cast<uint32_t>(rd) << 12;
        Emit32(encoding);
      } else {
        ...
        CHECK_GE(offset, 0);
        CHECK_LT(offset, (1 << 8));
        int32_t encoding = B14 | B11 | static_cast<uint32_t>(rd) << 8 | offset;
        Emit16(encoding);
      }
    } else if (must_be_32bit) {
