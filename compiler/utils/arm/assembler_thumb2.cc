...
uint32_t Thumb2Assembler::AdjustFixups() {
  ...
  DCHECK(IsAligned<2>(current_code_size)) << current_code_size;
...
void Thumb2Assembler::EmitLiterals() {
  if (!literals_.empty()) {
    ...
    uint32_t code_size = buffer_.Size();
    DCHECK(IsAligned<2>(code_size)) << code_size;
    if ((code_size & 2u) != 0u) {
      Emit16(0);
    }
    for (Literal& literal : literals_) {
...
inline int16_t Thumb2Assembler::BEncoding16(int32_t offset, Condition cond) {
  DCHECK(IsAligned<2>(offset)) << offset;
...
inline int16_t Thumb2Assembler::LdrLitEncoding16(Register rt, int32_t offset) {
  ...
  DCHECK(IsAligned<4>(offset)) << offset;
    ...
    case ADD:
      if (so.IsRegister()) {
        ...
        if (rd == SP && rn == SP) {
          ...
          CHECK(IsAligned<4>(immediate)) << immediate;
  ...
  if (ad.IsImmediate()) {
    ...
    if (must_be_32bit) {
      ...
      if (sp_relative) {
        ...
        CHECK(IsAligned<4>(offset)) << offset;
        ...
        if (byte) {
          ...
          // 6 bit offset, shifted by 1.
          CHECK_LT(offset, (1 << 6));
          CHECK(IsAligned<2>(offset)) << offset;
          offset >>= 1;
        } else {
          // 7 bit offset, shifted by 2.
          CHECK_LT(offset, (1 << 7));
