...
void Thumb2Assembler::EmitJumpTables() {
  if (!jump_tables_.empty()) {
    ...
    for (Thumb2JumpTable& table : jump_tables_) {
      // Bulk ensure capacity, as this may be large.
      size_t orig_size = buffer_.Size();
      buffer_.ExtendCapacity(buffer_.Size() + table.GetSize());
#ifndef NDEBUG
      ...
      DCHECK(table.Bound());
      int32_t anchor_position = GetFixup(table.GetDispatchFixupId())->GetLocation() + 4;
...
inline int16_t Thumb2Assembler::AddPcEncoding16(Register rm) {
  return B14 | B10 | /* PC: */ (B7 | B2 | B1 | B0) | (rm << 3);
}
...
void Thumb2Assembler::FinalizeTables() {
  EmitJumpTables();
}
...
uint32_t Thumb2Assembler::Fixup::AdjustSizeIfNeeded(uint32_t current_code_size) {
  ...
  switch (GetSize()) {
    ...
    case kAddPc16Bit:
      // This is only to track the location, always 16 Bit encoding.
      break;
...
void Thumb2Assembler::Fixup::Emit(AssemblerBuffer* buffer, uint32_t code_size) const {
  switch (GetSize()) {
    ...
    case kAddPc16Bit: {
      DCHECK(type_ == kAddPc);
      int16_t add_pc_encoding = AddPcEncoding16(rn_);
      buffer->Store<int16_t>(location_, add_pc_encoding);
      break;
    }
...
void Thumb2Assembler::CmpConstant(Register rn, int32_t value, Condition cond) {
  ...
  // the readability of generated code for some constants.
  ShifterOperand shifter_op;
  if (ShifterOperandCanHold(rn, rn, CMP, value, &shifter_op)) {
    cmp(rn, shifter_op, cond);
  } else {
    CHECK(rn != IP);
    if (ShifterOperandCanHold(rn, rn, CMN, ~value, &shifter_op)) {
...
JumpTable* Thumb2Assembler::CreateJumpTable(std::vector<Label*>&& labels, Register base_reg) {
  jump_tables_.emplace_back(std::forward<std::vector<Label*>>(labels));
...
void Thumb2Assembler::EmitJumpTableDispatch(JumpTable* jump_table, Register displacement_reg) {
  CHECK(!IsForced32Bit()) << "Forced 32-bit dispatch not implemented yet";  // TODO

  Thumb2JumpTable* thumb2_jump_table = down_cast<Thumb2JumpTable*>(jump_table);
  uint32_t location = buffer_.Size();
  Fixup::Size size = IsForced32Bit() ? Fixup::kAddPc16Bit : Fixup::kAddPc16Bit;
  FixupId fixup_id = AddFixup(Fixup::AddPc(location, displacement_reg, size));
  Emit16(static_cast<uint16_t>(thumb2_jump_table->GetAnchorLabel()->position_));
  if (IsForced32Bit()) {
    Emit16(0);
  }
  thumb2_jump_table->Bind(fixup_id);
}
