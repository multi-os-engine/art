...
bool Arm32Assembler::ShifterOpereandCanHoldArm32(uint32_t immediate, ShifterOperand* shifter_op) {
...
bool Arm32Assembler::ShifterOperandCanHold(Register rd ATTRIBUTE_UNUSED,
  ...
  return ShifterOpereandCanHoldArm32(immediate, shifter_op);
}
...
void Arm32Assembler::AddConstant(Register rd, Register rn, int32_t value,
  ...
  ShifterOperand shifter_op;
  if (ShifterOpereandCanHoldArm32(value, &shifter_op)) {
    add(rd, rn, shifter_op, cond);
  } else if (ShifterOpereandCanHoldArm32(-value, &shifter_op)) {
