...
uint32_t Address::encodingThumb(int version) const {
  ...
  if (version == 2) {
      ...
      // PUW must be xx1 or 100
      uint32_t PUW = am_ >> 21;
      if (offset_ >= 0 && offset_ < 256 &&
        ((PUW & 1) == 1 || PUW == 0b100)) {
          // T4 encoding
          encoding |= B11 | PUW << 8 | offset_;
      } else {
...
// TODO: implement these for thumb
bool Address::CanHoldLoadOffsetThumb(int version, LoadOperandType type, int offset) {
  switch (type) {
    case kLoadSignedByte:
    case kLoadSignedHalfword:
    case kLoadUnsignedHalfword:
...
bool Address::CanHoldStoreOffsetThumb(int version, StoreOperandType type, int offset) {
  switch (type) {
    case kStoreHalfword:
    case kStoreWordPair:
      return IsAbsoluteUint(8, offset);  // Addressing mode 3.
