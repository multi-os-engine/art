  ...
  if (null_allowed) {
    ...
    if (in_reg.IsNoRegister()) {
      LoadFromOffset(kLoadWord, out_reg.AsCoreRegister(),
                     SP, handlescope_offset.Int32Value());
      in_reg = out_reg;
    }
    ...
    AddConstant(out_reg.AsCoreRegister(), SP, handlescope_offset.Int32Value(), NE);
  } else {
    AddConstant(out_reg.AsCoreRegister(), SP, handlescope_offset.Int32Value(), AL);
  }
}
...
void ArmAssembler::CreateHandleScopeEntry(FrameOffset out_off,
  ...
  if (null_allowed) {
    LoadFromOffset(kLoadWord, scratch.AsCoreRegister(), SP,
                   handlescope_offset.Int32Value());
    // Null values get a handle scope entry value of 0.  Otherwise, the handle scope entry is
    // the address in the handle scope holding the reference.
    // e.g. scratch = (scratch == 0) ? 0 : (SP+handlescope_offset)
    cmp(scratch.AsCoreRegister(), ShifterOperand(0));
    AddConstant(scratch.AsCoreRegister(), SP, handlescope_offset.Int32Value(), NE);
  } else {
    AddConstant(scratch.AsCoreRegister(), SP, handlescope_offset.Int32Value(), AL);
  }
