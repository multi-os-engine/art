...
class ShifterOperand {
 public:
  ...
  ShifterOperand(Register rm, Shift shift, uint32_t shift_imm) : type_(kRegister), rm_(rm),
      rs_(kNoRegister),
  ...
  static bool CanHoldThumb(int version, uint32_t immediate, ShifterOperand* shifter_op) {
      // TODO: implement
      return false;
    }
...
class Address {
 public:
  ...
  static bool CanHoldLoadOffsetThumb(int version, LoadOperandType type, int offset);
  static bool CanHoldStoreOffsetThumb(int version, StoreOperandType type, int offset);

  uint32_t encodingArm() const;
  uint32_t endodingThumb(int version) const;
...
// This is a abstract ARM assembler.  Subclasses provide assemblers for the individual
...
class ArmAssembler : public Assembler {
 public:
  ...
  virtual void EncodeUint32InTstInstructions(uint32_t data) = 0;
  ...
  // instruction stream.
  virtual void Emit(int32_t value) = 0;
