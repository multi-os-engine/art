...
class Address {
 public:
  ...
  explicit Address(Register rn, Register rm, Mode am = Offset) : rn_(rn), rm_(rm), offset_(0),
      am_(am), is_immed_offset_(false), shift_(LSL) {
    CHECK_NE(rm, PC);
  }

  explicit Address(Register rn, Register rm, Shift shift, uint32_t count, Mode am = Offset) :
