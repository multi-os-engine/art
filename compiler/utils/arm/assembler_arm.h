...
class Literal {
 public:
  ...
  Literal(uint32_t size, const void* data)
      : label_(), size_(size) {
...
class ArmAssembler : public Assembler {
 public:
  ...
  virtual uint32_t AdjustedPosition(uint32_t old_position) = 0;
  ...
  Literal* NewLiteral(typename std::decay<T>::type value) {
    ...
    return NewLiteral(sizeof(value), &value);
  }

  // Create a new literal with the given data.
  virtual Literal* NewLiteral(size_t size, const void* data) = 0;
