...
class ArmAssembler : public Assembler {
 public:
  ...
  // Add a label that will be tracked and its location fixed up in FinalizeCode().
  void AddTrackedLabel(Label* label) {
    tracked_labels_.push_back(label);
  }

  // Shorthand for binding a label that should be tracked.
  void BindTrackedLabel(Label* label) {
    Bind(label);
    AddTrackedLabel(label);
  }
