...
void Arm64Assembler::StoreRef(FrameOffset offs, ManagedRegister m_src) {
  ...
  StoreWToOffset(kStoreWord, src.AsOverlappingXRegisterLow(), SP,
                 offs.Int32Value());
}
...
void Arm64Assembler::StoreImmediateToFrame(FrameOffset offs, uint32_t imm,
  ...
  StoreWToOffset(kStoreWord, scratch.AsOverlappingXRegisterLow(), SP,
                 offs.Int32Value());
}
...
void Arm64Assembler::Load(Arm64ManagedRegister dest, XRegister base,
  ...
  if (dest.IsNoRegister()) {
    ...
    if (size == 4u) {
      ___ Ldr(reg_w(dest.AsOverlappingXRegisterLow()), MEM_OP(reg_x(base), offset));
    } else {
...
void Arm64Assembler::LoadRef(ManagedRegister m_dst, FrameOffset offs) {
  ...
  LoadWFromOffset(kLoadWord, dst.AsOverlappingXRegisterLow(), SP, offs.Int32Value());
}
...
void Arm64Assembler::LoadRef(ManagedRegister m_dst, ManagedRegister m_base,
  ...
  LoadWFromOffset(kLoadWord, dst.AsOverlappingXRegisterLow(), base.AsXRegister(),
                  offs.Int32Value());
}
...
void Arm64Assembler::CopyRef(FrameOffset dest, FrameOffset src,
  ...
  Arm64ManagedRegister scratch = m_scratch.AsArm64();
  CHECK(scratch.IsXRegister()) << scratch;
  LoadWFromOffset(kLoadWord, scratch.AsOverlappingXRegisterLow(),
                  SP, src.Int32Value());
  StoreWToOffset(kStoreWord, scratch.AsOverlappingXRegisterLow(),
                 SP, dest.Int32Value());
}
...
void Arm64Assembler::Copy(FrameOffset dest, FrameOffset src,
  ...
  if (size == 4) {
    LoadWFromOffset(kLoadWord, scratch.AsOverlappingXRegisterLow(), SP, src.Int32Value());
    StoreWToOffset(kStoreWord, scratch.AsOverlappingXRegisterLow(), SP, dest.Int32Value());
  } else if (size == 8) {
...
void Arm64Assembler::Copy(ManagedRegister m_dest, Offset dest_offset,
  ...
  if (size == 4) {
    if (scratch.IsWRegister()) {
      ...
      LoadWFromOffset(kLoadWord, scratch.AsOverlappingXRegisterLow(), src.AsXRegister(),
                    src_offset.Int32Value());
      StoreWToOffset(kStoreWord, scratch.AsOverlappingXRegisterLow(), dest.AsXRegister(),
                   dest_offset.Int32Value());
    }
  } else if (size == 8) {
...
void Arm64Assembler::Call(FrameOffset base, Offset offs, ManagedRegister m_scratch) {
  ...
  CHECK(scratch.IsXRegister()) << scratch;
  // Call *(*(SP + base) + offset)
  LoadWFromOffset(kLoadWord, scratch.AsOverlappingXRegisterLow(), SP, base.Int32Value());
...
void Arm64Assembler::CreateHandleScopeEntry(ManagedRegister m_out_reg, FrameOffset handle_scope_offs,
  ...
  if (null_allowed) {
    ...
    if (in_reg.IsNoRegister()) {
      LoadWFromOffset(kLoadWord, out_reg.AsOverlappingXRegisterLow(), SP,
                      handle_scope_offs.Int32Value());
      in_reg = out_reg;
    }
    ___ Cmp(reg_w(in_reg.AsOverlappingXRegisterLow()), 0);
...
void Arm64Assembler::CreateHandleScopeEntry(FrameOffset out_off, FrameOffset handle_scope_offset,
  ...
  CHECK(scratch.IsXRegister()) << scratch;
  if (null_allowed) {
    LoadWFromOffset(kLoadWord, scratch.AsOverlappingXRegisterLow(), SP,
                    handle_scope_offset.Int32Value());
    // Null values get a handle scope entry value of 0.  Otherwise, the handle scope entry is
    // the address in the handle scope holding the reference.
    // e.g. scratch = (scratch == 0) ? 0 : (SP+handle_scope_offset)
    ___ Cmp(reg_w(scratch.AsOverlappingXRegisterLow()), 0);
