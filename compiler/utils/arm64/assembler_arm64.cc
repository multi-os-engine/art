...
void Arm64Assembler::StoreImmediateToThread(ThreadOffset<4> offs, uint32_t imm,
                                            ManagedRegister m_scratch) {
  Arm64ManagedRegister scratch = m_scratch.AsArm64();
  CHECK(scratch.IsCoreRegister()) << scratch;
  LoadImmediate(scratch.AsCoreRegister(), imm);
...
void Arm64Assembler::StoreStackOffsetToThread(ThreadOffset<4> tr_offs,
                                              FrameOffset fr_offs,
                                              ManagedRegister m_scratch) {
  Arm64ManagedRegister scratch = m_scratch.AsArm64();
  CHECK(scratch.IsCoreRegister()) << scratch;
...
void Arm64Assembler::StoreStackPointerToThread(ThreadOffset<4> tr_offs) {
...
void Arm64Assembler::Load(ManagedRegister m_dst, ThreadOffset<4> src, size_t size) {
...
void Arm64Assembler::LoadRawPtrFromThread(ManagedRegister m_dst, ThreadOffset<4> offs) {
  Arm64ManagedRegister dst = m_dst.AsArm64();
  CHECK(dst.IsCoreRegister()) << dst;
...
void Arm64Assembler::CopyRawPtrToThread(ThreadOffset<4> tr_offs,
                                        FrameOffset fr_offs,
                                        ManagedRegister m_scratch) {
  Arm64ManagedRegister scratch = m_scratch.AsArm64();
  CHECK(scratch.IsCoreRegister()) << scratch;
...
void Arm64Assembler::Call(ThreadOffset<4> /*offset*/, ManagedRegister /*scratch*/) {
