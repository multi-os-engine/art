...
struct IntrusiveForwardListHook {
  ...
  IntrusiveForwardListHook* next_hook;
};
...
class IntrusiveForwardListIterator : public std::iterator<std::forward_iterator_tag, T> {
 ...
 private:
  ...
  operator==(
      const IntrusiveForwardListIterator<OtherT1, OtherTraits>& lhs,
...
class IntrusiveForwardList {
 public:
  ...
  void assign(InputIterator first, InputIterator last) {
    *this = IntrusiveForwardList(first, last);
  }
  ...
  iterator insert_after(const_iterator position, value_type& value) {
    IntrusiveForwardListHook* new_hook = hook_traits::GetHook(&value);
  ...
  void splice_after(const_iterator position,
    ...
    // Detach (first, last).
    IntrusiveForwardListHook* first_taken = first.hook_->next_hook;
  ...
  void merge(IntrusiveForwardList& other) {
    merge(other, [](const value_type& lhs, const value_type& rhs) { return lhs < rhs; });
  }
  void merge(IntrusiveForwardList&& other) {
  ...
  void reverse() {
    ...
    const_iterator insert_pos = reversed.before_begin();
    while (!empty()) {
      value_type& value = front();
      erase_after(before_begin());
      insert_pos = reversed.insert_after(insert_pos, value);
    }
  }
...
class IntrusiveForwardListMemberHook {
 public:
  ...
  static const T* GetValue(const IntrusiveForwardListHook* hook) {
    return reinterpret_cast<const T*>(
        reinterpret_cast<uintptr_t>(hook) - OFFSETOF_MEMBERPTR(T, NextPtr));
  }

 private:
  static uintptr_t HookToValueImpl(uintptr_t hook) {
    return hook - OFFSETOF_MEMBERPTR(T, NextPtr);
  }
};
