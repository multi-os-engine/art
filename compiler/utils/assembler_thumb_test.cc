...
void dump(std::vector<uint8_t>& code, const char* testname) {
  // This will only work on the host.  There is no as, objcopy of objdump on the
  // device.
  ...
  snprintf(cmd, sizeof(cmd), "arm-eabi-objdump -d %s.oo | grep '^  *[0-9a-f][0-9a-f]*:'", filename);
  if (kPrintResults) {
