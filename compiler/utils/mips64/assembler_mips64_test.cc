...
TEST_F(AssemblerMIPS64Test, Dext) {
  DriverStr(
      RepeatRRIbIb(&mips64::Mips64Assembler::Dext, 5, 5,
          "dext ${reg1}, ${reg2}, {imm1}, {imm2} + 1"),
      "Dext");
}
...
TEST_F(AssemblerMIPS64Test, Dinsu) {
  ...
  for (mips64::GpuRegister* reg1 : reg1_registers) {
    for (mips64::GpuRegister* reg2 : reg2_registers) {
      for (int32_t pos_less_32 = 0; pos_less_32 < 32; pos_less_32++) {
        for (int32_t size_less_one = 0; pos_less_32 + size_less_one < 32; size_less_one++) {
          __ Dinsu(*reg1, *reg2, pos_less_32, size_less_one);
          std::ostringstream instr;
          instr << "dinsu $" << *reg1 << ", $" << *reg2 << ", " << pos_less_32 + 32 << ", "
              << size_less_one + 1 << "\n";
          expected += instr.str();
        }
      }
