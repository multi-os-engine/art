...
void Mips64Assembler::Dext(GpuRegister rt, GpuRegister rs, int pos, int size_less_one) {
  DCHECK(pos >= 0 && pos < 32) << pos;
  DCHECK(size_less_one >= 0 && size_less_one < 32) << size_less_one;
...
void Mips64Assembler::Cvtsw(FpuRegister fd, GpuRegister rs) {
  EmitFR(0x11, 0x14, static_cast<FpuRegister>(0), static_cast<FpuRegister>(rs), fd, 0x20);
}

void Mips64Assembler::Cvtdw(FpuRegister fd, GpuRegister rs) {
  EmitFR(0x11, 0x14, static_cast<FpuRegister>(0), static_cast<FpuRegister>(rs), fd, 0x21);
}
...
//
//
// TODO: there are rumors that j imm26 and jal imm26 may be deprecated on R6.
// Consider using other R6 instructions.
//
...
void Mips64Assembler::B(Label* label) {
  if (label->IsBound()) {
    ...
    // The first branch forward will have 0 in its pc-relative address (copied from label's position).
    // It will be the terminator of the list of forward-reaching branches.
