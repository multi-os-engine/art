...
#ifdef  MIPS_LOAD_CONST_PATH
    a->loadConst32Path |= kMipsPathAddiu;
#endif
...
void LoadReplicatedConst32(Asm *a, Rtype rd, int64_t value) {
  ...
  if (IsUint<16>(x)) {
    ...
    a->LoadConst32(rd, x);
#ifdef  MIPS_LOAD_CONST_PATH
    DCHECK(a->loadConst32Path == kMipsPathLui ||
           a->loadConst32Path == kMipsPathLuiOri) << a->loadConst32Path;
      ...
      if (IsUint<16>(tmp)) {
        ...
        if (path1 == kMipsPathOri) {
          if (path2 == kMipsPathDahi) {
            CHECK_EQ(path3, kMipsPathDati) << std::hex << "0x" << (path1|path2|path3);
            a->loadConst64Path |= kMipsPathOriDahiDati;
          } else {
            CHECK_EQ(path2, kMipsPathDahi) << std::hex << "0x" << (path1|path2|path3);
          }
