...
void FinalEntrypointRelocationSetArm::Apply(uint8_t* code, const OatWriter* writer,
  ...
  uint32_t island = writer->GetCurrentTrampolineIsland();
  ...
  for (auto& reloc : relocations_) {
    ...
    switch (reloc.type_) {
      case REL_CALL: {
        ...
        uint32_t target = island + static_cast<uint32_t>(reloc.value_) * 4;
        int32_t delta = target - pc;

        if (debugprint) {
          LOG(INFO) << "applying final relocation for island " << island;
          LOG(INFO) << "pc: " << std::hex << pc << ", target: " << target <<
              ", reloc.value: " << reloc.value_ << ", delta: " << delta;
        }

        // Max range for a Thumb2 BL is 16MB.
        CHECK_GT(delta, -16*1024*1024);
