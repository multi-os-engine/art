...
bool OatWriter::WriteOatDexFiles(OutputStream* rodata,
  ...
  TimingLogger::ScopedTiming split("WriteOatDexFiles", timings);
...
size_t OatWriter::InitOatClasses(size_t offset) {
  ...
  for (OatDexFile& oat_dex_file : oat_dex_files_) {
    ...
    oat_header_->UpdateChecksum(oat_dex_file.class_offsets_.data(),
...
bool OatWriter::WriteTables(OutputStream* out, const size_t file_offset) {
  for (size_t i = 0; i != oat_dex_files_.size(); ++i) {
    ...
    if (!out->WriteFully(oat_dex_files_[i].class_offsets_.data(),
...
void OatWriter::OatClass::UpdateChecksum(OatHeader* oat_header) const {
  ...
  oat_header->UpdateChecksum(method_offsets_.data(),
                             sizeof(method_offsets_[0]) * method_offsets_.size());
}
...
bool OatWriter::OatClass::Write(OatWriter* oat_writer,
  ...
  if (!out->WriteFully(method_offsets_.data(),
                       sizeof(method_offsets_[0]) * method_offsets_.size())) {
