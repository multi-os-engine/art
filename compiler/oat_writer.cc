...
struct OatWriter::GcMapBinder {
  ALWAYS_INLINE
  static const std::vector<uint8_t>* GetMap(const CompiledMethod* compiled_method) {
...
struct OatWriter::VmapTableBinder {
  ...
  ALWAYS_INLINE
  static uint32_t* GetSizePtr(OatWriter* writer) {
    return &writer->size_vmap_table_;
  }
  ...
  bool ProcessMethod(size_t class_def_method_index, const ClassDataItemIterator& it) {
    ...
    if (compiled_method != NULL) {  // ie. not an abstract method
      ...
      if (map_size != 0u && map_offset == offset_) {
        ...
        *MapBinder::GetSizePtr(writer_) += map_size;
        offset_ += map_size;
      }
      DCHECK_OFFSET_();
    }
...
size_t OatWriter::InitOatClasses(size_t offset) {
  ...
  class InitOatClassesProcessor : public DexMethodProcessor {
   public:
    InitOatClassesProcessor(OatWriter* writer, size_t offset)
      : DexMethodProcessor(writer, offset),
        compiled_methods_(),
  ...
  InitOatClassesProcessor processor(this, offset);
  ProcessDexMethods(&processor);
