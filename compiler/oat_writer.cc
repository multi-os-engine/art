...
OatWriter::OatWriter(const std::vector<const DexFile*>& dex_files,
  ...
  : compiler_driver_(compiler),
    ...
    current_trampoline_island_(0xffffffff) {
  size_t offset;
  {
  ...
  if (compiled_method != nullptr) {
    ...
    if (portable_code != nullptr) {
      ...
      if (code_iter != code_offsets_.end()) {
        ...
        if (kIsDebugBuild) {
          // check that there are no missed bl calls
          const uint8_t *code = &(*quick_code)[0];
          for (uint32_t i = 0 ; i < code_size; i++) {
            const uint8_t* p = &code[i];
            if (p[0] == 0x00 && p[1] == 0xf0 && p[2] == 0x00 && p[3] == 0xd0) {
              LOG(FATAL) << "missed an entrypoint trampoline for method " <<
...
uint32_t OatWriter::AllocateTrampolineIslandIfNecessary(uint32_t offset) {
  uint32_t max_offset = 15*1024*1024;   // TODO: move this to compiler driver.
  uint32_t next_trampoline = current_trampoline_island_ + max_offset;
  if (current_trampoline_island_ == 0xffffffff || offset >= next_trampoline) {
    LOG(INFO) << "Need trampoline island at offset " << std::hex << offset;
    uint32_t size = compiler_driver_->GetEntrypointTrampolineSize();
    trampoline_islands_.push_back(offset);
    current_trampoline_island_ = offset;
    return size;
  }
  return 0;
}
...
uint32_t OatWriter::WriteTrampolineIslandIfNecessary(OutputStream* out, uint32_t offset) {
  for (size_t i = 0; i < trampoline_islands_.size(); ++i) {
    if (trampoline_islands_[i] == offset) {
      uint32_t size = compiler_driver_->GetEntrypointTrampolineSize();
      LOG(INFO) << "Writing trampoline island at offset " << std::hex << offset << " size: "
          << std::dec << size;
      const std::vector<uint8_t>& code = compiler_driver_->GetEntrypointTrampolineCode();
