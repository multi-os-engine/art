...
class OatWriter::WriteCodeMethodVisitor : public OatDexMethodVisitor {
        ...
        if (method_offsets.code_offset_ >= offset_) {
          ...
          for (const LinkerPatch& patch : compiled_method->GetPatches()) {
            if (patch.Type() == kLinkerPatchCallRelative) {
              ...
              // NOTE: Direct method pointers across oat files don't use linker patches.
              ...
              // NOTE: Direct type pointers across oat files don't use linker patches.
 ...
 private:
  ...
  void PatchObjectAddress(std::vector<uint8_t>* code, uint32_t offset, mirror::Object* object)
    ...
    uint32_t address = PointerToLowMemUInt32(writer_->image_writer_->GetImageAddress(object));
  ...
  void PatchCodeAddress(std::vector<uint8_t>* code, uint32_t offset, uint32_t target_offset)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    // TODO: This needs to be cleaned up. We're relying on the start_file_offset_ telling
    // us the correct offset from the end of the image to the oat data. However, that's
    // the case only if using ElfWriterQuick, not when using ElfWriterMclinker.
    DCHECK(writer_->image_writer_ != nullptr);
    uint32_t address = PointerToLowMemUInt32(writer_->image_writer_->GetOatFileBegin() +
                                             writer_->oat_data_file_offset_ + target_offset);
    DCHECK_LE(offset + 4, code->size());
    uint8_t* data = &(*code)[offset];
    data[0] = address & 0xffu;
    data[1] = (address >> 8) & 0xffu;
...
bool OatWriter::Write(OutputStream* out) {
  ...
  oat_data_file_offset_ = file_offset;
