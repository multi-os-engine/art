...
class OatWriter::RelativePatcher {
 public:
  ...
  virtual void PatchDexCacheReference(std::vector<uint8_t>* code, uint32_t literal_offset,
                                  uint32_t patch_offset, uint32_t target_offset) = 0;
...
class OatWriter::Arm64RelativePatcher FINAL : public ArmBaseRelativePatcher {
 public:
  ...
  virtual void PatchDexCacheReference(std::vector<uint8_t>* code ATTRIBUTE_UNUSED,
                                      ...
                                      uint32_t patch_offset ATTRIBUTE_UNUSED,
                                      uint32_t target_offset ATTRIBUTE_UNUSED) {
    // NOTE: Cortex-A53 Cat-A Erratum 843419 doesn't apply:
    //    - case 1: we never produce unaligned load/store,
    //    - case 2: we never produce a dead ADRP.

    DCHECK_EQ(patch_offset & 3u, 0u);
    DCHECK_EQ(target_offset & 3u, 0u);
    uint32_t insn = GetInsn(code, literal_offset);
    // Check it's an ADRP with imm == 0 (unset).
    DCHECK_EQ((insn & 0x9fffffe0u), 0x90000000u);
    uint32_t disp = target_offset - (patch_offset & ~0xfffu);
    ...
    // Look for the 32-bit LDR with the same base register.
    uint32_t base = insn & 0x1fu;
    uint32_t ldr_offset = literal_offset;
    do {
      ldr_offset += 4u;
      insn = GetInsn(code, ldr_offset);
    } while ((insn & 0xffc003e0) != (0xb9400000 + (base << 5)));
          ...
          if (!compiled_method->GetPatches().empty()) {
            ...
            for (const LinkerPatch& patch : compiled_method->GetPatches()) {
              if (patch.Type() == kLinkerPatchCallRelative) {
                ...
                uint32_t literal_offset = patch.LiteralOffset();
                writer_->relative_patcher_->PatchDexCacheReference(&patched_code_, literal_offset,
  ...
  uint32_t GetDexCacheOffset(const LinkerPatch& patch) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    if (writer_->image_writer_ != nullptr) {
      auto* element = writer_->image_writer_->GetDexCacheArrayElementImageAddress(
              patch.TargetDexCacheDexFile(), patch.TargetDexCacheArrayOffset());
      return reinterpret_cast<const uint8_t*>(element) - writer_->image_writer_->GetOatFileBegin();
    } else {
