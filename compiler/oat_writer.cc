...
bool OatWriter::WriteDexFiles(OutputStream* rodata,
  ...
  if (!WriteDexFiles(rodata, file) ||
      !ExtendForTypeLookupTables(rodata, file) ||
      !OpenDexFiles(file) ||
      !WriteTypeLookupTables() ||
      !WriteOatDexFiles(rodata)) {
    return false;
  }
...
bool OatWriter::ExtendForTypeLookupTables(OutputStream* rodata, File* file) {
  TimingLogger::ScopedTiming split("ExtendForTypeLookupTables", timings_);

  size_t size_after_dex_files = size_;
  for (OatDexFile& oat_dex_file : oat_dex_files_) {
    oat_dex_file.ReserveTypeLookupTable(this);
  }
  size_t size_after_type_lookup_tables = size_;
  DCHECK_LE(size_after_dex_files, size_after_type_lookup_tables);
  if (size_after_type_lookup_tables != size_after_dex_files) {
    int64_t new_length =
        oat_data_offset_ + dchecked_integral_cast<int64_t>(size_after_type_lookup_tables);
...
bool OatWriter::OpenDexFiles(File* file) {
  ...
  std::string error_msg;
  dex_files_map_.reset(MemMap::MapFile(length,
  ...
  for (OatDexFile& oat_dex_file : oat_dex_files_) {
    ...
    const uint8_t* raw_dex_file =
        dex_files_map_->Begin() + oat_dex_file.dex_file_offset_ - map_offset;
    ...
    owned_dex_files_.emplace_back(DexFile::Open(raw_dex_file,
                                                oat_dex_file.dex_file_size_,
                                                oat_dex_file.GetLocation(),
                                                oat_dex_file.dex_file_location_checksum_,
                                                /* oat_dex_file */ nullptr,
                                                &error_msg));
    if (owned_dex_files_.back() == nullptr) {
...
bool OatWriter::WriteTypeLookupTables() {
  ...
  for (size_t i = 0, size = owned_dex_files_.size(); i != size; ++i) {
    ...
    if (oat_dex_file->lookup_table_offset_ != 0u) {
      ...
      uint8_t* lookup_table = dex_files_map_->Begin() + (lookup_table_offset - map_offset);
      owned_dex_files_[i]->CreateTypeLookupTable(lookup_table);
    }
  }

  if (msync(dex_files_map_->BaseBegin(), dex_files_map_->BaseSize(), MS_SYNC) != 0) {
    PLOG(ERROR) << "Failed to msync() type lookup tables. Map: " << dex_files_map_->GetName();
    return false;
  }
