...
void CommonCompilerTest::SetUp() {
  CommonRuntimeTest::SetUp();
  {
    ...
    std::unique_ptr<const InstructionSetFeatures> instruction_set_features(
        InstructionSetFeatures::FromFeatureString(instruction_set,
                                                  Runtime::GetDefaultInstructionSetFeatures(),
                                                  &error_msg));
    CHECK(instruction_set_features.get() != nullptr) << error_msg;

    {
      // Sanity check the instruction set features using /proc/cpuinfo.
      ...
      EXPECT_TRUE(cpuinfo_features->Equals(instruction_set_features.get()))
          << *instruction_set_features.get() << " != " << *cpuinfo_features.get();
    }
    {
      // Sanity check the instruction set features using /proc/cpuinfo.
      ...
      EXPECT_TRUE(hwcap_features->Equals(instruction_set_features.get()))
          << *instruction_set_features.get() << " != " << *hwcap_features.get();
    }
