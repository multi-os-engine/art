 ...
 *
 * If `visualizer` is non null, it is invoked by Execute() to dump the
 * graph after the optimisation has been performed (by the Run()
 * method).
 */
class Optimization : public ValueObject {
 public:
  Optimization(HGraph* graph,
               bool is_in_ssa_form,
 ...
 private:
  template <typename T>
  void CheckWithChecker(T* checker) {
