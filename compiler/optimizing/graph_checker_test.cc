...
// Test case with a binary operation containing a constant not
// located at the end of its inputs.
TEST(GraphChecker, BinaryOperationWithConstantInputNotAtTheEnd) {
  ArenaPool pool;
  ArenaAllocator allocator(&pool);

  /**
   * A control-flow graph with a binary operation (4: Sub) having one
   * non-constant input (3: Add) and one constant input
   * (2: IntConstant).
   *
   *   BasicBlock 0, succ: 1
   *     0: IntConstant [3]
  ...
  HGraph* graph = new (&allocator) HGraph(&allocator);
  HBasicBlock* entry_block = new (&allocator) HBasicBlock(graph);
  HIntConstant* one = new (&allocator) HIntConstant(1);
  entry_block->AddInstruction(one);
  HIntConstant* two = new (&allocator) HIntConstant(2);
  entry_block->AddInstruction(two);
  HIntConstant* three = new (&allocator) HIntConstant(3);
  entry_block->AddInstruction(three);
  HAdd* addition = new (&allocator) HAdd(Primitive::kPrimInt, one, two);
  entry_block->AddInstruction(addition);
  // Left-hand input is not constant, right-hand one is.
  HSub* subtraction =
    new (&allocator) HSub(Primitive::kPrimInt, addition, three);
  entry_block->AddInstruction(subtraction);
  entry_block->AddInstruction(new (&allocator) HGoto());
  graph->AddBlock(entry_block);
  graph->SetEntryBlock(entry_block);
  HBasicBlock* exit_block = new (&allocator) HBasicBlock(graph);
  ...
  // Swap the inputs of `subtraction` to make it invalid (as having a
  // constant not located at the end of its inputs).
  entry_block->ReplaceAndRemoveInstructionWith(subtraction,
                                               new (&allocator) HSub(Primitive::kPrimInt,
                                                                     three, addition));
  graph_checker.VisitInsertionOrder();
  ASSERT_FALSE(graph_checker.IsValid());
}
