...
void ParallelMoveResolverNoSwap::EmitNativeCode(HParallelMove* parallel_move) {
  DCHECK_EQ(GetNumOfPendingMoves(), static_cast<size_t>(0));
  DCHECK(moves_.IsEmpty());
  ...
  // Perform the moves with constant sources and register destinations.
  for (size_t i = 0; i < moves_.Size(); ++i) {
    MoveOperands* move = moves_.Get(i);
...
void ParallelMoveResolverNoSwap::PerformMove(size_t index) {
  ...
  // This move's destination may be used as scratch location. And the move can
  // be luckily performed.
  ...
  if (IsBlockedByMoves(destination)) {
    ...
    Location scratch = AllocateScratchLocation(source);
    // Perform (C -> scratch)
    ...
    // Add (scratch -> dst)
    AddPendingMove(scratch, destination);
  } else {
    // This move is not blocked.
    EmitMove(index);
    move->Eliminate();
  ...
  // Moves in pending list should not block any other moves. But to perform
  // unblocked moves in pending list can free scratch registers, so we do this
  // as early as possible.
  MoveOperands* pending_move;
  for (; (pending_move = GetUnblockedPendingMove(source)) != nullptr;) {
    Location pending_source = pending_move->GetSource();
    Location pending_destination = pending_move->GetDestination();
    pending_move->Eliminate();
    ...
    for (size_t i = 0; i < scratches_.Size(); ++i) {
      ...
      if (scratch.OverlapWith(pending_source) && !IsBlockedByMoves(scratch)) {
...
void ParallelMoveResolverNoSwap::UpdateMoveSource(Location from, Location to) {
  ...
  // (from -> to) has been performed. Since there should no moves with the same
  // destination, (to -> X) can not be blocked while (from -> X) might still be
  // blocked. For example, there are moves like (0 -> 1) (1 -> 2) (1 -> 3). After
  // (1 -> 2) was performed, the left moves are (0 -> 1) and (1 -> 3). There is
  // dependency between the two. If we update source location from 1 to 2, we will
  // get (0 -> 1) and (2 -> 3). There is no dependency between the two.
  //
  // This is not something must do, but we can use less scratch locations with this
  // trick. For example, we can avoid using additional scratch locations for moves
  // (0 -> 1), (1 -> 2), (1 -> 0).
  for (size_t i = 0; i < moves_.Size(); ++i) {
    MoveOperands* move = moves_.Get(i);
...
void ParallelMoveResolverNoSwap::AddPendingMove(Location source, Location destination) {
  MoveOperands* move = nullptr;
  for (size_t i = 0; i < pending_moves_.Size(); ++i) {
    MoveOperands* cur = pending_moves_.Get(i);
    if (cur->IsEliminated()) {
      move = cur;
      break;
    }
  ...
  if (move == nullptr) {
    move = new (allocator_) MoveOperands(source, destination, nullptr);
    pending_moves_.Add(move);
  } else {
...
MoveOperands* ParallelMoveResolverNoSwap::GetUnblockedPendingMove(Location loc) {
  for (size_t i = 0; i < pending_moves_.Size(); ++i) {
    ...
    if (destination.OverlapWith(loc) && !IsBlockedByMoves(destination)) {
      return move;
    }
  }
  return nullptr;
...
size_t ParallelMoveResolverNoSwap::GetNumOfPendingMoves() {
  size_t num = 0;
  for (size_t i = 0; i < pending_moves_.Size(); ++i) {
    num = pending_moves_.Get(i)->IsEliminated() ? num : num + 1;
  }
  return num;
}
