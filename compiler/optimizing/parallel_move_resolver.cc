...
void ParallelMoveResolverNoSwap::PerformMove(size_t index) {
  ...
  if (move->GetSource().Equals(move->GetDestination())) {
    ...
    // be luckily performed. For example, (0 -> 1) (1 -> 0) (1 -> 2). There is
    // a cycle (0 -> 1) (1 -> 0), we will resolve the cycle as (1 -> scratch)
    // (0 -> 1) (scratch -> 0). In this case, (1 -> 2) will already have been
    // performed if we use 2 as the scratch location.
    move->Eliminate();
    return;
  }
  ...
  // Moves in pending list should not block any other moves. But performing
  ...
  for (; (pending_move = GetUnblockedPendingMove(source)) != nullptr;) {
    ...
    // We do not depends on the pending move index. So just delete the move
    // instead of eliminating it to make the pending list cleaner.
...
void ParallelMoveResolverNoSwap::AddPendingMove(Location source, Location destination) {
  pending_moves_.Add(new (allocator_) MoveOperands(source, destination, nullptr));
}
