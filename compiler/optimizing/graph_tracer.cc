 ...
 */
class HTracerPrinter : public HGraphVisitor {
 public:
  HTracerPrinter(HGraph* graph, std::ostream& output)
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(HTracerPrinter);
};

HGraphTracer::HGraphTracer(std::ostream* output,
                           HGraph* graph,
  ...
  is_enabled_ = true;
  HTracerPrinter printer(graph, *output_);
...
void HGraphTracer::TraceGraph(const char* pass_name) {
  if (!is_enabled_) {
    return;
  }
  HTracerPrinter printer(graph_, *output_);
