...
TYPED_TEST(ParallelMoveTest, MultiCycles) {
  ...
  }
  {
    ...
    HParallelMove* moves = new (&allocator) HParallelMove(&allocator);
    moves->AddMove(
        Location::RegisterPairLocation(1, 2),
    ...
    moves->AddMove(
        ...
        nullptr);
    if (TestFixture::has_swap) {
      // The swap version doesn't support overlap like (0,1) and (1,2). This is OK, as we always
      // create even/even+1 register pairs in optimizing compiler.
    } else {
      resolver.EmitNativeCode(moves);
      ASSERT_STREQ("(4,5 -> 10,11) (1,2 -> 4,5) (10,11 -> 0,1) (0,1 -> 2,3)",
          resolver.GetMessage().c_str());
    }
  }
}
