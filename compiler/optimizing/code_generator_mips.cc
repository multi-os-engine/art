...
void CodeGeneratorMIPS::GenerateFrameExit() {
  ...
  if (!HasEmptyFrame()) {
    ...
    // For better instruction scheduling restore RA before other GPRs.
    ofs = GetFrameSize();
    for (int i = arraysize(kCoreCalleeSaves) - 1; i >= 0; --i) {
      Register reg = kCoreCalleeSaves[i];
      if (allocated_registers_.ContainsCoreRegister(reg)) {
...
void LocationsBuilderMIPS::VisitInvokeStaticOrDirect(HInvokeStaticOrDirect* invoke) {
  ...
  if (invoke->HasPcRelativeDexCache()) {
    // TODO: review. The current assumption is that kDexCachePcRelative and
    // kDirectAddressWithFixup/kCallDirectWithFixup are mutually exclusive.
...
void CodeGeneratorMIPS::GenerateStaticOrDirectCall(HInvokeStaticOrDirect* invoke, Location temp) {
  ...
  switch (code_ptr_location) {
    ...
    case HInvokeStaticOrDirect::CodePtrLocation::kCallDirectWithFixup:
      ...
      if (isR6) {
        __ LoadLiteralR6(T9, DeduplicateMethodCodeLiteral(invoke->GetTargetMethod()));
      } else {
        __ LoadLiteralR2(T9, base_reg, DeduplicateMethodCodeLiteral(invoke->GetTargetMethod()));
      }
      break;
    default:
      break;
  ...
  switch (method_load_kind) {
    ...
    case HInvokeStaticOrDirect::MethodLoadKind::kDirectAddressWithFixup:
      if (isR6) {
        __ LoadLiteralR6(temp.AsRegister<Register>(),
                         DeduplicateMethodAddressLiteral(invoke->GetTargetMethod()));
      } else {
        __ LoadLiteralR2(temp.AsRegister<Register>(),
                         base_reg,
                         DeduplicateMethodAddressLiteral(invoke->GetTargetMethod()));
      }
      break;
