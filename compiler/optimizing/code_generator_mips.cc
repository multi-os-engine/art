...
void InstructionCodeGeneratorMIPS::HandleShift(HBinaryOperation* instr) {
  ...
  switch (type) {
    ...
    case Primitive::kPrimLong: {
      ...
      // TODO: Implement shl-long, shr-long and ushr-long witout resorting to the runtime.
...
void InstructionCodeGeneratorMIPS::VisitDiv(HDiv* div) {
  ...
  switch (type) {
    ...
    case Primitive::kPrimLong:
      // TODO: Implement div-long witout resorting to the runtime.
...
void InstructionCodeGeneratorMIPS::VisitMul(HMul* mul) {
  ...
  switch (type) {
    ...
    case Primitive::kPrimLong:
      // TODO: Implement mul-long witout resorting to the runtime.
  ...
  switch (type) {
    ...
    case Primitive::kPrimDouble:
      __ NegD(FromLowSToD(out.AsFpuRegisterPairLow<FRegister>()),
              FromLowSToD(in.AsFpuRegister<FRegister>()));
      break;
...
void InstructionCodeGeneratorMIPS::VisitRem(HRem* instruction) {
  ...
  switch (type) {
    ...
    case Primitive::kPrimFloat:
    case Primitive::kPrimDouble: {
      // TODO: Implement rem-float and rem-double witout resorting to the runtime?
...
void InstructionCodeGeneratorMIPS::VisitTypeConversion(HTypeConversion* conversion) {
  ...
  if (Primitive::IsIntegralType(result_type) && Primitive::IsIntegralType(input_type)) {
    ...
    if (input_type != Primitive::kPrimLong) {
      __ Mtc1(FTMP, locations->InAt(0).AsRegister<Register>());
      if (result_type == Primitive::kPrimFloat) {
        __ Cvtsw(locations->Out().AsFpuRegister<FRegister>(), FTMP);
      } else {
        __ Cvtdw(FromLowSToD(locations->Out().AsFpuRegisterPairLow<FRegister>()), FTMP);
      }
    } else {
    ...
    if (result_type == Primitive::kPrimFloat) {
      __ Cvtsd(locations->Out().AsFpuRegister<FRegister>(),
               FromLowSToD(locations->InAt(0).AsFpuRegisterPairLow<FRegister>()));
    } else {
      __ Cvtds(FromLowSToD(locations->Out().AsFpuRegisterPairLow<FRegister>()),
