...
Location InvokeDexCallingConventionVisitorMIPS::GetNextLocation(Primitive::Type type) {
  ...
  switch (type) {
    ...
    // Note: both float and double types are stored in even fpu registers.On 32 bit FPU, double will
    // take up the even/odd pair, while floats are stored in even regs only. On 64 bit FPU, both
    // double and float are stored in even registers only.
    case Primitive::kPrimFloat:
    case Primitive::kPrimDouble: {
...
void ParallelMoveResolverMIPS::EmitSwap(size_t index) {
  ...
  if (loc1.IsRegister() && loc2.IsRegister()) {
    ...
    Exchange(loc1.GetStackIndex(), loc2.GetStackIndex(), false);
  } else if (loc1.IsDoubleStackSlot() && loc2.IsDoubleStackSlot()) {
    Exchange(loc1.GetStackIndex(), loc2.GetStackIndex(), true);
  } else {
...
void ParallelMoveResolverMIPS::RestoreScratch(int reg) {
  // Pop reg.
  __ Lw(Register(reg), SP, 0);
  __ DecreaseFrameSize(kMipsWordSize);
}

void ParallelMoveResolverMIPS::SpillScratch(int reg) {
  // Push reg.
  __ IncreaseFrameSize(kMipsWordSize);
  __ Sw(Register(reg), SP, 0);
}
...
void ParallelMoveResolverMIPS::Exchange(int index1, int index2, bool double_slot) {
  ...
  for (int i = 0; i <= static_cast<int>(double_slot); i++, stack_offset += kMipsWordSize) {
...
void CodeGeneratorMIPS::LoadCurrentMethod(Register current_method) {
  DCHECK(RequiresCurrentMethod());
  __ Lw(current_method, SP, kCurrentMethodStackOffset);
}
...
void CodeGeneratorMIPS::InvokeRuntime(int32_t entry_point_offset,
  ...
  if (is_direct_entrypoint) {
    __ IncreaseFrameSize(16);
  }
  __ LoadFromOffset(kLoadWord, T9, TR, entry_point_offset);
  __ Jalr(T9);
  __ Nop();
  if (is_direct_entrypoint) {
    __ DecreaseFrameSize(16);
  }
...
void InstructionCodeGeneratorMIPS::VisitCompare(HCompare* instruction) {
  ...
  switch (in_type) {
    case Primitive::kPrimLong: {
      ...
      // Note: implementation same as in quick, refer to MipsMir2Lir::GenCmpLong for details.
...
void InstructionCodeGeneratorMIPS::VisitMonitorOperation(HMonitorOperation* instruction) {
  uint32_t quick_entrypoint;
  bool direct;
  if (instruction->IsEnter()) {
    quick_entrypoint = QUICK_ENTRY_POINT(pLockObject);
    direct = IsDirectEntrypoint(kQuickLockObject);

  } else {
    quick_entrypoint = QUICK_ENTRY_POINT(pUnlockObject);
    direct = IsDirectEntrypoint(kQuickUnlockObject);
  }
  codegen_->InvokeRuntime(quick_entrypoint,
                          instruction,
                          instruction->GetDexPc(),
                          nullptr,
                          direct);
  CheckEntrypointTypes<kQuickLockObject, void, mirror::Object*>();  // TODO: split lock/unlock?
}
...
void InstructionCodeGeneratorMIPS::VisitRem(HRem* instruction) {
  ...
  switch (type) {
    ...
    case Primitive::kPrimDouble: {
      ...
      bool direct = (type == Primitive::kPrimFloat) ? IsDirectEntrypoint(kQuickFmodf)
                                                    : IsDirectEntrypoint(kQuickFmod);
      codegen_->InvokeRuntime(entry_offset, instruction, instruction->GetDexPc(), nullptr, direct);
      break;
    }
    default:
      LOG(FATAL) << "Unexpected rem type " << type;
...
void LocationsBuilderMIPS::VisitTypeConversion(HTypeConversion* conversion) {
  ...
  if (call_kind == LocationSummary::kNoCall) {
    ...
    if (Primitive::IsFloatingPointType(input_type)) {
      ...
      // Here if input_type is kPrimLong.
      locations->SetInAt(0, Location::RegisterPairLocation(
                 calling_convention.GetRegisterAt(0), calling_convention.GetRegisterAt(1)));
    }
