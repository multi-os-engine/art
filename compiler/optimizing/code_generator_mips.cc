...
class ReadBarrierMarkSlowPathMIPS : public SlowPathCodeMIPS {
 public:
  ReadBarrierMarkSlowPathMIPS(HInstruction* instruction, Location obj)
      : SlowPathCodeMIPS(instruction), obj_(obj) {
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    //
    // Instead of using the standard runtime calling convention (input in A0
    // and output in V0):
    //
    //   A0 <- obj
    ...
    int32_t entry_point_offset =
        CodeGenerator::GetReadBarrierMarkEntryPointsOffset<kMipsWordSize>(reg);
...
void InstructionCodeGeneratorMIPS::GenerateGcRootFieldLoad(HInstruction* instruction,
  ...
  if (kEmitCompilerReadBarrier) {
    if (kUseBakerReadBarrier) {
      ...
      SlowPathCodeMIPS* slow_path =
          new (GetGraph()->GetArena()) ReadBarrierMarkSlowPathMIPS(instruction, root);
      codegen_->AddSlowPath(slow_path);
