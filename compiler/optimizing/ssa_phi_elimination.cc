...
void SsaRedundantPhiElimination::Run() {
  ...
  ArenaBitVector visited_phis_in_cycle(
      graph_->GetArena(), graph_->GetCurrentInstructionId(), false);
  ...
  while (!worklist_.empty()) {
    ...
    if (candidate == nullptr) {
      for (size_t i = 0; i < cycle_worklist.size(); ++i) {
        ...
        DCHECK(!current->IsLoopHeaderPhi() || current->GetBlock()->IsLoopPreHeaderFirstPredecessor());
