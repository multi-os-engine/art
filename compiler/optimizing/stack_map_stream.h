...
// Helper to build art::StackMapStream::DexRegisterLocationCatalogEntriesIndices.
class DexRegisterLocationCatalogEntriesIndicesEmptyFn {
 public:
...
// Hash function for art::StackMapStream::DexRegisterLocationCatalogEntriesIndices.
...
class StackMapStream : public ValueObject {
 public:
  explicit StackMapStream(ArenaAllocator* allocator)
      : allocator_(allocator),
        ...
        dex_register_location_catalog_entries_(allocator, 4),
  ...
  size_t ComputeDexRegisterLocationCatalogSize() const {
    ...
    for (size_t location_catalog_entry_index = 0;
         location_catalog_entry_index < dex_register_location_catalog_entries_.Size();
         ++location_catalog_entry_index) {
      DexRegisterLocation dex_register_location =
          dex_register_location_catalog_entries_.Get(location_catalog_entry_index);
  ...
  size_t ComputeDexRegisterMapSize(const StackMapEntry& entry) const {
    ...
    size_t map_entries_size_in_bits =
        DexRegisterMap::SingleEntrySizeInBits(dex_register_location_catalog_entries_.Size())
        * number_of_live_dex_registers;
  ...
  void FillIn(MemoryRegion region) {
    ...
    code_info.SetNumberOfDexRegisterLocationCatalogEntries(
        dex_register_location_catalog_entries_.Size());
    ...
    for (size_t i = 0, e = dex_register_location_catalog_entries_.Size(); i < e; ++i) {
      DexRegisterLocation dex_register_location = dex_register_location_catalog_entries_.Get(i);
  ...
  void AddDexRegisterEntry(uint16_t dex_register, DexRegisterLocation::Kind kind, int32_t value) {
    ...
    if (kind != DexRegisterLocation::Kind::kNone) {
      ...
      // if it is already in the location catalog.  If not, insert it (in
      // the location catalog and the hash map) and use the newly created
      // index.
      auto it = dex_register_location_catalog_entries_indices_.Find(location);
      if (it != dex_register_location_catalog_entries_indices_.end()) {
        ...
        size_t index = dex_register_location_catalog_entries_.Size();
        dex_register_location_catalog_entries_.Add(location);
        dex_register_locations_.Add(index);
        dex_register_location_catalog_entries_indices_.Insert(std::make_pair(location, index));
      }
 ...
 private:
  ...
  GrowableArray<DexRegisterLocation> dex_register_location_catalog_entries_;
  // Map from Dex register location catalog entries to their indices in the
  // location catalog.
  typedef HashMap<DexRegisterLocation, size_t, DexRegisterLocationCatalogEntriesIndicesEmptyFn,
                  DexRegisterLocationHashFn> DexRegisterLocationCatalogEntriesIndices;
  DexRegisterLocationCatalogEntriesIndices dex_register_location_catalog_entries_indices_;

  // A set of concatenated maps of Dex register locations indices to
  // `dex_register_location_catalog_entries_`.
