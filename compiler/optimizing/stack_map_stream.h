...
// Helper to build art::StackMapStream::DexRegisterLocationSetEntriesIndices.
class DexRegisterLocationSetEntriesIndicesEmptyFn {
 public:
...
// Hash function for art::StackMapStream::DexRegisterLocationSetEntriesIndices.
...
class StackMapStream : public ValueObject {
 public:
  explicit StackMapStream(ArenaAllocator* allocator)
      : allocator_(allocator),
        ...
        dex_register_location_set_entries_(allocator, 4),
  ...
  size_t ComputeNeededSize() {
    size_t size = CodeInfo::kFixedSize
        + ComputeDexRegisterLocationSetSize()
  ...
  // Compute the size of the Dex register location set of `entry`.
  size_t ComputeDexRegisterLocationSetSize() const {
    size_t size = DexRegisterLocationSet::kFixedSize;
    for (size_t location_set_entry_index = 0;
         location_set_entry_index < dex_register_location_set_entries_.Size();
         ++location_set_entry_index) {
      DexRegisterLocation dex_register_location =
          dex_register_location_set_entries_.Get(location_set_entry_index);
      size += DexRegisterLocationSet::EntrySize(dex_register_location);
    }
    return size;
  }
  ...
  size_t ComputeDexRegisterMapSize(const StackMapEntry& entry) const {
    ...
    size_t map_entries_size_in_bits =
        DexRegisterMap::SingleEntrySizeInBits(dex_register_location_set_entries_.Size())
        * number_of_live_dex_registers;
  ...
  size_t ComputeDexRegisterLocationSetStart() const {
    return CodeInfo::kFixedSize;
  }

  size_t ComputeStackMapsStart() const {
    return ComputeDexRegisterLocationSetStart() + ComputeDexRegisterLocationSetSize();
  }
  ...
  void FillIn(MemoryRegion region) {
    ...
    // Set the Dex register location set.
    code_info.SetNumberOfDexRegisterLocationSetEntries(dex_register_location_set_entries_.Size());
    MemoryRegion dex_register_location_set_region = region.Subregion(
        ComputeDexRegisterLocationSetStart(),
        ComputeDexRegisterLocationSetSize());
    DexRegisterLocationSet dex_register_location_set(dex_register_location_set_region);
    // Offset in `dex_register_location_set` where to store the next
    // register location.
    size_t location_set_offset = DexRegisterLocationSet::kFixedSize;
    for (size_t i = 0, e = dex_register_location_set_entries_.Size(); i < e; ++i) {
      DexRegisterLocation dex_register_location = dex_register_location_set_entries_.Get(i);
      dex_register_location_set.SetRegisterInfo(location_set_offset, dex_register_location);
      location_set_offset += DexRegisterLocationSet::EntrySize(dex_register_location);
    }
    // Ensure we reached the end of the Dex registers location_set.
    DCHECK_EQ(location_set_offset, dex_register_location_set_region.size());
        ...
        if (entry_with_same_map != kNoSameDexMapFound) {
          ...
          for (size_t dex_register_number = 0, index_in_dex_register_locations = 0;
            ...
            if (entry.live_dex_registers_mask->IsBitSet(dex_register_number)) {
              ...
              dex_register_map.SetLocationSetEntryIndex(index_in_dex_register_locations,
                                                        location_set_entry_index,
                                                        entry.num_dex_registers,
                                                        dex_register_location_set_entries_.Size());
              ++index_in_dex_register_locations;
            }
          }
        }
  ...
  void AddDexRegisterEntry(uint16_t dex_register, DexRegisterLocation::Kind kind, int32_t value) {
    ...
    if (kind != DexRegisterLocation::Kind::kNone) {
      ...
      // Look for Dex register `location` in the location set (using the
      // companion hash map of locations to indices) and use its index
      // if it is already in the location set.  If not, insert it (in
      // the location set and the hash map) and use the newly created
      // index.
      auto it = dex_register_location_set_entries_indices_.Find(location);
      if (it != dex_register_location_set_entries_indices_.end()) {
        // Retrieve the index from the hash map.
        dex_register_locations_.Add(it->second);
      } else {
        // Create a new entry in the location set and the hash map.
        size_t index = dex_register_location_set_entries_.Size();
        dex_register_location_set_entries_.Add(location);
        dex_register_locations_.Add(index);
        dex_register_location_set_entries_indices_.Insert(std::make_pair(location, index));
      }
 ...
 private:
  ...
  // A location set of unique [location_kind, register_value] pairs (per method).
  GrowableArray<DexRegisterLocation> dex_register_location_set_entries_;
  // Map from Dex register location set entries to their indices in the
  // location set.
  typedef HashMap<DexRegisterLocation, size_t, DexRegisterLocationSetEntriesIndicesEmptyFn,
                  DexRegisterLocationHashFn> DexRegisterLocationSetEntriesIndices;
  DexRegisterLocationSetEntriesIndices dex_register_location_set_entries_indices_;

  // A set of concatenated maps of Dex register locations indices to
  // `dex_register_location_set_entries_`.
