...
class StackMapStream : public ValueObject {
 public:
  explicit StackMapStream(ArenaAllocator* allocator)
      : stack_maps_(allocator, 10),
        ...
        dex_register_tables_(allocator, 10 * 4),
  ...
  void AddStackMapEntry(uint32_t dex_pc,
    ...
    switch (dex_register_map_encoding) {
      ...
      case kDexRegisterLocationDictionary:
        entry.dex_register_maps_start_index = dex_register_tables_.Size();
        break;
    }
  ...
  void AddDexRegisterEntry(DexRegisterMap::LocationKind kind, int32_t value) {
    ...
    switch (dex_register_map_encoding) {
      ...
      case kDexRegisterLocationDictionary: {
        ...
        dex_register_tables_.Add(index);
        break;
      }
    }
  }
  ...
  size_t ComputeNeededSize() const {
    ...
    switch (dex_register_map_encoding) {
      ...
      case kDexRegisterLocationDictionary:
        return common_size + ComputeDexRegisterDictionarySize();
    }
  }
  ...
  static DexRegisterCompressedMap::LocationKind ComputeCompressedMapLocationKind(const DexRegisterEntry& entry) {
    ...
    switch (entry.kind) {
      ...
      case DexRegisterMap::kInStack:
        ...
        return IsUint<DexRegisterCompressedMap::kValueBits>(entry.value / kFrameSlotSize)
            ...
            : DexRegisterCompressedMap::kInStackLargeOffset;
      ...
      case DexRegisterMap::kConstant:
        return IsUint<DexRegisterCompressedMap::kValueBits>(entry.value)
            ...
            : DexRegisterCompressedMap::kConstantBigValue;
  ...
  static size_t ComputeEntrySizeAsCompressedLocation(const DexRegisterEntry& entry) {
    DCHECK(dex_register_map_encoding == kDexRegisterCompressedLocationList);
    // TODO: Directly pass `entry` when
    // DexRegisterCompressedMap::DexRegisterLocation replaces
    // DexRegisterEntry.
    //
    // Note that this is bit hacky at the moment, as DexRegisterEntry
    // contains a DexRegisterMap::LocationKind, while
    // DexRegisterCompressedMap::DexRegisterLocation contains a
    // DexRegisterCompressedMap::LocationKind (hence the static_cast).
    DexRegisterCompressedMap::DexRegisterLocation location = {
      static_cast<DexRegisterCompressedMap::LocationKind>(entry.kind),
      entry.value
    };
    return DexRegisterCompressedMap::EntrySize(location);
  }
    ...
    switch (dex_register_map_encoding) {
      ...
      case kDexRegisterLocationDictionary:
        return stack_maps_.Size() * DexRegisterTable::kFixedSize
            // For each dex register entry.
            + dex_register_tables_.Size() * DexRegisterTable::SingleEntrySize();
  ...
  void FillIn(MemoryRegion region) {
    ...
    switch (dex_register_map_encoding) {
      case kDexRegisterLocationList:
      case kDexRegisterCompressedLocationList:
        DCHECK(dex_register_dictionary_.IsEmpty());
        DCHECK(dex_register_tables_.IsEmpty());
        break;

      case kDexRegisterLocationDictionary:
        DCHECK(dex_register_maps_.IsEmpty());
        break;
      }
    }
    ...
    if (dex_register_map_encoding == kDexRegisterLocationDictionary) {
      ...
      for (size_t i = 0, e = dex_register_dictionary_.Size(); i < e; ++i) {
          ...
          case kDexRegisterLocationDictionary: {
            ...
            for (size_t j = 0; j < entry.num_dex_registers; ++j) {
              DexRegisterMapIndex entry_index =
                  dex_register_tables_.Get(entry.dex_register_maps_start_index + j);
 ...
 private:
  ...
  // Data structure for Dex register locations used when the encoding is
  // list-based.
  GrowableArray<DexRegisterEntry> dex_register_maps_;

  // Data structures for Dex register locations used when the encoding is
  // dictionary-based.
  //
  ...
  GrowableArray<DexRegisterMapIndex> dex_register_tables_;
