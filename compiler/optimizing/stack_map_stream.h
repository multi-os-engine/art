...
class StackMapStream : public ValueObject {
 public:
  ...
  void AddStackMapEntry(uint32_t dex_pc,
    ...
    entry.live_dex_reg_mask = live_dex_reg_mask;
  ...
  size_t ComputeDexRegisterMapSize() const {
    ...
    for (size_t i = 0, e = stack_maps_.Size(); i < e; ++i) {
      ...
      total += DexRegisterMap::ComputeNeededSize(entry.num_dex_registers, *entry.live_dex_reg_mask);
    }
    return total;
  }
    ...
    for (size_t i = 0, e = stack_maps_.Size(); i < e; ++i) {
      ...
      if (entry.num_dex_registers != 0) {
        ...
        MemoryRegion register_region = dex_register_maps_region.Subregion(
            ...
            DexRegisterMap::ComputeNeededSize(entry.num_dex_registers, *entry.live_dex_reg_mask));
        next_dex_register_map_offset += register_region.size();
        DexRegisterMap dex_register_map =
            DexRegisterMap::Create(register_region,
                                   entry.num_dex_registers,
                                   *entry.live_dex_reg_mask);
        stack_map.SetDexRegisterMapOffset(register_region.start() - memory_start);

        for (size_t j = 0, k = 0; j < entry.num_dex_registers; ++j) {
          if (!entry.live_dex_reg_mask->IsBitSet(j)) {
            continue;
          }
          DexRegisterEntry register_entry =
              dex_register_maps_.Get(k + entry.dex_register_maps_start_index);
          dex_register_map.SetRegisterInfo(k, register_entry.kind, register_entry.value);
          k++;
        }
      } else {
