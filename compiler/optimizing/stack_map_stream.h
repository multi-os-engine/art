  ...
  static DexRegisterLocation::Kind GetCompressedMapLocationKind(const DexRegisterLocation& entry) {
    switch (entry.kind) {
      ...
      case DexRegisterLocation::Kind::kInRegister:
        DCHECK_GE(entry.value, 0);
        DCHECK_LT(entry.value, 1 << DexRegisterMap::kValueBits);
      ...
      case DexRegisterLocation::Kind::kInStack:
        ...
        return IsUint<DexRegisterMap::kValueBits>(entry.value / kFrameSlotSize)
            ? DexRegisterLocation::Kind::kInStack
            : DexRegisterLocation::Kind::kInStackLargeOffset;

      case DexRegisterLocation::Kind::kConstant:
        return IsUint<DexRegisterMap::kValueBits>(entry.value)
    ...
    for (size_t i = 0, e = stack_maps_.Size(); i < e; ++i) {
      ...
      if (entry.num_dex_registers != 0) {
        ...
        for (size_t j = 0; j < entry.num_dex_registers; ++j) {
          ...
          dex_register_map.SetRegisterInfo(offset, compressed_map_kind, register_entry.value);
