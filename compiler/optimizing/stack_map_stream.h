...
class StackMapStream : public ValueObject {
 public:
  ...
  void AddStackMapEntry(uint32_t dex_pc,
    ...
    native_pc_max_ = std::max(native_pc_max_, native_pc_offset);
  }
  ...
  size_t ComputeStackMapsSize() const {
    return stack_maps_.Size() * StackMap::ComputeStackMapSize(ComputeStackMaskSize(), ComputeInlineInfoSize(), ComputeDexRegisterMapsSize(), dex_pc_max_, native_pc_max_);
  }
  ...
  void FillIn(MemoryRegion region) {
    ...
    code_info.SetEncoding(inline_info_size, dex_register_map_size, dex_pc_max_, native_pc_max_);
    ...
    for (size_t i = 0, e = stack_maps_.Size(); i < e; ++i) {
      ...
      if (entry.num_dex_registers != 0) {
        ...
        stack_map.SetDexRegisterMapOffset(code_info, register_region.start() - dex_register_locations_region.start());
      ...
      if (entry.inlining_depth != 0) {
        ...
        stack_map.SetInlineDescriptorOffset(code_info, inline_region.start() - dex_register_locations_region.start());
