...
class InductionVarAnalysisTest : public testing::Test {
 public:
  ...
  // Inserts an array store with given local as subscript at depth d.
  // This enables tests to inspect the computed induction easily.
...
TEST_F(InductionVarAnalysisTest, FindBasicInductionVar) {
  ...
  // Classification of the increment itself.
  EXPECT_STREQ(
...
TEST_F(InductionVarAnalysisTest, FindDeepLoopInduction) {
  ...
  HInstruction *add = InsertInstruction(
      new (&allocator_) HAdd(
          Primitive::kPrimInt, constant100_, InsertLocalLoad(induc_, 9)), 9);
  InsertLocalStore(induc_, add, 9);
  HInstruction* store = InsertArrayStore(induc_, 9);
  PerformInductionVarAnalysis();

  // TODO: make less picky on phi number?
  EXPECT_STREQ(
       "((3:Constant) * i + ((3:Constant) + (165:Phi)))",
       iva_->InductionToString(GetLoopInfo(9), store->InputAt(1)).c_str());

  // Classification of the nested increments themselves.
  for (int d = 0; d < 10; d++) {
    EXPECT_STREQ(
        "((2:Constant) * i + ((1:Constant) + (2:Constant)))",
