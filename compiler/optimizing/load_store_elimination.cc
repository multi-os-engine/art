...
class ReferenceInfo : public ArenaObject<kArenaAllocMisc> {
 public:
  ReferenceInfo(HInstruction* reference, size_t pos) : ref_(reference), pos_(pos) {
    is_singleton_ = true;
    ...
    for (HUseIterator<HInstruction*> use_it(ref_->GetUses());
      ...
      if (use->IsPhi() || use->IsInvoke() ||
          ...
          (use->IsUnresolvedInstanceFieldSet() && (ref_ == use->InputAt(1))) ||
          (use->IsStaticFieldSet() && (ref_ == use->InputAt(1))) ||
          (use->IsUnresolvedStaticFieldSet() && (ref_ == use->InputAt(1))) ||
  ...
  // Returns true if ref_ is a singleton and not returned to the caller.
 ...
 private:
  ...
  bool is_singleton_and_not_returned_;  // ref_ is singleton and not returned to caller.
};
...
class HeapLocationCollector : public HGraphVisitor {
 public:
  ...
  static constexpr uint32_t kInitialBitVectorSize = 9;  // Good for 5 heap locations.
  ...
  explicit HeapLocationCollector(HGraph* graph)
      : HGraphVisitor(graph),
        ...
        aliasing_matrix_(graph->GetArena(), kInitialBitVectorSize, true),
 ...
 private:
  ...
  ReferenceInfo* UpdateReferenceInfo(HInstruction* ref) {
  ...
  HeapLocation* UpdateHeapLocations(HInstruction* ref,
    ...
    ReferenceInfo* ref_info = UpdateReferenceInfo(original_ref);
  ...
  void VisitFieldAccess(HInstruction* field_access,
    ...
    HeapLocation* location = UpdateHeapLocations(ref, offset, nullptr, declaring_class_def_index);
    if (is_store &&
  ...
  // since we can't accurately track the fields.
  ...
  void VisitNewInstance(HNewInstance* new_instance) OVERRIDE {
    ...
    UpdateReferenceInfo(new_instance);
  }
...
class LSEVisitor : public HGraphVisitor {
 ...
 private:
  ...
  bool Equal(HInstruction* heap_value, HInstruction* value) {
    ...
    return GetDefaultValue(value->GetType()) == value;
  }
