...
class ReferenceInfo : public ArenaObject<kArenaAllocMisc> {
 public:
  ReferenceInfo(HInstruction* reference, size_t pos)
      : ref_(reference), pos_(pos) {
    is_singleton_ = true;
    is_returned_ = false;
    if (!ref_->IsNewInstance() && !ref_->IsNewArray()) {
  ...
  // The allocation and stores into ref_ may be eliminated for such cases.
 ...
 private:
  ...
  size_t pos_;                // position in HeapLocationCollector's ref_info_array_.
...
class HeapLocation : public ArenaObject<kArenaAllocMisc> {
 ...
 private:
  ...
  const bool is_final_;                // whether the field is final.
...
class HeapLocationCollector : public HGraphVisitor {
 public:
  ...
  explicit HeapLocationCollector(HGraph* graph)
      : HGraphVisitor(graph),
        ref_info_array_(graph->GetArena(), 0),
        heap_locations_(graph->GetArena(), 0),
        aliasing_matrix_(graph->GetArena(), 9, true),
        has_heap_stores_(false),
        has_volatile_(false),
        has_monitor_ops_(false),
 ...
 private:
  // An allocation cannot alias with a name which already pre-exists, such
  ...
  // Compute if two locations may alias to each other. The result is saved
  // in a matrix represented as a BitVector.
  bool ComputeMayAlias(size_t index1, size_t index2) const {
    HeapLocation* loc1 = heap_locations_.Get(index1);
    HeapLocation* loc2 = heap_locations_.Get(index2);
  ...
  HeapLocation* UpdateHeapLocations(HInstruction* ref,
                                    size_t offset,
                                    HInstruction* index,
                                    int16_t declaring_class_def_index,
                                    bool is_final) {
    ref = HuntForOriginalReference(ref);
    ...
    size_t heap_location_id = FindHeapLocationIndex(
    ...
    if (heap_location_id == kHeapLocationNotFound) {
      HeapLocation* heap_loc = new (GetGraph()->GetArena())
          HeapLocation(ref_info, offset, index, declaring_class_def_index, is_final);
  ...
  void VisitFieldAccess(HInstruction* field_access,
    ...
    HeapLocation* location = UpdateHeapLocations(
        ref, offset, nullptr, declaring_class_def_index, field_info.IsFinal());
    if (is_store &&
  ...
  void VisitArrayAccess(HInstruction* array, HInstruction* index) {
    UpdateHeapLocations(array, HeapLocation::kInvalidFieldOffset,
        index, HeapLocation::kDeclaringClassDefIndexForArrays, false);
  }
  ...
  void VisitInvokeStaticOrDirect(HInvokeStaticOrDirect* invoke ATTRIBUTE_UNUSED) OVERRIDE {
    may_deoptimize_ = true;
  }

  void VisitInvokeVirtual(HInvokeVirtual* invoke ATTRIBUTE_UNUSED) OVERRIDE {
    may_deoptimize_ = true;
  }
  ...
  GrowableArray<ReferenceInfo*> ref_info_array_;   // All references used for heap accesses.
  GrowableArray<HeapLocation*> heap_locations_;    // All heap locations.
...
class LSEVisitor : public HGraphVisitor {
    ...
    if (block->IsLoopHeader()) {
      ...
      if (side_effects_.GetLoopEffects(block).DoesAnyWrite()) {
        ...
        for (size_t i = 0; i < heap_values->Size(); i++) {
          // Don't kill a singleton's value if the value cannot become
 ...
 private:
  ...
  void VisitGetLocation(HInstruction* instruction,
                        ...
                        int16_t declaring_class_def_index) {
    ref = HuntForOriginalReference(ref);
    ReferenceInfo* ref_info = heap_location_collector_.FindReferenceInfoOf(ref);
    ...
    HInstruction* heap_value = heap_values->Get(idx);
    ...
    if (heap_value == kUnknownHeapValue) {
      ...
      heap_values->Put(idx, instruction);
    }
  }
  ...
  void VisitSetLocation(HInstruction* instruction,
    ...
    heap_values->Put(idx, value);
    for (size_t i = 0; i < heap_values->Size(); i++) {
      if (heap_values->Get(i) == value ||
          heap_values->Get(i) == kUnknownHeapValue) {
        continue;
      }
      if (heap_values->Get(i) != kDefaultHeapValue &&
          heap_location_collector_.GetHeapLocation(i)->IsFinal()) {
        // Once a final field is set, it cannot be set again.
        continue;
      }
  ...
  void HandleInvoke(HInstruction* invoke) {
    ...
    for (size_t i = 0; i < heap_values->Size(); i++) {
      ...
      if (!heap_location_collector_.GetHeapLocation(i)->IsFinal() ||
          heap_values->Get(i) == kDefaultHeapValue) {
        // A final field cannot be set again once it's set.
        heap_values->Put(i, kUnknownHeapValue);
      }
    }
  }
