...
static void GenBitCount(LocationSummary* locations,
  ...
  // to a(n unrolled) loop based algorithm which required 38 instructions.
  //
  ...
  if (type == Primitive::kPrimInt) {
    ...
    __ MuluR6(out, out, TMP);
    __ Srl(out, out, 24);
  } else if (type == Primitive::kPrimLong) {
    __ Dsrl(TMP, in, 1);

    // LoadConst64(AT, 0x5555555555555555);
    __ LoadConst32(AT, 0x55555555);
    ...
    // LoadConst64(AT, 0x3333333333333333);
    __ LoadConst32(AT, 0x33333333);
    __ Dinsu(AT, AT, 32, 32);
    ...
    // LoadConst64(AT, 0x0F0F0F0F0F0F0F0F);
    __ LoadConst32(AT, 0x0F0F0F0F);
    __ Dinsu(AT, AT, 32, 32);

    __ And(out, out, AT);

    // LoadConst64(AT, 0x0101010101010101);
    __ LoadConst32(TMP, 0x01010101);
    __ Dinsu(TMP, TMP, 32, 32);
