...
#ifndef ART_COMPILER_OPTIMIZING_COMMON_DOMINATOR_FINDER_H_
#define ART_COMPILER_OPTIMIZING_COMMON_DOMINATOR_FINDER_H_

#include "nodes.h"

namespace art {

class CommonDominatorFinder {
 public:
  // Convenience function to find the common dominator of 2 blocks.
  static HBasicBlock* CommonDominator(HBasicBlock* block1, HBasicBlock* block2) {
    CommonDominatorFinder finder(block1);
    finder.Update(block2);
    return finder.CommonDominator();
  }

  // Create a finder starting with a given block.
  // Note: Attribute memory allocation to code generator.
  explicit CommonDominatorFinder(HBasicBlock* block)
  ...
  HBasicBlock* CommonDominator() const {
    return dominator_;
  }
...
#endif  // ART_COMPILER_OPTIMIZING_COMMON_DOMINATOR_FINDER_H_
