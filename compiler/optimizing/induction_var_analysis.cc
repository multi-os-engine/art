...
void HInductionVarAnalysis::ClassifyNonTrivial(HLoopInformation* loop) {
  ...
  if (it != cycle_.end()) {
    ...
    if (it->second->induction_class == kInvariant) {
      AssignInfo(loop, phi, NewInduction(kLinear, it->second, initial));
      for (size_t i = 0; i < size - 1; i++) {
        ClassifyTrivial(loop, scc_[i]);
      }
    } else if (it->second->induction_class == kPeriodic) {
      InductionInfo* seq = it->second;
      for (size_t i = 2; i <= size; i++) {
        AssignInfo(loop, scc_[size - i], seq);
        seq = PeriodToEnd(seq->op_b, seq->op_a);
      }
      AssignInfo(loop, phi, seq);
    }
  }
}

HInductionVarAnalysis::InductionInfo* HInductionVarAnalysis::PeriodToEnd(InductionInfo* seq, InductionInfo* last) {
  if (seq->induction_class == kInvariant) {
    return NewInduction(kPeriodic, seq, last);
  }
  return NewInduction(kPeriodic, seq->op_a, PeriodToEnd(seq->op_b, last));
}
...
HInductionVarAnalysis::InductionInfo* HInductionVarAnalysis::TransferShl(InductionInfo* a,
  ...
  if (a != nullptr && b != nullptr && b->induction_class == kInvariant && b->operation == kFetch) {
    ...
    if (IsInt(b->fetch, &value) && 0 <= value && value < 31) {
      ...
      return TransferMul(a, NewInvariantFetch(new_fetch));
    }
  }
  return nullptr;
}
