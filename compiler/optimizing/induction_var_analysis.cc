 ...
 * (referred to as mu-operation in the Gerlek paper).
 */
...
void HInductionVarAnalysis::VisitNode(HLoopInformation* loop,
  ...
  const int d1 = ++global_depth_;
  map_.Overwrite(id, NodeInfo(kOnStack, d1));
  ...
  if (low < d1) {
    ...
    while (!stack_.empty()) {
      ...
      scc_.push_back(x);
      stack_.pop_back();
      map_.find(x->GetId())->second.state = kDone;
      if (x == instruction) {
        break;
      }
    }  // while stack
...
int HInductionVarAnalysis::VisitDescendant(HLoopInformation* loop,
  ...
  if (!IsVisitedNode(id)) {
    ...
    return it->second.state == kOnStack ? it->second.depth : global_depth_;
  }
}
...
void HInductionVarAnalysis::ClassifyNonTrivial(HLoopInformation* loop) {
  ...
  // Inspect remainder of the cycle that resides in scc_. The cycle_
  // mapping assigns a temporary meaning to its nodes.
  ...
  for (size_t i = 0; i < size - 1; i++) {
    ...
    if (operation->IsPhi()) {
      ...
      update = TransferCycleOverAddSub(loop,
                                       operation->InputAt(0),
                                       operation->InputAt(1), kAdd);
      if (update == nullptr) {  // try other way around
        update = TransferCycleOverAddSub(loop,
                                         operation->InputAt(1),
                                         operation->InputAt(0), kAdd);
      }
    } else if (operation->IsSub()) {
      update = TransferCycleOverAddSub(loop,
                                       operation->InputAt(0),
                                       operation->InputAt(1), kSub);
    }
    if (update == nullptr) {
      return;
    }
...
HInductionVarAnalysis::TransferAddSub(InductionInfo* a,
                                      InductionInfo* b,
                                      InductionOp op) {
  // Transfer over an addition or subtraction.
  if (a != nullptr && b != nullptr) {
    if (a->induction_class == kInvariant &&
        b->induction_class == kInvariant) {
...
HInductionVarAnalysis::TransferMul(InductionInfo* a, InductionInfo* b) {
  // Transfer over a multiplication.
  if (a != nullptr && b != nullptr) {
    if (a->induction_class == kInvariant &&
        b->induction_class == kInvariant) {
...
HInductionVarAnalysis::TransferNeg(InductionInfo* a) {
  // Transfer over an unary negation.
  if (a != nullptr) {
    if (a->induction_class == kInvariant) {
...
HInductionVarAnalysis::TransferCycleOverPhi(HInstruction* phi) {
  // Transfer within a cycle over a phi.
...
HInductionVarAnalysis::TransferCycleOverAddSub(HLoopInformation* loop,
                                               ...
                                               HInstruction* stride,
                                               InductionOp op) {
  // Transfer within a cycle over an addition or subtraction.
...
void HInductionVarAnalysis::PutInfo(int loopId, int id, InductionInfo* info) {
  auto it = induction_.find(loopId);
  if (it == induction_.end()) {
    it = induction_.Put(
        loopId,
        ArenaSafeMap<int, InductionInfo*>(
            std::less<int>(),
            graph_->GetArena()->Adapter()));
  }
  it->second.Overwrite(id, info);
}

HInductionVarAnalysis::InductionInfo* HInductionVarAnalysis::GetInfo(int loopId,
                                                                     int id) {
  auto it = induction_.find(loopId);
  if (it != induction_.end()) {
...
HInductionVarAnalysis::LookupInfo(HLoopInformation* loop,
  ...
  const int loopId = loop->GetHeader()->GetBlockId();
  const int id = instruction->GetId();
  InductionInfo* info = GetInfo(loopId, id);
  if (info == nullptr && IsLoopInvariant(loop, instruction)) {
    info = NewInductionInfo(
