...
void HInductionVarAnalysis::VisitTripCount(HLoopInformation* loop,
  ...
  InductionInfo* trip_count
      = CreateInvariantOp(kDiv, CreateInvariantOp(kSub, u_expr, l_expr), stride);
  ...
  switch (cmp) {
    case kCondLT:
      return stride_value == 1 ||
          (IsIntAndGet(u_expr, &value) && value <= (max - stride_value + 1));
    case kCondLE:
      return (IsIntAndGet(u_expr, &value) && value <= (max - stride_value));
    case kCondGT:
      return stride_value == -1 ||
          (IsIntAndGet(u_expr, &value) && value >= (min - stride_value - 1));
    case kCondGE:
