...
/**
 * Returns true if instruction provides proper mu.
 */
static bool IsMu(HLoopInformation* loop, HInstruction* instruction) {
  return instruction->IsPhi() && instruction->GetBlock() == loop->GetHeader()
}
...
void HInductionVarAnalysis::ClassifyNonTrivial(HLoopInformation* loop) {
  // Header phi at start with external link as initial value?
  const size_t size = scc_.size();
  CHECK(size >= 1);
  HInstruction* phi = scc_[size - 1];
  if (!IsMu(phi) || phi->InputCount() != 2) {
    return;
  }
  ...
  // Singleton phi may be wrap-around.
  if (size == 1) {
  ...
  // Inspect cycle of phi/add/sub with zero update for phi.
  cycle_.clear();
