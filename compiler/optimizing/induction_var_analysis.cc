...
void HInductionVarAnalysis::ClassifyOne(HLoopInformation* loop,
  ...
  // TODO: perform a pre-filter for efficiency

  // Perform lattice operations based on a "on-demand" classification.
  // TODO: inspect more operators
  if (instruction->IsPhi()) {
    const size_t count = instruction->InputCount();
    if (count > 0) {
      InductionInfo *first = LookupInfo(loop, instruction->InputAt(0));
      if (first != nullptr) {
        for (size_t i = 1; i < count; i++) {
          if (!InductionEqual(first, LookupInfo(loop, instruction->InputAt(i))))
            return;
        }
        info = first;
      }
...
HInductionVarAnalysis::LookupInfo(HLoopInformation* loop,
  ...
  if (IsLoopInvariant(loop, instruction)) {
    InductionInfo* info = NewInductionInfo(
        kInvariant, kNop, nullptr, nullptr, instruction);
    induction_[loopId][id] = info;
    return info;
  }
  return nullptr;
...
bool HInductionVarAnalysis::InductionEqual(InductionInfo* info1,
  ...
  if (info1 != nullptr && info2 != nullptr) {
    return
        info1->class_ == info2->class_ &&
        info1->op_    == info2->op_    &&
        info1->i_     == info2->i_     &&
        InductionEqual(info1->a_, info2->a_) &&
        InductionEqual(info1->b_, info2->b_);
  }
  return info1 == info2;
}
