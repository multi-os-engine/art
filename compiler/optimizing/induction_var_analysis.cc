    ...
    switch (induction->induction_class) {
      ...
      case kPeriodic:
        ...
        for (size_t i = 2; i <= size; i++) {
          ...
          induction = PeriodToEnd(induction->op_b, induction->op_a);
        }
...
HInductionVarAnalysis::InductionInfo* HInductionVarAnalysis::PeriodToEnd(InductionInfo* seq,
                                                                         InductionInfo* last) {
...
HInductionVarAnalysis::InductionInfo* HInductionVarAnalysis::TransferShl(InductionInfo* a,
  ...
  if (a != nullptr && b != nullptr && b->induction_class == kInvariant && b->operation == kFetch) {
    ...
    if (IsInt(b->fetch, &value) && 0 <= value && value < 31) {
      ...
      HInstruction* new_fetch = graph_->GetIntConstant(1 << value);
