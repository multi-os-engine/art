...
void HInductionVarAnalysis::ClassifyNonTrivial(HLoopInformation* loop) {
  ...
  for (size_t i = 1; i < size; i++) {
    ...
    if (instruction->IsPhi()) {
      update = SolvePhi(loop, phi, instruction, /* input_index */ 0);
    } else if (instruction->IsAdd()) {
      update = SolveAddSub(
  ...
  InductionInfo* induction = SolvePhi(loop, phi, phi, /* input_index */ 1);
...
HInductionVarAnalysis::InductionInfo* HInductionVarAnalysis::SolvePhi(HLoopInformation* loop,
  ...
  // If all inputs are considered, try to solve within a tight cycle that is
  // formed by exactly two entry-phi instructions.
  if (input_index == 0 && phi-> IsLoopHeaderPhi() && phi->InputCount() == 2) {
