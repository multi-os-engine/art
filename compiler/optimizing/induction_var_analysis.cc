...
void HInductionVarAnalysis::VisitNode(HLoopInformation* loop, HInstruction* instruction) {
  ...
  for (const HUserRecord<HInstruction*>& input_record : instruction->GetInputRecords()) {
    low = std::min(low, VisitDescendant(loop, input_record.GetInstruction()));
  }
...
HInductionVarAnalysis::InductionInfo* HInductionVarAnalysis::TransferPhi(HLoopInformation* loop,
  ...
  DCHECK_LT(input_index, input_records.size());
  InductionInfo* a = LookupInfo(loop, input_records[input_index].GetInstruction());
  for (size_t i = input_index + 1; i < input_records.size(); i++) {
    InductionInfo* b = LookupInfo(loop, input_records[i].GetInstruction());
...
HInductionVarAnalysis::InductionInfo* HInductionVarAnalysis::SolvePhi(HInstruction* phi,
  ...
  ArrayRef<HUserRecord<HInstruction*>> input_records = phi->GetInputRecords();
  DCHECK_LT(input_index, input_records.size());
  auto ita = cycle_.find(input_records[input_index].GetInstruction());
  if (ita != cycle_.end()) {
    for (size_t i = input_index + 1; i < input_records.size(); i++) {
      auto itb = cycle_.find(input_records[i].GetInstruction());
      if (itb == cycle_.end() ||
