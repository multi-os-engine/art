...
HInductionVarAnalysis::InductionInfo* HInductionVarAnalysis::TransferShl(InductionInfo* a,
  ...
  if (a != nullptr && IsIntAndGet(b, &value)) {
    ...
    if ((tp == Primitive::kPrimInt  && 0 <= value && value < 31) ||
        (tp == Primitive::kPrimLong && 0 <= value && value < 63)) {
      return TransferMul(a, CreateConstant(1 << value, tp));
    }
  }
  return nullptr;
}
...
void HInductionVarAnalysis::VisitControl(HLoopInformation* loop) {
  ...
  if (control->IsIf()) {
    ...
    if (if_expr->IsCondition()) {
      ...
      Primitive::Type tp = condition->InputAt(0)->GetType();
      ...
      if (tp != Primitive::kPrimInt && tp != Primitive::kPrimLong) {
        // Loop control is not 32/64-bit integral.
      } else if (if_true->GetLoopInformation() != loop && if_false->GetLoopInformation() == loop) {
        VisitCondition(loop, a, b, tp, condition->GetOppositeCondition());
      } else if (if_true->GetLoopInformation() == loop && if_false->GetLoopInformation() != loop) {
        VisitCondition(loop, a, b, tp, condition->GetCondition());
      }
    }
  }
}
...
void HInductionVarAnalysis::VisitCondition(HLoopInformation* loop,
  ...
  if (a != nullptr && b != nullptr) {
    if (a->induction_class == kInvariant && b->induction_class == kLinear) {
      ...
      switch (cmp) {
        case kCondLT: VisitCondition(loop, b, a, tp, kCondGT); break;
        case kCondLE: VisitCondition(loop, b, a, tp, kCondGE); break;
        case kCondGT: VisitCondition(loop, b, a, tp, kCondLT); break;
        case kCondGE: VisitCondition(loop, b, a, tp, kCondLE); break;
        default: break;
      }
      ...
      if (IsIntAndGet(stride, &value)) {
        if ((value > 0 && (cmp == kCondLT || cmp == kCondLE)) ||
          ...
          VisitTripCount(loop, lo_val, hi_val, stride, value, tp, is_strict);
        }
      }
    }
  }
...
void HInductionVarAnalysis::VisitTripCount(HLoopInformation* loop,
  ...
  //       path is taken at least once. Otherwise TC is 0. Also, the expression assumes
  //       the loop does not have any early-exits. Otherwise, TC is an upper bound.
  //
  ...
  if (!cancels) {
    ...
    if (is_strict) {
      ...
      hi_val = CreateInvariantOp(op, hi_val, CreateConstant(1, tp));
    }
...
HInductionVarAnalysis::InductionInfo* HInductionVarAnalysis::CreateConstant(int64_t value,
                                                                            Primitive::Type tp) {
  if (tp == Primitive::kPrimInt) {
    return CreateInvariantFetch(graph_->GetIntConstant(value));
  }
  DCHECK_EQ(tp, Primitive::kPrimLong);
