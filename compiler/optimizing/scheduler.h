...
//
// Here are a few factors impacting the quality of the scheduling:
...
// - Dependencies in the dependency graph should be minimized as much as
//   possible to enable more scheduling possibilities. For example de-aliasing
//   can allow possibilities for memory accesses reordering.
...
//   that generate multiple assembly instructions or code that varies.
...
//    | n1, n2                    | n3                     |
//
// One of the candidates is selected. Let it be `n2`. It is scheduled before
// previously scheduled nodes (in program order). There are no other nodes to
...
class SchedulingNode : public ArenaObject<kArenaAllocScheduling> {
 public:
  ...
  void MaybeUpdateDelay(int other_delay) {
    delay_ = std::max(delay_, other_delay);
  }
 ...
 private:
  // The latency of an instruction is used by the scheduling heuristics to
  // measure this instruction's priority.
  int latency_;

  // The maximum delay of an instruction is used by the scheduling heuristics to
  // measure this instructions' priority.
  // We define it as the critical path from this node to the end of scheduling.
  //     node.delay_ = node.latency_ + max(delay_ of uses)
  // (Note that here 'uses' is equivalent to 'data successors'.)
  int delay_;
 ...
 * the execution latency of instructions.
 */
class SchedulingLatencyVisitor : public HGraphDelegateVisitor {
 public:
  ...
  int CalculateLatency(SchedulingNode* node) {
...
class CriticalPathSchedulingNodeSelector : public SchedulingNodeSelector {
 public:
  ...
  SchedulingNode* GetHigherPrioritySchedulingNode(SchedulingNode* candidate,
    ...
    int delay_candidate = candidate->GetDelay();
    int delay_check = check->GetDelay();
    // First look at the delay.
    if (delay_check != delay_candidate) {
      return delay_check < delay_candidate ? check : candidate;
    }
    // If both delays are equal, schedule instructions with a higher latency
...
class HScheduler {
 ...
 protected:
  ...
  void CalculateLatency(SchedulingNode* node) {
    node->SetLatency(latency_visitor_->CalculateLatency(node));
  }
