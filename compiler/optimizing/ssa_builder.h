  ...
  GrowableArray<HInstruction*>* GetLocalsFor(HBasicBlock* block) {
    ...
    if (locals == nullptr) {
      ...
      if (block->IsCatchBlock()) {
        // We record incoming inputs of catch phis at throwing instructions. We
        // must therefore eagerly create the catch phis. Unused phis will be
        // removed in the dead phi analysis.
