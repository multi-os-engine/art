...
static const Register kParameterCoreRegisters[] = {
  x1, x2, x3, x4, x5, x6, x7
};
static constexpr size_t kParameterCoreRegistersLength = arraysize(kParameterCoreRegisters);
static const FPRegister kParameterFPRegisters[] = {
  d0, d1, d2, d3, d4, d5, d6, d7
};
static constexpr size_t kParameterFPRegistersLength = arraysize(kParameterFPRegisters);

const Register tr = x19;                        // Thread Register
static const Register kArtMethodRegister = x0;  // Method register on invoke.

const CPURegList vixl_reserved_core_registers(ip0, ip1);
const CPURegList vixl_reserved_fp_registers(d31);

const CPURegList runtime_reserved_core_registers(tr, lr);

// Callee-saved registers AAPCS64 (without x19 - Thread Register)
const CPURegList callee_saved_core_registers(CPURegister::kRegister,
                                                   kXRegSize,
                                                   x20.GetCode(),
                                                   x30.GetCode());
const CPURegList callee_saved_fp_registers(CPURegister::kFPRegister,
                                                 kDRegSize,
                                                 d8.GetCode(),
                                                 d15.GetCode());
...
static const Register kRuntimeParameterCoreRegisters[] =
...
class InvokeDexCallingConvention : public CallingConvention<Register, FPRegister> {
 public:
...
class FieldAccessCallingConventionARM64 : public FieldAccessCallingConvention {
 public:
  ...
  Location GetObjectLocation() const OVERRIDE {
    return helpers::LocationFrom(x1);
  }
  Location GetFieldIndexLocation() const OVERRIDE {
    return helpers::LocationFrom(x0);
  }
  ...
  Location GetSetValueLocation(Primitive::Type type, bool is_instance) const OVERRIDE {
    return Primitive::Is64BitType(type)
        ? helpers::LocationFrom(x2)
        : (is_instance
            ? helpers::LocationFrom(x2)
            : helpers::LocationFrom(x1));
  }
  Location GetFpuLocation(Primitive::Type type ATTRIBUTE_UNUSED) const OVERRIDE {
    return helpers::LocationFrom(d0);
  }
...
#undef DECLARE_VISIT_INSTRUCTION
 ...
 private:
  void GenerateClassInitializationCheck(SlowPathCodeARM64* slow_path, Register class_reg);
...
class ParallelMoveResolverARM64 : public ParallelMoveResolverNoSwap {
 ...
 private:
  ...
  CodeGeneratorARM64* const codegen_;
  UseScratchRegisterScope vixl_temps_;
...
class CodeGeneratorARM64 : public CodeGenerator {
 public:
  ...
  static const int kNumberOfAllocatableRegisters = kNumberOfRegisters - 1;
  static const int kNumberOfAllocatableFPRegisters = kNumberOfFPRegisters;
  ...
  void MoveConstant(CPURegister destination, HConstant* constant);
  void MoveConstant(Location destination, int32_t value) OVERRIDE;
  void MoveLocation(Location dst, Location src, Primitive::Type dst_type) OVERRIDE;
  void AddLocationAsTemp(Location location, LocationSummary* locations) OVERRIDE;

  void Load(Primitive::Type type, CPURegister dst, const vixl::aarch64::MemOperand& src);
  void Store(Primitive::Type type, CPURegister src, const vixl::aarch64::MemOperand& dst);
  void LoadAcquire(HInstruction* instruction,
                   CPURegister dst,
                   const vixl::aarch64::MemOperand& src,
                   bool needs_null_check);
  void StoreRelease(Primitive::Type type, CPURegister src, const vixl::aarch64::MemOperand& dst);
  ...
  vixl::aarch64::Label* NewPcRelativeDexCacheArrayPatch(const DexFile& dex_file,
