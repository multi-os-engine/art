...
class CodeGeneratorARM64 : public CodeGenerator {
 public:
  ...
  virtual size_t GetNumberOfRegisters() const OVERRIDE { return kNumberOfAllocatableRegisters; }

  virtual Location GetStackLocation(HLoadLocal* load) const OVERRIDE;

  virtual size_t GetNumberOfCoreRegisters() const OVERRIDE {
    return kNumberOfXRegisters;
  }
  ...
  // can easily be mapped via to or from their type and index or code. The task
  // would be more complex for xzr, wzr, and sp, which respectively use the enum
  // values 32, 31, and, 31 in ART, and other values in VIXL.
