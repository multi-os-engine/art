...
void IntrinsicsRecognizer::Run() {
  ...
  for (HReversePostOrderIterator it(*graph_); !it.Done(); it.Advance()) {
    ...
    for (HInstructionIterator inst_it(block->GetInstructions()); !inst_it.Done();
      ...
      if (inst->IsInvokeStaticOrDirect()) {
        InlineMethod method;
        if (inliner->IsIntrinsic(inst->AsInvokeStaticOrDirect()->GetIndexInDexCache(), &method)) {
          ...
          if (intrinsic != Intrinsics::kNone) {
            inst->AsInvoke()->SetIntrinsic(intrinsic);
          }
        }
      } else if (inst->IsInvokeVirtual()) {
        InlineMethod method;
        if (inliner->IsIntrinsic(inst->AsInvokeVirtual()->GetMethodIndex(), &method)) {
          Intrinsics intrinsic = Intrinsics::kNone;
          switch (method.opcode) {
            case kIntrinsicReferenceGetReferent:
