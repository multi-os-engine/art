...
void HGenerateSelects::TryGeneratingSelects(HBasicBlock* block)
  ...
  // Handle Conditions as well as boolean inputs to the HIf.
  HInstruction* if_input = if_instruction->InputAt(0);
  HCondition* if_condition = if_input->AsCondition();

  // Can the backend handle these types?
  Primitive::Type cond_type =
    if_condition ? if_condition->InputAt(0)->GetType() : Primitive::kPrimBoolean;
  ...
  if (if_condition) {
    // Take the information from the condition.
    cond = if_condition->GetCondition();
    cond_left = if_condition->GetLeft();
    cond_right = if_condition->GetRight();
  } else {
    // Compare the boolean value to 'true'.
    cond = kCondEQ;
    cond_left = if_input;
    cond_right = graph_->GetIntConstant(1, if_instruction->GetDexPc());
  }

  HConditionalSelect* replacement =
      new (graph_->GetArena()) HConditionalSelect(cond,
                                                  cond_left,
                                                  cond_right,
                                                  true_value,
                                                  false_value,
  ...
  // Remove the original input to the HIf if it is now unused.
  if (!if_input->HasUses()) {
    if_input->GetBlock()->RemoveInstructionOrPhi(if_input);
  }
