...
static bool IsBlockTooComplexForSelect(HBasicBlock* block) {
  ...
  // Don't generate a select for a dangerous instruction, such as an invoke, volatile, write, etc.
  return !instr->CanBeMoved();
}
...
void HGenerateSelects::TryGeneratingSelects(HBasicBlock* block)
  ...
  HX86SelectValue* replacement =
      new (graph_->GetArena()) HX86SelectValue(if_condition->AsCondition(), true_value, false_value);
