 ...
 public:
  ...
  virtual void EmitNativeCode(CodeGenerator* codegen_in) OVERRIDE {
    ...
    if (invoke_->IsInvokeStaticOrDirect()) {
      GenerateStaticOrDirectCall(invoke_->AsInvokeStaticOrDirect(), codegen);
    } else {
...
static void HandleInvoke(HInvoke* invoke, LocationSummary::CallKind kind, ArenaAllocator* arena) {
  LocationSummary* locations = new (arena) LocationSummary(invoke, kind);
...
bool IntrinsicLocationsBuilderX86_64::VisitStringCharAt(HInvokeStaticOrDirect* invoke) {
  // Uses an IntrinsicSlowPath for fallback. Must use standard HandleInvoke.
  HandleInvoke(invoke, LocationSummary::kCallOnSlowPath, arena_);
...
bool IntrinsicCodeGeneratorX86_64::VisitStringCharAt(HInvokeStaticOrDirect* invoke) {
  ...
  GetAssembler()->movl(temp, idx);                          // temp := idx.
  GetAssembler()->addl(temp, Address(obj, offset_offset));  // temp := offset + idx.
  GetAssembler()->movl(out, Address(obj, value_offset));    // obj := obj.array.
  // out = out[2*temp].
  GetAssembler()->movzxw(out, Address(out, temp, ScaleFactor::TIMES_2, data_offset));

  GetAssembler()->Bind(slow_path->GetExitLabel());
...
void LocationsBuilderX86_64::VisitInvokeStaticOrDirect(HInvokeStaticOrDirect* invoke) {
  ...
  HandleInvoke(invoke, LocationSummary::kCall, GetGraph()->GetArena());
}
...
void InstructionCodeGeneratorX86_64::VisitInvokeStaticOrDirect(HInvokeStaticOrDirect* invoke) {
  ...
  GenerateStaticOrDirectCall(invoke, codegen_);
}
