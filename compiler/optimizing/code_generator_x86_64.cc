 ...
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    if (instruction_->IsCheckCast()) {
      // The codegen for the instruction overrides `temp`, so put it back in place.
...
void InstructionCodeGeneratorX86_64::VisitInstanceOf(HInstanceOf* instruction) {
  ...
  if (instruction->GetTypeCheckKind() == TypeCheckKind::kInterfaceCheck) {
  ...
  switch (instruction->GetTypeCheckKind()) {
    ...
    case TypeCheckKind::kArrayObjectCheck: {
      ...
      __ testl(out, out);
      __ j(kEqual, &zero);
      __ movzxw(out, Address(out, primitive_offset));
      __ cmpl(out, Immediate(Primitive::kPrimNot));
      __ j(kNotEqual, &zero);
      __ movl(out, Immediate(1));
      __ jmp(&done);
      break;
...
void InstructionCodeGeneratorX86_64::VisitCheckCast(HCheckCast* instruction) {
  ...
  switch (instruction->GetTypeCheckKind()) {
    ...
    case TypeCheckKind::kArrayObjectCheck: {
      ...
      __ testl(temp, temp);
      __ j(kEqual, slow_path->GetEntryLabel());
      __ movzxw(temp, Address(temp, primitive_offset));
      __ cmpl(temp, Immediate(Primitive::kPrimNot));
      __ j(kNotEqual, slow_path->GetEntryLabel());
      break;
    }
    case TypeCheckKind::kInterfaceCheck:
