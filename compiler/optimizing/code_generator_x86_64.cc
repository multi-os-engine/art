...
void InstructionCodeGeneratorX86_64::GenerateCompareTest(HInstruction* instruction) {
  LocationSummary* locations = instruction->GetLocations();

  if (!instruction->IsCondition()) {
    // This is a boolean value than needs to be compared to 0 to set the right CC.
    Location result = locations->Out();
    if (result.IsRegister()) {
      CpuRegister result_reg = result.AsRegister<CpuRegister>();
      __ testl(result_reg, result_reg);
    } else {
      DCHECK(result.IsStackSlot());
      __ cmpl(Address(CpuRegister(RSP), result.GetStackIndex()), Immediate(0));
    }
    return;
  }
...
void InstructionCodeGeneratorX86_64::VisitSelect(HSelect* select) {
  ...
  if (SelectCanUseCMOV(select)) {
    ...
    HInstruction* condition = select->GetCondition();
    // Can we use a previously generated condition code?
    if (!(IsBooleanValueOrMaterializedCondition(condition) &&
          AreEflagsSetFrom(condition, select))) {
      GenerateCompareTest(condition);
    }
    Condition cond;
    if (condition->IsCondition()) {
      cond = X86_64IntegerCondition(condition->AsCondition()->GetCondition());
    } else {
      // Must be a boolean condition, which was compared to 0.
      cond = kNotEqual;
    }
