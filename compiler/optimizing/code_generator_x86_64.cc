...
void InstructionCodeGeneratorX86_64::VisitDiv(HDiv* div) {
  ...
  switch (div->GetResultType()) {
    case Primitive::kPrimInt: {
      ...
      __ j(kEqual, &negation);  // eax already have the right value
...
void InstructionCodeGeneratorX86_64::VisitDivZeroCheck(HDivZeroCheck* instruction) {
  ...
  if (value.IsRegister()) {
    __ cmpl(value.As<CpuRegister>(), Immediate(0));
  } else if (value.IsStackSlot()) {
    __ cmpl(Address(CpuRegister(RSP), value.GetStackIndex()), Immediate(0));
  } else {
    DCHECK(value.IsConstant()) << value;
    // We don't generate the check if the constant is != 0 so check that it holds.
    DCHECK_EQ(value.GetConstant()->AsIntConstant()->GetValue(), 0);
