...
void CodeGeneratorX86_64::Move(HInstruction* instruction,
  ...
  HConstant* const_to_move = instruction->IsConstant()
      ? instruction->AsConstant()
      : (locations != nullptr && locations->Out().IsConstant()
          ? locations->Out().GetConstant()
          : nullptr);

  if (const_to_move != nullptr) {
