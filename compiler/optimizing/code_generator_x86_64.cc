...
void InstructionCodeGeneratorX86_64::VisitPackedSwitch(HPackedSwitch* switch_instr) {
  ...
  if (num_entries <= kPackedSwitchJumpTableThreshold) {
    ...
    int32_t compare_to_value;
    Condition first_condition, second_condition;
    uint32_t index;
    const ArenaVector<HBasicBlock*>& successors = switch_instr->GetBlock()->GetSuccessors();
    if (lower_bound != 0) {
      first_condition = kLess;
      second_condition = kEqual;
      __ cmpl(value_reg_in, Immediate(lower_bound));
      __ j(first_condition, codegen_->GetLabelOf(default_block));
      __ j(second_condition, codegen_->GetLabelOf(successors[0]));

      index = 1;
      compare_to_value = lower_bound + 2;
    } else {
      first_condition = kBelow;
      second_condition = kEqual;
      __ cmpl(value_reg_in, Immediate(1));
      __ j(first_condition, codegen_->GetLabelOf(successors[0]));
      __ j(second_condition, codegen_->GetLabelOf(successors[1]));
    ...
    for (; index + 1 < num_entries; index += 2) {
      ...
      __ j(second_condition, codegen_->GetLabelOf(successors[index + 1]));
    }
