...
static bool GenMinMaxFP(LocationSummary* locations, bool is_min, bool is_double,
  ...
  if (op1_loc.Equals(op2_loc)) {
    if (!out_loc.Equals(op1_loc) && !out_loc.Equals(op2_loc)) {
      // Copy to out.
      assembler->movsd(out_loc.AsFpuRegister<XmmRegister>(), op1_loc.AsFpuRegister<XmmRegister>());
    }
    return true;
  }
  ...
  if (!out_loc.Equals(op1_loc)) {
    if (out_loc.Equals(op2_loc)) {
      ...
      // Copy op1 to out.
      if (is_double) {
        assembler->movsd(out_loc.AsFpuRegister<XmmRegister>(),
                         op1_loc.AsFpuRegister<XmmRegister>());
      } else {
        assembler->movss(out_loc.AsFpuRegister<XmmRegister>(),
                         op1_loc.AsFpuRegister<XmmRegister>());
      }
    }
  }
