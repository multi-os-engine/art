...
void InstructionCodeGeneratorX86_64::GemDivRemIntegral(HBinaryOperation* instruction) {
  ...
  switch (type) {
    case Primitive::kPrimInt:
    case Primitive::kPrimLong: {
      CpuRegister out_reg = out.As<CpuRegister>();
    ...
    default:
      LOG(FATAL) << "Unexpected type for GenearateDivRemIntegral " << type;
  }
}
...
void LocationsBuilderX86_64::VisitRem(HRem* rem) {
  ...
  switch (rem->GetResultType()) {
    ...
    case Primitive::kPrimLong: {
      locations->SetInAt(0, Location::RegisterLocation(RAX));
      locations->SetInAt(1, Location::RegisterLocation(RCX));
      // Intel uses edx:eax as the dividend and puts the remainder in edx
