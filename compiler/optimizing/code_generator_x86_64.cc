...
void ParallelMoveResolverX86_64::EmitMove(size_t index) {
  ...
  if (source.IsRegister()) {
    ...
    if (constant->IsIntConstant()) {
      int32_t constant = constant->AsIntConstant()->GetValue();
      if (destination.IsRegister()) {
        if (constant == 0) {
          __ xorl(destination.AsRegister<CpuRegister>(), destination.AsRegister<CpuRegister>());
        } else {
          __ movl(destination.AsRegister<CpuRegister>(), Immediate(constant));
        }
      } else {
        DCHECK(destination.IsStackSlot()) << destination;
        __ movl(Address(CpuRegister(RSP), destination.GetStackIndex()), Immediate(constant));
      }
