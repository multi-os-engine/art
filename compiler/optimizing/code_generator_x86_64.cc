...
// Base class factoring read barrier slow paths common code.
class ReadBarrierSlowPathX86_64 : public SlowPathCode {
 protected:
  ReadBarrierSlowPathX86_64(HInstruction* instruction, Location out)
      : instruction_(instruction), out_(out) {}
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(ReadBarrierSlowPathX86_64);
};

// Slow path generating a read barrier for a heap reference.
class ReadBarrierForHeapReferenceSlowPathX86_64 : public ReadBarrierSlowPathX86_64 {
 public:
  ReadBarrierForHeapReferenceSlowPathX86_64(HInstruction* instruction,
                                            ...
                                            uint32_t offset,
                                            Location index)
      : ReadBarrierSlowPathX86_64(instruction, out),
        ref_(ref),
        obj_(obj),
        offset_(offset),
        index_(index) {
    DCHECK(kForceReadBarrier || kUseReadBarrier);
    ...
    //   codegen_->GenerateReadBarrier(instruction, out_loc, out_loc, out_loc, super_offset);
    //
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    if (ShouldSaveAndRestoreParameterRegisters()) {
      ...
      SaveParameterRegisters(codegen, out_);
    } else {
    ...
    if (index_.IsValid()) {
      ...
      if (instruction_->IsArrayGet()) {
        ...
        if (codegen->IsCoreCalleeSaveRegister(index_reg)) {
          ...
          Register free_reg = FindNextAvailableRegister(codegen).AsRegister();
  ...
  bool ShouldSaveAndRestoreParameterRegisters() const {
    ...
    // registers.
    //
    // HInvokeStaticOrDirect instructions only use a read barrier for
    // GC roots, hence we'll never end up in this slow path when
    // visiting a HInvokeStaticOrDirect instruction, except when they
    // are intrinsified.
    return instruction_->IsInvokeVirtual() || instruction_->IsInvokeInterface();
  }

  // Make this slow path save the temporary register `temp` before the
  // runtime call, and restore it afterwards, as if it were a real
  // live register.
  void AddLiveTempRegister(Location live_temp) {
    DCHECK(live_temp.IsRegister());
    live_temp_registers_.push_back(live_temp);
    DCHECK_LE(live_temp_registers_.size(), kMaxLiveTempRegistersForReadBarrier);
  }
 ...
 private:
  ...
  // A set of "live temporary registers" that must be considered live
  // across the read barrier slow path, and hence saved before and
  // restored after the call the runtime call in this slow path.
  std::vector<Location> live_temp_registers_;

  CpuRegister FindNextAvailableRegister(CodeGenerator* codegen);
  DISALLOW_COPY_AND_ASSIGN(ReadBarrierForHeapReferenceSlowPathX86_64);
};

// Slow path generating a read barrier for a GC root.
class ReadBarrierForRootSlowPathX86_64 : public ReadBarrierSlowPathX86_64 {
 public:
  ...
  // Is this read barrier (for a GC root) generated before a call,
  // thus requiring us to save the parameter registers before the
  // call, and restore them after?
  bool ShouldSaveAndRestoreParameterRegisters() const {
    // HInvokeStaticOrDirect is the only instruction requiring a read
    // barrier for a GC root before an actual method call, for which
    // we need to save and restore parameter registers.
    return instruction_->IsInvokeStaticOrDirect();
  }
...
void LocationsBuilderX86_64::VisitInvokeInterface(HInvokeInterface* invoke) {
  HandleInvoke(invoke);
  // Add the hidden argument.  This register is possibly used for read barrier too.
...
void InstructionCodeGeneratorX86_64::VisitInvokeInterface(HInvokeInterface* invoke) {
  ...
  if (receiver.IsStackSlot()) {
    ...
    codegen_->MaybeGenerateReadBarrier(invoke, temp_loc, temp_loc, temp2_loc, class_offset);
  } else {
    // /* HeapReference<Class> */ temp = receiver->klass_
    __ movl(temp, Address(receiver.AsRegister<CpuRegister>(), class_offset));
    codegen_->MaybeRecordImplicitNullCheck(invoke);
  ...
  // barrier, as it overwrites RAX.
  CpuRegister hidden_reg = temp2_loc.AsRegister<CpuRegister>();
...
void LocationsBuilderX86_64::VisitArraySet(HArraySet* instruction) {
  ...
  LocationSummary* locations = new (GetGraph()->GetArena()) LocationSummary(
      instruction,
      (may_need_runtime_call || kForceReadBarrier || kUseReadBarrier) ?
          LocationSummary::kCallOnSlowPath :
          LocationSummary::kNoCall);
    ...
    case Primitive::kPrimNot: {
      ...
      if (may_need_runtime_call) {
        ...
        codegen_->MaybeGenerateReadBarrier(
            instruction, temp_loc, temp_loc, array_loc, class_offset);
        ...
        if (kForceReadBarrier || kUseReadBarrier) {
          ...
          // The following read barrier may trash `temp`, as it is a
          // temporary register, and as such is not saved along with
          // live registers before calling the runtime (nor restored
          // afterwards).  As we need to preserve the value of `temp`
          // (for the subsequent comparison below), add it
          // "temporarily" to the set of live registers and remove it
          // after the call to the read barrier runtime entry point in
          // the slow path, by using
          // CodeGeneratorX86_64::GenerateReadBarrierWithLiveTemp
          // instead of CodeGeneratorX86_64::GenerateReadBarrier.
          //
          // Note that the area on the stack used to save live
          // registers is increased by one slot for this purpose; see
          // comments in CodeGenerator::InitializeCodeGeneration.
          //
          // TODO: Extend the register allocator to support a new
          // "locally live temp" location?
          codegen_->GenerateReadBarrierWithLiveTemp(
          ...
          // art::x86_64::CodeGeneratorX86_64::GenerateReadBarrier.
          __ cmpl(temp, temp2);
        } else {
          // /* HeapReference<Class> */ temp = temp->component_type_
          __ movl(temp, Address(temp, component_offset));
          // If heap poisoning is enabled, no need to unpoison `temp`,
          // as we are comparing two poisoned references.
        ...
        if (instruction->StaticTypeOfArrayIsObjectArray()) {
          ...
          if (kForceReadBarrier || kUseReadBarrier) {
            ...
            // already been unpoisoned earlier in a previous call to
            // art::x86::CodeGeneratorX86::GenerateReadBarrier.
            ...
            codegen_->GenerateReadBarrier(instruction, temp_loc, temp_loc, temp2_loc, super_offset);
          } else {
            // If heap poisoning is enabled, the `temp` reference has
            ...
            // `temp`, as we are comparing against null.
          }
          __ testl(temp, temp);
...
void InstructionCodeGeneratorX86_64::VisitLoadClass(HLoadClass* cls) {
  ...
  Location current_method_loc = locations->InAt(0);
  CpuRegister current_method = current_method_loc.AsRegister<CpuRegister>();
  if (cls->IsReferrersClass()) {
    DCHECK(kForceReadBarrier || kUseReadBarrier || !cls->CanCallRuntime());
    DCHECK(!cls->MustGenerateClinitCheck());
    if (kForceReadBarrier || kUseReadBarrier) {
      // /* GcRoot<mirror::Class>* */ out = &(current_method->declaring_class_)
      __ leaq(out, Address(current_method, ArtMethod::DeclaringClassOffset().Int32Value()));
...
void InstructionCodeGeneratorX86_64::VisitLoadString(HLoadString* load) {
  ...
  if (kForceReadBarrier || kUseReadBarrier) {
    ...
    __ leaq(out, Address(out, CodeGenerator::GetCacheOffset(load->GetStringIndex())));
...
void LocationsBuilderX86_64::VisitInstanceOf(HInstanceOf* instruction) {
  ...
  switch (type_check_kind) {
    ...
    case TypeCheckKind::kArrayObjectCheck:
      call_kind = (kForceReadBarrier || kUseReadBarrier) ?
          LocationSummary::kCallOnSlowPath :
          LocationSummary::kNoCall;
      break;
...
void InstructionCodeGeneratorX86_64::VisitInstanceOf(HInstanceOf* instruction) {
  ...
  switch (instruction->GetTypeCheckKind()) {
    ...
    case TypeCheckKind::kAbstractClassCheck: {
      ...
      __ Bind(&loop);
      Location temp_loc = (kForceReadBarrier || kUseReadBarrier) ?
          locations->GetTemp(0) :
          Location::NoLocation();
      if (kForceReadBarrier || kUseReadBarrier) {
...
void LocationsBuilderX86_64::VisitCheckCast(HCheckCast* instruction) {
  ...
  switch (type_check_kind) {
    ...
    case TypeCheckKind::kClassHierarchyCheck:
    case TypeCheckKind::kArrayObjectCheck:
      call_kind = (throws_into_catch || kForceReadBarrier || kUseReadBarrier) ?
          LocationSummary::kCallOnSlowPath :
          LocationSummary::kNoCall;  // In fact, call on a fatal slow path (not returning).
      break;
    case TypeCheckKind::kArrayCheck:
    case TypeCheckKind::kUnresolvedCheck:
    case TypeCheckKind::kInterfaceCheck:
...
void InstructionCodeGeneratorX86_64::VisitCheckCast(HCheckCast* instruction) {
  ...
  switch (type_check_kind) {
    ...
    case TypeCheckKind::kAbstractClassCheck: {
      ...
      __ Bind(&loop);
      Location temp2_loc = (kForceReadBarrier || kUseReadBarrier) ?
          locations->GetTemp(1) :
          Location::NoLocation();
      if (kForceReadBarrier || kUseReadBarrier) {
        // Save the value of `temp` into `temp2` before overwriting it
      ...
      // Othewise, jump to the slow path to throw the exception.
      //
    ...
    case TypeCheckKind::kArrayObjectCheck: {
      // Do an exact check.
      NearLabel check_primitive_type;
      if (cls.IsRegister()) {
        __ cmpl(temp, cls.AsRegister<CpuRegister>());
      } else {
        DCHECK(cls.IsStackSlot()) << cls;
      ...
      codegen_->MaybeGenerateReadBarrier(
          instruction, temp_loc, temp_loc, temp2_loc, component_offset);
      __ testl(temp, temp);
      __ j(kNotEqual, &check_primitive_type);
...
void CodeGeneratorX86_64::GenerateReadBarrierWithLiveTemp(HInstruction* instruction,
                                                          Location out,
                                                          Location ref,
                                                          Location obj,
                                                          uint32_t offset,
  ...
  // CodeGeneratorX86_64::GenerateReadBarrier is `live_temp`, passed
  // to the slow path.
  DCHECK(kForceReadBarrier || kUseReadBarrier);
  ReadBarrierForHeapReferenceSlowPathX86_64* slow_path = new (GetGraph()->GetArena())
      ReadBarrierForHeapReferenceSlowPathX86_64(instruction, out, ref, obj, offset, index);
  slow_path->AddLiveTempRegister(live_temp);
  AddSlowPath(slow_path);
  __ jmp(slow_path->GetEntryLabel());
  __ Bind(slow_path->GetExitLabel());
}
...
void ReadBarrierSlowPathX86_64::SaveParameterRegisters(CodeGenerator* codegen, Location out) {
  // TODO: This implementation saves all the (core) parameter
  ...
  //
  // This strategy could also be used to save floating-point parameter
  // registers as well (see the next TODO about this, below).
  ...
  for (size_t i = 0; i < kParameterCoreRegistersLength; ++i) {
    ...
    if (!codegen->IsCoreCalleeSaveRegister(reg)
        && reg != static_cast<size_t>(out.AsRegister<CpuRegister>().AsRegister())) {
  ...
  // TODO: We should save floating-point parameter registers as well
  // (not just core parameter registers). Currently we assume the
  // GCC/Clang-compiled slow path and read barrier related code in the
  // runtime does not touch any of the float parameter registers, but
  // that assumption is not verified.
}

void ReadBarrierSlowPathX86_64::RestoreParameterRegisters(CodeGenerator* codegen, Location out) {
  size_t stack_offset = codegen->GetFirstRegisterSlotInSlowPath();
  for (size_t i = 0; i < kParameterCoreRegistersLength; ++i) {
    size_t reg = static_cast<size_t>(kParameterCoreRegisters[i]);
    if (!codegen->IsCoreCalleeSaveRegister(reg)
...
CpuRegister ReadBarrierForHeapReferenceSlowPathX86_64::FindNextAvailableRegister(
