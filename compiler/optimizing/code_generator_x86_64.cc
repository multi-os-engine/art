...
class BoundsCheckSlowPathX86_64 : public SlowPathCode {
 public:
  ...
  virtual void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    __ Bind(GetEntryLabel());
    x64_codegen->Move(X86_64CpuLocation(RDI), index_location_);
    x64_codegen->Move(X86_64CpuLocation(RSI), length_location_);
    __ gs()->call(Address::Absolute(QUICK_ENTRYPOINT_OFFSET(kX86_64WordSize, pThrowArrayBounds), true));
    codegen->RecordPcInfo(dex_pc_);
  }

 private:
  const uint32_t dex_pc_;
  Location index_location_;
  Location length_location_;
