...
class ArraySetSlowPathX86_64 : public SlowPathCode {
 public:
  ArraySetSlowPathX86_64(HInstruction* instruction) : instruction_(instruction) {}
  ...
  switch (value_type) {
    ...
    case Primitive::kPrimByte: {
      ...
      Address address = index.IsConstant()
          ? Address(obj, (index.GetConstant()->AsIntConstant()->GetValue() << TIMES_1) + offset)
          : Address(obj, index.AsRegister<CpuRegister>(), TIMES_1, offset);
      if (value.IsRegister()) {
    ...
    case Primitive::kPrimChar: {
      ...
      Address address = index.IsConstant()
          ? Address(obj, (index.GetConstant()->AsIntConstant()->GetValue() << TIMES_2) + offset)
          : Address(obj, index.AsRegister<CpuRegister>(), TIMES_2, offset);
      if (value.IsRegister()) {
    ...
    case Primitive::kPrimNot: {
      ...
      Address address = index.IsConstant()
          ? Address(obj, (index.GetConstant()->AsIntConstant()->GetValue() << TIMES_4) + offset)
          : Address(obj, index.AsRegister<CpuRegister>(), TIMES_4, offset);
      if (!value.IsRegister()) {
        // Just setting null.
      ...
      if (may_need_runtime_call) {
        ...
        if (instruction->IsObjectArrayCheckUseful()) {
          __ j(kEqual, &do_put);
          __ MaybeUnpoisonHeapReference(temp);
    ...
    case Primitive::kPrimLong: {
      ...
      Address address = index.IsConstant()
          ? Address(obj, (index.GetConstant()->AsIntConstant()->GetValue() << TIMES_8) + offset)
          : Address(obj, index.AsRegister<CpuRegister>(), TIMES_8, offset);
      if (value.IsRegister()) {
