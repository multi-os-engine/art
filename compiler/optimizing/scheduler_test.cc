...
TEST_F(SchedulerTest, DependencyGraph) {
  ...
  std::vector<HInstruction*> block_instructions = { add1, add2, mul, div_check,
                                                    div, array_get, array_set };
  ...
  SchedulingGraph scheduling_graph(arena);
  for (auto instr : block_instructions) {
    scheduling_graph.AddNode(instr);
  }
  arm64::HArm64Scheduler scheduler(arena);
  scheduler.CalculateDelayForAllNodes(&scheduling_graph);

  ASSERT_FALSE(scheduling_graph.HasDependency(c1, add1));
  ASSERT_FALSE(scheduling_graph.HasDependency(c2, add2));

  // Define-use dependency.
  ASSERT_TRUE(scheduling_graph.HasDependency(add1, add2));
  ASSERT_FALSE(scheduling_graph.HasDependency(add2, add1));
  ASSERT_TRUE(scheduling_graph.HasDependency(add2, div_check));
  ASSERT_FALSE(scheduling_graph.HasDependency(add1, div_check));
  ASSERT_TRUE(scheduling_graph.HasDependency(div_check, div));
  ASSERT_TRUE(scheduling_graph.HasDependency(add1, array_set));
  ASSERT_TRUE(scheduling_graph.HasDependency(add2, array_set));

  // Read-write dependency.
  ASSERT_TRUE(scheduling_graph.HasDependency(array_get, array_set));

  // Env dependency.
  ASSERT_TRUE(scheduling_graph.HasDependency(mul, div_check));
  ASSERT_FALSE(scheduling_graph.HasDependency(div_check, mul));

  // CanThrow.
  ASSERT_TRUE(scheduling_graph.HasDependency(div_check, array_set));
}
...
static CodeGenerator* CreateTestCodegen(HGraph* graph,
  ...
  switch (instruction_set) {
    case kArm64:
    case kX86_64:
      return CodeGenerator::Create(graph,
                                   kRuntimeISA,
                                   *isa_features,
                                   compiler_options);
    case kArm:
    case kThumb2:
      return new TestCodeGeneratorARM(graph,
                                      *isa_features->AsArmInstructionSetFeatures(),
                                      compiler_options);
    case kX86:
      return new TestCodeGeneratorX86(graph,
...
static void CompileWithRandomSchedulerAndRun(const uint16_t* data,
  ...
  if (kRuntimeISA == kX86) {
    ...
    x86::PcRelativeFixups pc_relative_fixups(graph, &stats);
