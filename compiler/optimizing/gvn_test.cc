...
TEST(GVNTest, LoopSideEffects) {
  ...
  // Check that the side effects of the outer loop does not affect the inner loop.
  {
    ...
    ASSERT_TRUE(
        side_effects.GetLoopEffects(outer_loop_header).Exclude(kChangesGC).HasSideEffects());
    ASSERT_TRUE(side_effects.GetLoopEffects(inner_loop_header).Equals(kChangesGC));
  }
  ...
  // Check that the side effects of the inner loop affects the outer loop.
  {
    ...
    ASSERT_TRUE(
        side_effects.GetLoopEffects(outer_loop_header).Exclude(kChangesGC).HasSideEffects());
    ASSERT_TRUE(
        side_effects.GetLoopEffects(inner_loop_header).Exclude(kChangesGC).HasSideEffects());
  }
}
}  // namespace art
