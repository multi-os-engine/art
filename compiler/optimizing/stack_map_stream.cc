...
void StackMapStream::AddDexRegisterEntry(DexRegisterLocation::Kind kind, int32_t value) {
  ...
  if (in_inline_info_) {
    ...
    stack_maps_.back().flags |= StackMap::Flags::kHasDexRegisterMap;
  }
}
...
size_t StackMapStream::PrepareForFillIn() {
  ...
  size_t stack_map_size = stack_map_encoding_.SetFromSizes(max_stack_mask_bits,
                                                           inline_infos_size,
                                                           max_flags,
                                                           max_dex_pc,
...
void StackMapStream::PrepareDexRegisterMap() {
  ...
  for (StackMapEntry& entry : stack_maps_) {
    ...
    if (entry.num_dex_registers > 0) {
      ...
      DCHECK_NE(entry.flags & StackMap::Flags::kHasDexRegisterMap, 0u);  // Flag set.
    } else {
