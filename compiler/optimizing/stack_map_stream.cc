...
size_t StackMapStream::PrepareForFillIn() {
  ...
  header_.non_header_size.Set(non_header_size, &scratch);
  header_.stack_map_encoding = stack_map_encoding_;
  header_.number_of_stack_maps.Set(stack_maps_.size(), &scratch);
  header_.stack_map_size_in_bytes.Set(stack_map_size, &scratch);
  header_.number_of_location_catalog_entries.Set(location_catalog_entries_.size(), &scratch);
  header_.header_size = dchecked_integral_cast<uint8_t>(
     sizeof(CodeInfo::Header<0>) + sizeof(uint32_t) * (scratch - header_.scratch_space));
  ...
  dex_register_location_catalog_start_ = header_.header_size + stack_maps_size_;
  ...
  needed_size_ = header_.header_size + non_header_size;
  return needed_size_;
}
...
void StackMapStream::FillIn(MemoryRegion region) {
  ...
  region.CopyFrom(0, MemoryRegion(&header_, header_.header_size));
