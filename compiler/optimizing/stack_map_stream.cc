...
size_t StackMapStream::PrepareForFillIn() {
  ...
  stack_map_encoding_ = StackMapEncoding::ComputeFromSizes(stack_mask_size_,
                                                       inline_info_size_,
                                                       dex_register_maps_size_,
                                                       dex_pc_max_,
                                                       native_pc_offset_max_,
                                                       register_mask_max_);
  stack_maps_size_ = stack_maps_.Size() * stack_map_encoding_.StackMapSize();
...
void StackMapStream::FillIn(MemoryRegion region) {
  ...
  DCHECK_EQ(code_info.GetStackMapsSize(stack_map_encoding_), stack_maps_size_);
  ...
  for (size_t i = 0, e = stack_maps_.Size(); i < e; ++i) {
    ...
    if (entry.num_dex_registers == 0) {
      ...
      if (entry.same_dex_register_map_as_ != kNoSameDexMapFound) {
        ...
        stack_map.SetDexRegisterMapOffset(stack_map_encoding_,
