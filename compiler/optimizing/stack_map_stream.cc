...
void StackMapStream::BeginStackMapEntry(uint32_t dex_pc,
  ...
  if (is_catch_stack_map) {
    num_catch_stack_maps_++;
  } else {
    DCHECK_EQ(num_catch_stack_maps_, 0u)
        << "Non-catch stack maps must be at the beginning of the stream";
    num_instruction_stack_maps_++;
  }
}
...
StackMap StackMapStream::GetStackMapFromCodeInfo(const CodeInfo& code_info, size_t index) const {
  ...
  return (index < num_instruction_stack_maps_)
      ? code_info.GetStackMapAt(index, stack_map_encoding_)
      : code_info.GetCatchStackMapAt(index - num_instruction_stack_maps_, stack_map_encoding_);
}
...
void StackMapStream::FillIn(MemoryRegion region) {
  ...
  code_info.SetNumberOfStackMaps(num_instruction_stack_maps_);
  code_info.SetNumberOfCatchStackMaps(num_catch_stack_maps_);
  DCHECK_EQ(code_info.GetStackMapsSize(code_info.ExtractEncoding()) +
