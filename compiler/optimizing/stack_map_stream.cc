...
size_t StackMapStream::ComputeDexRegisterMapSize(uint32_t num_dex_registers,
                                                 BitVector* live_dex_registers_mask) const {
...
size_t StackMapStream::ComputeDexRegisterMapsSize() const {
  ...
  for (size_t i = 0; i < stack_maps_.Size(); ++i) {
    ...
    for (size_t j = 0; j < entry.inlining_depth; ++j) {
      ...
      size += ComputeDexRegisterMapSize(inline_entry.num_dex_registers, inline_entry.live_dex_registers_mask);
    }
  }
  return size;
}
  ...
  for (size_t i = 0, e = stack_maps_.Size(); i < e; ++i) {
    ...
    if (entry.inlining_depth != 0) {
      ...
      for (size_t j = 0; j < entry.inlining_depth; ++j) {
        ...
        inline_info.SetMethodIndexAtDepth(j, inline_entry.method_index);
        inline_info.SetDexPcAtDepth(j, inline_entry.dex_pc);
        if (inline_entry.num_dex_registers == 0) {
          // No dex map available.
          inline_info.SetDexRegisterMapOffsetAtDepth(j, StackMap::kNoDexRegisterMap);
        } else {
          MemoryRegion register_region = dex_register_locations_region.Subregion(
              next_dex_register_map_offset,
          ...
          inline_info.SetDexRegisterMapOffsetAtDepth(
            j, register_region.start() - dex_register_locations_region.start());
