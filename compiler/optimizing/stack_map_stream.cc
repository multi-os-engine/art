...
size_t StackMapStream::PrepareForFillIn() {
  ...
  size_t stack_map_size = stack_map_encoding_.SetFromSizes(stack_mask_number_of_bits,
                                                           inline_info_size_,
  ...
  CompressedUint32::unaligned_uint32_t* scratch = header_.scratch_space;
  header_.stack_map_encoding_ = stack_map_encoding_;
  header_.number_of_stack_maps.Set(stack_maps_.size(), &scratch);
  header_.stack_map_size_in_bytes_.Set(stack_map_size, &scratch);
  header_.number_of_location_catalog_entries.Set(location_catalog_entries_.size(), &scratch);
  const size_t scratch_size = sizeof(uint32_t) * (scratch - header_.scratch_space);
  const size_t header_size = sizeof(CodeInfo::Header<0>) + scratch_size;
  header_.header_size_ = dchecked_integral_cast<uint8_t>(header_size);

  // Note: use RoundUp to word-size here if you want CodeInfo objects to be word aligned.
  needed_size_ = header_size
      + stack_maps_size_
  ...
  dex_register_location_catalog_start_ = header_size + stack_maps_size_;
...
void StackMapStream::FillIn(MemoryRegion region) {
  ...
  DCHECK_GE(region.size(), header_.header_size_);
  memcpy(region.start(), &header_, header_.header_size_);
  ...
  for (size_t i = 0, e = stack_maps_.size(); i < e; ++i) {
    ...
    if (entry.sp_mask != nullptr) {
      int highest_bit_set = entry.sp_mask->GetHighestBitSet();
      for (int bit = 0; bit <= highest_bit_set; bit++) {
        if (entry.sp_mask->IsBitSet(bit)) {
          stack_map.SetStackMaskBit(stack_map_encoding_, bit, true);
        }
      }
    }
