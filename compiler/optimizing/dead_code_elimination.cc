...
static bool HasInput(HInstruction* instruction, HInstruction* input) {
...
static HConstant* Evaluate(HCondition* condition, HInstruction* left, HInstruction* right) {
  if (left == right) {
    if (condition->IsEqual()) {
      return condition->GetBlock()->GetGraph()->GetIntConstant(1);
    } else if (condition->IsNotEqual()) {
      return condition->GetBlock()->GetGraph()->GetIntConstant(0);
    }
  }
  ...
  for (HReversePostOrderIterator it(*graph_); !it.Done(); it.Advance()) {
    ...
    if (last->IsIf() &&
      ...
      bool has_only_phi_condition_and_if =
          !has_only_phi_and_if &&
          first->IsCondition() &&
          HasInput(first, block->GetFirstPhi()) &&
          (first->GetNext() == last) &&
