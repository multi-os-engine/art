...
static void MarkReachableBlocks(HGraph* graph, ArenaBitVector* visited) {
  ...
  while (!worklist.empty()) {
    ...
    HBasicBlock* selected_successor = nullptr;
    ...
    if (last_instruction->IsIf()) {
      ...
      if (condition->IsIntConstant()) {
        if (condition->AsIntConstant()->IsOne()) {
          selected_successor = if_instruction->IfTrueSuccessor();
        } else {
          DCHECK(condition->AsIntConstant()->IsZero());
          selected_successor = if_instruction->IfFalseSuccessor();
        }
      }
      ...
      if (switch_input->IsIntConstant()) {
        ...
        if (switch_index < static_cast<size_t>(switch_instruction->GetNumEntries())) {
          selected_successor = block->GetSuccessor(switch_index);
        } else {
          selected_successor = switch_instruction->GetDefaultBlock();
        }
      }
    }

    ArrayRef<HBasicBlock* const> successors = (selected_successor != nullptr)
        ? ArrayRef<HBasicBlock* const>(&selected_successor, 1)
