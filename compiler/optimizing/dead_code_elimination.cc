    ...
    if (last_instruction->IsIf()) {
      ...
      if (condition->IsIntConstant()) {
        if (condition->AsIntConstant()->IsOne()) {
          successors = successors.SubArray(0u, 1u);
          DCHECK_EQ(successors[0], if_instruction->IfTrueSuccessor());
        } else {
          DCHECK(condition->AsIntConstant()->IsZero());
          successors = successors.SubArray(1u, 1u);
          DCHECK_EQ(successors[0], if_instruction->IfFalseSuccessor());
        }
      }
      ...
      if (switch_input->IsIntConstant()) {
        ...
        if (switch_index < switch_instruction->GetNumEntries()) {
          successors = successors.SubArray(switch_index, 1u);
          DCHECK_EQ(successors[0], block->GetSuccessor(switch_index));
        } else {
          successors = successors.SubArray(switch_instruction->GetNumEntries(), 1u);
          DCHECK_EQ(successors[0], switch_instruction->GetDefaultBlock());
        }
      }
    }
