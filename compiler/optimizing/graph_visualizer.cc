...
class HGraphVisualizerDisassembler {
 public:
  HGraphVisualizerDisassembler(InstructionSet instruction_set, const uint8_t* base_address)
    ...
    disassembler_ = std::unique_ptr<Disassembler>(
        (*create_disassembler)(instruction_set,
...
void HGraphVisualizer::DumpGraphWithDisassembly(const DisassemblyInformation* disasm_info) const {
  DCHECK(output_ != nullptr);
  if (!graph_->GetBlocks().IsEmpty()) {
