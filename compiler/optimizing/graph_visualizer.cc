...
#include <optimizing/graph_visualizer.h>
...
HGraphDump::HGraphDump(HGraph* graph, const DexCompilationUnit& cu)
    : graph_(graph) {
  method_name_ = PrettyMethod(cu.GetDexMethodIndex(), *cu.GetDexFile(), false);
}

void HGraphDump::DumpGraph(const char* pass_name ATTRIBUTE_UNUSED,
                           const char* pass_attr ATTRIBUTE_UNUSED) const {
}

HGraphC1visualizerDump::HGraphC1visualizerDump(HGraph* graph,
                                               std::ostream& output,
                                               const char* method_filter,
                                               const CodeGenerator& codegen,
                                               const DexCompilationUnit& cu)
    : HGraphDump(graph, cu), output_(output), codegen_(codegen) {
...
void HGraphC1visualizerDump::DumpGraph(const char* pass_name,
...
HGraphTestDump::HGraphTestDump(HGraph* graph,
                               const DexCompilationUnit& cu)
    : HGraphDump(graph, cu) {
  LOG(INFO) << "------------------------------------";
...
HGraphTestDump::~HGraphTestDump() {
  LOG(INFO) << "END_METHOD " << method_name_;
  LOG(INFO) << "------------------------------------";
  LOG(INFO) << std::endl;
}

void HGraphTestDump::DumpGraph(const char* pass_name,
