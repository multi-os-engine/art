...
class HGraphVisualizerPrinter : public HGraphVisitor {
 public:
  ...
  void DumpInterval(const LiveInterval& interval) {
    StringList ranges;
    for (LiveRange* range = interval.GetFirstRange(); range != nullptr; range = range->GetNext()) {
      range->Dump(ranges.NewEntryStream());
    }
    StartAttributeStream("ranges") << ranges;

    if (interval.GetFirstUse() != nullptr) {
      StringList uses;
      for (LiveRange* use = interval.GetFirstUse(); use != nullptr; use = use->GetNext()) {
        use->Dump(uses.NewEntryStream());
      }
      StartAttributeStream("uses") << uses;
    }

    if (interval.GetFirstEnvironmentUse() != nullptr) {
      StringList env_uses;
      for (LiveRange* env_use = interval.GetFirstEnvironmentUse();
           env_use != nullptr;
           env_use = env_use->GetNext()) {
        env_use->Dump(env_uses.NewEntryStream());
      }
      StartAttributeStream("env_uses") << env_uses;
    }
  ...
  void PrintInstruction(HInstruction* instruction) {
    ...
    if (IsPass(SsaLivenessAnalysis::kLivenessPassName)
      ...
      if (instruction->HasLiveInterval()) {
        DumpInterval(*instruction->GetLiveInterval());
      }
