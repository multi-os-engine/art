...
void IntrinsicCodeGeneratorX86::VisitStringNewStringFromBytes(HInvoke* invoke) {
  X86Assembler* assembler = GetAssembler();
  LocationSummary* locations = invoke->GetLocations();
  LOG(INFO) << " x86 - GENERATING INTRINSIC STRING FROM BYTES";
...
void IntrinsicCodeGeneratorX86::VisitStringNewStringFromChars(HInvoke* invoke ATTRIBUTE_UNUSED) {
  X86Assembler* assembler = GetAssembler();
  LOG(INFO) << " x86 - GENERATING INTRINSIC STRING FROM CHARS";
...
void IntrinsicCodeGeneratorX86::VisitStringNewStringFromString(HInvoke* invoke) {
  X86Assembler* assembler = GetAssembler();
  LocationSummary* locations = invoke->GetLocations();
  LOG(INFO) << " x86 - GENERATING INTRINSIC STRING FROM STRING";
