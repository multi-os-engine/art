...
static void MoveFromReturnRegister(Location trg,
  ...
  switch (type) {
    ...
    case Primitive::kPrimLong: {
      ...
      if (trg_reg_lo != EAX) {
        __ movl(trg_reg_lo, EAX);
      }
      if (trg_reg_hi != EDX) {
        __ movl(trg_reg_hi, EDX);
      }
      break;
    }
...
class IntrinsicSlowPathX86 : public SlowPathCodeX86 {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen_in) OVERRIDE {
    ...
    if (invoke_->IsInvokeStaticOrDirect()) {
      codegen->GenerateStaticOrDirectCall(invoke_->AsInvokeStaticOrDirect(), temp_);
    } else {
 ...
 private:
  ...
  // A register available for use.
  Register const temp_;
...
static void GenMinMaxFP(LocationSummary* locations, bool is_min, bool is_double,
  ...
  __ Bind(&nan);
  if (is_double) {
    __ pushl(Immediate(0x7FF80000));
    __ pushl(Immediate(0));
    __ movsd(out, Address(ESP, 0));
    __ addl(ESP, Immediate(8));
  } else {
    __ pushl(Immediate(0x7FC00000));
