...
void IntrinsicLocationsBuilderX86::VisitSystemArrayCopyChar(HInvoke* invoke) {
  // We need at least one of the positions or length to be an integer constant,
  // or else we won't have enough free registers.  We would need 8 registers, and
  // there are only 7 available registers.
...
void IntrinsicCodeGeneratorX86::VisitSystemArrayCopyChar(HInvoke* invoke) {
  ...
  // Bail if the source and destination are the same (to handle overlap).
  __ cmpl(src, dest);
  ...
  if (!length.IsConstant()) {
    ...
    __ cmpl(length.AsRegister<Register>(), Immediate(128));
