...
void IntrinsicCodeGeneratorX86::VisitMathRoundFloat(HInvoke* invoke) {
  ...
  //  if (in - result >= 0.5f)
  //    result = result + 1.0f;
  __ roundss(tmp, in, Immediate(1));
  __ subss(in, tmp);
  __ comiss(in, codegen_->LiteralInt32Address(bit_cast<int32_t, float>(0.5f), constant_area));
  __ j(kBelow, &skip_incr);
  __ addss(tmp, codegen_->LiteralInt32Address(bit_cast<int32_t, float>(1.0f), constant_area));
  __ Bind(&skip_incr);
  ...
  __ comiss(tmp, in);
  __ j(kUnordered, &nan);
  __ j(kAboveEqual, &done);  // clip to max (already in out)
  __ cvttss2si(out, tmp);
  __ jmp(&done);

  __ Bind(&nan);
  __ xorl(out, out);  // NaN maps to 0
