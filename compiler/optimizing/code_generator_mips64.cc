...
class LoadStringSlowPathMIPS64 : public SlowPathCodeMIPS64 {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    mips64_codegen->MoveLocation(locations->Out(), calling_convention.GetReturnLocation(type),
        type);
 ...
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    if (instruction_->IsInstanceOf()) {
      mips64_codegen->InvokeRuntime(QUICK_ENTRY_POINT(pInstanceofNonTrivial), instruction_,
          dex_pc_, this);
...
void CodeGeneratorMIPS64::GenerateFrameEntry() {
  ...
  if (do_overflow_check) {
    __ LoadFromOffset(kLoadWord, ZERO, SP,
...
void CodeGeneratorMIPS64::MoveLocation(Location destination,
  ...
  if (destination.IsRegister() || destination.IsFpuRegister()) {
    ...
    if (source.IsStackSlot() || source.IsDoubleStackSlot()) {
      ...
      if (!Primitive::IsFloatingPointType(type)) {
        __ LoadFromOffset(load_type, destination.AsRegister<GpuRegister>(), SP,
            source.GetStackIndex());
      } else {
        __ LoadFpuFromOffset(load_type, destination.AsFpuRegister<FpuRegister>(), SP,
            source.GetStackIndex());
      }
    ...
    if (source.IsRegister() || source.IsFpuRegister()) {
      ...
      if (source.IsRegister()) {
        __ StoreToOffset(store_type, source.AsRegister<GpuRegister>(), SP,
            destination.GetStackIndex());
      } else {
        __ StoreFpuToOffset(store_type, source.AsFpuRegister<FpuRegister>(), SP,
            destination.GetStackIndex());
      }
    } else if (source.IsConstant()) {
...
void CodeGeneratorMIPS64::SwapLocations(Location loc1, Location loc2) {
  ...
  if (loc2.IsRegister() && loc1.IsRegister()) {
    ...
    if (reg_loc.IsFpuRegister()) {
      __ StoreFpuToOffset(store_type, reg_loc.AsFpuRegister<FpuRegister>(), SP,
...
void CodeGeneratorMIPS64::MarkGCCard(GpuRegister object, GpuRegister value) {
  ...
  __ Beqzc(value, &done);
  __ LoadFromOffset(kLoadDoubleword, card, S1,
...
void InstructionCodeGeneratorMIPS64::GenerateSuspendCheck(HSuspendCheck* instruction,
  ...
  __ LoadFromOffset(kLoadUnsignedHalfword, TMP, S1,
...
void InstructionCodeGeneratorMIPS64::VisitCheckCast(HCheckCast* instruction) {
  ...
  SlowPathCodeMIPS64* slow_path = new (GetGraph()->GetArena()) TypeCheckSlowPathMIPS64(
      instruction, locations->InAt(1), Location::RegisterLocation(obj_cls),
      instruction->GetDexPc());
  codegen_->AddSlowPath(slow_path);
...
void LocationsBuilderMIPS64::VisitNewArray(HNewArray* instruction) {
  ...
  // TODO: use calling_convention.GetReturnLocation() instead of V0?
...
void InstructionCodeGeneratorMIPS64::VisitBooleanNot(HBooleanNot* instruction) {
  ...
  __ Xori(locations->Out().AsRegister<GpuRegister>(),
      locations->InAt(0).AsRegister<GpuRegister>(), 1);
}
