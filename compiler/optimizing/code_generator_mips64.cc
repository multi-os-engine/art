...
void LocationsBuilderMIPS64::HandleInvoke(HInvoke* invoke) {
  ...
  locations->AddTemp(Location::RegisterLocation(A0));
...
void InstructionCodeGeneratorMIPS64::VisitInvokeInterface(HInvokeInterface* invoke) {
  ...
  __ LoadFromOffset(kLoadUnsignedWord, temp, temp, method_offset);
...
void InstructionCodeGeneratorMIPS64::VisitLoadClass(HLoadClass* cls) {
  ...
  if (cls->IsReferrersClass()) {
    ...
    __ LoadFromOffset(kLoadUnsignedWord, out, out, mirror::ArtMethod::DeclaringClassOffset().Int32Value());
  } else {
    DCHECK(cls->CanCallRuntime());
    codegen_->LoadCurrentMethod(out);
    __ LoadFromOffset(kLoadUnsignedWord,
