...
// We need an extra temporary/scratch register (in addition to AT) in some cases.
...
Location InvokeDexCallingConventionVisitor::GetNextLocation(Primitive::Type type) {
  ...
  if (Primitive::IsFloatingPointType(type) &&
    ...
    next_location = Location::FpuRegisterLocation(calling_convention.GetFpuRegisterAt(fp_index_++));
 ...
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    codegen->EmitParallelMoves(
        index_location_, Location::RegisterLocation(calling_convention.GetRegisterAt(0)),  Primitive::kPrimInt,
        length_location_, Location::RegisterLocation(calling_convention.GetRegisterAt(1)), Primitive::kPrimInt);
...
class LoadStringSlowPathMIPS64 : public SlowPathCodeMIPS64 {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    mips64_codegen->MoveLocation(locations->Out(), calling_convention.GetReturnLocation(type), type);
...
class TypeCheckSlowPathMIPS64 : public SlowPathCodeMIPS64 {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    InvokeRuntimeCallingConvention calling_convention;
    codegen->EmitParallelMoves(
        class_to_check_, Location::RegisterLocation(calling_convention.GetRegisterAt(0)), Primitive::kPrimNot,
        object_class_, Location::RegisterLocation(calling_convention.GetRegisterAt(1)), Primitive::kPrimNot);

    if (instruction_->IsInstanceOf()) {
      mips64_codegen->InvokeRuntime(QUICK_ENTRY_POINT(pInstanceofNonTrivial), instruction_, dex_pc_, this);
...
CodeGeneratorMIPS64::CodeGeneratorMIPS64(HGraph* graph, const CompilerOptions& compiler_options)
    : CodeGenerator(graph,
...
#undef __
#define __ reinterpret_cast<Mips64Assembler*>(GetAssembler())->
#define QUICK_ENTRY_POINT(x) QUICK_ENTRYPOINT_OFFSET(kMips64WordSize, x).Int32Value()
...
void ParallelMoveResolverMIPS64::Exchange(int index1, int index2, bool double_slot) {
  LoadOperandType load_type = kLoadWord;
  StoreOperandType store_type = kStoreWord;
  if (double_slot) {
  ...
  __ LoadFromOffset(load_type, GpuRegister(ensure_scratch.GetRegister()),
                    SP, index1 + stack_offset);
  __ LoadFromOffset(load_type, TMP, SP, index2 + stack_offset);
  __ StoreToOffset(store_type, GpuRegister(ensure_scratch.GetRegister()),
                   SP, index2 + stack_offset);
...
void CodeGeneratorMIPS64::GenerateFrameEntry() {
  ...
  if (do_overflow_check) {
    __ LoadFromOffset(kLoadWord, ZERO, SP, -static_cast<int32_t>(GetStackOverflowReservedBytes(kMips64)));
  ...
  DCHECK(IsInt<16>(kCurrentMethodStackOffset));
  __ Sw(A0, SP, kCurrentMethodStackOffset);
}
...
void CodeGeneratorMIPS64::MoveLocation(Location destination, Location source, Primitive::Type type) {
  ...
  if (destination.IsRegister() || destination.IsFpuRegister()) {
    ...
    if (source.IsStackSlot() || source.IsDoubleStackSlot()) {
      ...
      if (!Primitive::IsFloatingPointType(type)) {
        __ LoadFromOffset(load_type, destination.AsRegister<GpuRegister>(), SP, source.GetStackIndex());
      } else {
        __ LoadFpuFromOffset(load_type, destination.AsFpuRegister<FpuRegister>(), SP, source.GetStackIndex());
      }
    ...
    if (source.IsRegister() || source.IsFpuRegister()) {
      ...
      if (source.IsRegister()) {
        __ StoreToOffset(store_type, source.AsRegister<GpuRegister>(), SP, destination.GetStackIndex());
      } else {
        __ StoreFpuToOffset(store_type, source.AsFpuRegister<FpuRegister>(), SP, destination.GetStackIndex());
      }
    } else if (source.IsConstant()) {
...
void CodeGeneratorMIPS64::SwapLocations(Location loc1, Location loc2) {
  ...
  if (loc2.IsRegister() && loc1.IsRegister()) {
    ...
    // TODO: Can MOV.S/MOV.D be used here to save one instruction? Need to distinguish float from double, right?
    __ Dmfc1(TMP, r2);
    __ Dmfc1(AT, r1);
    __ Dmtc1(TMP, r1);
    __ Dmtc1(AT, r2);
    ...
    if (reg_loc.IsFpuRegister()) {
      __ StoreFpuToOffset(store_type, reg_loc.AsFpuRegister<FpuRegister>(), SP, mem_loc.GetStackIndex());
...
void CodeGeneratorMIPS64::Move(HInstruction* instruction, Location location, HInstruction* move_for) {
  ...
  if (instruction->IsIntConstant()
    ...
    if (location.IsRegister()) {
      ...
      if (instruction->IsNullConstant() || instruction->IsIntConstant()) {
        ...
        __ LoadConst64(dst, GetInt64ValueOf(instruction->AsConstant()));
      }
    } else {
      ...
      if (location.IsStackSlot()) {
        ...
        __ LoadConst64(TMP, GetInt64ValueOf(instruction->AsConstant()));
...
void CodeGeneratorMIPS64::MarkGCCard(GpuRegister object, GpuRegister value) {
  ...
  __ Beqzc(value, &done);
  __ LoadFromOffset(kLoadDoubleword, card, S1, Thread::CardTableOffset<kMips64WordSize>().Int32Value());
...
void CodeGeneratorMIPS64::SetupBlockedRegisters(bool is_baseline) const {
  // ZERO, K0, K1, SP, RA are always reserved and can't be allocated.
  ...
  // TODO: define/use dedicated enums for these?
...
void CodeGeneratorMIPS64::LoadCurrentMethod(GpuRegister current_method) {
  ...
  __ Lwu(current_method, SP, kCurrentMethodStackOffset);
}
...
void CodeGeneratorMIPS64::InvokeRuntime(int32_t entry_point_offset,
  ...
  // TODO: anything related to T9/GP/GOT/PIC/.so's?
  __ LoadFromOffset(kLoadDoubleword, T9, S1, entry_point_offset);
  __ Jalr(T9);
...
void InstructionCodeGeneratorMIPS64::GenerateSuspendCheck(HSuspendCheck* instruction,
  ...
  __ LoadFromOffset(kLoadUnsignedHalfword, TMP, S1, Thread::ThreadFlagsOffset<kMips64WordSize>().Int32Value());
...
InstructionCodeGeneratorMIPS64::InstructionCodeGeneratorMIPS64(HGraph* graph, CodeGeneratorMIPS64* codegen)
      : HGraphVisitor(graph),
    ...
    case Primitive::kPrimLong: {
      ...
      if (right->IsConstant()) {
        ...
        if (instruction->IsAnd() || instruction->IsOr() || instruction->IsXor()) {
          // TODO: figure out why only values up to 0x7FFF arrive here as constants.
          // All values from 0 to 0xFFFF inclusive should arrive as constants and
          // we should be able to use them as immediate right-hand-side operands in
          // instructions andi, ori and xori.
          // However, 0x8000+ will arrive pre-loaded in a register.
          // Also, if it's a 64-bit operation, all constant appear to arrive
          // pre-loaded in registers.
          // It looks like a bug in the generic part of the compiler.
          // Does it have anything to do with DEX LIT16?
          // Does this happen only when optimizations are disabled?
          can_use_imm = IsUint<16>(imm);
#if 0
          // TODO: remove this test/debug aid once the above is resolved.
          if (instruction->IsXor()) {
            if (!can_use_imm)
              LOG(WARNING) << "can't xori with " << imm;
            else
              LOG(WARNING) << "can   xori with " << imm;
          }
#endif
...
void InstructionCodeGeneratorMIPS64::VisitBoundsCheck(HBoundsCheck* instruction) {
  ...
  // Unsigned comparison of length and index checks for index >= length
  // and for index < 0 simultaneously.
...
void InstructionCodeGeneratorMIPS64::VisitCheckCast(HCheckCast* instruction) {
  ...
  SlowPathCodeMIPS64* slow_path = new (GetGraph()->GetArena()) TypeCheckSlowPathMIPS64(
      instruction, locations->InAt(1), Location::RegisterLocation(obj_cls), instruction->GetDexPc());
  codegen_->AddSlowPath(slow_path);

  // TODO: avoid this check if we know obj is not null.
  __ Beqzc(obj, slow_path->GetExitLabel());
  // Compare the class of `obj` with `cls`.
  // TODO: signed or unsigned load?
...
void InstructionCodeGeneratorMIPS64::VisitClinitCheck(HClinitCheck* check) {
  ...
  codegen_->AddSlowPath(slow_path);
  GenerateClassInitializationCheck(slow_path, check->GetLocations()->InAt(0).AsRegister<GpuRegister>());
}
...
void LocationsBuilderMIPS64::VisitCompare(HCompare* compare) {
  ...
  if (Primitive::IsFloatingPointType(in_type)) {
    call_kind = LocationSummary::kCall;
  }

  LocationSummary* locations =
      new (GetGraph()->GetArena()) LocationSummary(compare, call_kind);
  ...
  switch (in_type) {
    ...
    case Primitive::kPrimDouble: {
      ...
      locations->SetInAt(1, Location::FpuRegisterLocation(
...
void InstructionCodeGeneratorMIPS64::VisitCompare(HCompare* instruction) {
  ...
  switch (in_type) {
    ...
    case Primitive::kPrimFloat:
    case Primitive::kPrimDouble: {
      __ Xori(ZERO, ZERO, 14);
      int32_t entry_point_offset;
      if (in_type == Primitive::kPrimFloat) {
        entry_point_offset = instruction->IsGtBias() ? QUICK_ENTRY_POINT(pCmpgFloat) : QUICK_ENTRY_POINT(pCmplFloat);
      } else {
        entry_point_offset = instruction->IsGtBias() ? QUICK_ENTRY_POINT(pCmpgDouble) : QUICK_ENTRY_POINT(pCmplDouble);
      }
...
void InstructionCodeGeneratorMIPS64::VisitDoubleConstant(HDoubleConstant* constant) {
...
void InstructionCodeGeneratorMIPS64::VisitExit(HExit* exit) {
  UNUSED(exit);
}
...
void InstructionCodeGeneratorMIPS64::VisitFloatConstant(HFloatConstant* constant) {
  // Will be generated at use site.
  UNUSED(constant);
}
...
void LocationsBuilderMIPS64::HandleFieldGet(HInstruction* instruction, const FieldInfo& field_info) {
  UNUSED(field_info);
  LocationSummary* locations =
      new (GetGraph()->GetArena()) LocationSummary(instruction, LocationSummary::kNoCall);
  locations->SetInAt(0, Location::RequiresRegister());
...
void LocationsBuilderMIPS64::HandleFieldSet(HInstruction* instruction, const FieldInfo& field_info) {
  UNUSED(field_info);
  LocationSummary* locations =
      new (GetGraph()->GetArena()) LocationSummary(instruction, LocationSummary::kNoCall);
  locations->SetInAt(0, Location::RequiresRegister());
...
void InstructionCodeGeneratorMIPS64::HandleFieldSet(HInstruction* instruction,
                                                    const FieldInfo& field_info) {
  // __ Xori(ZERO, ZERO, 20);
  Primitive::Type type = field_info.GetFieldType();
  LocationSummary* locations = instruction->GetLocations();

  GpuRegister obj = locations->InAt(0).AsRegister<GpuRegister>();
  StoreOperandType store_type;
...
void InstructionCodeGeneratorMIPS64::VisitInstanceOf(HInstanceOf* instruction) {
  ...
  // Compare the class of `obj` with `cls`.
  // TODO: signed or unsigned load?
  __ LoadFromOffset(kLoadWord, out, out, mirror::Object::ClassOffset().Int32Value());
  if (instruction->IsClassFinal()) {
    ...
    SlowPathCodeMIPS64* slow_path =
        new (GetGraph()->GetArena()) TypeCheckSlowPathMIPS64(
...
void InstructionCodeGeneratorMIPS64::VisitIntConstant(HIntConstant* constant) {
  // Will be generated at use site.
  UNUSED(constant);
}
...
void InstructionCodeGeneratorMIPS64::VisitNullConstant(HNullConstant* constant) {
  // Will be generated at use site.
  UNUSED(constant);
}
...
void LocationsBuilderMIPS64::HandleInvoke(HInvoke* invoke) {
  ...
  locations->AddTemp(Location::RegisterLocation(V0));  // TODO: review
...
void InstructionCodeGeneratorMIPS64::VisitInvokeInterface(HInvokeInterface* invoke) {
  ...
  Offset entry_point = mirror::ArtMethod::EntryPointFromQuickCompiledCodeOffset(kMips64WordSize);
  ...
  if (receiver.IsStackSlot()) {
    ...
    __ LoadFromOffset(kLoadWord, temp, receiver.AsRegister<GpuRegister>(), class_offset);
  }
...
void CodeGeneratorMIPS64::GenerateStaticOrDirectCall(HInvokeStaticOrDirect* invoke, GpuRegister temp) {
  ...
  if (!invoke->IsRecursive()) {
    ...
    __ LoadFromOffset(kLoadDoubleword, T9, temp,
...
void InstructionCodeGeneratorMIPS64::VisitLoadClass(HLoadClass* cls) {
  __ Xori(ZERO, ZERO, 25);
  GpuRegister out = cls->GetLocations()->Out().AsRegister<GpuRegister>();
  // TODO: sort out the lw vs lwu issue with ints vs references
  if (cls->IsReferrersClass()) {
    ...
    DCHECK(cls->CanCallRuntime());
    codegen_->LoadCurrentMethod(out);
    __ LoadFromOffset(kLoadWord, out, out, mirror::ArtMethod::DexCacheResolvedTypesOffset().Int32Value());
...
void InstructionCodeGeneratorMIPS64::VisitLoadException(HLoadException* load) {
  __ Xori(ZERO, ZERO, 26);
  GpuRegister out = load->GetLocations()->Out().AsRegister<GpuRegister>();
  // TODO: review; signed or unsigned load?
  __ LoadFromOffset(kLoadWord, out, S1, Thread::ExceptionOffset<kMips64WordSize>().Int32Value());
  __ StoreToOffset(kStoreWord, ZERO, S1, Thread::ExceptionOffset<kMips64WordSize>().Int32Value());
}
...
void InstructionCodeGeneratorMIPS64::VisitLoadLocal(HLoadLocal* load) {
...
void InstructionCodeGeneratorMIPS64::VisitLongConstant(HLongConstant* constant) {
  // Will be generated at use site.
  UNUSED(constant);
}
...
void LocationsBuilderMIPS64::VisitNewArray(HNewArray* instruction) {
  ...
  locations->SetOut(Location::RegisterLocation(V0));  // TODO: use calling_convention.GetReturnLocation()?
...
void InstructionCodeGeneratorMIPS64::VisitNewArray(HNewArray* instruction) {
  ...
  codegen_->InvokeRuntime(GetThreadOffset<kMips64WordSize>(instruction->GetEntrypoint()).Int32Value(),
                          instruction,
                          instruction->GetDexPc(),
                          nullptr);
...
void InstructionCodeGeneratorMIPS64::VisitBooleanNot(HBooleanNot* instruction) {
  LocationSummary* locations = instruction->GetLocations();
  __ Xori(locations->Out().AsRegister<GpuRegister>(), locations->InAt(0).AsRegister<GpuRegister>(), 1);
}
...
void LocationsBuilderMIPS64::VisitParallelMove(HParallelMove* instruction) {
  UNUSED(instruction);
  LOG(FATAL) << "Unreachable";
}
...
void InstructionCodeGeneratorMIPS64::VisitPhi(HPhi* instruction) {
  UNUSED(instruction);
  LOG(FATAL) << "Unreachable";
}
...
void InstructionCodeGeneratorMIPS64::VisitReturn(HReturn* ret) {
  UNUSED(ret);
...
void InstructionCodeGeneratorMIPS64::VisitReturnVoid(HReturnVoid* ret) {
  UNUSED(ret);
  codegen_->GenerateFrameExit();
}
...
void InstructionCodeGeneratorMIPS64::VisitStoreLocal(HStoreLocal* store) {
  UNUSED(store);
}
...
void InstructionCodeGeneratorMIPS64::VisitTemporary(HTemporary* temp) {
  // Nothing to do, this is driven by the code generator.
  UNUSED(temp);
}
...
void LocationsBuilderMIPS64::VisitBoundType(HBoundType* instruction) {
  // Nothing to do, this should be removed during prepare for register allocator.
  UNUSED(instruction);
  LOG(FATAL) << "Unreachable";
}

void InstructionCodeGeneratorMIPS64::VisitBoundType(HBoundType* instruction) {
  // Nothing to do, this should be removed during prepare for register allocator.
  UNUSED(instruction);
  LOG(FATAL) << "Unreachable";
}
