...
void SchedulingGraph::AddDependencies(HInstruction* instruction, bool is_scheduling_barrier) {
  ...
  for (HUseIterator<HInstruction*> it(instruction->GetUses()); !it.Done(); it.Advance()) {
    AddDataDependency(GetNode(it.Current()->GetUser()), instruction_node);
  }
  ...
  if (!is_scheduling_barrier) {
    for (HUseIterator<HEnvironment*> it(instruction->GetEnvUses()); !it.Done(); it.Advance()) {
      ...
      AddOtherDependency(GetNode(it.Current()->GetUser()->GetHolder()), instruction_node);
    }
  }
}
...
static const std::string InstrucitonTypedId(const HInstruction* instruction) {
  std::string id;
...
static void DumpAsDotNode(std::ostream& output, SchedulingNode* node) {
  ...
  std::string instruction_id = InstrucitonTypedId(instruction);
  ...
  HInputIterator it(instruction);
  while (!it.Done()) {
    output << InstrucitonTypedId(it.Current());
    it.Advance();
    if (!it.Done()) output << ',';
  }
  output << "]";
  ...
  for (const SchedulingNode* predecessor : node->GetDataPredecessors()) {
    ...
    output << InstrucitonTypedId(predecessor_instruction) << ":s -> " << instruction_id << ":n "
...
void SchedulingGraph::DumpAsDotGraph(const std::string& description,
  ...
  if (initial_candidates != nullptr) {
    // Create a fake 'end_of_scheduling' node to help visualization of delays.
    for (auto node : *initial_candidates) {
      const HInstruction* instruction = node->GetInstruction();
      output << InstrucitonTypedId(instruction) << ":s -> end_of_scheduling:n "
...
void HScheduler::Schedule(HBasicBlock* block) {
  ...
  for (const auto& it : *scheduling_graph_.GetSchedulingNodes()) {
    ...
    if (!node->HasUnscheduledSuccessors()) {
      node->MaybeUpdateDelay(node->GetLatency());
...
void HScheduler::Schedule(SchedulingNode* scheduling_node) {
  ...
  int scheduling_delay = scheduling_node->GetDelay();
  for (SchedulingNode* predecessor : scheduling_node->GetDataPredecessors()) {
    predecessor->MaybeUpdateDelay(scheduling_delay + predecessor->GetLatency());
  ...
  for (SchedulingNode* predecessor : scheduling_node->GetOtherPredecessors()) {
    ...
    // correctness. So we do not use them to compute the delay.
    predecessor->DecrementNumberOfUnscheduledSuccessors();
    if (!predecessor->HasUnscheduledSuccessors()) {
      candidates_.push_back(predecessor);
    }
...
bool HScheduler::IsSchedulable(const HInstruction* instruction) const {
  ...
  DCHECK(!instruction->IsLoadLocal() &&
         ...
         !instruction->IsStoreLocal()) << "unexpected instruction " << instruction->DebugName();
