...
void SsaLivenessAnalysis::ComputeLiveness() {
  ...
  // Compute the live ranges, was well as the initial live_in, live_out, and kill sets.
  // This method does not handle backward branches, therefore live_in and live_out sets
  // are not yet correct.
  ComputeLiveRanges();
...
void SsaLivenessAnalysis::ComputeLiveRanges() {
  ...
  for (HLinearPostOrderIterator it(linear_post_order_); !it.Done(); it.Advance()) {
    ...
    for (InstructionBitVectorIterator it(*live_in, instructions_); !it.Done(); it.Advance()) {
