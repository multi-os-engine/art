    ...
    for (HBackwardInstructionIterator back_it(block->GetInstructions()); !back_it.Done();
      ...
      if (current->IsEmittedAtUseSite()) {
        ...
        for (size_t i = 0, e = current->InputCount(); i < e; ++i) {
          ...
          bool has_out_location = input->HasSsaIndex();
          DCHECK_EQ(has_out_location, input->GetLocations()->Out().IsValid());

          if (has_in_location) {
            DCHECK(has_out_location) << input->DebugName() << " ---" << i << "--> " << current->DebugName();
            ...
            for (size_t i2 = 0, e2 = input->InputCount(); i2 < e2; ++i2) {
              ...
              if (input->GetLocations()->InAt(i2).IsValid()) {
                ...
                inlined_input->GetLiveInterval()->AddUse(
                    /* owner */ input, /* environment */ nullptr, i2, /* real_user */ current);
              }
            }
          }
        }
