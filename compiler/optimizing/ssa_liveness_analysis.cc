  ...
  for (HLinearPostOrderIterator it(*graph_); !it.Done(); it.Advance()) {
    ...
    for (size_t i = 0, e = block->GetSuccessors().Size(); i < e; ++i) {
      ...
      if (successor->IsCatchBlock()) {
        // Inputs of catch phis need to be kept alive so that the runtime can
        // copy their values to the corresponding catch phi spill slots when an
        // exception is thrown, but this is already achieved by their environment
        // uses in the respective throwing instructions.
