...
void SsaLivenessAnalysis::NumberInstructions() {
  int ssa_index = 0;
  for (HDominatorOrderIterator it(graph_); !it.Done(); it.Advance()) {
...
void SsaLivenessAnalysis::ComputeInitialSets() {
  // Do a post-dominator visit, adding inputs of instructions live in the block where
  // that instruction is defined, and killing instructions that are being visited.
  for (HPostDominatorOrderIterator it(graph_); !it.Done(); it.Advance()) {
    HBasicBlock* block = it.Current();
