...
static void ProcessInstructionInputs(HInstruction* instruction,
  ...
  for (size_t i = 0, e = instruction->InputCount(); i < e; ++i) {
    ...
    bool has_ssa_index = input->HasSsaIndex();
    bool has_valid_loc = instruction->GetLocations()->InAt(i).IsValid();

    if (has_ssa_index && has_valid_loc) {
      live_in->SetBit(input->GetSsaIndex());
      input->GetLiveInterval()->AddUse(instruction, /* environment */ nullptr, i, real_user);
    } else if (!has_ssa_index && !has_valid_loc) {
      DCHECK(IsNonMaterializedCondition(input));
      // Inputs of a non-materialized condition need to stay live until the
      // user. Process them recursively. Note that it is not possible for the
      // inputs of the condition to be another non-materialized condition.
        ...
        for (size_t i = 0, e = environment->Size(); i < e; ++i) {
          ...
          if (instruction != nullptr) {
            instruction->GetLiveInterval()->AddUse(
                current, environment, i, /* real_user */ nullptr, should_be_live);
          }
        }
      }

      // All inputs of an instruction must be live. Skip for non-materialized
      // conditions because their inputs were already accounted for by the users.
