...
void BisectionController::Init(const StringPiece& config) {
  size_t delim_pos = config.find(',');
  optimize_up_to_method_ = ParseOption("method:", config.substr(0, delim_pos));
  StringPiece pass_config;
  if (delim_pos != std::string::npos) {
    pass_config = config.substr(delim_pos + 1);
  }
