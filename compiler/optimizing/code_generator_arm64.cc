...
void InstructionCodeGeneratorARM64::VisitArraySet(HArraySet* instruction) {
  ...
  if (!needs_write_barrier) {
    ...
    SlowPathCodeARM64* slow_path = nullptr;
    {
      ...
      UseScratchRegisterScope temps(masm);
      Register temp = temps.AcquireSameSizeAs(array);
      Location temp_loc = LocationFrom(temp);
      if (index.IsConstant()) {
        offset += Int64ConstantFrom(index) << Primitive::ComponentSizeShift(value_type);
        destination = HeapOperand(array, offset);
      } else {
      ...
      if (may_need_runtime_call_for_type_check) {
        ...
        if (kEmitCompilerReadBarrier && !kUseBakerReadBarrier) {
          ...
          if (kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
            ...
            Register temp3 = WRegisterFrom(locations->GetTemp(1));
          ...
          if (instruction->StaticTypeOfArrayIsObjectArray()) {
            ...
            if (kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
              ...
              // Same comment as above about `temp2` not being
              // allocated from VIXL's scratch register pool with
              // Baker read barriers.
              Register temp2 = WRegisterFrom(locations->GetTemp(0));
