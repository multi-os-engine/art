...
class LoadClassSlowPathARM64 : public SlowPathCodeARM64 {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    if (do_clinit_) {
      CheckEntrypointTypes<kQuickInitializeStaticStorage, void*, uint32_t, ArtMethod*>();
    } else {
      CheckEntrypointTypes<kQuickInitializeType, void*, uint32_t, ArtMethod*>();
    }
...
class LoadStringSlowPathARM64 : public SlowPathCodeARM64 {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    CheckEntrypointTypes<kQuickResolveString, void*, uint32_t, ArtMethod*>();
...
void CodeGeneratorARM64::GenerateStaticOrDirectCall(HInvokeStaticOrDirect* invoke, Register temp) {
  ...
  if (!invoke->IsRecursive()) {
    ...
    __ Ldr(temp, HeapOperand(temp, ArtMethod::DexCacheResolvedMethodsOffset()));
    // temp = temp[index_in_cache];
    __ Ldr(temp, HeapOperand(temp, index_in_cache));
    // lr = temp->entry_point_from_quick_compiled_code_;
    __ Ldr(lr, HeapOperand(temp, ArtMethod::EntryPointFromQuickCompiledCodeOffset(
        kArm64WordSize)));
    // lr();
    __ Blr(lr);
  } else {
...
void InstructionCodeGeneratorARM64::VisitLoadClass(HLoadClass* cls) {
  ...
  if (cls->IsReferrersClass()) {
    ...
    __ Ldr(out, HeapOperand(out, ArtMethod::DeclaringClassOffset()));
  } else {
    DCHECK(cls->CanCallRuntime());
    codegen_->LoadCurrentMethod(out);
    __ Ldr(out, HeapOperand(out, ArtMethod::DexCacheResolvedTypesOffset()));
