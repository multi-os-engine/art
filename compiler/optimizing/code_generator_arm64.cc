...
arm64::Arm64ManagedRegister Location::AsArm64() const {
  // TODO(ARM64): Fix me.
  // The register allocator is not aware of types, so we cannot infer the type
  // of register required from the location. Anyway, for now, we cannot allocate
  // floating-point registers, so we produce an X register. In the arm64 codegen
  // code, the helpers will take care of producing the correct register type by
  // looking at the instruction type.
  return arm64::Arm64ManagedRegister(arm64::Arm64ManagedRegister::kXRegisterType,
                                     reg().RegId());
}
...
static Register RegisterFrom(const Location location, Arm64ManagedRegister::RegisterType type);
static Register RegisterFrom(const Location location, Primitive::Type type);
static Register OutputRegister(HInstruction* instr);
static Register InputRegisterAt(HInstruction* instr, int input_index);
...
CodeGeneratorARM64::CodeGeneratorARM64(HGraph* graph)
    : CodeGenerator(graph, kNumberOfAllocatableRegisters),
...
void CodeGeneratorARM64::SetupBlockedRegisters(bool* blocked_registers) const {
  ...
  while (!reserved_core_registers.IsEmpty()) {
    blocked_registers[reserved_core_registers.PopLowestIndex().code()] = true;
  }
}

ManagedRegister CodeGeneratorARM64::AllocateFreeRegister(Primitive::Type type,
  ...
  ssize_t reg = AllocateFreeRegisterInternal(blocked_registers, kNumberOfXRegisters);
  if (reg != -1) {
  ...
  switch (type) {
    ...
    case Primitive::kPrimInt:
    case Primitive::kPrimNot:
      return Arm64ManagedRegister::FromWRegister(static_cast<WRegister>(reg));
    case Primitive::kPrimLong:
      return Arm64ManagedRegister::FromXRegister(static_cast<XRegister>(reg));
    case Primitive::kPrimFloat:
      return Arm64ManagedRegister::FromSRegister(static_cast<SRegister>(reg));
    case Primitive::kPrimDouble:
      return Arm64ManagedRegister::FromDRegister(static_cast<DRegister>(reg));
    default:
      LOG(FATAL) << "UNREACHABLE";
  }
...
void InstructionCodeGeneratorARM64::VisitInvokeStatic(HInvokeStatic* invoke) {
  Register temp = RegisterFrom(invoke->GetLocations()->GetTemp(0),
                               Arm64ManagedRegister::kWRegisterType);
...
void InstructionCodeGeneratorARM64::VisitInvokeVirtual(HInvokeVirtual* invoke) {
  ...
  vixl::Register temp = RegisterFrom(locations->GetTemp(0), Arm64ManagedRegister::kXRegisterType);
...
static Register RegisterFrom(const Location location, Arm64ManagedRegister::RegisterType type) {
  ...
  Arm64ManagedRegister mreg = location.AsArm64();
  mreg.SetRegType(type);
  DCHECK_EQ(mreg.IsGPRegister(), location.AsArm64().IsGPRegister());
  return mreg.AsVixlRegister();
}

static Register RegisterFrom(const Location location, Primitive::Type type) {
  return RegisterFrom(location, Arm64ManagedRegister::RegTypeFor(type));
}
...
static Location LocationFrom(const Register& reg) {
  return Location::RegisterLocation(Arm64ManagedRegister::FromVixlReg(reg));
}
