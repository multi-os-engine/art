...
void InstructionCodeGeneratorARM64::GenerateReferenceLoadOneRegister(HInstruction* instruction,
                                                                     ...
                                                                     uint32_t offset,
                                                                     Location temp) {
  Register out_reg = RegisterFrom(out, Primitive::kPrimNot);
  if (kEmitCompilerReadBarrier) {
    if (kUseBakerReadBarrier) {
      ...
      // read barrier below.
      __ Mov(RegisterFrom(temp, Primitive::kPrimNot), out_reg);
      // /* HeapReference<Object> */ out = *(out + offset)
...
void InstructionCodeGeneratorARM64::GenerateReferenceLoadTwoRegisters(HInstruction* instruction,
                                                                      ...
                                                                      Location temp) {
  Register out_reg = RegisterFrom(out, Primitive::kPrimNot);
  Register obj_reg = RegisterFrom(obj, Primitive::kPrimNot);
  if (kEmitCompilerReadBarrier) {
    if (kUseBakerReadBarrier) {
      // Load with fast path based Baker's read barrier.
...
void InstructionCodeGeneratorARM64::GenerateGcRootFieldLoad(HInstruction* instruction,
  ...
  if (kEmitCompilerReadBarrier) {
    if (kUseBakerReadBarrier) {
      ...
      // TODO: Use a temporary register from a VIXL pool instead of `ip1`?
      Register temp = ip1.W();
...
void CodeGeneratorARM64::GenerateReferenceLoadWithBakerReadBarrier(HInstruction* instruction,
  ...
  __ Ldr(temp_reg, HeapOperand(obj, monitor_offset));
  if (needs_null_check) {
    MaybeRecordImplicitNullCheck(instruction);
  }
  ...
  __ And(temp_reg, temp_reg, Operand(LockWord::kReadBarrierStateMask));
  static_assert(
  ...
  // ip1 = rb_state & ~LockWord::kReadBarrierStateMask = 0
  __ Bic(temp2, temp_reg, Operand(LockWord::kReadBarrierStateMask));
  // obj is unchanged by this operation, but its value now depends on
  // ip1, which depends on temp_reg.
  ...
  __ Cmp(temp_reg, ReadBarrier::gray_ptr_);
