...
void LocationsBuilderARM64::VisitArm64BitwiseNegatedRight(HArm64BitwiseNegatedRight* instr) {
  DCHECK(Primitive::IsIntegralType(instr->GetType()));
  LocationSummary* locations = new (GetGraph()->GetArena()) LocationSummary(instr);
  locations->SetInAt(0, Location::RequiresRegister());
...
void InstructionCodeGeneratorARM64::VisitArm64BitwiseNegatedRight(HArm64BitwiseNegatedRight* instr) {
  Register dst = OutputRegister(instr);
  Register lhs = InputRegisterAt(instr, 0);
  Register rhs = InputRegisterAt(instr, 1);

  switch (instr->GetOpKind()) {
    case HInstruction::kAnd: __ Bic(dst, lhs, rhs); break;
    case HInstruction::kOr:  __ Orn(dst, lhs, rhs); break;
    case HInstruction::kXor: __ Eon(dst, lhs, rhs); break;
