...
static constexpr uint32_t kPackedSwitchJumpTableThreshold = 8;
...
void JumpTableARM64::EmitTable(CodeGeneratorARM64* codegen) {
  ...
  for (uint32_t i = 0; i < num_entries; i++) {
    ...
    ptrdiff_t jump_offset = target_label->location() - jump_base_.location();
...
void InstructionCodeGeneratorARM64::VisitPackedSwitch(HPackedSwitch* switch_instr) {
  ...
  // ADR has a limited range(+/-1Mb), so make sure we don't emit it if the target may run out of
  // range. 1Mb is 250k arm64 assembly instructions. Current instruction id is an upper bound of
  // the number of HIRs in the graph. We conservatively set the threshold as 10k to ensure the
  // table label can be encoded in ADR.
  ...
  static constexpr int32_t kJumpTableInstructionThreshold = 10000;
  ...
  if (num_entries < kPackedSwitchJumpTableThreshold ||
    ...
    Register jump_offset = table_base.W();
    __ Ldr(jump_offset, MemOperand(table_base, index, UXTW, 2));

    Register target_address = index.X();
    // Jump to target block by branching to base(pc related) + offset.
    __ Bind(jump_table->GetJumpBaseLabel());
    __ Adr(target_address, jump_table->GetJumpBaseLabel());
    __ Add(target_address, target_address, Operand(jump_offset, SXTW));
