...
void ParallelMoveResolverARM64::PrepareForEmitNativeCode() {
  ...
  // VIXL uses at most 1 GPR. While there are 2 GPR and 1 FPR VIXL temps. And there should be no
  // interacted cycles on ARM64. So we always have 1 GPR + 1 FPR available VIXL temps to resolve the
  // dependency.
...
Location ParallelMoveResolverARM64::AllocateScratchLocation(Location loc) {
