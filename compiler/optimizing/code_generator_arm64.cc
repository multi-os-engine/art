...
inline Condition ARM64FPCondition(IfCondition cond) {
  switch (cond) {
    case kCondEQ: return eq;  // Equal.
    case kCondNE: return ne;  // Not equal, or unordered.
    case kCondLT: return lt;  // Less than, or unordered.
    case kCondLE: return le;  // Less than, equal, or unordered.
    case kCondGT: return hi;  // Greater than, or unordered.
    case kCondGE: return hs;  // Greater than, equal, or unordered.
  }
  LOG(FATAL) << "Unreachable";
  UNREACHABLE();
}
  ...
  switch (instruction->InputAt(0)->GetType()) {
    ...
    case Primitive::kPrimDouble: {
      ...
      if (locations->InAt(1).IsConstant()) {
        if (kIsDebugBuild) {
          HInstruction* right = locations->InAt(1).GetConstant();
          DCHECK((right->IsFloatConstant() && (right->AsFloatConstant()->GetValue() == 0.0f)) ||
                  (right->IsDoubleConstant() && (right->AsDoubleConstant()->GetValue() == 0.0)));
        }
        // 0.0 is the only immediate that can be encoded directly in an FCMP instruction.
      ...
      vixl::Label true_label, false_label, done_label;
      GenerateFPJumps(instruction, &true_label, &false_label);
...
void InstructionCodeGeneratorARM64::GenerateFPJumps(HCondition* cond,
                                                    vixl::Label* true_label,
                                                    vixl::Label* false_label) {
  bool gt_bias = cond->IsGtBias();
  IfCondition if_cond = cond->GetCondition();
  Condition ccode = ARM64FPCondition(if_cond);

  switch (if_cond) {
    case kCondEQ:
      if (!gt_bias) {
        __ B(vs, false_label);  // VS for unordered.
      }
      break;
    case kCondNE:
      if (!gt_bias) {
        __ B(vs, true_label);  // VS for unordered.
      }
      break;
    case kCondLT:
      if (gt_bias) {
    ...
    switch (type) {
      ...
      case Primitive::kPrimDouble: {
        ...
        if (condition->GetLocations()->InAt(1).IsConstant()) {
          if (kIsDebugBuild) {
            HInstruction* rhs = condition->GetLocations()->InAt(1).GetConstant();
            DCHECK((rhs->IsFloatConstant() && (rhs->AsFloatConstant()->GetValue() == 0.0f)) ||
                   (rhs->IsDoubleConstant() && (rhs->AsDoubleConstant()->GetValue() == 0.0)));
          }
          // 0.0 is the only immediate that can be encoded directly in an FCMP instruction.
          __ Fcmp(lhs, 0.0);
        } else {
