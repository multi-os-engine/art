...
void InstructionCodeGeneratorARM64::VisitLoadClass(HLoadClass* cls) {
  ...
  if (cls->IsReferrersClass()) {
    ...
    __ Ldr(out, HeapOperand(current_method, ArtMethod::DeclaringClassOffset()));
  } else {
    DCHECK(cls->CanCallRuntime());
    __ Ldr(out, HeapOperand(current_method, ArtMethod::DexCacheResolvedTypesOffset()));
