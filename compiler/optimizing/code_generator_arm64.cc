...
void CodeGeneratorARM64::MoveLocation(Location destination, Location source, Primitive::Type type) {
  ...
  if (destination.IsRegister() || destination.IsFpuRegister()) {
    ...
    if (source.IsRegister() || source.IsFpuRegister()) {
      ...
      CPURegister temp;
      if (src_cst->IsIntConstant() || src_cst->IsLongConstant()) {
        temp = src_cst->IsIntConstant() ? temps.AcquireW() : temps.AcquireX();
      } else {
        DCHECK(src_cst->IsFloatConstant() || src_cst->IsDoubleConstant());
        temp = src_cst->IsFloatConstant() ? temps.AcquireS() : temps.AcquireD();
      }
