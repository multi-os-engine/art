...
void HGraphBuilder::PotentiallyAddSuspendCheck(HBasicBlock* target, uint32_t dex_pc) {
  ...
  if (last_in_target != nullptr && (last_in_target->IsReturn() || last_in_target->IsReturnVoid())) {
    return;
  }
  ...
  HInstruction* last_instruction = current_block_->GetLastInstruction();
  if ((last_instruction == nullptr) || !last_instruction->IsSuspendCheck()) {
