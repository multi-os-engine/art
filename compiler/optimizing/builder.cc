...
void HGraphBuilder::InitializeLocals(uint16_t count) {
  ...
  for (int i = 0; i < count; i++) {
    HLocal* local = new (arena_) HLocal(i, entry_block_->GetDexPc());
...
bool HGraphBuilder::BuildGraph(const DexFile::CodeItem& code_item) {
  ...
  exit_block_->AddInstruction(new (arena_) HExit(exit_block_->GetLastDexPc()));
  // Add the suspend check to the entry block.
  entry_block_->AddInstruction(new (arena_) HSuspendCheck(0));
  entry_block_->AddInstruction(new (arena_) HGoto(entry_block_->GetLastDexPc()));
...
void HGraphBuilder::MaybeUpdateCurrentBlock(size_t dex_pc) {
  ...
  if (current_block_ != nullptr) {
    ...
    current_block_->AddInstruction(new (arena_) HGoto(block->GetLastDexPc()));
...
template<typename T>
void HGraphBuilder::Unop_12x(const Instruction& instruction, Primitive::Type type, uint32_t dex_pc) {
...
template<typename T>
void HGraphBuilder::Binop_12x_shift(const Instruction& instruction, Primitive::Type type, uint32_t dex_pc) {
...
void HGraphBuilder::BuildReturn(const Instruction& instruction, Primitive::Type type, uint32_t dex_pc) {
...
void HGraphBuilder::UpdateLocal(int register_index, HInstruction* instruction, uint32_t dex_pc) const {
  HLocal* local = GetLocalAt(register_index);
  current_block_->AddInstruction(new (arena_) HStoreLocal(local, instruction, dex_pc));
}

HInstruction* HGraphBuilder::LoadLocal(int register_index, Primitive::Type type, uint32_t dex_pc) const {
  HLocal* local = GetLocalAt(register_index);
