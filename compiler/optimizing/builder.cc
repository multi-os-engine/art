...
void HGraphBuilder::BuildSwitchJumpTable(const SwitchTable& table,
  ...
  current_block_->AddInstruction(new (arena_) HSwitch(starting_key, num_entries, value, dex_pc));
...
void HGraphBuilder::BuildPackedSwitch(const Instruction& instruction, uint32_t dex_pc) {
  ...
  // This simplification is currently supported on x86_64.
  // TODO: Implement it for other backends.
  InstructionSet instruction_set = graph_->GetInstructionSet();
  if (num_entries > kSmallSwitchThreshold &&
        (instruction_set == kX86_64 || instruction_set == kX86)) {
