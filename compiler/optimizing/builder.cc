...
void HGraphBuilder::InsertTryBoundaryBlocks(const DexFile::CodeItem& code_item) {
  ...
  // Keep a map of all try blocks and their respective TryItems. We use the block
  // id as key to make sure iteration over the container is deterministic.
  ArenaSafeMap<uint32_t, const DexFile::TryItem*> try_block_info(
      std::less<uint32_t>(), arena_->Adapter());

  // Obtain TryItem information for blocks with throwing instructions.
