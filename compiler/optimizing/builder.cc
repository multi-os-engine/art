...
void HGraphBuilder::InsertTryBoundaryBlocks(const DexFile::CodeItem& code_item) {
  ...
  for (size_t block_id = 1, e = graph_->GetBlocks().Size(); block_id < e; ++block_id) {
    ...
    // Exit block has not been added to the graph by this point.
    DCHECK_NE(try_block, exit_block_);
    // Blocks with TryBoundary are added at the end and not iterated over.
  ...
  while (code_ptr < code_end) {
    ...
    if (instruction.IsBranch()) {
      ...
      for (size_t i = 0; i < num_entries; ++i) {
        ...
        // Create a block for the switch-case logic. It gets the dex_pc of the
        // SWITCH instruction because its part of its semantics.
