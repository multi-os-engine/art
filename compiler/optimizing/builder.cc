...
bool HGraphBuilder::BuildInvoke(const Instruction& instruction,
  ...
  if (optimized_invoke_type == kVirtual) {
    ...
    // TODO: Try to factor common code with
    // art::HGraphBuilder::BuildStaticFieldAccess.
    if (optimized_invoke_type == kStatic) {
      // TODO: Try to prune (or not to emit) clinit checks
      // requirements when they are not needed.  E.g. when the
      // declaring class is the same as/a super class for the
      // declaring class of the calling method, it must have been
      // initialized.  (See also what happens in code generators
      // regarding implicit clinit checks).
