...
mirror::Class* HGraphBuilder::ResolveReferencedClassOfMethod(uint16_t method_idx) {
...
ArtMethod* HGraphBuilder::ResolveMethod(uint16_t method_idx, InvokeType invoke_type) {
  ...
  if (invoke_type == kSuper) {
    Handle<mirror::Class> methods_class(hs.NewHandle(ResolveReferencedClassOfMethod(method_idx)));
    bool default_methods_enabled = Runtime::Current()->AreExperimentalFlagsEnabled(
        ExperimentalFlags::kDefaultMethods);
    if (compiling_class.Get() == nullptr || methods_class.Get() == nullptr) {
      ...
      if (actual_method != resolved_method &&
        ...
        // TODO: AFAICT the back-end code generator relies on this check in order to ensure that it
        //       will not attempt to read the dex_cache with a dex_method_index that is not from the
        //       correct dex_file. If we didn't do this check then the dex_method_index will not be
        //       updated in the builder, which means that the code-generator (and compiler driver
        //       during sharpening and inliner, maybe) might invoke an incorrect method.
