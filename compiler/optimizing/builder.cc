...
// Returns true iff `block` has only one instruction and that instruction is
// a TryBoundary. Note that Builder emits a TryBoundary only when splitting an
// edge and always by itself.
...
bool HGraphBuilder::BuildInvoke(const Instruction& instruction,
  ...
  latest_result_ = latest_result_cursor_ = invoke;
...
void HGraphBuilder::BuildFilledNewArray(uint32_t dex_pc,
  ...
  latest_result_cursor_ = current_block_->GetLastInstruction();
}
