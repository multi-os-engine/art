...
void HGraphBuilder::Binop_23x(const Instruction& instruction,
      Primitive::Type type, uint32_t dex_pc) {
  HInstruction* first = LoadLocal(instruction.VRegB(), type);
  HInstruction* second = LoadLocal(instruction.VRegC(), type);
...
void HGraphBuilder::BuildCheckedDiv(uint16_t out_dalvik_reg,
                                    uint16_t first_dalvik_reg,
                                    int64_t second_dalvik_reg_or_constant,
                                    uint32_t dex_pc,
                                    Primitive::Type type,
                                    bool second_is_constant) {
  DCHECK(type == Primitive::kPrimInt || type == Primitive::kPrimLong);

  HInstruction* first = LoadLocal(first_dalvik_reg, type);
  HInstruction* second = nullptr;
  if (second_is_constant) {
    if (type == Primitive::kPrimInt) {
      second = GetIntConstant(second_dalvik_reg_or_constant);
    } else {
      second = GetLongConstant(second_dalvik_reg_or_constant);
    }
  } else {
    second = LoadLocal(second_dalvik_reg_or_constant, type);
  }
  ...
  UpdateLocal(out_dalvik_reg, current_block_->GetLastInstruction());
}
