...
void HGraphBuilder::PotentiallyAddSuspendCheck(int32_t target_offset, uint32_t dex_pc) {
  ...
  // If this is a backward jump to the return block, there's no need to add
  // a suspend check. This is a useful optimization because the return block
  // tends to be placed at the beginning of the file.
  HBasicBlock* target = FindBlockStartingAt(dex_pc + target_offset);
  DCHECK(target != nullptr);
