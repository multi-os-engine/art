...
bool HGraphBuilder::InitializeParameters(uint16_t number_of_parameters) {
  if (dex_compilation_unit_ == nullptr) return true;
...
void HGraphBuilder::binop_22s(const Instruction& instruction, bool reverse) {
  ...
  HInstruction* second;
  if (reverse) {
    first = GetConstant(instruction.VRegC_22s());
    second = LoadLocal(instruction.VRegB());
  } else {
    second = GetConstant(instruction.VRegC_22s());
    first = LoadLocal(instruction.VRegB());
  }
  current_block_->AddInstruction(new (arena_) T(Primitive::kPrimInt, first, second));
  UpdateLocal(instruction.VRegA(), current_block_->GetLastInstruction());
}
...
void HGraphBuilder::binop_22b(const Instruction& instruction, bool reverse) {
  ...
  if (reverse) {
    first = LoadLocal(instruction.VRegB());
    second = GetConstant(instruction.VRegC_22b());
  } else {
    second = LoadLocal(instruction.VRegB());
    first = GetConstant(instruction.VRegC_22b());
  }
  current_block_->AddInstruction(new (arena_) T(Primitive::kPrimInt, first, second));
  UpdateLocal(instruction.VRegA(), current_block_->GetLastInstruction());
}
