...
void HGraphBuilder::InsertTryBoundaryBlocks(const DexFile::CodeItem& code_item) {
  ...
  // Scan blocks and obtain TryItem information for blocks with throwing instructions.
  // NOTE: We're appending new blocks inside the loop, so we need to use index
  ...
  for (size_t i = 0, e = graph_->GetBlocks().size(); i < e; ++i) {
    ...
    bool has_throwing_instructions = false;
    for (HInstructionIterator insn(block->GetInstructions()); !insn.Done(); insn.Advance()) {
      if (insn.Current()->CanThrow()) {
        has_throwing_instructions = true;
        break;
      }
    }
    if (!has_throwing_instructions) {
      continue;
    }

    // Catch blocks are always considered an entry point into the TryItem in
    // order to avoid splitting exceptional edges. We split the block after the
    // move-exception (if present) and mark the first part non-throwing. Later
    // on, a TryBoundary will be inserted between the two blocks.
    ...
    // Try to find a TryItem covering the block. Store the information if found.
    DCHECK_NE(throwing_block->GetDexPc(), kNoDexPc)
        << "Throwing block must have a dex_pc to find its TryItem.";
    int32_t try_item_idx = DexFile::FindTryItem(code_item, throwing_block->GetDexPc());
    if (try_item_idx != -1) {
      const DexFile::TryItem* try_item = DexFile::GetTryItems(code_item, try_item_idx);
      try_block_info.Put(throwing_block->GetBlockId(), try_item);
    }
  }
  ...
  for (auto entry : try_block_info) {
    ...
    if (needs_try_entry) {
      ...
      try_block->SplitAtTop()->AddInstruction(try_entry);
  ...
  for (auto entry : try_block_info) {
    HBasicBlock* try_block = graph_->GetBlock(entry.first);
    // NOTE: Do not use iterators because SplitEdge replaces the successor.
    for (size_t i = 0, e = try_block->GetSuccessors().size(); i < e; ++i) {
      HBasicBlock* successor = try_block->GetSuccessor(i);

      // If the successor is a try block, all of its predecessors must be covered
      // by the same TryItem. Otherwise the previous pass would have created
      // a non-throwing boundary block.
      if (GetTryItem(successor, try_block_info) != nullptr) {
        DCHECK_EQ(entry.second, GetTryItem(successor, try_block_info));
        continue;
      }

      // Preserve the the invariant that Return(Void) always jumps to Exit by
      // moving it outside the try block if necessary.
