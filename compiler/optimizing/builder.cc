...
void HGraphBuilder::ComputeBranchTargets(const uint16_t* code_ptr,
  ...
  while (code_ptr < code_end) {
    ...
    if (instruction.IsBranch()) {
      ...
      size_t offset = instruction.Opcode() == Instruction::PACKED_SWITCH ? 1 : num_entries;
...
bool HGraphBuilder::BuildPackedSwitch(const Instruction& instruction, uint32_t dex_pc) {
  ...
  uint16_t num_entries = table.GetNumEntries();
  // On zero cases just punt. This is mostly a safety thing, as this should be caught by the
  // verifier.
  if (num_entries == 0) {
    return false;
  }
