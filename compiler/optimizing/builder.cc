...
static int ParseInsn(const uint16_t* code_ptr, DecodedInstruction* decoded_instruction) {
...
HGraph* HGraphBuilder::BuildGraph(const uint16_t* code_ptr, const uint16_t* code_end) {
  ...
  while (code_ptr < code_end) {
    int width = ParseInsn(code_ptr, &insn);
    if (!AnalyzeDexInstruction(insn)) return nullptr;
    code_ptr += width;
  }
...
bool HGraphBuilder::AnalyzeDexInstruction(const DecodedInstruction& instruction) {
