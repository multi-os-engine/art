...
ArtMethod* HGraphBuilder::ResolveMethod(uint16_t method_idx, InvokeType invoke_type) {
  ...
  if (invoke_type == kSuper) {
    if (compiling_class.Get() == nullptr) {
      // Invoking a super method requires knowing the actual super class. Bail out if at AOT
      // time we could not resolve the method's declaring class.
  ...
  // Check incompatible class change. The class linker has a fast path for looking into
  // the dex cache and does not check incompatible class change if it hits it.
...
HClinitCheck* HGraphBuilder::ProcessClinitCheckForInvoke(
  ...
  Handle<mirror::DexCache> outer_dex_cache(hs.NewHandle(
      outer_compilation_unit_->GetClassLinker()->FindDexCache(
          self, *outer_compilation_unit_->GetDexFile())));
  ...
  if (outer_class.Get() != nullptr &&
      !outer_class->IsInterface() &&
