...
void HGraphBuilder::MaybeUpdateCurrentBlock(int index) {
  ...
  if (current_block_ != nullptr) {
    ...
    current_block_->AddInstruction(new (arena_) HGoto(current_block_));
...
// Find all instructions that start a new block, and populate branch_targets_ with
...
bool HGraphBuilder::AnalyzeDexInstruction(const Instruction& instruction, int32_t dex_offset) {
  ...
  switch (instruction.Opcode()) {
    ...
    case Instruction::IF_EQ: {
      // TODO(ngeoffray): Read the dex register.
      HBasicBlock* target = FindBlockStartingAt(instruction.GetTargetOffset() + dex_offset);
      current_block_->AddInstruction(new (arena_) HIf(current_block_));
