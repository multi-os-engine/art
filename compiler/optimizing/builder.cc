...
static bool IsTryBoundaryBlock(HBasicBlock* block) {
  bool is_boundary = block->GetLastInstruction() != nullptr
                     && block->GetLastInstruction()->IsTryBoundary();
  DCHECK(!is_boundary || block->GetFirstInstruction() == block->GetLastInstruction());
  return is_boundary;
}

// static bool IsTryBoundaryToHandlerEdge(HBasicBlock* predecessor, HBasicBlock* block) {
//   bool is_handler_edge =
//       block->IsCatchBlock()
//       && IsTryBoundaryBlock(predecessor)
//       && predecessor->GetLastInstruction()->AsTryBoundary()->HasExceptionHandler(block);

//   if (kIsDebugBuild) {
//     if (is_handler_edge) {
//       // We need to verify that `block` is not the normal-flow successor of the
//       // TryBoundary block. If it was, we would know that these two blocks
//       // *could* represent a handler edge but we would not be sure of it.
//       // The caller of this function must make sure that it is not called on


//       // THIS IS WRONG - CAN HAPPEN!!!



//       // HTryBoundary* try_boundary = predecessor->GetLastInstruction()->AsTryBoundary();
//       // DCHECK_NE(block, try_boundary->GetNormalFlowSuccessor());
//     }
//   }

//   return is_handler_edge;
// }

bool HGraphBuilder::IsBlockOutsidePcRange(HBasicBlock* block, uint32_t pc_start, uint32_t pc_end) {
  ...
  for (size_t idx = 0; idx < code_item.tries_size_; ++idx) {
    ...
    for (uint32_t inner_pc = try_start; inner_pc < try_end; ++inner_pc) {
      ...
      for (size_t i = 0; i < block->GetPredecessors().Size(); ++i) {
        ...
        if (IsTryBoundaryBlock(predecessor)) {
          ...
          if (try_boundary->GetNormalFlowSuccessor() == block
            ...
            // handler. As exception flow, we skip it.
          ...
          // split yet.
          try_boundary = new (arena_) HTryBoundary();
          try_boundary->SetIsTryEntry();
      ...
      for (size_t i = 0; i < block->GetSuccessors().Size(); ++i) {
        ...
        HTryBoundary* try_boundary;
        if (IsTryBoundaryBlock(successor)) {
...
bool HGraphBuilder::BuildInvoke(const Instruction& instruction,
  ...
  latest_result_ = latest_cursor_ = invoke;
...
void HGraphBuilder::BuildFilledNewArray(uint32_t dex_pc,
  ...
  latest_cursor_ = current_block_->GetLastInstruction();
}
...
#define IF_XX(comparison, cond) \
    ...
    case Instruction::MOVE_RESULT_OBJECT: {
      ...
      // `latest_cursor_` holds the instruction after which the MoveResult
      // should be inserted.

      HStoreLocal* update_local = new (arena_) HStoreLocal(GetLocalAt(instruction.VRegA()),
                                                           latest_result_);
      latest_cursor_->GetBlock()->InsertInstructionAfter(update_local, latest_cursor_);
      latest_result_ = latest_cursor_ = nullptr;
      break;
    }
