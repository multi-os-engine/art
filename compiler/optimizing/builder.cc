...
bool HGraphBuilder::BuildFieldAccess(const Instruction& instruction,
                                     uint32_t dex_offset,
                                     bool is_put) {
  uint32_t source_or_dest = instruction.VRegA_22c();
  uint32_t obj = instruction.VRegB_22c();
  ...
  HInstruction* object = LoadLocal(obj, Primitive::kPrimNot);
  ...
  if (is_put) {
    ...
    HInstruction* value = LoadLocal(source_or_dest, resolved_field->GetTypeAsPrimitiveType());
    ...
    UpdateLocal(source_or_dest, current_block_->GetLastInstruction());
  }
  return true;
}
