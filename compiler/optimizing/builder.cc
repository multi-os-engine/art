...
bool HGraphBuilder::HandleStringInit(HInvoke* invoke,
  ...
  invoke->AsInvokeStaticOrDirect()->RemoveLastArgumentOfStringInit();
  current_block_->AddInstruction(invoke);
  uint32_t orig_this_reg = is_range ? register_index : args[0];
  UpdateLocal(orig_this_reg, invoke, invoke->GetDexPc());

  // Retrieve String init map from the verifier and update aliasing vregs.
  const SafeMap<uint32_t, std::set<uint32_t>>& string_init_map =
      verified_method->GetStringInitPcRegMap();
  auto map_it = string_init_map.find(invoke->GetDexPc());
  if (map_it != string_init_map.end()) {
    for (uint32_t reg : map_it->second) {
      UpdateLocal(reg, invoke, invoke->GetDexPc());
    }
  }
