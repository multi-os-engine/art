...
//   One scratch location is enough if one location can not be blocked by multiple moves. But on
//   ARM32, there are register pairs(such as S0,S1) which might cause the issue, i.e. S2->S0,
//   S3->S1, S0,S1->S2,S3. In this case, GetScratchLocationFor() should have the ability to allocate
//   a new scratch register. Also some cleanup code might be needed in FinishEmitNativeCode().
class ParallelMoveResolverNoSwap : public ParallelMoveResolver {
 ...
 protected:
  ...
  // Allocate a scratch location to perform a move from input location. A subclass should implement
  // this to get the best fit location. If there is no suitable physical register, it can also
  // returns a stack slot. In this case, the backend need to make sure it won't pollute the current
  // frame.
  virtual Location AllocateScratchLocation(Location loc) = 0;

  // Called when a move has been performed which takes a scratch location as source. A subclass
