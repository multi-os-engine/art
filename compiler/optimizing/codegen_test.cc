...
template <typename Expected>
static Expected Execute(InstructionSet target_isa, Expected (*f)());
...
static void Run(const InternalCodeAllocator& allocator,
  ...
  Expected result = Execute<Expected>(target_isa, f);
  if (has_result) {
...
static ::std::vector<InstructionSet> GetTargetISAs() {
  ...
  // Add kRuntimeISA to run the test on hardware.
  v.push_back(kRuntimeISA);

  // Add all ISAs that have simulator support.
  const ::std::vector<InstructionSet> simulator_isa_candidates = {
    kArm,
    kArm64,
    kThumb2,
    kX86,
  ...
  for (auto target_isa : simulator_isa_candidates) {
