...
static void RunCodeOptimized(CodeGenerator* codegen, HGraph* graph, bool has_result, int32_t expected) {
...
static void RunCodeOptimized(HGraph* graph, bool has_result, int32_t expected) {
  if (kRuntimeISA == kX86) {
    x86::CodeGeneratorX86 codegenX86(graph);
    RunCodeOptimized(&codegenX86, graph, has_result, expected);
  } else if (kRuntimeISA == kArm || kRuntimeISA == kThumb2) {
    arm::CodeGeneratorARM codegenARM(graph);
    RunCodeOptimized(&codegenARM, graph, has_result, expected);
  } else if (kRuntimeISA == kX86_64) {
    x86_64::CodeGeneratorX86_64 codegenX86_64(graph);
    RunCodeOptimized(&codegenX86_64, graph, has_result, expected);
  }
}
...
TEST(CodegenTest, MateralizedCondition) {
  ArenaPool pool;
