...
class ExecutableMemoryAllocator : public CodeAllocator {
 public:
  ...
  virtual uint8_t* Allocate(size_t size) {
    ...
    mapped_region_.reset(MemMap::MapAnonymous("Executable code", nullptr, size,
                                              PROT_READ | PROT_EXEC | PROT_WRITE, false,
                                              &error_msg));
    CHECK(mapped_region_.get() != nullptr) << error_msg;
    return reinterpret_cast<uint8_t*>(mapped_region_->BaseBegin());
  }

  uint8_t* BaseBegin() const {
    return reinterpret_cast<uint8_t*>(mapped_region_->BaseBegin());
  }

 private:
  UniquePtr<MemMap> mapped_region_;
...
static void TestCode(const uint16_t* data, int length) {
  ...
  typedef void (*fptr)();
#if defined(__i386__)
  reinterpret_cast<fptr>(allocator.BaseBegin())();
#endif
