 ...
 * - SetProperty(x) replaces what is currently held by the context.
...
class HContextualizedPass : public Visitor {
 public:
  ...
  explicit HContextualizedPass(HGraph* graph)
    : Visitor(graph),
      graph_(std::less<HBasicBlock*>(), graph->GetArena()->Adapter()) {}
  ...
  void RunToConvergence() {
    ...
    while (changed) {
      ...
      for (HReversePostOrderIterator it(*Visitor::GetGraph()); !it.Done(); it.Advance()) {
        ...
        MergePredecessors();
        BlockProperties in = *BlockSafeGet(current_block_);
        BeforeBlock(current_block_);
        VisitBasicBlock(current_block_);

        changed = changed || in != *BlockSafeGet(current_block_);
      }
    }
  }
 ...
 protected:
  ...
  void SetProperty(HInstruction* instr, const T& property) {
    BlockProperties* block_props = BlockSafeGet(current_block_);
 ...
 private:
  ...
  void MergePredecessors() {
    ...
    BlockSafeGet(current_block_)->clear();

    for (size_t i = 0, preds_size = preds.Size(); i < preds_size; ++i) {
      const BlockProperties* cur_pred = BlockSafeGet(preds.Get(i));
  ...
  BlockProperties* BlockSafeGet(HBasicBlock* block) {
    typename GraphProperties::const_iterator block_properties = graph_.find(block);
    if (block_properties == graph_.end() || block_properties->second == nullptr) {
      BlockProperties* block_map = new BlockProperties(
          std::less<HInstruction*>(),
          Visitor::GetGraph()->GetArena()->Adapter());
      graph_.Overwrite(block, block_map);
      return block_map;
    }
