...
void CodeGenerator::PotentiallyBlockPhysicalRegisters(Location loc, bool is_output) const {
  // The DCHECKS below check that a register is not specified twice in
  // the summary.
  // Note that fixed output registers are allowed to overlap with input and
  // temp registers.
  if (loc.IsRegister()) {
...
void CodeGenerator::AllocateRegistersLocally(HInstruction* instruction) const {
  ...
  for (size_t i = 0, e = locations->GetInputCount(); i < e; ++i) {
    ...
    PotentiallyBlockPhysicalRegisters(loc, false);
  }
  ...
  PotentiallyBlockPhysicalRegisters(result_location, true);
