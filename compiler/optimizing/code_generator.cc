...
void CodeGenerator::CreateUnresolvedFieldLocationSummary(
    ...
    const FieldAccessCallingConvetion& calling_convetion) {
  ...
  locations->AddTemp(calling_convetion.GetFieldIndexLocation());

  if (is_instance) {
    // Add the `this` object for instance field accesses.
    locations->SetInAt(0, calling_convetion.GetObjectLocation());
  }

  // TODO (fixme): pSetXXStatic/pGetXXStatic always takes/returns an int or int64
  // regardless of the the type.
  if (is_get) {
    if (Primitive::IsFloatingPointType(field_type)) {
      // The return value will be stored in regular registers while HIR expects
      // it in a floating point register. Allocate a temp for it and make
      // the transfer at codegen.
      AddLocationAsTemp(calling_convetion.GetReturnLocation(field_type), locations);
      locations->SetOut(calling_convetion.GetFpuLocation(field_type));
    } else {
      locations->SetOut(calling_convetion.GetReturnLocation(field_type));
    }
  } else {
     ...
     if (Primitive::IsFloatingPointType(field_type)) {
      ...
      AddLocationAsTemp(calling_convetion.GetSetValueLocation(field_type, is_instance), locations);
      locations->SetInAt(set_index, calling_convetion.GetFpuLocation(field_type));
    } else {
      locations->SetInAt(set_index,
          calling_convetion.GetSetValueLocation(field_type, is_instance));
    }
  }
}
