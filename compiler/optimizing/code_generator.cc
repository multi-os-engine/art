...
void CodeGenerator::CompileInternal(CodeAllocator* allocator, bool is_baseline) {
  ...
  for (size_t e = block_order_->size(); current_block_index_ < e; ++current_block_index_) {
    ...
    for (HInstructionIterator it(block->GetInstructions()); !it.Done(); it.Advance()) {
      ...
      RecordNativeDebugInfo(current, current->GetDexPc());
...
size_t CodeGenerator::ComputeNativeDebugStackMapSize() {
  return native_debug_stack_map_stream_.PrepareForFillIn();
}

void CodeGenerator::BuildNativeDebugStackMap(MemoryRegion region) {
  native_debug_stack_map_stream_.FillIn(region);
}

void CodeGenerator::RecordPcInfo(HInstruction* instruction,
                                 uint32_t dex_pc,
                                 SlowPathCode* slow_path) {
  EmitStackMapInfo(stack_map_stream_, instruction, dex_pc, slow_path);
  EmitStackMapInfo(native_debug_stack_map_stream_, instruction, dex_pc, slow_path);
}

void CodeGenerator::RecordNativeDebugInfo(HInstruction* instruction, uint32_t dex_pc) {
  if (compiler_options_.GetGenerateDebugInfo() && dex_pc != kNoDexPc) {
    uint32_t native_pc = GetAssembler()->CodeSize();
    SrcMapElem entry{static_cast<uint32_t>(native_pc), static_cast<int32_t>(dex_pc)};  // NOLINT.
    if (!native_debug_src_map_.empty() && native_debug_src_map_.back().from_ == native_pc) {
      // We generated no code since last entry, so just overwrite the last entry.
      native_debug_src_map_.back() = entry;
    } else {
      native_debug_src_map_.push_back(entry);
    }
    if (instruction->GetLocations() != nullptr) {
      EmitStackMapInfo(native_debug_stack_map_stream_,
                       instruction, dex_pc, nullptr);
    }
  }
}

void CodeGenerator::EmitStackMapInfo(StackMapStream& stack_map_stream,
  ...
  EmitEnvironment(stack_map_stream, instruction->GetEnvironment(), slow_path);
  stack_map_stream.EndStackMapEntry();
}
...
void CodeGenerator::EmitEnvironment(StackMapStream& stack_map_stream,
  ...
  if (environment->GetParent() != nullptr) {
    ...
    EmitEnvironment(stack_map_stream, environment->GetParent(), slow_path);
    stack_map_stream.BeginInlineInfoEntry(environment->GetMethodIdx(),
