...
void CodeGenerator::RecordPcInfo(HInstruction* instruction, uint32_t dex_pc) {
  ...
  for (size_t i = 0; i < environment_size; ++i) {
    ...
    if (current == nullptr) {
      stack_map_stream_.AddDexRegisterEntry(LocationKind::kNone, 0);
      continue;
    }
    ...
    switch (location.GetKind()) {
      case Location::kConstant: {
        ...
        if (current->IsLongConstant()) {
          ...
          stack_map_stream_.AddDexRegisterEntry(LocationKind::kConstant, Low32Bits(value));
          stack_map_stream_.AddDexRegisterEntry(LocationKind::kConstant, High32Bits(value));
          ++i;
          ...
          stack_map_stream_.AddDexRegisterEntry(LocationKind::kConstant, value);
        } else {
      ...
      case Location::kStackSlot: {
        stack_map_stream_.AddDexRegisterEntry(LocationKind::kInStack, location.GetStackIndex());
        break;
      }

      case Location::kDoubleStackSlot: {
        stack_map_stream_.AddDexRegisterEntry(LocationKind::kInStack, location.GetStackIndex());
        stack_map_stream_.AddDexRegisterEntry(LocationKind::kInStack,
      ...
      case Location::kRegister : {
        int id = location.reg();
        stack_map_stream_.AddDexRegisterEntry(LocationKind::kInRegister, id);
      ...
      case Location::kFpuRegister : {
        int id = location.reg();
        stack_map_stream_.AddDexRegisterEntry(LocationKind::kInFpuRegister, id);
      ...
      case Location::kFpuRegisterPair : {
        stack_map_stream_.AddDexRegisterEntry(LocationKind::kInFpuRegister, location.low());
        stack_map_stream_.AddDexRegisterEntry(LocationKind::kInFpuRegister, location.high());
        ++i;
        DCHECK_LT(i, environment_size);
        break;
      }
      ...
      case Location::kRegisterPair : {
        stack_map_stream_.AddDexRegisterEntry(LocationKind::kInRegister, location.low());
        stack_map_stream_.AddDexRegisterEntry(LocationKind::kInRegister, location.high());
        ++i;
        DCHECK_LT(i, environment_size);
        break;
      }
