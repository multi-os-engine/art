...
void CodeGenerator::BuildNativeGCMap(
  ...
  for (size_t i = 0; i < descriptors_.Size(); i++) {
    uint32_t native_offset = descriptors_.Get(i).native_pc;
    if (native_offset > max_native_offset) {
      max_native_offset = native_offset;
    }
  }

  GcMapBuilder builder(data, descriptors_.Size(), max_native_offset, dex_gc_map.RegWidth());
  for (size_t i = 0; i < descriptors_.Size(); i++) {
    struct PcDescriptor descriptor = descriptors_.Get(i);
    uint32_t native_offset = descriptor.native_pc;
    uint32_t dex_pc = descriptor.dex_pc;
...
void CodeGenerator::BuildMappingTable(std::vector<uint8_t>* data) const {
  ...
  uint32_t pc2dex_entries = descriptors_.Size();
  ...
  for (size_t i = 0; i < pc2dex_entries; i++) {
    struct PcDescriptor descriptor = descriptors_.Get(i);
    pc2dex_data_size += UnsignedLeb128Size(descriptor.native_pc - pc2dex_offset);
    pc2dex_data_size += SignedLeb128Size(descriptor.dex_pc - pc2dex_dalvik_offset);
    pc2dex_offset = descriptor.native_pc;
    pc2dex_dalvik_offset = descriptor.dex_pc;
  }
  ...
  for (size_t i = 0; i < pc2dex_entries; i++) {
    struct PcDescriptor descriptor = descriptors_.Get(i);
    DCHECK(pc2dex_offset <= descriptor.native_pc);
    write_pos = EncodeUnsignedLeb128(write_pos, descriptor.native_pc - pc2dex_offset);
    write_pos = EncodeSignedLeb128(write_pos, descriptor.dex_pc - pc2dex_dalvik_offset);
    pc2dex_offset = descriptor.native_pc;
    pc2dex_dalvik_offset = descriptor.dex_pc;
  }
  ...
  if (kIsDebugBuild) {
    ...
    for (size_t i = 0; i < pc2dex_entries; i++) {
      struct PcDescriptor descriptor = descriptors_.Get(i);
      CHECK_EQ(descriptor.native_pc, it.NativePcOffset());
      CHECK_EQ(descriptor.dex_pc, it.DexPc());
      ++it;
    }
