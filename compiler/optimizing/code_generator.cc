...
void CodeGenerator::CompileBaseline(CodeAllocator* allocator, bool is_leaf) {
  ...
  ComputeFrameSize(GetGraph()->GetNumberOfLocalVRegs()
                     ...
                     + 1 /* filler */,
                   0, /* no saving registers at slow path */
...
void CodeGenerator::SaveLiveRegisters(LocationSummary* locations) {
  ...
  for (size_t i = 0, e = GetNumberOfCoreRegisters(); i < e; ++i) {
    if (register_set->ContainsCoreRegister(i)) {
      size_t stack_offset = first_register_slot_in_slow_path_ + count * GetWordSize();
      ++count;
      SaveCpuRegister(Location::StackSlot(stack_offset), i);
...
void CodeGenerator::RestoreLiveRegisters(LocationSummary* locations) {
  ...
  for (size_t i = 0, e = GetNumberOfCoreRegisters(); i < e; ++i) {
    if (register_set->ContainsCoreRegister(i)) {
      size_t stack_offset = first_register_slot_in_slow_path_ + count * GetWordSize();
      ++count;
      RestoreCpuRegister(Location::StackSlot(stack_offset), i);
    }
  }
