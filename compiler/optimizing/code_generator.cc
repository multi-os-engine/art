...
static void CheckLoopEntriesCanBeUsedForOsr(const HGraph& graph,
  ...
  ArenaVector<size_t> covered(graph.GetArena()->Adapter(kArenaAllocMisc));
  for (size_t i = 0; i < loop_headers.size(); ++i) {
    covered.push_back(0);
  }
  ...
  for (size_t i = 0; i < covered.size(); ++i) {
    size_t covered_check = covered[i];
    HSuspendCheck* check = loop_headers[i];
    DCHECK_NE(covered_check, 0u) << "Loop in compiled code has no dex branch equivalent";
    DCHECK_GE(covered_check, check->GetBlock()->GetLoopInformation()->NumberOfBackEdges())
        << "Loop in compiled code has more back edges than in dex code";
  }
}
...
void CodeGenerator::RecordPcInfo(HInstruction* instruction,
  ...
  if (instruction->IsSuspendCheck()) {
    ...
    if (info != nullptr &&
        info->GetSuspendCheck() == instruction &&
        graph_->IsCompilingOsr() &&
        inlining_depth == 0) {
      // We duplicate the stack map as a marker that this stack map can be an OSR entry.
      // Duplicating it avoids avoids having the runtime recognize and skip an OSR stack map.
