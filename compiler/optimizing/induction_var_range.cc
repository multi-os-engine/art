...
bool InductionVarRange::CanGenerateCode(HInstruction* context,
                                        HInstruction* instruction,
                                        bool* top_test) {
  return GenerateCode(context, instruction, nullptr, nullptr, nullptr, top_test);
}
      ...
      *top_test = trip->operation != HInductionVarAnalysis::kTripCountInLoop;
      return
          ((info->induction_class == HInductionVarAnalysis::kInvariant) ||
              GenerateCode(info, trip, preheader, graph, lower, in_body, /* is_min */ true)) &&
          GenerateCode(info, trip, preheader, graph, upper, in_body, /* is_min */ false);
    }
  }
  return false;
}
  ...
  if (info != nullptr) {
    ...
    switch (info->induction_class) {
      case HInductionVarAnalysis::kInvariant:
        ...
        switch (info->operation) {
          case HInductionVarAnalysis::kAdd:
            if (GenerateCode(info->op_a, trip, preheader, graph, &opa, in_body, is_min) &&
                GenerateCode(info->op_b, trip, preheader, graph, &opb, in_body, is_min)) {
              if (graph != nullptr) {
                *result = Insert(preheader, new (graph->GetArena()) HAdd(type, opa, opb));
              }
              return true;
            }
            break;
          case HInductionVarAnalysis::kSub:  // second reversed!
            if (GenerateCode(info->op_a, trip, preheader, graph, &opa, in_body, is_min) &&
                GenerateCode(info->op_b, trip, preheader, graph, &opb, in_body, !is_min)) {
              if (graph != nullptr) {
                *result = Insert(preheader, new (graph->GetArena()) HSub(type, opa, opb));
              }
              return true;
            }
            break;
          case HInductionVarAnalysis::kNeg:  // second reversed!
            if (GenerateCode(info->op_b, trip, preheader, graph, &opb, in_body, !is_min)) {
              if (graph != nullptr) {
                *result = Insert(preheader, new (graph->GetArena()) HNeg(type, opb));
              }
              return true;
            }
            break;
          ...
          case HInductionVarAnalysis::kTripCountInLoop:
            if (!in_body && !is_min) {  // one extra!
              return GenerateCode(info->op_b, trip, preheader, graph, result, in_body, is_min);
            }
            FALLTHROUGH_INTENDED;
          case HInductionVarAnalysis::kTripCountInBody:
            if (is_min) {
      ...
      case HInductionVarAnalysis::kLinear:
        ...
        if (GetConstant(info->op_a, &value)) {
          if (value == 1 || value == -1) {
            ...
            if (GenerateCode(trip,       trip, preheader, graph, &opa, in_body, is_min_a) &&
                GenerateCode(info->op_b, trip, preheader, graph, &opb, in_body, is_min)) {
              if (graph != nullptr) {
                *result = Insert(preheader, new (graph->GetArena()) HAdd(type, opa, opb));
              }
