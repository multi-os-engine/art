...
/** Returns true if 32-bit addition can be done safely (and does not denote unknown range). */
...
/** Returns true if 32-bit subtraction can be done safely (and does not denote unknown range). */
...
/** Returns true if 32-bit multiplication can be done safely (and does not denote unknown range). */
...
/** Returns true if 32-bit division can be done safely (and does not denote unknown range). */
...
InductionVarRange::Value InductionVarRange::GetMinInduction(HLoopInformation* loop, HInstruction* instruction) {
  if (IsInduction(loop, instruction)) {
    // pass induc only if not in header!
    HInductionVarAnalysis::InductionInfo* induc = induction_->LookupInfo(loop, loop->GetHeader()->GetLastInstruction());
    if (induc != nullptr)
      induc = induction_->CreateInvariantOp(HInductionVarAnalysis::kNop, induc, induc);
    Value v = GetMin(induction_->LookupInfo(loop, instruction), induc);
    std::cout << "BIK MIN " << instruction->GetId() << " = "
            << HInductionVarAnalysis::InductionToString(induction_->LookupInfo(loop, instruction))
            << " yields " <<  v.ToString() << std::endl;
    return v;
  }
  return Value(INT_MIN);
}

InductionVarRange::Value InductionVarRange::GetMaxInduction(HLoopInformation* loop, HInstruction* instruction) {
  if (IsInduction(loop, instruction)) {
    // pass induc only if not in header!
    HInductionVarAnalysis::InductionInfo* induc = induction_->LookupInfo(loop, loop->GetHeader()->GetLastInstruction());
    if (induc != nullptr)
      induc = induction_->CreateInvariantOp(HInductionVarAnalysis::kNop, induc, induc);
    Value v = GetMax(induction_->LookupInfo(loop, instruction), induc);
    std::cout << "BIK MAX " << instruction->GetId() << " = "
             << HInductionVarAnalysis::InductionToString(induction_->LookupInfo(loop, instruction))
            << " yields " <<  v.ToString() << std::endl;
    return v;
  }
      ...
      case HInductionVarAnalysis::kInvariant:
        ...
        switch (info->operation) {
          ...
          case HInductionVarAnalysis::kFetch:
            // Represent any int within a valid range as constant. Otherwise, simply the instruction.
