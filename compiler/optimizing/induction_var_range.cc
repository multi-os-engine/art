...
InductionVarRange::Value InductionVarRange::GetLinear(HInductionVarAnalysis::InductionInfo* info,
  ...
  // analysis (e.g. finding max a * (TC - 1) + O for a == 1 and TC = U - O) to avoid losing
  // information with an intermediate result that only incorporates a single instruction.
  if (trip != nullptr && trip->op_a->operation == HInductionVarAnalysis::kSub) {
    int32_t min_value = 0;
    int32_t stride_value = 0;
    if (IsConstantRange(info->op_a, &min_value, &stride_value) && min_value == stride_value) {
      if (!is_min && stride_value == 1) {
        // Test original trip's negative operand (trip->op_a->op_b) against the offset of the linear induction.
        if (HInductionVarAnalysis::InductionEqual(trip->op_a->op_b, info->op_b)) {
          // Analyze cancelled trip with just the positive operand (trip->op_a->op_a).
          HInductionVarAnalysis::InductionInfo cancelled_trip(
              trip->induction_class, trip->operation, trip->op_a->op_a, trip->op_b, nullptr);
          return GetVal(&cancelled_trip, trip, in_body, is_min);
        }
      } else if (is_min && stride_value == -1) {
        // Test original trip's positive operand (trip->op_a->op_a) against the offset of the linear induction.
        if (HInductionVarAnalysis::InductionEqual(trip->op_a->op_a, info->op_b)) {
          // Analyze cancelled trip with just the negative operand (trip->op_a->op_b).
