...
InductionVarRange::Value InductionVarRange::GetMinInduction(HInstruction* context,
  ...
  if (loop != nullptr) {
    return GetVal(induction_analysis_->LookupInfo(loop, instruction), GetTripCount(loop, context), true);
  }
  return Value();
}
...
InductionVarRange::Value InductionVarRange::GetMaxInduction(HInstruction* context,
  ...
  if (loop != nullptr) {
    return SimplifyMax(
        GetVal(induction_analysis_->LookupInfo(loop, instruction), GetTripCount(loop, context), false));
  }
  return Value();
}
...
InductionVarRange::Value InductionVarRange::GetMul(HInductionVarAnalysis::InductionInfo* info1,
                                                   ...
                                                   HInductionVarAnalysis::InductionInfo* trip,
                                                   bool is_min) {
  Value v1_min = GetVal(info1, trip, true);
  Value v1_max = GetVal(info1, trip, false);
  Value v2_min = GetVal(info2, trip, true);
  Value v2_max = GetVal(info2, trip, false);
...
InductionVarRange::Value InductionVarRange::MergeVal(Value v1, Value v2, bool is_min) {
  if (v1.is_known && v2.is_known) {
    if (v1.instruction == v2.instruction && v1.a_constant == v2.a_constant) {
      return is_min ? Value(v1.instruction, v1.a_constant, std::min(v1.b_constant, v2.b_constant))
                    : Value(v1.instruction, v1.a_constant, std::max(v1.b_constant, v2.b_constant));
    }
  }
  return Value();
}
