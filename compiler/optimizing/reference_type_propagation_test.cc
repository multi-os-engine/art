...
class ReferenceTypePropagationTest : public CommonCompilerTest {
 public:
  ...
  ReferenceTypeInfo MergeTypes(const ReferenceTypeInfo& a,
    ...
    return propagation_->MergeTypes(graph_, a, b);
  }
  ...
  ReferenceTypeInfo ObjectType(bool is_exact = true) SHARED_REQUIRES(Locks::mutator_lock_) {
    return graph_->CreateReferenceTypeInfo(propagation_->handle_cache_.GetObjectClassHandle(), is_exact);
  }
  ...
  ReferenceTypeInfo StringType(bool is_exact = true) SHARED_REQUIRES(Locks::mutator_lock_) {
    return graph_->CreateReferenceTypeInfo(propagation_->handle_cache_.GetStringClassHandle(), is_exact);
  }
  ...
  ReferenceTypeInfo ThrowableType(bool is_exact = true) SHARED_REQUIRES(Locks::mutator_lock_) {
    return graph_->CreateReferenceTypeInfo(propagation_->handle_cache_.GetThrowableClassHandle(), is_exact);
  }
...
TEST_F(ReferenceTypePropagationTest, CreateErroneousType) {
  ...
  // Trying to obtain an erroneous type yields the inexact object type.
  ReferenceTypeInfo t(ThrowableType());
  EXPECT_TRUE(t.IsValid());
  EXPECT_FALSE(t.IsExact());
  EXPECT_TRUE(t.IsEqual(ObjectType(false)));
}
