...
class LiveInterval : public ArenaObject {
 public:
  ...
  void AddUse(HInstruction* instruction) {
    ...
    } else if (ranges_.Peek().start_ == start_block_position) {
      // There is a use later in the same block. 
      DCHECK_LE(position, ranges_.Peek().end_);
    } else if (ranges_.Peek().start_ == end_block_position + 1) {
      // Last use is in a following block.
      LiveRange existing = ranges_.Pop();
      ranges_.Add(LiveRange(start_block_position, existing.end_));
    } else {
  ...
  void AddRange(size_t start, size_t end) {
    ...
    } else if (ranges_.Peek().start_ == end + 1) {
      // There is a use in the following block.
      LiveRange existing = ranges_.Pop();
      ranges_.Add(LiveRange(start, existing.end_));
    } else {
      // There is a hole in the interval. Create a new range.
      ranges_.Add(LiveRange(start, end));
    }
  ...
  void AddLoopRange(size_t start, size_t end) {
    ...
    while (!ranges_.IsEmpty() && ranges_.Peek().end_ < end) {
      DCHECK_LE(start, ranges_.Peek().start_);
      ranges_.Pop();
    }
    if (ranges_.IsEmpty()) {
      ...
      ranges_.Add(LiveRange(start, range.end_));
    }
  }
  ...
  void SetFrom(size_t from) {
    DCHECK(!ranges_.IsEmpty());
    LiveRange existing = ranges_.Pop();
    ranges_.Add(LiveRange(from, existing.end_));
  }
...
class SsaLivenessAnalysis : public ValueObject {
 public:
  explicit SsaLivenessAnalysis(const HGraph& graph)
      : graph_(graph),
        ...
        instructions_(graph.GetArena(), graph.GetBlocks().Size() << 1),
  ...
  HInstruction* GetInstructionBySsaIndex(size_t index) {
 ...
 private:
  ...
  GrowableArray<HInstruction*> instructions_;
