...
class LiveInterval : public ArenaObject<kArenaAllocMisc> {
 ...
 private:
  ...
  bool HasSynthesizeUseAt(size_t position) const {
    ...
    void AddUseInternal(HInstruction* instruction,
                      size_t input_index,
                      bool is_environment,
                      size_t position,
                      bool is_in_loop,
                      bool keep_alive = false) {
    if (!is_environment && is_in_loop) {
    ...
    if (instruction->IsPhi()) {
      // Phi uses do no need to update live ranges: the liveness analysis algorithm
      // takes care of them.
      return;
    }
