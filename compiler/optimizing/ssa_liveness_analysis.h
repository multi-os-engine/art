...
class LiveInterval : public ArenaObject<kArenaAllocMisc> {
 ...
 private:
  ...
  LiveRange* FindRangeForPosition(size_t position) {
    ...
    // If this method was called earlier on a lower position, use that result
    // as a starting point. If the algorithm calls this method on non-sequential
    // positions, we have to search the entire range list.
    LiveRange* current;
