...
class LiveInterval : public ArenaObject<kArenaAllocMisc> {
 public:
  ...
  bool Covers(size_t position) {
    LiveRange* search_start = use_cache ? cached_range_search_ : first_range_;
  ...
  size_t FirstIntersectionWith(LiveInterval* other) const {
    ...
    LiveRange* my_range;
    if (use_cache) {
      my_range = FindRangeAtOrAfter(other->GetStart(), cached_range_search_);
    } else {
  ...
  void AdvanceCache(size_t position) {
    DCHECK(position >= debug_cache_position_ || IsFixed());

    cached_range_search_ = FindRangeAtOrAfter(position, cached_range_search_);
    if (kIsDebugBuild) {
      debug_cache_position_ = position;
    }
  }
 ...
 private:
  LiveInterval(ArenaAllocator* allocator,
      ...
      : allocator_(allocator),
        ...
        last_range_(nullptr),
        debug_cache_position_(SIZE_MAX),
