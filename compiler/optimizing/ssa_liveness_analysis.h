...
class LiveInterval : public ArenaObject<kArenaAllocMisc> {
 public:
  ...
  // Only ranges at or after the current linear scan position (set with
  // AdvanceScanPosition) are tested. Use Covers_Slow if `position` may
  // be smaller than that and performance is not an issue.
  bool Covers(size_t position) const {
    return FindRangeAt(position, range_search_start_) != nullptr;
  }

  // Same as Covers but tests all ranges, regardless of the position
  // currently inspected by linear scan.
  ...
  LiveInterval* SplitAt(size_t position) {
    ...
    ResetScanPosition();
    new_interval->ResetScanPosition();
    return new_interval;
  }
