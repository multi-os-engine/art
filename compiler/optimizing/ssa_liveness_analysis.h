...
class LiveInterval : public ArenaObject<kArenaAllocSsaLiveness> {
 public:
  ...
  // `environment` is not null and as register use otherwise. If `real_user` is
  // specified, the use will be recorded at its lifetime position.
  void AddUse(HInstruction* instruction,
              HEnvironment* environment,
              size_t input_index,
              HInstruction* real_user = nullptr,
              bool keep_alive = false) {
    bool is_environment = (environment != nullptr);
    LocationSummary* locations = instruction->GetLocations();
    if (real_user == nullptr) {
      real_user = instruction;
    }
