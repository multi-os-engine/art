 ...
 public:
  ...
  void AddUse(HInstruction* instruction,
    ...
    if (!is_environment && instruction->IsInLoop()) {
      // Add synthesize uses at the back edge of loops to help the register allocator.
      // The use positions are stored in increasing order. So the outer backedge use needs to be
      // the use having `next_` as the current `first_use_`.
      UsePosition* latest_use = nullptr;
      UsePosition* previous_first_use = first_use_;
      for (HLoopInformationOutwardIterator it(*instruction->GetBlock());
           !it.Done();
           it.Advance()) {
        HLoopInformation* current = it.Current();
        if ((previous_first_use != nullptr)
            && (previous_first_use->GetPosition() <= current->GetSingleBackEdge()->GetLifetimeEnd())) {
          // There was a use already seen in this loop. Therefore the latest loop use
        ...
        UsePosition* new_use = new (allocator_) UsePosition(
            ...
            current->GetSingleBackEdge()->GetLifetimeEnd(), nullptr);

        if (latest_use != nullptr) {
          // Going outward. The latest created use needs to point to the new use.
          latest_use->SetNext(new_use);
        } else {
          // This is the most inner loop.
          DCHECK_EQ(current, instruction->GetBlock()->GetLoopInformation());
          first_use_ = new_use;
        }
        latest_use = new_use;
      }
      // Set the latest created use to be the previously saved first use.
      if (latest_use != nullptr) {
        latest_use->SetNext(previous_first_use);
      }
    }
  ...
  size_t FirstRegisterUseAfter(size_t position) const {
    ...
    if ((position == GetStart()) && IsParent() && DefinitionRequiresRegister()) {
      return position;
    }
