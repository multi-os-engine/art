...
bool PrimitiveTypePropagation::TypePhiFromInputs(HPhi* phi) {
  ...
  for (HInputIterator it(phi); !it.Done(); it.Advance()) {
    ...
    if (input->IsPhi() && input->AsPhi()->IsDead()) {
      // All phis with uses were initially live. If an input is a dead phi, it must
      // have been made dead due to type conflict. Mark this phi conflicting too.
      return false;
    }
    ...
    if (common_type == Primitive::kPrimVoid) {
      ...
      // If there is a conflict, it will be detected during input typing.
      DCHECK(Primitive::IsFloatingPointType(input_type) || input_type == Primitive::kPrimNot);
      common_type = input_type;
    } else if (Primitive::IsIntegralType(input_type)) {
      // Input is integral, common type is not. If there is a conflict, it will
      // be detected during typing of inputs.
...
void PrimitiveTypePropagation::AddDependentInstructionsToWorklist(HInstruction* instruction) {
  // If `instruction` is a dead phi, type conflict was just identified and we
  // add all its live phi users to the worklist. Otherwise we only add those
  // with a different type.
  bool add_all_users = instruction->IsPhi() && instruction->AsPhi()->IsDead();

  for (HUseIterator<HInstruction*> it(instruction->GetUses()); !it.Done(); it.Advance()) {
    HInstruction* user = it.Current()->GetUser();
    if (user->IsPhi() && user->AsPhi()->IsLive()) {
      if (add_all_users || user->GetType() != instruction->GetType()) {
