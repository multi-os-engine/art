...
static void GenNumberOfTrailingZeroes(LocationSummary* locations,
  ...
  if (is64bit) {
    ...
    // Give a in_lo a dummy value to keep the compiler from complaining.
...
void IntrinsicCodeGeneratorMIPS::VisitMemoryPeekShortNative(HInvoke* invoke) {
  ...
  if (codegen_->GetInstructionSetFeatures().IsR6()) {
    ...
    // Avoided using lwr/lwl instructions here because it's possible that
    // the half-word being loaded is in the last two bytes on a page, and the
    // page at the next consecutive address isn't mapped. In that case the
    // lwl instruction would fault, and cause a false failure.
    __ Lb(AT, adr, 0);
    __ Andi(AT, AT, 0xFF);      // Make sure that the byte is zero-extended
...
void IntrinsicCodeGeneratorMIPS::VisitMemoryPokeShortNative(HInvoke* invoke) {
  ...
  if (codegen_->GetInstructionSetFeatures().IsR6()) {
    ...
    // Avoided using swr/swl instructions here because it's possible that
    // the half-word being stored is in the last two bytes on a page, and the
    // page at the next consecutive address isn't mapped. In that case the
    // swl instruction would fault, and cause a false failure.
    __ Sb(val, adr, 0);
    __ Srl(AT, val, 8);
    __ Sb(AT, adr, 1);
  }
