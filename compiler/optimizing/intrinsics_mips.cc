...
void IntrinsicCodeGeneratorMIPS::VisitMemoryPeekShortNative(HInvoke* invoke) {
  ...
  if (IsR6()) {
    ...
    __ Lbu(AT, adr, 0);  // This byte should be zero-extended.  If it's not
                         // the "or" instruction below will destroy the upper
                         // 24 bits of the final result.
    __ Lb(out, adr, 1);  // This byte should be sign-extended.
    __ Sll(out, out, 8);
    __ Or(out, out, AT);
  }
}
...
void IntrinsicCodeGeneratorMIPS::VisitMemoryPokeShortNative(HInvoke* invoke) {
  ...
  if (IsR6()) {
    ...
    // Avoided using swr/swl instructions here because they would change
    // four bytes of memory. This function should change only two bytes
    // of memory.
    __ Sb(val, adr, 0);
    __ Srl(AT, val, 8);
    __ Sb(AT, adr, 1);
