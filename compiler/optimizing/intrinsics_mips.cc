...
static void GenNumberOfTrailingZeroes(LocationSummary* locations,
  ...
  if (is64bit) {
    ...
    //     return 64;
    // }
    __ LoadConst32(out, 64);
    __ Or(AT, in_lo, in_hi);
    __ Beqz(AT, &done);
    ...
    if (isR6) {
      __ Seleqz(AT, in_hi, in_lo);
      __ Selnez(TMP, in_lo, in_lo);
      __ Or(out, AT, TMP);
    } else {
      __ Move(TMP, in_lo);
      __ Movz(TMP, in_hi, in_lo);
      __ Move(out, TMP);
    }
  ...
  if (invoke->InputAt(1)->IsIntConstant()) {
    ...
    if (type == Primitive::kPrimInt) {
      ...
      if (shift == 0) {
        ...
        __ Sll(AT, in_lo, shift);
        __ Srl(out_lo, in_hi, 32-shift);
        __ Or(out_lo, out_lo, AT);
        __ Sll(AT, in_hi, shift);
        __ Srl(out_hi, in_lo, 32-shift);
        __ Or(out_hi, out_hi, AT);
      } else {
        __ Srl(AT, in_lo, 64-shift);
        __ Sll(out_lo, in_hi, shift-32);
        __ Or(out_lo, out_lo, AT);
        __ Srl(AT, in_hi, 64-shift);
        __ Sll(out_hi, in_hi, shift-32);
        __ Or(out_hi, out_hi, AT);
      }
    }
  } else {      // !invoke->InputAt(1)->IsIntConstant()
    ...
    if (type == Primitive::kPrimInt) {
      ...
      __ Sltu(TMP, TMP, AT);
      __ Beqz(TMP, &gt32);
      __ Sll(out_lo, in_lo, shamt);
      __ Sll(out_hi, in_hi, shamt);
      __ Subu(TMP, AT, shamt);
      __ Srl(AT, in_hi, TMP);
      __ Or(out_lo, out_lo, AT);
      __ Srl(AT, in_lo, TMP);
      ...
      __ LoadConst32(TMP, 64);
      __ Subu(TMP, TMP, shamt);
      __ Srl(out_lo, in_lo, TMP);
      __ Srl(out_hi, in_hi, TMP);
      __ Addiu(TMP, shamt, -32);
      __ Sll(AT, in_hi, TMP);
      __ Or(out_lo, out_lo, AT);
      __ Sll(AT, in_lo, TMP);
