...
void IntrinsicCodeGeneratorMIPS::VisitMemoryPeekShortNative(HInvoke* invoke) {
  ...
  if (IsR6()) {
    ...
    // Unlike for words, there are no lhl/lhr to load unaligned halfwords so
    // the code loads individual bytes, in case the address isn't
    // halfword-aligned, and assembles them into a signed halfword.
