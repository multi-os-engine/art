...
void IntrinsicLocationsBuilderARM::VisitStringCompareTo(HInvoke* invoke) {
  ...
  locations->SetInAt(0, Location::RequiresRegister());
  locations->SetInAt(1, Location::RequiresRegister());
  // Using instruction cbz requires a low register.
  locations->AddTemp(Location::RegisterLocation(R1));
  locations->AddTemp(Location::RequiresRegister());
  locations->AddTemp(Location::RequiresRegister());
  locations->SetOut(Location::RegisterLocation(R0), Location::kOutputOverlap);
}
...
void IntrinsicCodeGeneratorARM::VisitStringCompareTo(HInvoke* invoke) {
  ...
  // out = length diff.
  __ sub(out, temp2, ShifterOperand(temp1));
  // temp0 = min(len(str), len(arg)).
  __ cmp(temp2, ShifterOperand(temp1));
  ...
  // Unrolled loop to compare 2x16-bit characters at a time (ok because of string data alignment).
  __ Bind(&loop);
  ...
  __ rbit(temp1, temp1);
  __ clz(temp1, temp1);
  // If the number of 16-bit chars remaining <= the index where the difference occurs (0-1), then
