...
void IntrinsicCodeGeneratorARM::VisitStringCompareTo(HInvoke* invoke) {
  ...
  // temp0 = number of 16-bit characters remaining to compare, on the interval [-1, temp0]
  // (it could be < 1 if a difference is found after the first SUB in the comparison loop, and
  // after the end of the strsng data. This shouldn't happen because string data is zero-padded,
