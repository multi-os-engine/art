...
void IntrinsicCodeGeneratorARM::VisitStringEquals(HInvoke* invoke) {
  ...
  DCHECK(IsAligned<4>(value_offset));
  DCHECK(IsAligned<4>(kObjectAlignment));

  // Load the value field's offset plus the length of the string in bytes (twice the number of
  // UTF-16 characters) into a register.
  __ Lsl(temp, temp, 1);
  __ add(temp1, temp, ShifterOperand(value_offset));
  // Reset temp1 back one position (2 bytes in a UTF-16-encoded string) to the offset of the last
  // character of the string.
  __ sub(temp1, temp1, ShifterOperand(2));
  // Adjust result to account for even/odd string lengths.
  __ bic(temp1, temp1, ShifterOperand(2));

  // Loop to compare strings 2 characters at a time starting at the end of the string.
  // Ok to do this because strings with an odd length are zero-padded, handled above.
  __ Bind(&loop);
