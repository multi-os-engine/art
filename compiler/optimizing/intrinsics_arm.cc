...
static void GenLongRotate(LocationSummary* locations,
  ...
  if (rhs.IsConstant()) {
    ...
    // (e.g. left *or* right by -2 bits == 62 bits in the same direction.)
    uint32_t rot = rhs.GetConstant()->AsIntConstant()->GetValue() & 0x3F;
    ...
    Register rot = locations->GetTemp(0).AsRegister<Register>();
    Register tmp = locations->GetTemp(1).AsRegister<Register>();
    Label end;
    Label right;

    __ mov(tmp, ShifterOperand(is_left ? -1 : 1));

    // Map to a +ve. in-range rotation.
    __ and_(rot, rhs.AsRegister<Register>(), ShifterOperand(0x3F));

    // For rotates over a word in size, flip dir and invert distance.
    __ cmp(rot, ShifterOperand(0x20));
    __ it(GT, kItThen);
    __ rsb(tmp, tmp, ShifterOperand(0), GT);
    __ rsb(rot, rot, ShifterOperand(0x40), GT);
    __ cmp(tmp, ShifterOperand(0));
    __ b(&right, GT);

    // out_reg_hi = (reg_hi << rot) | (reg_lo >> inv_rot).
    // out_reg_lo = (reg_lo << rot) | (reg_hi >> inv_rot).
    __ rsb(tmp, rot, ShifterOperand(0x20));
    __ Lsl(out_reg_hi, in_reg_hi, rot);
    __ Lsr(out_reg_lo, in_reg_lo, tmp);
    __ orr(out_reg_hi, out_reg_hi, ShifterOperand(out_reg_lo));
    __ Lsl(out_reg_lo, in_reg_lo, rot);
    __ Lsr(rot, in_reg_hi, tmp);
    __ orr(out_reg_lo, out_reg_lo, ShifterOperand(rot));
    __ b(&end);
    ...
    __ rsb(tmp, rot, ShifterOperand(0x20));
    __ Lsr(out_reg_hi, in_reg_hi, rot);
    __ Lsl(out_reg_lo, in_reg_lo, tmp);
    __ orr(out_reg_hi, out_reg_hi, ShifterOperand(out_reg_lo));
    __ Lsr(out_reg_lo, in_reg_lo, rot);
    __ Lsl(rot, in_reg_hi, tmp);
    __ orr(out_reg_lo, out_reg_lo, ShifterOperand(rot));
