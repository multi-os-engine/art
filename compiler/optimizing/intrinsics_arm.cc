...
void IntrinsicCodeGeneratorARM::VisitStringNewStringFromBytes(HInvoke* invoke) {
  ArmAssembler* assembler = GetAssembler();
  LocationSummary* locations = invoke->GetLocations();
  LOG(INFO) << " arm - GENERATING INTRINSIC STRING FROM BYTES";
...
void IntrinsicCodeGeneratorARM::VisitStringNewStringFromChars(HInvoke* invoke ATTRIBUTE_UNUSED) {
  ArmAssembler* assembler = GetAssembler();
  LOG(INFO) << " arm - GENERATING INTRINSIC STRING FROM CHARS";
...
void IntrinsicCodeGeneratorARM::VisitStringNewStringFromString(HInvoke* invoke) {
  ArmAssembler* assembler = GetAssembler();
  LocationSummary* locations = invoke->GetLocations();
  LOG(INFO) << " arm - GENERATING INTRINSIC STRING FROM STRING";
  ...
  __ LoadFromOffset(
      kLoadWord, LR, TR, QUICK_ENTRYPOINT_OFFSET(kArmWordSize, pAllocStringFromString).Int32Value());
  __ blx(LR);
  __ Bind(slow_path->GetExitLabel());
}
