...
void IntrinsicCodeGeneratorARM::VisitStringEquals(HInvoke* invoke) {
  ...
  // Load the value offset plus the length of the string in bytes (twice the number of UTF-16
  // characters) into a register. Adjust back one position (2 bytes in a UFT-16-encoded string) to
  // the offset of the last character of the string.
  __ Lsl(temp, temp, 1);
  __ add(temp1, temp, ShifterOperand(value_offset - 2));
