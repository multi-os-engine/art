...
static DRegister GetDRegister(Location loc) {
  if (loc.IsFpuRegister()) {
    return loc.AsFpuRegister<DRegister>();
  } else {
    DCHECK(loc.IsFpuRegisterPair());
    SRegister sreg = loc.AsFpuRegisterPairLow<SRegister>();
    DCHECK_EQ(static_cast<int>(sreg) % 2, 0);
    return static_cast<DRegister>(static_cast<int>(sreg) / 2);
  }
}
...
static void MoveFPToInt(LocationSummary* locations, bool is64bit, ArmAssembler* assembler) {
  ...
  if (is64bit) {
    __ vmovrrd(output.AsRegisterPairLow<Register>(),
               ...
               GetDRegister(input));  // .AsFpuRegister<DRegister>());
  } else {
...
static void MoveIntToFP(LocationSummary* locations, bool is64bit, ArmAssembler* assembler) {
  ...
  Location output = locations->Out();
  if (is64bit) {
    __ vmovdrr(GetDRegister(output),  // output.AsFpuRegister<DRegister>(),
...
static void MathAbsFP(LocationSummary* locations, bool is64bit, ArmAssembler* assembler) {
  ...
  if (is64bit) {
    __ vabsd(GetDRegister(out), GetDRegister(in));
  } else {
...
static void GenAbsInteger(LocationSummary* locations,
  ...
  if (is64bit) {
    ...
    if (out_reg_lo == in_reg_hi) {
      UNIMPLEMENTED(FATAL) << "Diagonal overlap not yet implemented!";
    }
...
static void GenMinMax(LocationSummary* locations,
                      ...
                      ArmAssembler* assembler) {
  if (is_long) {
    // Quick did not implement this. We could try to.
    UNIMPLEMENTED(FATAL);
  }
...
void IntrinsicCodeGeneratorARM::VisitMathSqrt(HInvoke* invoke) {
  ...
  __ vsqrtd(GetDRegister(locations->Out()), GetDRegister(locations->InAt(0)));
}
...
void IntrinsicCodeGeneratorARM::VisitMemoryPeekLongNative(HInvoke* invoke) {
  ...
  // Worst case: SCTLR.A = 0, so can't use ldrd as addr may be unaligned.
...
static void CreateIntIntIntIntToVoid(ArenaAllocator* arena, Primitive::Type type, HInvoke* invoke) {
  LocationSummary* locations = new (arena) LocationSummary(invoke,
                                                           LocationSummary::kNoCall,
                                                           kIntrinsified);
  locations->SetInAt(0, Location::NoLocation());        // Unused receiver.
  ...
  if (type == Primitive::kPrimLong) {
    ...
    // TODO: Refactor so instruction-set-features is available here so we don't always ask
    //       for temps.
    // Note:
...
void IntrinsicLocationsBuilderARM::VisitUnsafePut(HInvoke* invoke) {
  CreateIntIntIntIntToVoid(arena_, Primitive::kPrimInt, invoke);
}
...
void IntrinsicLocationsBuilderARM::VisitUnsafePutObject(HInvoke* invoke) {
  CreateIntIntIntIntToVoid(arena_, Primitive::kPrimNot, invoke);
}
...
void IntrinsicLocationsBuilderARM::VisitUnsafePutLong(HInvoke* invoke ATTRIBUTE_UNUSED) {
  CreateIntIntIntIntToVoid(arena_, Primitive::kPrimLong, invoke);
}
void IntrinsicLocationsBuilderARM::VisitUnsafePutLongOrdered(HInvoke* invoke ATTRIBUTE_UNUSED) {
  CreateIntIntIntIntToVoid(arena_, Primitive::kPrimLong, invoke);
}
void IntrinsicLocationsBuilderARM::VisitUnsafePutLongVolatile(HInvoke* invoke ATTRIBUTE_UNUSED) {
  CreateIntIntIntIntToVoid(arena_, Primitive::kPrimLong, invoke);
}
...
static Register GetRegOrLow(Location l, Primitive::Type type) {
  if (type == Primitive::kPrimLong) {
    return l.AsRegisterPairLow<Register>();
  } else {
    return l.AsRegister<Register>();
  }
}
...
static void GenCas(LocationSummary* locations, Primitive::Type type, CodeGeneratorARM* codegen) {
  ...
  Register expected_lo = GetRegOrLow(locations->InAt(3), type);        // Expected.
  Register value_lo = GetRegOrLow(locations->InAt(4), type);           // Value.
  ...
  if (type == Primitive::kPrimLong) {
    Register tmp_hi = locations->GetTemp(2).AsRegister<Register>();            // Value in memory.
    Register expected_hi = locations->InAt(3).AsRegisterPairHigh<Register>();  // Expected (hi).
    Register value_hi = locations->InAt(4).AsRegisterPairHigh<Register>();     // Value (hi).

    __ ldrexd(tmp_lo, tmp_hi, tmp_ptr);

    __ sub(tmp_lo, tmp_lo, ShifterOperand(expected_lo));
    __ sub(tmp_hi, tmp_hi, ShifterOperand(expected_hi));
    __ orrs(tmp_lo, tmp_lo, ShifterOperand(tmp_hi));

    __ it(EQ, ItState::kItT);
    __ strexd(tmp_lo, value_lo, value_hi, tmp_ptr, EQ);
...
void IntrinsicLocationsBuilderARM::VisitUnsafeCASLong(HInvoke* invoke ATTRIBUTE_UNUSED) {
  // Currently turned off. Needs too many registers. We can hardwire IP and LR, but it was
...
void IntrinsicCodeGeneratorARM::VisitUnsafeCASLong(HInvoke* invoke) {
