...
static void CreateFPToFPCallLocations(ArenaAllocator* arena, HInvoke* invoke) {
  ...
  // registers and fails, so we force it to use specific registers, even though there might
  // be a better solution.
  locations->SetInAt(0, Location::FpuRegisterPairLocation(calling_convention.GetFpuRegisterAt(0),
                                                          calling_convention.GetFpuRegisterAt(1)));
  locations->SetOut(Location::FpuRegisterPairLocation(calling_convention.GetFpuRegisterAt(0),
                                                      calling_convention.GetFpuRegisterAt(1)));
...
static void CreateFPFPToFPCallLocations(ArenaAllocator* arena, HInvoke* invoke) {
  ...
  locations->SetInAt(0, Location::FpuRegisterPairLocation(calling_convention.GetFpuRegisterAt(0),
                                                          calling_convention.GetFpuRegisterAt(1)));
  locations->SetInAt(1, Location::FpuRegisterPairLocation(calling_convention.GetFpuRegisterAt(2),
