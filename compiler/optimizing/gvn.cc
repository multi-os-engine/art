...
class ValueSet : public ArenaObject<kArenaAllocGvn> {
 public:
  ...
  // Erases all values in this set and populates it with values from `to_copy`.
  ...
  // Returns true if `this` has enough buckets so that if `set` is copied into
...
class GlobalValueNumberer : public ValueObject {
 ...
 private:
  ...
  // (b) it can hold a copy of `to_copy`.
  HBasicBlock* FindVisitedBlockWithRecyclableSet(HBasicBlock* block, const ValueSet& to_copy) const;
...
HBasicBlock* GlobalValueNumberer::FindVisitedBlockWithRecyclableSet(HBasicBlock* block,
                                                                    const ValueSet& to_copy) const {
  HBasicBlock* secondary_match = nullptr;

  for (size_t block_id : visited_blocks.Indexes()) {
