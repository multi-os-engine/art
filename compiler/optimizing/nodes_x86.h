...
// X86 version of HSwitch that holds a pointer to the base method address.
class HX86Switch : public HTemplateInstruction<2> {
 public:
  HX86Switch(int32_t start_value,
             int32_t num_entries,
             HInstruction* input,
  ...
  DECLARE_INSTRUCTION(X86Switch);

 private:
  int32_t start_value_;
  int32_t num_entries_;

  DISALLOW_COPY_AND_ASSIGN(HX86Switch);
};
