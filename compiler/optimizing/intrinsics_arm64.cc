...
void IntrinsicCodeGeneratorARM64::VisitStringNewStringFromBytes(HInvoke* invoke) {
  vixl::MacroAssembler* masm = GetVIXLAssembler();
  LocationSummary* locations = invoke->GetLocations();
  LOG(INFO) << " arm_64 - GENERATING INTRINSIC STRING FROM BYTES";
  ...
  __ Ldr(
      lr, MemOperand(tr, QUICK_ENTRYPOINT_OFFSET(kArm64WordSize, pAllocStringFromBytes).Int32Value()));
  __ Blr(lr);
  __ Bind(slow_path->GetExitLabel());
}
...
void IntrinsicCodeGeneratorARM64::VisitStringNewStringFromChars(HInvoke* invoke ATTRIBUTE_UNUSED) {
  vixl::MacroAssembler* masm = GetVIXLAssembler();
  LOG(INFO) << " arm_64 - GENERATING INTRINSIC STRING FROM CHARS";

  __ Ldr(
      lr, MemOperand(tr, QUICK_ENTRYPOINT_OFFSET(kArm64WordSize, pAllocStringFromChars).Int32Value()));
  __ Blr(lr);
}
...
void IntrinsicCodeGeneratorARM64::VisitStringNewStringFromString(HInvoke* invoke) {
  vixl::MacroAssembler* masm = GetVIXLAssembler();
  LocationSummary* locations = invoke->GetLocations();
  LOG(INFO) << " arm_64 - GENERATING INTRINSIC STRING FROM STRING";
  ...
  __ Ldr(
      lr, MemOperand(tr, QUICK_ENTRYPOINT_OFFSET(kArm64WordSize, pAllocStringFromString).Int32Value()));
  __ Blr(lr);
  __ Bind(slow_path->GetExitLabel());
}
