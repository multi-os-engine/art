...
static void GenMathRound(HInvoke* invoke, bool is_double, vixl::MacroAssembler* masm) {
  ...
  //
  // We didn't find a single instruction in ARMv8 that can support above definition.
  ...
  // For most inputs (positive values, zero or NaN), this instruction does the job.
  ...
  // If we use this instruction, for most inputs, more handling code is needed.
