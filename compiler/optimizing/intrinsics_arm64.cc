...
void IntrinsicCodeGeneratorARM64::VisitStringCompareTo(HInvoke* invoke) {
  ...
  // Promote temp0 to an X reg, ready for 64-bit loads.
  temp0 = temp0.X();

  // Loop to compare 4x16-bit characters at a time (ok because string data is zero padded).
  __ Bind(&loop);
  ...
  // Promote temp1 to an X reg, ready for a 64-bit xor.
  temp1 = temp1.X();
  ...
  __ Rbit(temp1, temp1);
  __ Clz(temp1, temp1);
  __ Bic(temp1, temp1, Operand(0xf));
