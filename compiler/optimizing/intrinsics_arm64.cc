...
void IntrinsicCodeGeneratorARM64::VisitStringEqualsLoop(HInvoke* invoke) {
  ...
  Register count = WRegisterFrom(locations->InAt(2));
  Register out = XRegisterFrom(locations->Out());

  UseScratchRegisterScope scratch_scope(masm);
  Register temp = scratch_scope.AcquireX();
  ...
  __ Cbz(count, &return_true);
  // Store offset of string value in preparation for comparison loop
  __ Mov(temp, value_offset);
  ...
  __ Bind(&loop);
  __ Ldr(out, MemOperand(str.X(), temp));
  __ Ldr(temp1, MemOperand(arg.X(), temp));
  __ Add(temp, temp, Operand(sizeof(uint64_t)));
  __ Cmp(out, temp1);
  __ B(&return_false, ne);
  __ Sub(count, count, Operand(4), SetFlags);
  __ B(&loop, gt);
