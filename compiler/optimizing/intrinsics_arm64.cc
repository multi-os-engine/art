...
static void GenMathRound(HInvoke* invoke,
                         bool is_double ,
                         vixl::MacroAssembler* masm) {
  // According to Java 8 API definition, Math.round(double) and Math.round(float) perform:
  ...
  FPRegister tmp_fp = is_double ? temps.AcquireD() : temps.AcquireS();
  vixl::Label done;

  // Round to nearest integer, ties away from zero.
  __ Fcvtas(out_reg, in_reg);

  // If sign bit of result is 0, which means rounding result >= 0, rounding is done.
  ...
  // Handle cases where input is a negative tie,
  // change the rounding direction of the result:
  // from "away from zero" to "toward positive infinity".
  __ Add(out_reg, out_reg, 1);
