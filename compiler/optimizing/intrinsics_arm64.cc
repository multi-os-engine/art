...
static void GenCompare(LocationSummary* locations, bool is_long, vixl::MacroAssembler* masm) {
  ...
  // Selection instructions nicely map to Csinc/sinv on zero register.
  __ Cmp(op1_reg, op2_reg);
  __ Mov(out_reg, 0);                          // out == 0
  __ Csel(out_reg, out_reg, Operand(+1), le);  // out == +1 if GT, unchanged otherwise
  __ Csel(out_reg, out_reg, Operand(-1), ge);  // out == -1 if LT, unchanged otherwise
}
