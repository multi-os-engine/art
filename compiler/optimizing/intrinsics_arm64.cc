...
static void GenSystemArrayCopyAddresses(vixl::MacroAssembler* masm,
  ...
  const int32_t element_offset = Primitive::ComponentSizeShift(type);
  ...
  if (src_pos.IsConstant()) {
    ...
    __ Add(src_base, src_base, Operand(XRegisterFrom(src_pos), LSL, element_offset));
  }
  ...
  if (dst_pos.IsConstant()) {
    ...
    __ Add(dst_base, dst_base, Operand(XRegisterFrom(dst_pos), LSL, element_offset));
  }
  ...
  if (copy_length.IsConstant()) {
    ...
    __ Add(src_end, src_base, Operand(XRegisterFrom(copy_length), LSL, element_offset));
  }
}
