...
void IntrinsicCodeGeneratorARM64::VisitStringEquals(HInvoke* invoke) {
  ...
  const int64_t count_offset = static_cast<int64_t>(mirror::String::CountOffset().Int32Value());
  const int64_t value_offset = static_cast<int64_t>(mirror::String::ValueOffset().Int32Value());
  const int64_t class_offset = static_cast<int64_t>(mirror::Object::ClassOffset().Int32Value());
  ...
  __ Bind(&loop);
  __ Ldr(out.X(), MemOperand(str.X(), temp1.X()));
  __ Ldr(temp2.X(), MemOperand(arg.X(), temp1.X()));
  __ Add(temp1.X(), temp1.X(), Operand(sizeof(uint64_t)));
  __ Cmp(out.X(), temp2.X());
  __ B(&return_false, ne);
