...
class RegisterAllocatorGraphColor : public RegisterAllocatorCommon {
 ...
 private:
  ...
  void ProcessFixedInputLocationsFor(HInstruction* instruction);

  // If an instruction requires its output in a specific registers, create
  // a fixed interval for that output.
  void ProcessFixedOutputLocationFor(HInstruction* instruction);
  ...
  void ProcessCatchPhiFor(HInstruction* instruction);
  ...
  // necessary.
  void ColorInterferenceGraph(size_t num_registers, bool processing_core_regs);
  ...
  //       to better show the dependencies among different functions.
  ...
  // All intervals that will be spilled to the stack, decided by graph coloring.
