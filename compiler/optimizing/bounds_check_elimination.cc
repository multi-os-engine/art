...
class BCEVisitor : public HGraphVisitor {
 public:
  ...
  void VisitBasicBlock(HBasicBlock* block) {
    first_bounds_check_map_.clear();
    HGraphVisitor::VisitBasicBlock(block);
    if (need_to_revisit_block_) {
      AddCondInstructionsWithDeoptimizationFallback(block);
      need_to_revisit_block_ = false;
    }
  }
 ...
 private:
  ...
  void VisitBoundsCheck(HBoundsCheck* bounds_check) {
    ...
    if (!index->IsIntConstant()) {
      ...
      if (first_bounds_check_map_.find(array_length->GetId()) ==
          first_bounds_check_map_.end()) {
        first_bounds_check_map_.Put(array_length->GetId(), bounds_check);
      } else {
  ...
  void AddCondInstructionWithDeoptimizationFallback(HInstruction* array_length,
                                                    HBasicBlock* block) {
    DCHECK(array_length->IsArrayLength());
    ...
    HBoundsCheck* bounds_check =
        first_bounds_check_map_.Get(array_length->GetId())->AsBoundsCheck();
    HCondition* cond = new (GetGraph()->GetArena()) HLessThan(
        array_length, lower_const, true, bounds_check->GetDexPc());
    block->InsertInstructionBefore(cond, bounds_check);
    cond->SetEnvironment(bounds_check->GetEnvironment());
  ...
  void AddCondInstructionsWithDeoptimizationFallback(HBasicBlock* block) {
    for (ArenaSafeMap<int, HBoundsCheck*>::iterator it = first_bounds_check_map_.begin();
         it != first_bounds_check_map_.end();
         ++it) {
      ...
      if (counter >= 2) {
        AddCondInstructionWithDeoptimizationFallback(array_length, block);
      }
    }
  }
  ...
  SafeMap<int, HBoundsCheck*> first_bounds_check_map_;
