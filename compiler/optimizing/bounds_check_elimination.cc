  ...
  bool AddDeoptimization(const ArrayAccessInsideLoopFinder& finder) {
    ...
    if (CanAddDeoptimizationConstant(const_comparing_instruction,
      ...
      if (!is_constant_proven) {
        AddDeoptimizationConstant(const_comparing_instruction, const_compared_to,
            deopt_block, loop_entry_test_block_added);
      }
      if (!is_length_proven) {
        AddDeoptimizationArrayLength(array_length_comparing_instruction, array_length,
            array_length_offset, deopt_block, loop_entry_test_block_added);
      }
      return true;
    }
    return false;
...
class BCEVisitor : public HGraphVisitor {
 public:
  ...
  bool IsAddedBlock(HBasicBlock* block) { return block->GetBlockId() >= initial_block_size_; }
