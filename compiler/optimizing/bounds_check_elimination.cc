  ...
  void AddComparesWithDeoptimization(HBasicBlock* block) {
    for (ArenaSafeMap<int, HBoundsCheck*>::iterator it =
      ...
      if (counter >= kThresholdForAddingDeoptimize) {
        // INT_MAX as an index will definitely throw AIOOBE, we'll keep the bounds checks
        // in place instead of triggering deoptimization.
        if (lower_bound_const_instr->GetValue() != INT_MAX) {
