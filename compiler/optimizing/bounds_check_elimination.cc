...
class BCEVisitor : public HGraphVisitor {
 public:
  ...
  // bother to apply the deoptimization technique since it's likely some AIOOBE will be thrown.
  static constexpr int32_t kMaxLengthForAddingDeoptimize =
  ...
  BCEVisitor(HGraph* graph,
      ...
      : HGraphVisitor(graph),
        ...
        has_block_based_dynamic_bce_(false),
 ...
 private:
  ...
  void VisitBoundsCheck(HBoundsCheck* bounds_check) OVERRIDE {
    ...
    if (try_dynamic_bce) {
      ...
      // Prepare block-based dynamic elimination.
  ...
  /**
    ...
    * }
    *
    * Note: this optimization is no longer applied after block-based dynamic deoptimization
  ...
  void VisitArrayGet(HArrayGet* array_get) OVERRIDE {
    if (!has_block_based_dynamic_bce_ && array_get->IsInLoop()) {
  ...
  // Perform block-based dynamic elimination on suitable set of bounds checks.
  void AddCompareWithDeoptimization(HBasicBlock* block,
    ...
    // and base+3, since we must assume any in between value may occur too.
    ...
    has_block_based_dynamic_bce_ = true;
  }

  // Attempt block-based dynamic elimination on remaining candidates.
  void AddComparesWithDeoptimization(HBasicBlock* block) {
    for (const auto& it : first_index_bounds_check_map_) {
      ...
      for (HUseIterator<HInstruction*> it2(array_length->GetUses()); !it2.Done(); it2.Advance()) {
        ...
        if (user->IsBoundsCheck() && block->Dominates(other_block)) {
          ...
          if (array_length == other_array_length && base == other_value.GetInstruction()) {
            ...
            // Since a subsequent dominates block could be under an conditional, only accept
            // the other bounds check if it is in same block, does not extend the range found
            // so far, or both blocks dominate the exit.
            ...
            if (block == user->GetBlock() || (min_c <= other_c && other_c <= max_c) ||
      ...
      // Perform block-based deoptimization if it seems profitable. Note that we reject cases
      ...
      if (candidates.size() >= threshold &&
          (base != nullptr || min_c >= 0) &&  // reject certain OOB
           distance <= kMaxLengthForAddingDeoptimize) {  // reject likely deopt
  ...
  // Flag that denotes whether block-based dynamic elimination has occurred.
  bool has_block_based_dynamic_bce_;
