...
// Pseudo vreg number for array_length_in_loop_body_if_needed.
static const int kVregNumberArrayLength = -1;
...
class ValueBound : public ValueObject {
 public:
  ...
  static bool Equal(HInstruction* instruction1, HInstruction* instruction2) {
    ...
    if (instruction1->IsArrayLength() && instruction2->IsArrayLength()) {
      return instruction1->InputAt(0) == instruction2->InputAt(0);
    }
  ...
  void Run() {
    ...
    for (HBlocksInLoopIterator it_loop(*loop_info); !it_loop.Done(); it_loop.Advance()) {
      ...
      for (HInstruction* instruction = block->GetFirstInstruction();
        ...
        HInstruction* length_value = index->InputAt(1);
        ...
        if (length_value->IsPhi()) {
          // On the way to be removed already.
          continue;
        }
...
class MonotonicValueRange : public ValueRange {
 public:
  ...
  void TransformLoopForDeoptimization() {
    HBasicBlock* header = induction_variable_->GetBlock();
    DCHECK(header->IsLoopHeader());
    HBasicBlock* pre_header = header->GetLoopInformation()->GetPreHeader();
  ...
  void AddDeoptimizationConstant(HInstruction* value,
    ...
    HGraph* graph = header->GetGraph();
    HBasicBlock* pre_header = header->GetLoopInformation()->GetPreHeader();
    HBasicBlock* deopt_block = pre_header->GetDominator()->GetSuccessors().Get(1);
  ...
  bool CanAddDeoptimizationArrayLength(HInstruction* value,
    ...
    *is_proven = false;
    ...
    if (array_length->GetBlock() == header) {
      // array_length_in_loop_body_if_needed only has right value in the loop body.
      // We bail out in this case. Usually array_length defined in loop header
      // is already hoisted by licm.
      return false;
    } else {
      // array_length is defined either before loop header already, or in loop body.
      // If it's defined in loop body, a phi array_length_in_loop_body_if_needed
  ...
  void AddDeoptimizationArrayLength(HInstruction* value,
    ...
    if (!array_length->GetBlock()->Dominates(deopt_block)) {
      ...
      deopt_block->InsertInstructionBefore(new_array_length,
          deopt_block->GetLastInstruction());

      HPhi* phi = new (graph->GetArena()) HPhi(
          graph->GetArena(), kVregNumberArrayLength, 2, Primitive::kPrimInt);
    ...
    HDeoptimize* deoptimize = new (graph->GetArena())
        HDeoptimize(cond, suspend_check->GetDexPc());
    deopt_block->InsertInstructionBefore(add, deopt_block->GetLastInstruction());
    deopt_block->InsertInstructionBefore(cond, deopt_block->GetLastInstruction());
    deopt_block->InsertInstructionBefore(deoptimize, deopt_block->GetLastInstruction());
    deoptimize->CopyEnvironmentFromWithLoopPhiAdjustment(
        suspend_check->GetEnvironment(), header);
  ...
  bool AddDeoptimization(const ArrayAccessInsideLoopFinder& finder) {
    ...
    HInstruction* instruction = finder.GetFoundArrayLength();
    DCHECK(instruction->IsArrayLength());
    HArrayLength* array_length = instruction->AsArrayLength();
    ...
    if (increment_ == 1) {
      ...
      if (CanAddDeoptimizationConstant(initial_, -offset_low, &is_constant_proven) &&
        ...
        if (!is_constant_proven || !is_length_proven) {
          TransformLoopForDeoptimization();
  ...
  // when the induction variable is at initial_ and end_ must be in legal range.
...
class BCEVisitor : public HGraphVisitor {
 public:
  ...
  explicit BCEVisitor(HGraph* graph)
      : HGraphVisitor(graph),
        // May insert 4 additional blocks for each loop.
        // Each loop has at least a pre-header, header and back edge.
        // Reserve some room.
        maps_(graph->GetBlocks().Size() + graph->GetBlocks().Size() / 3 * 4),
        need_to_revisit_block_(false),
        initial_block_size_(graph->GetBlocks().Size()) {}

  void VisitBasicBlock(HBasicBlock* block) OVERRIDE {
    if (block == last_visited_block_) {
      // Deoptimization can add blocks before loop header
      // and the loop header can appear later again in
      // reverse post order.
      DCHECK(block->IsLoopHeader());
      return;
    }

    if (block->GetBlockId() >= initial_block_size_) {
 ...
 private:
  ...
  void VisitBoundsCheck(HBoundsCheck* bounds_check) {
    ...
    if (array_length->IsPhi()) {
      array_length = array_length->AsPhi()->InputAt(1);
  ...
  // a loop header. The loop header will be rememberd here so that it's not
  // processed again.
  HBasicBlock* last_visited_block_;
