...
class BCEVisitor : public HGraphVisitor {
 public:
  ...
  // Very small/large constant index is considered as an anomaly. This is a threshold
 ...
 private:
  ...
  void AddComparesWithDeoptimization(HBasicBlock* block) {
    for (ArenaSafeMap<int, HBoundsCheck*>::iterator it =
             first_index_bounds_check_map_.begin();
         it != first_index_bounds_check_map_.end();
         ++it) {
      ...
      for (HUseIterator<HInstruction*> it2(array_length->GetUses());
        ...
        if (user->IsBoundsCheck() && block->Dominates(user->GetBlock())) {
      ...
      // Perform block-based deoptimization if it seems profitable.
