...
class BCEVisitor : public HGraphVisitor {
 ...
 private:
  ...
  void AddCompareWithDeoptimization(HBasicBlock* block,
    ...
    // Construct deoptimization on single bound or both bounds on "range".
    // E.g. either for a[0]..a[3] just 3 or for a[base-1]..a[base+3] both base-1 and base+3.
    ...
    // Set new range on the array length. Future comparisons against a symbolic
    // bound are safe due to the two deoptimization tests above.
    ValueRange* range = new (GetGraph()->GetArena())
        ValueRange(GetGraph()->GetArena(), ValueBound(base, max_c + 1), ValueBound::Max());
    AssignRange(block, array_length, range);
  }
  ...
  void AddComparesWithDeoptimization(HBasicBlock* block) {
    for (ArenaSafeMap<int, HBoundsCheck*>::iterator it =
      ...
      // Count invariant indexing for which bounds checks are still there and
      // that are all related as "a[base + constant]" for a fixed invariant base
      ...
      // a[base+1] and a[base+2] are considered as one set, compared to the first).
