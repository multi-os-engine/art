 ...
 private:
  ...
  void VisitArrayGet(HArrayGet* array_get) OVERRIDE {
    if (array_get->IsInLoop() && array_get->GetEnvUses().IsEmpty()) {
      ...
      if (IsLoopInvariant(loop, array_get->InputAt(0)) &&
          IsLoopInvariant(loop, array_get->InputAt(1))) {
   ...
   * deoptimization in case bounds may go out of range. Otherwise, the loop is executed
  ...
  void TryDynamicBCE(HInstruction* instruction) {
    ...
    bool needs_top_test = false;
    if (DynamicBCESeemsProfitable(loop, instruction->GetBlock()) &&
        induction_range_.CanGenerateCode(instruction, index, &needs_top_test) &&
        CanHandleLength(loop, length, needs_top_test)) {
      ...
      // guard against any out of bounds (including arithmetic wrap-around cases).
      HBasicBlock* block = TransformLoopForDeoptimizationIfNeeded(loop, needs_top_test);
      induction_range_.GenerateCode(instruction, index, GetGraph(),  block, &lower, &upper);
      if (lower != nullptr) {
   ...
   */
  bool CanHandleLength(HLoopInformation* loop, HInstruction* length, bool needs_top_test) {
    if (IsLoopInvariant(loop, length)) {
      return true;
    } else if (length->IsArrayLength() &&
               length->GetEnvUses().IsEmpty() &&
               length->GetBlock()->GetLoopInformation() == loop) {
      if (CanHandleNullCheck(loop, length->InputAt(0), needs_top_test)) {
   ...
   */
  bool CanHandleNullCheck(HLoopInformation* loop, HInstruction* check, bool needs_top_test) {
    if (IsLoopInvariant(loop, check)) {
      return true;
  ...
  void HoistOutOfLoop(HLoopInformation* loop, HInstruction* instruction) {
    ...
    auto it = top_tested_loop_.find(loop);
    if (it != top_tested_loop_.end()) {
      block = it->second;
    }
   ...
   *             header
   */
  HBasicBlock* TransformLoopForDeoptimizationIfNeeded(HLoopInformation* loop, bool needs_top_test) {
  ...
  void InsertPhiNodes() {
    ...
    for (auto it1 = top_tested_loop_.begin(); it1 != top_tested_loop_.end(); ++it1) {
  ...
  static bool IsLoopInvariant(HLoopInformation* loop, HInstruction* instruction) {
    ...
    return other_loop != loop && (other_loop == nullptr || !other_loop->IsIn(*loop));
  }
  ...
  // Top tested loop bookkeeping.
  ArenaSafeMap<HLoopInformation*, HBasicBlock*> top_tested_loop_;
