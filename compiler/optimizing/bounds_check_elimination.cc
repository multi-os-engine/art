...
class ArrayAccessInsideLoopFinder {
 public:
  explicit ArrayAccessInsideLoopFinder(HInstruction* induction_variable)
      : induction_variable_(induction_variable),
        array_length_(nullptr),
        offset_low_(INT_MAX),
        offset_high_(INT_MIN) {
    Run();
  }

  HArrayLength* GetArrayLength() const { return array_length_; }
  int32_t GetOffsetLow() const { return offset_low_; }
  int32_t GetOffsetHigh() const { return offset_high_; }

  void Run() {
    HGraph* graph = induction_variable_->GetBlock()->GetGraph();
    HReversePostOrderIterator it(*graph);
    for (; !it.Done(); it.Advance()) {
      HBasicBlock* block = it.Current();
      if (block->GetLoopInformation() !=
          induction_variable_->GetBlock()->GetLoopInformation()) {
        // Doesn't belong to the loop.
        continue;
      }
      ...
      for (HInstruction* instruction = block->GetFirstInstruction();
        ...
        if (array_length_ != nullptr && array_length_ != array_length) {
      ...
      if (block == block->GetLoopInformation()->GetBackEdges().Get(0)) {
        // We've reached the back edge. All blocks inside the loop should've
        // been visited.
        return;
      };
    }
    ...
    if (kIsDebugBuild) {
      // Make sure in reverse post order blocks following back edge aren't
      // in the loop.
      for (; !it.Done(); it.Advance()) {
        DCHECK_NE(it.Current()->GetLoopInformation(),
                  induction_variable_->GetBlock()->GetLoopInformation());
      }
    }
  }
 ...
 private:
  // The instruction that corresponds to an MonotonicValueRange.
  HInstruction* induction_variable_;

  // The array length of the array that's accessed inside the loop.
  HArrayLength* array_length_;

  // The lowest and highest offsets from instruction_ in all array accesses.
...
class MonotonicValueRange : public ValueRange {
 public:
  MonotonicValueRange(ArenaAllocator* allocator,
      ...
      : ValueRange(allocator, ValueBound::Min(), ValueBound::Max()),
        ...
        bound_(bound),
        deopt_instruction_list_(allocator, 0) {}
  ...
  void RemoveDeoptimizationInstructions() {
    for (size_t i = deopt_instruction_list_.Size(); i > 0; i--) {
      HInstruction* instruction = deopt_instruction_list_.Get(i - 1);
      instruction->GetBlock()->RemoveInstruction(instruction);
    }
  }

  // Check that (value >= constant), deoptimize otherwise.
  ...
  bool AddDeoptimizationConstant(HInstruction* value, int32_t constant) {
    ...
    deopt_instruction_list_.Add(cond);
    deopt_instruction_list_.Add(deoptimize);
    return true;
  }

  // Check that (value <= array_length + offset), deoptimize otherwise.
  ...
  bool AddDeoptimizationArrayLength(HInstruction* value,
    ...
    if (!array_length->GetBlock()->Dominates(pre_header)) {
      ...
      // We've already made sure array graph->GetNullConstant()is defined before the loop when collecting
      ...
      if (null_check != nullptr && !null_check->GetBlock()->Dominates(pre_header)) {
        ...
        deopt_instruction_list_.Add(null_check_cond);
        deopt_instruction_list_.Add(null_check_deoptimize);
      }
    ...
    deopt_instruction_list_.Add(add);
    deopt_instruction_list_.Add(cond);
    deopt_instruction_list_.Add(deoptimize);
    return true;
  }
  ...
  // it's not necessary.
  bool AddDeoptimization(ArrayAccessInsideLoopFinder* finder) {
    int32_t offset_low = finder->GetOffsetLow();
    int32_t offset_high = finder->GetOffsetHigh();
    HArrayLength* array_length = finder->GetArrayLength();
    ...
    deopt_instruction_list_.Reset();
    if (increment_ == 1) {
      if (!AddDeoptimizationConstant(initial_, -offset_low)) {
        DCHECK(deopt_instruction_list_.IsEmpty());
        return false;
      }

      int32_t offset = inclusive_ ? -offset_high - 1 : -offset_high;
      if (AddDeoptimizationArrayLength(end_, array_length, offset)) {
        return true;
      }
      ...
      if (AddDeoptimizationConstant(end_, constant)) {
        return true;
      }
    }
  ...
  ValueRange* NarrowWithDeoptimization() {
    ...
    if (finder.GetArrayLength() == nullptr) {
    ...
    ValueBound upper = ValueBound(finder.GetArrayLength(), -1 - finder.GetOffsetHigh());
 ...
 private:
  ...
  GrowableArray<HInstruction*> deopt_instruction_list_;
    ...
    if (cond == kCondLT || cond == kCondLE) {
      if (left_monotonic_range != nullptr) {
        ...
        if (left_monotonic_range->GetInductionVariable() == left &&
            ...
            block == left_monotonic_range->GetInductionVariable()->GetBlock() &&
    ...
    if (instruction->InputAt(0)->IsCondition()) {
      ...
      if (cmp == kCondGT || cmp == kCondGE ||
        ...
        if (left_range->IsMonotonicValueRange() &&
          ...
          ValueRange* new_left_range = LookupValueRange(left, instruction->IfFalseSuccessor());
          if (new_left_range == left_range) {
            ...
            new_left_range = left_range->AsMonotonicValueRange()->NarrowWithDeoptimization();
            if (new_left_range != left_range) {
