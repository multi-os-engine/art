...
class BCEVisitor : public HGraphVisitor {
 ...
 private:
  ...
  /**
    ...
    * may be exposed underneath which can be hoisted out of the loop.
    *
    * for (int i = 0; i < n; i++) {
    *                                <-------+
    *   for (int j = 0; j < n; j++)          |
    ...
    * }
    *
    * TODO: remove restriction on empty env uses, here and below
    */
  void VisitArrayGet(HArrayGet* array_get) OVERRIDE {
    if (array_get->IsInLoop() && array_get->GetEnvUses().IsEmpty()) {
  ...
  bool InductionRangeFitsIn(ValueRange* array_range,
    ...
    if (v1.is_known && (v1.a_constant == 0 || v1.a_constant == 1) &&
      ...
      *try_dynamic_bce = !index_range.GetLower().LessThan(array_range->GetLower()) &&
      ...
      // Use analysis only for static bce if loop is finite.
   ...
   * the bounds check dynamically by means of runtime tests that trigger a deoptimization in
   * case bounds may go out of range (we want to be rather certain of that given the slowdown of
   ...
   */
  void TryDynamicBCE(HInstruction* instruction) {
    ...
    if (DynamicBCESeemsProfitable(loop, instruction->GetBlock()) &&
        induction_range_.CanGenerateCode(instruction, index, &needs_finite_test, &needs_taken_test) &&
        CanHandleInfiniteLoop(loop, index, needs_finite_test) &&
        CanHandleLength(loop, length, needs_taken_test)) {
      HInstruction* lower = nullptr;
      HInstruction* upper = nullptr;
      // Generate:                  if (lower > upper || upper >= length) deopt;
      // or for an invariant, just: if (upper >= length) deopt;
      // By restricting dynamic bce to unit strides only (with a maximum of 32-bit iterations)
      // and by not combining access (e.g. a[i], a[i-3], a[i+5] etc.), these tests correctly
      // guard against any possible OOB (including arithmetic wrap-around cases).
  ...
  bool DynamicBCESeemsProfitable(HLoopInformation* loop, HBasicBlock* block) {
    if (loop != nullptr) {
      // Does loop have early-exits?
      auto it = early_exit_loop_.find(loop);
      if (it != early_exit_loop_.end()) {
        // Loop has been analyzed earlier for early-exit, don't repeat.
        if (it->second) {
          return false;
        }
      } else {
        ...
        for (it_loop.Advance(); !it_loop.Done(); it_loop.Advance()) {
          for (HBasicBlock* successor : it_loop.Current()->GetSuccessors()) {
             if (!loop->Contains(*successor)) {
               early_exit_loop_.Put(loop, true);
               return false;
             }
           }
        }
        early_exit_loop_.Put(loop, false);
      }
  ...
  bool CanHandleLength(HLoopInformation* loop, HInstruction* length, bool needs_taken_test) {
    if (loop->IsLoopInvariant(length, false)) {
      return true;
    } else if (length->IsArrayLength() &&
               length->GetEnvUses().IsEmpty() &&
               length->GetBlock()->GetLoopInformation() == loop) {
      if (CanHandleNullCheck(loop, length->InputAt(0), needs_taken_test)) {
        HoistOutOfLoop(loop, length);
        return true;
      }
    }
    return false;
  ...
  bool CanHandleNullCheck(HLoopInformation* loop, HInstruction* check, bool needs_taken_test) {
    if (loop->IsLoopInvariant(check, false)) {
      return true;
    } else if (check->IsNullCheck() &&
               check->GetEnvUses().IsEmpty() &&
               check->GetBlock()->GetLoopInformation() == loop) {
      HInstruction* array = check->InputAt(0);
      if (loop->IsLoopInvariant(array, false)) {
        // Generate: if (array == null) deopt;
        HBasicBlock* block = TransformLoopForDeoptimizationIfNeeded(loop, needs_taken_test);
        HInstruction* cond = new (GetGraph()->GetArena()) HEqual(array, GetGraph()->GetNullConstant());
  ...
  /**
   * Returns true if compiler is allowed to apply dynamic bce to loops that may be
   * infinite (e.g. for (int i = 0; i <= U; i++) with U = MAX_INT), which would invalidate
   * the range analysis evaluation code. Since deoptimization would be a bad choice,
   * and there is no other version of the loop to use, dynamic bce is such cases
   * is only allowed if other tests ensure the loop is finite.
   */
  bool CanHandleInfiniteLoop(HLoopInformation* loop, HInstruction* index, bool needs_infinite_test) {
    if (needs_infinite_test) {
      ...
      if (finite_loop_.find(loop) != finite_loop_.end()) {
        return true;
      }
      // Otherwise, allow dynamic bce if the index (which is necessarily an induction at this
      // point) is the direct loop index (viz. a[i]), since then the runtime tests ensure upper
      // bound cannot cause an infinite loop.
  ...
  void InsertDeopt(HLoopInformation* loop, HBasicBlock* block, HInstruction* condition) {
    ...
    block->InsertInstructionBefore(deopt, block->GetLastInstruction());
    if (suspend->HasEnvironment()) {
      deopt->CopyEnvironmentFromWithLoopPhiAdjustment(suspend->GetEnvironment(), loop->GetHeader());
    }
  }

  /** Hoists instruction out of the loop. */
  void HoistOutOfLoop(HLoopInformation* loop, HInstruction* instruction) {
    // Use preheader unless there is an earlier generated deopt block since
    // hoisted expressions may depend on and/or used by the deopt tests.
    HBasicBlock* preheader = loop->GetPreHeader();
    HBasicBlock* block = preheader;
    auto it = taken_test_loop_.find(loop);
   ...
   *          old_preheader
   *               |
   *            if_block          <- taken-test protects deopt block
   *            /      \
  ...
  HBasicBlock* TransformLoopForDeoptimizationIfNeeded(HLoopInformation* loop, bool needs_taken_test) {
    ...
    induction_range_.GenerateTakenTest(header->GetLastInstruction(), GetGraph(), if_block, &condition);
    DCHECK(condition != nullptr);
    if_block->RemoveInstruction(if_block->GetLastInstruction());
    if_block->AddInstruction(new (GetGraph()->GetArena()) HIf(condition));

    taken_test_loop_.Put(loop, true_block);
    return true_block;
  }
  ...
  void InsertPhiNodes() {
    ...
    for (auto it1 = taken_test_loop_.begin(); it1 != taken_test_loop_.end(); ++it1) {
      HBasicBlock* block = it1->second;
      uint32_t vreg = kNoRegNumber;
      // Scan all instructions in a new deoptimization block.
      for (HInstructionIterator it(block->GetInstructions()); !it.Done(); it.Advance()) {
        HInstruction* instruction = it.Current();
        Primitive::Type type = instruction->GetType();
        HPhi* phi = nullptr;
        // Scan all uses of an instruction to find ones that reach the loop.
        for (HUseIterator<HInstruction*> it2(instruction->GetUses()); !it2.Done(); it2.Advance()) {
          HInstruction* user = it2.Current()->GetUser();
          if (user->GetBlock() != block) {
            if (phi == nullptr) {
              // Construct a phi(0, instruction) in the preheader to fix the dominance relation.
              // These are synthetic phi nodes, so each gets a non-existing register to keep
              // the SSA checker happy.
              ...
              switch (type) {
                ...
                default: zero = graph->GetConstant(instruction->GetType(), 0); break;
              }
              phi = new (graph->GetArena()) HPhi(graph->GetArena(), vreg--, 2, type);
              phi->SetRawInputAt(0, instruction);
              phi->SetRawInputAt(1, zero);
              block->GetSuccessors()[0]->AddPhi(phi);
            }
            // Replace use inside loop with the phi node.
  ...
  ArenaSafeMap<HLoopInformation*, bool> early_exit_loop_;

  // Taken-test loop bookkeeping.
  ArenaSafeMap<HLoopInformation*, HBasicBlock*> taken_test_loop_;

  // Finite loop bookkeeping.
  ArenaSet<HLoopInformation*> finite_loop_;
