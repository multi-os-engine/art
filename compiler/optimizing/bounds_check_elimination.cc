...
class MonotonicValueRange : public ValueRange {
 public:
  ...
  // at the end of the loop pre-header.
  //
  // Note: HDeoptimize may define values (hoisted HNullCheck/HArrayLength). So
  // it's sort of violating SSA form by doing this. However it's safe to do so since
  // if the loop header ever defines HNullCheck/HArrayLength, it should have been
  // hoisted by LICM which happens before BCE. So HDeoptimize can only hoist
  // HNullCheck/HArrayLength defined in the loop body (so can only be used in the
  // loop body). It's ok to enter loop header without those values and exit the
  // loop without entering the loop body.
