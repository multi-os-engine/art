...
class BCEVisitor : public HGraphVisitor {
 public:
  ...
  BCEVisitor(HGraph* graph,
      ...
      : HGraphVisitor(graph),
        ...
        has_deoptimization_barrier_(false),
 ...
 private:
  ...
  void VisitArrayGet(HArrayGet* array_get) OVERRIDE {
    if (has_deoptimization_barrier_) {
      // Unsafe to hoist array references across deoptimization barrier.
    } else if (array_get->IsInLoop()) {
  ...
  void AddCompareWithDeoptimization(HInstruction* array_length,
    ...
    // Indicate presence of deoptimization barrier.
    has_deoptimization_barrier_ = true;
  }
  ...
  void TryDynamicBCE(HBoundsCheck* instruction) {
    ...
    if (DynamicBCESeemsProfitable(loop, instruction->GetBlock()) &&
      ...
      //     if (upper >= length) deoptimize;
      // as runtime test. By restricting dynamic bce to unit strides only (with a maximum of 32-bit
  ...
  void InsertDeopt(HLoopInformation* loop, HBasicBlock* block, HInstruction* condition) {
    ...
    HDeoptimize* deoptimize = new (GetGraph()->GetArena()) HDeoptimize(condition, suspend->GetDexPc());
    block->InsertInstructionBefore(deoptimize, block->GetLastInstruction());
    if (suspend->HasEnvironment()) {
      deoptimize->CopyEnvironmentFromWithLoopPhiAdjustment(suspend->GetEnvironment(), loop->GetHeader());
    }
  }
  ...
  void InsertPhiNodes() {
    ...
    for (auto it1 = taken_test_loop_.begin(); it1 != taken_test_loop_.end(); ++it1) {
      ...
      HBasicBlock* true_block = block->GetSuccessors()[0];
      ...
      for (HInstructionIterator it(block->GetInstructions()); !it.Done(); it.Advance()) {
        ...
        HPhi* phi = nullptr;
        // Scan all uses of an instruction and replace each use inside loop with a phi node.
        for (HUseIterator<HInstruction*> it2(instruction->GetUses()); !it2.Done(); it2.Advance()) {
          HInstruction* user = it2.Current()->GetUser();
          if (user->GetBlock() != block) {
            if (phi == nullptr) {
              phi = MakePhi(true_block, instruction, type, vreg--);
            }
            user->ReplaceInput(phi, it2.Current()->GetIndex());
          }
        }
        // Scan all environment uses of an instruction and replace each use inside loop with a phi node.
        for (HUseIterator<HEnvironment*> it2(instruction->GetEnvUses()); !it2.Done(); it2.Advance()) {
          HEnvironment* user = it2.Current()->GetUser();
          if (user->GetHolder()->GetBlock() != block) {
            if (phi == nullptr) {
              phi = MakePhi(true_block, instruction, type, vreg--);
            }
  ...
  /**
   * Construct a phi(instruction, 0) in the preheader to fix the dominance relation. These are
   * synthetic phi nodes, so each gets a non-existing register to keep the SSA checker happy.
   * TODO: is there a better way?
   */
  HPhi* MakePhi(HBasicBlock* true_block, HInstruction* instruction, Primitive::Type type, int vreg) {
    ...
    HPhi* phi = new (graph->GetArena()) HPhi(graph->GetArena(), vreg, 2, type);
    phi->SetRawInputAt(0, instruction);
    phi->SetRawInputAt(1, zero);
    true_block->AddPhi(phi);
    return phi;
  }
  ...
  // Flag that denotes whether deoptimization barriers are inserted.
  bool has_deoptimization_barrier_;
