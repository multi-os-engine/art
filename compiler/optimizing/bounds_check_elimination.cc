...
class MonotonicValueRange : public ValueRange {
 public:
  ...
  //   }
  //
  // We basically first go through the loop and find those array accesses whose index
  // is at a constant offset from the induction variable ('i' in the above example),
  ...
  // (It's the same as saying if deoptimization is triggered, the loop must throw AIOOBE).
  ...
  // 2) We only collect array accesses of blocks in the loop that dominate
