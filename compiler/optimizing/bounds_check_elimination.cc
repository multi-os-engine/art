  ...
  void VisitBoundsCheck(HBoundsCheck* bounds_check) OVERRIDE {
    ...
    if (try_dynamic_bce) {
      ...
      if (DynamicBCESeemsProfitable(loop, bounds_check->GetBlock()) &&
          ...
          CanHandleLength(loop, array_length, needs_taken_test)) {  // do this test last (may code gen)
  ...
  void TransformLoopForDynamicBCE(HLoopInformation* loop, HBoundsCheck* bounds_check) {
    ...
    if ((base != nullptr || min_c >= 0) &&  // reject certain OOB
      ...
      if (base == nullptr) {
        // Constants only.
        DCHECK_GE(min_c, 0);
        DCHECK(min_lower == nullptr && min_upper == nullptr && max_lower == nullptr && max_upper != nullptr);
      ...
      InsertDeoptInLoop(loop, block, new (GetGraph()->GetArena()) HAboveOrEqual(max_upper, array_length));
    } else {
