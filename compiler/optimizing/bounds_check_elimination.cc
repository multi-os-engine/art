...
class ValueRange : public ArenaObject<kArenaAllocMisc> {
 public:
  ...
  bool IsConstantValueRange() { return lower_.IsConstant() and upper_.IsConstant(); }
...
class MonotonicValueRange : public ValueRange {
 public:
  ...
  HBasicBlock* GetLoopBodySuccessor() {
    ...
    if (header->GetLoopInformation()->Contains(*h_if->IfFalseSuccessor())) {
      loop_body_successor = h_if->IfFalseSuccessor();
    } else {
      loop_body_successor = h_if->IfTrueSuccessor();
      DCHECK(header->GetLoopInformation()->Contains(*loop_body_successor));
    }
    return loop_body_successor;
  }
  ...
  HBasicBlock* TransformLoopForDeoptimizationIfNeeded() {
    ...
    // there is no need for tranformating the loop. In that case, deoptimization
  ...
  void AddDeoptimizationConstant(HInstruction* value,
    ...
    if (loop_entry_test_block_added) {
      DCHECK(deopt_block == header->GetDominator()->GetDominator()->GetSuccessors().Get(1));
    }
  ...
  bool CanAddDeoptimizationArrayLength(HInstruction* value,
    ...
    *is_proven = false;
    ...
    if (array_length->GetBlock() == header) {
      ...
      // the loop body since it's used in the loop body. If it's defined in loop body,
      ...
      if (loop_entry_test_block_added) {
        ...
        if (kIsDebugBuild) {
          for (HUseIterator<HInstruction*> it(phi->GetUses());
            ...
            DCHECK(GetLoopBodySuccessor()->Dominates(user->GetBlock()));
          }
        }
      } else {
  ...
  bool AddDeoptimization(const ArrayAccessInsideLoopFinder& finder) {
    ...
    if (increment_ == 1) {
      ...
      if (CanAddDeoptimizationConstant(initial_, -offset_low, &is_constant_proven) &&
        ...
        if (!is_constant_proven) {
          AddDeoptimizationConstant(
              initial_, -offset_low, deopt_block, loop_entry_test_block_added);
        }
        if (!is_length_proven) {
          AddDeoptimizationArrayLength(
              end_, array_length, offset, deopt_block, loop_entry_test_block_added);
        }
        return true;
      }
      ...
      if (CanAddDeoptimizationConstant(end_, constant, &is_constant_proven) &&
        ...
        if (!is_constant_proven) {
          AddDeoptimizationConstant(end_, constant, deopt_block, loop_entry_test_block_added);
        }
        if (!is_length_proven) {
          AddDeoptimizationArrayLength(initial_, array_length, -offset_high - 1,
              deopt_block, loop_entry_test_block_added);
        }
        return true;
      }
    }
    ...
    if (instruction->InputAt(0)->IsCondition()) {
      ...
      if (cmp == kCondGT || cmp == kCondGE ||
        ...
        if (left_range->IsMonotonicValueRange() &&
          ...
          HBasicBlock* loop_body_successor =
            left_range->AsMonotonicValueRange()->GetLoopBodySuccessor();
