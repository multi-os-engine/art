...
class BCEVisitor : public HGraphVisitor {
 public:
  ...
  BCEVisitor(HGraph* graph,
      ...
      : HGraphVisitor(graph),
        ...
        has_constant_array_deoptimization_(false),
 ...
 private:
  ...
  /**
    ...
    * }
    *
    * Note: this optimization is no longer applied after deoptimization on constant
    * array references has occurred, since in those cases it would be unsafe to hoist
    * the array references across the deoptimization instruction inside the loop.
    */
  void VisitArrayGet(HArrayGet* array_get) OVERRIDE {
    if (!has_constant_array_deoptimization_ && array_get->IsInLoop()) {
  ...
  void AddCompareWithDeoptimization(HInstruction* array_length,
    ...
    has_constant_array_deoptimization_ = true;
  }
      ...
      for (HInstructionIterator it(true_block->GetInstructions()); !it.Done(); it.Advance()) {
        ...
        for (HUseIterator<HInstruction*> it2(instruction->GetUses()); !it2.Done(); it2.Advance()) {
          ...
          if (user->GetBlock() != true_block) {
            if (phi == nullptr) {
              phi = NewPhi(new_preheader, instruction, type, vreg--);
            }
   ...
   *
   * TODO: is there a better way that avoids the fake registers?
   */
  HPhi* NewPhi(HBasicBlock* new_preheader,
    ...
    HPhi* phi = new (graph->GetArena()) HPhi(graph->GetArena(), vreg, /*number_of_inputs*/ 2, type);
  ...
  // Flag that denotes whether deoptimization on constant array references has occurred.
  bool has_constant_array_deoptimization_;
