        ...
        for (HUseIterator<HInstruction*> it2(instruction->GetUses()); !it2.Done(); it2.Advance()) {
          ...
          if (user->GetBlock() != block) {
            if (phi == nullptr) {
              ...
              HInstruction* zero = type == Primitive::Type::kPrimNot
                  ? graph->GetNullConstant()
                  : graph->GetConstant(instruction->GetType(), 0);
              phi = new (graph->GetArena()) HPhi(graph->GetArena(), kNoRegNumber, 2, type);
