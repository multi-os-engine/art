  ...
  ValueBound Add(int c) const {
    ...
    if (c > 0) {
      if (constant_ >= INT_MAX - c) {
        // Constant part overflows. Set it to INT_MAX.
      ...
      if (constant_ <= INT_MIN - c) {
        // Constant part underflows. Set it to INT_MIN.
...
class BCEVisitor : public HGraphVisitor {
 ...
 private:
  ...
  // Narrow the lower bound of the value range of 'instruction' at the end of
  // 'basic_block' with 'bound', and push the narrowed value range to 'successor'.
  ...
  // Narrow the upper bound of the value range of 'instruction' at the end of
  // 'basic_block' with 'bound', and push the narrowed value range to 'successor'.
  ...
  void HandleIf(HIf* instruction, HInstruction* left, HInstruction* right, IfCondition cond) {
    ...
    ValueBound bound = GetValueBoundFromValue(right);
    ...
    if (cond == kCondLT || cond == kCondLE) {
      if (!upper.Equals(ValueBound::Max())) {
        ...
        ApplyUpperBound(left, block, true_successor, new_upper);
      }
      ...
      if (!lower.Equals(ValueBound::Min()) && !lower.IsRelativeToArrayLength()) {
        ...
        ApplyLowerBound(left, block, false_successor, new_lower);
      }
      ...
      if (!lower.Equals(ValueBound::Min()) && !lower.IsRelativeToArrayLength()) {
        ...
        ValueBound new_lower = lower.Add(compensation);  // lower bound is inclusive
        ApplyLowerBound(left, block, true_successor, new_lower);
      }

      if (!upper.Equals(ValueBound::Max())) {
        int compensation = (cond == kCondGE) ? -1 : 0;
        ValueBound new_upper = upper.Add(compensation);  // upper bound is inclusive
        ApplyUpperBound(left, block, false_successor, new_upper);
      }
    }
  }
  ...
  void VisitPhi(HPhi* phi) {
    if (phi->IsInLoop() && phi->InputCount() == 2 && phi->GetType() == Primitive::kPrimInt) {
