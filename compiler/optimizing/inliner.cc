...
bool HInliner::TryInlineAndReplace(HInvoke* invoke_instruction, ArtMethod* method, bool do_rtp) {
  ...
  if (do_rtp) {
    RunRTPIfReturnTypeMoreSpecific(invoke_instruction, return_replacement);
  }
  return true;
}
...
bool HInliner::TryBuildAndInlineHelper(HInvoke* invoke_instruction,
  ...
  RunRTPIfArgumentTypesMoreSpecific(invoke_instruction,
                                    resolved_method,
                                    callee_graph,
                                    dex_compilation_unit);
...
static bool IsReferenceTypeInfoRefinement(ReferenceTypeInfo declared_rti,
...
ReferenceTypeInfo HInliner::CreateRerefenceTypeInfoIfAdmissible(mirror::Class* klass) {
...
void HInliner::RunRTPIfArgumentTypesMoreSpecific(HInvoke* invoke_instruction,
                                                 ArtMethod* resolved_method,
                                                 HGraph* callee_graph,
                                                 const DexCompilationUnit& dex_compilation_unit) {
  bool rtp_beneficial = false;

  if (!resolved_method->IsStatic()) {
    mirror::Class* cls = resolved_method->GetDeclaringClass();
    ReferenceTypeInfo declared_object_rti = CreateRerefenceTypeInfoIfAdmissible(cls);
    if (IsReferenceTypeInfoRefinement(declared_object_rti,
  ...
  if (!rtp_beneficial) {
    ...
    for (size_t param_idx = 0,
      ...
      size_t pointer_size = Runtime::Current()->GetClassLinker()->GetImagePointerSize();
      mirror::Class* cls = resolved_method->GetDexCacheResolvedType(
          param_list->GetTypeItem(param_idx).type_idx_, pointer_size);
      ReferenceTypeInfo declared_param_rti = CreateRerefenceTypeInfoIfAdmissible(cls);
      if (IsReferenceTypeInfoRefinement(declared_param_rti,
...
void HInliner::RunRTPIfReturnTypeMoreSpecific(HInvoke* invoke_instruction,
                                              HInstruction* return_replacement) {
  bool rtp_beneficial = false;
  ...
  if (return_replacement != nullptr) {
    if (return_replacement->GetType() == Primitive::kPrimNot) {
      ...
      if (IsReferenceTypeInfoRefinement(invoke_instruction->GetReferenceTypeInfo(),
                                        /* declared_can_be_null */ true,
                                        return_replacement)) {
        rtp_beneficial = true;
      }
  ...
  if (return_replacement != nullptr) {
    if (return_replacement->GetType() == Primitive::kPrimNot) {
      if (!return_replacement->GetReferenceTypeInfo().IsValid()) {
        ...
        return_replacement->SetReferenceTypeInfo(CreateRerefenceTypeInfoIfAdmissible(cls));
      }
    }
  }
}
