...
bool HInliner::TryInline(HInvoke* invoke_instruction, ArtMethod* method, bool do_rtp) {
  ...
  if (!compiler_driver_->MayInline(method->GetDexFile(),
    ...
    if (TryPatternReplacement(invoke_instruction, method, do_rtp)) {
...
ArtField* GetResolvedFieldForIGetOrIPut(ArtMethod* resolved_method,
                                        const InlineIGetIPutData& data,
                                        size_t pointer_size)
    SHARED_REQUIRES(Locks::mutator_lock_) {
  ArtField* resolved_field =
      resolved_method->GetDexCache()->GetResolvedField(data.field_idx, pointer_size);
  DCHECK(resolved_field != nullptr);
  DCHECK_EQ(resolved_field->GetOffset().Uint32Value(), data.field_offset);
  DCHECK_EQ(resolved_field->IsVolatile() ? 1u : 0u, data.is_volatile);
  return resolved_field;
}

// Try to recognize known simple patterns and replace invoke call with appropriate instructions.
bool HInliner::TryPatternReplacement(HInvoke* invoke_instruction,
                                     ArtMethod* resolved_method,
                                     bool do_rtp) {
  ...
  switch (inline_method.opcode) {
    ...
    case kInlineOpIGet: {
      ...
      ArtField* resolved_field = GetResolvedFieldForIGetOrIPut(
          resolved_method, data, InstructionSetPointerSize(codegen_->GetInstructionSet()));
      HInstruction* obj = GetInvokeInputForArgVRegIndex(invoke_instruction, data.object_arg);
      HInstanceFieldGet* iget = CreateInstanceFieldGet(resolved_field, obj);
    ...
    case kInlineOpIPut: {
      ...
      HInstanceFieldSet* iput = CreateInstanceFieldSet(resolved_field, obj, value);
      invoke_instruction->GetBlock()->InsertInstructionBefore(iput, invoke_instruction);
      if (data.return_arg_plus1 != 0u) {
        return_replacement = GetInvokeInputForArgVRegIndex(
            invoke_instruction, data.return_arg_plus1 - 1u);
      }
      break;
    }
    default:
...
HInstanceFieldGet* HInliner::CreateInstanceFieldGet(ArtField* resolved_field,
                                                    HInstruction* obj)
    SHARED_REQUIRES(Locks::mutator_lock_) {
  Handle<mirror::DexCache> dex_cache(handles_->NewHandle(resolved_field->GetDexCache()));
...
HInstanceFieldSet* HInliner::CreateInstanceFieldSet(ArtField* resolved_field,
                                                    HInstruction* obj,
                                                    HInstruction* value)
    SHARED_REQUIRES(Locks::mutator_lock_) {
  Handle<mirror::DexCache> dex_cache(handles_->NewHandle(resolved_field->GetDexCache()));
