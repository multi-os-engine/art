...
bool HInliner::TryBuildAndInline(Handle<mirror::ArtMethod> resolved_method,
  ...
  // Logic that checks limitations allows throwers but requires that invoke is not in try/catch block.
  ...
  for (; !it.Done(); it.Advance()) {
    ...
    for (HInstructionIterator instr_it(block->GetInstructions());
      ...
      bool ends_method_context = current->IsNullCheck() || current->IsBoundsCheck() ||
          current->IsDivZeroCheck();  // Simple under-approximation of throwers supported.
      if (ends_method_context &&
          !invoke_in_try_catch &&
          !callee_graph->HasCatchHandler(current->GetId()) &&
          !graph_->IsDebuggable()) {
        DCHECK_EQ(current->CanThrow(), true);
        DCHECK_EQ(current->CanBeMoved(), true);
        // This thrower passed criteria, so allow it to be inlined.
        continue;
      } else if (current->CanThrow()) {
        VLOG(compiler) << "Method " << PrettyMethod(method_index, caller_dex_file)
