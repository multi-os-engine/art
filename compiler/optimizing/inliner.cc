...
bool HInliner::TryBuildAndInline(ArtMethod* resolved_method,
  ...
  // When merging the graph we might create a new NullConstant in the caller graph which does not
  // not get properly typed. Make sure that it gets properly type so that we can keep the
  // assertion that every reference as a valid type. It also simplifies checks along the way.
