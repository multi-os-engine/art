...
bool HInliner::TryBuildAndInlineHelper(HInvoke* invoke_instruction,
  ...
  // Allocate `inline_stats` on the heap instead of on the stack, as
  // Clang might produce a stack frame too large for this function,
  // that would not fit the requirements of the `-Wframe-larger-than`
  // option.
  std::unique_ptr<OptimizingCompilerStats> inline_stats = MakeUnique<OptimizingCompilerStats>() ;
