...
static uint32_t FindClassIndexIn(mirror::Class* cls, const DexFile& dex_file)
  ...
  } else if (cls->GetDexFile().GetLocation().compare(dex_file.GetLocation()) == 0) {
...
bool HInliner::TryInline(HInvoke* invoke_instruction) {
  ...
  if (invoke_instruction->IsInvokeStaticOrDirect()) {
    ...
    ArtMethod* method = FindVirtualOrInterfaceTarget(invoke_instruction, resolved_method);
    ArtMethod* caller = graph_->GetArtMethod();
    if (method != nullptr) {
      return TryInline(invoke_instruction, method);
    } else if (caller != nullptr && caller->GetProfilingInfo(pointer_size) != nullptr) {
...
bool HInliner::TryInlineMonomorphicCall(HInvoke* invoke_instruction,
  ...
  if (!TryInline(invoke_instruction, resolved_method)) {
    return false;
  }
...
bool HInliner::TryInlinePolymorphicCall(HInvoke* invoke_instruction ATTRIBUTE_UNUSED,
                                        ArtMethod* resolved_method ATTRIBUTE_UNUSED,
                                        const InlineCache& ic ATTRIBUTE_UNUSED) {
  // TODO
  return false;
}

bool HInliner::TryInline(HInvoke* invoke_instruction, ArtMethod* method) {
  const DexFile& caller_dex_file = *caller_compilation_unit_.GetDexFile();
  uint32_t method_index = FindMethodIndexIn(method, caller_dex_file, invoke_instruction->GetDexMethodIndex());
  ...
  if (!TryBuildAndInline(method, invoke_instruction, same_dex_file)) {
    return false;
  }
