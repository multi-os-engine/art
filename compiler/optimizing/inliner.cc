  ...
  for (; !it.Done(); it.Advance()) {
    ...
    for (HInstructionIterator instr_it(block->GetInstructions());
      ...
      if (can_throw &&
          ...
          !graph_->IsDebuggable()) {
        // At the time this code was written, no throwers used dex cache. In order to
        // reduce overhead of check, simply made it a debug check.
        DCHECK_EQ(dex_cache_issue, false) << "Found thrower which uses dex cache " <<
            current->DebugName();
