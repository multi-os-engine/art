...
void InstructionCodeGeneratorX86::DivRemOneOrMinusOne(HBinaryOperation* instruction) {
  ...
  LocationSummary* locations = instruction->GetLocations();
  DCHECK(locations->InAt(1).IsConstant() && locations->InAt(1).GetConstant()->IsIntConstant());
...
void InstructionCodeGeneratorX86::GenerateDivRemIntegral(HBinaryOperation* instruction) {
  ...
  switch (instruction->GetResultType()) {
    case Primitive::kPrimInt: {
      ...
      if (second.IsConstant() && second.GetConstant()->IsIntConstant()) {
        ...
        if (imm == 0) {
          ...
          DivByPowerOfTwo(static_cast<HDiv*>(instruction));
        } else {
          DCHECK(imm <= -2 || imm >= 2);
...
void LocationsBuilderX86::VisitDiv(HDiv* div) {
  ...
  switch (div->GetResultType()) {
    case Primitive::kPrimInt: {
      ...
      if (div->InputAt(1)->IsConstant() && div->InputAt(1)->IsIntConstant()) {
        locations->AddTemp(Location::RequiresRegister());
      }
      break;
    }
