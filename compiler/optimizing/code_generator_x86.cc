...
class ReadBarrierSlowPathX86 : public SlowPathCodeX86 {
 public:
  ...
  void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    ...
    if (index_.IsValid()) {
      if (instruction_->IsArrayGet()) {
        ...
        // Old `index_` is already saved in SaveLiveRegisters(), unless
        // it's a callee save register.
        Register reg = index_.AsRegister<Register>();
        DCHECK(locations->GetLiveRegisters()->ContainsCoreRegister(reg));
        // In this case let's find a register that is not callee save
        if (codegen->IsCoreCalleeSaveRegister(reg)) {
          Register free_reg = FindNextAvailableRegister(codegen);
          __ movl(free_reg, reg);
          reg = free_reg;
          index = Location::RegisterLocation(reg);
        }
        // Shifting the index value contained in `reg` by the scale
        // factor (2) cannot overflow in practice, as the runtime is
        // unable to allocate object arrays with a size larger than
        // 2^26 - 1 (that is, 2^28 - 4 bytes).
        __ shll(reg, Immediate(TIMES_4));
        __ AddImmediate(reg, Immediate(offset_));
      } else {
        DCHECK(instruction_->IsInvoke());
        DCHECK(instruction_->GetLocations()->Intrinsified());
  ...
  // Is this read barrier generated before a call?
  bool IsRunBeforeCall() const {
 ...
 private:
  ...
  const Location index_;  // For HArrayGet and intrinsic UnsafeGetObject
...
void SlowPathCodeX86::SaveParameterRegisters(CodeGenerator* codegen, Location out) {
  ...
  // TODO: Should we save float parameter registers? Currently we
  // assume the GCC/Clang-compiled read barrier code would not touch
  // any of the float parameter registers.
}
