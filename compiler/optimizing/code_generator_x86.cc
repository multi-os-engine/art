  ...
  switch (add->GetResultType()) {
    ...
    case Primitive::kPrimFloat: {
      if (second.IsFpuRegister()) {
        ...
        HX86LoadFromConstantTable* const_area =
          add->InputAt(1)->AsX86LoadFromConstantTable();
  ...
  switch (sub->GetResultType()) {
    ...
    case Primitive::kPrimFloat: {
      if (second.IsFpuRegister()) {
        ...
        HX86LoadFromConstantTable* const_area =
          sub->InputAt(1)->AsX86LoadFromConstantTable();
  ...
  switch (mul->GetResultType()) {
    ...
    case Primitive::kPrimFloat: {
      ...
      if (second.IsFpuRegister()) {
        ...
        HX86LoadFromConstantTable* const_area =
          mul->InputAt(1)->AsX86LoadFromConstantTable();
  ...
  switch (div->GetResultType()) {
    ...
    case Primitive::kPrimFloat: {
      if (second.IsFpuRegister()) {
        ...
        HX86LoadFromConstantTable* const_area =
          div->InputAt(1)->AsX86LoadFromConstantTable();
...
void LocationsBuilderX86::VisitX86LoadFromConstantTable(
  ...
  switch (insn->GetType()) {
    ...
    default:
      LOG(FATAL) << "Unreachable type " << insn->GetType();
  }
}

void InstructionCodeGeneratorX86::VisitX86LoadFromConstantTable(
                                        HX86LoadFromConstantTable* insn) {
  if (!insn->NeedsMaterialization()) {
    return;
  }
...
class ConstantHandlerVisitor : public HGraphVisitor {
 ...
 private:
  ...
  void ReplaceInput(HInstruction* insn, HConstant* value, int input_num, bool materialize);

  HX86ComputeBaseMethodAddress* base_;
};

void ConstantHandlerVisitor::Initialize(HInstruction* dep) {
...
void ConstantHandlerVisitor::ReplaceInput(HInstruction* insn,
                                          HConstant* value,
                                          int input_num,
                                          bool materialize) {
  ...
  insn->ReplaceInput(load_constant, input_num);
}

void ConstantHandlerVisitor::HandleInvoke(HInvoke* invoke) {
