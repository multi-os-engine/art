  ...
  switch (mul->GetResultType()) {
    ...
    case Primitive::kPrimLong: {
      ...
      if (mul->InputAt(1)->IsConstant()) {
        // This is needed because we have no way to request Register or Stack.
        // If (when) we add that this can be removed and let the allocator
        // copy the constants.
...
template <typename T>
void InstructionCodeGeneratorX86::LongMul(Register eax,
                                          Register edx,
                                          Register in1_hi,
                                          Register in1_lo,
                                          const T& in2_hi,
  ...
  switch (mul->GetResultType()) {
    ...
    case Primitive::kPrimLong: {
      ...
      if (second.IsRegisterPair()) {
        ...
        Register tmp_hi = locations->GetTemp(2).As<Register>();
        Register tmp_lo = locations->GetTemp(3).As<Register>();
        int64_t value = second.GetConstant()->AsLongConstant()->GetValue();
        __ movl(tmp_hi, Immediate(High32Bits(value)));
        __ movl(tmp_lo, Immediate(Low32Bits(value)));
        LongMul(eax, edx, in1_hi, in1_lo, tmp_hi, tmp_lo);
      } else {
