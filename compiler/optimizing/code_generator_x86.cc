...
void CodeGeneratorX86::LoadCurrentMethod(Register reg) {
  assembler_.movl(reg, Address(ESP, kCurrentMethodStackOffset));
}
...
void InstructionCodeGeneratorX86::VisitLoadClass(HLoadClass* cls) {
  ...
  if (cls->IsReferrersClass()) {
    ...
    __ movl(out, Address(out, index_in_cache));
  }
}
...
void InstructionCodeGeneratorX86::VisitClinitCheck(HClinitCheck* check) {
  ...
  __ testl(in, in);
  __ j(kEqual, slow_path->GetEntryLabel());
  __ cmpl(Address(in,  mirror::Class::StatusOffset().Int32Value()),
