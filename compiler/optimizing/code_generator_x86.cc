...
void LocationsBuilderX86::HandleFieldGet(HInstruction* instruction) {
  ...
  bool is_volatile = instruction->IsInstanceFieldGet()
    ? instruction->AsInstanceFieldGet()->IsVolatile()
    : instruction->AsStaticFieldGet()->IsVolatile();
  Primitive::Type field_type = instruction->GetType();

  if (is_volatile && (field_type == Primitive::kPrimLong)) {
    locations->AddTemp(Location::RequiresFpuRegister());
  }
}
