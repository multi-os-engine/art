...
class ReadBarrierSlowPathX86 : public SlowPathCode {
 protected:
  ReadBarrierSlowPathX86(HInstruction* instruction, Location out)
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(ReadBarrierSlowPathX86);
};

// Slow path generating a read barrier for a heap reference.
class ReadBarrierForHeapReferenceSlowPathX86 : public ReadBarrierSlowPathX86 {
 public:
  ReadBarrierForHeapReferenceSlowPathX86(HInstruction* instruction,
                                         ...
                                         uint32_t offset,
                                         Location index)
      : ReadBarrierSlowPathX86(instruction, out),
        ref_(ref),
        obj_(obj),
        offset_(offset),
        index_(index) {
    ...
    if (index_.IsValid()) {
      ...
      if (instruction_->IsArrayGet()) {
        ...
        if (codegen->IsCoreCalleeSaveRegister(index_reg)) {
          ...
          Register free_reg = FindNextAvailableRegister(codegen);
...
class ReadBarrierForRootSlowPathX86 : public ReadBarrierSlowPathX86 {
 public:
...
void InstructionCodeGeneratorX86::VisitInvokeInterface(HInvokeInterface* invoke) {
  ...
  Location temp3_loc =
      (kForceReadBarrier || kUseReadBarrier) ? locations->GetTemp(2) : Location::NoLocation();
...
void LocationsBuilderX86::HandleFieldGet(HInstruction* instruction, const FieldInfo& field_info) {
  ...
  if (Primitive::IsFloatingPointType(instruction->GetType())) {
    ...
    // the object's location.  Likewise when emitting a read barrier.
    locations->SetOut(
        Location::RequiresRegister(),
        (kForceReadBarrier || kUseReadBarrier || instruction->GetType() == Primitive::kPrimLong) ?
...
void LocationsBuilderX86::VisitArrayGet(HArrayGet* instruction) {
  ...
  if (Primitive::IsFloatingPointType(instruction->GetType())) {
    ...
    // The output overlaps in case of long: we don't want the low move to overwrite
    // the array's location.  Likewise when emitting a read barrier.
    locations->SetOut(
        Location::RequiresRegister(),
        (kForceReadBarrier || kUseReadBarrier || instruction->GetType() == Primitive::kPrimLong) ?
            Location::kOutputOverlap :
    ...
    case Primitive::kPrimNot: {
      ...
      if (may_need_runtime_call) {
        ...
        if (kForceReadBarrier || kUseReadBarrier) {
          ...
          // CodeGeneratorX86::GenerateReadBarrierWithLiveTemp instead
          // of CodeGeneratorX86::GenerateReadBarrier.
          //
...
void InstructionCodeGeneratorX86::VisitLoadClass(HLoadClass* cls) {
  ...
  Register current_method = current_method_loc.AsRegister<Register>();
  if (cls->IsReferrersClass()) {
    DCHECK(kForceReadBarrier || kUseReadBarrier || !cls->CanCallRuntime());
    DCHECK(!cls->MustGenerateClinitCheck());
    if (kForceReadBarrier || kUseReadBarrier) {
      // /* GcRoot<mirror::Class>* */ out = &(current_method->declaring_class_)
      __ leal(out, Address(current_method, ArtMethod::DeclaringClassOffset().Int32Value()));
...
void InstructionCodeGeneratorX86::VisitLoadString(HLoadString* load) {
  ...
  if (kForceReadBarrier || kUseReadBarrier) {
    ...
    __ leal(out, Address(out, CodeGenerator::GetCacheOffset(load->GetStringIndex())));
...
void InstructionCodeGeneratorX86::VisitInstanceOf(HInstanceOf* instruction) {
  ...
  switch (instruction->GetTypeCheckKind()) {
    ...
    case TypeCheckKind::kInterfaceCheck: {
      ...
      DCHECK(locations->OnlyCallsOnSlowPath());
      slow_path =
          new (GetGraph()->GetArena()) TypeCheckSlowPathX86(instruction, /* is_fatal */ false);
      codegen_->AddSlowPath(slow_path);
      __ jmp(slow_path->GetEntryLabel());
      if (zero.IsLinked()) {
        __ jmp(&done);
...
void CodeGeneratorX86::GenerateReadBarrierWithLiveTemp(HInstruction* instruction,
                                                       Location out,
                                                       Location ref,
                                                       Location obj,
                                                       uint32_t offset,
  ...
  // CodeGeneratorX86::GenerateReadBarrier is `live_temp`, passed
  // to the slow path.
  DCHECK(kForceReadBarrier || kUseReadBarrier);
  ReadBarrierForHeapReferenceSlowPathX86* slow_path = new (GetGraph()->GetArena())
      ReadBarrierForHeapReferenceSlowPathX86(instruction, out, ref, obj, offset, index);
...
void ReadBarrierSlowPathX86::SaveParameterRegisters(CodeGenerator* codegen, Location out) {
  ...
  for (size_t i = 0; i < kParameterCoreRegistersLength; ++i) {
    ...
    if (!codegen->IsCoreCalleeSaveRegister(reg)
        && reg != static_cast<size_t>(out.AsRegister<Register>())) {
...
void ReadBarrierSlowPathX86::RestoreParameterRegisters(CodeGenerator* codegen, Location out) {
  size_t stack_offset = codegen->GetFirstRegisterSlotInSlowPath();
  for (size_t i = 0; i < kParameterCoreRegistersLength; ++i) {
    size_t reg = static_cast<size_t>(kParameterCoreRegisters[i]);
    if (!codegen->IsCoreCalleeSaveRegister(reg)
...
Register ReadBarrierForHeapReferenceSlowPathX86::FindNextAvailableRegister(CodeGenerator* codegen) {
