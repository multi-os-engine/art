...
void InstructionCodeGeneratorX86::GenerateShlLong(const Location& first, Register second_reg) {
  Label done;
  __ shld(first.AsRegisterPairHigh<Register>(), first.AsRegisterPairLow<Register>(), second_reg);
  __ shll(first.AsRegisterPairLow<Register>(), second_reg);
...
void InstructionCodeGeneratorX86::GenerateShrLong(const Location& first, Register second_reg) {
  Label done;
  __ shrd(first.AsRegisterPairLow<Register>(), first.AsRegisterPairHigh<Register>(), second_reg);
  __ sarl(first.AsRegisterPairHigh<Register>(), second_reg);
  __ testl(second_reg, Immediate(32));
  __ j(kEqual, &done);
  // if we're greater than 32 we need to move the entire low part to the high part.
...
void InstructionCodeGeneratorX86::GenerateUShrLong(const Location& first, Register second_reg) {
  Label done;
  __ shrd(first.AsRegisterPairLow<Register>(), first.AsRegisterPairHigh<Register>(), second_reg);
  __ shrl(first.AsRegisterPairHigh<Register>(), second_reg);
  __ testl(second_reg, Immediate(32));
  __ j(kEqual, &done);
  // if we're greater than 32 we need to move the entire low part to the high part.
  __ movl(first.AsRegisterPairLow<Register>(), first.AsRegisterPairHigh<Register>());
