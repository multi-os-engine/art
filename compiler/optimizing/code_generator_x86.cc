...
void InstructionCodeGeneratorX86::HandleFieldGet(HInstruction* instruction,
  ...
  switch (field_type) {
    ...
    case Primitive::kPrimNot: {
      Location temp_loc = (kEmitCompilerReadBarrier && kUseBakerReadBarrier) ?
          locations->GetTemp(0) :
          Location::NoLocation();
      // /* HeapReference<Object> */ out = *(base + offset)
      if (kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
        codegen_->GenerateFieldLoadWithBakerReadBarrier(instruction, out, base, offset, temp_loc);
      } else {
        __ movl(out.AsRegister<Register>(), Address(base, offset));
        // Note: read barriers other than Baker's are handled in the
        // call to CodeGeneratorX86::GenerateReadBarrierSlow below.
      }
      break;
    }
  ...
  if (field_type == Primitive::kPrimNot && kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
    // A potential implicit null check, in the case of a reference
    // field with Baker's read barriers, is already handled in the
    // previous call to
    // CodeGeneratorX86::GenerateFieldLoadWithBakerReadBarrier.
  } else if (field_type == Primitive::kPrimLong) {
    // A potential implicit null check, in the case of a long field,
    // is already handled in the previous switch statement.
  } else {
    codegen_->MaybeRecordImplicitNullCheck(instruction);
  }
  ...
  if (field_type == Primitive::kPrimNot) {
    if (kEmitCompilerReadBarrier) {
      if (kUseBakerReadBarrier) {
        // A read barrier emission, in the case of a reference field
        // with Baker's read barriers, is already handled in the
        // previous call to
        // CodeGeneratorX86::GenerateFieldLoadWithBakerReadBarrier.
      } else {
        // Other types of read barriers are addressed by the slow path
        // approach implemented by
        // CodeGeneratorX86::GenerateReadBarrierSlow.
        codegen_->GenerateReadBarrierSlow(instruction, out, out, base_loc, offset);
      }
    } else {
...
void InstructionCodeGeneratorX86::VisitArrayGet(HArrayGet* instruction) {
    ...
    case Primitive::kPrimNot: {
      ...
      if (kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
        ...
        codegen_->GenerateArrayLoadWithBakerReadBarrier(
            instruction, out_loc, obj, data_offset, index, temp);
      } else {
        Register out = out_loc.AsRegister<Register>();
        if (index.IsConstant()) {
          __ movl(out, Address(obj,
  ...
  if (type == Primitive::kPrimNot && kEmitCompilerReadBarrier && kUseBakerReadBarrier) {
    // A potential implicit null check, in the case of an object array
    // with Baker's read barriers, is already handled in the previous
    // call to CodeGeneratorX86::GenerateArrayLoadWithBakerReadBarrier.
  } else if (type == Primitive::kPrimLong) {
    // A potential implicit null check, in the case of a long array,
    // is already handled in the previous switch statement.
  } else {
    codegen_->MaybeRecordImplicitNullCheck(instruction);
  }
  ...
  if (type == Primitive::kPrimNot) {
    if (kEmitCompilerReadBarrier) {
      if (kUseBakerReadBarrier) {
        // A read barrier emission, in the case of an object array
        // with Baker's read barriers, is already handled in the
        // previous call to
        // CodeGeneratorX86::GenerateArrayLoadWithBakerReadBarrier.
      } else {
        // Other types of read barriers are addressed by the slow path
        // approach implemented by
        // CodeGeneratorX86::GenerateReadBarrierSlow.
        ...
        if (index.IsConstant()) {
          uint32_t offset =
              (index.GetConstant()->AsIntConstant()->GetValue() << TIMES_4) + data_offset;
          codegen_->GenerateReadBarrierSlow(instruction, out_loc, out_loc, obj_loc, offset);
        } else {
      ...
      __ MaybeUnpoisonHeapReference(out_loc.AsRegister<Register>());
    }
  }
}
...
void InstructionCodeGeneratorX86::VisitInstanceOf(HInstanceOf* instruction) {
  ...
  Location out_loc = locations->Out();
  Register out = out_loc.AsRegister<Register>();
  Location temp_loc = (kEmitCompilerReadBarrier &&
      (kUseBakerReadBarrier ||
       type_check_kind == TypeCheckKind::kAbstractClassCheck ||
       type_check_kind == TypeCheckKind::kClassHierarchyCheck ||
       type_check_kind == TypeCheckKind::kArrayObjectCheck)) ?
...
void InstructionCodeGeneratorX86::VisitCheckCast(HCheckCast* instruction) {
  ...
  Location temp2_loc = (kEmitCompilerReadBarrier &&
      (kUseBakerReadBarrier ||
       type_check_kind == TypeCheckKind::kAbstractClassCheck ||
       type_check_kind == TypeCheckKind::kClassHierarchyCheck ||
       type_check_kind == TypeCheckKind::kArrayObjectCheck)) ?
...
void InstructionCodeGeneratorX86::GenerateReferenceLoadOneRegister(HInstruction* instruction,
  ...
  if (kEmitCompilerReadBarrier) {
    if (kUseBakerReadBarrier) {
      ...
      codegen_->GenerateFieldLoadWithBakerReadBarrier(instruction, out, out_reg, offset, temp);
    } else {
...
void InstructionCodeGeneratorX86::GenerateReferenceLoadTwoRegisters(HInstruction* instruction,
  ...
  if (kEmitCompilerReadBarrier) {
    if (kUseBakerReadBarrier) {
      ...
      codegen_->GenerateFieldLoadWithBakerReadBarrier(instruction, out, obj_reg, offset, temp);
    } else {
      // Load with slow path based read barrier.
      // /* HeapReference<Object> */ out = *(obj + offset)
...
void InstructionCodeGeneratorX86::GenerateGcRootFieldLoad(HInstruction* instruction,
  ...
  if (kEmitCompilerReadBarrier) {
    if (kUseBakerReadBarrier) {
      ...
      NearLabel done;
      __ fs()->cmpw(Address::Absolute(Thread::IsGcMarkingOffset<kX86WordSize>().Int32Value()),
                    Immediate(0));
      __ j(kNotEqual, slow_path->GetEntryLabel());
      __ Bind(slow_path->GetExitLabel());
      __ Bind(&done);
...
void CodeGeneratorX86::GenerateFieldLoadWithBakerReadBarrier(HInstruction* instruction,
  ...
  GenerateReferenceLoadWithBakerReadBarrier(instruction, ref, obj, src, temp);
}
