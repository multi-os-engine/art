...
void CodeGeneratorX86::Move64(Location destination, Location source) {
  ...
  if (destination.IsRegisterPair()) {
    ...
    if (source.IsRegisterPair()) {
      ...
      int32_t val_low = Low32Bits(val);
      int32_t val_high = High32Bits(val);
      __ movl(Address(ESP, destination.GetStackIndex()), Immediate(val_low));
      __ movl(Address(ESP, destination.GetHighStackIndex(kX86WordSize)),
              Immediate(val_high));
    } else {
...
void InstructionCodeGeneratorX86::VisitInvokeInterface(HInvokeInterface* invoke) {
  ...
  __ movd(locations->GetTemp(1).AsFpuRegister<XmmRegister>(), temp);
    ...
    case Primitive::kPrimByte:
      switch (input_type) {
        ...
        case Primitive::kPrimChar:
          ...
          // TODO: Figure out how to only force registers into byte registers.
          //       Loading from memory is okay.
          locations->SetInAt(0,
            Location::ByteRegisterOrConstant(ECX, conversion->InputAt(0)));
...
#if 0
          // This causes problems with LSRA
          locations->SetInAt(0, Location::RegisterLocation(EAX));
          locations->SetOut(Location::RegisterPairLocation(EAX, EDX));
#else
...
#else
          // TODO: Replace this with above code.
          Register low = out.AsRegisterPairLow<Register>();
          Register high = out.AsRegisterPairHigh<Register>();
          if (in.IsRegister()) {
            __ movl(high, in.AsRegister<Register>());
            __ movl(low, in.AsRegister<Register>());
            __ sarl(high, Immediate(31));
          } else if (in.IsStackSlot()) {
            __ movl(low, Address(ESP, in.GetStackIndex()));
            __ movl(high, low);
            __ sarl(high, Immediate(31));
          } else {
            DCHECK(in.GetConstant()->IsIntConstant());
            int32_t value = in.GetConstant()->AsIntConstant()->GetValue();
            __ movl(low, Immediate(value));
            __ movl(high, Immediate(value < 0 ? -1 : 0));
          }
#endif
          }
          break;
  ...
  switch (add->GetResultType()) {
    ...
    case Primitive::kPrimLong: {
      if (second.IsRegisterPair()) {
        ...
        int64_t val = second.GetConstant()->AsLongConstant()->GetValue();
        int32_t val_low = Low32Bits(val);
        int32_t val_high = High32Bits(val);
        __ addl(first.AsRegisterPairLow<Register>(), Immediate(val_low));
        __ adcl(first.AsRegisterPairHigh<Register>(), Immediate(val_high));
      } else {
  ...
  switch (sub->GetResultType()) {
    ...
    case Primitive::kPrimLong: {
      if (second.IsRegisterPair()) {
        ...
        int32_t val_low = Low32Bits(val);
        int32_t val_high = High32Bits(val);
        __ subl(first.AsRegisterPairLow<Register>(), Immediate(val_low));
        __ sbbl(first.AsRegisterPairHigh<Register>(), Immediate(val_high));
      } else {
  ...
  switch (mul->GetResultType()) {
    ...
    case Primitive::kPrimLong: {
      ...
      if (second.IsConstant()) {
        ...
        // Generate basic code for now.
        // TODO: better constant multiplication.

        // input: in1 - 64 bits, in2 - constant value.
        // output: in1
...
void ParallelMoveResolverX86::EmitMove(size_t index) {
  ...
  if (source.IsRegister()) {
    ...
    if (destination.IsRegisterPair()) {
      // May have overlap if the source is EAX:EDX from an invoke.
      // This may also lead to copying a register to itself.
      Register dest_low = destination.AsRegisterPairLow<Register>();
      Register dest_high = destination.AsRegisterPairHigh<Register>();
      Register src_low = source.AsRegisterPairLow<Register>();
      Register src_high = source.AsRegisterPairHigh<Register>();
      if (dest_low == src_high) {
        DCHECK_NE(src_low, dest_high);
        if (dest_high != src_high) {
          __ movl(dest_high, src_high);
        }
        if (dest_low != src_low) {
          __ movl(dest_low, src_low);
        }
      } else {
        if (dest_low != src_low) {
          __ movl(dest_low, src_low);
...
void ParallelMoveResolverX86::EmitSwap(size_t index) {
  ...
  if (source.IsRegister() && destination.IsRegister()) {
  ...
  } else if (source.IsRegisterPair() && destination.IsDoubleStackSlot()) {
    Exchange(source.AsRegisterPairLow<Register>(), destination.GetStackIndex());
    Exchange(source.AsRegisterPairHigh<Register>(), destination.GetHighStackIndex(kX86WordSize));
    ...
    // Move the high double to the low double.
    __ psrldq(reg, Immediate(8));
  } else if (destination.IsRegisterPair() && source.IsRegisterPair()) {
    Register dest_low = destination.AsRegisterPairLow<Register>();
    Register dest_high = destination.AsRegisterPairHigh<Register>();
    Register src_low = source.AsRegisterPairLow<Register>();
    Register src_high = source.AsRegisterPairHigh<Register>();
    DCHECK_NE(dest_low, dest_high);
    DCHECK_NE(src_low, src_high);
    DCHECK_NE(dest_low, src_low);
    DCHECK_NE(dest_high, src_high);
    __ xchgl(dest_low, src_low);
    __ xchgl(dest_high, src_high);
  } else if (destination.IsRegisterPair() && source.IsDoubleStackSlot()) {
    Exchange(destination.AsRegisterPairLow<Register>(), source.GetStackIndex());
    Exchange(destination.AsRegisterPairHigh<Register>(), source.GetHighStackIndex(kX86WordSize));
