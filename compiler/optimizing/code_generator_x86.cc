...
void CodeGeneratorX86::Move(Location destination, Location source, Primitive::Type dst_type) {
  ...
  if ((destination.IsRegisterPair() || destination.IsDoubleStackSlot()) &&
      (source.IsRegisterPair() || source.IsDoubleStackSlot())) {
  ...
  if (source.IsRegister()) {
    ...
    if (constant->IsIntConstant() || constant->IsNullConstant()) {
      ...
      if (destination.IsDoubleStackSlot()) {
        ...
        DCHECK(destination.IsRegisterPair());
...
void CodeGeneratorX86::MoveFromReturnRegister(Location target, Primitive::Type type) {
  ...
  Move(target, InvokeDexCallingConventionVisitorX86().GetReturnLocation(type), type);
}
