...
class ConstantHandlerVisitor : public HGraphVisitor {
 public:
  ConstantHandlerVisitor(HGraph* graph) : HGraphVisitor(graph), base_(nullptr) {}
 ...
 private:
  ...
  void Initialize();
  void InitializeConstantAreaPointer() {
    if (base_ != nullptr) {
      return;
    }
  ...
  void BinaryFP(HBinaryOperation* bin) {
    if (bin->InputAt(1)->IsConstant() && Primitive::IsFloatingPointType(bin->GetResultType())) {
      InitializeConstantAreaPointer();
      bin->SetRawInputAt(2, base_);
      base_->AddUseAt(bin, 2);
    }
  }
...
class RIPFixup : public AssemblerFixup, public ArenaObject<kArenaAllocMisc> {
  ...
  private:
    void Process(const MemoryRegion& region, int pos) OVERRIDE {
      ...
      int relative_position = constant_offset - codegen_.GetMethodAddressOffset();;
