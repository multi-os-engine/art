...
class RIPFixup : public AssemblerFixup, public ArenaObject<kArenaAllocCodeGenerator> {
 public:
  RIPFixup(CodeGeneratorX86& codegen, int32_t offset)
      : codegen_(&codegen), offset_into_constant_area_(offset) {}

 protected:
  void SetOffset(int32_t offset) { offset_into_constant_area_ = offset; }
  ...
  public:
    ...
    void CreateJumpTable() {
      ...
      for (int32_t i = 0; i < num_entries; i++) {
        ...
        assembler->AddUniqueInt32(offset_to_block);
      }
    }
...
void CodeGeneratorX86::Finalize(CodeAllocator* allocator) {
  ...
  if (!assembler->IsConstantAreaEmpty() || !fixups_to_jump_tables_.empty()) {
    ...
    for (size_t i = 0, e = fixups_to_jump_tables_.size(); i < e; i++) {
      fixups_to_jump_tables_[i]->CreateJumpTable();
    }
...
class JumpTableFixup : public AssemblerFixup, public ArenaObject<kArenaAllocMisc> {
 public:
  JumpTableFixup(const CodeGeneratorX86& codegen, HX86PackedSwitch* switch_instr)
...
Address CodeGeneratorX86::LiteralCaseTable(HX86PackedSwitch* switch_instr, Register reg) {
  // Create a fixup to be used to create and address the jump table.
  JumpTableRIPFixup* table_fixup = new (GetGraph()->GetArena()) JumpTableRIPFixup(*this, switch_instr);

  // We have to populate the jump tables.
  fixups_to_jump_tables_.push_back(table_fixup);

  return Address(reg, kDummy32BitOffset, table_fixup);
}
 ...
 private:
  ...
  void VisitPackedSwitch(HPackedSwitch* switch_insn) OVERRIDE {
    ...
    HX86PackedSwitch* x86_switch =
        new (graph->GetArena()) HX86PackedSwitch(
    ...
    DCHECK_EQ(block->GetLastInstruction(), switch_insn);
