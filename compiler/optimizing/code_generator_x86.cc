...
void CodeGeneratorX86::Move64(Location destination, Location source) {
  ...
  if (destination.IsRegister()) {
    if (source.IsRegister()) {
      ...
      __ movl(destination.AsX86().AsRegisterPairHigh(),
              Address(ESP, source.GetStackIndex() + kX86WordSize));
    }
    ...
    if (source.IsRegister()) {
      ...
      __ movl(EAX, Address(ESP, source.GetStackIndex() + kX86WordSize));
    ...
    if (source.IsRegister()) {
      ...
      __ movl(Address(ESP, destination.GetStackIndex() + kX86WordSize),
              source.AsX86().AsRegisterPairHigh());
    } else if (source.IsQuickParameter()) {
      InvokeDexCallingConvention calling_convention;
      uint32_t argument_index = source.GetQuickParameterIndex();
      ...
      __ movl(EAX,
              Address(ESP,
                      calling_convention.GetStackOffsetOf(argument_index + 1) + GetFrameSize()));
      __ movl(Address(ESP, destination.GetStackIndex() + kX86WordSize), EAX);
    } else {
      DCHECK(source.IsDoubleStackSlot());
      __ movl(EAX, Address(ESP, source.GetStackIndex()));
      __ movl(Address(ESP, destination.GetStackIndex()), EAX);
...
void CodeGeneratorX86::Move(HInstruction* instruction, Location location, HInstruction* move_for) {
  if (instruction->AsIntConstant() != nullptr) {
    ...
    if (location.IsRegister()) {
      ...
      __ movl(Address(ESP, location.GetStackIndex() + kX86WordSize), Immediate(High32Bits(value)));
    }
...
void LocationsBuilderX86::VisitStoreLocal(HStoreLocal* store) {
  ...
  switch (store->InputAt(1)->GetType()) {
    ...
    default:
      LOG(FATAL) << "Unimplemented local type";
  }
...
void LocationsBuilderX86::VisitReturn(HReturn* ret) {
  ...
  switch (ret->InputAt(0)->GetType()) {
    ...
    default:
      LOG(FATAL) << "Unimplemented return type";
  }
...
void LocationsBuilderX86::VisitPushArgument(HPushArgument* argument) {
  ...
  switch (argument->InputAt(0)->GetType()) {
    ...
    default:
      LOG(FATAL) << "Unimplemented argument type";
  }
...
void LocationsBuilderX86::VisitParameterValue(HParameterValue* instruction) {
  ...
  switch (instruction->GetType()) {
    ...
    default:
      LOG(FATAL) << "Unimplemented parameter type";
  }
  instruction->SetLocations(locations);
}
