...
void LocationsBuilderX86::VisitInvokeStaticOrDirect(HInvokeStaticOrDirect* invoke) {
  ...
  if (intrinsic.TryDispatch(invoke)) {
    if (invoke->GetLocations()->CanCall() && invoke->HasPcRelDexCache()) {
  ...
  if (invoke->IsInvokeStaticOrDirect() && invoke->AsInvokeStaticOrDirect()->HasPcRelDexCache()) {
    invoke->GetLocations()->SetInAt(invoke->AsInvokeStaticOrDirect()->GetCurrentMethodInputIndex(),
...
Register CodeGeneratorX86::GetInvokeStaticOrDirectExtraParameter(HInvokeStaticOrDirect* invoke,
  ...
  if (!location.IsRegister()) {
    __ movl(temp.AsRegister<Register>(), Address(ESP, location.GetStackIndex()));
    return temp.AsRegister<Register>();
  }
  // For register locations, check if the location was overwritten by parameters.
  InvokeDexCallingConventionVisitorX86 calling_convention;
  for (size_t i = 0, num_args = invoke->GetNumberOfArguments(); i != num_args; ++i) {
    Location cc_loc = calling_convention.GetNextLocation(invoke->InputAt(i)->GetType());
    if (cc_loc.OverlapsWith(location)) {
      // Overwritten, so get it from the stack. It must have been saved by the slow path.
      SlowPathCode* slow_path = GetCurrentSlowPath();
      DCHECK(slow_path->IsCoreRegisterSaved(location.AsRegister<Register>()));
      int stack_offset = slow_path->GetStackOffsetOfCoreRegister(location.AsRegister<Register>());
      __ movl(temp.AsRegister<Register>(), Address(ESP, stack_offset));
      return temp.AsRegister<Register>();
    }
  }
  return location.AsRegister<Register>();
...
void CodeGeneratorX86::GenerateStaticOrDirectCall(HInvokeStaticOrDirect* invoke, Location temp) {
  ...
  switch (invoke->GetMethodLoadKind()) {
    ...
    case HInvokeStaticOrDirect::MethodLoadKind::kDexCachePcRelative: {
      Register base_reg = GetInvokeStaticOrDirectExtraParameter(invoke, temp);
      int32_t offset = dchecked_integral_cast<int32_t>(invoke->GetDexCacheArrayOffset());
      __ movl(temp.AsRegister<Register>(), Address(base_reg, kDummy32BitOffset));
      // Add the patch entry and bind its label at the end of the instruction.
      pc_rel_dex_cache_patches_.emplace_back(*invoke->GetTargetMethod().dex_file, offset);
...
void CodeGeneratorX86::EmitLinkerPatches(ArenaVector<LinkerPatch>* linker_patches) {
  ...
  for (const PcRelativeDexCacheAccessInfo& info : pc_rel_dex_cache_patches_) {
    linker_patches->push_back(LinkerPatch::DexCacheArrayPatch(info.label.Position() - 4u,
