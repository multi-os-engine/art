...
TEST(StackMapTest, TestShareDexRegisterMap) {
  ...
  uint32_t dex_reg_ = 2;
  // First stack map.
  stream.AddStackMapEntry(0, 64, 0x3, &sp_mask, dex_reg_, 0);
  ...
  DexRegisterMap dex_registers0 = ci.GetDexRegisterMapOf(sm0, dex_reg_);
  ASSERT_EQ(0, dex_registers0.GetMachineRegister(0, dex_reg_));
  ASSERT_EQ(-2, dex_registers0.GetConstant(1, dex_reg_));

  // Verify second stack map.
  StackMap sm1 = ci.GetStackMapAt(1);
  DexRegisterMap dex_registers1 = ci.GetDexRegisterMapOf(sm1, dex_reg_);
  ASSERT_EQ(0, dex_registers1.GetMachineRegister(0, dex_reg_));
  ASSERT_EQ(-2, dex_registers1.GetConstant(1, dex_reg_));

  // Verify third stack map.
  StackMap sm2 = ci.GetStackMapAt(2);
  DexRegisterMap dex_registers2 = ci.GetDexRegisterMapOf(sm2, dex_reg_);
  ASSERT_EQ(2, dex_registers2.GetMachineRegister(0, dex_reg_));
  ASSERT_EQ(-2, dex_registers2.GetConstant(1, dex_reg_));
