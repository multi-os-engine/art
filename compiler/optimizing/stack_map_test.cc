...
TEST(StackMapTest, Test1) {
  ...
  ASSERT_EQ(0u, index0);
  ASSERT_EQ(1u, index1);
  DexRegisterLocation location0 = dictionary.GetDexRegisterLocation(index0);
  DexRegisterLocation location1 = dictionary.GetDexRegisterLocation(index1);
...
TEST(StackMapTest, TestNonLiveDexRegisters) {
  ...
  size_t index1 =  dex_register_map.GetDictionaryEntryIndex(
      1, number_of_dex_registers, number_of_dictionary_entries);
  ASSERT_EQ(DexRegisterDictionary::NoLocationEntryIndex(), index0);
  ASSERT_EQ(0u, index1);
  DexRegisterLocation location0 = dictionary.GetDexRegisterLocation(index0);
  DexRegisterLocation location1 = dictionary.GetDexRegisterLocation(index1);
