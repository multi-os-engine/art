...
class OptimizingCFITest  : public CFITest {
 public:
  ...
  void TestImpl(InstructionSet isa, const char* isa_str,
    ...
    compiler_options.SetGenerateGDBInformation(true);
    compiler_options.SetIncludeDebugSymbols(true);
    std::unique_ptr<const InstructionSetFeatures> isa_features;
    std::string error;
    isa_features.reset(InstructionSetFeatures::FromVariant(isa, "default", &error));
    HGraph graph(&arena);
    // Generate simple frame with some spills.
    auto code_gen = CodeGenerator::Create(&graph, isa, *isa_features.get(),
                                          compiler_options);
