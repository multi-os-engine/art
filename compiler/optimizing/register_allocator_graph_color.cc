...
void RegisterAllocatorGraphColor::AllocateRegisters() {
  ...
  for (bool processing_core_regs : {true, false}) {
    ...
    while (!successful && attempt++ < kMaxGraphColoringAttempts) {
      ...
      // TODO: Use a stack interface if possible.
      ArenaVector<LiveInterval*> pruned_intervals = PruneInterferenceGraph(num_registers);
...
ArenaVector<LiveInterval*> RegisterAllocatorGraphColor::PruneInterferenceGraph(size_t num_regs) {
  ...
  ArenaSet<LiveInterval*, decltype(choose_next_to_prune)> high_degree_worklist_(
  ...
  for (auto it = interference_graph_.begin(); it != interference_graph_.end(); ++it) {
    ...
    if (interval->HasRegister()) {
      ...
      // allowing us to count the number intervals live at this point.
      // (Otherwise, safepoint intervals would not be counting intervals
      // pruned before them.)
      low_degree_worklist_.push_front(interval);
  ...
  auto prune_interval = [this, num_regs,
    ...
    pruned_intervals.push_back(interval);
  ...
  while (!low_degree_worklist_.empty() || !high_degree_worklist_.empty()) {
    while (!low_degree_worklist_.empty()) {
      LiveInterval* interval = low_degree_worklist_.front();
      low_degree_worklist_.pop_front();
