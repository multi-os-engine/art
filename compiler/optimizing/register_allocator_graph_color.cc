...
void RegisterAllocatorGraphColor::AllocateRegisters() {
  LOG(ERROR) << "Allocating registers with graph coloring!";  // TODO
  ProcessInstructions();
...
//       those that don't.
static bool ChooseNextToPrune(LiveInterval* lhs, LiveInterval* rhs) {
  if (lhs->RequiresRegister() != rhs->RequiresRegister()) {
    ...
    return RegisterAllocatorGraphColor::CmpIntervalPtr(lhs, rhs);
  }
}
...
void RegisterAllocatorGraphColor::PruneInterferenceGraph(
  ...
  ArenaSet<LiveInterval*, decltype(&ChooseNextToPrune)> high_degree_worklist(
