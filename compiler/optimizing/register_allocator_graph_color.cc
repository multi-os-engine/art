...
void RegisterAllocatorGraphColor::AllocateRegisters() {
  ProcessInstructions();
  for (bool processing_core_registers : {true, false}) {
    ArenaVector<LiveInterval*>& intervals = processing_core_registers
        ? core_intervals_
        : fp_intervals_;
    size_t num_registers = processing_core_registers
        ? codegen_->GetNumberOfCoreRegisters()
        : codegen_->GetNumberOfFloatingPointRegisters();
    BuildInterferenceGraph(intervals);
    PruneInterferenceGraph(num_registers);
    ColorInterferenceGraph(num_registers);
  }
...
void RegisterAllocatorGraphColor::ProcessInstructions() {
  for (HLinearPostOrderIterator it(*codegen_->GetGraph()); !it.Done(); it.Advance()) {
    ...
    for (HInstructionIterator instr_it(block->GetInstructions()); !instr_it.Done(); instr_it.Advance())
      if (instr_it.Current()->GetLocations() != nullptr)
        ProcessInstruction(instr_it.Current());

    for (HInstructionIterator phi_it(block->GetPhis()); !phi_it.Done(); phi_it.Advance())
      if (phi_it.Current()->GetLocations() != nullptr)
...
void RegisterAllocatorGraphColor::ProcessInstruction(HInstruction* instruction) {
  ...
  ProcessFixedInputLocations(instruction);
  ...
  // TODO: Safe points! Apparently need to AddSafepoint for intervals when necessary

  ProcessFixedOutputLocations(instruction);
...
// TODO: Factor out into register_allocator_common
void RegisterAllocatorGraphColor::ProcessFixedInputLocations(HInstruction* instruction) {
  LocationSummary* locations = instruction->GetLocations();
  size_t position = instruction->GetLifetimePosition();
...
// TODO: Factor out into register_allocator_common
void RegisterAllocatorGraphColor::ProcessFixedOutputLocations(HInstruction* instruction) {
  LocationSummary* locations = instruction->GetLocations();
  LiveInterval* interval = instruction->GetLiveInterval();
  size_t position = instruction->GetLifetimePosition();
...
void RegisterAllocatorGraphColor::PruneInterferenceGraph(size_t num_regs) {
  ...
  for (auto& node_and_adj : interference_graph_) {
    if (node_and_adj.first->IsFixed()) {
      // Never prune physical register intervals
      // TODO: What about constants?
      DCHECK(node_and_adj.first->HasRegister());
    } else if (node_and_adj.second.size() < num_regs) {
      low_degree_worklist_.push_back(node_and_adj.first);
    } else {
      high_degree_worklist_.Insert(node_and_adj.first);
    }
  }
...
void RegisterAllocatorGraphColor::InsertParallelMoveAtExitOf(HBasicBlock* block,
                                                             ...
                                                             Location source,
                                                             Location destination) const {
  DCHECK(IsValidDestination(destination)) << destination << " " << instruction->GetKind();
  if (source.Equals(destination)) return;
