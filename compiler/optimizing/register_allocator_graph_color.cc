...
void RegisterAllocatorGraphColor::AllocateRegisters() {
  ...
  for (bool processing_core_regs : {true, false}) {
    ...
    while (!successful && attempt++ < kMaxGraphColoringAttempts) {
      ...
      successful = ColorInterferenceGraph(num_registers, processing_core_regs);
    }

    // TODO
    if (kIsDebugBuild && !successful) {
      Locks::mutator_lock_->ReaderLock(Thread::Current());
      DCHECK(false) << "Exceeded max graph coloring attempts for method: "
...
void RegisterAllocatorGraphColor::SplitAtRegisterUses(LiveInterval* interval) {
  ...
  while (use != kNoLifetime) {
    ...
    if (interval->GetStart() + 1 < use && use < interval->GetEnd() + 1) {
      interval = Split(interval, use - 1);
    } else {
    ...
    if (interval->GetStart() < use + 1 && use + 1 < interval->GetEnd()) {
      interval = Split(interval, use + 1);
    } else {
      // Not enough room to split.
    }
...
void RegisterAllocatorGraphColor::BuildInterferenceGraph(ArenaVector<LiveInterval*>& intervals,
  ...
  for (LiveInterval* interval : intervals) {
    for (LiveInterval* sibling = interval; sibling != nullptr; sibling = sibling->GetNextSibling()) {
...
void RegisterAllocatorGraphColor::PruneInterferenceGraph(size_t num_regs) {
  ...
  // Note that this closure must *not* rely on state of the LiveInterval
  // that can change while pruning the interference graph.
...
bool RegisterAllocatorGraphColor::ColorInterferenceGraph(size_t num_regs,
                                                         bool processing_core_regs) {
  DCHECK_LE(num_regs, 64u) << "If there are more than 64 available registers, "
      << "conflict_mask will need to be a bit vector rather than a uint64";
  ...
  while (!pruned_intervals_.empty()) {
    ...
    for (LiveInterval* adj : interference_graph_.at(interval)) {
      if (adj->HasRegister()) {
        ...
        DCHECK(adj->GetParent()->GetDefinedBy()->IsConstant()
               || adj->GetParent()->HasSpillSlot()
               || !successful) << "Previous interval has not been assigned "
               << "a location, yet the register allocator claims success";
      }
    }
  ...
  if (!successful) {
    // Reset all register assignments for the next coloring attempt.
...
void RegisterAllocatorGraphColor::AllocateSpillSlotFor(LiveInterval* interval) {
  ...
  if (parent->HasSpillSlot()) {
    ...
    // TODO: Could technically color stack slots if we wanted to.
  }
}
...
void RegisterAllocatorGraphColor::Resolve() {
  codegen_->InitializeCodeGeneration(spill_slot_counter_,
                                     max_safepoint_live_core_regs_,  // TODO: These should probably be maxed out at the total number of registers
                                     max_safepoint_live_fp_regs_,
                                     reserved_out_slots_,
