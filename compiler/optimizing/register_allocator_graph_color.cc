...
struct CoalesceOpportunity : public ArenaObject<kArenaAllocRegisterAllocator> {
  CoalesceOpportunity(InterferenceNode* _a,
        ...
        : a(_a),
          ...
          priority(ComputeCoalescePriorityFromKind(kind)) {}
  ...
  // TODO: Add priority based on loop information.
  size_t priority;
};
...
static size_t LoopDepthAt(HInstruction* instruction) {
  // TODO: Right now the only information we have is whether a block is in a loop;
  //       we don't know the nesting depth of the loop. This is a temporary workaround
  //       to compute it.
...
// Returns the cost of spilling a register used at a particular instruction, based on heuristics.
static size_t SpillWeightForUseAt(HInstruction* instruction) {
  size_t weight = 1;
  if (instruction->GetBlock()->IsSingleJump()) {
    weight *= kSingleJumpBlockWeightMultiplier;
  }
...
class InterferenceNode : public ArenaObject<kArenaAllocRegisterAllocator> {
 public:
  ...
  const ArenaSet<InterferenceNode*>& GetAdjacentNodes() const {
    return adjacent_nodes_;
  }

  size_t GetOutDegree() const {
    // Pre-colored nodes have infinite degree.
    return interval_->HasRegister() ? std::numeric_limits<size_t>::max() : out_degree_;
  }
 ...
 private:
  ...
  ArenaSet<InterferenceNode*> adjacent_nodes_;
...
// This method resides here, because it needs to refer to the interference node class above.
void RegisterAllocatorGraphColor::SetupPhysicalNodes() {
  // Before we ask for blocked registers, set them up in the code generator.
  codegen_->SetupBlockedRegisters();
  ...
  for (size_t i = 0; i < codegen_->GetNumberOfCoreRegisters(); ++i) {
    ...
    if (codegen_->IsBlockedCoreRegister(i)) {
      interval->AddRange(0, liveness_.GetMaxLifetimePosition());
    }
  }
    ...
    while (true) {
      ...
      if (successful) {
        ...
        if (processing_core_regs) {
          max_safepoint_live_core_regs_ = max_safepoint_live_regs;
        } else {
          max_safepoint_live_fp_regs_= max_safepoint_live_regs;
        }
...
void RegisterAllocatorGraphColor::BuildInterferenceGraph(
  ...
  // from the live set. Nodes interfere if they are ever in the live set at the same time.
  ...
  for (InterferenceNode* node : prunable_nodes_) {
    ...
    if (next_sibling != nullptr) {
      ...
      if (it != interval_node_map_.end()) {
        ...
        CreateCoalesceOpportunity(node, sibling_node, CoalesceKind::kAdjacentSibling);
      }
    }
...
void RegisterAllocatorGraphColor::PruneInterferenceGraph(size_t num_regs) {
  ...
  while (true) {
    if (!simplify_worklist_.empty()) {
      ...
      DCHECK_LT(node->GetOutDegree(), num_regs) << "Nodes in simplify worklist should be low degree";
      DCHECK(!node->IsMoveRelated()) << "Nodes in the simplify worklist should not be move related";
      ...
      if (node->stage == NodeStage::kFreezeWorklist) {
        DCHECK_LT(node->GetOutDegree(), num_regs) << "Nodes in the freeze worklist should be low degree";
        DCHECK(node->IsMoveRelated()) << "Nodes in the freeze worklist should be move related";
        FreezeMoves(node);
        PruneNode(node, num_regs);
      }
      ...
      if (node->stage == NodeStage::kSpillWorklist) {
        DCHECK_GE(node->GetOutDegree(), num_regs) << "Nodes in the spill worklist should be high degree";
        FreezeMoves(node);
        PruneNode(node, num_regs);
      }
    } else {
...
// TODO: Use this in coalesce heuristics too.
