...
class InterferenceNode : public ArenaObject<kArenaAllocRegisterAllocator> {
 public:
  ...
  // Notice that we enforce that pre-colored nodes do not have outgoing interferences.
  // This is to save memory.
  void AddInterference(InterferenceNode* other) {
    DCHECK(!interval_->HasRegister());
...
CoalesceOpportunity::CoalesceOpportunity(InterferenceNode* a_in, InterferenceNode* b_in)
  ...
  DCHECK_EQ(a->Interval()->HasHighInterval(), b->Interval()->HasHighInterval())
  ...
  for (bool processing_core_regs : {true, false}) {
    ...
    while (true) {
      ...
      ArenaVector<InterferenceNode*> safepoints(
          allocator_->Adapter(kArenaAllocRegisterAllocator));
      BuildInterferenceGraph(intervals, safepoints);
...
static void AddPotentialInterference(InterferenceNode* from,
                                     InterferenceNode* to,
...
void RegisterAllocatorGraphColor::CreateCoalesceOpportunity(InterferenceNode* a,
                                                            InterferenceNode* b) {
  DCHECK(a->Interval()->HasHighInterval() == b->Interval()->HasHighInterval())
...
void RegisterAllocatorGraphColor::FindCoalesceOpportunities() {
  ...
  for (InterferenceNode* node : prunable_nodes_) {
    ...
    // TODO: The following two blocks are partially copied from liveness analysis.
...
bool RegisterAllocatorGraphColor::ChooseHigherPriorityNode(const InterferenceNode* lhs,
  ...
  // TODO: Not necessary for a deque.
...
void RegisterAllocatorGraphColor::PruneInterferenceGraph(size_t num_regs) {
  ...
  // TODO: Check for duplicates if kIsDebugBuild
}
...
bool RegisterAllocatorGraphColor::PrecoloredHeuristic(InterferenceNode* from,
  ...
  if (num_regs < 100) return false;  // TODO: PrecoloredHeuristic currently causes failed checks.
  LOG(ERROR) << "Merging into " << into->Interval()->GetRegister()
      << " " << codegen_->IsCoreCalleeSaveRegister(into->Interval()->GetRegister())
      << " " << from->HasInterference(into)
      << " " << this->DumpInterval(from->Interval());

  // Reasons an adjacent node can be "ok":
  // (1) If `adj` is low degree, interference with `into` will not affect its existing
  //     colorable guarantee. (Notice that coalescing does not affect its degree.)
  // (2) If `adj` is precolored, it already interferes with `into`. See (3).
  // (3) If there's already an interference with `into`, coalescing will not add interferences.
  for (InterferenceNode* adj : from->Adj()) {
    if (/*(1)*/adj->Degree() < num_regs ||
        /*(2)*/adj->Precolored() ||
        /*(3)*/adj->HasInterference(into)) {
    } else {
      return false;
    }
  }
...
// TODO: Lots of room for mistakes here.
...
bool RegisterAllocatorGraphColor::ColorInterferenceGraph(size_t num_regs,
  ...
  while (!pruned_nodes_.empty()) {
    ...
    InterferenceNode* rep = node->Alias();
    if (rep != node) {
      // This node was coalesced with another.
      LiveInterval* rep_interval = rep->Interval();
      if (rep_interval->HasRegister()) {
        reg = rep_interval->GetRegister();
      } else {
        LOG(FATAL) << node->Degree() << " " << rep->Degree() << " "
