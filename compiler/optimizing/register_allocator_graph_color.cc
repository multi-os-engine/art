...
RegisterAllocatorGraphColor::RegisterAllocatorGraphColor(ArenaAllocator* allocator,
      ...
      : RegisterAllocator(allocator, codegen, liveness),
        ...
        coloring_attempt_allocator_(new (allocator) ArenaAllocator(allocator->GetArenaPool())),
        node_id_counter_(0),
        interval_node_map_(coloring_attempt_allocator_->Adapter(kArenaAllocRegisterAllocator)),
        prunable_nodes_(coloring_attempt_allocator_->Adapter(kArenaAllocRegisterAllocator)),
        pruned_nodes_(coloring_attempt_allocator_->Adapter(kArenaAllocRegisterAllocator)),
        simplify_worklist_(coloring_attempt_allocator_->Adapter(kArenaAllocRegisterAllocator)),
        freeze_worklist_(coloring_attempt_allocator_->Adapter(kArenaAllocRegisterAllocator)),
...
void RegisterAllocatorGraphColor::BuildInterferenceGraph(
  ...
  // Nodes live at the current position in the line sweep.
  ...
  // Line sweep. When we encounter the beginning of a range, we add the corresponding node to the
...
static std::bitset<kMaxNumRegs> BuildConflictMask(Container& intervals) {
  ...
  for (InterferenceNode* adjacent : intervals) {
    LiveInterval* conflicting = adjacent->Interval();
...
size_t RegisterAllocatorGraphColor::ComputeMaxSafepointLiveRegisters(
  ...
  for (InterferenceNode* safepoint : safepoints) {
    ...
    std::bitset<kMaxNumRegs> conflict_mask = BuildConflictMask(safepoint->Adj());
