...
void RegisterAllocatorGraphColor::AllocateRegisters() {
  ...
  // (5) Resolve assigned locations.
  Resolve();

  // (6) Deconstruct SSA form.
  SsaDeconstruction(allocator_, codegen_, liveness_,
                    max_safepoint_live_core_regs_
                    + max_safepoint_live_fp_regs_).DeconstructSsa();
}
...
void RegisterAllocatorGraphColor::Resolve() {
  ...
  ResolveTempLocations(temp_intervals_);
}
