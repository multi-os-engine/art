...
void RegisterAllocatorGraphColor::ProcessInstruction(HInstruction* instruction) {
  ...
  ProcessCatchPhiFor(instruction);
...
void RegisterAllocatorGraphColor::ProcessCatchPhiFor(HInstruction* instruction) {
  ...
  for (auto it = range_endpoints.begin(); it != range_endpoints.end(); ++it) {
    ...
    if (interval->IsSlowPathSafepoint()) {
      if (is_range_beginning) {
        ...
        // By adding the following interval in the algorithm, we can compute this
        // maximum before updating locations.
        size_t max_live_regs = 0;
        for (LiveInterval* live_interval : live) {
          if (!live_interval->HasSpillSlot()) {
            ++max_live_regs;
          }
        }
        ...
        *max_safepoint_live_regs = max_live_regs;
      } else {
        // Ignore end of slow path safe point
      }
    } else if (is_range_beginning) {
      DCHECK(live.Find(interval) == live.end());
      if (interference_graph_.count(interval) == 0) {
        interference_graph_[interval] = std::unordered_set<LiveInterval*>();
      }
      for (LiveInterval* conflicting : live) {
        interference_graph_[interval].insert(conflicting);
        interference_graph_[conflicting].insert(interval);
      }
      live.Insert(interval);
    } else {
      DCHECK(live.Find(interval) != live.end());
      live.Erase(live.Find(interval));
    }
  }
  DCHECK(live.Empty());

  // TODO: Should check that there are no self edges, since that would
  //       mean that there is a conflict between intervals that both
  //       need the same physical register.
}
...
void RegisterAllocatorGraphColor::PruneInterferenceGraph(size_t num_regs) {
  ...
  // TODO: Just to be extra sure: does this create references to the original
  //       adjacency list, or references to pair that copies the adjacency list?
  //       (General C++ question here)
  ...
  while (!low_degree_worklist_.empty() || !high_degree_worklist_.Empty()) {
    ...
    if (!high_degree_worklist_.Empty()) {
      ...
      //       Also, should prioritize intervals in loops. Need to make sure too
      //       that short intervals for instructions that require a register
      //       location should *not* be pruned here.
...
void RegisterAllocatorGraphColor::ColorInterferenceGraph(size_t num_regs,
                                                         bool processing_core_regs) {
  // TODO: Can we assume <= 64 registers? If so, just use an unsigned long long
  std::vector<bool> free_regs(num_regs);  // TODO: Always use ArenaVector?
  while (!pruned_intervals_.empty()) {
    // TODO: Need to handle high/low intervals carefully
...
void RegisterAllocatorGraphColor::AllocateSpillSlotFor(LiveInterval* interval) {
  ...
  if (parent->HasSpillSlot()) {
    ...
    // TODO: Color stack slots
    // Note: Will eventually need to care about register pairs, and also be
    //       careful about parent vs. siblings, and correct stack size.
    DCHECK(false);
  }
}
...
void RegisterAllocatorGraphColor::Resolve() {
  codegen_->InitializeCodeGeneration(spilled_intervals_.size(),
                                     max_safepoint_live_core_regs_,
                                     max_safepoint_live_fp_regs_,
                                     reserved_out_slots_,
