...
static inline bool HasShifterOperand(HInstruction* instr) {
  bool res = instr->IsAdd() || instr->IsAnd() || instr->IsOr() || instr->IsSub() || instr->IsXor();
  // Check that any instruction with a shifter operand is a binary operation.
  DCHECK(!res || instr->IsBinaryOperation());
  return res;
}

static inline bool ShifterOperandSupportsExtension(HInstruction* instruction) {
  DCHECK(HasShifterOperand(instruction));
  return instruction->IsAdd() || instruction->IsAnd() || instruction->IsSub();
}
