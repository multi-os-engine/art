...
static int NumberOfLiveXMMNonVolatiles(CodeGeneratorX86_64* codegen, LocationSummary* locations) {
...
static void SaveLiveXMMNonVolatiles(CodeGeneratorX86_64* codegen, LocationSummary* locations) {
  RegisterSet* register_set = locations->GetLiveRegisters();
...
static void RestoreLiveXMMNonVolatiles(CodeGenerator* codegen, LocationSummary* locations) {
  RegisterSet* register_set = locations->GetLiveRegisters();
  size_t stack_offset = 0;
  for (size_t i = 0, e = codegen->GetNumberOfFloatingPointRegisters(); i < e; ++i) {
    if (codegen->IsFloatingPointCalleeSaveRegister(i) &&
...
static void GenFPToFPCall(HInvoke* invoke, CodeGeneratorX86_64* codegen,
  ...
  int num_xmms_to_save = NumberOfLiveXMMNonVolatiles(codegen, locations);
  ...
  if (num_xmms_to_save != 0) {
    ...
    SaveLiveXMMNonVolatiles(codegen, locations);
  }
  __ gs()->call(Address::Absolute(GetThreadOffset<kX86_64WordSize>(entry), true));
  codegen->RecordPcInfo(invoke, invoke->GetDexPc());
  if (num_xmms_to_save != 0) {
    RestoreLiveXMMNonVolatiles(codegen, locations);
