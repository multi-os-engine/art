...
void IntrinsicCodeGeneratorX86_64::VisitMathRoundFloat(HInvoke* invoke) {
  ...
  __ movl(out, Immediate(0));  // NaN maps to 0 (do not set flags)
...
void IntrinsicCodeGeneratorX86_64::VisitMathRoundDouble(HInvoke* invoke) {
  ...
  __ cvtsi2sd(t2, out, true);
  __ comisd(t1, t2);
  __ j(kAboveEqual, &done);  // clip to max (already in out)
  __ movq(out, Immediate(0));  // NaN maps to 0 (do not set flags)
  __ j(kUnordered, &done);
  __ cvttsd2si(out, t1, true);
  __ Bind(&done);
}
