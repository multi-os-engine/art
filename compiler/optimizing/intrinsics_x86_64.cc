...
void IntrinsicCodeGeneratorX86_64::VisitStringNewStringFromBytes(HInvoke* invoke) {
  X86_64Assembler* assembler = GetAssembler();
  LocationSummary* locations = invoke->GetLocations();
  LOG(INFO) << " x86_64 - GENERATING INTRINSIC STRING FROM BYTES";
...
void IntrinsicCodeGeneratorX86_64::VisitStringNewStringFromChars(HInvoke* invoke ATTRIBUTE_UNUSED) {
  X86_64Assembler* assembler = GetAssembler();
  LOG(INFO) << " x86_64 - GENERATING INTRINSIC STRING FROM CHARS";
...
void IntrinsicCodeGeneratorX86_64::VisitStringNewStringFromString(HInvoke* invoke) {
  X86_64Assembler* assembler = GetAssembler();
  LocationSummary* locations = invoke->GetLocations();
  LOG(INFO) << " x86_64 - GENERATING INTRINSIC STRING FROM STRING";
