...
static void MathAbsFP(LocationSummary* locations, bool is64bit,
  ...
  if (output.IsFpuRegister()) {
    ...
    if (is64bit) {
      __ movsd(xmm_temp, codegen->LiteralInt64(INT64_C(0x7FFFFFFFFFFFFFFF)));
      __ andpd(output.AsFpuRegister<XmmRegister>(), xmm_temp);
    } else {
      __ movss(xmm_temp, codegen->LiteralInt32(INT32_C(0x7FFFFFFF)));
...
static void GenMinMaxFP(LocationSummary* locations, bool is_min, bool is_double,
  ...
  __ Bind(&nan);
  if (is_double) {
    __ movsd(out, codegen->LiteralInt64(INT64_C(0x7FF8000000000000)));
  } else {
    __ movss(out, codegen->LiteralInt32(INT32_C(0x7FC00000)));
  }
  __ jmp(&done);
