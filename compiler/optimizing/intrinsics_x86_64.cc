...
void IntrinsicCodeGeneratorX86_64::VisitSystemArrayCopy(HInvoke* invoke) {
  ...
  // forward copying.
  if (srcPos.IsConstant()) {
    int32_t src_constant = srcPos.GetConstant()->AsIntConstant()->GetValue();
    if (destPos.IsConstant()) {
      // Checked when building locations.
      DCHECK(!optimizations.GetDestinationIsSource()
             || (src_constant >= destPos.GetConstant()->AsIntConstant()->GetValue()));
    } else {
      if (!optimizations.GetDestinationIsSource()) {
        __ j(kNotEqual, &ok);
      }
      __ cmpl(destPos.AsRegister<CpuRegister>(), Immediate(src_constant));
    ...
    if (destPos.IsConstant()) {
      int32_t dest_constant = destPos.GetConstant()->AsIntConstant()->GetValue();
      __ cmpl(srcPos.AsRegister<CpuRegister>(), Immediate(dest_constant));
      __ j(kLess, slow_path->GetEntryLabel());
    } else {
      __ cmpl(srcPos.AsRegister<CpuRegister>(), destPos.AsRegister<CpuRegister>());
      __ j(kLess, slow_path->GetEntryLabel());
    }
  }
  ...
  if (srcPos.IsConstant()) {
    int32_t constant = srcPos.GetConstant()->AsIntConstant()->GetValue();
    __ leal(temp1, Address(src, element_size * constant + offset));
  } else {
    __ leal(temp1, Address(src, srcPos.AsRegister<CpuRegister>(), ScaleFactor::TIMES_4, offset));
  }

  if (destPos.IsConstant()) {
    int32_t constant = destPos.GetConstant()->AsIntConstant()->GetValue();
    __ leal(temp2, Address(dest, element_size * constant + offset));
  } else {
    __ leal(temp2, Address(dest, destPos.AsRegister<CpuRegister>(), ScaleFactor::TIMES_4, offset));
  }
