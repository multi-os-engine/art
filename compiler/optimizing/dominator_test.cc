...
static void TestCode(const uint16_t* data, int length, const int* Block numbers, size_t blocks_length) {
  ArenaPool pool;
  ...
  ASSERT_EQ(graph->blocks()->Size(), Block numbers_length);
  for (size_t i = 0; i < Block numbers_length; i++) {
    if (blocks[i] == -1) {
