...
bool DeadPhiHandling::UpdateType(HPhi* phi) {
  ...
  if (new_type == Primitive::kPrimNot) {
    ...
    if (ref_equivalent != nullptr) {
      // Since `ref_equivalent` may be dead, we keep `phi` which must be live.
      ref_equivalent->ReplaceWith(phi);
      ref_equivalent->SetDead();
    }
  }
    ...
    for (HExceptionHandlerIterator it(try_entry); !it.Done(); it.Advance()) {
      ...
      for (size_t vreg = 0, e = current_locals_->size(); vreg < e; ++vreg) {
        ...
        if (local_value == nullptr) {
          ...
          // Vreg defined at all instructions throwing into `catch_block` so far.
          // Record the local value in the catch phi.
