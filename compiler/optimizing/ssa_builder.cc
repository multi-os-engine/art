...
void SsaBuilder::BuildSsa() {
  ...
  // 3) Remove dead phis to remove phis only used by environments. At the DEX level,
  // the type of these phis does not need to be consistent, but our code generator
  // will complain if the inputs of a phi do not have the same type (modulo the
