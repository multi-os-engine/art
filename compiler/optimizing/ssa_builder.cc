...
static HDoubleConstant* GetDoubleEquivalent(HLongConstant* constant) {
  // We place the floating point constant next to this constant. If there is
  // already a constant with the expected type, we know it is the floating
  // point equivalent of this constant.
...
static HPhi* GetFloatOrDoubleEquivalentOfPhi(HPhi* phi, Primitive::Type type) {
  ...
  if (next == nullptr
    ...
    for (size_t i = 0, e = phi->InputCount(); i < e; ++i) {
      // Copy the inputs. Note that the grpah may not be correctly typed by doing this copy,
