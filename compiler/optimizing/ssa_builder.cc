...
void HandleEqualityWithNullConstant::Run() {
  ...
  for (HReversePostOrderIterator itb(*graph_); !itb.Done(); itb.Advance()) {
    for (HInstructionIterator it(itb.Current()->GetInstructions()); !it.Done(); it.Advance()) {
      ...
      // If any of the sides is a phi make sure that we don't let alive equivalent phis
      // (i.e. having the same dex register and the same type).
      // Otherwise continue to the next equality instruction.
      if (phi == nullptr) {
        continue;
      }
      HInstruction* next = phi->GetNext();
      // Go over equivalent phis that are not reference (and therefore floating point).
      while (next != nullptr
          && next->IsPhi()
