    ...
    for (size_t i = 0, e = phi->InputCount(); i < e; ++i) {
      ...
      if (input->GetType() != common_type) {
        ...
        if (equivalent->IsPhi()) {
          ...
          AddDependentInstructionsToWorklist(input, worklist);
        }
      }
    }
...
static bool IsArrayGetOnIntegralArray(HArrayGet* aget) SHARED_REQUIRES(Locks::mutator_lock_) {
  ...
  DCHECK(array_type.IsPrimitiveArrayClass()) << aget->GetId() << array_type;
  Handle<mirror::Class> array_type_handle = array_type.GetTypeHandle();
...
void SsaBuilder::FixAmbiguousArrayGets() {
  DCHECK(after_rtp_);
...

  {
    ...
    for (HArrayGet* aget_int : ambiguous_agets_) {
      HArrayGet* aget_float = aget_int->GetPrevious()->AsArrayGet();
      DCHECK(aget_float != nullptr && aget_float->IsEquivalentOf(aget_int));
  ...
  ambiguous_agets_.clear();

  RunPrimitiveTypePropagation();
}

void SsaBuilder::MarkDeadPhis() {
  ArenaVector<HPhi*> worklist(GetGraph()->GetArena()->Adapter(kArenaAllocSsaBuilder));
  ...
  ArenaSet<HPhi*> initially_live(GetGraph()->GetArena()->Adapter());
  ...
  for (HReversePostOrderIterator it(*GetGraph()); !it.Done(); it.Advance()) {
    ...
    for (HInstructionIterator inst_it(block->GetPhis()); !inst_it.Done(); inst_it.Advance()) {
      ...
      bool is_live = false;
      if (GetGraph()->IsDebuggable() && phi->HasEnvironmentUses()) {
        is_live = true;
      } else {
...
void SsaBuilder::EliminateDeadPhis() {
    ...
    while (current != nullptr) {
      ...
      if (phi->IsDead()) {
        ...
        block->RemovePhi(
            phi,
            /* ensure_safety */ false,
            // Do not bother removing uses. All users will be removed too.
            /* remove_as_user */ false);
      }
      current = next;
    }
  }
...
HArrayGet* SsaBuilder::GetFloatOrDoubleEquivalentOfArrayGet(HArrayGet* aget) {
  ...
  if (after_rtp_) {
    ...
    HArrayGet* equivalent = aget->GetPrevious()->AsArrayGet();
    DCHECK(equivalent->IsEquivalentOf(aget));
...
void SsaBuilder::VisitArrayGet(HArrayGet* aget) {
  ...
  if (Primitive::IsIntOrLongType(type)) {
    HArrayGet* equivalent = new (GetGraph()->GetArena()) HArrayGet(
        aget->GetArray(),
        aget->GetIndex(),
        type == Primitive::kPrimInt ? Primitive::kPrimFloat : Primitive::kPrimDouble,
        aget->GetDexPc());
    aget->GetBlock()->InsertInstructionBefore(equivalent, aget);
