...
void SsaBuilder::VisitBasicBlock(HBasicBlock* block) {
  ...
  // Infer try block membership from any of block's already visited
  // predecessors. Choosing the first predecessor avoid selecting a back edge.
  // Catch blocks are always assumed not in a try block.
  HTryBoundary* try_entry = nullptr;
  if (!block->IsEntryBlock() && !block->IsCatchBlock()) {
    ...
    try_entry = first_predecessor->ComputeTryEntryAfter();
  }
  block->SetTryEntry(try_entry);

  if (block->IsCatchBlock()) {
    // Catch phis were already created and inputs collected from throwing sites.
    // All that is left to do is add the phis into the block.
...
void SsaBuilder::VisitInstruction(HInstruction* instruction) {
  ...
  if (instruction->CanThrow() && instruction->GetBlock()->IsInTry()) {
    HTryBoundary* current_try_block = instruction->GetBlock()->GetTryEntry();
    for (HExceptionHandlerIterator it(*current_try_block); !it.Done(); it.Advance()) {
      ...
      for (size_t i = 0, e = current_locals_->Size(); i < e; ++i) {
        ...
        if (handler_phi != nullptr) {
          DCHECK(handler_phi->IsPhi());
          if (current_value == nullptr) {
            handler_phi->GetBlock()->RemovePhi(handler_phi->AsPhi());
