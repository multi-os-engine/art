...
static bool IsArrayGetOnIntegralArray(HArrayGet* aget) SHARED_REQUIRES(Locks::mutator_lock_) {
  ...
  // Assume that ReferenceTypePropagation computed the type of the array.
  // The only case when there is not enough typing information in the bytecode
  // is when the array is null. However, such methods are punted to interpreter.
...
HArrayGet* SsaBuilder::GetFloatOrDoubleEquivalentOfArrayGet(HArrayGet* aget) {
  ...
  if (agets_fixed_) {
    ...
    // This is an ambiguous ArrayGet which has not been resolved yet. In step 1),
    // SsaBuilder should have created a float/double equivalent which we will
    // use until the type is resolved.
