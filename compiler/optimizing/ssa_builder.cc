...
static Primitive::Type GetArrayComponentType(HInstruction* array)
...
bool SsaBuilder::FixAmbiguousArrayOps() {
...

  {
    ...
    for (HArrayGet* aget_int : ambiguous_agets_) {
      ...
      Primitive::Type array_type = GetArrayComponentType(array);
    ...
    for (HArraySet* aset : ambiguous_asets_) {
      ...
      if (Primitive::IsFloatingPointType(array_type)) {
        if (!Primitive::IsFloatingPointType(value_type)) {
          ...
          // Array elements are floating-point but the value is integral.
          // Replace the value with its float equivalent. This should always
          // succeed in verified code.
...
HArrayGet* SsaBuilder::GetFloatOrDoubleEquivalentOfArrayGet(HArrayGet* aget) {
  ...
  if (agets_fixed_) {
    ...
    if (kIsDebugBuild) {
      ScopedObjectAccess soa(Thread::Current());
      DCHECK(Primitive::IsIntOrLongType(GetArrayComponentType(aget->GetArray())));
    }
    return nullptr;
  } else {
