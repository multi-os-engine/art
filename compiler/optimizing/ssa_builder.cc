...
void SsaBuilder::BuildSsa() {
  ...
  // 3) Mark dead phis. This will mark phis which are not used by instructions or
  // other live phis. If compiling as debuggable code, phis will also be kept live
  // if they have an environment use. The marking allows type propagation to know
  // which phis it needs to handle. We mark but do not eliminate: the elimination
  // will be done in step 10).
  ...
  // 6) Mark dead phis again. Step 4) may have introduced new phis. Step 5) might
  // have enable death of new phis.
...
HInstruction* SsaBuilder::GetFloatOrDoubleEquivalent(HInstruction* user,
  ...
  if (value->IsArrayGet()) {
    ...
    if (aget->GetType() != type && aget->IsTypeFixed()) {
      DCHECK(value->GetBlock()->GetGraph()->IsDebuggable()) << aget->GetType() << type;
      DCHECK(user->IsPhi());
      return nullptr;
    }
    aget->SetType(type);
...
void SsaBuilder::VisitLoadLocal(HLoadLocal* load) {
  ...
  if (value->IsArrayGet() && !value->AsArrayGet()->IsTypeFixed()) {
    for (HUseIterator<HInstruction*> use_it(load->GetUses()); !use_it.Done(); use_it.Advance()) {
      ...
      if (!user->IsPhi() && !user->IsStoreLocal() && (!user->IsArraySet() || index != 2)) {
