...
class CodeGenerator : public ArenaObject {
 public:
  ...
  void RecordPcDescriptor(uint32_t dex_pc) {
    struct PcDescriptor descriptor;
    descriptor.dex_pc = dex_pc;
    descriptor.native_pc = GetAssembler()->CodeSize();
 ...
 protected:
  explicit CodeGenerator(HGraph* graph)
      : frame_size_(0),
        ...
        descriptors_(graph->GetArena(), 32) {
 ...
 private:
  ...
  GrowableArray<PcDescriptor> descriptors_;
