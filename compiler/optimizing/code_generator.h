...
class CallingConvention {
 public:
  ...
  int GetNumberOfRegisters() const { return number_of_registers_; }

  T GetRegisterAt(int index) const { return registers_[index]; }

  uint8_t GetStackOffsetOf(int index) const {
 ...
 private:
  const T* registers_;
  const int number_of_registers_;
