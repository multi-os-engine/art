...
void InstructionCodeGeneratorARM::VisitPackedSwitch(HPackedSwitch* switch_instr) {
  ...
  if (num_entries < kPackedSwitchJumpTableThreshold || !codegen_->GetAssembler()->IsThumb()) {
    ...
    Register table_base_reg = locations->GetTemp(1).AsRegister<Register>();
    ...
    // TODO: &&.
    JumpTable* table = __ CreateJumpTable(labels, table_base_reg);

    // Remove the bias.
    Register key_reg;
    if (lower_bound != 0) {
      key_reg = temp_reg;
      __ sub(key_reg, value_reg, ShifterOperand(lower_bound));
    } else {
      key_reg = value_reg;
    }

    // Check whether the value is in the table, jump to default block if not.
    __ cmp(key_reg, ShifterOperand(num_entries - 1));
    __ b(codegen_->GetLabelOf(default_block), Condition::HI);

    // Load the displacement from the table.
    __ ldr(temp_reg, Address(table_base_reg, key_reg, Shift::LSL, 2));

    // Dispatch is a direct add to the PC (for Thumb2).
    __ Bind(table->GetAnchorLabel());
    __ add(PC, PC, ShifterOperand(temp_reg));
  }
}
