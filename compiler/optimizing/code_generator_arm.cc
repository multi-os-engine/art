...
class StackOverflowCheckSlowPathARM : public SlowPathCode {
 public:
  ...
  virtual void EmitNativeCode(CodeGenerator* codegen) OVERRIDE {
    __ Bind(GetEntryLabel());
    __ LoadFromOffset(kLoadWord, PC, TR, QUICK_ENTRYPOINT_OFFSET(kArmWordSize, pThrowStackOverflow).Int32Value());
  }
...
void CodeGeneratorARM::GenerateFrameEntry() {
  ...
  if (!skip_overflow_check) {
    if (kExplicitStackOverflowCheck) {
      ...
      __ AddConstant(IP, SP, -static_cast<int32_t>(kArmStackOverflowReservedBytes));
