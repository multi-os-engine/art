...
inline Condition ARMSignedCondition(IfCondition cond) {
  switch (cond) {
...
inline Condition ARMFPCondition(IfCondition cond) {
  switch (cond) {
...
void InstructionCodeGeneratorARM::GenerateFPJumps(HCondition* cond,
  ...
  Condition ccode = ARMFPCondition(if_cond);
...
void InstructionCodeGeneratorARM::GenerateLongComparesAndJumps(HCondition* cond,
  ...
  if (right.IsConstant()) {
    ...
    if (if_cond == kCondNE) {
      __ b(true_label, ARMSignedCondition(true_high_cond));
    } else if (if_cond == kCondEQ) {
      __ b(false_label, ARMSignedCondition(false_high_cond));
    } else {
      __ b(true_label, ARMSignedCondition(true_high_cond));
      __ b(false_label, ARMSignedCondition(false_high_cond));
    }
...
void InstructionCodeGeneratorARM::GenerateTestAndBranch(HInstruction* instruction,
  ...
  if (cond->IsIntConstant()) {
    ...
    if (!cond->IsCondition() || cond->AsCondition()->NeedsMaterialization()) {
      ...
      __ b(true_target, ARMSignedCondition(cond->AsCondition()->GetCondition()));
    }
  }
  if (false_target != nullptr) {
    __ b(false_target);
...
void InstructionCodeGeneratorARM::VisitCondition(HCondition* cond) {
  ...
  switch (cond->InputAt(0)->GetType()) {
    default: {
      ...
      __ it(ARMSignedCondition(cond->GetCondition()), kItElse);
      __ mov(locations->Out().AsRegister<Register>(), ShifterOperand(1),
             ARMSignedCondition(cond->GetCondition()));
      __ mov(locations->Out().AsRegister<Register>(), ShifterOperand(0),
             ARMSignedCondition(cond->GetOppositeCondition()));
      return;
    }
    case Primitive::kPrimLong:
