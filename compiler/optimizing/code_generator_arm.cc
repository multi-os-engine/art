...
class NullCheckSlowPath : public SlowPathCode {
 public:
  NullCheckSlowPath(uint32_t dex_pc) : dex_pc_(dex_pc) {}
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(NullCheckSlowPath);
};
...
static constexpr int kNumberOfPushedRegistersAtEntry = 1 + 2; // LR, R6, R6
...
void InstructionCodeGeneratorARM::VisitNullCheck(HNullCheck* instruction) {
  SlowPathCode* slow_path = new (GetGraph()->GetArena()) NullCheckSlowPath(instruction->GetDexPc());
