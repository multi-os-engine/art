...
void LocationsBuilderARM::HandleFieldGet(HInstruction* instruction, const FieldInfo& field_info) {
  ...
  bool object_field_get_with_read_barrier =
      (kForceReadBarrier || kUseReadBarrier) && (field_info.GetFieldType() == Primitive::kPrimNot);
  LocationSummary* locations =
  ...
  bool volatile_for_double = field_info.IsVolatile()
      ...
      && !codegen_->GetInstructionSetFeatures().HasAtomicLdrdAndStrd();
  bool overlap = object_field_get_with_read_barrier ||
    ...
    case Primitive::kPrimNot: {
      ...
      if (may_need_runtime_call_for_type_check) {
        ...
        codegen_->MaybeGenerateReadBarrier(
            instruction, temp1_loc, temp1_loc, array_loc, class_offset);

        if (kForceReadBarrier || kUseReadBarrier) {
          // Save the value of `temp1` into `temp2` before overwriting it
          ...
          // CodeGeneratorARM::GenerateReadBarrierWithLiveTemp instead
          // of CodeGeneratorARM::GenerateReadBarrier.
          //
          ...
          // art::arm::CodeGeneratorARM::GenerateReadBarrier.
        ...
        if (instruction->StaticTypeOfArrayIsObjectArray()) {
          ...
          if (kForceReadBarrier || kUseReadBarrier) {
            ...
            codegen_->GenerateReadBarrier(
                instruction, temp1_loc, temp1_loc, temp2_loc, super_offset);
          } else {
            // If heap poisoning is enabled, the `temp1` reference has
...
void CodeGeneratorARM::GenerateReadBarrierWithLiveTemp(HInstruction* instruction,
                                                       Location out,
                                                       Location ref,
                                                       Location obj,
                                                       uint32_t offset,
  ...
  // CodeGeneratorARM::GenerateReadBarrier is `live_temp`, passed
  // to the slow path.
  DCHECK(kForceReadBarrier || kUseReadBarrier);
  ReadBarrierForHeapReferenceSlowPathARM* slow_path = new (GetGraph()->GetArena())
      ReadBarrierForHeapReferenceSlowPathARM(instruction, out, ref, obj, offset, index);
