...
void InstructionCodeGeneratorARM::GenerateVolatileLoad(Register addr,
                                                       uint32_t offset,
                                                       Register out_lo,
                                                       Register out_hi) {
  if (offset != 0) {
...
void InstructionCodeGeneratorARM::GenerateVolatileStore(Register addr,
                                                        uint32_t offset,
                                                        Register value_lo,
                                                        Register value_hi,
                                                        Register temp1,
...
void LocationsBuilderARM::HandleFieldSet(HInstruction* instruction) {
  ...
  bool is_volatile = instruction->IsInstanceFieldSet()
    ? instruction->AsInstanceFieldSet()->IsVolatile()
    : instruction->AsStaticFieldSet()->IsVolatile();
  Primitive::Type field_type = instruction->IsInstanceFieldSet()
    ? instruction->AsInstanceFieldSet()->GetFieldType()
    : instruction->AsStaticFieldSet()->GetFieldType();
  ...
  if (CodeGenerator::StoreNeedsWriteBarrier(field_type, instruction->InputAt(1))) {
    ...
    if (codegen_->GetInstructionSet() == InstructionSet::kArm) {
      // For ARM the registers need to be consecutive (ldrexd/strexd).
      locations->AddTemp(Location::RegisterLocation(R0));
      locations->AddTemp(Location::RegisterLocation(R1));
    } else {
      locations->AddTemp(Location::RequiresRegister());
      locations->AddTemp(Location::RequiresRegister());
    }
...
void InstructionCodeGeneratorARM::HandleFieldSet(HInstruction* instruction) {
  ...
  uint32_t offset = instruction->IsInstanceFieldSet()
    ? instruction->AsInstanceFieldSet()->GetFieldOffset().Uint32Value()
    : instruction->AsStaticFieldSet()->GetFieldOffset().Uint32Value();
  bool is_volatile = instruction->IsInstanceFieldSet()
    ? instruction->AsInstanceFieldSet()->IsVolatile()
    : instruction->AsStaticFieldSet()->IsVolatile();
  Primitive::Type field_type = instruction->IsInstanceFieldSet()
  ...
  switch (field_type) {
    ...
    case Primitive::kPrimLong: {
      if (is_volatile) {
        // TODO: compiler_driver->GetInstructionSetFeatures()->
        //       AsArmInstructionSetFeatures()->HasAtomicLdrdAndStrd())
        GenerateVolatileStore(base, offset,
                              value.AsRegisterPairLow<Register>(),
...
void LocationsBuilderARM::HandleFieldGet(HInstruction* instruction) {
  ...
  bool is_volatile = instruction->IsInstanceFieldGet()
    ? instruction->AsInstanceFieldSet()->IsVolatile()
    : instruction->AsStaticFieldSet()->IsVolatile();
  Primitive::Type field_type = instruction->GetType();
...
void InstructionCodeGeneratorARM::HandleFieldGet(HInstruction* instruction) {
  ...
  LocationSummary* locations = instruction->GetLocations();
  Register base = locations->InAt(0).AsRegister<Register>();
  Location out = locations->InAt(1);
  uint32_t offset = instruction->IsInstanceFieldGet()
    ? instruction->AsInstanceFieldGet()->GetFieldOffset().Uint32Value()
    : instruction->AsStaticFieldGet()->GetFieldOffset().Uint32Value();
  bool is_volatile = instruction->IsInstanceFieldSet()
    ? instruction->AsInstanceFieldSet()->IsVolatile()
    : instruction->AsStaticFieldSet()->IsVolatile();
  Primitive::Type field_type = instruction->GetType();
  ...
  switch (field_type) {
    ...
    case Primitive::kPrimLong: {
      if (is_volatile) {
        GenerateVolatileLoad(base, offset,
                             out.AsRegisterPairLow<Register>(),
                             out.AsRegisterPairHigh<Register>());
      } else {
    ...
    case Primitive::kPrimDouble: {
      ...
      if (is_volatile) {
        ...
        GenerateVolatileLoad(base, offset, lo, hi);
