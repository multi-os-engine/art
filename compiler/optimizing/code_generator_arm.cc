...
void InstructionCodeGeneratorARM::VisitArrayGet(HArrayGet* instruction) {
  ...
  switch (type) {
    ...
    case Primitive::kPrimInt: {
      if (index.IsConstant()) {
        uint32_t full_offset = data_offset + (index.GetConstant()->AsIntConstant()->GetValue()
  ...
  switch (value_type) {
    ...
    case Primitive::kPrimNot: {
      ...
      if (index.IsConstant()) {
        ...
        DCHECK(index.IsRegister()) << index;
        __ add(IP, array, ShifterOperand(data_offset));
        Address mem_address(IP, index.AsRegister<Register>(), Shift::LSL, TIMES_4);
        __ str(source, mem_address);
      }
