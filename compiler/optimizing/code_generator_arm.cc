  ...
  switch (invoke->GetCodePtrLocation()) {
    ...
    case HInvokeStaticOrDirect::CodePtrLocation::kDirectCodeFixup:
      if (invoke->GetTargetMethod().dex_file != &GetGraph()->GetDexFile()) {
        // For calls across dex files use absolute patch. Even if both dex files are within
        // the boot image or within the same multi-dex apk, these calls are more likely to
        // exceed the available BL range.
        // TODO: Reconsider this behavior inherited from Quick.
        // LR = address from literal pool with link-time patch.
...
void CodeGeneratorARM::EmitLinkerPatches(ArenaVector<LinkerPatch>* linker_patches) {
  ...
  for (const CallPatchData& data : relative_call_patches_) {
    uint32_t literal_offset = data.label.Position();
    linker_patches->push_back(LinkerPatch::MethodPatch(literal_offset,
