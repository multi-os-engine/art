...
bool LocationsBuilderARM::CanEmbedConstant(HConstant* input_cst, Opcode opcode) {
  uint64_t value = static_cast<uint64_t>(Int64FromConstant(input_cst));
  return CanEmbedConstant(Low32Bits(value), opcode) &&
      (!Primitive::Is64BitType(input_cst->GetType()) ||
          CanEmbedConstant(High32Bits(value), opcode));
}

bool LocationsBuilderARM::CanEmbedConstant(uint32_t value, Opcode opcode) {
  ShifterOperand so;
...
void LocationsBuilderARM::HandleBitwiseOperation(HBinaryOperation* instruction, Opcode opcode) {
  ...
  if (input_cst != nullptr && CanEmbedConstant(input_cst, opcode)) {
    uint32_t cst_idx = (input_cst == instruction->GetRight()) ? 1u : 0u;
    uint32_t other_idx = 1u - cst_idx;
    locations->SetInAt(other_idx, Location::RequiresRegister());
    locations->SetInAt(cst_idx, Location::ConstantLocation(input_cst));
  } else {
    locations->SetInAt(0, Location::RequiresRegister());
    locations->SetInAt(1, Location::RequiresRegister());
  }
