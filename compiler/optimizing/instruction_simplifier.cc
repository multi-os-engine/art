...
bool InstructionSimplifierVisitor::TryDeMorganNegationFactoring(HBinaryOperation* op) {
  ...
  if (((left->IsNot() && right->IsNot()) ||
    ...
    //    NOT dest, or
    bool boolean_not = left->IsBooleanNot();
    HInstruction* src_left = boolean_not ? left->AsBooleanNot()->GetInput()
                                         : left->AsNot()->GetInput();
    HInstruction* src_right = boolean_not ? right->AsBooleanNot()->GetInput()
                                          : right->AsNot()->GetInput();
...
void InstructionSimplifierVisitor::VisitXor(HXor* instruction) {
  ...
  if (((left->IsNot() && right->IsNot()) ||
    ...
    HInstruction* src_right = boolean_not ? right->AsBooleanNot()->GetInput()
                                          : right->AsNot()->GetInput();
    instruction->ReplaceInput(src_left, 0);
    instruction->ReplaceInput(src_right, 1);
    left->GetBlock()->RemoveInstruction(left);
    right->GetBlock()->RemoveInstruction(right);
    RecordSimplification();
    return;
