...
void InstructionSimplifierVisitor::VisitMul(HMul* instruction) {
  ...
  // Constant folding handle aero case pretty good but sometimes
  // it is possible that zero constand can appear as input of mul
  // due to other simplification happened. So handle this case.
  if (input_cst->IsZero()) {
    // Replace code looking like
    //    MUL dst, src, 0
    // with
