...
bool InstructionSimplifierVisitor::TryApplyDeMorgan(HBinaryOperation* op) {
  ...
  if (left->IsNot() && right->IsNot() &&
      left->HasOnlyOneNonEnvironmentUse() && right->HasOnlyOneNonEnvironmentUse()) {
    // Replace code looking like
    //    NOT nota, a
    //    NOT notb, b
    ...
    auto dex_pc = op->GetDexPc();

    HBinaryOperation* hbin;
    if (op->IsAnd()) {
      hbin = new (GetGraph()->GetArena()) HOr(type, src_left, src_right);
    } else {
      hbin = new (GetGraph()->GetArena()) HAnd(type, src_left, src_right);
    }
