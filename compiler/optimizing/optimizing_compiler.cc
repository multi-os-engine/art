...
static void RunOptimizations(HOptimization* optimizations[],
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    {
      PassInfo(optimization->GetPassName(), pass_info_printer);
      optimization->Run();
    }
    optimization->Check();
  }
...
CompiledMethod* OptimizingCompiler::CompileOptimized(HGraph* graph,
  ...
  {
    PassInfo(kLivenessPassName, pass_info_printer);
    liveness.Analyze();
  }

  {
    PassInfo(kRegisterAllocatorPassName, pass_info_printer);
...
CompiledMethod* OptimizingCompiler::Compile(const DexFile::CodeItem* code_item,
  ...
  {
    PassInfo(kBuilderPassName, &pass_info_printer);
  ...
  if (run_optimizations_ && can_optimize && can_allocate_registers) {
    ...
    {
      PassInfo(kSsaBuilderPassName, &pass_info_printer);
