...
static HOptimization* BuildOptimization(
  ...
  if (opt_name == arm::InstructionSimplifierArm::kInstructionSimplifierArmPassName) {
    ...
    return new (arena) BoundsCheckElimination(
      graph, *most_recent_side_effects, most_recent_induction);
    ...
    return new (arena) IntrinsicsRecognizer(
      graph, driver, stats);
    ...
    return new (arena) mips::DexCacheArrayFixups(graph, stats);
...
static std::vector<HOptimization*> BuildOptimizations(
...
void OptimizingCompiler::RunOptimizations(HGraph* graph,
  ...
  if (driver->GetPassesToRun() != nullptr) {
    std::vector<HOptimization*> optimizations = BuildOptimizations(
      *driver->GetPassesToRun(),
      arena,
      graph,
      stats,
      codegen,
