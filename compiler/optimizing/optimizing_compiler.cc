...
static void RunArchOptimizations(HGraph* graph,
                                 ...
                                 OptimizingCompilerStats* stats,
                                 PassObserver* pass_observer) {
  ArenaVector<HOptimization*> arch_optimizations(graph->GetArena()->Adapter());
  codegen->ListArchOptimizations(&arch_optimizations, graph, stats);
  RunOptimizations(arch_optimizations.data(), arch_optimizations.size(), pass_observer);
}
