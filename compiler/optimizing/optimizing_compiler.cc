...
CodeGenerator* OptimizingCompiler::TryCompile(ArenaAllocator* arena,
  ...
  if (run_optimizations_) {
...

    {
      ...
      if (result != kSsaBuilt) {
        switch (result) {
          case kNonNaturalLoop:
            MaybeRecordStat(MethodCompilationStat::kNotCompiledNonNaturalLoop);
            break;
          case kTryCatchLoop:
            MaybeRecordStat(MethodCompilationStat::kNotCompiledTryCatchLoop);
            break;
          case kUntypedArrayAccess:
            LOG(WARNING) << "Method " << pass_observer.GetMethodName() << " cannot be typed.";
            MaybeRecordStat(MethodCompilationStat::kNotCompiledUntypedArrayAccess);
            break;
          case kSsaBuilt:
            UNREACHABLE();
        }
