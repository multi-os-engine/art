...
void OptimizingCompiler::RunArchOptimizations(InstructionSet instruction_set,
  ...
  switch (instruction_set) {
    ...
    case kArm: {
      ...
      GVNOptimization* gvn = new (arena) GVNOptimization(
        graph,
        *side_effects,
        GVNOptimization::kGlobalValueNumberingAfterArchPassName);
...
void OptimizingCompiler::RunOptimizations(HGraph* graph,
  ...
  OptimizingCompilerStats* stats = compilation_stats_.get();
  ArenaAllocator* arena = graph->GetArena();
  if (driver->GetPassesToRun() != nullptr) {
    const std::vector<std::string>& passes = *driver->GetPassesToRun();
    HOptimizationFactory factory(arena,
                                 graph,
                                 stats,
                                 codegen,
                                 driver,
                                 dex_compilation_unit,
                                 handles);
    std::vector<HOptimization*> optimizations = factory.BuildOptimizations(passes);
    RunOptimizations(&optimizations[0], optimizations.size(), pass_observer);
    return;
  }

  HDeadCodeElimination* dce1 = new (arena) HDeadCodeElimination(
      graph, stats, HDeadCodeElimination::kDeadCodeEliminationPassName);
  ...
  HConstantFolding* fold2 = new (arena) HConstantFolding(
    graph, HConstantFolding::kConstantFoldingAfterInliningPassName);
  HConstantFolding* fold3 = new (arena) HConstantFolding(
    graph, HConstantFolding::kConstantFoldingAfterBCEPassName);
  SideEffectsAnalysis* side_effects = new (arena) SideEffectsAnalysis(graph);
  ...
  InstructionSimplifier* simplify2 = new (arena) InstructionSimplifier(
      graph, stats, InstructionSimplifier::kInstructionSimplifierAfterBCEPassName);
  InstructionSimplifier* simplify3 = new (arena) InstructionSimplifier(
      graph, stats, InstructionSimplifier::kInstructionSimplifierBeforeCodegenPassName);
...
CodeGenerator* OptimizingCompiler::TryCompile(ArenaAllocator* arena,
...

  {
    ...
    RunOptimizations(graph,
                     ...
                     &pass_observer,
                     &handles);
    AllocateRegisters(graph, codegen.get(), &pass_observer);
