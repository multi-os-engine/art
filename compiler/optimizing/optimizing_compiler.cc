...
class OptimizingCompiler FINAL : public Compiler {
 ...
 private:
  ...
  CompiledMethod* CompileOptimized(ArenaAllocator* arena,
                                   CodeVectorAllocator* code_allocator,
                                   CodeGenerator* codegen,
                                   CompilerDriver* driver) const;

  // Just compile without doing optimizations.
  CompiledMethod* CompileBaseline(ArenaAllocator* arena,
                                  CodeVectorAllocator* code_allocator,
                                  CodeGenerator* codegen,
                                  CompilerDriver* driver) const;
...
CompiledMethod* OptimizingCompiler::CompileOptimized(ArenaAllocator* arena,
                                                     CodeVectorAllocator* code_allocator,
                                                     CodeGenerator* codegen,
...
CompiledMethod* OptimizingCompiler::Compile(const DexFile::CodeItem* code_item,
  ...
  if (compiler_driver->IsMethodVerifiedWithoutFailures(method_idx, class_def_idx, dex_file)
    ...
    if (codegen.get() != nullptr) {
      if (run_optimizations_) {
        method = CompileOptimized(&arena, &code_allocator, codegen.get(), compiler_driver);
      } else {
        method = CompileBaseline(&arena, &code_allocator, codegen.get(), compiler_driver);
      }
    }
  } else {
