 ...
 */
static bool kIsTraceEnabled = false;

/**
 * Filter to apply to the tracer. Methods whose name contain that filter will
 * be in the file.
 */
...
OptimizingCompiler::OptimizingCompiler(CompilerDriver* driver) : QuickCompiler(driver) {
  if (kIsTraceEnabled) {
    tracer_output_.reset(new std::ofstream("art.cfg"));
  }
}
...
CompiledMethod* OptimizingCompiler::TryCompile(const DexFile::CodeItem* code_item,
  ...
  HGraphTracer tracer(tracer_output_.get(), graph, kStringFilter, dex_compilation_unit);
  tracer.TraceGraph("builder");
