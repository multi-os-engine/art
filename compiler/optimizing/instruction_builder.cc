...
bool HBasicBlockBuilder::ContainsThrowingInstructions(HBasicBlock* block) {
  uint32_t start_dex_pc = block->GetDexPc();
  DCHECK_NE(start_dex_pc, kNoDexPc);

  for (BytecodeIterator it(code_item_, start_dex_pc); !it.Done(); it.Advance()) {
    uint32_t dex_pc = it.CurrentDexPc();
    if (dex_pc != start_dex_pc && GetBlockAt(dex_pc) != nullptr) {
      break;
    } else if (CanBytecodeInstructionThrow(it.CurrentInstruction())) {
      return true;
    }
  }
...
bool HBasicBlockBuilder::MightHaveLiveNormalPredecessors(HBasicBlock* catch_block) {
  ...
  const Instruction& first = GetBytecodeInstructionAt(code_item_, catch_block->GetDexPc());
...
bool HBasicBlockBuilder::CreateBranchTargets() {
  ...
  for (BytecodeIterator it(code_item_); !it.Done(); it.Advance()) {
    uint32_t dex_pc = it.CurrentDexPc();
    ...
    if (instruction.IsBranch()) {
      ...
      for (SwitchTableIterator s_it(table); !s_it.Done(); s_it.Advance()) {
        ...
        if (table.ShouldBuildDecisionTree() && !s_it.IsLast()) {
          ...
          MaybeCreateBlockAt(s_it.GetDexPcForCurrentIndex(), dex_pc);
        }
      }
...
void HBasicBlockBuilder::InsertTryBoundaryBlocks() {
  ...
  for (uint32_t idx = 0; idx < handlers_size; ++idx) {
    ...
    for (; iterator.HasNext(); iterator.Next()) {
      ...
      if (MightHaveLiveNormalPredecessors(catch_block) ||
          ContainsThrowingInstructions(catch_block)) {
  ...
  for (size_t i = 0; i < num_blocks; ++i) {
    ...
    if (ContainsThrowingInstructions(block)) {
