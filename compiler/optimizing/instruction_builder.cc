...
bool HInstructionBuilder::GenerateInstructions() {
...
void HInstructionBuilder::InitializeLocals(uint16_t count) {
...
void HInstructionBuilder::UpdateLocal(uint32_t register_index,
                                      HInstruction* instruction,
                                      uint32_t dex_pc) const {
  current_block_->AddInstruction(
      new (arena_) HStoreLocal(locals_[register_index], instruction, dex_pc));
}

HInstruction* HInstructionBuilder::LoadLocal(uint32_t register_index,
                                             Primitive::Type type,
                                             uint32_t dex_pc) const {
  current_block_->AddInstruction(new (arena_) HLoadLocal(locals_[register_index], type, dex_pc));
  return current_block_->GetLastInstruction();
}

void HInstructionBuilder::InitializeParameters(uint16_t number_of_parameters) {
