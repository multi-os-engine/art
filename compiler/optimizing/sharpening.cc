...
void HSharpening::ProcessLoadString(HLoadString* load_string) {
  ...
  uint64_t address = 0u;  // String or dex cache element address.
  {
    ...
    Handle<mirror::DexCache> dex_cache =
        (&dex_file == compilation_unit_.GetDexFile())
    ...
    if (compiler_driver_->IsBootImage()) {
      ...
      if (runtime->UseJit()) {
        ...
        if (string != nullptr && IsStringInBootImage(string)) {
      ...
      } else if (string != nullptr && IsStringInBootImage(string)) {
...
bool HSharpening::IsStringInBootImage(mirror::String* string) {
  DCHECK(string != nullptr);
  for (const auto* space : Runtime::Current()->GetHeap()->GetBootImageSpaces()) {
    if (space->Contains(string)) {
      return true;
    }
  }
  return false;
