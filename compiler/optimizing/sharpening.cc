...
void HSharpening::ProcessInvokeVirtualOrInterface(HInvoke* invoke) {
  ...
  uint32_t dex_method_index = target_method.dex_method_index;
  if (!IsSameDexFile(*target_method.dex_file, invoke->GetDexFile())) {
    // Codegens need a valid dex method index for kDexCacheViaMethod (other kinds are OK).
    // Inliner always needs a valid dex method index.
    dex_method_index = actual_method->FindDexMethodIndexInOtherDexFile(
        invoke->GetDexFile(),
