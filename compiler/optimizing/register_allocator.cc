...
void RegisterAllocator::AllocateRegistersInternal() {
  ...
  // Sort call positions in ascending order.
  std::sort(call_positions_.begin(), call_positions_.end());
...
void RegisterAllocator::ProcessInstruction(HInstruction* instruction) {
  ...
  if (locations->WillCall()) {
    ...
    // We only record call positions for real calls, i.e. not slow path calls.
    call_positions_.push_back(position);
  }
...
RegisterAllocator::CallerCalleePreference RegisterAllocator::GetCallerCalleePreference(LiveInterval* interval) const {
  ...
  if (kIsDebugBuild) {
    DCHECK(std::is_sorted(call_positions_.begin(), call_positions_.end()));
  }

  CallerCalleePreference reg_pref = CallerCalleePreference::kCallerSavedRegister;
  for (std::vector<size_t>::const_iterator it = call_positions_.begin(); it != call_positions_.end(); ++it) {
    size_t current_call_position = *it;
