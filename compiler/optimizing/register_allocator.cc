...
void RegisterAllocator::BlockAllRegisters(size_t start, size_t end, bool caller_save_only) {
...
void RegisterAllocator::ProcessInstruction(HInstruction* instruction) {
  ...
  if (locations->WillCall()) {
    BlockAllRegisters(position, position + 1, /* caller_save_only */ true);
  }
...
void RegisterAllocator::LinearScan() {
  while (!unhandled_->IsEmpty()) {
    ...
    // HInstruction* defined_by = current->GetParent()->GetDefinedBy();
    // if (defined_by != nullptr && defined_by->IsAdd()) {
    //   LOG(INFO) << "Allocating " << defined_by->DebugName() << defined_by->GetId();
...
bool RegisterAllocator::TryAllocateFreeReg(LiveInterval* current) {
  ...
  if (defined_by != nullptr && current->IsParent()) {
...
void RegisterAllocator::AllocateSpillSlotFor(LiveInterval* interval) {
  ...
  size_t end = parent->GetLastSibling()->GetEnd();
...
void RegisterAllocator::AllocateSpillSlotForCatchPhi(HPhi* phi) {
  ...
  if (kIsDebugBuild) {
    if (previous_phi != nullptr) {
      DCHECK(previous_phi->AsPhi()->GetRegNumber() <= phi->GetRegNumber())
          << "Phis must be stored and processed in ascending order of VReg numbers";
    }
  }

  if (phi->IsVRegEquivalentOf(previous_phi)) {
    // This is a constant equivalent of the previous phi. We need to assign the
    // same catch phi slot.
    DCHECK(previous_phi->GetLiveInterval()->HasSpillSlot());
    interval->SetSpillSlot(previous_phi->GetLiveInterval()->GetSpillSlot());
  } else {
    // Allocate a new spill slot for this catch phi.
    // TODO: Reuse spill slots when intervals do not overlap.
