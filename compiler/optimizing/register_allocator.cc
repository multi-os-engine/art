...
RegisterAllocator::RegisterAllocator(ArenaAllocator* allocator, const CodeGenerator& codegen)
      : allocator_(allocator),
        ...
        blocked_registers_(static_cast<bool*>(allocator->Alloc(
            codegen.GetNumberOfRegisters() * sizeof(bool), kArenaAllocData))) {
...
void RegisterAllocator::AllocateRegistersInternal(const SsaLivenessAnalysis& liveness) {
  ...
  registers_array_ = reinterpret_cast<size_t*>(
      allocator_->Alloc(number_of_registers_ * sizeof(size_t), kArenaAllocMisc));
...
bool RegisterAllocator::ValidateInternal(const SsaLivenessAnalysis& liveness,
  ...
  return ValidateIntervals(
      intervals, codegen_, allocator_, processing_core_registers_, log_fatal_on_failure);
}
...
void RegisterAllocator::LinearScan() {
  while (!unhandled_.IsEmpty()) {
    ...
    //     Move active intervals that a lifetime hole at this position to inactive.
...
bool RegisterAllocator::TryAllocateFreeReg(LiveInterval* current) {
  ...
  for (size_t i = 0, e = inactive_.Size(); i < e; ++i) {
    ...
    if (next_intersection == kNoLifetime) continue;
