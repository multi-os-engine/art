...
void RegisterAllocator::LinearScan() {
  while (!unhandled_->empty()) {
    ...
    //     to inactive.
    auto active_copied_end = active_.begin();
    for (auto it = active_.begin(), end = active_.end(); it != end; ++it) {
      ...
      if (interval->IsDeadAt(position)) {
        ...
        *active_copied_end++ = interval;
      }
    }
    active_.erase(active_copied_end, active_.end());
    ...
    auto inactive_copied_end = inactive_.begin();
    ...
    for (auto it = inactive_.begin(); it != inactive_to_handle_end; ++it) {
      ...
      if (interval->IsDeadAt(position)) {
        ...
        *inactive_copied_end++ = interval;
      }
    }
    inactive_.erase(inactive_copied_end, inactive_to_handle_end);
...
static ArenaVector<LiveInterval*>::iterator RemoveIntervalIncludingOtherHalf(
...
bool RegisterAllocator::TrySplitNonPairOrUnalignedPairIntervalAt(size_t position,
  ...
  for (auto it = active_.begin(), end = active_.end(); it != end; ++it) {
    ...
    if (!active->IsLowInterval() ||
      ...
      RemoveIntervalIncludingOtherHalf(&active_, it);
      AddSorted(unhandled_, split);
      return true;
    }
  }
        ...
        if (!current->IsSplit() && !inactive->IsFixed()) {
          ...
          if (next_intersection != kNoLifetime) {
            if (inactive->IsFixed()) {
              ...
              it = RemoveIntervalIncludingOtherHalf(&inactive_, it);
              erased = true;
