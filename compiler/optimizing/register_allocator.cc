    ...
    for (size_t i = 0, e = safepoints_.Size(); i < e; ++i) {
      ...
      switch (source.GetKind()) {
        case Location::kRegister: {
          ...
          if (processing_core_registers_) {
            DCHECK_LE(locations->GetNumberOfLiveRegisters(), maximum_number_of_live_core_registers_);
          } else {
            DCHECK_LE(locations->GetNumberOfLiveRegisters(), maximum_number_of_live_fp_registers_);
          }
