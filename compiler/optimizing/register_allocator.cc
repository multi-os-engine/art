  ...
  for (size_t i = 0, e = intervals.Size(); i < e; ++i) {
    for (AllRangesIterator it(intervals.Get(i)); !it.Done(); it.Advance()) {
      ...
      if (current->GetParent()->HasSpillSlot()
           ...
           && !(defined_by != nullptr && (defined_by->IsParameterValue() || defined_by->IsCurrentMethod()))) {
...
void RegisterAllocator::ConnectSiblings(LiveInterval* interval) {
  ...
  if (current->HasSpillSlot() && current->HasRegister() && !interval->GetDefinedBy()->IsCurrentMethod()) {
