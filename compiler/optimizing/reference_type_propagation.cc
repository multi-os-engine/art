...
void ReferenceTypePropagation::BoundTypeForIfInstanceOf(HBasicBlock* block) {
  ...
  // InstanceOf returns 0 when True.
  // TODO: fix this non intuitive case.
  if (!previousIf->IsIf()
      || (previousIf->AsIf()->IfFalseSuccessor() != block)) {
    return;
  }
  ...
  // Narrow the type as much as possible.
  {
    ...
    if (!class_rti.IsExact() || class_rti.IsSupertypeOf(obj_rti)) {
  ...
  previousBlock->InsertInstructionBefore(bound_type, previousIf);
...
// Merges new_rti with input_rti in place.
...
bool ReferenceTypePropagation::UpdateReferenceTypeInfo(HInstruction* instr) {
  ...
  ReferenceTypeInfo existing_rti = instr->GetReferenceTypeInfo();
  ReferenceTypeInfo new_rti = instr->InputAt(0)->GetReferenceTypeInfo();

  if (instr->IsBoundType()) {
    // Be sure that we don't go over the bounded type.
    ReferenceTypeInfo top_rti = instr->AsBoundType()->GetTopType();
