      ...
      for (HInstructionIterator iti(block->GetInstructions()); !iti.Done(); iti.Advance()) {
        ...
        if (instr->GetType() == Primitive::kPrimNot) {
          ...
          if (instr->IsBoundType()) {
            DCHECK(instr->AsBoundType()->BoundsOnlyNull()
                || instr->AsBoundType()->GetBoundType().IsValid());
    ...
    if (notNullBlock->Dominates(user->GetBlock())) {
      if (bound_type == nullptr) {
        bound_type = new (graph_->GetArena()) HBoundType(obj,
            ReferenceTypeInfo::Create(object_class_handle_, true));
...
void ReferenceTypePropagation::UpdateBoundType(HBoundType* instr) {
  ...
  // In case we need to bound more than not-nulls
  // be sure that we don't go over the bounded type.
  if (!instr->BoundsOnlyNull()) {
