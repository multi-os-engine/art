...
static HInstanceOf* MatchIfNotInstanceOf(HIf* ifInstruction) {
  ...
  if (input->IsEqual()) {
    LOG(INFO) << "If input is HEqual";
    HInstruction* rhs = input->AsEqual()->GetConstantRight();
    if (rhs != nullptr) {
      HInstruction* lhs = input->AsEqual()->GetLeastConstantLeft();
      LOG(INFO) << "RHS is " << rhs->DebugName();
      LOG(INFO) << "LHS is " << lhs->DebugName();
      if (rhs->IsIntConstant()) {
        LOG(INFO) << "Const is " << rhs->AsIntConstant()->GetValue();
      }
      if (lhs->IsInstanceOf() && rhs->IsIntConstant() && rhs->AsIntConstant()->IsZero()) {
...
void ReferenceTypePropagation::BoundTypeForIfInstanceOf(HBasicBlock* block) {
  ...
  if (instanceOf != nullptr) {
    LOG(INFO) << "Matched If InstanceOf " << instanceOf->GetId();
    instanceOfTrueBlock = ifInstruction->IfTrueSuccessor();
  } else {
    instanceOf = MatchIfNotInstanceOf(ifInstruction);
    if (instanceOf != nullptr) {
      LOG(INFO) << "Matched not If InstanceOf " << instanceOf->GetId();
