    ...
    for (HReversePostOrderIterator it(*graph_); !it.Done(); it.Advance()) {
      ...
      for (HInstructionIterator iti(block->GetInstructions()); !iti.Done(); iti.Advance()) {
        ...
        if (instr->GetType() == Primitive::kPrimNot) {
          ...
          } else if (instr->IsNullCheck() && !instr->InputAt(0)->IsNullConstant()) {
            // Verify NullCheck type consistency only if the input is not the NullConstant.
            // This can happen when we inline a function that returns null.
