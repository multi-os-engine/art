...
static bool ShouldCreateBoundType(HInstruction* position,
  ...
  // Note: Invalid upper_bound is used to indicate Object.class to avoid creating extra Handle<>
  // if not necessary. Only the Object.class itself is a supertype of Object.class.
  if (upper_bound.IsValid() ? existing_bound_type->GetUpperBound().IsSupertypeOf(upper_bound)
                            : existing_bound_type->GetUpperBound().IsObjectClass()) {
    if (kIsDebugBuild) {
  ...
  for (HUseIterator<HInstruction*> it(obj->GetUses()); !it.Done(); it.Advance()) {
    ...
    if (notNullBlock->Dominates(user->GetBlock())) {
      if (bound_type == nullptr) {
        ...
        // Invalid RTI is used to indicate Object.class for ShouldCreateBoundType().
        ReferenceTypeInfo invalid_rti = ReferenceTypeInfo::CreateInvalid();
        if (ShouldCreateBoundType(insert_point, obj, invalid_rti, nullptr, notNullBlock)) {
...
void ReferenceTypePropagation::RTPVisitor::SetClassAsTypeInfo(HInstruction* instr,
  ...
  if (instr->IsInvokeStaticOrDirect() && instr->AsInvokeStaticOrDirect()->IsStringInit()) {
    ...
    ScopedObjectAccess soa(Thread::Current());
    ReferenceTypeInfo::TypeHandle handle = handle_cache_->Handles()->NewHandle(klass);
...
void ReferenceTypePropagation::RTPVisitor::VisitLoadClass(HLoadClass* instr) {
  ...
  if (resolved_class != nullptr) {
    instr->SetLoadedClassRTI(ReferenceTypeInfo::Create(
        handle_cache_->Handles()->NewHandle(resolved_class), /* is_exact */ true));
  }
  instr->SetReferenceTypeInfo(
...
ReferenceTypeInfo ReferenceTypePropagation::MergeTypes(const ReferenceTypeInfo& a,
  ...
  if (a.GetTypeHandle().Get() == b.GetTypeHandle().Get()) {
    ...
    result_type_handle =
        handle_cache_.Handles()->NewHandle(a_type_handle->GetCommonSuperClass(b_type_handle));
    is_exact = false;
  } else {
    // This can happen if:
...
void ReferenceTypePropagation::UpdateArrayGet(HArrayGet* instr, HandleCache* handle_cache) {
  ...
  if (handle->IsObjectArrayClass()) {
    ReferenceTypeInfo::TypeHandle component_handle =
        handle_cache->Handles()->NewHandle(handle->GetComponentType());
    instr->SetReferenceTypeInfo(
