...
class RTPVisitor : public HGraphDelegateVisitor {
 ...
 private:
  ...
  // Contains instruction which should be included in the fix point iteration.
...
void RTPVisitor::VisitNullCheck(HNullCheck* instr) {
  ...
  if (!parent_rti.IsValid()) {
    // Parent could be a phi or an array get and we might not have any valid
...
static bool UpdateArrayGet(HArrayGet* instr,
  ...
  if (!parent_rti.IsValid()) {
    ...
    // The parent could be a phi or a null check for which we might have not have
