...
void RTPVisitor::VisitCheckCast(HCheckCast* check_cast) {
  ...
  if (bound_type == nullptr || bound_type->InputAt(0) != check_cast->InputAt(0)) {
    ...
    // the class must be unresolved and this must be an RTP pass after SsaBuilder.
    DCHECK(!class_rti.IsValid());
  } else if (bound_type->GetUpperBound().IsValid()) {
    // BoundType already has an upper bound. This must be an RTP pass after SsaBuilder.
    DCHECK(bound_type->GetUpperBound().IsEqual(class_rti));
    DCHECK(bound_type->GetUpperCanBeNull());
  } else if (class_rti.IsValid()) {
