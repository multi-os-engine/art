...
void ReferenceTypePropagation::VisitNewInstance(HNewInstance* instr) {
  ...
  if (resolved_class != nullptr) {
    ...
    SetReferenceTypeInfo(instr, instr->GetBlock()->GetBlockId(), ReferenceTypeInfo(handle));
  } else {
    SetReferenceTypeInfo(instr, instr->GetBlock()->GetBlockId(), ReferenceTypeInfo());
  }
}
...
void ReferenceTypePropagation::VisitLoadClass(HLoadClass* instr) {
  ...
  if (resolved_class != nullptr) {
    ...
    load_class_map_.Put(instr->GetId(), ReferenceTypeInfo());
  }
  Handle<mirror::Class> class_handle = handles_->NewHandle(mirror::Class::GetJavaLangClass());
  SetReferenceTypeInfo(instr, instr->GetBlock()->GetBlockId(), ReferenceTypeInfo(class_handle));
}
...
void ReferenceTypePropagation::VisitBasicBlock(HBasicBlock* block) {
  ...
  if (block->IsLoopHeader()) {
    for (HInstructionIterator it(block->GetPhis()); !it.Done(); it.Advance()) {
      ...
      SetReferenceTypeInfo(phi, block_id, phi->InputAt(0)->GetReferenceTypeInfo(block_id));
    }
  } else {
    ScopedObjectAccess soa(Thread::Current());
    for (HInstructionIterator it(block->GetPhis()); !it.Done(); it.Advance()) {
