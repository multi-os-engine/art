...
ReferenceTypePropagation::ReferenceTypePropagation(HGraph* graph,
  ...
  if (kIsDebugBuild) {
    ...
    DCHECK(ReferenceTypeInfo::HasValidHandle(object_class_handle_));
    DCHECK(ReferenceTypeInfo::HasValidHandle(class_class_handle_));
    DCHECK(ReferenceTypeInfo::HasValidHandle(string_class_handle_));
  }
}
    ...
    for (HReversePostOrderIterator it(*graph_); !it.Done(); it.Advance()) {
      ...
      for (HInstructionIterator iti(block->GetInstructions()); !iti.Done(); iti.Advance()) {
        ...
        if (instr->GetType() == Primitive::kPrimNot) {
          DCHECK(instr->GetReferenceTypeInfo().HasValidHandle())
...
void RTPVisitor::VisitArrayGet(HArrayGet* instr) {
  ...
  if (!instr->GetReferenceTypeInfo().HasValidHandle()) {
