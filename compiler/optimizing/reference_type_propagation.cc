...
void ReferenceTypePropagation::BoundTypeForIfInstanceOf(HBasicBlock* block) {
  ...
  // The boolean simplifier should transform `if (a instanceof A)` such that
  // the input of the HIf is either an HInstanceOf or an HBooleanNot with an
  // HInstanceOf as the input.
  //
  // TODO: Is this guaranteed? If not we should reinstate the detection of HEqual.
