...
void RTPVisitor::VisitLoadException(HLoadException* instr) {
  ...
  const CatchHandlerInformation& catch_info = instr->GetBlock()->GetCatchHandlerInformation();

  if (catch_info.IsCatchAll()) {
    instr->SetReferenceTypeInfo(
        ReferenceTypeInfo::Create(throwable_class_handle_, /* is_exact */ false));
  } else {
