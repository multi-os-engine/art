...
void RTPVisitor::VisitCheckCast(HCheckCast* check_cast) {
  ...
  DCHECK(bound_type == nullptr || check_cast->InputAt(0) == bound_type->InputAt(0));

  ScopedObjectAccess soa(Thread::Current());
  if (class_rti.IsValid()) {
    DCHECK(bound_type != nullptr) << "Builder should have created a BoundType after CheckCast.";
    bound_type->SetUpperBound(class_rti, true /* CheckCast succeeds for nulls. */);
  } else {
    // Class is unresolved. Remove the BoundType if this is the first visit.
    if (bound_type != nullptr) {
