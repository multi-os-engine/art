 ...
 public:
  ...
  void VisitInstruction(HInstruction* instr) OVERRIDE {
    if (instr->GetType() == Primitive::kPrimNot) {
      ctx_.SetProp(instr, instr->CanBeNull());
    }
  }
 private:
...
void ReferenceTypePropagation::VisitPhi(HPhi* phi) {
  ...
  if (phi->GetBlock()->IsLoopHeader()) {
    ...
    ctx_.SetProp(phi, phi->CanBeNull());
  } else {
...
void RTPVisitor::VisitInvoke(HInvoke* instr) {
  ...
  if (!(instr->IsInvokeStaticOrDirect() && instr->AsInvokeStaticOrDirect()->IsStatic() == kStatic)) {
    ctx_.SetProp(instr->InputAt(0), false);
  }
...
bool ReferenceTypePropagation::UpdateNullability(HInstruction* instr) {
  ...
  for (size_t i = 0; i < phi->InputCount(); i++) {
    new_can_be_null |= ctx_.GetProp(phi->InputAt(i)).can_be_null;
  }
  phi->SetCanBeNull(new_can_be_null);
  ctx_.SetProp(phi, new_can_be_null);
