...
static bool ShouldCreateBoundType(HInstruction* position,
  ...
  if (existing_bound_type->GetUpperBound().IsSupertypeOf(upper_bound)) {
    ...
    // an update on the instruction and possible its dependents. So we need to pass
    // the worklist here.
  }
  return false;
}
  ...
  for (HUseIterator<HInstruction*> it(obj->GetUses()); !it.Done(); it.Advance()) {
    ...
    if (notNullBlock->Dominates(user->GetBlock())) {
      if (bound_type == nullptr) {
        ...
        ReferenceTypeInfo object_rti = ReferenceTypeInfo::Create(object_class_handle_, true);
  ...
  for (HUseIterator<HInstruction*> it(obj->GetUses()); !it.Done(); it.Advance()) {
    ...
    if (instanceOfTrueBlock->Dominates(user->GetBlock())) {
      if (bound_type == nullptr) {
        ...
        if (ShouldCreateBoundType(insert_point, obj,
              load_class->GetLoadedClassRTI(), nullptr, instanceOfTrueBlock)) {
          bound_type = CreateBoundType(graph_->GetArena(), obj, load_class,
  ...
  for (HUseIterator<HInstruction*> it(obj->GetUses()); !it.Done(); it.Advance()) {
    ...
    if (check_cast->StrictlyDominates(user)) {
      if (bound_type == nullptr) {
        ...
        if (ShouldCreateBoundType(check_cast->GetNext(), obj,
              load_class->GetLoadedClassRTI(), check_cast, nullptr)) {
          bound_type = CreateBoundType(GetGraph()->GetArena(), obj, load_class,
...
void ReferenceTypePropagation::VisitPhi(HPhi* phi) {
  ...
  // will not update if the phi value doesn't change. During the first run of
  // the type propagation this is not an issue as everything started from a
  // fresh state. However, in subsequent runs, eg. after inlining, we might have
  // more precise information for the phi. To not miss updates on dependents we
  // need to add them to the worklist for re-processing.
...
static void UpdateArrayGet(HArrayGet* instr,
  ...
  if (!parent_rti.IsValid()) {
    ...
    // The array could be a Phi/NullCheck/ArrayGet/BoundType for which we might not have
