...
class SelectFixups : public HGraphVisitor {
 ...
 private:
  void VisitSelect(HSelect* select) OVERRIDE {
    ...
    HBasicBlock* rest_of_block = GetGraph()->InsertDiamondAfter(select);
    HBasicBlock* true_block = rest_of_block->GetPredecessors()[0];
    HBasicBlock* false_block = rest_of_block->GetPredecessors()[1];
