...
static bool InputsAreDefinedBeforeLoop(HInstruction* instruction) {
  ...
  if (instruction->HasEnvironment()) {
    ...
    for (size_t i = 0, e = environment->Size(); i < e; ++i) {
      ...
      if (input != nullptr) {
        HLoopInformation* current = input->GetBlock()->GetLoopInformation();
        if (current != nullptr && current->IsIn(*info)) {
          // We can move can instruction that takes a loop header phi in the environment:
          // we will just replace that phi with its first input.
          bool is_loop_header_phi = input->IsPhi() && input->GetBlock() == info->GetHeader();
...
static void UpdateLoopPhisIn(HEnvironment* environment, HLoopInformation* info) {
  for (size_t i = 0, e = environment->Size(); i < e; ++i) {
    HInstruction* input = environment->GetInstructionAt(i);
    if (input != nullptr) {
      if (input->IsPhi() && input->GetBlock() == info->GetHeader()) {
