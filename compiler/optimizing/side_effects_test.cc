...
void testWriteAndReadSanity(SideEffects write, SideEffects read) {
  ...
  // A side effect representing a read of a reference (object) field in an object.
  SideEffects read_object_field = SideEffects::FieldReadOfType(Primitive::kPrimNot,
                                                               /* is_volatile */ false);
  // A side effect representing a read of a reference (object) in the cell of an array.
  SideEffects read_object_array = SideEffects::ArrayReadOfType(Primitive::kPrimNot);
  ...
  if ((kForceReadBarrier || kUseReadBarrier) &&
      (read.Includes(read_object_field) || read.Includes(read_object_array))) {
    // When read barriers are enabled, any read operation can depend
    // on reading an object reference (in an object's field or in an
    // array), as this operation can trigger the GC.
    EXPECT_TRUE(all.MayDependOn(read));
  } else {
