...
void GraphChecker::VisitBasicBlock(HBasicBlock* block) {
  ...
  for (auto& sc : successors_count) {
    ...
    for (HBasicBlock* s_p : s->GetPredecessors()) {
      if (s_p == block) {
...
void GraphChecker::VisitTryBoundary(HTryBoundary* try_boundary) {
  ...
  for (HExceptionHandlerIterator it(*try_boundary); !it.Done(); it.Advance()) {
    ...
    const ArenaVector<HBasicBlock*>& successors = current_block_->GetSuccessors();
    const size_t skip = it.CurrentSuccessorIndex() + 1;
    if (std::find(successors.begin() + skip, successors.end(), handler) != successors.end()) {
