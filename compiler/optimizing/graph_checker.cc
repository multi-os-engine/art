...
void GraphChecker::VisitBinaryOperation(HBinaryOperation* instruction) {
  // Ensure constants used as inputs of an arithmetic operation appear
  // at the end only.
  if (instruction->InputAt(0)->IsConstant()
      && !instruction->InputAt(1)->IsConstant()) {
    std::stringstream error;
...
void GraphChecker::VisitInstruction(HInstruction* instruction) {
  // Manually dispatch to VisitBinaryOperation if `instruction` is a
  // HBinaryOperation.  This workaround is needed since
  // HBinaryOperation is not a concrete node in the HInstruction
  // hierarchy (i.e. it is not a leaf node in the class tree).
  if (instruction->IsBinaryOperation()) {
    VisitBinaryOperation(instruction->AsBinaryOperation());
  }
