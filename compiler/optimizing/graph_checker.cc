  ...
  if (block->IsCatchBlock()) {
    if (block->IsTryBlock()) {
      AddError(StringPrintf("Catch blocks should not be try blocks but catch block %d "
                            "has try entry %s:%d.",
                            block->GetBlockId(),
                            block->GetTryCatchInformation()->GetTryEntry()->DebugName(),
                            block->GetTryCatchInformation()->GetTryEntry()->GetId()));
    }
    ...
    for (size_t i = 0; i < block->GetPredecessors().Size(); ++i) {
      ...
      if (block->IsTryBlock()) {
        HTryBoundary* stored_try_entry = block->GetTryCatchInformation()->GetTryEntry();
