...
void SSAChecker::CheckLoop(HBasicBlock* block) {
  ...
  if (block->GetPredecessors().Size() < 2) {
    ...
    errors_.Insert(error.str());
  } else if (block->GetPredecessors().Size() > 2) {
    std::stringstream error;
  ...
  size_t num_back_edges = block->GetLoopInformation().GetBackEdges().Size();
  if (num_back_edges != 1) {
      std::stringstream error;
...
void SSAChecker::VisitPhi(HPhi* phi) {
  ...
  if (phi->InputCount() != predecessors.Size()) {
    ...
    size_t index = 0;
    for (HInputIterator input_it(phi); !input_it.Done();
         input_it.Advance(), ++index) {
      HInstruction* input = input_it.Current();
      HBasicBlock* predecessor = predecessors.Get(index);
      if (!input->GetBlock() == predecessor
          && !input->GetBlock()->Dominates(predecessor)) {
        std::stringstream error;
        error << "Input " << input->GetId() << " at index " << index
              << " of phi " << phi->GetId()
              << " from block " << phi->GetBlock()->GetBlockId()
              << " is not defined in predecessor number " << index
