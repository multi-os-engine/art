...
void GraphChecker::VisitBasicBlock(HBasicBlock* block) {
  ...
  for (HInstructionIterator it(block->GetPhis()); !it.Done(); it.Advance()) {
    ...
    if (current->GetNext() == nullptr && current != block->GetLastPhi()) {
      AddError(StringPrintf("Phi %d in block %d has no successor but does not match "
                            "the recorded last phi.",
                            current->GetId(),
                            current_block_->GetBlockId()));
    }
    current->Accept(this);
  ...
  for (HInstructionIterator it(block->GetInstructions()); !it.Done(); it.Advance()) {
    ...
    if (current->GetNext() == nullptr && current != block->GetLastInstruction()) {
      AddError(StringPrintf("Instruction %d in block %d has no successor but does not match "
                            "the recorded last instruction.",
                            current->GetId(),
                            current_block_->GetBlockId()));
    }
    current->Accept(this);
