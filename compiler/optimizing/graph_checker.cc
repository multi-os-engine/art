...
void GraphChecker::VisitInstruction(HInstruction* instruction) {
  if (seen_ids_.IsBitSet(instruction->GetId())) {
    AddError(StringPrintf("Duplicate id in graph: %d.", instruction->GetId()));
  } else {
...
void SSAChecker::CheckLoop(HBasicBlock* loop_header) {
  ...
  if (num_preds < 2) {
    AddError(StringPrintf(
        "Loop header %d has less than two predecessors (namely %zu).",
        id,
        num_preds));
  } else if (num_preds > 2) {
    AddError(StringPrintf(
        "Loop header %d has more than two predecessors (namely %zu).",
        id,
        num_preds));
  } else {
  ...
  if (num_back_edges == 0) {
    ...
    AddError(StringPrintf(
        "Loop defined by header %d has several back edges (namely %zu).",
        id,
        num_back_edges));
  }
