...
void GraphChecker::VisitTryBoundary(HTryBoundary* try_boundary) {
  // Ensure that all exception handlers are catch blocks and that handlers are
  // not listed multiple times.
  // Note that before catch block simplification, normal-flow successor may be
  // a catch block and therefore is not tested before SSA form is built.
...
void SSAChecker::VisitPhi(HPhi* phi) {
  ...
  if (phi->IsCatchPhi()) {
    // Number of inputs of a catch phi corresponds to the total number of
    // throwing instructions which may throw an exception to its catch block.
  } else {
