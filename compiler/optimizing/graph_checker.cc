...
void GraphChecker::VisitBasicBlock(HBasicBlock* block) {
  ...
  // Ensure that the only Return(Void) and Throw jump to Exit. TryBoundary might
  // be between Throw and Exit if the Throw is in a try block.
  if (block->IsExitBlock()) {
    for (size_t i = 0, e = block->GetPredecessors().Size(); i < e; ++i) {
      HBasicBlock* predecessor = block->GetPredecessors().Get(i);
      if (predecessor->IsSingleTryBoundary()) {
        DCHECK(!predecessor->GetLastInstruction()->AsTryBoundary()->IsEntry());
        ...
        if (!last_instruction->IsThrow()) {
          AddError(StringPrintf("Unexpected try boundary between %s:%d and Exit.",
        ...
        if (!last_instruction->IsReturn()
          ...
          AddError(StringPrintf("Instruction %s:%d jumps into the exit block.",
                                last_instruction->DebugName(),
                                last_instruction->GetId()));
        }
      }
...
void SSAChecker::VisitBasicBlock(HBasicBlock* block) {
  ...
  // Ensure that exceptional predecessors of catch blocks can throw and are
  // in .
  if (block->IsCatchBlock()) {
    ...
    if (!block->GetExceptionalPredecessors().IsEmpty()) {
      AddError(StringPrintf("Normal block %d has exceptional predecessors.",
                            block->GetBlockId()));
    }
  }
