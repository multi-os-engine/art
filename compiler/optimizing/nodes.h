...
#define OPTIMIZING_INTRINSICS(Name, IsStatic, NeedsEnvironmentOrCache, SideEffects, Exceptions) k ## Name,
...
class HInvoke : public HInstruction {
 public:
  ...
  bool CanBeMoved() const OVERRIDE {
    return !can_throw_ && GetSideEffects().DoesNothing();  // free to move around graph
  }

  bool InstructionDataEquals(HInstruction* other) const OVERRIDE {
    HInvoke* other_invoke = other->AsInvoke();
    return other_invoke != nullptr &&
        intrinsic_ != Intrinsics::kNone &&
        intrinsic_ == other_invoke->intrinsic_;
  }
