...
class HGraph : public ArenaObject<kArenaAllocMisc> {
 public:
  ...
  bool TryBuildingSsa() {
    BuildDominatorTree();
    // The SSA builder requires loops to be natural. Specifically, the dead phi
    // elimination phase checks the consistency of the graph when doing a post-order
    // visit for eliminating dead phis: a dead phi can only have loop header phis
    // remaining when being visited.
