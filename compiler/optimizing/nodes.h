...
class HEnvironment : public ArenaObject<kArenaAllocMisc> {
 public:
  ...
  void Populate(HEnvironment* env);
  ...
  void RecordEnvUse(HUseListNode<HEnvironment*>* env_use) {
    ...
    VRegInfo vreg = vregs_.Get(index);
    DCHECK(vreg.vreg_ != nullptr);
    DCHECK(vreg.node_ == nullptr);
    vreg.node_ = env_use;
    vregs_.Put(index, vreg);
  }
...
#undef DECLARE_KIND
  ...
  size_t ExpansiveComputeNumberOfUses() const {
