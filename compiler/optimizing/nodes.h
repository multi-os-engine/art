...
class HTryBoundary : public HTemplateInstruction<0> {
 public:
  ...
  bool HasExceptionHandler(const HBasicBlock& handler) const {
    ...
    const ArenaVector<HBasicBlock*>& successors = GetBlock()->GetSuccessors();
    DCHECK(!successors.empty());
    constexpr size_t skip = 1u;  // Skip first successor.
    return std::find(successors.begin() + skip, successors.end(), &handler) != successors.end();
  }
