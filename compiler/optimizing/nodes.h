...
class HDivZeroCheck : public HExpression<1> {
 public:
  explicit HDivZeroCheck(HInstruction* value, uint32_t dex_pc)
...
class HNullCheck : public HExpression<1> {
 public:
  explicit HNullCheck(HInstruction* value, uint32_t dex_pc)
      : HExpression(value->GetType(), SideEffects::None(), dex_pc) {
    SetRawInputAt(0, value);
  }
...
class HThrow : public HTemplateInstruction<1> {
 public:
  explicit HThrow(HInstruction* exception, uint32_t dex_pc)
