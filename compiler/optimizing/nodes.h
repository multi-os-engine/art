...
class ReferenceTypeInfo : ValueObject {
 public:
  ...
  static bool HasValidHandle(TypeHandle handle) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    return handle.GetReference() != nullptr;
  }

  bool HasValidHandle() const SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    return HasValidHandle(type_handle_);
  }
  ...
  bool IsEqual(ReferenceTypeInfo rti) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    if (!HasValidHandle() && !rti.HasValidHandle()) {
 ...
 private:
  ...
  ReferenceTypeInfo(TypeHandle type_handle, bool is_exact)
    ...
    if (kIsDebugBuild) {
      ...
      DCHECK(HasValidHandle(type_handle));
    }
  }
