...
class HInvokeStaticOrDirect : public HInvoke {
 public:
  ...
  enum class CodePtrLocation {
    ...
    kDirectCodeFixup,     // Use direct code address with linker fixup (boot->boot or app->boot).
    kDirectCode,          // Use direct code address (app->boot call, non-relocatable).
  ...
  HInvokeStaticOrDirect(ArenaAllocator* arena,
      ...
      : HInvoke(arena,
                ...
                1u + (clinit_check_requirement == ClinitCheckRequirement::kExplicit ? 1u : 0u)
                   + (method_load_type == MethodLoadType::kStringInit ? 1u : 0u),
                return_type,
                dex_pc,
                method_index,
  ...
  bool HasPcRelDexCache() const { return method_load_type_ == MethodLoadType::kDexCachePcRelative; }
