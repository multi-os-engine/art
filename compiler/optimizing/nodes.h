...
class SideEffects : public ValueObject {
 public:
  ...
  bool MayDependOn(SideEffects other) const {
    // TODO: use the more exact first code deffed-out
    //       the current version falls back to the
    //       old 2-bit behavior (changes/uses)
#if 0
    const uint64_t reads = (flags_ & kAllReads) >> kFieldReadOffset;
    return (other.flags_ & reads);
#else
    return (flags_ & kAllReads) && (other.flags_ & kAllWrites);
#endif
  }
