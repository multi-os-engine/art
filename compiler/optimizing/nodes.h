...
class HTemplateInstruction: public HInstruction {
 public:
  explicit HTemplateInstruction<N>(SideEffects side_effects, uint32_t dex_pc = kNoDexPc)
...
class HDeoptimize : public HTemplateInstruction<1> {
 public:
  explicit HDeoptimize(HInstruction* cond, uint32_t dex_pc = kNoDexPc)
      : HTemplateInstruction(SideEffects::None(), dex_pc) {
...
class HLocal : public HTemplateInstruction<0> {
 public:
  explicit HLocal(uint16_t reg_number, uint32_t dex_pc = kNoDexPc)
...
class HDivZeroCheck : public HExpression<1> {
 public:
  explicit HDivZeroCheck(HInstruction* value, uint32_t dex_pc = kNoDexPc)
...
class HNullCheck : public HExpression<1> {
 public:
  explicit HNullCheck(HInstruction* value, uint32_t dex_pc = kNoDexPc)
      : HExpression(value->GetType(), SideEffects::None(), dex_pc) {
    SetRawInputAt(0, value);
  }
...
class HArrayGet : public HExpression<2> {
 public:
  HArrayGet(HInstruction* array, HInstruction* index, Primitive::Type type, uint32_t dex_pc = kNoDexPc)
...
class HClinitCheck : public HExpression<1> {
 public:
  explicit HClinitCheck(HLoadClass* constant, uint32_t dex_pc = kNoDexPc)
      : HExpression(
            Primitive::kPrimNot,
...
class HThrow : public HTemplateInstruction<1> {
 public:
  explicit HThrow(HInstruction* exception, uint32_t dex_pc = kNoDexPc)
