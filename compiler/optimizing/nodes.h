...
class ReferenceTypeInfo : ValueObject {
 public:
  ...
  // (e.g. tops are equal but that can be the result of a merge).
  bool IsEqual(ReferenceTypeInfo rti) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    if (IsExact() != rti.IsExact()) {
      return false;
    }
    if (IsTop() && rti.IsTop()) {
      // `Top` mean java.lang.Object, so the types are equivalent.
      return true;
    }
    if (IsTop() ^ rti.IsTop()) {
 ...
 private:
  ...
  // flag acts as a substitute. When true,u the TypeHandle should not be used as
  // it might refer to a null pointer.
  bool is_top_;
};
