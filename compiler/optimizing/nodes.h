...
class HUseList : public ValueObject {
 public:
  HUseList() : first_(nullptr), last_(nullptr) {}
  ...
  bool HasOnlyOneUse() const {
    return first_ != nullptr && first_ == last_;
  }

 private:
  HUseListNode<T>* first_;
  HUseListNode<T>* last_;
};
...
class HEnvironment : public ArenaObject<kArenaAllocMisc> {
 public:
  HEnvironment(ArenaAllocator* arena, size_t number_of_vregs)
     : vregs_(arena, number_of_vregs), vregs_env_use_(arena, number_of_vregs) {
    vregs_.SetSize(number_of_vregs);
    vregs_env_use_.SetSize(number_of_vregs);
    for (size_t i = 0; i < number_of_vregs; i++) {
      vregs_.Put(i, nullptr);
      vregs_env_use_.Put(i, nullptr);
    }
  }
  ...
  void SetRawEnvAt(size_t index, HInstruction* instruction) {
    ...
    vregs_env_use_.Put(index, nullptr);
  }

  void RecordEnvUse(HUseListNode<HEnvironment>* env_use) {
    size_t index = env_use->GetIndex();
    DCHECK(env_use->GetUser() == this);
    DCHECK(vregs_.Get(index) != nullptr);
    DCHECK(vregs_env_use_.Get(index) == nullptr);
    vregs_env_use_.Put(index, env_use);
  }
  ...
  HUseListNode<HEnvironment>* GetInstructionEnvUseAt(size_t index) const {
    return vregs_env_use_.Get(index);
  }
 ...
 private:
  ...
  GrowableArray<HUseListNode<HEnvironment>*> vregs_env_use_;
...
#undef DECLARE_KIND
  ...
  size_t NumberOfUses() const {
    ...
    size_t result = 0;
    HUseListNode<HInstruction>* current = uses_.GetFirst();
