...
class HShl : public HBinaryOperation {
 public:
  ...
  T Compute(T value, U distance, U max_shift_distance) const {
...
class HRor : public HBinaryOperation {
 public:
  ...
  template <typename T, typename U>
  T Compute(T x, U y, U max_shift_value) const {
    typedef typename std::make_unsigned<T>::type V;
  ...
  HConstant* Evaluate(HIntConstant* x, HIntConstant* y) const OVERRIDE {
    return GetBlock()->GetGraph()->GetIntConstant(
        Compute(x->GetValue(), y->GetValue(), kMaxIntShiftDistance), GetDexPc());
  }
  HConstant* Evaluate(HLongConstant* x, HIntConstant* y) const OVERRIDE {
    return GetBlock()->GetGraph()->GetLongConstant(
        Compute(x->GetValue(), y->GetValue(), kMaxLongShiftDistance), GetDexPc());
  }
  HConstant* Evaluate(HLongConstant* x ATTRIBUTE_UNUSED,
