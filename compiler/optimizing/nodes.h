...
#undef DECLARE_KIND
  ...
  auto GetInputs() {
    return MakeTransformArrayRef(
        GetInputRecords(),
        [](const HUserRecord<HInstruction*>& record) -> HInstruction* {
...
#undef INSTRUCTION_TYPE_CHECK
  ...
  virtual size_t ComputeHashCode() const {
    ...
    for (const HUserRecord<HInstruction*>& input_record : GetInputRecords()) {
      result = (result * 31) + input_record.GetInstruction()->GetId();
    }
    return result;
  }
...
class HLoadString FINAL : public HExpression<1> {
 public:
  ...
  HLoadString(HCurrentMethod* current_method,
    ...
    // HLoadString::GetInputRecords() would return an empty array at this point,
    // so use the GetInputRecords() from the base class to set the input record.
    HExpression<1>::GetInputRecords()[0] = HUserRecord<HInstruction*>(current_method);
  }
  ...
  using HInstruction::GetInputRecords;  // Keep the const version visible.
  ArrayRef<HUserRecord<HInstruction*>> GetInputRecords() OVERRIDE {
    ArrayRef<HUserRecord<HInstruction*>> all_records = HExpression<1>::GetInputRecords();
    return all_records.SubArray(0, (all_records[0].GetInstruction() != nullptr) ? 1u : 0u);
  }
...
inline void HLoadString::AddSpecialInput(HInstruction* special_input) {
  ...
  // so use the GetInputRecords() from the base class to set the input record.
  ArrayRef<HUserRecord<HInstruction*>> input_records = HExpression<1>::GetInputRecords();
  DCHECK(input_records[0].GetInstruction() == nullptr);
  input_records[0] = HUserRecord<HInstruction*>(special_input);
