...
class HInvokeStaticOrDirect : public HInvoke {
 public:
  ...
  enum class MethodLoadKind {
    ...
    kDirectAddressWithFixup,  // Direct address with linker fixup (boot->boot call).
    kDexCacheViaMethod,       // Load from dex_cache_resolved_methods_ of its own ArtMethod*.
    kDexCachePcRelative,      // Load from dex cache using PC-relative load.
  };
  ...
  enum class CodePtrLocation {
    ...
    kCallPCRelative,        // PC-relative call within oat file (boot->boot or app->app call).
    kCallDirectWithFixup,   // Use direct code address with linker fixup (boot->boot or app->boot).
    kCallDirect,            // Use direct code address (app->boot call, non-relocatable).
    kCallArtMethod,         // Use code pointer from the ArtMethod*.
  };
  ...
  HInvokeStaticOrDirect(ArenaAllocator* arena,
      ...
      : HInvoke(arena,
                ...
                1u + (clinit_check_requirement == ClinitCheckRequirement::kExplicit ? 1u : 0u)
                   + (method_load_kind == MethodLoadKind::kStringInit ? 1u : 0u),
                return_type,
                dex_pc,
                method_index,
        ...
        method_load_kind_(method_load_kind),
  ...
  MethodLoadKind GetMethodLoadKind() const { return method_load_kind_; }
  CodePtrLocation GetCodePtrLocation() const { return code_ptr_location_; }
  bool IsRecursive() const { return method_load_kind_ == MethodLoadKind::kRecursive; }
  bool NeedsDexCache() const OVERRIDE { return !IsRecursive() && !IsStringInit(); }
  bool IsStringInit() const { return method_load_kind_ == MethodLoadKind::kStringInit; }
  uint32_t GetCurrentMethodInputIndex() const { return GetNumberOfArguments(); }
  bool HasMethodAddress() const { return method_load_kind_ == MethodLoadKind::kDirectAddress; }
  bool HasPcRelDexCache() const { return method_load_kind_ == MethodLoadKind::kDexCachePcRelative; }
  bool HasDirectCodePtr() const { return code_ptr_location_ == CodePtrLocation::kCallDirect; }
 ...
 private:
  ...
  const MethodLoadKind method_load_kind_;
