...
class HInputIterator : public ValueObject {
 public:
  ...
  inline bool Done() const { return index_ == instruction_->InputCount(); }
  inline HInstruction* Current() const { return instruction_->InputAt(index_); }
  inline void Advance() { index_++; }
...
class HInstructionIterator : public ValueObject {
 public:
  ...
  inline HInstruction* Current() const { return instruction_; }
  inline void Advance() {
    instruction_ = next_;
    next_ = Done() ? nullptr : instruction_->next();
  }
...
class HLocal : public HTemplateInstruction<0> {
 public:
  HLocal(uint32_t reg_number) : reg_number_(reg_number) { }

  DECLARE_INSTRUCTION(Local)

 private:
  // The register number in Dex.
  uint32_t reg_number_;
...
class HIntConstant : public HTemplateInstruction<0> {
 public:
  HIntConstant(int value) : value_(value) { }
