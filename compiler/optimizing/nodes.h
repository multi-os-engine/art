...
class HIntConstant : public HConstant {
 ...
 private:
  ...
  // Note: Passing a `Primitive::kPrimBool` as argument to HConstant's
  // ctor raises an internal compiler error in clang++.
...
class HEqual : public HCondition {
 public:
  ...
  HConstant* Evaluate(HIntConstant* x, HIntConstant* y) const OVERRIDE {
    return this->MakeConstantCondition(Compute(x->GetValue(), y->GetValue()));
  }
...
class HShl : public HBinaryOperation {
 public:
  ...
  T Compute(T x, T y, U max_shift_value) const {
    static_assert(std::is_same<U, typename std::make_unsigned<T>::type>::value,
                  "U is not the unsigned integer type corresponding to T");
...
class HUShr : public HBinaryOperation {
 public:
  ...
  T Compute(T x, T y, U max_shift_value) const {
    ...
    U uy = static_cast<U>(y) & max_shift_value;
    return static_cast<T>(ux >> uy);
  }
