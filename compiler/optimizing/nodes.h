...
// This information is only set for catch blocks before SSA form is built. In
// SSA, HBasicBlock cannot be both a try block and a catch block simultaneously.
class TryCatchInformation : public ArenaObject<kArenaAllocMisc> {
 public:
  // Try block information constructor.
  explicit TryCatchInformation(HTryBoundary* try_entry)
  ...
  bool IsCatchAllTypeIndex() const { return catch_type_index_ == DexFile::kDexNoIndex16; }
  uint16_t GetCatchTypeIndex() const { return catch_type_index_; }
  const DexFile& GetCatchDexFile() const { return *catch_dex_file_; }

 private:
  // One of possibly several TryBoundary instructions entering the block's try.
  HTryBoundary* try_entry_;

  // Exception type information for catch blocks.
