...
class HUserRecord : public ValueObject {
 public:
  ...
  HUserRecord(const HUserRecord<T>& old_record, typename HUseList<T>::iterator before_use_node)
      : instruction_(old_record.instruction_), before_use_node_(before_use_node) {
    DCHECK(instruction_ != nullptr);
  }

  HInstruction* GetInstruction() const { return instruction_; }
  typename HUseList<T>::iterator GetBeforeUseNode() const { return before_use_node_; }
  typename HUseList<T>::iterator GetUseNode() const {
    return ++typename HUseList<T>::iterator(before_use_node_);
  }
...
#undef DECLARE_KIND
  ...
  void AddUseAt(HInstruction* user, size_t index) {
    ...
    HUseList<HInstruction*>::iterator before_use = uses_.before_begin();
    user->SetRawInputRecordAt(index,
                              HUserRecord<HInstruction*>(user->InputRecordAt(index), before_use));
    HUseList<HInstruction*>::iterator next = ++uses_.begin();
    if (next != uses_.end()) {
      HInstruction* next_user = next->GetUser();
      size_t next_index = next->GetIndex();
      HUserRecord<HInstruction*> old_record = next_user->InputRecordAt(next_index);
      DCHECK(old_record.GetBeforeUseNode() == uses_.before_begin());
      next_user->SetRawInputRecordAt(next_index,
                                     HUserRecord<HInstruction*>(old_record, uses_.begin()));
    }
  }
  ...
  void AddEnvUseAt(HEnvironment* user, size_t index) {
    ...
    user->vregs_[index] = HUserRecord<HEnvironment*>(user->vregs_[index], env_uses_.before_begin());
    HUseList<HEnvironment*>::iterator next = ++env_uses_.begin();
    if (next != env_uses_.end()) {
      HEnvironment* next_user = next->GetUser();
      size_t next_index = next->GetIndex();
      HUserRecord<HEnvironment*>& next_record = next_user->vregs_[next_index];
      DCHECK(next_record.GetBeforeUseNode() == env_uses_.before_begin());
      next_record = HUserRecord<HEnvironment*>(next_record, env_uses_.begin());
    }
  }
  ...
  void RemoveAsUserOfInput(size_t input) {
    ...
    HUseList<HInstruction*>::iterator use_node =
        ++HUseList<HInstruction*>::iterator(before_use_node);
    HUseList<HInstruction*>::iterator next = ++HUseList<HInstruction*>::iterator(use_node);
    if (next != input_use.GetInstruction()->uses_.end()) {
      HInstruction* next_user = next->GetUser();
      size_t next_index = next->GetIndex();
      HUserRecord<HInstruction*> old_record = next_user->InputRecordAt(next_index);
      DCHECK(old_record.GetBeforeUseNode() == use_node);
      next_user->SetRawInputRecordAt(next_index,
                                     HUserRecord<HInstruction*>(old_record, before_use_node));
    }
