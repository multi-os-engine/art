 ...
 * integer field read), the access type (e.g.  a reference array write cannot
 ...
 * write/read dependence]) and offset (e.g. a field write cannot modify the
 * value of a field read with different offset). The analysis makes conservative
 * points-to assumptions on reference types (e.g. two same typed arrays are
 * assumed to be the same, and any reference read depends on any reference read
 * without further regard of its type).
 *
...
class SideEffects : public ValueObject {
 public:
  ...
  static SideEffects All() {
    return SideEffects(kAllChangeBits | kAllDependOnBits, kDataOffsetAny);
  }

  static SideEffects AllChanges() {
    return SideEffects(kAllChangeBits, kDataOffsetAny);
  }

  static SideEffects AllDependencies() {
    return SideEffects(kAllDependOnBits, kDataOffsetAny);
  }
  ...
  static SideEffects CanTriggerGC() {
    return SideEffects(1ULL << kCanTriggerGCBit, 0);
  }

  static SideEffects DependsOnGC() {
    return SideEffects(1ULL << kDependsOnGCBit, 0);
  }
  ...
  SideEffects Union(SideEffects other) const {
    ...
    if ((flags_ & (kAllWrites | kAllReads)) == 0) {
      data_offset = other.data_offset_;
    } else if ((other.flags_ & (kAllWrites | kAllReads)) == 0) {
      data_offset = data_offset_;
    } else {
      data_offset = data_offset_ != other.data_offset_ ? kDataOffsetAny : data_offset_;
    }
  ...
  SideEffects Exclusion(SideEffects other) const {
    uint64_t flags = flags_ & ~other.flags_;
    uint32_t data_offset = (flags & (kAllWrites | kAllReads)) ? data_offset_ : 0;
    return SideEffects(flags, data_offset);
  }

  bool Includes(SideEffects other) const {
    const bool may_include = ((other.flags_ & flags_) == other.flags_);
    if (!may_include) {
      return false;
    }
 ...
 private:
  ...
  static constexpr uint64_t kFieldWrites =
      ((1ULL << kFieldArrayAnalysisBits) - 1) << kFieldWriteOffset;
  static constexpr uint64_t kArrayWrites =
      ((1ULL << kFieldArrayAnalysisBits) - 1) << kArrayWriteOffset;
  static constexpr uint64_t kFieldReads =
      ((1ULL << kFieldArrayAnalysisBits) - 1) << kFieldReadOffset;
  static constexpr uint64_t kArrayReads =
      ((1ULL << kFieldArrayAnalysisBits) - 1) << kArrayReadOffset;
  static_assert(kAllWrites == (kFieldWrites | kArrayWrites), "write bits doesn't match");
  static_assert(kAllReads == (kFieldReads | kArrayReads), "read bits doesn't match");
...
class HArraySet : public HTemplateInstruction<3> {
 public:
  HArraySet(HInstruction* array,
            ...
            uint32_t dex_pc)
      : HTemplateInstruction(
            SideEffects::ArrayWriteOfType(expected_component_type, index).Union(
...
inline void SideEffects::SetArrayAccessIndex(HInstruction* index) {
  DCHECK(flags_ & (kArrayReads | kArrayWrites));
