...
class ReferenceTypeInfo : ValueObject {
 ...
 private:
  ReferenceTypeInfo() : type_handle_(TypeHandle()), is_exact_(false), is_valid_(false) {}

  ReferenceTypeInfo(TypeHandle type_handle, bool is_exact)
      : type_handle_(type_handle), is_exact_(is_exact), is_valid_(true) {
    if (kIsDebugBuild) {
  ...
  bool is_exact_;
  // Whether or not this handle contains a valid value.
  // Cached in the class to avoid acquiring the lock.
  bool is_valid_;
};
...
#undef DECLARE_KIND
  ...
  virtual void SetReferenceTypeInfo(ReferenceTypeInfo reference_type_info) {
    if (kIsDebugBuild) {
      DCHECK_EQ(GetType(), Primitive::kPrimNot);
      ScopedObjectAccess soa(Thread::Current());
      DCHECK(reference_type_info.IsValid()) << "Invalid RTI for " << DebugName();
    }
...
class HBoundType : public HExpression<1> {
 public:
  ...
  void SetReferenceTypeInfo(ReferenceTypeInfo rti) OVERRIDE {
    if (kIsDebugBuild) {
      ScopedObjectAccess soa(Thread::Current());
      DCHECK(bound_type_.IsSupertypeOf(rti));
    }
    HInstruction::SetReferenceTypeInfo(rti);
  }
