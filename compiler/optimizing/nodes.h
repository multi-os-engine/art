...
class HSelect : public HExpression<3> {
 public:
  HSelect(HInstruction* condition,
    ...
    // First input must be one of the values to allow codegens to use the
    // SameAsFirstInput allocation policy. If it is `false_value`, architectures
    // which implement HSelect as a conditional move also will not need to invert
    // the condition.
