...
bool InstructionSimplifierArm64Visitor::TryMergeIntoShifterOperand(HInstruction* binop,
                                                                   HInstruction* bitfield_op,
                                                                   bool do_merge) {
  DCHECK(HasShifterOperand(binop));
  DCHECK(binop->IsBinaryOperation() || binop->IsNeg());
  ...
  HInstruction* left;
  HInstruction* right;
  if (binop->IsBinaryOperation()) {
    left = binop->InputAt(0);
    right = binop->InputAt(1);
  } else {
    DCHECK(binop->IsNeg());
    right = binop->AsNeg()->InputAt(0);
  ...
  bool is_commutative = binop->IsBinaryOperation() && binop->AsBinaryOperation()->IsCommutative();
