...
static HGraph* BuildIfElseWithPhi(ArenaAllocator* allocator, HPhi** phi, HInstruction** input1, HInstruction** input2) {
  HGraph* graph = new (allocator) HGraph(allocator);
  ...
  HInstruction* test = new (allocator) HInstanceFieldGet(parameter, Primitive::kPrimBoolean, MemberOffset(22));
...
static HGraph* BuildFieldReturn(ArenaAllocator* allocator, HInstruction** field, HInstruction** ret) {
  HGraph* graph = new (allocator) HGraph(allocator);
  HBasicBlock* entry = new (allocator) HBasicBlock(graph);
  graph->AddBlock(entry);
  graph->SetEntryBlock(entry);
...
static HGraph* BuildTwoAdds(ArenaAllocator* allocator, HInstruction** first_add, HInstruction** second_add) {
  HGraph* graph = new (allocator) HGraph(allocator);
  HBasicBlock* entry = new (allocator) HBasicBlock(graph);
  graph->AddBlock(entry);
  graph->SetEntryBlock(entry);
