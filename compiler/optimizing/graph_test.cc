...
static HBasicBlock* createIfBlock(HGraph* graph, ArenaAllocator* allocator) {
  ...
  HInstruction* const4 = graph->GetIntConstant(4);
  HInstruction* equal = new (allocator) HEqual(const4, const4);
  if_block->AddInstruction(equal);
  const4 = new (allocator) HIf(equal);
  if_block->AddInstruction(const4);
  return if_block;
}
