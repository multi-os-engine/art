...
void HLoopInformation::Populate() {
  ...
  if (!is_irreducible_loop && graph->IsCompilingOsr()) {
    ...
    // from the interpreter. We treat this OSR entry just like an extra entry to
    // an irreducible loop, so we need to mark the method's loops as irreducible.
    // This does not apply to inlined loops which do not act as OSR entry.
