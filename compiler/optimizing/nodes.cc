...
void HGraph::SimplifyCatchBlocks() {
  ...
  for (size_t block_id = 0u, end = blocks_.size(); block_id != end; ++block_id) {
    ...
    if (!exceptional_predecessors_only) {
      ...
      // this does not apply to trivially dead code which has not been removed
      // at this stage. This will be enforced by GraphChecker after initial dead
      // block elimination (b/25492628).
      HBasicBlock* normal_block = catch_block->SplitCatchBlockAfterHeader();
      if (normal_block == nullptr) {
        // Catch block is either empty or contains only move-exception. It must
        // be dead, so will be removed during initial DCE. Do nothing.
...
HBasicBlock* HBasicBlock::SplitCatchBlockAfterHeader() {
  DCHECK(!graph_->IsInSsaForm()) << "Support for SSA form not implemented.";
