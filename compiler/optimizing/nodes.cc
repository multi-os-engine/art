...
void HEnvironment::RemoveAsUserOfInput(size_t index) const {
  ...
  HUseList<HEnvironment*>::iterator before_use_node = env_use.GetBeforeUseNode();
  HUseList<HEnvironment*>::iterator use_node = ++HUseList<HEnvironment*>::iterator(before_use_node);
  HUseList<HEnvironment*>::iterator next = ++HUseList<HEnvironment*>::iterator(use_node);
  if (next != env_use.GetInstruction()->env_uses_.end()) {
    ...
    DCHECK(next_record.GetBeforeUseNode() == use_node);
    next_record = HUserRecord<HEnvironment*>(next_record, before_use_node);
  }
  env_use.GetInstruction()->env_uses_.erase_after(before_use_node);
}
...
void HInstruction::ReplaceWith(HInstruction* other) {
  ...
  for (const HUseListNode<HInstruction*>& current : GetUses()) {
  ...
  for (const HUseListNode<HEnvironment*>& current : GetEnvUses()) {
