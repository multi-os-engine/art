...
void HInstruction::SetReferenceTypeInfo(ReferenceTypeInfo rti) {
  if (kIsDebugBuild) {
    ...
    if (IsBoundType()) {
      ...
      ReferenceTypeInfo bound_type_rti = AsBoundType()->GetBoundType();
      DCHECK(AsBoundType()->BoundsOnlyNull() || bound_type_rti.IsSupertypeOf(rti))
          << " bounds only null: " << AsBoundType()->BoundsOnlyNull()
          << " bounded_type: " << bound_type_rti
          << " rti: " << rti;
      DCHECK(!bound_type_rti.GetTypeHandle()->IsFinal() || rti.IsExact());
    }
  }
