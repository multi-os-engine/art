...
void HBasicBlock::ReplaceInstruction(HInstruction* initial,
  ...
  DCHECK(initial->GetBlock()->GetBlockId() == GetBlockId());
...
bool HInstructionList::FoundBefore(const HInstruction* instruction1,
  ...
  LOG(FATAL) << "Cannot find an order between two instructions of the same block";
  return true;
}
  ...
  if (block != other_block) {
    ...
    if (IsPhi()) {
      if (!other_instruction->IsPhi()) {
        ...
        LOG(FATAL) << "Cannot compute whether a phi dominates another phi.";
        return false;
      }
    } else {
      // `this` is not a phi.
