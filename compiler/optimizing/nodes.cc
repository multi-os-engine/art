...
void HBasicBlock::DisconnectAndDelete() {
  ...
  for (HBasicBlock* predecessor : predecessors_) {
    ...
    if (last_instruction->IsTryBoundary() && !IsCatchBlock()) {
      // This block is the normal-flow successor of the TryBoundary which makes
      // `predecessor` dead. Since catch blocks were already visited, remaining
      // ones must be live and we should remove `predecessor` from them first.
