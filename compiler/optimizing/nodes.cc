...
void HLoopInformation::PopulateIrreducibleRecursive(HBasicBlock* block, ArenaBitVector* visited) {
  size_t block_id = block->GetBlockId();

  // If `block` is in `visited`, we know its membership in the loop has been
  // finalized and it does not need to be revisited.
  if (visited->IsBitSet(block_id)) {
    return;
  }

  bool is_loop_member = false;
  if (block->IsLoopHeader()) {
    ...
    PopulateIrreducibleRecursive(pre_header, visited);
    ...
    for (HBasicBlock* predecessor : block->GetPredecessors()) {
      PopulateIrreducibleRecursive(predecessor, visited);
      if (!is_loop_member && blocks_.IsBitSet(predecessor->GetBlockId())) {
        is_loop_member = true;
        block->SetInLoop(this);
        blocks_.SetBit(block_id);
        visited->SetBit(block_id);
