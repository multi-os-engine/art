...
HBasicBlock* HBasicBlock::SplitAfter(HInstruction* cursor) {
  ...
  new_block->instructions_.SetBlock(new_block);
...
void HInstructionList::SetBlock(HBasicBlock* block) const {
...
void HBasicBlock::MergeWith(HBasicBlock* other) {
  DCHECK(successors_.IsEmpty() || (successors_.Size() == 1 && successors_.Get(0) == other));
  ...
  other->GetInstructions().SetBlock(this);
...
void HGraph::InlineInto(HGraph* outer_graph, HInvoke* invoke) {
  ...
  if (GetBlocks().Size() == 3) {
    ...
    body->GetInstructions().SetBlock(invoke->GetBlock());
    ...
    // to not `HReturn` because `HGoto` instead. Also collect the return
