...
BuildSsaResult HGraph::TryBuildingSsa(StackHandleScopeCollection* handles) {
  ...
  // Create the bottom reference type, i.e. inexact Object.
  ScopedObjectAccess soa(Thread::Current());
  ClassLinker* linker = Runtime::Current()->GetClassLinker();
  bottom_rti_ = ReferenceTypeInfo::Create(
...
HNullConstant* HGraph::GetNullConstant(uint32_t dex_pc) {
  ...
  if ((cached_null_constant_ == nullptr) || (cached_null_constant_->GetBlock() == nullptr)) {
    if (kIsDebugBuild) {
      ScopedObjectAccess soa(Thread::Current());
      DCHECK(bottom_rti_.IsValid());
    }
    cached_null_constant_ = new (arena_) HNullConstant(dex_pc);
    cached_null_constant_->SetReferenceTypeInfo(bottom_rti_);
