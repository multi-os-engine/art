...
void HGraph::UpdateLoopAndTryInformationOfInlinedBlock(HBasicBlock* block,
  ...
  if (GetBlocks().size() == 3) {
    ...
    for (HReversePostOrderIterator it(*this); !it.Done(); it.Advance()) {
      ...
      if (current != exit_block_ && current != entry_block_ && current != first) {
        ...
        UpdateLoopAndTryInformationOfInlinedBlock(current, at,  /* replace_if_back_edge */ false);
      }
    }
    ...
    UpdateLoopAndTryInformationOfInlinedBlock(to, at, /* replace_if_back_edge */ true);
  }
