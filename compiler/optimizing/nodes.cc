...
void HGraph::InlineInto(HGraph* outer_graph, HInvoke* invoke) {
  ...
  for (HInstructionIterator it(entry_block_->GetInstructions()); !it.Done(); it.Advance()) {
    ...
    if (current->IsConstant()) {
      ...
      // from outer graph. Assign it a new id that is specific to the containing graph.
  ...
  // Ideally we want to add the instructions using HBasicBlock::AddInstruction.
  // However, since that has a larger overhead because it treats each instruction
  // independently, we fix the links manually.
  if (first != last) {
