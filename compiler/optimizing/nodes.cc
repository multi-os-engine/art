...
static void Remove(HInstructionList* instruction_list,
  ...
  if (environment != nullptr) {
    for (size_t i = 0, e = environment->Size(); i < e; ++i) {
      ...
      if (vreg != nullptr) {
        vreg->RemoveEnvironmentUser(instruction, i);
      }
    }
  }
}
...
void HInstruction::RemoveEnvironmentUser(HInstruction* user, size_t input_index) {
  RemoveFromUseList(user->GetEnvironment(), input_index, &env_uses_);
}
