...
static void RemoveAsUser(HInstruction* instruction) {
  for (size_t i = 0; i < instruction->InputCount(); i++) {
    HUseListNode<HInstruction*>* input_use = instruction->InputUseAt(i);
    instruction->InputAt(i)->RemoveUser(input_use);
  }
  ...
  if (environment != nullptr) {
    for (size_t i = 0, e = environment->Size(); i < e; ++i) {
      ...
      if (vreg != nullptr) {
        ...
        vreg->RemoveEnvironmentUser(env_use);
      }
    }
  }
}
...
void HPhi::AddInput(HInstruction* input) {
  ...
  inputs_.Add(HUseRecord<HInstruction*>(input));
