...
void HGraph::ComputeTryBlockInformation() {
  ...
  for (HReversePostOrderIterator it(*this); !it.Done(); it.Advance()) {
    ...
    if (try_entry != nullptr &&
      ...
      // has changed. Otherwise leave as is.
...
void HBasicBlock::DisconnectAndDelete() {
  ...
  for (HBasicBlock* predecessor : predecessors_) {
    ...
    if (last_instruction->IsTryBoundary() && !IsCatchBlock()) {
      // This block is the normal-flow successor of the TryBoundary. If all
      // handlers of this TryBoundary were dead, they would have been deleted
      // already (and the instruction replaced with a Goto) since DCE deletes
      // blocks in post order. The predecessor must be dead and we should remove
      // it as predecessor of its catch blocks first.
      DCHECK_EQ(last_instruction->AsTryBoundary()->GetNormalFlowSuccessor(), this);
      while (predecessor->GetSuccessors().size() > 1) {
        HBasicBlock* handler = predecessor->GetSuccessors().back();
    ...
    if (num_pred_successors == 1u) {
      ...
      // had an HIf, HPackedSwitch or HTryBundary, as they have more than one
