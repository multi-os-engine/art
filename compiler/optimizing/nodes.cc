...
void HGraph::SimplifyLoop(HBasicBlock* header) {
  ...
  if (kIsDebugBuild && ContainsElement(header->GetPredecessors(), GetEntryBlock())) {
    DCHECK_EQ(number_of_incomings, 1u);
    DCHECK(header->GetPredecessors()[0]->IsEntryBlock());
    DCHECK(!info->IsBackEdge(*GetEntryBlock()));
  }
  if (number_of_incomings != 1 || header->GetPredecessors()[0]->IsEntryBlock()) {
