...
static bool EndsWithAnIf(HBasicBlock* block) {
  ...
  return last_instruction != nullptr && last_instruction->IsIf();
}
...
// Returns true if 'block1' and 'block2' merge into the same single successor,
// and the successor can only be reached from them.
static bool MergeTogether(HBasicBlock* block1, HBasicBlock* block2) {
  DCHECK(block1->IsSingleGoto());
  DCHECK(block2->IsSingleGoto());
...
static HInstruction* OppositeCondition(HInstruction* cond) {
...
void HBooleanNotSimplifier::Run() {
  for (HPostOrderIterator it(*graph_); !it.Done(); it.Advance()) {
    ...
    if (!true_block->IsSingleGoto()
        || !false_block->IsSingleGoto()
        || !MergeTogether(true_block, false_block)) {
      continue;
    }
    HBasicBlock* merge_block = true_block->GetSuccessors().Get(0);
    if (!HasSinglePhi(merge_block)) continue;
    ...
    if (NegatesCondition(true_value, false_value)) {
      replacement = OppositeCondition(if_condition);
    ...
    // Remove the original condition if it is now redundant.
    if (if_condition->GetUses().IsEmpty()) {
