...
void ImageWriter::SetImageOffset(mirror::Object* object, size_t offset) {
  ...
  LockWord lw(object->GetLockWord(true));
  switch (lw.GetState()) {
  ...
  object->SetLockWord(LockWord::FromForwardingAddress(offset), true);
  DCHECK(IsImageOffsetAssigned(object));
}
...
bool ImageWriter::IsImageOffsetAssigned(mirror::Object* object) const {
  DCHECK(object != nullptr);
  return object->GetLockWord(true).GetState() == LockWord::kForwardingAddress;
}

size_t ImageWriter::GetImageOffset(mirror::Object* object) const {
  DCHECK(object != nullptr);
  DCHECK(IsImageOffsetAssigned(object));
  LockWord lock_word = object->GetLockWord(true);
...
void ImageWriter::CopyAndFixupObjects()
  ...
  for (const std::pair<mirror::Object*, uint32_t>& hash_pair : saved_hashes_) {
    hash_pair.first->SetLockWord(LockWord::FromHashCode(hash_pair.second), true);
  }
  saved_hashes_.clear();
...
void ImageWriter::CopyAndFixupObjectsCallback(Object* obj, void* arg) {
  ...
  // word.
  copy->SetLockWord(LockWord(), true);
