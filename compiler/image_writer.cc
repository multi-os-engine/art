...
ObjectArray<Object>* ImageWriter::CreateImageRoots() const {
  ...
  Handle<ObjectArray<Object> > image_roots(
...
void ImageWriter::WalkInstanceFields(mirror::Object* obj, mirror::Class* klass) {
  ...
  Handle<mirror::Class> h_class(Thread::Current(), klass);
...
void ImageWriter::WalkFieldsInOrder(mirror::Object* obj) {
  if (!IsImageOffsetAssigned(obj)) {
    ...
    Thread* self = Thread::Current();
    Handle<mirror::Object> h_obj(self, obj);
    Handle<mirror::Class> klass(self, obj->GetClass());
...
void ImageWriter::CalculateNewObjectOffsets(size_t oat_loaded_size, size_t oat_data_offset) {
  ...
  Thread* self = Thread::Current();
  Handle<ObjectArray<Object> > image_roots(self, CreateImageRoots());
...
static ArtMethod* GetTargetMethod(const CompilerDriver::CallPatchInformation* patch)
  ...
  ClassLinker* class_linker = Runtime::Current()->GetClassLinker();
  Thread* self = Thread::Current();
  Handle<mirror::DexCache> dex_cache(self, class_linker->FindDexCache(*patch->GetTargetDexFile()));
...
static Class* GetTargetType(const CompilerDriver::TypePatchInformation* patch)
  ...
  ClassLinker* class_linker = Runtime::Current()->GetClassLinker();
  Thread* self = Thread::Current();
  Handle<mirror::DexCache> dex_cache(self, class_linker->FindDexCache(patch->GetDexFile()));
  Handle<mirror::ClassLoader> class_loader(self, nullptr);
