...
void ImageWriter::PruneNonImageClasses() {
  ...
  std::list<jobject> dex_caches;
  {
    ReaderMutexLock mu(self, *class_linker->DexLock());
    dex_caches = class_linker->GetDexCaches();
  }
  for (jobject weak_root : dex_caches) {
    // Since we know that GC isn't running at this point we can be sure that the system weak will
    // be valid.
