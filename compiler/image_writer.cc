    ...
    if (h_obj->IsClass()) {
      ...
      for (size_t i = 0; i < 2; ++i) {
        for (size_t j = 0; j < num_fields[i]; ++j) {
          ...
          if (obj->AsClass()->IsReferenceClass()) {
            LOG(INFO) << i << ":" << j << " " << PrettyField(field);
          }
...
void ImageWriter::CreateHeader(size_t oat_loaded_size, size_t oat_data_offset) {
  ...
  cur_pos += bitmap_size;
  LOG(INFO) << "IMAGE SIZE BYTES=" << cur_pos;
  ...
  if (klass->IsArrayClass() &&
    ...
    if (it != dex_cache_array_indexes_.end()) {
      ...
      if (target_ptr_size_ == 4) {
        ...
        dst->SetClass(mirror::IntArray::GetArrayClass());
      } else {
        DCHECK_EQ(target_ptr_size_, 8u);
        dst->SetClass(mirror::LongArray::GetArrayClass());
      }
      mirror::Array* dest_array = dst->AsArray<kVerifyNone>();
