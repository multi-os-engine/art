    ...
    if (h_obj->IsClass()) {
      ...
      for (LengthPrefixedArray<ArtField>* cur_fields : fields) {
        ...
        if (cur_fields != nullptr) {
          const size_t array_length = LengthPrefixedArray<ArtField>::OffsetOfElement(
              cur_fields->Length(), sizeof(ArtField));
          ...
          CHECK(it == native_object_relocations_.end()) << "Field array " << cur_fields
                                                  << " already forwarded";
          const size_t array_offset = bin_slot_sizes_[kBinArtField];
          native_object_relocations_.emplace(
              cur_fields, NativeObjectRelocation {
                  array_offset, kNativeObjectRelocationTypeArtFieldArray });
          bin_slot_sizes_[kBinArtField] += array_length;
          ...
          for (size_t i = 0, count = cur_fields->Length(); i < count; ++i) {
            ...
            ArtField* field = &cur_fields->At(i, sizeof(ArtField));
            ...
            const size_t offset = array_offset +
                LengthPrefixedArray<ArtField>::OffsetOfElement(i, sizeof(ArtField));
            native_object_relocations_.emplace(
      ...
      for (auto& array : method_arrays) {
        ...
        for (auto& m : array) {
          AssignMethodOffset(&m, any_dirty ? kNativeObjectRelocationTypeArtMethodDirty :
...
void ImageWriter::CalculateNewObjectOffsets() {
  ...
  const auto image_method_type = kNativeObjectRelocationTypeArtMethodDirty;
  ...
  for (auto* m : image_methods_) {
    ...
    AssignMethodOffset(m, image_method_type);
  }
  ...
  for (auto& pair : native_object_relocations_) {
    ...
    switch (relocation.type) {
      ...
      case kNativeObjectRelocationTypeArtFieldArray: {
        memcpy(dest, pair.first, LengthPrefixedArray<ArtField>::OffsetOfElement(0, 0));
        break;
      }
      case kNativeObjectRelocationTypeArtMethodArrayClean:
      case kNativeObjectRelocationTypeArtMethodArrayDirty: {
        memcpy(dest, pair.first, LengthPrefixedArray<ArtMethod>::OffsetOfElement(0, 0));
        break;
      }
    }
  }
