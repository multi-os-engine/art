...
void ImageWriter::SetImageOffset(mirror::Object* object,
  ...
  // LOG(INFO) << "Object offset " << reinterpret_cast<void*>(offset) << " max=" << reinterpret_cast<void*>(max_offset);
...
void ImageWriter::PrepareDexCacheArraySlots() {
  ...
  for (size_t idx = 0; idx < dex_cache_count; ++idx) {
    ...
    dex_cache_array_indexes_.Put(dex_cache->GetResolvedTypes(),
                                 std::make_pair(size + layout.TypesOffset(), types_size));
    dex_cache_array_indexes_.Put(dex_cache->GetResolvedMethods(),
                                 std::make_pair(size + layout.MethodsOffset(), methods_size));
    dex_cache_array_indexes_.Put(dex_cache->GetResolvedFields(),
                                 std::make_pair(size + layout.FieldsOffset(), fields_size));
    dex_cache_array_indexes_.Put(dex_cache->GetStrings(),
                                 std::make_pair(size + layout.StringsOffset(), strings_size));
    size += layout.Size();
    ...
    if (object->IsClass()) {
      ...
      if (!component_type->IsPrimitive() || component_type->IsPrimitiveInt() ||
        ...
        if (it != dex_cache_array_indexes_.end()) {
          ...
          current_offset = it->second.first;  // Use prepared offset defined by the DexCacheLayout.
          object_size = it->second.second;  // Override incase of cross compilation.
    ...
    if (h_obj->IsClass()) {
      ...
      for (size_t i = 0; i < 2; ++i) {
        for (size_t j = 0; j < num_fields[i]; ++j) {
          ...
          art_field_reloc_.insert(std::make_pair(field, bin_slot_sizes_[kBinArtField]));
...
void ImageWriter::CreateHeader(size_t oat_loaded_size, size_t oat_data_offset) {
  ...
  cur_pos += bitmap_size;
  LOG(INFO) << image_end_ << " fields=" << fields_size << " total=" << bitmap_size;
...
void ImageWriter::CopyAndFixupObject(Object* obj) {
  ...
  if (klass->IsArrayClass() &&
      (klass->GetComponentType()->IsPrimitiveInt() ||
          klass->GetComponentType()->IsPrimitiveLong())) {
    auto it = dex_cache_array_indexes_.find(obj);  // Is this a dex cache array?
    if (it != dex_cache_array_indexes_.end()) {
      CHECK_EQ(reinterpret_cast<Object*>(
          image_->Begin() + it->second.first + image_objects_offset_begin_), dst);
      ...
      const bool is_int = klass->GetComponentType()->IsPrimitiveInt();
      ...
      if (target_ptr_size_ == 4) {
        ...
        dst->SetClass(
            down_cast<mirror::Class*>(GetImageAddress(mirror::LongArray::GetArrayClass())));
      }
      mirror::Array* dest_array = down_cast<mirror::Array*>(dst);
      dest_array->SetLength(num_elements);
      for (size_t i = 0, count = num_elements; i < count; ++i) {
        ArtField* field = reinterpret_cast<ArtField*>(
            is_int ? arr->AsIntArray()->GetWithoutChecks(i) :
                arr->AsLongArray()->GetWithoutChecks(i));
        uint8_t* fixup_location = nullptr;
        if (field != nullptr) {
          auto it2 = art_field_reloc_.find(field);
          if (it2 != art_field_reloc_.end()) {
            // Already relocated.
            fixup_location = image_begin_ + it2->second;
          } else {
            LOG(WARNING) << "Not found in map " << field;
          }
        }
        if (target_ptr_size_ == 4) {
...
void ImageWriter::FixupObject(Object* orig, Object* copy) {
  ...
  const bool is_class = orig->IsClass();
  if (is_class) {
  ...
  if (is_class && orig->AsClass()->ShouldHaveEmbeddedImtAndVTable()) {
    auto orig_ifields = down_cast<Class*>(copy)->GetIFields();
    auto orig_sfields = down_cast<Class*>(copy)->GetSFields();
    FixupClassVisitor visitor(this, copy);
    orig->VisitReferences<true /*visit class*/>(visitor, visitor);
    CHECK_EQ(orig_ifields, down_cast<Class*>(copy)->GetIFields());
    CHECK_EQ(orig_sfields, down_cast<Class*>(copy)->GetSFields());
  } else {
