...
bool ImageWriter::Write(const std::string& image_filename,
  ...
  // Create header after CopyAndFixupNativeData since we need to put updated ArtMethod pointers in
  // it.
...
void ImageWriter::CreateHeader(size_t oat_loaded_size, size_t oat_data_offset) {
  ...
  // Set and forward the image method roots.
  for (size_t i = 0; i < ImageHeader::kImageMethodCount; ++i) {
    ...
    CHECK_GE(native_reloc.offset, fields_offset + fields_size);
    CHECK_EQ(native_reloc.bin_type, kBinArtMethod);
    auto* dest = reinterpret_cast<ArtMethod*>(image_->Begin() + it->second.offset);
...
void ImageWriter::FixupPointerArray(mirror::Object* dst, mirror::PointerArray* arr,
  ...
  for (size_t i = 0, count = num_elements; i < count; ++i) {
    ...
    if (elem != nullptr) {
      ...
      CHECK(it != native_object_reloc_.end()) << "No relocation for native object " << elem;
