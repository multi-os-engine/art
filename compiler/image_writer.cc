...
class FixupClassVisitor : public FixupVisitor {
 public:
  ...
  void operator()(Object* obj, MemberOffset offset, bool /*is_static*/) const
    ...
    // This will be turned on when we embed method quick entry point into embedded vtable.
    if (false) {
      // Set quick entry point for embedded imt/vtable.
      Object* ref = obj->GetFieldObject<Object, kVerifyNone>(offset);
      if (ref != nullptr && ref->IsArtMethod()) {
        ...
        DCHECK((offset.Uint32Value() >= sizeof(mirror::Class)) &&
...
// This follows the same logic of FixupMethod() below to get
