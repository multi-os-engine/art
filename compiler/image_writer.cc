...
void ImageWriter::PrepareDexCacheArraySlots() {
  ...
  for (size_t idx = 0; idx < dex_cache_count; ++idx) {
    ...
    pointer_arrays_.emplace(dex_cache->GetResolvedMethods(), kBinArtMethod);
    dex_cache_array_indexes_.Put(
...
void ImageWriter::AssignImageBinSlot(mirror::Object* object) {
  ...
  if (kBinObjects) {
    ...
    if (object->IsClass()) {
      ...
      if (vtable != nullptr) {
        pointer_arrays_.emplace(vtable, kBinArtMethod);
      }
      ...
      if (iftable != nullptr) {
        for (int32_t i = 0; i < klass->GetIfTableCount(); ++i) {
          if (iftable->GetMethodArrayCount(i) > 0) {
            ...
            pointer_arrays_.emplace(method_array, kBinArtMethod);
          }
        }
      }
...
void ImageWriter::AssignMethodOffset(ArtMethod* method) {
  ...
  CHECK(it == native_object_reloc_.end()) << "Method " << method << " already assigned "
      << PrettyMethod(method);
  native_object_reloc_.emplace(
      method, NativeObjectReloc { bin_slot_sizes_[kBinArtMethod], kBinArtMethod });
...
void ImageWriter::CalculateNewObjectOffsets() {
  ...
  // Update the ArtMethods and ArtFields by adding their bin sums.
...
void ImageWriter::CreateHeader(size_t oat_loaded_size, size_t oat_data_offset) {
  ...
  CHECK_EQ(image_objects_offset_begin_ + GetBinSizeSum(kBinArtField), field_section.Offset());
  cur_pos = field_section.End();
  // Add method section.
  auto& methods_section = sections[ImageHeader::kSectionArtMethods];
  methods_section = ImageSection(cur_pos, bin_slot_sizes_[kBinArtMethod]);
  CHECK_EQ(image_objects_offset_begin_ + GetBinSizeSum(kBinArtMethod), methods_section.Offset());
...
void ImageWriter::CopyAndFixupNativeData() {
  ...
  for (auto& pair : native_object_reloc_) {
    ...
    if (native_reloc.bin_type == kBinArtField) {
      ...
      auto* dest = image_->Begin() + native_reloc.offset;
      DCHECK_GE(dest, image_->Begin() + image_end_);
      if (reinterpret_cast<ArtMethod*>(pair.first)->IsRuntimeMethod()) {
        LOG(ERROR) << "Copy " << pair.first << " to "
            << reinterpret_cast<void*>(image_begin_ + native_reloc.offset);
        LOG(ERROR) << sizeof(mirror::Class) << " " << sizeof(void*) << " " << target_ptr_size_;
      }
  ...
  const auto& methods_section = image_header->GetImageSection(ImageHeader::kSectionArtMethods);
  for (size_t i = 0; i < ImageHeader::kImageMethodsCount; ++i) {
    ...
    CHECK(methods_section.Contains(native_reloc.offset)) << native_reloc.offset << " not in "
        << methods_section;
    CHECK_EQ(native_reloc.bin_type, kBinArtMethod);
  ...
  for (size_t i = 0, count = num_elements; i < count; ++i) {
    ...
    if (elem != nullptr) {
      ...
      if (it == native_object_reloc_.end()) {
        if (array_type == kBinArtMethod) {
          auto* method = reinterpret_cast<ArtMethod*>(elem);
          if (method->IsMiranda()) {
            // A miranda method of something which wasn't an image class. Set the entry to null.
            elem = nullptr;
          } else {
...
void ImageWriter::CopyAndFixupMethod(ArtMethod* orig, ArtMethod* copy) {
...
#if 0
  // It makes no sense to copy the pointers from the original method, set specia placeholder values
  // to catch errors.
  copy->SetEntryPointFromInterpreterPtrSize(
      reinterpret_cast<EntryPointFromInterpreter*>(0x1), target_ptr_size_);
  copy->SetEntryPointFromJniPtrSize(reinterpret_cast<void*>(0x2), target_ptr_size_);
  copy->SetEntryPointFromQuickCompiledCodePtrSize(reinterpret_cast<void*>(0x3), target_ptr_size_);
#endif
  ...
  if (UNLIKELY(orig == runtime->GetResolutionMethod())) {
    ...
    for (size_t i = 0; i < static_cast<size_t>(Runtime::kLastCalleeSaveType); ++i) {
      ...
      found_one = found_one ||
          (runtime->HasCalleeSaveMethod(idx) && runtime->GetCalleeSaveMethod(idx) == orig);
    }
