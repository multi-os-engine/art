...
bool ImageWriter::IsInImageSpace(const void* obj) const {
  if (!compile_app_image_) {
    // If we are compiling the boot image, then there should not be an existing image space.
    return false;
  }
  // Can not use FindContinuousSpaceFromObject since it only works on heap objects.
  for (gc::space::ContinuousSpace* space : Runtime::Current()->GetHeap()->GetContinuousSpaces()) {
    if (space->IsImageSpace()) {
      DCHECK(compile_app_image_);
      gc::space::ImageSpace* image_space = space->AsImageSpace();
      const uint8_t* image_begin = image_space->Begin();
      // Real image end including ArtMethods and ArtField sections.
      const uint8_t* image_end = image_begin + image_space->GetImageHeader().GetImageSize();
...
void ImageWriter::PrepareDexCacheArraySlots() {
  ...
  for (jobject weak_root : class_linker->GetDexCaches()) {
    ...
    if (dex_cache == nullptr || IsInImageSpace(dex_cache)) {
      continue;
    }
...
bool ImageWriter::IsImageClass(Class* klass) {
  ...
  std::string temp;
  return compiler_driver_.IsImageClass(klass->GetDescriptor(&temp)) || IsInImageSpace(klass);
}
...
void ImageWriter::CheckNonImageClassesRemovedCallback(Object* obj, void* arg) {
  ...
  if (obj->IsClass() && !image_writer->IsInImageSpace(obj)) {
...
ObjectArray<Object>* ImageWriter::CreateImageRoots() const {
  ...
  size_t dex_cache_count = 0;
  {
    ...
    for (jobject weak_root : class_linker->GetDexCaches()) {
      mirror::DexCache* dex_cache = down_cast<mirror::DexCache*>(self->DecodeJObject(weak_root));
      dex_cache_count += IsInImageSpace(dex_cache) ? 0u : 1u;
    }
  }
  ...
  CHECK(dex_caches.Get() != nullptr) << "Failed to allocate a dex cache array.";
  {
    ...
    for (jobject weak_root : class_linker->GetDexCaches()) {
      mirror::DexCache* dex_cache = down_cast<mirror::DexCache*>(self->DecodeJObject(weak_root));
      non_image_dex_caches += IsInImageSpace(dex_cache) ? 0u : 1u;
    }
    ...
    if (h_obj->IsClass()) {
      ...
      for (LengthPrefixedArray<ArtField>* cur_fields : fields) {
        ...
        if (cur_fields != nullptr) {
          ...
          DCHECK(!IsInImageSpace(cur_fields));
          native_object_relocations_.emplace(
...
void ImageWriter::CalculateNewObjectOffsets() {
  ...
  for (auto* m : image_methods_) {
    ...
    DCHECK_EQ(compile_app_image_, IsInImageSpace(m)) << "Trampolines should be in boot image";
    if (!IsInImageSpace(m)) {
...
void ImageWriter::CopyAndFixupNativeData() {
  ...
  for (auto& pair : native_object_relocations_) {
    ...
    DCHECK(!IsInImageSpace(pair.first));
...
void ImageWriter::FixupPointerArray(mirror::Object* dst, mirror::PointerArray* arr,
  ...
  for (size_t i = 0, count = num_elements; i < count; ++i) {
    ...
    if (elem != nullptr && !IsInImageSpace(elem)) {
...
T* ImageWriter::NativeLocationInImage(T* obj) {
  return (obj == nullptr || IsInImageSpace(obj))
      ? obj
  ...
  if (orig->ShouldHaveEmbeddedImtAndVTable()) {
    for (int32_t i = 0; i < orig->GetEmbeddedVTableLength(); ++i) {
      copy->SetEmbeddedVTableEntryUnchecked(
          i,
          NativeLocationInImage(orig->GetEmbeddedVTableEntry(i, target_ptr_size_)),
          target_ptr_size_);
    }
...
void ImageWriter::FixupDexCache(mirror::DexCache* orig_dex_cache,
  ...
  if (orig_methods != nullptr) {
    ...
    for (size_t i = 0, num = orig_dex_cache->NumResolvedMethods(); i != num; ++i) {
      ...
      ArtMethod* copy = IsInImageSpace(orig) ? orig : NativeLocationInImage(orig);
  ...
  if (orig_fields != nullptr) {
    ...
    for (size_t i = 0, num = orig_dex_cache->NumResolvedFields(); i != num; ++i) {
      ...
      ArtField* copy = IsInImageSpace(orig) ? orig : NativeLocationInImage(orig);
...
const uint8_t* ImageWriter::GetQuickCode(ArtMethod* method, bool* quick_is_interpreted) {
  ...
  DCHECK(!IsInImageSpace(method)) << PrettyMethod(method);
