...
void ImageWriter::PrepareDexCacheArraySlots() {
  ...
  for (size_t idx = 0; idx < dex_cache_count; ++idx) {
    ...
    DexCacheArraysLayout layout(start_offset, dex_file);
    dex_cache_array_slots_.Put(dex_cache->GetResolvedTypes(), layout.TypesOffset());
    dex_cache_array_slots_.Put(dex_cache->GetResolvedMethods(), layout.MethodsOffset());
    dex_cache_array_slots_.Put(dex_cache->GetResolvedFields(), layout.FieldsOffset());
    dex_cache_array_slots_.Put(dex_cache->GetStrings(), layout.StringsOffset());
    dex_cache_array_starts_.Put(dex_file, layout.StartOffset());
    start_offset = layout.EndOffset();
  }
  bin_slot_sizes_[kBinDexCacheArray] = start_offset;

  // Grow the image closer to the end by the size of the arrays we just assigned.
  image_end_ += start_offset;
...
void ImageWriter::AssignImageBinSlot(mirror::Object* object) {
  ...
  if (kBinObjects) {
    ...
    if (object->IsClass()) {
      ...
      auto it = dex_cache_array_slots_.find(object);
      if (it != dex_cache_array_slots_.end()) {
