...
void ImageWriter::PrepareDexCacheArraySlots() {
  ...
  for (size_t idx = 0; idx < dex_cache_count; ++idx) {
    ...
    DCHECK(layout.Valid());
    dex_cache_array_slots_.Put(dex_cache->GetResolvedTypes(), size + layout.TypesOffset());
    dex_cache_array_slots_.Put(dex_cache->GetResolvedMethods(), size + layout.MethodsOffset());
    dex_cache_array_slots_.Put(dex_cache->GetResolvedFields(), size + layout.FieldsOffset());
    dex_cache_array_slots_.Put(dex_cache->GetStrings(), size + layout.StringsOffset());
    size += layout.Size();
  }
  bin_slot_sizes_[kBinDexCacheArray] = size;

  // Grow the image closer to the end by the size of the arrays we just assigned.
  // (This is done early to avoid DCHECK() failures in IsImageBinSlotAssigned()
...
void ImageWriter::AssignImageBinSlot(mirror::Object* object) {
  ...
  if (kBinObjects) {
    ...
    //   special PC-relative addressing, we arbitrarily keep the at the beginning.
