...
void ImageWriter::AddDexCacheArrayReloc(void* array, size_t offset) {
  if (array != nullptr) {
    native_object_relocations_.emplace(array, NativeObjectRelocation {
      offset, kNativeObjectRelocationTypeDexCacheArray });
  }
}
  ...
  for (auto& pair : native_object_relocations_) {
    ...
    switch (relocation.type) {
      ...
      case kNativeObjectRelocationTypeDexCacheArray:
        // Nothing to copy here, everything is done in FixupDexCacheArrays().
        break;
      }
    }
  }
...
void ImageWriter::FixupObject(Object* orig, Object* copy) {
  ...
  if (orig->IsClass()) {
    ...
    if (klass == mirror::Method::StaticClass() || klass == mirror::Constructor::StaticClass()) {
      ...
      if (klass ==
          Runtime::Current()->GetClassLinker()->GetClassRoot(ClassLinker::kJavaLangDexCache)) {
