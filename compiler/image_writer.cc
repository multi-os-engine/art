...
void ImageWriter::PatchOatCodeAndMethods() {
  ...
  const std::vector<const CompilerDriver::TypePatchInformation*>& klasses_to_patch =
      compiler_driver_.GetKlassesToPatch();
  for (size_t i = 0; i < klasses_to_patch.size(); i++) {
    const CompilerDriver::TypePatchInformation* patch = klasses_to_patch[i];
