  ...
  void MakeExecutable(mirror::ArtMethod* method) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    ...
    if (compiled_method != nullptr) {
      ...
      if (code != nullptr) {
        ...
        uint32_t vmap_table_offset = vmap_table.empty() ? 0u : code_size;
        const std::vector<uint8_t>& mapping_table = compiled_method->GetMappingTable();
        uint32_t mapping_table_offset = mapping_table.empty() ? 0u : code_size + vmap_table.size();
        ...
        chunk->insert(chunk->end(), vmap_table.begin(), vmap_table.end());
        chunk->insert(chunk->end(), mapping_table.begin(), mapping_table.end());
        CHECK_EQ(size, chunk->size());
        code_ptr = &(*chunk)[sizeof(method_header)];
      } else {
