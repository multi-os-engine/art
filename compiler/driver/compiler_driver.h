...
class CompilerDriver {
 public:
  ...
  void AddMethodPatch(const DexFile* dex_file,
                      ...
                      size_t literal_offset)
      LOCKS_EXCLUDED(compiled_methods_lock_);
  void AddKlassPatch(const DexFile* dex_file,
                     uint16_t referrer_class_def_idx,
                     uint32_t referrer_method_idx,
                     uint32_t target_method_idx,
                     size_t literal_offset)
  ...
  const std::vector<const TypePatchInformation*>& GetKlassesToPatch() const {
    return klasses_to_patch_;
  }
  ...
  // Get a resolved class from a dex file if it's resolved.
  mirror::Class* GetResolvedClass(const DexFile& dex_file, uint32_t type_idx);
 ...
 private:
  ...
  std::vector<const TypePatchInformation*> klasses_to_patch_;
