...
inline int CompilerDriver::IsFastInvoke(
  ...
  // FIXME: invoke-super relies on this check failing to prevent invoking an incorrect method.
  // Otherwise the wrong method will be chosen since they did not update the dex_method_index field
  ...
  bool can_sharpen_virtual_based_on_type = same_dex_file &&
      (*invoke_type == kVirtual) && (resolved_method->IsFinal() || methods_declaring_class->IsFinal());
  ...
  // TODO I am not sure if this is good enough. I believe that the dex_method_index should be the
  // same as the one of the method resolved by the builder so long as the same_dex_file but I am not
  // entirely sure.
