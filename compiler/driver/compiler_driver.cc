...
uint32_t CompilerDriver::AddEntrypointTrampoline(ThreadOffset entrypoint) {
  return entrypoint_trampolines_.AddEntrypoint(Thread::Current(), entrypoint);
}


void CompilerDriver::BuildEntrypointTrampolineCode() {
  const auto& vec = entrypoint_trampolines_.GetTrampolines();
  for (uint32_t offset : vec) {
    if (instruction_set_ == kThumb2) {
      BuildArmEntrypointTrampolineCall(offset);
    } else {
        LOG(FATAL) << "No entrypoint trampolines for this architecture";
    }
  }
}
...
FinalEntrypointRelocationSet* CompilerDriver::AllocateFinalEntrypointRelocationSet(
  ...
  switch (instruction_set_) {
    case kArm:
    case kThumb2:
      return new FinalEntrypointRelocationSetArm(this, cu);
    default:
      LOG(FATAL) << "Cannot allocate FinalEntrypointRelocationSet for non-ARM";
      return nullptr;
  }
}
