...
static DexToDexCompilationLevel GetDexToDexCompilationlevel(
  ...
  if (status.IsVerified()) {
    ...
    return kOptimize;
  } else if (status.IsCompileTimeVerified()) {
    // Class verification has soft-failed. Anyway, ensure at least correctness.
    DCHECK_EQ(mirror::Class::kStatusRetryVerificationAtRuntime, status.Value());
    return kRequired;
  } else {
...
static void ResolveExceptionsForMethod(MutableMethodHelper* mh,
  ...
  for (size_t i = 0; i < num_encoded_catch_handlers; i++) {
    ...
    for (int32_t j = 0; j < encoded_catch_handler_size; j++) {
      ...
      mirror::Class* catch_handler_type = mh->GetMethod()->GetInterfaceMethodIfProxy()
          ->GetDexCacheResolvedType(encoded_catch_handler_handlers_type_idx);
      if (catch_handler_type == nullptr) {
        // NOTE: Not adding erroneous exception types.
...
static void VerifyClass(const ParallelCompilationManager* manager, size_t class_def_index)
  ...
  if (klass.Get() == nullptr) {
    ...
    CHECK(status.IsCompileTimeVerified() || status.IsErroneous())
        << PrettyDescriptor(klass.Get()) << ": state=" << status;
  }
...
void CompilerDriver::RecordClassStatus(ClassReference ref, mirror::Class::Status status) {
  ...
  if (it == compiled_classes_.end() || it->second->GetStatus() != status) {
    ...
    if (it != compiled_classes_.end()) {
      CHECK_GT(status.Value(), it->second->GetStatus().Value());
      delete it->second;
    }
    switch (status.Value()) {
