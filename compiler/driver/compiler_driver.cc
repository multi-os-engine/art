...
void CompilerDriver::PreCompileSummary() {
  StaticAnalyzer* static_analyzer = Runtime::Current()->GetClassLinker()->GetStaticAnalyzer();
  if (static_analyzer != nullptr) {
    StaticAnalysisPass* pass = static_analyzer->GetPass("MethodLogisticsAnalysis");
    if (pass != nullptr) {
      MethodLogisticsAnalysis* logistics_pass = down_cast<MethodLogisticsAnalysis*>(pass);
      if ((static_cast <float>(logistics_pass->num_of_native_methods_)
          / static_cast <float>(logistics_pass->num_of_methods_)) > 0.50) {
        // TODO: There is a considerable number of native methods.
        // TODO: Make appropriate decision
      }
    }
  }
}
