...
class ClinitImageUpdate {
 public:
  explicit ClinitImageUpdate(std::set<std::string>* image_class_descriptors, Thread* self,
                             ClassLinker* linker)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) :
    ...
    dex_cache_class_ = linker->LookupClass(self, "Ljava/lang/reflect/ArtMethod;", nullptr);
  ...
  bool Initialized() {
    return art_method_class_ != nullptr && dex_cache_class_ != nullptr;
  }
...
void CompilerDriver::UpdateImageClasses(TimingLogger* timings) {
  if (IsImage()) {
    ...
    ClinitImageUpdate update(image_classes_.get(), Thread::Current(), current->GetClassLinker());
    CHECK(update.Initialized());  // TODO: Soft failure?
