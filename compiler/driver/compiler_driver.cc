...
bool CompilerDriver::CanEmbedTypeInCode(const DexFile& dex_file, uint32_t type_idx,
  ...
  if (GetCompilerOptions().GetCompilePic() || !GetCompilerOptions().GetCompilePic()) {
  ...
  *direct_method = 0;
  ...
  if (use_dex_cache) {
    ...
    if (image_space != nullptr) {
      const auto& method_section = image_space->GetImageHeader().GetImageSection(
          ImageHeader::kSectionArtMethods);
...
bool CompilerDriver::ComputeInvokeInfo(const DexCompilationUnit* mUnit, const uint32_t dex_pc,
  ...
  mirror::Class* referrer_class = resolved_method != nullptr
...
void CompilerDriver::CompileMethod(Thread* self, const DexFile::CodeItem* code_item,
  ...
  if ((access_flags & kAccNative) != 0) {
    ...
    if ((!compiler_options_->IsCompilationEnabled() &&
        InstructionSetHasGenericJniStub(instruction_set_)) || (false)) {
