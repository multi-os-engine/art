...
#define STATS_LOCK()
  ...
  void ResolvedInstanceFields(size_t resolved, size_t unresolved) {
    STATS_LOCK();
    resolved_instance_fields_ += resolved;
    unresolved_instance_fields_ += unresolved;
  }
  ...
  void ResolvedStaticFields(size_t local, size_t resolved, size_t unresolved) {
    STATS_LOCK();
    resolved_local_static_fields_ += local;
    resolved_static_fields_ += resolved;
    unresolved_static_fields_ += unresolved;
  }
...
void CompilerDriver::ComputeInstanceFieldAnnotations(const DexCompilationUnit* mUnit,
  ...
  for (auto it = annotations, end = annotations + count; it != end; ++it) {
    ...
    if (referrer_class->CanAccessResolvedField<false>(fields_class, resolved_field, field_idx)) {
      ...
      it->fast_put = (!resolved_field->IsFinal() || fields_class == referrer_class) ? 1u : 0u;
      ++resolved;
    }
  }
  stats_->ResolvedInstanceFields(resolved, count - resolved);
}
...
void CompilerDriver::ComputeStaticFieldAnnotations(const DexCompilationUnit* mUnit,
  ...
  stats_->ResolvedStaticFields(local, resolved, count - local - resolved);
}
