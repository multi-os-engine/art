...
void CompilerDriver::ComputeInstanceFieldAnnotations(const DexCompilationUnit* mUnit,
  ...
  for (auto it = annotations, end = annotations + count; it != end; ++it) {
    ...
    it->resolved_dex_file = fields_class->GetDexCache()->GetDexFile();
    it->resolved_class_idx = fields_class->GetDexTypeIndex();
    it->resolved_field_idx = resolved_field->GetDexFieldIndex();
