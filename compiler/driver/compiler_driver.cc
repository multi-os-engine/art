...
void CompilerDriver::ComputeInstanceFieldAnnotations(const DexCompilationUnit* mUnit,
  ...
  for (auto it = annotations, end = annotations + count; it != end; ++it) {
    ...
    it->field_offset = resolved_field->GetOffset();
    it->declaring_dex_file = fields_class->GetDexCache()->GetDexFile();
    it->declaring_class_idx = fields_class->GetDexTypeIndex();
    it->declaring_field_idx = resolved_field->GetDexFieldIndex();
    if (UNLIKELY(referrer_class.get() == nullptr)) {
      continue;
    }
...
void CompilerDriver::ComputeStaticFieldAnnotations(const DexCompilationUnit* mUnit,
  ...
  for (auto it = annotations, end = annotations + count; it != end; ++it) {
    ...
    if (fields_class == referrer_class.get()) {
      ...
      // Leave the unused it->storage_index as -1 just as ComputeStaticFieldInfo() does.
      ++local;
      continue;
    }
    if (referrer_class->CanAccessResolvedField(fields_class, resolved_field,
