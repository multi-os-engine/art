...
bool CompilerDriver::ComputeInstanceFieldInfo(uint32_t field_idx, const DexCompilationUnit* mUnit,
  ...
  mirror::DexCache* dex_cache;
  {
    ...
    Handle<mirror::DexCache> dex_cache_sirt(
        hs.NewHandle(mUnit->GetClassLinker()->FindDexCache(*mUnit->GetDexFile())));
    Handle<mirror::ClassLoader> class_loader_sirt(
        hs.NewHandle(soa.Decode<mirror::ClassLoader*>(mUnit->GetClassLoader())));
    Handle<mirror::ArtField> resolved_field_sirt(hs.NewHandle(
...
bool CompilerDriver::ComputeInvokeInfo(const DexCompilationUnit* mUnit, const uint32_t dex_pc,
      ...
      soa.Decode<mirror::ClassLoader*>(mUnit->GetClassLoader())));
  {
    ...
    Handle<mirror::ArtMethod> resolved_method_sirt(hs.NewHandle(
  ...
  if (klass.Get() != nullptr && !SkipClass(jclass_loader, dex_file, klass.Get())) {
    ...
    if (klass->IsVerified()) {
      ...
      if (!klass->IsInitialized()) {
        ...
        Handle<mirror::Class> sirt_klass(hs.NewHandle(klass->GetClass()));
