...
void CompilerDriver::LoadImageClasses(TimingLogger* timings)
  ...
  for (auto it = image_classes_->begin(), end = image_classes_->end(); it != end;) {
    ...
    Handle<mirror::Class> klass(self, class_linker->FindSystemClass(self, descriptor.c_str()));
    if (klass.Get() == NULL) {
  ...
  Handle<mirror::Class> java_lang_Throwable(self,
  ...
  do {
    ...
    for (const std::pair<uint16_t, const DexFile*>& exception_type : unresolved_exception_types) {
      ...
      Handle<mirror::DexCache> dex_cache(self, class_linker->FindDexCache(*dex_file));
      Handle<mirror::ClassLoader> class_loader(self, nullptr);
      Handle<mirror::Class> klass(self, class_linker->ResolveType(*dex_file, exception_type_idx,
...
bool CompilerDriver::CanAssumeStringIsPresentInDexCache(const DexFile& dex_file,
  ...
  if (IsImage()) {
    ...
    ScopedObjectAccess soa(Thread::Current());
    Handle<mirror::DexCache> dex_cache(soa.Self(), Runtime::Current()->GetClassLinker()->FindDexCache(dex_file));
...
bool CompilerDriver::ComputeInstanceFieldInfo(uint32_t field_idx, const DexCompilationUnit* mUnit,
  ...
  {
    Handle<mirror::DexCache> dex_cache_sirt(soa.Self(),
        mUnit->GetClassLinker()->FindDexCache(*mUnit->GetDexFile()));
    Handle<mirror::ClassLoader> class_loader_sirt(soa.Self(),
        soa.Decode<mirror::ClassLoader*>(mUnit->GetClassLoader()));
    Handle<mirror::ArtField> resolved_field_sirt(soa.Self(),
...
bool CompilerDriver::ComputeInvokeInfo(const DexCompilationUnit* mUnit, const uint32_t dex_pc,
  ...
  mirror::Class* referrer_class;
  Handle<mirror::DexCache> dex_cache(soa.Self(),
      mUnit->GetClassLinker()->FindDexCache(*mUnit->GetDexFile()));
  Handle<mirror::ClassLoader> class_loader(soa.Self(),
      soa.Decode<mirror::ClassLoader*>(mUnit->GetClassLoader()));
  {
    uint32_t method_idx = target_method->dex_method_index;
    Handle<mirror::ArtMethod> resolved_method_sirt(soa.Self(),
...
static void ResolveClassFieldsAndMethods(const ParallelCompilationManager* manager,
  ...
  if (!SkipClass(class_linker, jclass_loader, dex_file, class_def)) {
    ...
    Handle<mirror::DexCache> dex_cache(soa.Self(), class_linker->FindDexCache(dex_file));
    // Resolve the class.
...
static void VerifyClass(const ParallelCompilationManager* manager, size_t class_def_index)
  ...
  Handle<mirror::Class> klass(soa.Self(), class_linker->FindClass(soa.Self(), descriptor,
                                                                   class_loader));
...
static void InitializeClass(const ParallelCompilationManager* manager, size_t class_def_index)
  ...
  Handle<mirror::ClassLoader> class_loader(soa.Self(),
                                            soa.Decode<mirror::ClassLoader*>(jclass_loader));
  Handle<mirror::Class> klass(soa.Self(),
  ...
  if (klass.Get() != nullptr && !SkipClass(jclass_loader, dex_file, klass.Get())) {
    ...
    if (klass->IsVerified()) {
      ...
      if (!klass->IsInitialized()) {
        ...
        Handle<mirror::Class> sirt_klass(soa.Self(), klass->GetClass());
