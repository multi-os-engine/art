...
CompilerDriver::CompilerDriver(const CompilerOptions* compiler_options,
  ...
  if (Runtime::Current()->GetProfilerOptions().IsEnabled() && !profile_file.empty()) {
...
bool CompilerDriver::SkipCompilation(const std::string& method_name) {
  ...
  bool compile = data.GetTopKUsedPercentage() - data.GetUsedPercent()
                 <= compiler_options_->GetTopKMethodsThreshold();
  if (compile) {
    LOG(INFO) << "compiling method " << method_name << " because its usage is part of top "
        ...
        << " (topKThreshold=" << compiler_options_->GetTopKMethodsThreshold() << ")";
  } else {
    VLOG(compiler) << "not compiling method " << method_name << " because it's not part of leading "
        << compiler_options_->GetTopKMethodsThreshold() << "% samples)";
  }
  return !compile;
}
}  // namespace art
