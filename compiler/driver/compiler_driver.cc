...
static void CheckAndClearClassNotFoundException(Thread* self)
    SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  CHECK(self->IsExceptionPending());
  mirror::Throwable* exception = self->GetException(nullptr);
  CHECK_EQ(exception->GetClass()->GetDescriptor(), "Ljava/lang/NoClassDefFoundError;");
  self->ClearException();
}
...
static void ResolveClassFieldsAndMethods(const ParallelCompilationManager* manager,
  ...
  if (!SkipClass(class_linker, jclass_loader, dex_file, class_def)) {
    ...
    if (klass == NULL) {
      ...
      CheckAndClearClassNotFoundException(soa.Self());
