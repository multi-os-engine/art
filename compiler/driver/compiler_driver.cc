...
class ClinitImageUpdateData {
 public:
  ...
  std::set<std::string>* image_class_descriptors_;
  std::vector<mirror::Class*> image_classes_;
  mirror::Class* art_method_class_;
  mirror::Class* dex_cache_class_;
};

static bool FindImageClasses(mirror::Class* klass, void* arg)
    SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  ClinitImageUpdateData* data = reinterpret_cast<ClinitImageUpdateData*>(arg);
  std::string temp;
  ...
  // Check whether it's one of the interesting classes.
  if (strcmp("Ljava/lang/reflect/ArtMethod;", name) == 0) {
    data->art_method_class_ = klass;
  } else if (strcmp("Ljava/lang/DexCache;", name) == 0) {
    data->dex_cache_class_ = klass;
  }
...
void CompilerDriver::UpdateImageClasses(TimingLogger* timings) {
  if (IsImage()) {
    ...
    ClinitImageUpdateData data;
    data.compiler_driver_ = this;
    data.image_class_descriptors_ = image_classes_.get();
    ...
    // Find all the already-marked classes, and interesting classes.
    Runtime::Current()->GetClassLinker()->VisitClasses(FindImageClasses, &data);
    CHECK(data.art_method_class_ != nullptr);
    CHECK(data.dex_cache_class_ != nullptr);

    // Use them as roots for a search.
    for (mirror::Class* klass_root : data.image_classes_) {
      data.VisitClinitClassesObject(klass_root);
    }
