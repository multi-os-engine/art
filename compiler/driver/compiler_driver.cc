...
static optimizer::DexToDexCompilationLevel GetDexToDexCompilationlevel(
  ...
  if (runtime->UseJit() || driver.GetCompilerOptions().VerifyAtRuntime()) {
    ...
    return optimizer::kDontDexToDexCompile;
  }
  const char* descriptor = dex_file.GetClassDescriptor(class_def);
...
// Helper class. This is a friend class of CompilerDriver and allows us to access private fields,
// but hide the DexToDexCompilationLevel internals.
class CompileMethodHelper {
 public:
  static void CompileMethod(Thread* self, CompilerDriver* driver,
    ...
    if ((access_flags & kAccNative) != 0) {
      ...
      if (!driver->compiler_options_->IsCompilationEnabled() &&
          InstructionSetHasGenericJniStub(driver->instruction_set_)) {
        // Leaving this empty will trigger the generic JNI version
      } else {
        compiled_method = driver->compiler_->JniCompile(access_flags, method_idx, dex_file);
        CHECK(compiled_method != nullptr);
      }
    } else if ((access_flags & kAccAbstract) != 0) {
      // Abstract methods don't have code.
    } else {
      bool has_verified_method = driver->verification_results_
          ->GetVerifiedMethod(method_ref) != nullptr;
      bool compile = compilation_enabled &&
                     // Basic checks, e.g., not <clinit>.
                     driver->verification_results_
      ...
      if (compile) {
        ...
        compiled_method = driver->compiler_->Compile(code_item, access_flags, invoke_type,
                                                     class_def_idx, method_idx, class_loader,
                                                     dex_file);
      }
      if (compiled_method == nullptr &&
          dex_to_dex_compilation_level != optimizer::kDontDexToDexCompile) {
        ...
        compiled_method = optimizer::ArtCompileDEX(
            ...
            class_loader,
            dex_file,
            has_verified_method ? dex_to_dex_compilation_level : optimizer::kRequired);
      }
    }
    if (kTimeCompileMethod) {
      uint64_t duration_ns = NanoTime() - start_ns;
      if (duration_ns > MsToNs(driver->compiler_->GetMaximumCompilationTimeBeforeWarning())) {
    ...
    if (compiled_method != nullptr) {
      ...
      DCHECK(driver->GetCompiledMethod(method_ref) == nullptr)
          << PrettyMethod(method_idx, dex_file);
      {
        MutexLock mu(self, driver->compiled_methods_lock_);
        driver->compiled_methods_.Put(method_ref, compiled_method);
        driver->non_relative_linker_patch_count_ += non_relative_linker_patch_count;
      }
      DCHECK(driver->GetCompiledMethod(method_ref) != nullptr)
          << PrettyMethod(method_idx, dex_file);
    }
    ...
    if (driver->compiler_kind_ != Compiler::kOptimizing) {
      driver->verification_results_->RemoveVerifiedMethod(method_ref);
    }
...
class CompileClassVisitor : public CompilationVisitor {
 public:
  ...
  virtual void Visit(size_t class_def_index) REQUIRES(!Locks::mutator_lock_) OVERRIDE {
    ...
    optimizer::DexToDexCompilationLevel dex_to_dex_compilation_level =
        optimizer::kDontDexToDexCompile;
    {
      ScopedObjectAccess soa(self);
      StackHandleScope<1> hs(soa.Self());
      Handle<mirror::ClassLoader> class_loader(
          hs.NewHandle(soa.Decode<mirror::ClassLoader*>(jclass_loader)));
      dex_to_dex_compilation_level = GetDexToDexCompilationlevel(
          soa.Self(), *driver, class_loader, dex_file, class_def);
    }
    ClassDataItemIterator it(dex_file, class_data);
    // Skip fields
    while (it.HasNextStaticField()) {
    ...
    while (it.HasNextDirectMethod()) {
      ...
      CompileMethodHelper::CompileMethod(self, driver,
                                         it.GetMethodCodeItem(), it.GetMethodAccessFlags(),
...
void CompilerDriver::CompileOne(Thread* self, ArtMethod* method, TimingLogger* timings) {
      ...
      optimizer::kDontDexToDexCompile;
  {
    ...
    Handle<mirror::ClassLoader> class_loader(
        hs.NewHandle(soa.Decode<mirror::ClassLoader*>(jclass_loader)));
    dex_to_dex_compilation_level = GetDexToDexCompilationlevel(self, *this, class_loader, *dex_file,
                                                               class_def);
  }
  CompileMethodHelper::CompileMethod(self, this, code_item, access_flags, invoke_type,
                                     class_def_idx, method_idx, jclass_loader, *dex_file,
                                     dex_to_dex_compilation_level, true);
