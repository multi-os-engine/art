        ...
        if (!klass->IsInitialized()) {
          ...
          if (can_init_static_fields) {
            ...
            if (strcmp("Ljava/lang/Void;", descriptor) == 0) {
              ...
              fields->Get(0)->SetObjNonTransactional(klass.get(),
              ...
              runtime->ActivatePreinitializationTransaction(&transaction);
              ...
              runtime->DeactivatePreinitializationTransaction();

              if (!success) {
                CHECK(soa.Self()->IsExceptionPending());
                ThrowLocation throw_location;
                mirror::Throwable* exception = soa.Self()->GetException(&throw_location);
                // TODO use VLOG(compiler)
                LOG(INFO) << "Failed to initialize " << ClassHelper(klass.get()).GetDescriptor();
                LOG(INFO) << "  Pending exception " << PrettyTypeOf(exception)
                          << " thrown by '" << throw_location.Dump() << "'";
                LOG(INFO) << "  " << exception->Dump();
                soa.Self()->ClearException();
                transaction.Abort();
