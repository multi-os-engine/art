...
void DefaultSelectivity::SwitchMethodLists(PassDriver<PassDriverMEOpts>* driver,
                                            CompilationUnit* cu) {
  if (cu->compiler_driver->GetCompilerOptions().IsSmallMethod(
        cu->mir_graph.get()->GetInsnsSizeInCodeUnits())) {
