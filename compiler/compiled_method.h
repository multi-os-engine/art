...
class SrcMap FINAL : public std::vector<SrcMapElem, Allocator> {
 public:
  ...
  SrcMap& Sort() {
    // Sort by "from" and then by "to"
    std::sort(begin(), end());
    // If two consequitive entries map to the same value, remove the later.
    // E.g. for map {{0, 1}, {4, 1}, {8, 2}}, all values in [0,8) map to 1.
    resize(std::unique(begin(), end(), [] (const SrcMapElem& lhs, const SrcMapElem& rhs) -> bool {
  ...
  const_iterator Find(uint32_t from) const {
...
class CompiledMethod FINAL : public CompiledCode {
 ...
 private:
  ...
  // For quick code, a set of pairs (PC, DEX) mapping from native PC offset to DEX offest
