...
class LinkerPatch {
 public:
  static LinkerPatch RecordPosition(size_t literal_offset) {
    return LinkerPatch(literal_offset, kLinkerPatchRecordPosition, nullptr);
  }
  ...
  uint32_t PcInsnOffset() const {
    DCHECK(patch_type_ == kLinkerPatchStringRelative ||
           patch_type_ == kLinkerPatchDexCacheArray);
