...
class LinkerPatch {
 public:
  static LinkerPatch MethodPatch(size_t literal_offset,
    ...
    LinkerPatch patch(literal_offset, kLinkerPatchMethod);
    patch.method_idx_ = target_method_idx;
    patch.target_dex_file_ = target_dex_file;
    return patch;
  }
  ...
  static LinkerPatch CodePatch(size_t literal_offset,
                               const DexFile* target_dex_file,
                               uint32_t target_method_idx) {
    LinkerPatch patch(literal_offset, kLinkerPatchCall);
    patch.method_idx_ = target_method_idx;
    patch.target_dex_file_ = target_dex_file;
    return patch;
  }
  ...
  static LinkerPatch RelativeCodePatch(size_t literal_offset,
                                       const DexFile* target_dex_file,
                                       uint32_t target_method_idx) {
    LinkerPatch patch(literal_offset, kLinkerPatchCallRelative);
    patch.method_idx_ = target_method_idx;
    patch.target_dex_file_ = target_dex_file;
    return patch;
  }
  ...
  static LinkerPatch TypePatch(size_t literal_offset,
    ...
    LinkerPatch patch(literal_offset, kLinkerPatchType);
  ...
  static LinkerPatch DexCacheArrayPatch(size_t literal_offset,
    ...
    LinkerPatch patch(literal_offset, kLinkerPatchDexCacheArray);
 ...
 private:
  ...
  union {
    ...
    size_t element_offset_;   // Element offset in the dex cache arrays.
    static_assert(sizeof(element_offset_) == sizeof(cmp2_), "needed by relational operators");
    const DexFile* target_dex_file_;
    static_assert(sizeof(target_dex_file_) == sizeof(cmp2_), "needed by relational operators");
  };
