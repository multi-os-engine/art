...
class ElfBuilder FINAL {
 public:
  ...
  class Section : public OutputStream {
   public:
    ...
    void Start() {
      ...
      if (!owner_->sections_.empty()) {
        if (header_.sh_flags != owner_->sections_.back()->header_.sh_flags) {
  ...
  // Zero means we omit program headers from the ELF file.
  void Start(size_t max_program_headers = 16) {
    int size = sizeof(Elf_Ehdr) + sizeof(Elf_Phdr) * max_program_headers;
    stream_.Seek(size, kSeekSet);
    started_ = true;
    virtual_address_ += size;
    max_program_headers_ = max_program_headers;
  }
  ...
  void End() {
    ...
    if (max_program_headers_ != 0) {
      phdrs = MakeProgramHeaders();
      CHECK_LE(phdrs.size(), max_program_headers_);
 ...
 private:
  ...
  // Allocated program header space at the start of ELF file.  May be zero.
  size_t max_program_headers_;
