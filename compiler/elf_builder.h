...
class ElfBuilder FINAL {
 public:
    ...
    AbiflagsSection(ElfBuilder<ElfTypes>* owner,
      ...
      if (isa == kMips || isa == kMips64) {
        ...
        if (isa == kMips) {
          ...
          const InstructionSetFeatures* features = InstructionSetFeatures::FromBitmap(isa, bit_map);
  ...
  ElfBuilder(InstructionSet isa, OutputStream* output, uint32_t bit_map)
      : isa_(isa),
        bit_map_(bit_map),
        stream_(output),
  ...
  void End() {
    ...
    // The main ELF header.
    Elf_Ehdr elf_header = MakeElfHeader(isa_, bit_map_);
 ...
 private:
  static Elf_Ehdr MakeElfHeader(InstructionSet isa, uint32_t bit_map) {
    ...
    switch (isa) {
      ...
      case kMips: {
        bool isR6 = InstructionSetFeatures::FromBitmap(isa, bit_map)->AsMipsInstructionSetFeatures()
            ->IsR6();
        ...
        elf_header.e_flags = (EF_MIPS_NOREORDER |
                              ...
                              EF_MIPS_CPIC      |
                              EF_MIPS_ABI_O32   |
                              isR6 ? EF_MIPS_ARCH_32R6 : EF_MIPS_ARCH_32R2);
        break;
      }
      case kMips64: {
        elf_header.e_machine = EM_MIPS;
