...
class ElfBuilder FINAL {
 public:
  ...
  class DynamicSection FINAL : public Section {
   public:
    ...
    virtual Elf_Word GetSize() const OVERRIDE {
      return (dynamics_.size() + 1 /* DT_NULL */) * sizeof(Elf_Dyn);
    }

    virtual bool Write(File* elf_file) OVERRIDE {
  ...
  // The write is done lazily using the provided lambda.
  class OatSection FINAL : public Section {
   public:
    OatSection(const std::string& name, Elf_Word type, Elf_Word flags,
               ...
               Elf_Word entsize, Elf_Word size, LazyWrite write)
        : Section(name, type, flags, link, info, align, entsize),
          size_(size), write_(write) {
    }
  ...
  class NoBitsSection FINAL : public Section {
   public:
    ...
    virtual bool Write(File* elf_file ATTRIBUTE_UNUSED) OVERRIDE {
  ...
  class SymtabSection FINAL : public Section {
   public:
    ...
    void AddSymbol(const std::string& name, const Section* section,
      ...
      symbols_.push_back(ElfSymbolState { name, section, addr, size,
          is_relative, MakeStInfo(binding, type), other, name_idx });
    }
  ...
  class HashSection FINAL : public Section {
   public:
    ...
    virtual bool Write(File* const elf_file) OVERRIDE {
  ...
  ElfBuilder(InstructionSet isa,
             Elf_Word rodata_size, LazyWrite write_rodata,
             Elf_Word text_size, LazyWrite write_text,
             Elf_Word bss_size)
    : isa_(isa),
      ...
      rodata_(".rodata", SHT_PROGBITS, SHF_ALLOC,
              nullptr, 0, kPageSize, 0, rodata_size, write_rodata),
      text_(".text", SHT_PROGBITS, SHF_ALLOC | SHF_EXECINSTR,
            nullptr, 0, kPageSize, 0, text_size, write_text),
      bss_(".bss", bss_size),
  ...
  bool Write(File* elf_file) {
    ...
    // This must be done after we have build all sections and know their size.
    Elf_Word file_offset = kProgramHeadersOffset + kProgramHeadersSize;
    Elf_Word load_address = file_offset;
    ...
    for (auto* section : sections) {
      ...
      int alignment = header->sh_addralign > 0 ? header->sh_addralign : 1;
    ...
    Elf_Word section_headers_offset = RoundUp(file_offset, sizeof(Elf_Word));
 ...
 private:
  static bool SeekTo(File* elf_file, Elf_Word offset) {
    Elf_Word current_offset = lseek(elf_file->Fd(), 0, SEEK_CUR);
    CHECK_LE(current_offset, offset) << "Seeking backwards";
