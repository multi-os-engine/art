...
// Writes ELF file.
//
// The basic layout of the elf file: (some sections are optional)
...
class ElfBuilder FINAL {
 public:
  const int kMaxProgramHeaders = 16;
   ...
   public:
    Section(ElfBuilder<ElfTypes>* owner, const std::string& name,
        ...
        : OutputStream(name), owner_(owner), header_(),
          ...
          started_(false), finished_(false), write_failed_(false) {
      DCHECK_GE(align, 1u);
    ...
    virtual void Start() {
      ...
      if (header_.sh_type != SHT_NOBITS) {
        if (!owner_->AlignOutput(header_.sh_addralign, &header_.sh_offset)) {
          PLOG(ERROR) << "Failed to seek when starting ELF section " << name_;
          write_failed_ = true;
        }
      }
    ...
    virtual void End() {
      ...
      if (header_.sh_type == SHT_NOBITS) {
        ...
        off_t file_offset = owner_->output_->Seek(0, kSeekCurrent);
        if (file_offset == (off_t)-1) {
          if (!write_failed_) {
            PLOG(ERROR) << "Failed to seek in ELF section " << name_;
            write_failed_ = true;
            return;
          }
        }
    ...
    bool WriteFully(const void* buffer, size_t byte_count) OVERRIDE {
      CHECK(started_);
      CHECK(!finished_);
      if (!owner_->output_->WriteFully(buffer, byte_count)) {
        if (!write_failed_) {
          PLOG(ERROR) << "Failed to write data in ELF section " << name_;
          write_failed_ = true;
        }
        return false;
      }
    ...
    off_t Seek(off_t offset, Whence whence) OVERRIDE {
      ...
      return owner_->output_->Seek(offset, whence);
    }
  ...
  class StringSection FINAL : public Section {
   public:
    ...
    void Start() OVERRIDE {
      Section::Start();
      char zero = 0;
      // The first entry must be empty string.
      this->WriteFully(&zero, 1);
      current_offset += 1;
    }

    Elf_Word Write(const std::string& name) {
      Elf_Word offset = current_offset;
      this->WriteFully(name.c_str(), name.length() + 1);
      current_offset += name.length() + 1;
      return offset;
    }

   private:
    Elf_Word current_offset;
  };
  ...
  class SymbolSection FINAL : public Section {
   public:
    SymbolSection(ElfBuilder<ElfTypes>* owner, const std::string& name,
      ...
      symbols_.push_back(Elf_Sym());  // NULL symbol
    }
  ...
  ElfBuilder(InstructionSet isa, OutputStream* output)
    : isa_(isa),
      output_(output),
      rodata_(this, ".rodata", SHT_PROGBITS, SHF_ALLOC,
              nullptr, 0, kPageSize, 0),
      text_(this, ".text", SHT_PROGBITS, SHF_ALLOC | SHF_EXECINSTR,
            nullptr, 0, kPageSize, 0),
      bss_(this, ".bss", SHT_NOBITS, SHF_ALLOC,
           nullptr, 0, kPageSize, 0),
      dynstr_(this, ".dynstr", SHF_ALLOC, kPageSize),
      dynsym_(this, ".dynsym", SHT_DYNSYM, SHF_ALLOC, &dynstr_),
      hash_(this, ".hash", SHT_HASH, SHF_ALLOC,
            &dynsym_, 0, sizeof(Elf_Word), sizeof(Elf_Word)),
      dynamic_(this, ".dynamic", SHT_DYNAMIC, SHF_ALLOC,
               &dynstr_, 0, kPageSize, sizeof(Elf_Dyn)),
      eh_frame_(this, ".eh_frame", SHT_PROGBITS, SHF_ALLOC,
                nullptr, 0, kPageSize, 0),
      eh_frame_hdr_(this, ".eh_frame_hdr", SHT_PROGBITS, SHF_ALLOC,
                    nullptr, 0, 4, 0),
      strtab_(this, ".strtab", 0, kPageSize),
      symtab_(this, ".symtab", SHT_SYMTAB, 0, &strtab_),
      debug_frame_(this, ".debug_frame", SHT_PROGBITS, 0,
                   nullptr, 0, GetInstructionSetPointerSize(isa), 0),
  ...
  void WriteOatPatchesSection(Section* patched_section,
                              const std::vector<uintptr_t>* patch_locations) {
    std::vector<uint8_t> buffer;
    EncodeOatPatches(*patch_locations, &buffer);
    Section* s = new Section(this, patched_section->name_ + ".oat_patches",
                             SHT_OAT_PATCH, 0, nullptr, 0, 1, 0);
    s->Start();
    s->WriteFully(buffer.data(), buffer.size());
    s->End();
    other_sections_.push_back(std::unique_ptr<Section>(s));
  }

  void WriteSection(const char* name, const std::vector<uint8_t>* buffer,
                    const std::vector<uintptr_t>* patch_locations) {
    Section* s = new Section(this, name, SHT_PROGBITS, 0, nullptr, 0, 1, 0);
    s->Start();
    s->WriteFully(buffer->data(), buffer->size());
    s->End();
    other_sections_.push_back(std::unique_ptr<Section>(s));
    if (patch_locations != nullptr) {
      WriteOatPatchesSection(s, patch_locations);
    }
  }
  ...
  bool Start() {
    ...
    if (output_->Seek(size, kSeekSet) == -1) {
      PLOG(ERROR) << "Failed to seek when starting ELF file";
      return false;
    }
  ...
  bool End() {
    ...
    // If any of the sections had I/O error, fail.
    for (auto* section : sections_) {
      if (section->write_failed_) {
        return false;
      }
    }
    ...
    if (!AlignOutput(sizeof(Elf_Off), &section_headers_offset)) {
      PLOG(ERROR) << "Failed to align ELF section headers";
      return false;
    }
    if (!output_->WriteFully(shdrs.data(), shdrs.size() * sizeof(shdrs[0]))) {
      PLOG(ERROR) << "Failed to write ELF section headers";
      return false;
    }
    ...
    if (output_->Seek(0, kSeekSet) == -1) {
      PLOG(ERROR) << "Failed to seek back to the start of ELF file";
      return false;
    }
    if (!output_->WriteFully(&elf_header, sizeof(elf_header))) {
      PLOG(ERROR) << "Failed to write ELF header";
      return false;
    }
    if (!output_->WriteFully(phdrs.data(), phdrs.size() * sizeof(phdrs[0]))) {
      PLOG(ERROR) << "Failed to write ELF program headers";
      return false;
    }
  ...
  void WriteDynamicSection(const std::string& elf_file_path) {
    ...
    dynstr_.Start();
    dynsym_.Add(dynstr_.Write("oatdata"), &rodata_,
                0, true, rodata_.GetSize(), STB_GLOBAL, STT_OBJECT);
    dynsym_.Add(dynstr_.Write("oatexec"), &text_,
                0, true, text_.GetSize(), STB_GLOBAL, STT_OBJECT);
    dynsym_.Add(dynstr_.Write("oatlastword"), &text_,
                text_.GetSize() - 4, true, 4, STB_GLOBAL, STT_OBJECT);
    if (bss_.finished_) {
    ...
    dynsym_.Start();
    dynsym_.End();  // Flush buffered symbols.
    ...
    // Buckets.  Having just one makes it liner search.
 ...
 private:
  bool AlignOutput(int alignment, Elf_Off* offset) {
    const off_t current = output_->Seek(0, kSeekCurrent);
    if (current == -1) {
      return false;
    }
    const off_t aligned = RoundUp(current, alignment);
    if (output_->Seek(aligned, kSeekSet) == -1) {
      return false;
    }
    *offset = dchecked_integral_cast<Elf_Off>(aligned);
    return true;
  }
  ...
  std::vector<Elf_Phdr> MakeProgramHeaders() {
    ...
    for (auto* section : sections_) {
      ...
      if ((shdr.sh_flags & SHF_ALLOC) != 0) {
        ...
        Elf_Word phdr_flags = (section == &text_    ? PF_R | PF_X :
                               section == &bss_     ? PF_R | PF_W :
                               section == &dynamic_ ? PF_R | PF_W :
                                                      PF_R);
        Elf_Phdr& prev = phdrs.back();
        Elf_Phdr load = Elf_Phdr();
        load.p_type   = PT_LOAD;
        load.p_flags  = phdr_flags;
        ...
        Elf_Word phdr_type = (section == &dynamic_      ? PT_DYNAMIC :
                              section == &eh_frame_hdr_ ? PT_GNU_EH_FRAME :
                                                         0);
        if (phdr_type != 0) {
          Elf_Phdr phdr = Elf_Phdr();
