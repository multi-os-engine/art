...
#ifndef ART_COMPILER_ELF_WRITER_QUICK_INL_H_
#define ART_COMPILER_ELF_WRITER_QUICK_INL_H_
...
class LineTableGenerator FINAL : public Leb128Encoder {
 public:
  LineTableGenerator(int line_base, int line_range, int opcode_base,
                     std::vector<uint8_t>* data, addr_type current_address,
                     size_t current_line)
  ...
  void PutDelta(addr_type delta_addr, int delta_line) {
  ...
  void SetAddr(addr_type addr) {
    ...
    PushByte(data_, 1 + sizeof(addr_type));  // length: opcode_size + address_size
    PushByte(data_, DW_LNE_set_address);
    if (sizeof(addr_type) == 8) {
      PushWord(data_, addr);
      PushWord(data_, addr >> 32);
    } else {
      PushWord(data_, addr);
    }
  }
...
void ElfWriterQuick<Elf_Word, Elf_Sword, Elf_Addr, Elf_Dyn,
  ...
  if (dbg_line == nullptr) {
    ...
    LineTableGenerator<uint32_t> line_table_generator(LINE_BASE, LINE_RANGE, OPCODE_BASE,
                                                      dbg_line, 0, 1);
...
#endif  // ART_COMPILER_ELF_WRITER_QUICK_INL_H_
