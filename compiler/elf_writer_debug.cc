    ...
    void Write(const CompilationUnit& compilation_unit) {
      ...
      for (auto mi : compilation_unit.methods_) {
        ...
        if (mi->code_item_ != nullptr) {
          local_infos.reserve(mi->code_item_->registers_size_);
          dex->DecodeDebugLocalInfo(mi->code_item_,
                                    is_static,
        ...
        for (const DexFile::LocalInfo& var : local_infos) {
          info_.StartTag(var.is_arg_ ? DW_TAG_formal_parameter : DW_TAG_variable);
          WriteName(var.name_);
          if (var.is_this_) {
 ...
 public:
  ...
  size_t WriteCompilationUnit(CompilationUnit& compilation_unit) {
    ...
    for (const MethodDebugInfo* mi : compilation_unit.methods_) {
      ...
      const DexFile* dex = mi->dex_file_;
      if (mi->code_item_ != nullptr) {
        dex->DecodeDebugPositionInfo(mi->code_item_, PositionInfoCallback, &position_infos);
      }
