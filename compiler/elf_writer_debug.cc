...
void WriteCFISection(ElfBuilder<ElfTypes>* builder,
                       ...
                       : builder->GetEhFrame());
  {
    ...
    const Elf_Addr text_address = builder->GetText()->WasWritten()
                                  ? builder->GetText()->GetAddress() : 0;
  ...
  class CompilationUnitWriter {
   public:
    ...
    void Write(const CompilationUnit& compilation_unit) {
      ...
      const Elf_Addr text_address = owner_->builder_->GetText()->WasWritten()
                                    ? owner_->builder_->GetText()->GetAddress() : 0;
      const size_t cu_size = compilation_unit.high_pc_ - compilation_unit.low_pc_;
...
class DebugLineWriter {
 ...
 public:
  ...
  size_t WriteCompilationUnit(CompilationUnit& compilation_unit) {
    ...
    const Elf_Addr text_address = builder_->GetText()->WasWritten()
                                  ? builder_->GetText()->GetAddress() : 0;
...
void WriteDebugSymbols(ElfBuilder<ElfTypes>* builder,
  ...
  for (const MethodDebugInfo& info : method_infos) {
    ...
    const auto* text = builder->GetText()->WasWritten() ? builder->GetText() : nullptr;
