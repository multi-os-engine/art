...
template <typename ElfTypes>
static ArrayRef<const uint8_t> WriteDebugElfFileForClassesInternal(const InstructionSet isa,
                                                                   mirror::Class** types,
                                                                   size_t count)
  ...
  info_writer.Start();
  info_writer.WriteTypes(ArrayRef<mirror::Class*>(types, count));
  info_writer.End();
...
ArrayRef<const uint8_t> WriteDebugElfFileForClasses(const InstructionSet isa,
                                                    ...
                                                    size_t count) {
  if (Is64BitInstructionSet(isa)) {
    return WriteDebugElfFileForClassesInternal<ElfTypes64>(isa, types, count);
  } else {
    return WriteDebugElfFileForClassesInternal<ElfTypes32>(isa, types, count);
  }
}
