  ...
  class CompilationUnitWriter {
   public:
    ...
    void Write(const CompilationUnit& compilation_unit) {
      ...
      // DW_AT_high_pc can be size since DWARF4.
    ...
    void WriteRegLocation(const OatWriter::DebugInfo* method_info, uint16_t vreg,
      ...
      for (uint32_t s = 0; s < code_info.GetNumberOfStackMaps(); s++) {
        ...
        // Determine whether the location can be descried in single DWARF piece.
        // This is not possible if the value is split to two distinct locations.
        // (e.g. 64-bit value stored in two 32-bit machine registers)
        bool one_piece;
        if (is64bitValue) {
          switch (reg_lo.GetKind()) {
            case Kind::kInStack:
              one_piece = (reg_hi.GetKind() == Kind::kInStack &&
                  reg_hi.GetValue() == reg_lo.GetValue() + 4);
              break;
            case Kind::kInRegister:
              one_piece = (reg_hi.GetKind() == Kind::kInRegisterHigh &&
                  reg_hi.GetValue() == reg_lo.GetValue());
              break;
            case Kind::kInFpuRegister:
              one_piece = (reg_hi.GetKind() == Kind::kInFpuRegisterHigh &&
                  reg_hi.GetValue() == reg_lo.GetValue());
              break;
            case Kind::kNone:
              one_piece = (reg_hi.GetKind() == Kind::kNone);
              break;
            default:
              one_piece = false;
              break;
        ...
        uint8_t buffer[64];
        uint8_t* pos = buffer;
        for (int piece = 0; piece < (one_piece ? 1 : 2); piece++) {
          ...
          switch (kind) {
            ...
            case Kind::kInRegister: {
              int reg_num = GetDwarfCoreReg(isa, value).num();
              if (reg_num < 32) {
                *(pos++) = DW_OP_reg0 + reg_num;
              } else {
                *(pos++) = DW_OP_regx;
                pos = EncodeUnsignedLeb128(pos, reg_num);
              }
              break;
            }
            case Kind::kInFpuRegister: {
              int reg_num = GetDwarfFpReg(isa, value).num();
              if (reg_num < 32) {
                *(pos++) = DW_OP_reg0 + reg_num;
              } else {
                *(pos++) = DW_OP_regx;
            ...
            case Kind::kNone:
              break;
            default:
              // Cases kInStackLargeOffset and kConstantLargeValue
              // should have been hidden by the GetKind() call.
              // Cases kInRegisterHigh and kInFpuRegisterHigh are handled
              // if they just extend the low register to full 64 bits.
