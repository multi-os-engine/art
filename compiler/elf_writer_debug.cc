  ...
  switch (isa) {
    ...
    case kThumb2: {
      ...
      WriteDebugFrameCIE(is64bit, addr_type, return_reg,
                         opcodes, use_eh_frame_format, eh_frame);
      return;
    }
    case kArm64: {
      DebugFrameOpCodeWriter<> opcodes;
...
void WriteDebugFrame(const CompilerDriver* compiler,
  ...
  WriteDebugFrameCIE(isa, address_type, use_eh_frame_format, debug_frame);
    ...
    if (!mi.deduped_) {  // Only one FDE per unique address.
      ...
      if (opcodes != nullptr) {
        ...
        WriteDebugFrameFDE(Is64BitInstructionSet(isa), cie_offset,
                           ...
                           opcodes, use_eh_frame_format,
                           debug_frame, debug_frame_patches);
      }
    }
  }
