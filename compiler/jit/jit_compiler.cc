...
uint8_t* JitCompiler::WriteMethodHeaderAndCode(const CompiledMethod* compiled_method,
  ...
  LOG(INFO) << reinterpret_cast<const void*>(reserve_begin) << " " << reinterpret_cast<const void*>(reserve_end) << " " << reinterpret_cast<const void*>(quick_code);
...
bool JitCompiler::AddToCodeCache(mirror::ArtMethod* method, const CompiledMethod* compiled_method,
  ...
  LOG(INFO) << reinterpret_cast<void*>(code_reserve) << " " << reserve_size;
  // Write out the code.
  uint8_t* code_ptr;
  ...
  *out_method = OatFile::OatMethod(base, code_offset);
  ...
  LOG(INFO) << "Added " << PrettyMethod(method) << "@" << method << " ccache size="
