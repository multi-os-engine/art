...
class OatWriter {
 public:
  explicit OatWriter(bool compiling_boot_image);
  ...
  //   - WriteOatDexFile() for each dex file [*],
  //   - PrepareLayout(),
  //   - WriteRodata(),
  //   - WriteCode(),
  //   - WriteHeader().
  //
  // [*] in the same order as their locations passed to ReserveOatDexFile().

  // Reserve space for the OatHeader and in the .rodata section.
  void ReserveHeaders(InstructionSet instruction_set,
  ...
  // Write an OatDexFile for a dex file.
  bool WriteOatDexFile(OutputStream* rodata, const DexFile& dex_file);
  // Prepare layout of remaining data.
  void PrepareLayout(const std::vector<const DexFile*>& dex_files,
 ...
 private:
  ...
  size_t InitOatHeader(InstructionSet instruction_set,
                       ...
                       int32_t image_patch_delta,
                       SafeMap<std::string, std::string>* key_value_store);
  size_t InitOatDexFiles(size_t offset, const ArrayRef<const std::string>& dex_file_locations);
  ...
  class OatDexFile {
   public:
    OatDexFile(size_t offset, const std::string& dex_file_location);
    ...
    // const uint8_t* dex_file_location_data_;  // We do not keep this around.
