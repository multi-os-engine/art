...
class OatWriter {
 ...
 private:
  // The MapBinder classes are helper classes that provide access to members related to
  ...
  // The function ProcessDexMethods() below iterates through all the methods in all
  // the compiled dex files in order of their definitions. The method processor
  ...
  // Process all the methods in all the compiled dex files in their definition order
  // with a given DexMethodProcessor.
  ...
  class OatClass {
   public:
    ...
    // The number on non-NULL compiled methods.
    size_t num_non_null_compiled_methods_;

    // CompiledMethod* for each class_def_method_index, or NULL if no method is available.
