...
static const XRegister kCoreArgumentRegisters[] = {
      ...
      if (IsCurrentParamAFloatOrDouble()) {  // FP regs.
        ...
        if (gp_reg_index < 8) {
          if (IsCurrentParamALong() && (!IsCurrentParamAReference())) {
            entry_spills_.push_back(Arm64ManagedRegister::FromXRegister(kCoreArgumentRegisters[gp_reg_index]));
          } else {
...
ManagedRegister Arm64JniCallingConvention::CurrentParamRegister() {
  ...
  if (IsCurrentParamAFloatOrDouble()) {
    ...
    if (IsCurrentParamALong() || IsCurrentParamAReference() || IsCurrentParamJniEnv())  {
      return Arm64ManagedRegister::FromXRegister(kCoreArgumentRegisters[gp_reg]);
    } else {
