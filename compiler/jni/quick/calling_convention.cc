...
FrameOffset JniCallingConvention::SavedLocalReferenceCookieOffset() const {
  size_t start_of_sirt = SirtNumRefsOffset().Int32Value() +  kSirtPointerSize;
...
FrameOffset JniCallingConvention::CurrentParamSirtEntryOffset() {
  ...
  int result = SirtNumRefsOffset().Int32Value() + kSirtPointerSize;
  result += itr_refs_ * kSirtPointerSize;
