...
class ImageWriter FINAL {
 public:
  ...
  PtrType GetDexCacheArrayElementImageAddress(const DexFile* dex_file, uint32_t offset)
    ...
    return reinterpret_cast<PtrType>(
        image_begin_ + RoundUp(sizeof(ImageHeader), kObjectAlignment) +
        bin_slot_previous_sizes_[kBinDexCacheArray] + it->second + offset);
  }
 ...
 private:
  ...
  enum Bin {
    ...
    // we arbitrarily keep them at the end.
  ...
  void AddDexCacheArrayReloc(void* array, size_t offset) SHARED_REQUIRES(Locks::mutator_lock_);
  ...
  template <typename T>
  uintptr_t GetNativeObjectImageOffset(T* orig, Bin bin) SHARED_REQUIRES(Locks::mutator_lock_);
  template <typename T>
  T* GetNativeObjectImageAddress(T* orig, Bin bin) SHARED_REQUIRES(Locks::mutator_lock_);
  ...
  void FixupObject(mirror::Object* orig, mirror::Object* copy)
      SHARED_REQUIRES(Locks::mutator_lock_);
  void FixupDexCacheArrays() SHARED_REQUIRES(Locks::mutator_lock_);
