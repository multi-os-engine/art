...
class ImageWriter FINAL {
 public:
  ...
  uint8_t* GetOatFileBegin() const {
    return image_begin_ + RoundUp(
        image_end_ + bin_slot_sizes_[kBinArtField] + bin_slot_sizes_[kBinArtMethod], kPageSize);
  }
 ...
 private:
  ...
  // Special image offset for already copied objects. This is useful for copying vtable arrays in
  // classes since we don't want to overwrite the updated pointers.
  static const size_t kImageOffsetAlreadyCopied = 0x1;
  static_assert(kImageOffsetAlreadyCopied < sizeof(ImageHeader), "conflict");
  ...
  void AssignMethodOffset(ArtMethod* method) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);
  ...
  ArtMethod* image_methods_[ImageHeader::kImageMethodCount];
