...
void IFieldLoweringInfo::Resolve(CompilerDriver* compiler_driver, const DexCompilationUnit* mUnit,
  ...
  if (kIsDebugBuild) {
    ...
    for (auto it = field_infos, end = field_infos + count; it != end; ++it) {
      IFieldLoweringInfo unresolved(it->field_idx_);
...
void SFieldLoweringInfo::Resolve(CompilerDriver* compiler_driver, const DexCompilationUnit* mUnit,
                               SFieldLoweringInfo* field_infos, size_t count) {
  if (kIsDebugBuild) {
    DCHECK(field_infos != nullptr);
    DCHECK_NE(count, 0u);
    for (auto it = field_infos, end = field_infos + count; it != end; ++it) {
      SFieldLoweringInfo unresolved(it->field_idx_);
      DCHECK_EQ(memcmp(&unresolved, &*it, sizeof(*it)), 0);
    }
  }
