...
class Expression: public ArenaObject {
 public:
  ...
  virtual std::string ToString(const CompilationUnit * c_unit) const = 0;

  virtual void GetChildren(std::vector<const Expression*>& children) const {
  }
...
class VirtualRegister: public Expression {
 public:
   ...
   */
  bool IsVirtualRegister() const OVERRIDE FINAL {
    return true;
  }
  ...
  std::string ToString(const CompilationUnit * c_unit) const OVERRIDE;
 ...
 protected:
  const int low_ssa_reg_;
  const int high_ssa_reg_;
  const bool wide_;
...
class BytecodeExpression: public Expression {
 public:
   ...
   */
  bool IsBytecodeExpression() const OVERRIDE FINAL {
    return true;
  }

  const VirtualRegister* GetAssignmentTo() const {
   ...
   */
  ExpressionKind GetExpressionKind() const {
    return exp_kind_;
  }
 ...
 protected:
   ...
   */
  const VirtualRegister* assignment_to_;

  /**
   * @brief Associated MIR.
   */
  const MIR* const mir_;

  /**
   * @brief Keeps track of expression kind of this expression.
   */
  const ExpressionKind exp_kind_;
   ...
   */
  const ExpressionType exp_type_;
...
class BinaryExpression: public BytecodeExpression {
 public:
  ...
  bool EvaluatesToConstant() const OVERRIDE FINAL {
    return lhs_->EvaluatesToConstant() && rhs_->EvaluatesToConstant();
  }

  void GetChildren(std::vector<const Expression*>& children) const OVERRIDE FINAL;
...
class ConstantExpression: public Expression {
 public:
  ...
  desired_type GetValue() const {
    // We need to break strict aliasing rules so we quiet down compiler
    // by using char* first.
    const char *ptr = reinterpret_cast<const char *>(&value_);
