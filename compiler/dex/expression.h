...
class ConstantExpression: public Expression {
 public:
  explicit ConstantExpression(int32_t constant)
      : wide_(false) {
    value_ = static_cast<int64_t>(constant);
  }

  ConstantExpression(int32_t lowConstant, int32_t highConstant)
      : wide_(true) {
    value_ = (static_cast<int64_t>(highConstant) << 32) | (static_cast<int64_t>(lowConstant));
  }
  ...
  explicit ConstantExpression(int64_t constant)
      : wide_(true) {
    value_ = constant;
  }
   ...
   * @param low_initial_value The constant value of low bits.
   * @param high_initial_value The constant value of high bits(wide must be true)
   * @param wide Whether we are creating a wide(64-bit) constant.
   * @return Returns the constant expression instance created.
   */
  static ConstantExpression* NewExpression(ArenaAllocator* arena, int32_t low_initial_value,
                                           int32_t high_initial_value = 0, bool wide = false);
