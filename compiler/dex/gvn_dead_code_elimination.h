...
class GvnDeadCodeElimination : public DeletableArenaObject<kArenaAllocMisc> {
 ...
 private:
  ...
  static constexpr size_t kMaxNumTopChangesToRevert = 2;
  ...
  class VRegChains {
   public:
    ...
    uint16_t FindRevertHead(int v_reg, uint16_t cutoff);
  ...
  uint16_t* revert_heads_;  // For each vreg in revert_vregs_, the first change to kill.
