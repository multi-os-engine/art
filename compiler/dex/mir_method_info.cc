  ...
  for (auto it = method_infos, end = method_infos + count; it != end; ++it) {
    ...
    if (!it->IsQuickened()) {
      ...
      /* if (resolved_method == nullptr) {
        // If the method is null then it was probably a miranda method, in this case try
        // re-loading it, this time as an interface method.
        ...
        CHECK(resolved_method != nullptr);
        CHECK(!resolved_method->IsMiranda());
      } */
      if (resolved_method != nullptr) {
