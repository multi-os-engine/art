...
void MirMethodLoweringInfo::Resolve(CompilerDriver* compiler_driver,
  ...
  for (auto it = method_infos, end = method_infos + count; it != end; ++it) {
    ...
    if (!it->IsQuickened()) {
      ...
      const auto* dequicken_ref = verified_method->GetDequickenIndex(it->MethodIndex());
      CHECK(dequicken_ref != nullptr);
      it->method_idx_ = dequicken_ref->dex_method_index;
      it->target_dex_file_ = dequicken_ref->dex_file;
      it->target_method_idx_ = dequicken_ref->dex_method_index;
      CHECK(use_jit);
      ...
      current_dex_cache.Assign(cl->FindDexCache(*it->target_dex_file_));
      CHECK(current_dex_cache.Get() != nullptr);
      resolved_method = cl->ResolveMethod(
          *it->target_dex_file_, it->MethodIndex(), current_dex_cache, class_loader,
