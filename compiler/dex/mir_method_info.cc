...
void MirMethodLoweringInfo::Resolve(CompilerDriver* compiler_driver,
  ...
  for (auto it = method_infos, end = method_infos + count; it != end; ++it) {
    ...
    std::pair<bool, bool> is_intrinsic_or_special(false, false);
    if (inliner_map != nullptr) {
      auto* inliner = (it->declaring_dex_file_ == dex_file)
          ? default_inliner
          : inliner_map->GetMethodInliner(it->declaring_dex_file_);
      is_intrinsic_or_special = inliner->IsIntrinsicOrSpecial(it->declaring_method_idx_);
    }
    ...
    it->flags_ = other_flags |
        ...
        (is_intrinsic_or_special.first ? kFlagIsIntrinsic : 0u) |
        (is_intrinsic_or_special.second ? kFlagIsSpecial : 0u) |
