...
const GlobalValueNumberingTest::BBDef GlobalValueNumberingTestCatch::kCatchBbs[] = {
    ...
    DEF_BB(kDalvikByteCode, DEF_SUCC1(6), DEF_PRED1(3)),     // The throwing isns.
...
TEST_F(GlobalValueNumberingTestCatch, IFields) {
  ...
  static const MIRDef mirs[] = {
      DEF_UNIQUE_REF(3, Instruction::NEW_INSTANCE, 102u),
      DEF_IGET(4, Instruction::IGET, 1u, 100u, 0u),
      DEF_IPUT(4, Instruction::IPUT, 1u, 100u, 1u),
      DEF_IPUT(4, Instruction::IPUT, 1u, 101u, 0u),
      DEF_IPUT(4, Instruction::IPUT, 1u, 102u, 0u),
      DEF_IGET(5, Instruction::IGET, 5u, 100u, 0u),         // Differs from the top.
      DEF_IPUT(5, Instruction::IPUT, 5u, 100u, 1u),
      DEF_IPUT(5, Instruction::IPUT, 5u, 101u, 0u),
      DEF_IPUT(5, Instruction::IPUT, 5u, 102u, 0u),
      DEF_IGET(6, Instruction::IGET, 9u, 100u, 0u),         // Differs from both IGETs above.
      DEF_IGET(6, Instruction::IGET, 10u, 100u, 1u),        // Same as 9u.
      DEF_IGET(6, Instruction::IGET, 11u, 101u, 0u),        // Same as 9u.
      DEF_IGET(6, Instruction::IGET, 12u, 102u, 0u),        // Same as 9u.
  };
  ...
  EXPECT_NE(value_names_[1], value_names_[9]);
  EXPECT_NE(value_names_[5], value_names_[9]);
  EXPECT_EQ(value_names_[9], value_names_[10]);
  EXPECT_EQ(value_names_[9], value_names_[11]);
  EXPECT_EQ(value_names_[9], value_names_[12]);
}
...
TEST_F(GlobalValueNumberingTestCatch, Arrays) {
  static const MIRDef mirs[] = {
      DEF_UNIQUE_REF(3, Instruction::NEW_ARRAY, 101u),
      DEF_AGET(3, Instruction::AGET, 1u, 100u, 102u),
      DEF_APUT(3, Instruction::APUT, 1u, 100u, 103u),
      DEF_APUT(3, Instruction::APUT, 1u, 101u, 102u),
      DEF_APUT(3, Instruction::APUT, 1u, 101u, 103u),
      DEF_AGET(4, Instruction::AGET, 5u, 100u, 102u),       // Differs from the top.
      DEF_APUT(4, Instruction::APUT, 5u, 100u, 103u),
      DEF_APUT(4, Instruction::APUT, 5u, 101u, 102u),
      DEF_APUT(4, Instruction::APUT, 5u, 101u, 103u),
      DEF_AGET(5, Instruction::AGET, 9u, 100u, 102u),       // Differs from both IGETs above.
      DEF_AGET(5, Instruction::AGET, 10u, 100u, 103u),      // Same as 100u/102u.
      DEF_AGET(5, Instruction::AGET, 11u, 101u, 102u),      // Same as 100u/102u.
      DEF_AGET(5, Instruction::AGET, 12u, 101u, 103u),      // Same as 100u/102u.
  };
...
TEST_F(GlobalValueNumberingTestCatch, Phi) {
  static const MIRDef mirs[] = {
      ...
      DEF_CONST(4, Instruction::CONST, 3u, 1000),
      DEF_CONST(4, Instruction::CONST, 4u, 3000),
      DEF_MOVE(4, Instruction::MOVE, 5u, 4u),
      DEF_PHI2(5, 6u, 0u, 3u),
      DEF_PHI2(5, 7u, 0u, 4u),
      DEF_PHI2(5, 8u, 0u, 5u),
      DEF_PHI2(5, 9u, 1u, 3u),
      DEF_PHI2(5, 10u, 1u, 4u),
      DEF_PHI2(5, 11u, 1u, 5u),
      DEF_PHI2(5, 12u, 2u, 3u),
      DEF_PHI2(5, 13u, 2u, 4u),
      DEF_PHI2(5, 14u, 2u, 5u),
  };
  PrepareMIRs(mirs);
  PerformGVN();
  ASSERT_EQ(arraysize(mirs), value_names_.size());
  ASSERT_EQ(value_names_[3], value_names_[0]);  // Both CONSTs are 1000.
  EXPECT_EQ(value_names_[6], value_names_[0]);  // Merging CONST 0u and CONST 3u, both 1000.
  EXPECT_NE(value_names_[7], value_names_[0]);
  EXPECT_NE(value_names_[7], value_names_[4]);
  EXPECT_EQ(value_names_[8], value_names_[7]);
  EXPECT_NE(value_names_[9], value_names_[1]);
  EXPECT_NE(value_names_[9], value_names_[3]);
  EXPECT_NE(value_names_[9], value_names_[7]);
  EXPECT_NE(value_names_[10], value_names_[1]);
  EXPECT_NE(value_names_[10], value_names_[4]);
  EXPECT_NE(value_names_[10], value_names_[7]);
  EXPECT_NE(value_names_[10], value_names_[9]);
  EXPECT_EQ(value_names_[11], value_names_[10]);
  EXPECT_EQ(value_names_[12], value_names_[9]);
  EXPECT_EQ(value_names_[13], value_names_[10]);
  EXPECT_EQ(value_names_[14], value_names_[10]);
}
...
TEST_F(GlobalValueNumberingTest, Infinite) {
  // This is a pattern that lead to an infinite loop during the GVN development. The first time
  // the Phi evaluates to the same value name as CONST 0u.  After second evaluation, when the
  // BB #9 has been processed, the Phi receives its own value name. However, the index from the
  // first evaluation kept disappearing an reappearing in the LVN's aliasing_array_value_map_'s
  // load_value_map for BBs #9, #4, #5, #7 because of the DFS ordering of LVN evaluation.
  // TODO: Solve this issue. (This test is currently failing.)
  static const IFieldDef ifields[] = {
      { 0u, 1u, 0u, false },  // Object.
  };
  static const BBDef bbs[] = {
