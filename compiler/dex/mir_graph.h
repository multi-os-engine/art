...
enum DataFlowAttributePos {
  ...
  /**
   * @brief Used when semantics of instruction ensure clinit for a class is done.
   ...
   *
   * Note that this DF flag does not necessarily mean that clinit must be done
   * by this instruction. Some clinits are guaranteed to be done such as when
   * referrer class is same as method's class for static invokes. Also, if
   * instruction is marked with MIR_IGNORE_CLINIT_CHECK it means it has been
   * proven that before reaching this instruction, the clinit is guaranteed
   * to have been done for class.
   */
  kClInit,
};
...
class MIRGraph {
 public:
  ...
  bool HasClassInitializerInstructions() const {
