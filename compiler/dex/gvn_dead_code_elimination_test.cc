...
#define DEF_BINOP(bb, opcode, result, src1, src2) \
  ...
  void DoPrepareMIRs(const MIRDef* defs, size_t count) {
    ...
    for (size_t i = 0u; i != count; ++i) {
      ...
      mir->ssa_rep->uses = static_cast<int32_t*>(
          cu_.arena.Alloc(sizeof(int32_t) * def->num_uses, kArenaAllocDFInfo));
      std::copy_n(def->uses, def->num_uses, mir->ssa_rep->uses);
      mir->ssa_rep->fp_use = nullptr;  // Not used by LVN/DCE.
      mir->ssa_rep->num_defs = def->num_defs;
      mir->ssa_rep->defs = static_cast<int32_t*>(
          cu_.arena.Alloc(sizeof(int32_t) * def->num_defs, kArenaAllocDFInfo));
      std::copy_n(def->defs, def->num_defs, mir->ssa_rep->defs);
      mir->ssa_rep->fp_def = nullptr;  // Not used by LVN/DCE.
      mir->dalvikInsn.opcode = def->opcode;
...
TEST_F(GvnDeadCodeEliminationTestSimple, KillChain4) {
  static const IFieldDef ifields[] = {
      { 0u, 1u, 0u, false, kDexMemAccessWord },
  };
  static const MIRDef mirs[] = {
...
TEST_F(GvnDeadCodeEliminationTestSimple, KillChain5) {
  static const IFieldDef ifields[] = {
      { 0u, 1u, 0u, false, kDexMemAccessWord },
  };
  static const MIRDef mirs[] = {
...
TEST_F(GvnDeadCodeEliminationTestSimple, KeepChain1) {
  // KillChain4 without the final CONST.
  static const IFieldDef ifields[] = {
      { 0u, 1u, 0u, false, kDexMemAccessWord },
  };
  static const MIRDef mirs[] = {
...
TEST_F(GvnDeadCodeEliminationTestSimple, KeepChain2) {
  // KillChain2 with MIRs in the middle of the chain.
  static const IFieldDef ifields[] = {
      { 0u, 1u, 0u, false, kDexMemAccessWord },
  };
  static const MIRDef mirs[] = {
