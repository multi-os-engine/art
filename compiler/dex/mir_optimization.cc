...
bool MIRGraph::EliminateClassInitChecksGate() {
  ...
  uint32_t unique_class_count = 0u;
  {
    ...
    std::vector<MIR*, ScopedArenaAllocatorAdapter<MIR*> > static_invokes(allocator.Adapter());
    ...
    for (BasicBlock* bb = iter.Next(); bb != nullptr; bb = iter.Next()) {
      for (MIR* mir = bb->first_mir_insn; mir != nullptr; mir = mir->next) {
        ...
        if (mir->dalvikInsn.opcode >= Instruction::SGET &&
          ...
          static_invokes.push_back(mir);
        }
      }
    }
    unique_class_count = static_cast<uint32_t>(class_to_index_map.size());

    // Now go through all INVOKE_STATICs that may help avoid the checks.
    if (unique_class_count != 0u) {
      for (MIR* mir : static_invokes) {
        uint16_t index = 0xffffu;
        const MirMethodInfo& method_info = GetMethodLoweringInfo(mir);
        if (method_info.IsResolved()) {
          MapEntry entry = {
              method_info.DeclaringDexFile(),
              method_info.DeclaringClassIndex(),
              0u  // Not used.
          };
          auto it = class_to_index_map.find(entry);
          if (it != class_to_index_map.end()) {
            index = it->index;
          }
        }
        // Using offset/2 for index into temp_insn_data_.
