...
CompilerTemp* MIRGraph::GetNewCompilerTemp(CompilerTempType ct_type, bool wide) {
  ...
  if (ct_type == kCompilerTempSpecialMethodPtr) {
    ...
    compiler_temp->s_reg_low = AddNewSReg(StackVisitor::GetMethodPtrTempBaseReg());
    ...
    int32_t v_reg = StackVisitor::GetCompilerTempNonSpecialBaseReg() - num_non_special_compiler_temps_;
    ...
    if (wide) {
      ...
      // negative fashion, we need the smaller to refer to the low part. Thus, we redefine the s_reg_low.
      compiler_temp->s_reg_low = AddNewSReg(v_reg - 1);
      num_non_special_compiler_temps_++;
    }
  }
