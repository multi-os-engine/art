...
/* TODO: Find a way to traverse control flow diamonds when searching for String.<init> invoke.*/
MIR* MIRGraph::AdvanceMIR2(BasicBlock** p_bb, MIR* mir) {
  BasicBlock* bb = *p_bb;
  if (mir != NULL) {
    mir = mir->next;
    if (mir == NULL) {
...
bool MIRGraph::BasicBlockOpt(BasicBlock* bb) {
  ...
  while (bb != NULL) {
    for (MIR* mir = bb->first_mir_insn; mir != NULL; mir = mir->next) {
        ...
        case Instruction::NEW_INSTANCE: {
          ...
          if (PrettyType(type_idx, *cu_->dex_file) == "java.lang.String") {
            ...
            if (string_mir->dalvikInsn.opcode == Instruction::INVOKE_DIRECT) {
              ...
              if (cu_->verbose) {
                LOG(INFO) << "  " << string_mir->dalvikInsn.opcode
                          << " - vA:" << string_mir->dalvikInsn.vA
                          << " vB:" << std::hex << string_mir->dalvikInsn.vB << std::dec
                          << " arg[0]:" << string_mir->dalvikInsn.arg[0]
                          << " arg[1]:" << string_mir->dalvikInsn.arg[1]
                          << " arg[2]:" << string_mir->dalvikInsn.arg[2]
                          << " arg[3]:" << string_mir->dalvikInsn.arg[3];
              }
            } else {
            ...
            if (string_bb->predecessors->Size() == 1) {
              MIR* check_mir = GetBasicBlock(string_bb->predecessors->Get(0))->last_mir_insn;
      ...
      if (cu_->verbose) {
        if (static_cast<uint32_t>(mir->dalvikInsn.opcode) < kNumPackedOpcodes) {
          LOG(INFO) << " # " << mir->dalvikInsn.opcode << " " << std::hex << mir->dalvikInsn.vA
                    << " " << mir->dalvikInsn.vB << " " << mir->offset;
        } else {
          LOG(INFO) << " # " << static_cast<uint32_t>(mir->dalvikInsn.opcode) << " " << std::hex
                    << mir->dalvikInsn.vA<< " " << mir->dalvikInsn.vB << " " << mir->offset;
        }
      }
    }
