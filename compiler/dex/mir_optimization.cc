...
void MIRGraph::CombineBlocksEnd() {
  // If we eliminated any basic blocks, we need to recalculate DFS orders.
...
bool MIRGraph::EliminateNullChecks(BasicBlock* bb) {
  ...
  return DoEliminateNullChecks(bb, false);
}

bool MIRGraph::DoEliminateNullChecks(BasicBlock* bb, bool modify) {
...
void MIRGraph::EliminateNullChecksEnd() {
  // Now that the ending check flags have converged, update the MIRs.
  PreOrderDfsIterator iter(this);
  for (BasicBlock* bb = iter.Next(); bb != nullptr; bb = iter.Next()) {
    if (bb->block_type == kDalvikByteCode || bb->block_type == kEntryBlock) {
      bool change = DoEliminateNullChecks(bb, true);
