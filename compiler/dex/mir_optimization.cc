...
void MIRGraph::DoInlineCalls(BasicBlock* bb) {
  ...
  for (MIR* mir = bb->first_mir_insn; mir != NULL; mir = mir->next) {
    ...
    const MethodAnnotation& annotation = GetMethodAnnotation(mir);
    if (annotation.fast_path &&
        (annotation.sharp_type == kDirect || annotation.sharp_type == kStatic) &&
        annotation.method_idx >= 4096) {
      DCHECK(cu_->compiler_driver->GetMethodInlinerMap() != nullptr);
      cu_->compiler_driver->GetMethodInlinerMap()->GetMethodInliner(cu_->dex_file)
          ->GenInline(this, bb, mir, annotation.method_idx);
    }
  }
}
