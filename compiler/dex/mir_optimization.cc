    ...
    for (MIR* mir = bb->first_mir_insn; mir != nullptr; mir = mir->next) {
      ...
      if (opcode == Instruction::NEW_INSTANCE) {
        ...
        if (PrettyType(type_idx, *cu_->dex_file) == "java.lang.String") {
          bool debug = false;
          debug = (PrettyMethod(cu_->method_idx, *cu_->dex_file) == "java.lang.String java.lang.IntegralToString.convertInt(java.lang.AbstractStringBuilder, int)");
          if (debug) LOG(INFO) << "WORKING ON NEW INSTANCE OF STRING IN " << PrettyMethod(cu_->method_idx, *cu_->dex_file);
          ...
          */
          ...
          /*
          move_result_mir->ssa_rep = static_cast<struct SSARepresentation *>(arena_->Alloc(sizeof(SSARepresentation), kArenaAllocDFInfo));
          ...
          */
          ...
          /*
          if (debug) {
            for (BasicBlock* bb2 : block_list_) {
              LOG(INFO) << StringPrintf("Block %d (insn %04x - %04x%s)", bb2->id, bb2->start_offset,
                                        bb2->last_mir_insn ? bb2->last_mir_insn->offset : bb2->start_offset,
                                        bb2->last_mir_insn ? "" : " empty");
              if (bb2->taken != NullBasicBlockId) {
                LOG(INFO) << "  Taken branch: block " << bb2->taken
                          << "(0x" << std::hex << GetBasicBlock(bb2->taken)->start_offset << ")";
              }
              if (bb2->fall_through != NullBasicBlockId) {
                LOG(INFO) << "  Fallthrough : block " << bb2->fall_through
                          << " (0x" << std::hex << GetBasicBlock(bb2->fall_through)->start_offset << ")";
              }
              for (MIR* mir2 = bb2->first_mir_insn; mir2 != nullptr; mir2 = mir2->next) {
                LOG(INFO) << " " << Instruction::Name(mir2->dalvikInsn.opcode) << " " << mir2->dalvikInsn.vA
                          << " " << mir2->dalvikInsn.vB << " " << mir2->dalvikInsn.vC << "  "
                          << mir2->dalvikInsn.arg[0] << " " << mir2->dalvikInsn.arg[1] << " "
                          << mir2->dalvikInsn.arg[2] << " " << mir2->dalvikInsn.arg[3] << " ";
              }
            }
          }
          */
