...
bool MIRGraph::EliminateNullChecksAndInferTypes(BasicBlock* bb) {
  ...
  if (do_nce) {
    ...
    if ((bb->block_type == kEntryBlock) | bb->catch_entry) {
      ...
      CHECK(pred_bb->data_flow_info->ending_check_v != nullptr);
      ...
      while (pred_bb->data_flow_info->ending_check_v == nullptr) {
        ...
        CHECK(pred_bb != nullptr);
        CHECK(pred_bb->data_flow_info != nullptr);
      }
      ssa_regs_to_check->Copy(pred_bb->data_flow_info->ending_check_v);
      while (true) {
        pred_bb = GetBasicBlock(iter.Next());
        if (!pred_bb) break;
        CHECK(pred_bb->data_flow_info != nullptr);
        if ((pred_bb->data_flow_info == nullptr) ||
            (pred_bb->data_flow_info->ending_check_v == nullptr)) {
          continue;
        }
  ...
  if (do_nce) {
    if (bb->data_flow_info->ending_check_v == nullptr) {
      CHECK(temp_scoped_alloc_.get() != nullptr);
...
void MIRGraph::EliminateClassInitChecksStart() {
  if (kIsDebugBuild) {
    AllNodesIterator iter(this);
    for (BasicBlock* bb = iter.Next(); bb != nullptr; bb = iter.Next()) {
      CHECK(bb->data_flow_info == nullptr || bb->data_flow_info->ending_check_v == nullptr);
  ...
  uint32_t unique_class_count = 0u;
  {
    ...
    // First, find all SGET/SPUT that may need class initialization checks.
    AllNodesIterator iter(this);
    for (BasicBlock* bb = iter.Next(); bb != nullptr; bb = iter.Next()) {
    ...
    if (unique_class_count == 0u) {
      // All IGET/IPUT insns refer to initialized classes. Nothing to do.
      DCHECK_EQ(temp_bit_vector_size_, 0u);
      return;
    }

    // Now go through all IGET/IPUT/INVOKE insns that may help avoid the checks.
    iter.Reset();
    for (BasicBlock* bb = iter.Next(); bb != nullptr; bb = iter.Next()) {
      for (MIR* mir = bb->first_mir_insn; mir != nullptr; mir = mir->next) {
        DCHECK(bb->data_flow_info != nullptr);
        ...
        if (mir->dalvikInsn.opcode >= Instruction::IGET &&
          ...
          const MirFieldInfo& field_info = GetIFieldLoweringInfo(mir);
          resolved = field_info.IsResolved();
          entry.declaring_dex_file = field_info.DeclaringDexFile();
          entry.declaring_class_idx = field_info.DeclaringClassIndex();
        } else if (mir->dalvikInsn.opcode >= Instruction::INVOKE_VIRTUAL &&
            mir->dalvikInsn.opcode <= Instruction::INVOKE_INTERFACE_RANGE &&
            mir->dalvikInsn.opcode != Instruction::RETURN_VOID_BARRIER) {
          const MirMethodInfo& method_info = GetMethodLoweringInfo(mir);
          resolved = method_info.IsResolved();
          entry.declaring_dex_file = method_info.DeclaringDexFile();
          entry.declaring_class_idx = method_info.DeclaringClassIndex();
        } else {
          continue;
        }
        // NOTE: The uninitialized entry.index isn't used by find(), i.e. by MapEntryComparator.
        auto it = resolved ? class_to_index_map.find(entry) : class_to_index_map.end();
        // Using offset/2 for index into temp_insn_data_.
        temp_insn_data_[mir->offset / 2u] = (it != class_to_index_map.end()) ? it->index : 0xffffu;
      }
    }
  }
  ...
  CHECK_GT(temp_bit_vector_size_, 0u);
}
...
bool MIRGraph::EliminateClassInitChecks(BasicBlock* bb) {
  ...
  if (temp_bit_vector_size_ == 0u || bb->data_flow_info == NULL) {
    return false;
  }
  ...
  CHECK(classes_to_check != nullptr);
  if ((bb->block_type == kEntryBlock) | bb->catch_entry) {
    ...
    BasicBlock* pred_bb = GetBasicBlock(iter.Next());
    CHECK(pred_bb != NULL);
    CHECK(pred_bb->data_flow_info != NULL);
    while (pred_bb->data_flow_info->ending_check_v == nullptr) {
      pred_bb = GetBasicBlock(iter.Next());
      // At least one predecessor must have been processed before this bb.
      CHECK(pred_bb != nullptr);
  ...
  for (MIR* mir = bb->first_mir_insn; mir != nullptr; mir = mir->next) {
    if ((mir->dalvikInsn.opcode >= Instruction::IGET &&
         mir->dalvikInsn.opcode <= Instruction::SPUT_SHORT) ||
        (mir->dalvikInsn.opcode >= Instruction::INVOKE_VIRTUAL &&
         mir->dalvikInsn.opcode <= Instruction::INVOKE_INTERFACE_RANGE &&
         mir->dalvikInsn.opcode != Instruction::RETURN_VOID_BARRIER)) {
  ...
  if (bb->data_flow_info->ending_check_v == nullptr) {
    ...
    bb->data_flow_info->ending_check_v = new (temp_scoped_alloc_.get()) ArenaBitVector(
        temp_scoped_alloc_.get(), temp_bit_vector_size_, false, kBitMapClInitCheck);
    CHECK(bb->data_flow_info->ending_check_v != nullptr);
