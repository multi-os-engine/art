    ...
    for (MIR* mir = bb->first_mir_insn; mir != nullptr; mir = mir->next) {
      ...
      if (opcode == Instruction::NEW_INSTANCE) {
        ...
        if (PrettyMethod(method_idx, *cu_->dex_file, false) == "java.lang.String.<init>") {
          ...
          // Remove this pointer from string init and change to static call of StringFactory.
          mir->dalvikInsn.vA--;
          if (!is_range) {
          ...
          if (bb->predecessors.size() == 1) {
            MIR* check_mir = GetBasicBlock(bb->predecessors[0])->last_mir_insn;
            if (check_mir != NULL && static_cast<int>(check_mir->dalvikInsn.opcode) == kMirOpCheck) {
              check_mir->dalvikInsn.vA = mir->dalvikInsn.vA;
              check_mir->dalvikInsn.vB = mir->dalvikInsn.vB;
              check_mir->dalvikInsn.vC = mir->dalvikInsn.vC;
              check_mir->dalvikInsn.arg[0] = mir->dalvikInsn.arg[0];
              check_mir->dalvikInsn.arg[1] = mir->dalvikInsn.arg[1];
              check_mir->dalvikInsn.arg[2] = mir->dalvikInsn.arg[2];
              check_mir->dalvikInsn.arg[3] = mir->dalvikInsn.arg[3];
            }
          }
          ...
          const VerifiedMethod* verified_method = cu_->compiler_driver->GetVerifiedMethod(cu_->dex_file, cu_->method_idx);
          SafeMap<uint32_t, std::set<uint32_t>> string_init_map = verified_method->GetStringInitPcRegMap();
