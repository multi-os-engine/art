...
class ReferenceMapCalculatorTest : public MirGraphTest {
 public:
  ...
  void DoCalculateReferenceMaps() {
    ...
    const Pass* type_pass = GetPassInstance<NullCheckEliminationAndTypeInference>();
    type_pass->Start(&data_holder);
    WalkBasicBlocks<RepeatingTopologicalSortIterator>(&data_holder, type_pass);
