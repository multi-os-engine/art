 ...
 *   - value name for global memory
 *   - initially 0u, gets new value name for each invoke
 * unresolved_sfield_version_[kFieldTypeCount];
 * unresolved_ifield_version_[kFieldTypeCount];
 *   - value name for typed field memory
 *   - initially 0u, gets new value for each unresolved field store
 *
 * Static fields (always the same):
 *   - typed_memory_version ~ (global_memory_version_, unresolved_sfield_version_[type])
 *   - SGET field_memory_version: (typed_memory_version, field_id)
 * Instance fields:
 *   - if aliasing,
 *     - typed_memory_version ~ (global_memory_version_, unresolved_ifield_version_[type])
 *     - IGET field_memory_version ~ (typed_memory_version, field_id, last_stored_value)
 *   - if non-aliasing,
 *     - IGET field_memory_version ~ (base, field_id)
 *     - when escapes by PUT, loads should remain the same until field_id is used in aliased store
 *       (create a map of escaped field refs)
 * Arrays:
 *   - for range check, use (base, index); DETACH from value_names_
 *   - if aliasing,
 *     - typed_memory_version ~ (global_memory_version)
 *     - AGET field_memory_version ~ (typed_memory_version, index, last_stored_value)
 *   - if non-aliasing,
 *     - AGET field_memory_version ~ (typed_memory_version, base, index, last_stored_value)
 */

class LocalValueNumbering {
 public:
  static LocalValueNumbering* Create(CompilationUnit* cu, ScopedArenaAllocator* allocator) {
    void* addr = allocator->Alloc(sizeof(LocalValueNumbering), kArenaAllocMisc);
    return new(addr) LocalValueNumbering(cu, allocator);
  }
