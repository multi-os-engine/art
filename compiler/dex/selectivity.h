...
namespace Selectivity {
  ...
  bool (*precompile_summary_logic)(CompilerDriver* driver, VerificationResults* verifier);
  bool (*skip_class_compile)(const DexFile& dex_file, const DexFile::ClassDef&);
  bool (*skip_method_compile)(const DexFile::CodeItem* code_item, uint32_t method_idx,
                                     uint32_t* access_flags, uint16_t* class_def_idx,
                                     const DexFile& dex_file,
                                     DexToDexCompilationLevel* dex_to_dex_compilation_level);
  void (*analyze_resolved_method)(mirror::ArtMethod* method, const DexFile& dex_file);
  void (*analyze_verified_method)(verifier::MethodVerifier* verifier);
  void (*dump_selectivity_stats)();
  void (*toggle_analysis)(bool setting, std::string disable_passes);
}  // namespace Selectivity
}  // namespace art
