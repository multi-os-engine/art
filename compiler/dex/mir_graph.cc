    ...
    if (bb->block_type == kEntryBlock) {
      ...
      const MIR* mir;
        ...
        for (mir = bb->first_mir_insn; mir; mir = mir->next) {
            ...
            fprintf(file, "    {%04x %s %s %s %s %s %s\\l}%s\\\n", mir->offset,
...
static void FillTypeSizeString(const uint32_t type_size, std::string& decoded_mir) {
  ...
  switch (opsize) {
    case k32:
      decoded_mir.append("int");
      break;
    case k64:
      decoded_mir.append("long");
      break;
    case kSingle:
      decoded_mir.append("float");
      break;
    case kDouble:
      decoded_mir.append("double");
      break;
    case kUnsignedHalf:
      decoded_mir.append("char");
      break;
    case kSignedHalf:
      decoded_mir.append("short");
      break;
    case kUnsignedByte:
      decoded_mir.append("unsigned-byte");
      break;
    case kSignedByte:
      decoded_mir.append("byte");
      break;
    default:
      decoded_mir.append("unknown");
      break;
  }
  decoded_mir.append(")");
}
  ...
  switch (opcode) {
    ...
    case kMirOpCopy:
      if (ssa_rep != nullptr) {
        ...
        decoded_mir.append(StringPrintf(" = %s", GetSSANameWithConst(ssa_rep->uses[0], false).c_str()));
        if (uses > 1) {
    ...
    case kMirOpFusedCmpLong:
      if (ssa_rep != nullptr) {
        decoded_mir.append(StringPrintf(" %s", GetSSANameWithConst(ssa_rep->uses[0], false).c_str()));
        for (int i = 1; i < uses; i++) {
          decoded_mir.append(StringPrintf(", %s", GetSSANameWithConst(ssa_rep->uses[i], false).c_str()));
        }
      } else {
