...
void MIRGraph::ComputeTopologicalSortOrder() {
  ...
  topological_order_loop_ends_.reserve(num_blocks);
  topological_order_indexes_.clear();
  topological_order_indexes_.reserve(num_blocks);
  for (BasicBlockId i = 0; i != num_blocks; ++i) {
    topological_order_loop_ends_.push_back(0u);
    topological_order_indexes_.push_back(static_cast<uint16_t>(-1));
  }
