...
BasicBlock* MIRGraph::FindBlock(DexOffset code_offset, bool create,
                                ...
                                ArenaVector<uint16_t>* dex_pc_to_block_map) {
  ...
  while (code_ptr < code_end) {
    ...
    if (opcode == Instruction::NOP) {
      ...
      if (width == 1) {
        ...
        FindBlock(current_offset_ + width, /* create */ true, /* immed_pred_block_p */ nullptr, &dex_pc_to_block_map);
      }
    } else {
      cur_block->AppendMIR(insn);
    }
    ...
    if (flags & Instruction::kBranch) {
      ...
      cur_block = ProcessCanSwitch(cur_block, insn, current_offset_, width, flags, &dex_pc_to_block_map);
    }
