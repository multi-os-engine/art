...
class PassDriverME: public PassDriver<PassDriverType> {
  ...
  static void PrintPassOptions() {
    for (auto pass : PassDriver<PassDriverType>::g_default_pass_list) {
      ...
      if (me_pass->HasOptions()) {
        LOG(INFO) << "Pass default options for \"" << me_pass->GetName() << "\" are:";
        me_pass->PrintPassDefaultOptions();
      }
    }
  }
 ...
 protected:
   ...
   */
  void FillOverriddenPassSettings(const char* pass_name, SafeMap<const std::string, int>& settings_to_fill) {
