...
const MethodReference* VerifiedMethod::GetDequickenIndex(uint32_t dex_pc) const {
...
void VerifiedMethod::GenerateDequickenMap(verifier::MethodVerifier* method_verifier) {
  ...
  for (; inst < end; inst = inst->Next()) {
    ...
    if (is_virtual_quick || is_range_quick) {
      ...
      dequicken_map_.Put(dex_pc, MethodReference(method->GetDexFile(),
      ...
      dequicken_map_.Put(dex_pc, MethodReference(field->GetDexFile(), field->GetDexFieldIndex()));
    }
  }
}
