...
bool VerifiedMethod::GenerateGcMap(verifier::MethodVerifier* method_verifier) {
  ...
  GcMapBuilder builder(&dex_gc_map_, num_entries, pc_bits, ref_bitmap_bits);
  // Write table data.
  ...
  for (size_t i = 0; i < code_item->insns_size_in_code_units_; i++) {
    if (method_verifier->GetInstructionFlags(i).IsCompileTimeInfoPoint()) {
      ...
      DCHECK(line != nullptr);
      line->WriteReferenceBitMap(method_verifier, &builder);
    }
  }
  return true;
}
...
void VerifiedMethod::VerifyGcMap(verifier::MethodVerifier* method_verifier,
  ...
  const DexFile::CodeItem* code_item = method_verifier->CodeItem();
  GcMap gc_map(&data[0]);
  const size_t bytes = GcMap::ComputeSize(gc_map.NumEntries(), gc_map.KeyBits(),
                                          gc_map.BitmapBits());
  CHECK_EQ(data.size(), bytes);
  for (size_t i = 0; i < code_item->insns_size_in_code_units_; i++) {
    size_t bit_index = gc_map.Find(i);
    if (method_verifier->GetInstructionFlags(i).IsCompileTimeInfoPoint()) {
      CHECK_NE(bit_index, 0U);
      verifier::RegisterLine* line = method_verifier->GetRegLine(i);
      for (size_t j = 0; j < code_item->registers_size_; j++) {
        if (line->GetRegisterType(method_verifier, j).IsNonZeroReferenceTypes()) {
          CHECK_EQ(gc_map.GetBit(bit_index + j), 1U);
        } else if (j < gc_map.BitmapBits()) {
          CHECK_EQ(gc_map.GetBit(bit_index + j), 0U);
        }
      }
    } else {
      CHECK_EQ(bit_index, 0U);
    }
  }
}
...
void VerifiedMethod::ComputeGcMapSizes(verifier::MethodVerifier* method_verifier,
  ...
  size_t local_gc_points = 0;  // Number of GC points == number of GC map entries.
  size_t max_insn = 0;
  size_t max_ref_reg = 0;
  const DexFile::CodeItem* code_item = method_verifier->CodeItem();
  for (size_t i = 0; i < code_item->insns_size_in_code_units_; i++) {
    if (method_verifier->GetInstructionFlags(i).IsCompileTimeInfoPoint()) {
      local_gc_points++;
  ...
  *ref_bitmap_bits = max_ref_reg;
  *gc_points = local_gc_points;
  size_t i = 0;
  while ((1U << i) <= std::max(local_gc_points, max_insn)) {
    i++;
  }
  *log2_max_gc_pc = i;
}
