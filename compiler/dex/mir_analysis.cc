...
void MIRGraph::DoAnnotateCalledMethods() {
  ...
  for (BasicBlock* bb = iter.Next(); bb != nullptr; bb = iter.Next()) {
    ...
    for (MIR* mir = bb->first_mir_insn; mir != nullptr; mir = mir->next) {
      if (mir->dalvikInsn.opcode >= Instruction::INVOKE_VIRTUAL &&
        ...
        if (UNLIKELY(method_refs.get() == nullptr)) {
          method_refs.reset(new MIR*[max_refs]);
        }
  ...
  std::sort(method_refs.get(), method_refs.get() + invoke_count, Comparator());
