...
void MIRGraph::DoCacheMethodLoweringInfo() {
  ...
  // Find and count unique references, set method info indexes for duplicates.
  ...
  for (size_t pos = 1u; pos != invoke_count; ++pos) {
    ...
    if (Comparator()(last_unique_mir, mir)) {
      ...
      mir->meta.method_lowering_info = count - 1u;
    }
  }
  ...
  for (size_t pos = 0u; pos != count; ++pos) {
    MIR* mir = method_refs[pos];
    MirMethodLoweringInfo method_info(mir->meta.invoke_order.target_method_idx,
                                      static_cast<InvokeType>(mir->meta.invoke_order.invoke_type));
    if (mir->meta.invoke_order.devirt_target != nullptr) {
      method_info.SetDevirtualizationTarget(*mir->meta.invoke_order.devirt_target);
    }
    method_lowering_infos_.Insert(method_info);
    mir->meta.method_lowering_info = pos;
  }
