...
void MIRGraph::DoAnnotateUsedFields() {
  ...
  if (ifield_pos != 0u) {
    ...
    for (size_t pos = 0u; pos != ifield_pos; ++pos) {
      ifield_lowering_infos_.Insert(IFieldLoweringInfo(field_idxs[pos]));
    }
    IFieldLoweringInfo::Resolve(cu_->compiler_driver, GetCurrentDexCompilationUnit(),
  ...
  if (sfield_pos != size) {
    ...
    for (size_t pos = size; pos != sfield_pos;) {
      --pos;
      sfield_lowering_infos_.Insert(SFieldLoweringInfo(field_idxs[pos]));
    }
    SFieldLoweringInfo::Resolve(cu_->compiler_driver, GetCurrentDexCompilationUnit(),
                              sfield_lowering_infos_.GetRawStorage(),
                              size - sfield_pos);
  }
}
