...
template<>
uint16_t const PassDriver<PassDriverMECleanUp>::g_passes_size =
                          arraysize(PassDriver<PassDriverMECleanUp>::g_passes);

// The default pass list is used by the PassDriverME instance of PassDriver to initialize pass_list_.
template<>
std::vector<const Pass*> PassDriver<PassDriverMECleanUp>::g_default_pass_list(
                          PassDriver<PassDriverMECleanUp>::g_passes,
                          PassDriver<PassDriverMECleanUp>::g_passes +
                            PassDriver<PassDriverMECleanUp>::g_passes_size);

// By default, do not have a dump pass list.
template<>
std::unique_ptr<const char> PassDriver<PassDriverMECleanUp>::dump_pass_list_(nullptr);

// By default, do not have a print pass list.
template<>
std::unique_ptr<const char> PassDriver<PassDriverMECleanUp>::print_pass_list_(nullptr);

// By default, we do not print the pass' information.
template<>
bool PassDriver<PassDriverMECleanUp>::default_print_passes_ = false;
}  // namespace art
