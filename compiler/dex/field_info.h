...
#ifndef ART_COMPILER_DEX_FIELD_INFO_H_
#define ART_COMPILER_DEX_FIELD_INFO_H_
...
/*
 * Field lowering info is calculated from the perspective of the compilation unit that accesses
...
class FieldInfo {
 ...
 protected:
  ...
  FieldInfo(uint16_t field_idx, uint16_t flags)
...
class IFieldLoweringInfo : public FieldInfo {
 public:
  ...
  static void Resolve(CompilerDriver* compiler_driver, const DexCompilationUnit* mUnit,
                      IFieldLoweringInfo* field_infos, size_t count)
      LOCKS_EXCLUDED(Locks::mutator_lock_);

  // Construct an unresolved instance field lowering info.
  explicit IFieldLoweringInfo(uint16_t field_idx)
      : FieldInfo(field_idx, kFlagIsVolatile),  // Without kFlagIsStatic.
        field_offset_(0u) {
  }
...
class SFieldLoweringInfo : public FieldInfo {
 public:
  ...
  static void Resolve(CompilerDriver* compiler_driver, const DexCompilationUnit* mUnit,
                      SFieldLoweringInfo* field_infos, size_t count)
      LOCKS_EXCLUDED(Locks::mutator_lock_);

  // Construct an unresolved static field lowering info.
  explicit SFieldLoweringInfo(uint16_t field_idx)
      : FieldInfo(field_idx, kFlagIsVolatile | kFlagIsStatic),
        field_offset_(0u),
...
#endif  // ART_COMPILER_DEX_FIELD_INFO_H_
