...
LocalValueNumbering* GlobalValueNumbering::PrepareBasicBlock(BasicBlock* bb) {
  ...
  if (bb->block_type == kEntryBlock || bb->catch_entry) {
    ...
    if (merge_lvns_.size() == 1u) {
      work_lvn_->Copy(*merge_lvns_.front());
    } else {
      work_lvn_->Merge(merge_lvns_);
    }
    merge_lvns_.clear();
  }
...
bool GlobalValueNumbering::NullCheckedInAllPredecessors(
    ...
    const ScopedArenaVector<const LocalValueNumbering*>& pred_lvns,
    const ScopedArenaVector<uint16_t>& merge_names) const {
  DCHECK_EQ(pred_lvns.size(), merge_names.size());
  for (size_t i = 0, size = pred_lvns.size(); i != size; ++i) {
    const LocalValueNumbering* pred_lvn = pred_lvns[i];
    ...
    if (!pred_lvn->IsValueNullChecked(value_name)) {
      ...
      if (!HasNullCheckLastInsn(pred_bb, lvn.Id())) {
        return false;
      }
