...
class RegStorage {
 public:
  ...
  int GetReg() const {
    DCHECK(!IsPair());
    // TUNING: perhaps set kInvalidRegVal at construction time for invalids?
    return IsInvalid() ? kInvalidRegVal : (reg_ & kRegValMask);
  }
  ...
  // Combine 2 solo regs into a pair.
  RegStorage MakeRegPair(RegStorage low, RegStorage high) {
    DCHECK(!low.IsPair());
    DCHECK(!high.IsPair());
    return RegStorage(k64BitPair, low.GetReg(), high.GetReg());
  }

  RegStorage MakeRegPair(int low, int high) {
    return RegStorage(k64BitPair, low, high);
  }
