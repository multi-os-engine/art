...
inline void GvnDeadCodeElimination::VRegChains::AddMIR(const MIRData& data) {
...
inline const GvnDeadCodeElimination::MIRData* GvnDeadCodeElimination::VRegChains::GetMIR(
    size_t pos) const {
  DCHECK_LT(pos, mir_data_.size());
  return &mir_data_[pos];
}

inline GvnDeadCodeElimination::MIRData* GvnDeadCodeElimination::VRegChains::GetMIR(size_t pos) {
  DCHECK_LT(pos, mir_data_.size());
  return &mir_data_[pos];
}

uint16_t GvnDeadCodeElimination::VRegChains::GetLastVRegChange(int v_reg) {
  DCHECK_LT(static_cast<size_t>(v_reg), num_vregs_);
  return vreg_data_[v_reg].change;
}

uint16_t GvnDeadCodeElimination::VRegChains::GetCurrentVRegValue(int v_reg) {
  DCHECK_LT(static_cast<size_t>(v_reg), num_vregs_);
  return vreg_data_[v_reg].value;
}

uint16_t GvnDeadCodeElimination::VRegChains::MarkRevertDef(int v_reg) {
  DCHECK_LT(static_cast<size_t>(v_reg), num_vregs_);
...
void GvnDeadCodeElimination::VRegChains::ReplaceChange(uint16_t old_change, uint16_t new_change) {
  const MIRData* old_data = GetMIR(old_change);
...
GvnDeadCodeElimination::GvnDeadCodeElimination(const GlobalValueNumbering* gvn,
    ...
    : gvn_(gvn),
      ...
      vreg_data_(this), mir_data_(this) {
}
...
void GvnDeadCodeElimination::BackwardPass() {
  ...
  while (!mir_data_.empty()) {
    ...
    if (data.uses_all_vregs) {
      DCHECK_EQ(no_uses_all_since_, mir_data_.size());
      --no_uses_all_since_;
      while (no_uses_all_since_ != 0u &&
          !mir_data_[no_uses_all_since_ - 1u].uses_all_vregs) {
        --no_uses_all_since_;
      }
    }
    mir_data_.pop_back();
...
void GvnDeadCodeElimination::ValidateVRegChain(int v_reg) {
  CHECK_LT(static_cast<size_t>(v_reg), vreg_data_.size());
  uint16_t change = vreg_data_[v_reg].change;
...
void GvnDeadCodeElimination::ChangeBinOp2AddrToPlainBinOp(MIR* mir) {
  // Rewrite instr_2ADDR with plain instr before doing the register rename.
...
bool GvnDeadCodeElimination::IsSRegUsed(uint16_t first_change, uint16_t last_change,
                                        int s_reg) const {
...
void GvnDeadCodeElimination::RecordPassKillMoveByRenamingSrcDef(uint16_t src_change,
  ...
  MIRData* move_data = &mir_data_[move_change];
  ...
  RenameUses(src_change + 1u, mir_data_.size(), old_s_reg, new_s_reg);
  KillMIR(move_data);
  pthread_mutex_lock(&vmarko_mutex);
...
void GvnDeadCodeElimination::RecordPassTryToKillOverwrittenMoveOrMoveSrc(uint16_t check_change) {
  DCHECK_LT(check_change, mir_data_.size());
  MIRData* data = &mir_data_[check_change];
  ...
  if (data->is_move) {
    ...
    if (src_change == kNPos || !IsSRegUsed(src_change + 1u, mir_data_.size(), dest_s_reg)) {
      // We can simply change all uses of dest to src.
      size_t rename_end = (src_change != kNPos) ? src_change + 1u : mir_data_.size();
      RenameUses(check_change + 1u, rename_end, dest_s_reg, src_s_reg);
  ...
  if (data->is_move_src) {
    ...
    for (size_t c = check_change + 1u, size = mir_data_.size(); c != size; ++c) {
      MIRData* d = &mir_data_[c];
...
void GvnDeadCodeElimination::RecordPassTryToKillOverwrittenMoveOrMoveSrc() {
  ...
  if (data->prev_value.change != kNPos && data->prev_value.change + 1u >= no_uses_all_since_) {
    MIRData* check_data = &mir_data_[data->prev_value.change];
    ...
    if (!check_data->is_move && !check_data->is_move_src) {
      ...
      if (vreg_data_[check_data->vreg_def].change == data->prev_value.change) {
        try_to_kill = true;
      }
  ...
  if (data->wide_def && data->high_def_over_low_word &&
    ...
    MIRData* check_data = &mir_data_[data->prev_value_high.change];
    bool try_to_kill = false;
    if (!check_data->is_move && !check_data->is_move_src) {
      DCHECK(!try_to_kill);
    } else if (!check_data->wide_def) {
      // Narrow move; always fully overwritten by the last MIR.
      try_to_kill = true;
    } else if (vreg_data_[check_data->vreg_def + 1].change == data->prev_value_high.change) {
...
size_t GvnDeadCodeElimination::BackwardPassTryToKillRevertVRegs() {
  // RAII class to clean mir_data_[.].revert_*.
    ...
    ~CleanTempDepends() {
      if (start_ != kNPos) {
        for (size_t c = start_, size = dce_->mir_data_.size(); c != size; ++c) {
          dce_->mir_data_[c].revert_def = false;
          dce_->mir_data_[c].revert_def_high = false;
          dce_->mir_data_[c].revert_depends = false;
        }
      }
    }
  ...
  bool can_rename_s_regs = true;
  for (uint32_t v_reg : revert_vregs_->Indexes()) {
    uint16_t first_revert_change = vreg_chains_.MarkRevertDef(v_reg);
    if (first_revert_change == kNPos) {
      return 0u;
    }
    clean_temp_depends.UpdateStart(first_revert_change);
    if (mir_data_[first_revert_change].PrevChange(v_reg) == kNPos) {
      can_rename_s_regs = false;
    }
  }
  ...
  for (uint32_t v_reg : dependent_vregs_->Indexes()) {
    ...
    MIRData* data = &mir_data_[change];
    if (data->wide_def &&
        vreg_data_[(v_reg == data->vreg_def) ? v_reg + 1u : v_reg - 1u].change != change) {
...
void GvnDeadCodeElimination::BackwardPassTryToKillLastMIR() {
  ...
  if (num_dependent_vregs != 0u && num_dependent_vregs <= 2u) {
    ...
    size_t result = BackwardPassTryToKillRevertVRegs();
...
bool GvnDeadCodeElimination::RecordMIR(MIR* mir) {
  ...
  switch (opcode) {
    case kMirOpPhi: {
      ...
      int v_reg = mir_graph_->SRegToVReg(s_reg);
      DCHECK_EQ(vreg_chains_.GetCurrentVRegValue(v_reg), kNoValue);  // No previous def for v_reg.
      if (wide) {
        DCHECK_EQ(vreg_chains_.GetCurrentVRegValue(v_reg + 1), kNoValue);
      }
      if (kVerboseDebug) {
    ...
    case Instruction::MOVE_OBJECT_16: {
      ...
      int def_change = vreg_data_[mir_graph_->SRegToVReg(mir->ssa_rep->uses[0])].change;
      if (def_change != kNPos &&
          static_cast<int>(mir_data_[def_change].mir->dalvikInsn.opcode) != kMirOpPhi) {
        mir_data_[def_change].is_move_src = true;
      }
    }
  ...
  mir_data_.push_back(data);
  if (data.uses_all_vregs) {
    no_uses_all_since_ = mir_data_.size();
  }
  return true;
}
