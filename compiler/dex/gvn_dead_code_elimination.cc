...
GvnDeadCodeElimination::VRegChains::VRegChains(uint32_t num_vregs, ScopedArenaAllocator* alloc)
    : num_vregs_(num_vregs),
      vreg_data_(static_cast<VRegValue*>(alloc->Alloc(sizeof(VRegValue) * num_vregs,
                                                      kArenaAllocMisc))),
...
uint16_t GvnDeadCodeElimination::VRegChains::LastChange(int v_reg) {
  DCHECK_LT(static_cast<size_t>(v_reg), num_vregs_);
  return vreg_data_[v_reg].change;
}

uint16_t GvnDeadCodeElimination::VRegChains::CurrentValue(int v_reg) {
  DCHECK_LT(static_cast<size_t>(v_reg), num_vregs_);
  return vreg_data_[v_reg].value;
}

uint16_t GvnDeadCodeElimination::VRegChains::MarkRevertDef(int v_reg, uint16_t cutoff) {
...
void GvnDeadCodeElimination::VRegChains::RenameDefSReg(uint16_t change, int new_s_reg) {
  DCHECK_LT(change, mir_data_.size());
...
void GvnDeadCodeElimination::VRegChains::RenameUses(uint16_t first_change, uint16_t last_change,
...
void GvnDeadCodeElimination::VRegChains::ValidateChain(int v_reg) {
  CHECK_LT(static_cast<size_t>(v_reg), num_vregs_);
  uint16_t change = vreg_data_[v_reg].change;
  CHECK(change == kNPos || change < mir_data_.size());
  while (change != kNPos) {
    const MIRData* data = &mir_data_[change];
    CHECK(data->vreg_def == v_reg || data->vreg_def + 1 == v_reg);
    if (data->vreg_def == v_reg) {  // Low word, use prev_value.
      CHECK(data->prev_value.change == kNPos || data->prev_value.change < change);
      change = data->prev_value.change;
    } else {  // High word, use prev_value_high.
      CHECK(data->prev_value_high.change == kNPos || data->prev_value_high.change < change);
      change = data->prev_value_high.change;
    }
  }
}
...
GvnDeadCodeElimination::GvnDeadCodeElimination(const GlobalValueNumbering* gvn,
    ...
    : gvn_(gvn),
      ...
      revert_vregs_(new (alloc) ArenaBitVector(alloc, num_vregs_, false)),
      changed_revert_vregs_(new (alloc) ArenaBitVector(alloc, num_vregs_, false)),
...
void GvnDeadCodeElimination::KillMIR(MIR* mir) {
  if (kVerboseDebug3) {
    LOG(INFO) << "In BB#" << bb_->id << "@0x" << std::hex << mir->offset << ": ELIMINATING";
  }
...
void GvnDeadCodeElimination::RecordPassKillMoveByRenamingSrcDef(uint16_t src_change,
  ...
  if (kVerboseDebugKillMoveRenameSrc) {
    ...
    LOG(INFO) << "In BB#" << lvn_->Id() << "@0x" << std::hex << move_data->mir->offset << std::dec
        << " " << PrettyMethod(cu->method_idx, *cu->dex_file) << ": ELIMINATING (RENAME) "
        << move_data->mir->dalvikInsn.opcode << " " << move_data->vreg_def << ", "
        << src_data->vreg_def;
  }
  ...
  vreg_chains_.RenameUses(src_change + 1u, vreg_chains_.NumMIRs(),
...
void GvnDeadCodeElimination::RecordPassTryToKillOverwrittenMoveOrMoveSrc(uint16_t check_change) {
  ...
  if (data->is_move) {
    ...
    if (src_change == kNPos ||
      ...
      vreg_chains_.RenameUses(check_change + 1u, rename_end,
      ...
      if (kVerboseDebugKillMoveRenameDest) {
        ...
        LOG(INFO) << "In BB#" << lvn_->Id() << "@0x" << std::hex << data->mir->offset << std::dec
            ...
            << " when processing MIR @0x" << std::hex << vreg_chains_.LastMIRData()->mir->offset;
      }
...
void GvnDeadCodeElimination::RecordPassTryToKillLastMIR() {
  MIRData* data = vreg_chains_.LastMIRData();
  if (!data->must_keep &&
      data->has_def && vreg_chains_.CurrentValue(data->vreg_def) == data->prev_value.value
      && (!data->wide_def ||
    ...
    if (prev_change != kNPos) {
      vreg_chains_.RemoveLastMIRData();
      vreg_chains_.RenameDefSReg(prev_change, new_s_reg);
    } else {
      // Create a Phi, even if it has a single input, and use it instead of the original MIR.
      DCHECK(mir->ssa_rep->num_defs == 1 || mir->ssa_rep->num_defs == 2);
      bool wide = (mir->ssa_rep->num_defs != 1);
      bool fp = mir->ssa_rep->fp_def[0];
      MIR* phi = CreatePhi(new_s_reg, fp);
      MIR* last_phi = phi;
      if (wide) {
        DCHECK_EQ(new_s_reg + 1, mir->ssa_rep->defs[1]);
        DCHECK_EQ(fp, mir->ssa_rep->fp_def[1]);
        last_phi = CreatePhi(new_s_reg + 1, fp);
        phi->next = last_phi;
      }
      bb_->PrependMIRList(phi, last_phi);
      // Though the Phi has been added to the beginning, we can keep the MIRData at the end.
      data->mir = phi;
      // Reset the previous value to avoid eventually eliminating the Phi itself (unless unused).
      data->prev_value.value = kNoValue;
      data->prev_value_high.value = kNoValue;
    }
    KillMIR(mir);
    pthread_mutex_lock(&vmarko_mutex);
...
size_t GvnDeadCodeElimination::BackwardPassTryToKillRevertVRegs() {
  // RAII class to clean revert_* fields of MIR data.
  class CleanTempDepends {
   public:
    ...
    ~CleanTempDepends() {
      if (start_ != kNPos) {
        for (size_t c = start_, size = dce_->vreg_chains_.NumMIRs(); c != size; ++c) {
          MIRData* data = dce_->vreg_chains_.GetMIRData(c);
          data->revert_def = false;
          data->revert_def_high = false;
          data->revert_depends = false;
        }
      }
    }
  ...
  for (uint32_t v_reg : revert_vregs_->Indexes()) {
    uint16_t first_revert_change = vreg_chains_.MarkRevertDef(v_reg, no_uses_all_since_);
    if (first_revert_change == kNPos) {
      return 0u;
    }
    clean_temp_depends.UpdateStart(first_revert_change);
  }
  DCHECK_NE(clean_temp_depends.Start(), kNPos);

  changed_revert_vregs_->ClearAllBits();
  dependent_vregs_->ClearAllBits();
  for (size_t c = clean_temp_depends.Start(), size = vreg_chains_.NumMIRs(); c != size; ++c) {
    MIRData* data = vreg_chains_.GetMIRData(c);
    bool depends = false;
    // Permit changes to revert vregs.
    if (data->revert_def) {
      changed_revert_vregs_->SetBit(data->vreg_def);
      depends = true;  // We need to eliminate this insn.
    }
    if (data->revert_def_high) {
      changed_revert_vregs_->SetBit(data->vreg_def + 1);
      depends = true;  // We need to eliminate this insn.
    }
    // Check for insns that write the revert vregs that we already changed.
    if (!depends && data->has_def &&
        (changed_revert_vregs_->IsBitSet(data->vreg_def) ||
         (data->wide_def && changed_revert_vregs_->IsBitSet(data->vreg_def + 1)))) {
      depends = true;
    }
    if (!depends) {
    ...
    if (depends && data->has_def &&
        ((revert_vregs_->IsBitSet(data->vreg_def) &&
          !changed_revert_vregs_->IsBitSet(data->vreg_def)) ||
         (data->wide_def &&
          revert_vregs_->IsBitSet(data->vreg_def) &&
          !changed_revert_vregs_->IsBitSet(data->vreg_def)))) {
      return 0u;  // Can't eliminate; tries to write a revert vreg it's not yet allowed to modify.
    }
    // Finally, update the data.
    if (depends) {
      data->revert_depends = true;
      if (data->has_def) {
  ...
  for (uint32_t v_reg : dependent_vregs_->Indexes()) {
    uint16_t change = vreg_chains_.LastChange(v_reg);
    DCHECK_NE(change, kNPos);
    MIRData* data = vreg_chains_.GetMIRData(change);
    if (data->wide_def &&
        vreg_chains_.LastChange((v_reg == data->vreg_def) ? v_reg + 1u : v_reg - 1u) != change) {
      // Wide def with the other half overwritten. Not a real dependency.
      dependent_vregs_->ClearBit(v_reg);
    } else if (!revert_vregs_->IsBitSet(v_reg) && !allowed_dependent_vregs_->IsBitSet(v_reg)) {
  ...
  if (overwritten_needed_vregs == 0u) {
    ...
    DCHECK(last_data->has_def);
    for (uint32_t v_reg : revert_vregs_->Indexes()) {
      if (last_data->vreg_def != v_reg &&
          (!last_data->wide_def || last_data->vreg_def + 1u != v_reg)) {
        is_retry = true;
      }
    }
    if (kVerboseDebugKillLongSequences || is_retry) {
      std::string revert;
      {
        ...
        oss << "{";
        const char* s = " ";
        for (uint32_t idx : revert_vregs_->Indexes()) {
          oss << s << idx;
        }
        oss << " }";
        revert = oss.str();
      ...
      std::string dep_insns;
      {
        ...
        const char* s = " ";
        for (size_t c = clean_temp_depends.Start(), size = vreg_chains_.NumMIRs(); c != size; ++c) {
          if (vreg_chains_.GetMIRData(c)->revert_depends) {
            oss << s << vreg_chains_.GetMIRData(c)->mir->offset;
            s = ", ";
          }
        }
        oss << " }";
      ...
      CompilationUnit* cu = gvn_->GetCompilationUnit();
      LOG(INFO) << "In BB#" << lvn_->Id() << "@0x" << std::hex << last_data->mir->offset
          << " "
          << (cu->dex_file != nullptr ? PrettyMethod(cu->method_idx, *cu->dex_file) : "TEST")
          << ": ELIMINATING SEQUENCE "
          << last_data->mir->dalvikInsn.opcode << " revert MIRs " << dep_insns
          << " revert=" << revert << " dep=" << dep << " / start = " << clean_temp_depends.Start()
          << " @0x" << std::hex << vreg_chains_.GetMIRData(clean_temp_depends.Start())->mir->offset
          << (is_retry ? " RETRY" : "");
    }
    // Kill all MIRs marked as dependent.
    allowed_dependent_vregs_->Union(revert_vregs_);
    size_t c = vreg_chains_.NumMIRs();
    while (c != clean_temp_depends.Start()) {
      --c;
      MIRData* data = vreg_chains_.GetMIRData(c);
      if (data->revert_depends) {
        DCHECK(!data->must_keep);
        DCHECK(data->has_def);
        vreg_chains_.RemoveChange(c);
        KillMIR(data);
      }
    }
    recalculate_ssa_rep_ = true;
    pthread_mutex_lock(&vmarko_mutex);
    vmarko_killed_complex += 1u;
    pthread_mutex_unlock(&vmarko_mutex);
  }
...
bool GvnDeadCodeElimination::BackwardPassTryToKillLastMIR() {
  ...
  revert_vregs_->ClearAllBits();
  revert_vregs_->SetBit(data->vreg_def);
  if (data->wide_def) {
    revert_vregs_->SetBit(data->vreg_def + 1);
  }
  size_t num_dependent_vregs = BackwardPassTryToKillRevertVRegs();
  if (num_dependent_vregs != 0u && num_dependent_vregs <= 2u) {
    // Add dependent vregs to revert vregs and try again.
    ...
    revert_vregs_->Union(dependent_vregs_);
    num_dependent_vregs = BackwardPassTryToKillRevertVRegs();
    if (static_cast<int>(data->mir->dalvikInsn.opcode) == kMirOpNop) {
      DCHECK_EQ(num_dependent_vregs, 0u);
      CompilationUnit* cu = gvn_->GetCompilationUnit();
      LOG(INFO) << "In BB#" << lvn_->Id() << "@0x" << std::hex << data->mir->offset
          << " "
          << (cu->dex_file != nullptr ? PrettyMethod(cu->method_idx, *cu->dex_file) : "TEST")
  ...
  return static_cast<int>(data->mir->dalvikInsn.opcode) == kMirOpNop;
}
...
bool GvnDeadCodeElimination::RecordMIR(MIR* mir) {
  ...
  if (mir->ssa_rep->num_defs != 0) {
    ...
    if (kVerboseDebug) {
      LOG(INFO) << "In BB#" << bb_->id << "@0x" << std::hex << mir->offset << " " << v_reg
          << "[" << s_reg << "]: " << vreg_chains_.CurrentValue(v_reg)
          << " -> " << new_value << (wide ? " wide" : "");
    }
  } else {
    ...
    if (kVerboseDebug) {
      LOG(INFO) << "In BB#" << lvn_->Id() << "@0x" << std::hex << mir->offset << " NO DEFS";
    }
  }
