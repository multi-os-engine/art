...
uint16_t GvnDeadCodeElimination::MarkRevertDef(int v_reg) {
...
void GvnDeadCodeElimination::ReplaceChangeInVRegChain(int v_reg, uint16_t old_change,
                                                      uint16_t new_change) {
  uint16_t next_change = FindFirstValueChangeAfter(v_reg, old_change);
  if (next_change == kNPos) {
    DCHECK_EQ(vreg_data_[v_reg].change, old_change);
    vreg_data_[v_reg].change = new_change;
  } else if (mir_data_[next_change].vreg_def == v_reg) {
    DCHECK_EQ(mir_data_[next_change].prev_value.change, old_change);
    mir_data_[next_change].prev_value.change = new_change;
  } else {
    DCHECK_EQ(mir_data_[next_change].prev_value_high.change, old_change);
    mir_data_[next_change].prev_value_high.change = new_change;
  }
}
...
void GvnDeadCodeElimination::RemoveChangeFromVRegChain(int v_reg, uint16_t change) {
  ...
  MIRData* data = &mir_data_[change];
  DCHECK(data->vreg_def == v_reg || data->vreg_def + 1 == v_reg);
  uint16_t next_change = FindFirstValueChangeAfter(v_reg, change);
  if (next_change == kNPos) {
    DCHECK_EQ(vreg_data_[v_reg].change, change);
    vreg_data_[v_reg] = (data->vreg_def == v_reg) ? data->prev_value : data->prev_value_high;
  } else if (mir_data_[next_change].vreg_def == v_reg) {
    DCHECK_EQ(mir_data_[next_change].prev_value.change, change);
    if (data->vreg_def == v_reg) {
      mir_data_[next_change].prev_value = data->prev_value;
      mir_data_[next_change].low_def_over_high_word = data->low_def_over_high_word;
    } else {
      mir_data_[next_change].prev_value = data->prev_value_high;
      mir_data_[next_change].low_def_over_high_word =
          data->prev_value_high.value != kNPos && !data->high_def_over_low_word;
    }
  } else {
    DCHECK_EQ(mir_data_[next_change].prev_value_high.change, change);
    if (data->vreg_def == v_reg) {
      mir_data_[next_change].prev_value_high = data->prev_value;
      mir_data_[next_change].high_def_over_low_word =
          data->prev_value.value != kNPos && !data->low_def_over_high_word;
    } else {
      mir_data_[next_change].prev_value_high = data->prev_value_high;
      mir_data_[next_change].high_def_over_low_word = data->high_def_over_low_word;
    }
  }
}
...
void GvnDeadCodeElimination::RecordPassKillMoveByRenamingSrcDef(uint16_t src_change,
  ...
  RemoveChangeFromVRegChain(src_data->vreg_def, src_change);
  if (src_data->wide_def) {
    RemoveChangeFromVRegChain(src_data->vreg_def + 1, src_change);
  }
  ...
  ReplaceChangeInVRegChain(move_data->vreg_def, move_change, src_change);
  if (move_data->wide_def) {
    ReplaceChangeInVRegChain(move_data->vreg_def + 1, move_change, src_change);
  }
...
void GvnDeadCodeElimination::RecordPassTryToKillOverwrittenMoveOrMoveSrc(uint16_t check_change) {
  ...
  if (data->is_move) {
    ...
    uint16_t src_change = FindFirstValueChangeAfter(src_v_reg, check_change);
    bool wide = data->wide_def;
    if (wide) {
      uint16_t src_change_high = FindFirstValueChangeAfter(src_v_reg + 1, check_change);
    ...
    if (src_change == kNPos || !IsSRegUsed(src_change + 1u, mir_data_.size(), dest_s_reg)) {
      ...
      RemoveChangeFromVRegChain(dest_v_reg, check_change);
      if (data->wide_def) {
        RemoveChangeFromVRegChain(dest_v_reg + 1, check_change);
      }
      KillMIR(data);
      pthread_mutex_lock(&vmarko_mutex);
...
size_t GvnDeadCodeElimination::BackwardPassTryToKillLoopVRegDefs() {
  ...
  for (uint32_t v_reg : revert_vregs_->Indexes()) {
    uint16_t first_loop_change = MarkRevertDef(v_reg);
  ...
  for (uint32_t idx : dependent_vregs_->Indexes()) {
    ...
    } else if (!revert_vregs_->IsBitSet(idx) && !allowed_dependent_vregs_->IsBitSet(idx)) {
