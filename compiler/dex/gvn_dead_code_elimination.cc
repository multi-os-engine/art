...
bool GvnDeadCodeElimination::BackwardPassTryToKillLastMIR() {
  ...
  if (unused_vregs_->IsBitSet(last_data->vreg_def) ||
    ...
    if (kVerboseDebugKillUnused) {
      ...
      LOG(INFO) << "In BB#" << lvn_->Id() << "@0x" << std::hex << last_data->mir->offset
          ...
          << last_data->mir->dalvikInsn.opcode;
    }
  ...
  if (kVerboseDebugKillLongSequences || (num_killed_top_changes > 1u)) {
    std::string killed;
    {
      std::ostringstream oss;
      oss << "{";
      const char* s = " ";
      for (uint32_t idx : vregs_to_kill_->Indexes()) {
        oss << s << idx;
      }
      oss << " }";
      killed = oss.str();
    ...
    LOG(INFO) << "In BB#" << lvn_->Id() << "@0x" << std::hex
        ...
        << vreg_chains_.LastMIRData()->mir->dalvikInsn.opcode << " kill MIRs " << dep_insns
        << " kill=" << killed << " dep=" << dep << " / start = " << start
        << " @0x" << std::hex << vreg_chains_.GetMIRData(start)->mir->offset
        << ((num_killed_top_changes > 1u) ? " RETRY" : "");
  }
