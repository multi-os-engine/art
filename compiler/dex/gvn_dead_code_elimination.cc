...
uint16_t GvnDeadCodeElimination::VRegChains::FindRevertHead(int v_reg, uint16_t cutoff) {
...
GvnDeadCodeElimination::GvnDeadCodeElimination(const GlobalValueNumbering* gvn,
    ...
    : gvn_(gvn),
      ...
      allowed_dependent_vregs_(new (alloc) ArenaBitVector(alloc, vreg_chains_.NumVRegs(), false)),
      revert_vregs_(new (alloc) ArenaBitVector(alloc, vreg_chains_.NumVRegs(), false)),
      revert_heads_(alloc->AllocArray<uint16_t>(vreg_chains_.NumVRegs(), kArenaAllocMisc)),
...
uint16_t GvnDeadCodeElimination::FindChangesToKill(uint16_t first_change, uint16_t last_change) {
  ...
  for (size_t change = first_change; change != last_change; ++change) {
    ...
    if (data->has_def && revert_vregs_->IsBitSet(data->vreg_def)) {
      if (change < revert_heads_[data->vreg_def]) {
        must_not_depend = true;
      } else {
        depends = true;
      }
    }
    if (data->has_def && data->wide_def && revert_vregs_->IsBitSet(data->vreg_def + 1)) {
      if (change < revert_heads_[data->vreg_def + 1]) {
        must_not_depend = true;
      } else {
        depends = true;
      }
    ...
    if (depends && data->has_def &&
        vreg_chains_.IsTopChange(change) && !revert_vregs_->IsBitSet(data->vreg_def) &&
        !allowed_dependent_vregs_->IsBitSet(data->vreg_def) &&
        (!data->wide_def || !allowed_dependent_vregs_->IsBitSet(data->vreg_def + 1))) {
      // This is a top change but neither unnecessary nor one of the revert changes.
      return change;
    }
...
void GvnDeadCodeElimination::BackwardPassTryToKillRevertVRegs() {
  ...
  uint16_t revert_change = num_mirs - 1u;
  uint16_t start = num_mirs;
  size_t num_reverted_changes = 0u;
  while (num_reverted_changes != kMaxNumTopChangesToRevert &&
      revert_change != kNPos && revert_change != num_mirs) {
    ++num_reverted_changes;

    DCHECK(vreg_chains_.IsTopChange(revert_change));
    MIRData* data = vreg_chains_.GetMIRData(revert_change);
    int count = data->wide_def ? 2 : 1;
    for (int v_reg = data->vreg_def, end = data->vreg_def + count; v_reg != end; ++v_reg) {
      uint16_t revert_head = vreg_chains_.FindRevertHead(v_reg, no_uses_all_since_);
      if (revert_head == kNPos) {
        return;
      }
      revert_heads_[v_reg] = revert_head;
      revert_vregs_->SetBit(v_reg);
      start = std::min(start, revert_head);
    }
    DCHECK_LT(start, vreg_chains_.NumMIRs());

    revert_change = FindChangesToKill(start, num_mirs);
  }

  if (revert_change == num_mirs) {
    bool is_retry = (num_reverted_changes > 1u);
    if (kVerboseDebugKillLongSequences || is_retry) {
      ...
      std::string dep_insns;
      {
        ...
        oss << "{" << std::hex;
        const char* s = " ";
        for (uint16_t c : changes_to_kill_) {
      ...
      CompilationUnit* cu = gvn_->GetCompilationUnit();
      LOG(INFO) << "In BB#" << lvn_->Id() << "@0x" << std::hex
          << vreg_chains_.LastMIRData()->mir->offset << " "
          << (cu->dex_file != nullptr ? PrettyMethod(cu->method_idx, *cu->dex_file) : "TEST")
          << ": ELIMINATING SEQUENCE "
          << vreg_chains_.LastMIRData()->mir->dalvikInsn.opcode << " revert MIRs " << dep_insns
          << " revert=" << revert << " dep=" << dep << " / start = " << start
          << " @0x" << std::hex << vreg_chains_.GetMIRData(start)->mir->offset
          << (is_retry ? " RETRY" : "");
    }
    ...
    for (uint32_t v_reg : revert_vregs_->Indexes()) {
      ...
      if (data->vreg_def == v_reg) {
        MIRData* revert_head = vreg_chains_.GetMIRData(revert_heads_[v_reg]);
        RenameSRegDefOrCreatePhi(revert_head->PrevChange(v_reg), num_mirs, data->mir);
      } else {
        DCHECK_EQ(data->vreg_def + 1u, v_reg);
        DCHECK_EQ(vreg_chains_.GetMIRData(revert_heads_[v_reg - 1u])->PrevChange(v_reg - 1u),
                  vreg_chains_.GetMIRData(revert_heads_[v_reg])->PrevChange(v_reg));
      }
    }
...
bool GvnDeadCodeElimination::BackwardPassTryToKillLastMIR() {
  ...
  BackwardPassTryToKillRevertVRegs();
