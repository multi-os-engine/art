...
#define DEF_UNIQUE_REF(opcode, reg) \
  ...
  void PerformLVN() {
    ...
    for (size_t i = 0; i != mir_count_; ++i) {
      ...
      mirs_[i].optimization_flags &= MIR_IGNORE_CLINIT_CHECK;
      uint16_t new_num = check_lvn->GetValueNumber(&mirs_[i]);
      EXPECT_EQ(value_names_[i], new_num) << i;
      EXPECT_EQ(opt_flags, mirs_[i].optimization_flags) << i;
    }
    EXPECT_TRUE(gvn_->Good());
...
TEST_F(LocalValueNumberingTest, IGetIPutIGetIGetIGet) {
  ...
  for (size_t i = 0; i != arraysize(mirs); ++i) {
    EXPECT_EQ((i == 2u) ? MIR_IGNORE_NULL_CHECK : 0,
              mirs_[i].optimization_flags & ~MIR_IGNORE_CLINIT_CHECK) << i;
  }
}
...
TEST_F(LocalValueNumberingTest, UnresolvedSField) {
  ...
  for (size_t i = 0u; i != mir_count_; ++i) {
    EXPECT_EQ(0, mirs_[i].optimization_flags & ~MIR_IGNORE_CLINIT_CHECK) << i;
  }
}
...
TEST_F(LocalValueNumberingTest, SameValueInDifferentMemoryLocations) {
  ...
  for (size_t i = 0; i != arraysize(mirs); ++i) {
    ...
    EXPECT_EQ(expected_flags, mirs_[i].optimization_flags & ~MIR_IGNORE_CLINIT_CHECK) << i;
  }
}
...
TEST_F(LocalValueNumberingTest, EscapingRefs) {
  ...
  for (size_t i = 0u; i != mir_count_; ++i) {
    ...
    EXPECT_EQ(expected, mirs_[i].optimization_flags & ~MIR_IGNORE_CLINIT_CHECK) << i;
  }
}
