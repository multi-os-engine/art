...
#define DEF_UNIQUE_REF(opcode, reg) \
  ...
  void DoPrepareIFields(const IFieldDef* defs, size_t count) {
    ...
    for (size_t i = 0u; i != count; ++i) {
      ...
      IFieldLoweringInfo field_info(def->field_idx);
      if (def->declaring_dex_file != 0u) {
        field_info.declaring_dex_file_ = reinterpret_cast<const DexFile*>(def->declaring_dex_file);
        field_info.declaring_field_idx_ = def->declaring_field_idx;
        field_info.flags_ = 0u |  // Without kFlagIsStatic.
            (def->is_volatile ? IFieldLoweringInfo::kFlagIsVolatile : 0u);
      }
  ...
  void DoPrepareSFields(const SFieldDef* defs, size_t count) {
    ...
    for (size_t i = 0u; i != count; ++i) {
      ...
      SFieldLoweringInfo field_info(def->field_idx);
      if (def->declaring_dex_file != 0u) {
        field_info.declaring_dex_file_ = reinterpret_cast<const DexFile*>(def->declaring_dex_file);
        field_info.declaring_field_idx_ = def->declaring_field_idx;
        field_info.flags_ = SFieldLoweringInfo::kFlagIsStatic |
            (def->is_volatile ? SFieldLoweringInfo::kFlagIsVolatile : 0u);
      }
