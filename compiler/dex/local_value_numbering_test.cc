...
#define DEF_UNIQUE_REF(opcode, reg) \
  ...
  void DoPrepareIFields(const IFieldDef* defs, size_t count) {
    ...
    for (size_t i = 0u; i != count; ++i) {
      ...
      IFieldAnnotation annotation = IFieldAnnotation::Unresolved(def->field_idx);
      if (def->resolved_dex_file != 0u) {
        annotation.resolved_dex_file = reinterpret_cast<const DexFile*>(def->resolved_dex_file);
        annotation.resolved_field_idx = def->resolved_field_idx;
        annotation.is_volatile = def->is_volatile ? 1u : 0u;
      }
  ...
  void DoPrepareSFields(const SFieldDef* defs, size_t count) {
    ...
    for (size_t i = 0u; i != count; ++i) {
      ...
      SFieldAnnotation annotation = SFieldAnnotation::Unresolved(def->field_idx);
      if (def->resolved_dex_file != 0u) {
        annotation.resolved_dex_file = reinterpret_cast<const DexFile*>(def->resolved_dex_file);
        annotation.resolved_field_idx = def->resolved_field_idx;
        annotation.is_volatile = def->is_volatile ? 1u : 0u;
...
TEST_F(LocalValueNumberingTest, TestIGetIPutIGet) {
  static const IFieldDef ifields[] = {
      { 1u, 1u, 1u, false }
  };
  static const MIRDef mirs[] = {
      DEF_IGET(Instruction::IGET, 0u, 10u, 0u),
      DEF_IGET(Instruction::IPUT, 1u, 11u, 0u),  // May alias.
      DEF_IGET(Instruction::IGET, 2u, 10u, 0u),
  };
  ...
  PrepareMIRs(mirs);
  PerformLVN();
  ASSERT_EQ(value_names_.size(), 3u);
...
TEST_F(LocalValueNumberingTest, TestUniqueIGetIPutIGet1) {
  static const IFieldDef ifields[] = {
      { 1u, 1u, 1u, false }
  };
  static const MIRDef mirs[] = {
...
TEST_F(LocalValueNumberingTest, TestUniqueIGetIPutIGet2) {
  static const IFieldDef ifields[] = {
      { 1u, 1u, 1u, false }
  };
  static const MIRDef mirs[] = {
