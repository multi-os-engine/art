  ...
  void DoPrepareIFields(const IFieldDef* defs, size_t count) {
    ...
    for (size_t i = 0u; i != count; ++i) {
      ...
      if (def->declaring_dex_file != 0u) {
        annotation.declaring_dex_file = reinterpret_cast<const DexFile*>(def->declaring_dex_file);
        annotation.declaring_field_idx = def->declaring_field_idx;
