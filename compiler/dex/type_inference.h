 ...
 *   1. First, we find and process the SSA register types that are easy to
 *      identify. This includes register definitions by insns such as iget/sget
 *      (using the field type), new-instance (using the embedded type), etc.
 *      It also includes requirements on register usage, such as iput/sget
 *      source reg, invoke args, etc.
 *   2. Then we iteratively propagate the type in both directions, i.e. from
 *      inputs to outputs but also from outputs to inputs, and using the
 *      requirements of typed dex instructions, for example ADD_FLOAT inputs
 *      and outputs are marked as FP.
 *
 ...
 * dx treats constants as untyped bit patterns. We disable the register in that case.
 */
 ...
 private:
  struct Type {
    ...
    bool EqualArrayRef(Type ref_type) {
      ...
      return ((raw_bits_ ^ ref_type.raw_bits_) & ~kMaskNonArray) == 0;
    }
    ...
    // Merge array type required by an instruction.
    bool MergeArrayTypeStrong(Type src_type);

    // Merge array type not required by an instruction.
    bool MergeArrayTypeWeak(Type src_type);
    ...
    bool SetAll(Type src_type) {
      if (raw_bits_ != src_type.raw_bits_) {
        raw_bits_ = src_type.raw_bits_;
        return true;
      }
      return false;
    }
  ...
  class CheckCastData : public DeletableArenaObject<kArenaAllocMisc> {
   public:
    ...
    bool ProcessCheckCast(MIR* mir, Type* sregs);
  ...
  bool UpdateSRegFromLowWordTypeStrong(int32_t mod_s_reg, Type low_word_type);
  bool UpdateSRegFromLowWordTypeWeak(int32_t mod_s_reg, Type low_word_type);
