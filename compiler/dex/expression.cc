...
void UnaryExpression::GetChildren(std::vector<const Expression*>& children) const {
  children.push_back(operand_);
}

void BinaryExpression::GetChildren(std::vector<const Expression*>& children) const {
  children.push_back(lhs_);
  children.push_back(rhs_);
}

std::string BinaryExpression::ToString(const CompilationUnit* c_unit) const {
...
std::string UnaryExpression::ToString(const CompilationUnit* c_unit) const {
  std::string expression_string;
...
std::string VirtualRegister::ToString(const CompilationUnit* c_unit) const {
  std::stringstream ss;
...
Expression* Expression::MirToExpression(ArenaAllocator* arena, MIR* mir,
  ...
  if ((flags & Instruction::kAdd) != 0) {
    exp_kind = ExpKind_Add;
  } else if ((flags & Instruction::kSubtract) != 0) {
    exp_kind = ExpKind_Sub;
  } else if ((flags & Instruction::kMultiply) != 0) {
    exp_kind = ExpKind_Mul;
  } else if ((flags & Instruction::kDivide) != 0) {
    exp_kind = ExpKind_Div;
  } else if ((flags & Instruction::kRemainder) != 0) {
    exp_kind = ExpKind_Rem;
  } else if ((flags & Instruction::kAnd) != 0) {
    exp_kind = ExpKind_And;
  } else if ((flags & Instruction::kOr) != 0) {
    exp_kind = ExpKind_Or;
  } else if ((flags & Instruction::kXor) != 0) {
    exp_kind = ExpKind_Xor;
  } else if ((flags & Instruction::kShr) != 0) {
    exp_kind = ExpKind_Shr;
  } else if ((flags & Instruction::kShl) != 0) {
    exp_kind = ExpKind_Shl;
  } else if ((flags & Instruction::kUshr) != 0) {
    exp_kind = ExpKind_Ushr;
  } else if ((flags & Instruction::kCast) != 0) {
    exp_kind = ExpKind_Cast;
  } else if ((df_flags & DF_SETS_CONST) != 0) {
