...
Expression* Expression::MirToExpression(ArenaAllocator* arena, MIR* mir,
  ...
  if (flags & Instruction::kAddExpression) {
    exp_kind = ExpKind_Add;
  } else if (flags & Instruction::kSubtractExpression) {
    exp_kind = ExpKind_Sub;
  } else if (flags & Instruction::kMultiplyExpression) {
    exp_kind = ExpKind_Mul;
  } else if (flags & Instruction::kDivideExpression) {
    exp_kind = ExpKind_Div;
  } else if (flags & Instruction::kRemainderExpression) {
    exp_kind = ExpKind_Rem;
  } else if (flags & Instruction::kAndExpression) {
    exp_kind = ExpKind_And;
  } else if (flags & Instruction::kOrExpression) {
    exp_kind = ExpKind_Or;
  } else if (flags & Instruction::kXorExpression) {
    exp_kind = ExpKind_Xor;
  } else if (flags & Instruction::kShrExpression) {
    exp_kind = ExpKind_Shr;
  } else if (flags & Instruction::kShlExpression) {
    exp_kind = ExpKind_Shl;
  } else if (flags & Instruction::kUshrExpression) {
...
UnaryExpression* UnaryExpression::MirToExpression(ArenaAllocator* arena, MIR* mir,
  ...
  if (ssa_rep->num_uses == 0) {
    bool is_wide = false;
    int low_constant = 0, high_constant = 0;

    // Get the constant value.
    bool setsConst = mir->dalvikInsn.GetConstant(low_constant, high_constant, is_wide);

    // If we have a constant set expression, then we can build
    // a constant expression for the operand.
    if (setsConst == true) {
      operand = ConstantExpression::NewExpression(arena, low_constant, high_constant, is_wide);
    }
...
ConstantExpression* ConstantExpression::NewExpression(ArenaAllocator* arena, int32_t lowInitialValue,
                                                      int32_t highInitialValue, bool wide) {
  ConstantExpression* result = nullptr;

  if (wide == false) {
    result = new (arena) ConstantExpression(lowInitialValue);
  } else {
    result = new (arena) ConstantExpression(lowInitialValue, highInitialValue);
  }
