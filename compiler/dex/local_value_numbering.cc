...
uint16_t LocalValueNumbering::GetFieldId(const DexFile* dex_file, uint32_t field_idx) {
...
uint16_t LocalValueNumbering::GetMemoryVersion(uint16_t base, uint16_t field, uint16_t type) {
  ...
  return std::max(memory_version, global_memory_version_);
};
...
void LocalValueNumbering::HandlePutObject(MIR* mir) {
  ...
  // TODO: Keep track of fields that may store unique references and still consider them
  // unique until a GET_OBJECT that may cause aliasing.
...
uint16_t LocalValueNumbering::GetValueNumber(MIR* mir) {
  ...
  switch (opcode) {
    ...
    case Instruction::INVOKE_STATIC:
      AdvanceGlobalMemory();
      // TODO: Remove all arguments from unique_objects_.
      break;
    ...
    case Instruction::IGET_SHORT: {
        ...
        if (annotation.is_volatile) {
          ...
          DCHECK(annotation.resolved_dex_file != nullptr);  // Resolved.
          field_id = GetFieldId(annotation.resolved_dex_file, annotation.resolved_field_idx);
    ...
    case Instruction::IPUT_SHORT: {
        ...
        HandleNullCheck(mir, base);
        const IFieldAnnotation& annotation = cu_->mir_graph->GetIFieldAnnotation(mir);
        if (annotation.resolved_dex_file == nullptr) {
          ...
          AdvanceMemoryVersion(base, GetFieldId(annotation.resolved_dex_file,
                                                annotation.resolved_field_idx), type);
        }
      }
      break;
    ...
    case Instruction::SPUT_SHORT: {
        ...
        if (annotation.resolved_dex_file == nullptr) {
          ...
          AdvanceMemoryVersion(NO_VALUE, GetFieldId(annotation.resolved_dex_file,
                                                    annotation.resolved_field_idx), type);
        }
      }
      break;
