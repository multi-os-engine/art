...
uint16_t LocalValueNumbering::GetEndingVregValueNumberImpl(int v_reg, bool wide) const {
  // FIXME: We're using pruned SSA form, so we don't have all the Phi nodes. Thus,
  // vreg_to_ssa_map_exit is not updated correctly for vregs that were not considered
  // live during SSA conversion and we cannot rely on it.
