...
void LocalValueNumbering::InPlaceUnion(Map* work_map, const Map& other_map) {
  ...
  if (same_version) {
    ...
    for (const LocalValueNumbering* lvn : gvn_->merge_lvns_) {
      ...
      const AliasingIFieldValues* values =
          (it != lvn->aliasing_ifield_value_map_.end()) ? &it->second : nullptr;
      if (lvn == first_lvn) {
        cmp_values = values;
      } else if ((cmp_values == nullptr) && (values == nullptr)) {
        // Match.
      } else if ((cmp_values == nullptr) || (values == nullptr)) {
        // One of them is nullptr, the other must have no stores.
        if (((values != nullptr) ? values : cmp_values)->last_stored_value != kNoValue) {
          same_version = false;
          break;
        }
      } else if (cmp_values->last_stored_value != values->last_stored_value ||
          cmp_values->memory_version_before_stores != values->memory_version_before_stores ||
          cmp_values->store_ref_set != values->store_ref_set) {
        same_version = false;
        break;
      }
    }
  ...
  if (same_version) {
    ...
    for (const LocalValueNumbering* lvn : gvn_->merge_lvns_) {
      ...
      if (it != lvn->aliasing_ifield_value_map_.end()) {
        ...
        InPlaceUnion(&my_values->load_value_map, it->second.load_value_map);
      }
    }
  } else {
    my_values->memory_version_before_stores =
...
uint16_t LocalValueNumbering::HandlePhi(MIR* mir) {
  ...
  if (mir->ssa_rep->defs[0] != 0 &&
      gvn_->sreg_value_map_.find(mir->ssa_rep->defs[0] - 1) != gvn_->sreg_value_map_.end()) {
    // This is the high part of a wide reg. Ignore the Phi.
    return kNoValue;
  }
  bool wide = false;
  for (int16_t i = 0; i != num_uses; ++i) {
    if (gvn_->sreg_wide_value_map_.find(uses[i]) != gvn_->sreg_wide_value_map_.end()) {
      wide = true;
      break;
    }
  }
