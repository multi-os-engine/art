...
uint16_t LocalValueNumbering::AdvanceMemoryVersion(uint16_t base, uint16_t field, uint16_t type) {
  ...
  // if "base" isn't a unique reference and check it in GetMemoryVersion() on reads via non-unique
  // reference. A global memory version is set for method calls as a method can potentially write
  // to any memory location accessed via a non-unique reference.
