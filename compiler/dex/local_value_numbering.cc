...
class LocalValueNumbering::AliasingIFieldVersions {
 public:
  ...
  static uint16_t LookupValue(GlobalValueNumbering* gvn,
...
uint16_t LocalValueNumbering::AliasingValuesMergeGet(GlobalValueNumbering* gvn,
  ...
  if (it == map->end()) {
    ...
    value_name = Versions::LookupValue(gvn, key, location, start_version);
    ...
    if (load_it != it->second.load_value_map.end()) {
      ...
      value_name = Versions::LookupValue(gvn, key, location, it->second.last_load_memory_version);
    }
  }
  return value_name;
}
...
uint16_t LocalValueNumbering::HandleAliasingValuesGet(Map* map, const typename Map::key_type& key,
  ...
  if (values->store_loc_set.count(location) != 0u) {
    ...
    if (lb != values->load_value_map.end() && lb->first == location) {
      ...
      res = Versions::LookupValue(gvn_, key, location, values->last_load_memory_version);
...
void LocalValueNumbering::MergeAliasingValues(
    const AliasingIFieldValuesMap::value_type& entry, AliasingIFieldValuesMap::iterator hint) {
  const AliasingIFieldValuesMap::key_type& key = entry.first;
  ...
  if (same_version) {
    ...
    for (const LocalValueNumbering* lvn : gvn_->merge_lvns_) {
      auto it = lvn->aliasing_ifield_value_map_.find(key);
