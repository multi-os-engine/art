...
LocalValueNumbering::AliasingValues* LocalValueNumbering::GetAliasingValues(
  ...
  if (lb == map->end() || map->key_comp()(key, lb->first)) {
    map->PutHint(lb, key, AliasingValues(gvn_->allocator_));
...
uint16_t LocalValueNumbering::HandleAliasingValuesGet(Map* map, const typename Map::key_type& key,
  ...
  if (values->store_loc_set.count(location) != 0u) {
    ...
    if (lb != values->load_value_map.end() && lb->first == location) {
      ...
      values->load_value_map.PutHint(lb, location, res);
    }
  }
  return res;
}
...
void LocalValueNumbering::PruneNonAliasingRefsForCatch() {
  for (const LocalValueNumbering* lvn : gvn_->merge_lvns_) {
    const BasicBlock* bb = gvn_->GetBasicBlock(gvn_->GetBasicBlockId(lvn->Id()));
...
void LocalValueNumbering::IntersectMaps() {
  ...
  for (const auto& entry : least_entries_lvn->*map_ptr) {
    ...
    if (checked) {
      (this->*map_ptr).PutHint((this->*map_ptr).end(), entry.first, entry.second);
    }
  }
}
...
void LocalValueNumbering::LoadValueMapUnion(
    ScopedArenaSafeMap<uint16_t, uint16_t>* work_map,
    const ScopedArenaSafeMap<uint16_t, uint16_t>& other_map) {
  ...
  for (const auto& entry : other_map) {
    ...
    if (work_it != work_end && !cmp(entry, *work_it)) {
      // Already present. The values may actually differ if one of the predecessors needs an
      // update. This is unlikely to happen if the GVN uses the topological sort ordering
      // (though an example can conceivably be constructed since the ordering is not truly
      // topological if there's a cycle) but it's pretty common if the DFS ordering is used.
    } else {
...
void LocalValueNumbering::MergeAliasingValueLocations(AliasingValues* work_values,
  ...
  while (store_it != store_end || load_it != load_end) {
    ...
    if (work_it != work_end && !cmp(loc, work_it->first)) {
      ...
      work_values->load_value_map.PutHint(work_it, loc, 0u);
    }
  }
}
...
void LocalValueNumbering::MergeAliasingValues(const typename Map::value_type& entry,
  ...
  (this->*map_ptr).PutHint(hint, key, AliasingValues(gvn_->allocator_));
  ...
  if (same_version) {
    ...
    for (const LocalValueNumbering* lvn : gvn_->merge_lvns_) {
      ...
      if (it != (lvn->*map_ptr).end()) {
        ...
        LoadValueMapUnion(&my_values->load_value_map, it->second.load_value_map);
      }
    }
  } else {
    ...
    for (const LocalValueNumbering* lvn : gvn_->merge_lvns_) {
      auto it = (lvn->*map_ptr).find(key);
      if (it != (lvn->*map_ptr).end()) {
        MergeAliasingValueLocations(my_values, &it->second);
      }
    }
