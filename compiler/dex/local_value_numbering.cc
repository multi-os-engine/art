...
uint16_t LocalValueNumbering::MergeSFieldValues(
  ...
  for (const LocalValueNumbering* lvn : lvns) {
    value_name = lvn->ResolvedSFieldValue(field_id, type);
...
void LocalValueNumbering::MergeRangeChecked(
    const ScopedArenaVector<const LocalValueNumbering*>& lvns) {
  // We won't do anything complicated for range checks, just calculate the intersection.
  DCHECK_GE(lvns.size(), 2u);

  // Find the LVN with the least entries in range_checked_.
  const LocalValueNumbering* least_entries_lvn = lvns[0];
  for (const LocalValueNumbering* lvn : lvns) {
    if (lvn->range_checked_.size() < least_entries_lvn->range_checked_.size()) {
      least_entries_lvn = lvn;
    }
  }

  // For each key check if it's in all the LVNs.
  for (const auto& key : least_entries_lvn->range_checked_) {
    bool checked = true;
    for (const LocalValueNumbering* lvn : lvns) {
      if (lvn != least_entries_lvn && lvn->range_checked_.count(key) == 0u) {
        checked = false;
        break;
      }
    }
...
void LocalValueNumbering::Merge(const ScopedArenaVector<const LocalValueNumbering*>& lvns) {
  ...
  SFieldToValueMap tmp_map(std::less<uint16_t>(), gvn_->Allocator()->Adapter());
  tmp_map.swap(sfield_value_map_);
...
void LocalValueNumbering::Merge(const LocalValueNumbering& other) {
  ...
  // Calculate the intersection of null checked references.
...
uint16_t LocalValueNumbering::ResolvedSFieldValue(uint16_t field_id, uint16_t type) const {
  ...
  if (it != sfield_value_map_.end()) {
    ...
    return gvn_->LookupValue(kResolvedSFieldOp, field_id,
...
uint16_t LocalValueNumbering::HandleSGet(MIR* mir, uint16_t opcode) {
  ...
  if (!field_info.IsResolved() || field_info.IsVolatile()) {
    ...
    uint16_t field_id = gvn_->GetFieldId(field_info, type);
    res = ResolvedSFieldValue(field_id, type);
  }
