...
bool TypeInference::Type::MergeStrong(Type src_type) {
  ...
  if (src_type.ArrayDepth() != 0u) {
    if (ArrayDepth() == 0u) {
      ...
      changed |= Copy(Type((raw_bits_ & kMaskNonArray) | kFlagRef |
...
bool TypeInference::InferTypeAndSize(BasicBlock* bb, MIR* mir) {
  ...
  if (ssa_rep != nullptr) {
    ...
    if ((attrs & DF_IS_MOVE) != 0) {
      ...
      // The only exception is an inlined move where we know the type from the originnal invoke.
      DCHECK(sregs_[used_mod_s_reg].NonNull() || !sregs_[defd_mod_s_reg].NonNull() ||
             (mir->optimization_flags & MIR_CALLEE) != 0);
      changed |= UpdateSRegFromLowWordType(used_mod_s_reg, sregs_[defd_mod_s_reg].AsNonNull());
