...
TypeInference::Type TypeInference::Type::ArrayType(uint32_t array_depth, Type nested_type) {
...
TypeInference::Type TypeInference::Type::ArrayTypeFromComponent(Type component_type) {
...
bool TypeInference::Type::MergeArrayTypeStrong(Type src_type) {
  ...
  if (ArrayDepth() == src_type.ArrayDepth()) {
    return MergeBits(src_type, kMaskArrayWideAndType);
  }
  if (src_type.ArrayDepth() == 1u &&
  ...
  return SetAll(Type((raw_bits_ & kMaskNonArray) | kFlagRef |
                     (1u << kArrayTypeShift) | kFlagArrayRef | kFlagArrayFp));
}

bool TypeInference::Type::MergeArrayTypeWeak(Type src_type) {
  DCHECK_NE(src_type.ArrayDepth(), 0u);
  if (ArrayDepth() == 0u) {
    DCHECK_EQ(raw_bits_ & ~kMaskNonArray, 0u);
    return false;
  ...
  if (ArrayDepth() == 1u &&
    ...
    return MergeBits(ObjectArrayType(), kMaskArrayWideAndType);
  }
  // Mark the array value type with conflict - both ref and fp.
  return SetAll(Type((raw_bits_ & kMaskNonArray) | kFlagRef |
                     (1u << kArrayTypeShift) | kFlagArrayRef | kFlagArrayFp));
...
bool TypeInference::Type::MergeStrong(Type src_type) {
  ...
  if (src_type.ArrayDepth() != 0u) {
    changed |= MergeArrayTypeStrong(src_type);
  }
  return changed;
}
...
bool TypeInference::Type::MergeWeak(Type src_type) {
  bool changed = MergeNonArrayFlags(src_type);
  if (src_type.ArrayDepth() != 0u) {
    changed |= MergeArrayTypeWeak(src_type);
  }
  return changed;
}
...
void TypeInference::CheckCastData::InitializeCheckCastSRegs(Type* sregs) const {
  for (const auto& entry : check_cast_map_) {
    ...
    sregs[entry.second.modified_s_reg] = entry.second.type;
  }
}
...
void TypeInference::CheckCastData::MergeCheckCastConflicts(Type* sregs) const {
  for (const auto& entry : check_cast_map_) {
    DCHECK_LT(static_cast<size_t>(entry.second.modified_s_reg), num_sregs_);
    sregs[entry.first->ssa_rep->uses[0]].MergeNonArrayFlags(sregs[entry.second.modified_s_reg]);
  }
}
  ...
  for (auto& entry : split_sreg_data_) {
    ...
    if (entry.second.def_phi_blocks_->IsBitSet(bb->id)) {
      ...
      if (UNLIKELY(!merged_type.IsDefined())) {
        ...
        sregs[entry.second.current_mod_s_reg].SetLowWord();
        sregs[entry.second.current_mod_s_reg].SetHighWord();
      } else {
        DCHECK(merged_type.Narrow() && merged_type.LowWord() && !merged_type.HighWord());
        changed |= sregs[entry.second.current_mod_s_reg].SetAll(merged_type);
        for (BasicBlockId pred_id : bb->predecessors) {
          DCHECK_LT(static_cast<size_t>(ending_mod_s_reg[pred_id]), num_sregs_);
...
bool TypeInference::CheckCastData::ProcessCheckCast(MIR* mir, Type* sregs) {
  ...
  return sregs[mir_it->second.modified_s_reg].MergeStrong(mir_it->second.type);
}
...
bool TypeInference::Apply(BasicBlock* bb) {
  ...
  for (MIR* mir = bb->first_mir_insn; mir != nullptr; mir = mir->next) {
    changed |= InferTypeAndSize(bb, mir);
  }
  return changed;
}
...
void TypeInference::Finish() {
  ...
  for (size_t s_reg = 0; s_reg != num_sregs; ++s_reg) {
    if (sregs_[s_reg].SizeConflict()) {
      ...
      // TODO: Update reg_location_s anyway.
      return;
    }
  }
    ...
    for (MIR* mir = bb->first_mir_insn; mir != nullptr; mir = mir->next) {
      ...
      switch (opcode) {
        ...
        case Instruction::CHECK_CAST:
          ...
          check_cast_data_->ProcessCheckCast(mir, sregs_);
          break;
        case Instruction::ARRAY_LENGTH:
          sregs_[ModifiedSReg(mir->ssa_rep->uses[0])].MergeStrong(Type::UnknownArrayType());
          break;
        ...
        case Instruction::FILLED_NEW_ARRAY_RANGE: {
          ...
          DCHECK_EQ(mir->ssa_rep->num_uses, mir->dalvikInsn.vA);
          int32_t* uses = mir->ssa_rep->uses;
          for (size_t next = 0u, num_uses = mir->ssa_rep->num_uses; next != num_uses; ++next) {
            int32_t input_mod_s_reg = ModifiedSReg(uses[next]);
            sregs_[input_mod_s_reg].MergeNonArrayFlags(component_type);
          }
          break;
        }
        ...
        case Instruction::INVOKE_VIRTUAL_RANGE_QUICK: {
          ...
          for (size_t i = 0, size = signature->num_params; i != size; ++i)  {
            ...
            UpdateSRegFromLowWordTypeStrong(param_s_reg, param_type);
            next += param_type.Wide() ? 2 : 1;
          }
          DCHECK_EQ(next, mir->ssa_rep->num_uses);
        ...
        case Instruction::RETURN_WIDE:
          ...
          DCHECK_EQ(mir->ssa_rep->uses[0] + 1, mir->ssa_rep->uses[1]);
          DCHECK_EQ(ModifiedSReg(mir->ssa_rep->uses[0]), mir->ssa_rep->uses[0]);
          FALLTHROUGH_INTENDED;
        case Instruction::RETURN:
        case Instruction::RETURN_OBJECT: {
          int32_t mod_s_reg = ModifiedSReg(mir->ssa_rep->uses[0]);
          UpdateSRegFromLowWordTypeStrong(mod_s_reg, current_method_signature_.return_type);
          break;
        }
        ...
        case Instruction::IPUT_SHORT_QUICK:
          DCHECK_LT(mir->meta.ifield_lowering_info, mir_graph_->GetIFieldLoweringInfoCount());
          UpdateSRegFromLowWordTypeStrong(ModifiedSReg(mir->ssa_rep->uses[0]),
...
bool TypeInference::UpdateSRegFromLowWordTypeStrong(int32_t mod_s_reg, Type low_word_type) {
...
bool TypeInference::UpdateSRegFromLowWordTypeWeak(int32_t mod_s_reg, Type low_word_type) {
  DCHECK(low_word_type.LowWord());
  bool changed = sregs_[mod_s_reg].MergeWeak(low_word_type);
  if (!sregs_[mod_s_reg].Narrow()) {  // Wide without conflict with narrow.
  ...
  if (ssa_rep != nullptr) {
    ...
    if ((attrs & DF_NULL_TRANSFER_N) != 0) {
      ...
      if (UNLIKELY(!merged_type.IsDefined())) {
        ...
        changed |= sregs_[defs[0]].SetAll(merged_type);
        for (size_t pred_idx = 0; pred_idx != ssa_rep->num_uses; ++pred_idx) {
          int32_t input_mod_s_reg = PhiInputModifiedSReg(uses[pred_idx], bb, pred_idx);
          changed |= UpdateSRegFromLowWordTypeStrong(input_mod_s_reg, merged_type);
        }
      }
    ...
    if ((attrs & DF_CHK_CAST) != 0) {
      DCHECK(check_cast_data_ != nullptr);
      changed |= sregs_[uses[0]].SetRef();
      changed |= check_cast_data_->ProcessCheckCast(mir, sregs_);
      return changed;
    }
...
#undef PROCESS
    ...
    // INVOKE, FILLED_NEW_ARRAY, IGET/IPUT/SGET/SPUT and RETURN have had their types
    // fully processed before. No special handling remains to be done here.
    ...
    if ((attrs & DF_IS_MOVE) != 0) {
      ...
      changed |= UpdateSRegFromLowWordTypeStrong(used_mod_s_reg, sregs_[defd_mod_s_reg]);
      changed |= UpdateSRegFromLowWordTypeStrong(defd_mod_s_reg, sregs_[used_mod_s_reg]);
      if (UNLIKELY((attrs & DF_REF_A) == 0 && sregs_[used_mod_s_reg].Ref())) {
        // Mark conflict: move instead of move-object.
        sregs_[used_mod_s_reg].SetCore();
      }
    }
    ...
    if ((attrs & DF_HAS_RANGE_CHKS) != 0) {
      ...
      if (sregs_[base_mod_s_reg].ArrayDepth() == 0u) {
        changed |= sregs_[base_mod_s_reg].MergeStrong(
            ((attrs & DF_REF_A) != 0) ? Type::ObjectArrayType() : Type::UnknownArrayType());
        DCHECK_NE(sregs_[base_mod_s_reg].ArrayDepth(), 0u);
      }

      changed |= UpdateSRegFromLowWordTypeStrong(mod_s_reg, sregs_[base_mod_s_reg].ComponentType());
      if ((attrs & DF_DA) != 0) {
        changed |= UpdateSRegFromLowWordTypeStrong(base_mod_s_reg,
                                                   Type::ArrayTypeFromComponent(sregs_[mod_s_reg]));
      } else {
        changed |= UpdateSRegFromLowWordTypeWeak(base_mod_s_reg,
                                                 Type::ArrayTypeFromComponent(sregs_[mod_s_reg]));
      }
      if (UNLIKELY((attrs & DF_REF_A) == 0 && sregs_[mod_s_reg].Ref())) {
        // Mark conflict: aget/aput instead of aget/aput-object.
