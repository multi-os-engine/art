...
#define DEF_MIR(opcode, bb, field_info) \
  ...
  void DoPrepareMethods(const MethodDef* defs, size_t count) {
    cu_.mir_graph->method_lowering_infos_.Reset();
    cu_.mir_graph->method_lowering_infos_.Resize(count);
    for (size_t i = 0u; i != count; ++i) {
      const MethodDef* def = &defs[i];
      MirMethodLoweringInfo method_info(def->method_idx, kStatic);
      if (def->declaring_dex_file != 0u) {
        ...
        method_info.declaring_method_idx_ = def->declaring_field_idx;
        method_info.flags_ = MirSFieldLoweringInfo::kFlagIsStatic;
      }
      cu_.mir_graph->method_lowering_infos_.Insert(method_info);
    }
  }
  ...
  void DoPrepareMIRs(const MIRDef* defs, size_t count) {
    ...
    for (size_t i = 0u; i != count; ++i) {
      ...
      } else if (def->opcode == Instruction::INVOKE_STATIC ||
          def->opcode == Instruction::INVOKE_STATIC_RANGE) {
...
TEST_F(ClassInitCheckEliminationTest, SingleBlock) {
  ...
  static const MethodDef methods[] = {
      { 0u, 1u, 0u, 0u },  // Match declaring dex file and class index to sfields[0].
      { 1u, 0u, 0u, 0u },  // Unresolved.
  };
  ...
  static const MIRDef mirs[] = {
      DEF_MIR(Instruction::INVOKE_STATIC, 3u, 1u),  // Unresolved.
      DEF_MIR(Instruction::INVOKE_STATIC, 3u, 0u),
...
TEST_F(ClassInitCheckEliminationTest, Diamond) {
  ...
  static const MethodDef methods[] = {
      { 0u, 1u, 0u, 0u },  // Match declaring dex file and class index to sfields[0].
      { 1u, 1u, 1u, 1u },  // Match declaring dex file and class index to sfields[1].
      { 2u, 0u, 0u, 0u },  // Unresolved.
  };
  static const BBDef bbs[] = {
      DEF_BB(kNullBlock, DEF_SUCC0(), DEF_PRED0()),
      DEF_BB(kEntryBlock, DEF_SUCC1(3), DEF_PRED0()),
  ...
  static const MIRDef mirs[] = {
      ...
      DEF_MIR(Instruction::INVOKE_STATIC, 3u, 2u),  // Unresolved.
      DEF_MIR(Instruction::INVOKE_STATIC, 3u, 0u),
      DEF_MIR(Instruction::SGET, 6u, 0u),  // Eliminated (block #3 dominates #6).
      DEF_MIR(Instruction::INVOKE_STATIC, 4u, 1u),
      ...
      DEF_MIR(Instruction::SGET, 6u, 9u),  // Eliminated (with sfield[8] in block #4).
  };
  static const bool expected_ignore_clinit_check[] = {
