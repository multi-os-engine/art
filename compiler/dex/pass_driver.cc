...
PassDriver::PassDriver(CompilationUnit* const cu, bool createDefaultPasses) : cu_(cu) {
...
void PassDriver::InsertPass(Pass *newPass, bool warnOverride) {
  ...
  pass_map_.Put(name, newPass);
  pass_list_.push_back(newPass);
}
...
bool PassDriver::RunPass(CompilationUnit *cUnit, Pass *curPass, bool timeSplit) {
  ...
  if (cUnit == 0 || curPass == 0 || (strcmp(curPass->GetName(), "") == 0)) {
    return false;
  }
