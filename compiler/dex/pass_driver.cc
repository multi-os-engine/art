...
static std::vector<const Pass*> gDefaultPassList;
...
void PassDriver::CreatePasses() {
  // If gDefaultPassList is empty, we will need to create it.
  if (gDefaultPassList.empty()) {
    CreateDefaultPassList(std::string(""));
  }
