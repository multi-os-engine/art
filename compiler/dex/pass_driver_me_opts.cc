...
template<>
uint16_t const PassDriver<PassDriverMEOpts>::g_passes_size = arraysize(PassDriver<PassDriverMEOpts>::g_passes);

// The default pass list is used by the PassDriverME instance of PassDriver to initialize pass_list_.
template<>
std::vector<const Pass*> PassDriver<PassDriverMEOpts>::g_default_pass_list(PassDriver<PassDriverMEOpts>::g_passes, PassDriver<PassDriverMEOpts>::g_passes + PassDriver<PassDriverMEOpts>::g_passes_size);
...
void PassDriverMEOpts::ApplyPass(PassDataHolder* data, const Pass* pass) {
  ...
  if ((pass_me->GetFlag(kOptimizationBasicBlockChange) == true) ||
    ...
    bool build_loop_info = pass_me->GetFlag(kLoopStructureChange) == true;
    CompilationUnit* c_unit = pass_me_data_holder->c_unit;
    c_unit->mir_graph.get()->CalculateBasicBlockInformation(build_loop_info);
  }
}
}  // namespace art
