...
void Mir2Lir::InstallLiteralPools() {
  ...
  // Push klass literals.
  data_lir = klass_literal_list_;
  while (data_lir != NULL) {
    uint32_t target = data_lir->operands[0];
    cu_->compiler_driver->AddKlassPatch(cu_->dex_file,
                                        cu_->class_def_idx,
                                        cu_->method_idx,
                                        target,
                                        code_buffer_.size());
...
int Mir2Lir::AssignLiteralOffset(CodeOffset offset) {
  ...
  offset = AssignLiteralPointerOffsetCommon(klass_literal_list_, offset);
  return offset;
}
