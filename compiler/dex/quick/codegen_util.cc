...
CompiledMethod* Mir2Lir::GetCompiledMethod() {
  ...
  // All relocations;
  FinalRelocations* all_relocs = new FinalRelocations();
  ...
  if (trampoline_calls_.size() != 0) {
    ...
    for (size_t i = 0 ; i < trampoline_calls_.size(); ++i) {
      uint64_t call = trampoline_calls_[i];
      uint32_t offset = static_cast<uint32_t>(call >> 32);
      uint32_t trampoline = static_cast<uint32_t>(call & 0xffffffffU);
      ep_relocs->Add(offset, trampoline);
    }
  ...
  CompiledMethod* result =
      new CompiledMethod(*cu_->compiler_driver, cu_->instruction_set, code_buffer_, frame_size_,
                         ...
                         vmap_encoder.GetData(), native_gc_map_, cfi_info.get(), all_relocs);
  return result;
}
