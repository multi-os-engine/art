...
void Mir2Lir::InstallLiteralPools() {
  ...
  while (data_lir != NULL) {
    ...
    const DexFile* target_dex_file = reinterpret_cast<const DexFile*>(data_lir->operands[1]);
...
void Mir2Lir::LoadCodeAddress(const MethodReference& target_method, InvokeType type,
  ...
  if (data_target == NULL) {
    ...
    data_target->operands[1] = reinterpret_cast<uint32_t>(target_method.dex_file);
