...
class LazyDebugFrameOpCodeWriter FINAL : private DebugFrameOpCodeWriter<ArenaAllocatorAdapter<uint8_t>> {
 ...
 public:
  // Since the opcodes are lazy, we do not move the PC until patch time.
  static constexpr int kZeroPC = 0;

  // The register was unspilled.
  void Restore(int reg) {
    if (enable_writes_) {
      LazyAdvancePC();
      Base::Restore(kZeroPC, reg);
    }
  }
  ...
  void DefCFAOffset(int offset) {
    if (enable_writes_) {
      LazyAdvancePC();
      Base::DefCFAOffset(kZeroPC, offset);
    }
  ...
  void Offset(int reg, int offset) {
    if (enable_writes_) {
      LazyAdvancePC();
      Base::Offset(kZeroPC, reg, offset);
    }
  }
 ...
 private:
  ...
  void LazyAdvancePC() {
    ...
    DCHECK_EQ(this->current_pc_, kZeroPC);
