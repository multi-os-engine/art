...
#define NEW_CHECK_REG_DEP(mask, check) (mask.Intersects(*check->u.m.def_mask))
...
void Mir2Lir::ApplyLoadStoreElimination(LIR* head_lir, LIR* tail_lir) {
  ...
  for (this_lir = head_lir; this_lir != tail_lir; this_lir = NEXT_LIR(this_lir)) {
     ...
     */
    if ((target_flags & IS_BRANCH) || (target_flags & IS_EXCLUSIVE) ||
        ((target_flags & (IS_LOAD | IS_STORE)) == (IS_LOAD | IS_STORE)) ||
        ((target_flags & REG_USE012) == REG_USE012) ||
        ((target_flags & REG_DEF01) == REG_DEF01) ||
    ...
    for (check_lir = NEXT_LIR(this_lir); check_lir != tail_lir; check_lir = NEXT_LIR(check_lir)) {
       ...
       *  * Exclusive load/store
       */
      if (((check_flags & REG_USE012) == REG_USE012) ||
          ((check_flags & REG_DEF01) == REG_DEF01) ||
           (check_flags & IS_BRANCH) ||
           (check_flags & IS_EXCLUSIVE) ||
           (check_flags & IS_UNARY_OP)) {
        stop_here = true;
      ...
      /* Memory regions */
        ...
        if (alias_mem_mask.Equals(kEncodeLiteral)) {
          ...
          if (reg_compatible && same_offset &&
            ...
            if (check_lir->operands[0] != native_reg_id) {
              if (RegStorage::Is32Bit(native_reg_id)) {
                ConvertMemOpIntoMove(check_lir, RegStorage::Solo32(check_lir->operands[0]),
                                     RegStorage::Solo32(native_reg_id));
                NopLIR(check_lir);
              } else if (RegStorage::Is64BitSolo(native_reg_id)) {
                ConvertMemOpIntoMove(check_lir, RegStorage::Solo64(check_lir->operands[0]),
                                     RegStorage::Solo64(native_reg_id));
                NopLIR(check_lir);
              } else if (RegStorage::IsFloat(native_reg_id)) {
                ConvertMemOpIntoMove(check_lir, RegStorage::FloatSolo32(check_lir->operands[0]),
                                     RegStorage::FloatSolo32(native_reg_id));
                NopLIR(check_lir);
              } else if (RegStorage::IsDouble(native_reg_id)) {
                ConvertMemOpIntoMove(check_lir, RegStorage::FloatSolo64(check_lir->operands[0]),
                                     RegStorage::FloatSolo64(native_reg_id));
                NopLIR(check_lir);
              }
            } else {
              /* Same register */
      ...
      if (stop_here == false) {
        bool stop_alias = NEW_CHECK_REG_DEP(alias_reg_list_mask, check_lir);
        if (stop_alias) {
          ...
          for (auto &reg : alias_list) {
            ...
            stop_alias = NEW_CHECK_REG_DEP(alias_reg_mask, check_lir);
            if (stop_alias) {
        ...
        stop_here = NEW_CHECK_REG_DEP(stop_search_mask, check_lir);
        if (stop_here) {
          break;
        }
      } else {
