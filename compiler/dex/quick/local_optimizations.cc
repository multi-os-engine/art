...
static bool CheckEliminateDalvikReg(LIR* this_lir, LIR* check_lir,
                                    ...
                                    bool this_is_64bit,
                                    Mir2Lir* m2l) {
  // The base register should be the same (SP).
  DCHECK_EQ(this_lir->operands[1], check_lir->operands[1]);
  ...
  if (this_is_64bit && this_alias_reg + 1 == check_alias_reg) {
    // Clobbering upper half of 64b register.
    same_offset = true;
  } else if (DECODE_ALIAS_INFO_WIDE(check_lir->flags.alias_info) &&
      this_alias_reg == check_alias_reg + 1) {
    // This 64b store is clobbering us.
    same_offset = true;
  }
...
static bool CheckEliminateHeapRefs(LIR* this_lir, LIR* check_lir,
  ...
  bool same_offset = (m2l->GetInstructionOffset(this_lir) == check_lir_offset);

  // Base registers are aliases.
  if (alias_reg_list_mask.Intersects((check_lir->u.m.use_mask)->Without(kEncodeMem))) {
    ...
    // partially (have pointers into the middle of the object), so this should be correct.
      ...
      /* Memory regions */
        ...
        if (!alias_mem_mask.Intersects(kEncodeMem) && alias_mem_mask.Equals(kEncodeLiteral)) {
          ...
          if (CheckEliminateHeapRefs(this_lir, check_lir, reg_compatible, is_check_lir_load,
                                     is_this_lir_load, native_reg_id, alias_reg_list_mask, this)) {
            break;  // Stop here.
          }
        } else {
