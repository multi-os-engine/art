...
class DexFileMethodInliner {
  public:
     ...
     */
    bool AnalyseMethodCode(verifier::MethodVerifier* verifier, Thread* self)
        SHARED_REQUIRES(Locks::mutator_lock_) REQUIRES(!lock_);

    /**
     * Check whether a particular method index corresponds to an intrinsic or special function.
     */
    InlineMethodFlags IsIntrinsicOrSpecial(uint32_t method_index, Thread* self) REQUIRES(!lock_);

    /**
     * Check whether a particular method index corresponds to an intrinsic function.
     */
    bool IsIntrinsic(uint32_t method_index, InlineMethod* intrinsic, Thread* self) REQUIRES(!lock_);

    /**
     * Generate code for an intrinsic function invocation.
     */
    bool GenIntrinsic(Mir2Lir* backend, CallInfo* info, Thread* self) REQUIRES(!lock_);

    /**
     * Check whether a particular method index corresponds to a special function.
     */
    bool IsSpecial(uint32_t method_index, Thread* self) REQUIRES(!lock_);

    /**
     * Generate code for a special function.
     */
    bool GenSpecial(Mir2Lir* backend, uint32_t method_idx, Thread* self) REQUIRES(!lock_);

    /**
     * Try to inline an invoke.
     */
    bool GenInline(MIRGraph* mir_graph, BasicBlock* bb, MIR* invoke, uint32_t method_idx,
                   Thread* self) REQUIRES(!lock_);

    /**
     * Gets the thread pointer entrypoint offset for a string init method index and pointer size.
     */
    uint32_t GetOffsetForStringInit(uint32_t method_index, size_t pointer_size, Thread* self)
        REQUIRES(!lock_);

    /**
     * Check whether a particular method index is a string init.
     */
    bool IsStringInitMethodIndex(uint32_t method_index, Thread* self) REQUIRES(!lock_);
  ...
  private:
    ...
    bool AddInlineMethod(int32_t method_idx, const InlineMethod& method, Thread* self)
        REQUIRES(!lock_);
