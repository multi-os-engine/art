...
void Mir2Lir::GenSmallPackedSwitch(MIR* mir, DexOffset table_offset, RegLocation rl_src) {
  ...
  rl_src = LoadValue(rl_src, kCoreReg);
  int i = 0;
  for (; i < entries; i++, current_key++, iterator++) {
    if (!InexpensiveConstantInt(current_key, Instruction::Code::IF_EQ)) {
      // Switch to using a temp and add.
      break;
    }
    successor_block_info = *iterator;
    DCHECK(successor_block_info != nullptr);
    int case_block_id = successor_block_info->block;
    DCHECK(current_key == successor_block_info->key);
    OpCmpImmBranch(kCondEq, rl_src.reg, current_key, &block_label_list_[case_block_id]);
  }
  if (i < entries) {
    ...
    if (key_temp.Valid()) {
      ...
      for (; i < entries - 1; i++, current_key++, iterator++) {
        successor_block_info = *iterator;
        DCHECK(successor_block_info != nullptr);
        int case_block_id = successor_block_info->block;
        DCHECK(current_key == successor_block_info->key);
