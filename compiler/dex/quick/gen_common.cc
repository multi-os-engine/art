      ...
      class SlowPath : public LIRSlowPath {
       public:
        ...
        void Compile() {
          ...
          const CompilerOptions& compiler_options = m2l_->cu_->compiler_driver->GetCompilerOptions();
          if (compiler_options.GetGenerateHelperTrampolines()) {
            m2l_->OpRegCopy(m2l_->TargetReg(kArg0), r_method_);   // .eq
            m2l_->CallHelper(RegStorage::InvalidReg(), QUICK_ENTRYPOINT_OFFSET(pResolveString),
                             true);
          } else {
