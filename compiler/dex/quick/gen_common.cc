...
void Mir2Lir::GenSput(MIR* mir, RegLocation rl_src, bool is_long_or_double,
  ...
  if (annotation.FastPut() && !SLOW_FIELD_PATH) {
    ...
    if (is_long_or_double) {
      StoreBaseDispWide(r_base, annotation.FieldOffset().Int32Value(), rl_src.reg.GetReg(),
                        rl_src.reg.GetHighReg());
    } else {
      StoreWordDisp(r_base, annotation.FieldOffset().Int32Value(), rl_src.reg.GetReg());
    }
    if (annotation.IsVolatile()) {
...
void Mir2Lir::GenSget(MIR* mir, RegLocation rl_dest,
  ...
  if (annotation.FastGet() && !SLOW_FIELD_PATH) {
    ...
    if (is_long_or_double) {
      LoadBaseDispWide(r_base, annotation.FieldOffset().Int32Value(), rl_result.reg.GetReg(),
                       rl_result.reg.GetHighReg(), INVALID_SREG);
    } else {
      LoadWordDisp(r_base, annotation.FieldOffset().Int32Value(), rl_result.reg.GetReg());
    }
    FreeTemp(r_base);
    if (is_long_or_double) {
      StoreValueWide(rl_dest, rl_result);
  ...
  if (annotation.FastGet() && !SLOW_FIELD_PATH) {
    ...
    if (is_long_or_double) {
      ...
      if (cu_->instruction_set == kX86) {
        ...
        GenNullCheck(rl_obj.s_reg_low, rl_obj.reg.GetReg(), opt_flags);
        LoadBaseDispWide(rl_obj.reg.GetReg(), annotation.FieldOffset().Int32Value(),
        ...
        OpRegRegImm(kOpAdd, reg_ptr, rl_obj.reg.GetReg(), annotation.FieldOffset().Int32Value());
        rl_result = EvalLoc(rl_dest, reg_class, true);
      ...
      rl_result = EvalLoc(rl_dest, reg_class, true);
      GenNullCheck(rl_obj.s_reg_low, rl_obj.reg.GetReg(), opt_flags);
      LoadBaseDisp(rl_obj.reg.GetReg(), annotation.FieldOffset().Int32Value(),
...
void Mir2Lir::GenIPut(MIR* mir, int opt_flags, OpSize size,
  ...
  if (annotation.FastPut() && !SLOW_FIELD_PATH) {
    ...
    if (is_long_or_double) {
      ...
      StoreBaseDisp(rl_obj.reg.GetReg(), annotation.FieldOffset().Int32Value(),
