...
void Mir2Lir::GenSput(MIR* mir, RegLocation rl_src, bool is_long_or_double,
  ...
  if (annotation.fast_put && !SLOW_FIELD_PATH) {
    DCHECK_GE(annotation.field_offset.Int32Value(), 0);
    int r_base;
    if (annotation.is_referrers_class) {
      ...
      DCHECK_GE(annotation.storage_index, 0);
      ...
      LoadWordDisp(r_base, mirror::Array::DataOffset(sizeof(mirror::Object*)).Int32Value() +
                   sizeof(int32_t*) * annotation.storage_index, r_base);
      // r_base now points at static storage (Class*) or NULL if the type is not yet resolved.
      if (!annotation.is_initialized) {
        ...
        AddSlowPath(new (arena_) StaticFieldSlowPath(this,
                                                     ...
                                                     annotation.storage_index, r_base));
    ...
    if (is_long_or_double) {
      StoreBaseDispWide(r_base, annotation.field_offset.Int32Value(), rl_src.low_reg,
                        rl_src.high_reg);
    } else {
      StoreWordDisp(r_base, annotation.field_offset.Int32Value(), rl_src.low_reg);
    }
    if (annotation.is_volatile) {
    ...
    CallRuntimeHelperImmRegLocation(setter_offset, annotation.field_idx, rl_src, true);
  }
}
...
void Mir2Lir::GenSget(MIR* mir, RegLocation rl_dest,
  ...
  if (annotation.fast_get && !SLOW_FIELD_PATH) {
    ...
    if (is_long_or_double) {
      LoadBaseDispWide(r_base, annotation.field_offset.Int32Value(), rl_result.low_reg,
                       rl_result.high_reg, INVALID_SREG);
    } else {
      LoadWordDisp(r_base, annotation.field_offset.Int32Value(), rl_result.low_reg);
    }
    FreeTemp(r_base);
    if (is_long_or_double) {
      StoreValueWide(rl_dest, rl_result);
    ...
    CallRuntimeHelperImm(getterOffset, annotation.field_idx, true);
    if (is_long_or_double) {
  ...
  if (annotation.fast_get && !SLOW_FIELD_PATH) {
    ...
    if (is_long_or_double) {
      ...
      if (cu_->instruction_set == kX86) {
        ...
        GenNullCheck(rl_obj.s_reg_low, rl_obj.low_reg, opt_flags);
        LoadBaseDispWide(rl_obj.low_reg, annotation.field_offset.Int32Value(), rl_result.low_reg,
        ...
        OpRegRegImm(kOpAdd, reg_ptr, rl_obj.low_reg, annotation.field_offset.Int32Value());
        rl_result = EvalLoc(rl_dest, reg_class, true);
      ...
      rl_result = EvalLoc(rl_dest, reg_class, true);
      GenNullCheck(rl_obj.s_reg_low, rl_obj.low_reg, opt_flags);
      LoadBaseDisp(rl_obj.low_reg, annotation.field_offset.Int32Value(), rl_result.low_reg,
...
void Mir2Lir::GenIPut(MIR* mir, int opt_flags, OpSize size,
  ...
  if (annotation.fast_put && !SLOW_FIELD_PATH) {
    ...
    if (is_long_or_double) {
      ...
      StoreBaseDisp(rl_obj.low_reg, annotation.field_offset.Int32Value(), rl_src.low_reg, kWord);
      if (annotation.is_volatile) {
        GenMemBarrier(kLoadLoad);
      }
      if (is_object && !mir_graph_->IsConstantNullRef(rl_src)) {
