...
void Mir2Lir::GenInstanceofFinal(bool use_declaring_class, uint32_t type_idx, RegLocation rl_dest,
  ...
  if (cu_->instruction_set == kThumb2) {
    ...
    GenSelect(check_class, object_class, kCondEq, 1, 0, result_reg, kCoreReg);
  }
  LIR* target = NewLIR0(kPseudoTargetLabel);
...
void Mir2Lir::GenInstanceofCallingHelper(bool needs_access_check, bool type_known_final,
      ...
      class InitTypeSlowPath : public Mir2Lir::LIRSlowPath {
       public:
        ...
        void Compile() OVERRIDE {
          ...
          m2l_->LoadValueDirectFixed(rl_src_, m2l_->TargetReg(kArg0, kRef));  /* reload Ref */
  ...
  if (type_known_final) {
    ...
    GenSelect(TargetReg(kArg1, kRef), TargetReg(kArg2, kRef), kCondEq, 1, 0, rl_result.reg,
              kCoreReg);
  } else {
    if (cu_->instruction_set == kThumb2) {
