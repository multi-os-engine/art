...
bool Mir2Lir::HandleEasyFloatingPointDiv(RegLocation rl_dest, RegLocation rl_src1,
  ...
  if (!rl_src2.wide) {
    int32_t dividend = mir_graph_->ConstantValue(rl_src2);
    if (CanDivideByReciprocalMultiplyFloat(dividend)) {
      // Generate multiply by reciprocal instead of div.
      float recip = 1.0f/bit_cast<int32_t, float>(dividend);
      GenMultiplyByConstantFloat(rl_dest, rl_src1, bit_cast<float, int32_t>(recip));
      return true;
    }
  } else {
    int64_t dividend = mir_graph_->ConstantValueWide(rl_src2);
    if (CanDivideByReciprocalMultiplyDouble(dividend)) {
      // Generate multiply by reciprocal instead of div.
      double recip = 1.0/bit_cast<double, int64_t>(dividend);
