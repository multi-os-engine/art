...
void Mir2Lir::GenFilledNewArray(CallInfo* info) {
  ...
  if (info->is_range && (elems > 0)) {
    ...
    OpRegRegImm(kOpAdd, r_src, TargetRefReg(kSp), SRegOffset(rl_first.s_reg_low));
...
static void GenArithOpLongImpl(Mir2Lir* mir_to_lir, CompilationUnit* cu, Instruction::Code opcode,
  ...
  if (!call_out) {
    ...
    mir_to_lir->FlushAllRegs();   /* Send everything to home location */
    if (check_zero) {
      RegStorage r_tmp1 = RegStorage::MakeRegPair(mir_to_lir->TargetReg(kArg0, false),
                                                  mir_to_lir->TargetReg(kArg1, false));
      RegStorage r_tmp2 = RegStorage::MakeRegPair(mir_to_lir->TargetReg(kArg2, false),
                                                  mir_to_lir->TargetReg(kArg3, false));
      mir_to_lir->LoadValueDirectWideFixed(rl_src2, r_tmp2);
      RegStorage r_tgt = mir_to_lir->CallHelperSetup(func_offset);
      mir_to_lir->GenDivZeroCheckWide(RegStorage::MakeRegPair(mir_to_lir->TargetReg(kArg2, false),
                                                              mir_to_lir->TargetReg(kArg3, false)));
