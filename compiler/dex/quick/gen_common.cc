...
void Mir2Lir::GenShortPackedSwitch(MIR* mir, DexOffset table_offset, RegLocation rl_src) {
...
void Mir2Lir::GenPackedSwitch(MIR* mir, DexOffset table_offset, RegLocation rl_src) {
  ...
  const uint16_t entries = table[1];
  if (entries <= kShortSwitchThreshold) {
    GenShortPackedSwitch(mir, table_offset, rl_src);
  } else {
    // Use the backend-specific implementation.
    GenLongPackedSwitch(mir, table_offset, rl_src);
  }
}

void Mir2Lir::GenShortSparseSwitch(MIR* mir, DexOffset table_offset, RegLocation rl_src) {
  const uint16_t* table = cu_->insns + current_dalvik_offset_ + table_offset;
  const uint16_t entries = table[1];
  // Chained cmp-and-branch.
...
void Mir2Lir::GenSparseSwitch(MIR* mir, DexOffset table_offset, RegLocation rl_src) {
  ...
  const uint16_t entries = table[1];
  if (entries <= kShortSwitchThreshold) {
    GenShortSparseSwitch(mir, table_offset, rl_src);
  } else {
    // Use the backend-specific implementation.
    GenLongSparseSwitch(mir, table_offset, rl_src);
  }
}
