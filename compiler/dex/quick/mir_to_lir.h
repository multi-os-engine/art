...
class Mir2Lir : public Backend {
  public:
    ...
    struct RegisterInfo {
      ...
      int reg;                    // Reg number.
      RegStorage rs;
      bool in_use_;                // Has it been allocated?
      bool is_temp;               // Can allocate as temp?
      BitsUsed value_size;        // Size of the Dalvik value held (starting w/ s_reg)
      ...
      uint64_t def_use_mask;      // Resources for this element.
      uint32_t used_storage;      // 1 bit per 4 bytes of storage. Unused by aliases.
      uint32_t* master_storage;   // Pointer to controlling storage mask.
      uint32_t storage_mask;      // Track allocation of sub-units.
    ...
    struct RegisterPool {
       ...
       int next_core_reg;
       // TODO: does it make sense to split into single-precision & double-precision?
       int num_fp_regs;
       RegisterInfo *FPRegs;
       int next_fp_reg;
     };
