...
void Mir2Lir::MarkLiveReg(RegStorage reg, int s_reg) {
  RegisterInfo* info = GetRegInfo(reg);
  if (!info->IsTemp() || ((info->SReg() == s_reg) && info->IsLive())) {
    return;  // Already live.
  }
  if (info->IsLive()) {
    // Revoke old liveness associated with register.
    ClobberSReg(info->SReg());
  }
  // Revoke old liveness associated with s_reg.
  ClobberSReg(s_reg);
  info->SetIsLive(s_reg != INVALID_SREG);
  info->SetSReg(s_reg);
}
