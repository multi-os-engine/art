...
#if 1
// Temporarily mimic allocation strategy of previous version to assist A/B testing.
RegStorage Mir2Lir::AllocTempDouble() {
  int next = reg_pool_->next_sp_reg_ >> 1;  // Get sp_next and align to next even reg.
  RegStorage res = AllocTempBody(reg_pool_->dp_regs_, &next, true);
  if (!res.IsDouble()) {
  ...
  next = next << 1;
  if (next >= static_cast<int>(reg_pool_->sp_regs_.Size())) {
    next = 0;
  }
  reg_pool_->next_sp_reg_ = next;
  return res;
}
#else
RegStorage Mir2Lir::AllocTempDouble() {
