...
int Mir2Lir::SRegToPMap(int s_reg) {
  ...
  if (v_reg >= 0) {
     ...
     */
    DCHECK_LE(v_reg, CTEMP_BASE_REG);
    unsigned int position = std::abs(v_reg) - std::abs(CTEMP_BASE_REG);

    // The temporaries are placed after dalvik registers in the promotion map
    DCHECK_LT(position, mir_graph_->GetNumCompilerTemps());
...
void Mir2Lir::DoPromotion() {
  ...
  int num_regs = dalvik_regs + mir_graph_->GetNumCompilerTemps();
  ...
  for (unsigned int ct_idx = 0; ct_idx < mir_graph_->GetNumCompilerTemps(); ct_idx++) {
