...
Mir2Lir::RegisterPool::RegisterPool(Mir2Lir* m2l, ArenaAllocator* arena,
  ...
  for (std::vector<RegStorage>::const_iterator it = core_regs.begin(); it != core_regs.end();
       ++it) {
    RegisterInfo* info = new (arena) RegisterInfo(*it, m2l_->GetRegMaskCommon(*it));
    m2l_->reginfo_map_.Put(it->GetReg(), info);
    core_regs_.Insert(info);
  }
  for (std::vector<RegStorage>::const_iterator it = sp_regs.begin(); it != sp_regs.end(); ++it) {
    RegisterInfo* info = new (arena) RegisterInfo(*it, m2l_->GetRegMaskCommon(*it));
    m2l_->reginfo_map_.Put(it->GetReg(), info);
    sp_regs_.Insert(info);
  }
  for (std::vector<RegStorage>::const_iterator it = dp_regs.begin(); it != dp_regs.end(); ++it) {
    RegisterInfo* info = new (arena) RegisterInfo(*it, m2l_->GetRegMaskCommon(*it));
    m2l_->reginfo_map_.Put(it->GetReg(), info);
  ...
  for (std::vector<RegStorage>::const_iterator it = reserved_regs.begin();
       it != reserved_regs.end(); ++it) {
    m2l_->MarkInUse(*it);
  }

  // Mark temp regs - all others not in use can be used for promotion
  for (std::vector<RegStorage>::const_iterator it = core_temps.begin(); it != core_temps.end();
       ++it) {
    m2l_->MarkTemp(*it);
  }
  for (std::vector<RegStorage>::const_iterator it = sp_temps.begin(); it != sp_temps.end(); ++it) {
    m2l_->MarkTemp(*it);
  }
  for (std::vector<RegStorage>::const_iterator it = dp_temps.begin(); it != dp_temps.end(); ++it) {
    m2l_->MarkTemp(*it);
  }
}
...
void Mir2Lir::DumpRegPool(GrowableArray<RegisterInfo*>* regs) {
  ...
  for (RegisterInfo* info = it.Next(); info != nullptr; info = it.Next()) {
    LOG(INFO) << StringPrintf(
        "R[%d:%d:%c]: T:%d, U:%d, W:%d, p:%d, LV:%d, D:%d, SR:%d",
...
// Reserve a callee-save dp single register.
...
RegStorage Mir2Lir::AllocTempSingle() {
  return AllocTempBody(reg_pool_->sp_regs_, &reg_pool_->next_sp_reg_, true);
}

RegStorage Mir2Lir::AllocTempDouble() {
  return AllocTempBody(reg_pool_->dp_regs_, &reg_pool_->next_dp_reg_, true);
}

Mir2Lir::RegisterInfo* Mir2Lir::AllocLiveBody(GrowableArray<RegisterInfo*> &regs, int s_reg) {
  if (s_reg == -1)
    return NULL;
  GrowableArray<RegisterInfo*>::Iterator it(&regs);
  for (RegisterInfo* info = it.Next(); info != nullptr; info = it.Next()) {
...
Mir2Lir::RegisterInfo* Mir2Lir::AllocLive(int s_reg, int reg_class, bool wide) {
  RegisterInfo* res = NULL;
  switch (reg_class) {
    case kAnyReg:
      // We assume that for all targets, only w/ floating point could we have 2 distinct reg types.
      if (wide) {
        res = AllocLiveBody(reg_pool_->dp_regs_, s_reg);
      } else {
        res = AllocLiveBody(reg_pool_->sp_regs_, s_reg);
      }
      if (res)
        break;
      /* Intentional fallthrough */
    case kCoreReg:
      res = AllocLiveBody(reg_pool_->core_regs_, s_reg);
      break;
    case kFPReg:
      if (wide) {
        res = AllocLiveBody(reg_pool_->dp_regs_, s_reg);
...
void Mir2Lir::MarkLive(RegLocation loc) {
  ...
  if ((info->SReg() == loc.s_reg_low) && info->IsLive()) {
    return;  /* already live */
...
void Mir2Lir::CopyRegInfo(RegStorage new_reg, RegStorage old_reg) {
  ...
  bool is_temp = new_info->IsTemp();
  bool live = new_info->IsLive();
  bool dirty = new_info->IsDirty();
  *new_info = *old_info;
  // Restore target's temp, live, dirty status
  new_info->SetIsTemp(is_temp);
  new_info->SetIsLive(live);
  new_info->SetIsDirty(dirty);
}
...
void Mir2Lir::CopyRegInfoWide(RegStorage new_reg, RegStorage old_reg) {
  ...
  if (new_pair == old_pair) {
    if (new_pair) {
      CopyRegInfo(new_reg.GetLow(), old_reg.GetLow());
      CopyRegInfo(new_reg.GetHigh(), old_reg.GetHigh());
    } else {
      CopyRegInfo(new_reg, old_reg);
    }
  } else if (new_pair) {
    // New is a pair, old is a solo.  Will need to fix up sreg of high.
    CopyRegInfo(new_reg.GetLow(), old_reg);
    CopyRegInfo(new_reg.GetHigh(), old_reg);
    GetRegInfo(new_reg.GetHigh())->SetSReg(GetRegInfo(new_reg.GetLow())->SReg() + 1);
  } else {
...
RegLocation Mir2Lir::UpdateLoc(RegLocation loc) {
  ...
  if (loc.location != kLocPhysReg) {
    ...
    RegisterInfo* info_lo = AllocLive(loc.s_reg_low, kAnyReg, false);
    if (info_lo) {
      if (info_lo->IsWide()) {
        Clobber(info_lo->GetReg());
        if (info_lo->GetReg() != info_lo->Partner()) {
          Clobber(info_lo->Partner());
        }
        FreeTemp(info_lo->GetReg());
      } else {
...
// Re-intergrate x86 specialized version.
...
RegLocation Mir2Lir::UpdateLocWide(RegLocation loc) {
  ...
  if (loc.location != kLocPhysReg) {
    ...
    RegisterInfo* info_lo = AllocLive(loc.s_reg_low, kAnyReg, true);
    RegisterInfo* info_hi;
    bool register_pair = ((info_lo != nullptr) && (info_lo->IsWide()) &&
                          (info_lo->GetReg() != info_lo->Partner()));
    if (register_pair) {
      // Look for the allocation status of the high reg.
      info_hi = AllocLive(GetSRegHi(loc.s_reg_low), kAnyReg, true);
    } else {
      info_hi = info_lo;
    }
    bool match = true;
    ...
    match = match && info_lo->GetReg().IsFloat() == info_hi->GetReg().IsFloat();
    // If a pair of floating point singles, are they properly aligned?
    // TODO: eliminate this case once ARM and MIPS pair->double update complete.
    if (match && register_pair && info_lo->GetReg().IsFloat()) {
      match &= ((info_lo->GetReg().GetRegNum() & 0x1) == 0);
      match &= ((info_hi->GetReg().GetRegNum() - info_lo->GetReg().GetRegNum()) == 1);
    }
    // If previously used as a pair, it is the same pair?
    if (match && register_pair) {
      match = (info_lo->IsWide() == info_hi->IsWide());
      match &= ((info_lo->GetReg() == info_hi->Partner()) &&
            (info_hi->GetReg() == info_lo->Partner()));
    }
    if (match && !register_pair) {
    ...
    if (match) {
      ...
      loc.location = kLocPhysReg;
      if (register_pair) {
        loc.reg = RegStorage::MakeRegPair(info_lo->GetReg(), info_hi->GetReg());
      } else {
        loc.reg = info_lo->GetReg();
      }
      MarkWide(loc.reg);
    ...
    if (info_lo) {
      Clobber(info_lo->GetReg());
      FreeTemp(info_lo->GetReg());
      if (info_lo->IsWide() && (info_lo->GetReg() != info_lo->Partner()))
        Clobber(info_lo->Partner());
    }
    if (info_hi && (info_hi != info_lo)) {
      Clobber(info_hi->GetReg());
      FreeTemp(info_hi->GetReg());
      if (info_hi->IsWide() && (info_hi->GetReg() != info_hi->Partner())) {
        ...
        if ((promotion_map_[p_map_idx].fp_location == kLocPhysReg) &&
          ...
          if (((low_reg & 0x1) == 0) && ((low_reg + 1) == high_reg)) {
            ...
            if (cu_->instruction_set == kThumb2) {
              int dreg = ((low_reg & RegStorage::kRegNumMask) >> 1) | RegStorage::kFloatingPoint;
