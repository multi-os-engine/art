...
void Mir2Lir::DoPromotion() {
  ...
  for (int i = 0; i < mir_graph_->GetNumSSARegs(); i++) {
    ...
    if (curr->fp && promotion_map_[p_map_idx].fp_location == kLocPhysReg) {
      if (curr->wide && cu_->instruction_set == kThumb2) {
        if (promotion_map_[p_map_idx + 1].fp_location == kLocPhysReg) {
        ...
        reg = curr->wide ? RegStorage::FloatSolo64(reg_num) : RegStorage::FloatSolo32(reg_num);
      }
    } else if (!curr->fp && promotion_map_[p_map_idx].core_location == kLocPhysReg) {
      if (curr->wide && !cu_->target64) {
        ...
        reg = curr->wide ? RegStorage::Solo64(reg_num) : RegStorage::Solo32(reg_num);
      }
    }
    if (reg.Valid()) {
      curr->reg = reg;
