...
RegStorage Mir2Lir::LoadArg(int in_position, RegisterClass reg_class, bool wide) {
  ...
  if (cu_->target64) {
    ...
    if (!reg_arg.Valid()) {
      ...
      LoadBaseDisp(TargetRefReg(kSp), offset, new_reg, wide ? k64 : k32, kNotVolatile);
      return new_reg;
    } else {
  ...
  if (wide && !reg_arg_high.Valid()) {
    ...
    if (!reg_arg_low.Valid()) {
      ...
      LoadBaseDisp(TargetRefReg(kSp), offset, new_regs, k64, kNotVolatile);
      ...
      Load32Disp(TargetRefReg(kSp), offset_high, reg_arg_high);
  ...
  if (!reg_arg_low.Valid()) {
    ...
    Load32Disp(TargetRefReg(kSp), offset, reg_arg_low);
...
void Mir2Lir::LoadArgDirect(int in_position, RegLocation rl_dest) {
  ...
  if (!rl_dest.wide) {
    ...
    if (reg.Valid()) {
      ...
      Load32Disp(TargetRefReg(kSp), offset, rl_dest.reg);
    }
  } else {
    if (cu_->target64) {
      RegStorage reg = GetArgMappingToPhysicalReg(in_position);
      if (reg.Valid()) {
        ...
        LoadBaseDisp(TargetRefReg(kSp), offset, rl_dest.reg, k64, kNotVolatile);
      }
      return;
    }
    ...
    if (reg_arg_low.Valid() && reg_arg_high.Valid()) {
      ...
      int offset_high = offset + sizeof(uint32_t);
      Load32Disp(TargetRefReg(kSp), offset_high, rl_dest.reg.GetHigh());
    } else if (!reg_arg_low.Valid() && reg_arg_high.Valid()) {
      OpRegCopy(rl_dest.reg.GetHigh(), reg_arg_high);
      Load32Disp(TargetRefReg(kSp), offset, rl_dest.reg.GetLow());
    } else {
      LoadBaseDisp(TargetRefReg(kSp), offset, rl_dest.reg, k64, kNotVolatile);
    }
  }
