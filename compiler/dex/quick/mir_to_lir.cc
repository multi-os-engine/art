...
bool Mir2Lir::GenSpecialIGet(MIR* mir, const InlineMethod& special) {
  ...
  // TODO refactor
  OpSize size = k32;
  if (data.op_variant == InlineMethodAnalyser::IGetVariant(Instruction::IGET_OBJECT)) {
    size = kReference;
  } else if (data.op_variant == InlineMethodAnalyser::IGetVariant(Instruction::IGET_WIDE)) {
    size = k64;
  } else if (data.op_variant == InlineMethodAnalyser::IGetVariant(Instruction::IGET_SHORT)) {
    size = kSignedHalf;
  } else if (data.op_variant == InlineMethodAnalyser::IGetVariant(Instruction::IGET_CHAR)) {
    size = kUnsignedHalf;
    ...
    size = kSignedByte;
  } else if (data.op_variant == InlineMethodAnalyser::IGetVariant(Instruction::IGET)) {
  } else {
    LOG(FATAL) << "Unexpected variant " << data.op_variant;
  }
  // TODO fall through case
...
bool Mir2Lir::GenSpecialIPut(MIR* mir, const InlineMethod& special) {
  ...
  if (data.op_variant == InlineMethodAnalyser::IPutVariant(Instruction::IPUT_OBJECT)) {
    size = kReference;
  } else if (data.op_variant == InlineMethodAnalyser::IPutVariant(Instruction::IPUT_WIDE)) {
    size = k64;
  } else if (data.op_variant == InlineMethodAnalyser::IPutVariant(Instruction::IPUT_SHORT)) {
    size = kSignedHalf;
  } else if (data.op_variant == InlineMethodAnalyser::IPutVariant(Instruction::IPUT_CHAR)) {
    size = kUnsignedHalf;
...
void Mir2Lir::CompileDalvikInstruction(MIR* mir, BasicBlock* bb, LIR* label_list) {
  ...
  switch (opcode) {
    ...
    case Instruction::IGET_OBJECT:
      GenIGet(mir, opt_flags, kReference, rl_dest, rl_src[0], true);
      break;

    case Instruction::IGET_WIDE:
      GenIGet(mir, opt_flags, k64, rl_dest, rl_src[0], false);
      break;

    case Instruction::IGET:
      GenIGet(mir, opt_flags, k32, rl_dest, rl_src[0], false);
      break;

    case Instruction::IGET_CHAR:
      GenIGet(mir, opt_flags, kUnsignedHalf, rl_dest, rl_src[0], false);
      break;

    case Instruction::IGET_SHORT:
      GenIGet(mir, opt_flags, kSignedHalf, rl_dest, rl_src[0], false);
      break;

    case Instruction::IGET_BOOLEAN:
      GenIGet(mir, opt_flags, kUnsignedByte, rl_dest, rl_src[0], false);
      break;

    case Instruction::IGET_BYTE:
      GenIGet(mir, opt_flags, kSignedByte, rl_dest, rl_src[0], false);
      break;

    case Instruction::IPUT_WIDE:
      GenIPut(mir, opt_flags, k64, rl_src[0], rl_src[1], false);
      break;

    case Instruction::IPUT_OBJECT:
      GenIPut(mir, opt_flags, kReference, rl_src[0], rl_src[1], true);
      break;

    case Instruction::IPUT:
      GenIPut(mir, opt_flags, k32, rl_src[0], rl_src[1], false);
      break;

    case Instruction::IPUT_BYTE:
    case Instruction::IPUT_BOOLEAN:
      GenIPut(mir, opt_flags, kUnsignedByte, rl_src[0], rl_src[1], false);
      break;

    case Instruction::IPUT_CHAR:
      GenIPut(mir, opt_flags, kUnsignedHalf, rl_src[0], rl_src[1], false);
      break;

    case Instruction::IPUT_SHORT:
      GenIPut(mir, opt_flags, kSignedHalf, rl_src[0], rl_src[1], false);
      break;

    case Instruction::SGET_OBJECT:
      GenSget(mir, rl_dest, kReference, true);
      break;

    case Instruction::SGET:
      GenSget(mir, rl_dest, k32, false);
      break;

    case Instruction::SGET_CHAR:
      GenSget(mir, rl_dest, kUnsignedHalf, false);
      break;

    case Instruction::SGET_SHORT:
      GenSget(mir, rl_dest, kSignedHalf, false);
      break;

    case Instruction::SGET_BOOLEAN:
      GenSget(mir, rl_dest, kUnsignedByte, false);
      break;

    case Instruction::SGET_BYTE:
      GenSget(mir, rl_dest, kSignedByte, false);
      break;

    case Instruction::SGET_WIDE:
      GenSget(mir, rl_dest, k64, false);
      break;

    case Instruction::SPUT_OBJECT:
      GenSput(mir, rl_src[0], kReference, true);
      break;

    case Instruction::SPUT:
      GenSput(mir, rl_src[0], k32, false);
      break;

    case Instruction::SPUT_BYTE:
    case Instruction::SPUT_BOOLEAN:
      GenSput(mir, rl_src[0], kUnsignedByte, false);
      break;

    case Instruction::SPUT_CHAR:
      GenSput(mir, rl_src[0], kUnsignedHalf, false);
      break;

    case Instruction::SPUT_SHORT:
      GenSput(mir, rl_src[0], kSignedHalf, false);
      break;


    case Instruction::SPUT_WIDE:
      GenSput(mir, rl_src[0], k64, false);
      break;
