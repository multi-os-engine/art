...
void Mir2Lir::LockArg(int in_position, bool wide) {
  ...
  RegStorage reg_arg_high = wide ? GetArgMappingToPhysicalReg(in_position + 1) :
      RegStorage(RegStorage::kInvalid);

  if (!reg_arg_low.IsInvalid()) {
    LockTemp(reg_arg_low);
  }
  if (!reg_arg_high.IsInvalid() && reg_arg_low != reg_arg_high) {
...
RegStorage Mir2Lir::LoadArg(int in_position, bool wide) {
  ...
  if (wide && reg_arg_high.IsInvalid()) {
    ...
    if (reg_arg_low.IsInvalid()) {
      ...
      LoadBaseDispWide(TargetReg(kSp).GetReg(), offset, reg_arg_low.GetReg(),
                       reg_arg_high.GetReg(), INVALID_SREG);
    } else {
...
void Mir2Lir::LoadArgDirect(int in_position, RegLocation rl_dest) {
  ...
  if (!rl_dest.wide) {
    ...
    if (!reg_arg_low.IsInvalid() && !reg_arg_high.IsInvalid()) {
      OpRegCopyWide(rl_dest.reg.GetLowReg(), rl_dest.reg.GetHighReg(),
                    reg_arg_low.GetReg(), reg_arg_high.GetReg());
    } else if (!reg_arg_low.IsInvalid() && reg_arg_high.IsInvalid()) {
...
bool Mir2Lir::GenSpecialIGet(MIR* mir, const InlineMethod& special) {
  ...
  if (wide) {
    LoadBaseDispWide(reg_obj.GetReg(), data.field_offset, rl_dest.reg.GetLowReg(), rl_dest.reg.GetHighReg(), INVALID_SREG);
  } else {
...
bool Mir2Lir::GenSpecialIPut(MIR* mir, const InlineMethod& special) {
  ...
  if (wide) {
    StoreBaseDispWide(reg_obj.GetReg(), data.field_offset, reg_src.GetLowReg(),
                      reg_src.GetHighReg());
  } else {
