...
void Mir2Lir::CompileDalvikInstruction(MIR* mir, BasicBlock* bb, LIR* label_list) {
  ...
  switch (opcode) {
    ...
    case Instruction::RETURN_VOID:
      ...
      if (!LEAF_OPTIMIZATION || !mir_graph_->MethodIsLeaf()) {
    ...
    case Instruction::GOTO_32:
      if (mir_graph_->IsBackedge(bb, bb->taken) &&
          (LEAF_OPTIMIZATION || !mir_graph_->HasSuspendTestBetween(bb, bb->taken))) {
    ...
    case Instruction::IF_LE: {
      ...
      if (rl_src[0].is_const && rl_src[1].is_const) {
        ...
        if (mir_graph_->IsBackedge(bb, target_id) &&
            (LEAF_OPTIMIZATION || !mir_graph_->HasSuspendTestBetween(bb, target_id))) {
          GenSuspendTest(opt_flags);
        }
    ...
    case Instruction::INVOKE_STATIC_RANGE:
      ...
      if (!LEAF_OPTIMIZATION && (opt_flags & MIR_INLINED) == 0) {
