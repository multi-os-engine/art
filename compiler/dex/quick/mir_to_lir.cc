...
bool Mir2Lir::GenSpecialIGet(MIR* mir, const InlineMethod& special) {
  ...
  if (data.op_variant == InlineMethodAnalyser::IGetVariant(Instruction::IGET_WIDE)) {
    component_size = 8;
  } else if (data.op_variant == InlineMethodAnalyser::IGetVariant(Instruction::IGET_BOOLEAN) || data.op_variant == InlineMethodAnalyser::IGetVariant(Instruction::IGET_SHORT) || data.op_variant == InlineMethodAnalyser::IGetVariant(Instruction::IGET_CHAR)) {
    component_size = 2;
  }
...
bool Mir2Lir::GenSpecialIPut(MIR* mir, const InlineMethod& special) {
  ...
  if (data.op_variant == InlineMethodAnalyser::IPutVariant(Instruction::IPUT_WIDE)) {
    component_size = 8;
  } else if (data.op_variant == InlineMethodAnalyser::IPutVariant(Instruction::IPUT_BOOLEAN) || data.op_variant == InlineMethodAnalyser::IPutVariant(Instruction::IPUT_SHORT) || data.op_variant == InlineMethodAnalyser::IPutVariant(Instruction::IPUT_CHAR)) {
    component_size = 2;
  }
  bool wide = component_size == 8;
  bool ref = (data.op_variant == InlineMethodAnalyser::IGetVariant(Instruction::IGET_OBJECT));
