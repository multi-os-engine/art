...
static int SpillRegsPreSub(Arm64Mir2Lir* m2l, RegStorage base, uint32_t core_reg_mask,
  ...
  if (fp_reg_mask != 0) {
    ...
    m2l->SpillFPRegs(rs_sp, spill_offset, fp_reg_mask);
  }

  if (core_reg_mask != 0) {
    // Spill core regs.
    int spill_offset = frame_size - (core_count * kArm64PointerSize);
    m2l->SpillCoreRegs(rs_sp, spill_offset, core_reg_mask);
  }
  ...
  if (fp_count > 0) {
    ...
    if (fp_count == 1) {
      ...
      if (core_count % 2 == 0) {
        ...
        m2l->NewLIR4(WIDE(kA64StpPre4ffXD), RegStorage::FloatSolo64(reg1).GetReg(), rs_d0.GetReg(),
                     base.GetReg(), -all_offset);
