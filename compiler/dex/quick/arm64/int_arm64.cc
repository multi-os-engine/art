...
void Arm64Mir2Lir::GenCmpLong(RegLocation rl_dest, RegLocation rl_src1,
  ...
  NewLIR4(kA64Csinc4rrrc, rl_result.reg.GetReg(), rARM_ZR, rARM_ZR, kArmCondEq);
...
LIR* Arm64Mir2Lir::OpRegCopyNoInsert(RegStorage r_dest, RegStorage r_src) {
  bool dest_is_fp = ARM_FPREG(r_dest.GetLowReg());
  bool src_is_fp = ARM_FPREG(r_src.GetLowReg());
  ...
  if (LIKELY(dest_is_fp == src_is_fp)) {
    if (LIKELY(!dest_is_fp)) {
      ...
      opcode = ((UNLIKELY(r_dest == rs_rARM_SP || r_src == rs_rARM_SP)) ?
                kA64Add4RRdT : kA64Mov2rr);
      ...
      // Float/float copy.
#if 0
      // TODO(Arm64): fix ARM_DOUBLEREG and friends.
      bool dest_is_double = ARM_DOUBLEREG(r_dest.GetReg());
      bool src_is_double = ARM_DOUBLEREG(r_src.GetReg());
#else
      bool dest_is_double = r_dest.Is64Bit();
      bool src_is_double = r_src.Is64Bit();
#endif
  ...
  // TODO(Arm64): remove code below: pairs not supported by Arm64.
...
LIR* Arm64Mir2Lir::OpRegCopy(RegStorage r_dest, RegStorage r_src) {
...
bool Arm64Mir2Lir::SmallLiteralDivRem(Instruction::Code dalvik_opcode, bool is_div,
  ...
  NewLIR4(kA64Smaddl4xwwx, r_lo.GetReg(), r_magic.GetReg(), rl_src.reg.GetReg(), rARM_ZR);
  switch (pattern) {
    case Divide3:
...
LIR* Arm64Mir2Lir::GenRegMemCheck(ConditionCode c_code, RegStorage reg1, RegStorage base,
                                int offset, ThrowKind kind) {
  LOG(FATAL) << "Unexpected use of GenRegMemCheck for Arm64";
  return NULL;
}
...
void Arm64Mir2Lir::GenDivZeroCheck(RegStorage reg) {
...
void Arm64Mir2Lir::UnSpillCoreRegs(int base, int offset, uint32_t reg_mask, bool use_x_regs) {
  ...
  for (offset = (offset >> pop_log2_size) - 1; reg_mask; offset--) {
     reg_mask = GenPairWise(reg_mask, & reg1, & reg2);
    if (UNLIKELY(reg2 < 0)) {
      NewLIR3(kA64Ldr3rXD, reg1, base, offset);
    } else {
      NewLIR4(kA64Ldp4rrXD, reg1, reg2, base, offset);
    }
  }
}
...
void Arm64Mir2Lir::SpillCoreRegs(int base, int offset, uint32_t reg_mask, bool use_x_regs) {
  ...
  for (offset = (offset >> pop_log2_size) - 1; reg_mask; offset--) {
    reg_mask = GenPairWise(reg_mask, & reg1, & reg2);
    if (UNLIKELY(reg2 < 0)) {
      NewLIR3(kA64Str3rXD, reg1, base, offset);
    } else {
      NewLIR4(kA64Stp4rrXD, reg1, reg2, base, offset);
    }
  }
}
