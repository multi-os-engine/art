...
int Arm64Mir2Lir::SpillRegsFrameSize(RegStorage base, uint32_t core_reg_mask, uint32_t fp_reg_mask,
                                        int frame_size) {
  OpRegRegImm(kOpSub, rs_sp, rs_sp, frame_size);
...
int Arm64Mir2Lir::SpillRegs(RegStorage base, uint32_t core_reg_mask, uint32_t fp_reg_mask,
  ...
  if ((RoundUp(frame_size, 8) / 8 <= 63)) {
    return SpillRegsFrameSize(base, core_reg_mask, fp_reg_mask, frame_size);
  }
  ...
  if (fp_count > 0) {
    ...
    if (fp_count == 1) {
      ...
      if (core_count % 2 == 0) {
        NewLIR4(WIDE(kA64StpPre4ffXD), rs_d0.GetReg(), RegStorage::FloatSolo64(reg1).GetReg(),
                base.GetReg(), -all_offset);
      } else {
        NewLIR4(WIDE(kA64StpPre4ffXD), RegStorage::FloatSolo64(reg1).GetReg(), rs_d0.GetReg(),
                base.GetReg(), -all_offset);
      ...
      if (combined % 2 == 0) {
        ...
        NewLIR4(WIDE(kA64StpPre4ffXD), RegStorage::FloatSolo64(reg2).GetReg(),
  ...
  if (fp_count != 0) {
    for (; fp_reg_mask != 0;) {
      ...
      fp_reg_mask = GenPairWise(fp_reg_mask, &reg1, &reg2);
      if (UNLIKELY(reg2 < 0)) {
        NewLIR3(FWIDE(kA64Str3fXD), RegStorage::FloatSolo64(reg1).GetReg(), base.GetReg(),
                cur_offset);
    ...
    if (core_count % 2 == 1) {
      core_reg_mask = ExtractReg(core_reg_mask, &reg1);
      NewLIR3(WIDE(kA64Str3rXD), RegStorage::Solo64(reg1).GetReg(), base.GetReg(), cur_offset + 1);
