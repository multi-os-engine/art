...
static int DwarfCoreReg(int art_reg) {
  DCHECK_LT(art_reg, 32);
  return art_reg;
}

static int DwarfFpReg(int arg_reg) {
  DCHECK_LT(arg_reg, 32);
  return 64 + arg_reg;
}
...
static int SpillRegsPreIndexed(Arm64Mir2Lir* m2l, RegStorage base, uint32_t core_reg_mask,
  ...
  if (fp_count > 0) {
    ...
    if (fp_count == 1) {
      ...
      if (core_count % 2 == 0) {
        ...
        m2l->cfi().AdjustCFAOffset(all_offset * kRegSize);
        m2l->cfi().RelOffset(DwarfFpReg(reg1), kRegSize);
      } else {
        m2l->NewLIR4(WIDE(kA64StpPre4ffXD),
                     RegStorage::FloatSolo64(reg1).GetReg(),
                     RegStorage::FloatSolo64(reg1).GetReg(),
    ...
    if (core_count % 2 == 1) {
      ...
      m2l->cfi().AdjustCFAOffset(all_offset * kRegSize);
      m2l->cfi().RelOffset(DwarfCoreReg(reg1), kRegSize);
    } else {
  ...
  DCHECK_EQ(m2l->cfi().current_cfa_offset(), all_offset * kRegSize);
  ...
  if (fp_count != 0) {
    for (; fp_reg_mask != 0;) {
      ...
      if (UNLIKELY(reg2 < 0)) {
        m2l->NewLIR3(WIDE(kA64Str3fXD), RegStorage::FloatSolo64(reg1).GetReg(), base.GetReg(),
                     cur_offset);
        m2l->cfi().RelOffset(DwarfFpReg(reg1), cur_offset * kRegSize);
        ...
        m2l->cfi().RelOffset(DwarfFpReg(reg2), cur_offset * kRegSize);
        m2l->cfi().RelOffset(DwarfFpReg(reg1), (cur_offset + 1) * kRegSize);
        cur_offset += 2;
      }
    }
    ...
    if (core_count % 2 == 1) {
      ...
      m2l->NewLIR3(WIDE(kA64Str3rXD), RegStorage::Solo64(reg1).GetReg(), base.GetReg(),
                   cur_offset + 1);
      m2l->cfi().RelOffset(DwarfCoreReg(reg1), (cur_offset + 1) * kRegSize);
  ...
  for (; core_reg_mask != 0; cur_offset += 2) {
    ...
    m2l->cfi().RelOffset(DwarfCoreReg(reg2), cur_offset * kRegSize);
    m2l->cfi().RelOffset(DwarfCoreReg(reg1), (cur_offset + 1) * kRegSize);
  }
