...
LIR* Arm64Mir2Lir::OpCmpImmBranch(ConditionCode cond, int reg, int check_value,
  ...
  if (!skip && (check_value == 0) &&
    ...
    branch = NewLIR2((arm_cond == kArmCondEq) ? kA64CbzW : kA64CbnzW,
                     reg, 0);
  } else {
...
LIR* Arm64Mir2Lir::OpRegCopyNoInsert(int r_dest, int r_src) {
  ...
  if (UNLIKELY(r_dest == rARM_SP || r_src == rARM_SP)) {
    res = RawLIR(current_dalvik_offset_, kA64AddWWI12, r_dest, r_src, 0);
  } else {
    res = RawLIR(current_dalvik_offset_, kA64MovWW, r_dest, r_src);
  }
...
LIR* Arm64Mir2Lir::OpTestSuspend(LIR* target) {
  NewLIR3(kA64SubsWWI12, rARM_SUSPEND, rARM_SUSPEND, 1);
...
void Arm64Mir2Lir::GenMulLong(Instruction::Code opcode, RegLocation rl_dest,
    ...
    if (rl_src1.reg.GetReg() == rl_src2.reg.GetReg()) {
      res_hi = AllocTemp();
      NewLIR3(kA64MulWWW, tmp1, rl_src1.reg.GetReg(), rl_src1.reg.GetHighReg());
      ...
      NewLIR3(kA64MulWWW, tmp1, rl_src2.reg.GetReg(), rl_src1.reg.GetHighReg());
      if (special_case) {
