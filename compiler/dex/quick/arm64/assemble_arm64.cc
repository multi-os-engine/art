...
// TODO(Arm64): remove the macro below.
...
uint8_t* Arm64Mir2Lir::EncodeLIRs(uint8_t* write_pos, LIR* lir) {
  for (; lir != nullptr; lir = NEXT_LIR(lir)) {
    ...
    if (UNLIKELY(opcode == kA64x86Trampoline)) {
      uintptr_t addr = static_cast<uintptr_t>(A64_QUICK_ENTRYPOINT_INT_OFFS(pForeignCodeCall));
      write_pos[0] = 0x90;       // nop
      write_pos[1] = 0x64;       // call *%fs:addr
      write_pos[2] = 0xff;
      write_pos[3] = 0x15;
      write_pos[4] = addr;
      write_pos[5] = addr >> 8;
      write_pos[6] = addr >> 16;
      write_pos[7] = addr >> 24;
      write_pos[8] = frame_size_;
      write_pos[9] = frame_size_ >> 8;
      write_pos[10] = frame_size_ >> 16;
      write_pos[11] = frame_size_ >> 24;
      write_pos += 12;
      continue;
    }
    ...
    if (UNLIKELY(opcode == kA64x86BlR)) {
      ...
      write_pos[0] = operand0 << 5;  // brk #(0x8000 + operand[0])
      write_pos[1] = operand0 >> 3;
      write_pos[2] = 0x30;
      write_pos[3] = 0xd4;
      write_pos[4] = 0xe0;           // blr xzr
      write_pos[5] = 0x03;
      write_pos[6] = 0x3f;
      write_pos[7] = 0xd6;
      write_pos += 8;
  ...
  while (true) {
    ...
    while (lir != NULL) {
      ...
      switch (static_cast<FixupKind>(lir->flags.fixup)) {
        ...
        case kFixupAdr: {
          LIR* target_lir = lir->target;
          int32_t target_offs = ((target_lir->flags.generation == lir->flags.generation) ?
                                 0 : offset_adjustment);
          if (target_lir) {
            target_offs += target_lir->offset;
          } else if (lir->operands[2] >= 0) {
            EmbeddedData* tab = reinterpret_cast<EmbeddedData*>(UnwrapPointer(lir->operands[2]));
            target_offs += tab->offset;
          } else {
            break;
          }
          // TODO(Arm64): if (!IS_SIGNED_IMM21(delta)) error
          lir->operands[1] = target_offs - lir->offset;
          break;
        }
        default:
