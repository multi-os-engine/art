...
LIR* Arm64Mir2Lir::LoadConstantNoClobber(RegStorage r_dest, int value) {
  ...
  DCHECK_NE(r_dest.GetReg(), rARM_SP);
  DCHECK_NE(r_dest.GetReg(), rARM_ZR);
  ...
  if (LIKELY(low_fast || high_fast)) {
    ...
    if (LIKELY(low_bits == high_bits)) {
      ...
      res = NewLIR2(opcode, r_dest.GetReg(), rARM_ZR);
    } else {
    ...
    if (log_imm >= 0) {
      res = NewLIR3(kA64Orr3Rrl, r_dest.GetReg(), rARM_ZR, log_imm);
    } else {
      // Use 2 instructions.
...
LIR* Arm64Mir2Lir::OpRegRegImm(OpKind op, RegStorage r_dest, RegStorage r_src1, int value) {
  ...
  switch (OP_KIND_UNWIDE(op)) {
    ...
    case kOpSub:
      ...
      DCHECK_NE(r_dest.GetReg(), rARM_ZR);
      DCHECK_NE(r_src1.GetReg(), rARM_ZR);
      if (abs_value < 0x1000) {
...
LIR* Arm64Mir2Lir::OpRegImm64(OpKind op, RegStorage r_dest_src1, int64_t value, bool is_wide) {
  ...
  if (EncodingMap[opcode].flags & IS_QUAD_OP)
    return NewLIR4(opcode, r_dest_src1.GetReg(), r_dest_src1.GetReg(), abs_value, (shift) ? 1 : 0);
  else
    return NewLIR3(opcode, r_dest_src1.GetReg(), abs_value, (shift) ? 1 : 0);
}
...
LIR* Arm64Mir2Lir::LoadBaseIndexed(RegStorage r_base, RegStorage r_index, RegStorage r_dest,
  ...
  if (ARM_FPREG(r_dest.GetReg())) {
    bool is_double = ARM_DOUBLEREG(r_dest.GetReg());
    bool is_single = !is_double;
    DCHECK_EQ(is_single, ARM_SINGLEREG(r_dest.GetReg()));

    // If r_dest is a single, then size must be either kWord or kSingle.
    // If r_dest is a double, then size must be either kLong or kDouble.
    DCHECK(!is_single || size == kWord || size == kSingle);
    DCHECK(!is_double || size == kLong || size == kDouble);
  ...
  if (UNLIKELY((EncodingMap[opcode].flags & IS_TERTIARY_OP) != 0)) {
    ...
    DCHECK(scale == 0 || scale == (IS_WIDE(opcode) ? 3 : 2));
...
LIR* Arm64Mir2Lir::StoreBaseIndexed(RegStorage r_base, RegStorage r_index, RegStorage r_src,
  ...
  if (ARM_FPREG(r_src.GetReg())) {
    bool is_double = ARM_DOUBLEREG(r_src.GetReg());
    bool is_single = !is_double;
    DCHECK_EQ(is_single, ARM_SINGLEREG(r_src.GetReg()));

    // If r_src is a single, then size must be either kWord or kSingle.
    // If r_src is a double, then size must be either kLong or kDouble.
    DCHECK(!is_single || size == kWord || size == kSingle);
    DCHECK(!is_double || size == kLong || size == kDouble);
...
// TODO(Arm64): clean this up!
// Note: following change to avoid using pairs for doubles, replace conversion w/ DCHECK.
static void TolerateDoublePairForNow(RegStorage* rs) {
  if (rs->IsPair()) {
    DCHECK(ARM_FPREG(rs->GetHighReg()));
    *rs = RegStorage::Solo64(ARM_S2D(rs->GetLowReg(), rs->GetHighReg()));
  }
}
#define TOLERATE_DOUBLE_PAIR_FOR_NOW(potential_double_pair) \
  TolerateDoublePairForNow(& (potential_double_pair))
...
LIR* Arm64Mir2Lir::LoadBaseDispBody(RegStorage r_base, int displacement, RegStorage r_dest,
  ...
  switch (size) {
    ...
    case kLong:
      ...
      if (ARM_FPREG(r_dest.GetLowReg())) {
        TOLERATE_DOUBLE_PAIR_FOR_NOW(r_dest);
...
LIR* Arm64Mir2Lir::StoreBaseDispBody(RegStorage r_base, int displacement, RegStorage r_src,
  ...
  switch (size) {
    ...
    case kDouble:
      DCHECK_EQ(encoded_disp & 0x3, 0);
      if (ARM_FPREG(r_src.GetLowReg())) {
        TOLERATE_DOUBLE_PAIR_FOR_NOW(r_src);
