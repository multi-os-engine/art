  ...
  if (arg.IsFP()) {
    ...
    if (cur_core_reg_ < coreArgMappingToPhysicalRegSize) {
      ...
      if (result.Valid()) {
        ...
        result = (arg.IsWide() || arg.IsRef()) ? RegStorage::Solo64(res_reg) : RegStorage::Solo32(res_reg);
      }
    }
  }
  return result;
...
int Arm64Mir2Lir::GenDalvikArgsBulkCopy(CallInfo*/* info*/, int/* first*/, int count) {
  // REVISIT: I do not see any sense in simple copying of VR's here.
  //          GenDalvikArgs will handle it for us in a bit more efficient way
  //          At least no need to flush promoted registers to home location there.
  //          Also 64-bit args will be handled as 64-bit moves.
  /*
  GenDalvikArgsFlushPromoted(info, first);

  int current_start_offset = SRegOffset(info->args[fisrt].s_reg_low);
  int current_outs_offset = StackVisitor::GetOutVROffset(first, cu_->instruction_set);
