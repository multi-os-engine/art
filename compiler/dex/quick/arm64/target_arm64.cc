...
ResourceMask Arm64Mir2Lir::GetRegMaskCommon(const RegStorage& reg) const {
  ...
  if (UNLIKELY(reg == rs_wzr || reg == rs_xzr)) {
    ...
    return ResourceMask::RawMask(0u, 0u);
  }

  return ResourceMask::Bit(reg_raw & RegStorage::kRegTypeMask);
}
...
void Arm64Mir2Lir::SetupTargetResourceMasks(LIR* lir, uint64_t flags,
  ...
  if ((flags & (REG_DEF_SP | REG_USE_SP | REG_DEF_LR)) != 0) {
    if (flags & REG_DEF_SP) {
      def_mask->SetTargetSpecificBit(kArm64RegSP);
    }

    if (flags & REG_USE_SP) {
      use_mask->SetTargetSpecificBit(kArm64RegSP);
    }

    if (flags & REG_DEF_LR) {
      def_mask->SetTargetSpecificBit(kArm64RegLR);
    }
  }
}
