...
uint64_t Arm64Mir2Lir::GetRegMaskCommon(RegStorage reg) {
  // TODO(Arm64): remove hard coded numbers (e.g. 0x1c0).
  int reg_raw = reg.GetRawBits();
  // Check if the shape mask is zero (i.e. invalid).
  if (UNLIKELY((reg_raw & RegStorage::kShapeMask) == 0)) {
    // Reserved register (this must either be xzr or wzr).
    DCHECK(reg == rs_wzr || reg == rs_xzr);
    return 0;
  }
...
RegStorage Arm64Mir2Lir::LoadHelper(A64ThreadOffset offset) {
  ...
  LoadBaseDisp(rs_rA64_SELF, offset.Int32Value(), rs_rA64_LR, k64, INVALID_SREG);
  return rs_rA64_LR;
}
