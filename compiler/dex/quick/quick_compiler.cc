...
// If there's any unsupported opcodes, is it because they are experimental?
// Do not normally scan methods if the only opcodes unsupported are experimental.
static const bool kUnsupportedExperimentalOpcodesOnly[] = {
    // 0 = kNone.
    false,
    // 1 = kArm, unused (will use kThumb2).
    false,
...
static_assert(sizeof(kUnsupportedExperimentalOpcodesOnly) == 8 * sizeof(bool),
              "kUnsupportedExperimentalOpcodesOnly unexpected");
...
static bool SkipScanningUnsupportedOpcodes(InstructionSet instruction_set) {
  if (UNLIKELY(kUnsupportedOpcodesSize[instruction_set] == 0U)) {
    ...
    return true;
  } else if (LIKELY(!Runtime::Current()->AreExperimentalOpcodesEnabled())) {
    // Experimental opcodes are disabled.
    //
    // If all unsupported opcodes are experimental we don't need to do scanning.
    return kUnsupportedExperimentalOpcodesOnly[instruction_set];
  } else {
