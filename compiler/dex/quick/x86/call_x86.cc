...
void X86Mir2Lir::GenEntrySequence(RegLocation* ArgLocs, RegLocation rl_method) {
  ...
  method_entry_ = NewLIR0(kPseudoMethodEntry);

  /* Spill callee saves. Includes the stack decrement. */
  SpillRegs();
...
void X86Mir2Lir::GenExitSequence() {
  ...
  NewLIR0(kPseudoEpilogueStart);

  /* Unspill callee saves. Includes the stack increment. */
  UnSpillRegs();
