 ...
 * pairs.
 */
void X86Mir2Lir::GenLongSparseSwitch(MIR* mir, DexOffset table_offset, RegLocation rl_src) {
 ...
 * done:
 */
void X86Mir2Lir::GenLongPackedSwitch(MIR* mir, DexOffset table_offset, RegLocation rl_src) {
  const uint16_t* table = cu_->insns + current_dalvik_offset_ + table_offset;
  if (cu_->verbose) {
