...
/*
 * We override MarkPackedCaseLabels, because targets are
 * basic block ids, instead of dex offsets.
 */
...
/*
 * We convert and create a new packed switch table whose targets
 * are basic block ids by examining successor blocks.
 */
const uint16_t* X86Mir2Lir::ConvertPackedSwitchTable(MIR* mir, const uint16_t* table) {
   ...
   * format, except targets[] that stores basic block ids.
   *
   */
