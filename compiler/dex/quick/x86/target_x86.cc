...
int X86Mir2Lir::NumReservableVectorRegisters(bool fp_used) {
  ...
  // Leave a few temps for the backend as well
  return fp_used ? num_vector_temps - 2 : num_vector_temps - 1;
}
...
void X86Mir2Lir::GenMultiplyVectorSignedByte(RegStorage rs_dest_src1, RegStorage rs_src2) {
   ...
   */
  // FIXME: The comments above needs re-check.
...
void X86Mir2Lir::GenAddReduceVector(BasicBlock *bb, MIR *mir) {
  ...
  if (opsize == kSingle) {
    ...
    GenAddLong(Instruction::ADD_LONG_2ADDR, rl_dest, temp_loc, temp_loc);
