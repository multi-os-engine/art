  ...
  for (int i = 0; i < cu_->num_ins; i++) {
    ...
    if (reg.Valid()) {
      ...
      if (t_loc->location == kLocPhysReg) {
        ...
        // need flush
        StoreBaseDisp(TargetReg(kSp), SRegOffset(start_vreg + i), reg, t_loc->wide ? k64 : k32, kNotVolatile);
      }
    } else {
      // If arriving in frame & promoted
      if (t_loc->location == kLocPhysReg) {
        LoadBaseDisp(TargetReg(kSp), SRegOffset(start_vreg + i), t_loc->reg, t_loc->wide ? k64 : k32, kNotVolatile);
      }
    }
    if (t_loc->wide) {
