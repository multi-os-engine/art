...
void X86Mir2Lir::LoadMethodAddress(const MethodReference& method, InvokeType type,
  ...
  const DexFile* dex_file = method.dex_file;
  const DexFile::MethodId& id = dex_file->GetMethodId(dex_method_index);
  ...
  LIR *move = RawLIR(current_dalvik_offset_, kX86Mov32RI, TargetReg(symbolic_reg),
                     static_cast<int>(ptr), dex_method_index, reinterpret_cast<int>(dex_file), type);
  AppendLIR(move);
...
LIR *X86Mir2Lir::CallWithLinkerFixup(const MethodReference& method, InvokeType type) {
  /*
  ...
  LIR *call = RawLIR(current_dalvik_offset_, kX86CallI, static_cast<int>(ptr), dex_method_index,
                     reinterpret_cast<int>(dex_file), type);
  AppendLIR(call);
