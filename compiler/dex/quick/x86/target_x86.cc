...
static int DwarfCoreReg(bool is_x86_64, int reg_num) {
  if (is_x86_64) {
    DCHECK_LT(reg_num, 16);
    static const int dwarf_core_reg[8] = {
      0,  // rAX
      2,  // rCX
      1,  // rDX
      3,  // rBX
    ...
    return reg_num < 8 ? dwarf_core_reg[reg_num] : reg_num;
  } else {
    DCHECK_LT(reg_num, 8);
    return reg_num;
  }
...
static int DwarfFpReg(bool is_x86_64, int reg_num) {
  if (is_x86_64) {
    DCHECK_LT(reg_num, 16);
    return 17 + reg_num;
  } else {
