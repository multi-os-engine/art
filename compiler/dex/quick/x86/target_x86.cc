...
void X86Mir2Lir::FlushIns(RegLocation* ArgLocs, RegLocation rl_method) {
  ...
  for (int i = 0; i < cu_->num_ins; i++) {
    ...
    if (ArgLocs[i].fp) {
      reg = GetFloatArgMappingToPhysicalReg(fp_arg_count);
      fp_arg_count++;
    } else {
      reg = GetArgMappingToPhysicalReg(core_arg_count);
      core_arg_count++;
    }
...
int X86Mir2Lir::LoadArgRegs(CallInfo* info, int call_state,
  ...
  int last_arg_reg = arraysize(arg_regs) - 1;
  int last_fp_arg_reg = arraysize(arg_fp_regs) - 1;
  ...
  for (; (next_reg <= last_arg_reg || next_fp_reg <= last_fp_arg_reg) &&
         (next_arg < info->num_arg_words); next_reg++) {
    RegLocation rl_arg = info->args[next_arg++];
    rl_arg = UpdateRawLoc(rl_arg);
    bool is_float = rl_arg.fp;
    if (!is_float && next_reg <= last_arg_reg) {
      if (rl_arg.wide && (next_reg <= last_arg_reg)) {
        LoadValueDirectWideFixed(rl_arg, RegStorage::Solo64(arg_regs[next_reg].GetReg()));
        next_arg++;
      } else {
        if (rl_arg.wide) {
        ...
        LoadValueDirectFixed(rl_arg, RegStorage::Solo32(arg_regs[next_reg].GetReg()));
      }
    } else if (is_float && (next_fp_reg <= last_fp_arg_reg)) {
      if (rl_arg.wide) {
        LoadValueDirectWideFixed(rl_arg, arg_fp_regs[next_fp_reg]);
        next_arg++;
      } else {
        LoadValueDirectFixed(rl_arg, arg_fp_regs[next_fp_reg]);
      }
      next_fp_reg++;
    }
...
int X86Mir2Lir::GenDalvikArgsNoRange(CallInfo* info,
  ...
  // DCHECK_LE(info->num_arg_words, 5);  // TODO(64) Temporary disable this check
...
*/
    ...
    while (next_use < info->num_arg_words) {
      ...
      int outs_offset = (next_use + kPointerSize/sizeof(uint32_t)) * 4;
      if (rl_arg.wide && rl_arg.fp) {  // TODO(64) ||?
...
int X86Mir2Lir::GenDalvikArgsRange(CallInfo* info, int call_state,
  ...
  if (info->num_arg_words <= 5 || true)  // TODO(64): For now we utilize only NoRange
