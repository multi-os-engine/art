...
void X86Mir2Lir::SpillCoreRegs() {
  ...
  for (int reg = 0; mask; mask >>= 1, reg++) {
    if (mask & 0x1) {
      StoreWordDisp(rs_rX86_SP, offset, RegStorage(RegStorage::k32BitSolo, reg));
      offset += 4;
    }
  }
}
...
void X86Mir2Lir::UnSpillCoreRegs() {
  ...
  for (int reg = 0; mask; mask >>= 1, reg++) {
    if (mask & 0x1) {
      LoadWordDisp(rs_rX86_SP, offset, RegStorage(RegStorage::k32BitSolo, reg));
      offset += 4;
    }
  }
}
  ...
  if (loc.location == kLocPhysReg) {
    ...
    if (!RegClassMatches(reg_class, loc.reg)) {
      ...
      if (!IsFpReg(loc.reg.GetLowReg())) {
        ...
        low_reg = new_regs.GetLowReg();
        high_reg = new_regs.GetHighReg();
        DCHECK_NE(low_reg, high_reg);
        OpRegCopyWide(low_reg, high_reg, loc.reg.GetLowReg(), loc.reg.GetHighReg());
        CopyRegInfo(low_reg, loc.reg.GetLowReg());
...
bool X86Mir2Lir::GenInlinedIndexOf(CallInfo* info, bool zero_based) {
  ...
  LoadValueDirectFixed(rl_obj, RegStorage(RegStorage::k32BitSolo, rDX));
  GenNullCheck(rl_obj.s_reg_low, RegStorage(RegStorage::k32BitSolo, rDX), info->opt_flags);
  ...
  if (rl_char.is_const) {
    ...
    LoadValueDirectFixed(rl_char, RegStorage(RegStorage::k32BitSolo, rAX));
    launch_pad = RawLIR(0, kPseudoIntrinsicRetry, WrapPointer(info));
    intrinsic_launchpads_.Insert(launch_pad);
    OpCmpImmBranch(kCondGt, RegStorage(RegStorage::k32BitSolo, rAX), 0xFFFF, launch_pad);
  }
  ...
  if (zero_based) {
    ...
    if (rl_start.is_const) {
      ...
      // Is the start > count?
      length_compare = OpCmpImmBranch(kCondLe, RegStorage(RegStorage::k32BitSolo, rCX),
                                      start_value, nullptr);
      ...
      if (rl_start.location == kLocPhysReg) {
        length_compare = OpCmpBranch(kCondLe, RegStorage(RegStorage::k32BitSolo, rCX),
                                     rl_start.reg, nullptr);
        OpRegReg(kOpSub, rs_rCX, rl_start.reg);
      } else {
        // Compare to memory to avoid a register load.  Handle pushed EDI.
        int displacement = SRegOffset(rl_start.s_reg_low) + sizeof(uint32_t);
        OpRegMem(kOpCmp, rs_rDX, rs_rX86_SP, displacement);
  ...
  if (zero_based || rl_start.is_const) {
    if (start_value == 0) {
      OpRegCopy(RegStorage(RegStorage::k32BitSolo, rDI),
                RegStorage(RegStorage::k32BitSolo, rBX));
    } else {
