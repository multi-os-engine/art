...
void X86Mir2Lir::GenMemBarrier(MemBarrierKind barrier_kind) {
  ...
  if (barrier_kind == kStoreLoad) {
    ...
    // 1) If the last instruction is a store, then we can actually make it atomic by converting it to xchg.
    // 2) If last instruction does not provide full barrier, then insert an mfence.
    if (mem_barrier->opcode == kX86Mov32MR) {
      mem_barrier->opcode = kX86XchgMR;
    } else if (ProvidesFullMemoryBarrier(static_cast<X86OpCode>(mem_barrier->opcode)) == false) {
      mem_barrier = NewLIR0(kX86Mfence);
    }
  }
