...
void X86Mir2Lir::MarkPreservedDouble(int v_reg, RegStorage reg) {
  UNIMPLEMENTED(WARNING) << "MarkPreservedDouble";
#if 0
  LOG(FATAL) << "No support yet for promoted FP regs";
#endif
}
...
RegLocation X86Mir2Lir::GetReturnWideAlt() {
  ...
  DCHECK(res.reg.GetLowReg() == rAX);
  DCHECK(res.reg.GetHighReg() == rDX);
  Clobber(rs_rAX);
  Clobber(rs_rDX);
  MarkInUse(rs_rAX);
  MarkInUse(rs_rDX);
...
RegLocation X86Mir2Lir::UpdateLocWide(RegLocation loc) {
  ...
  if (loc.location != kLocPhysReg) {
    ...
    if (info_lo && info_lo->GetReg().IsFloat()) {
      // The matched register must be live as a double, not a single.
      bool match = info_lo->IsWide();
      ...
      bool match = true;
      match = match && (info_lo != NULL) && info_lo->IsWide();
      match = match && (info_hi != NULL) && info_hi->IsWide();
...
RegStorage X86Mir2Lir::AllocPreservedDouble(int s_reg) {
  UNIMPLEMENTED(WARNING) << "Promoting doubles not yet supported.";
  return RegStorage::InvalidReg();
}
