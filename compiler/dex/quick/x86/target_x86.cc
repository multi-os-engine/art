...
static int ARTRegIDToDWARFRegID(bool is_x86_64, int reg_id) {
  if (is_x86_64) {
    switch (reg_id) {
    case 3 : return 3;  // %rbx
    case 5 : return 6;  // %rbp
    case 12: return 12; // %r12 
    case 13: return 13; // %r13
    case 14: return 14; // %r14
    case 15: return 15; // %r15 
...
std::vector<uint8_t>* X86Mir2Lir::ReturnCallFrameInformation() {
  ...
  if (stack_decrement_ != nullptr) {
    ...
    const uint32_t kSpillInstLen = (cu_->instruction_set == kX86_64) ? 5 : 4;
    const int kDataAlignmentFactor = (cu_->instruction_set == kX86_64) ? -8 : -4;
    uint32_t mask = core_spill_mask_ & ~(1 << rs_rRET.GetRegNum());
    ...
    for (int reg = 0; mask; mask >>= 1, reg++) {
      if (mask & 0x1) {
        ...
        EncodeUnsignedLeb128(*cfi_info, ARTRegIDToDWARFRegID(cu_->instruction_set == kX86_64, reg));
