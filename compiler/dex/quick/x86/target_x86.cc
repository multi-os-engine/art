...
int X86Mir2Lir::NumReservableVectorRegisters(bool fp_used) {
  ...
  // Leave two temps for use by backend as scratch.
  return num_vector_temps - 2;
}
...
void X86Mir2Lir::GenMultiplyVectorLong(RegStorage rs_dest_src1, RegStorage rs_src2) {
   ...
   * For kMirOpPackedMultiply xmm0, xmm1:
   * - xmm0 is src/dest
   * - xmm1 is src
  ...
  RegStorage rs_tmp_vector_1 = Get128BitRegister(AllocTempDouble());
  NewLIR2(kX86MovdqaRR, rs_tmp_vector_1.GetReg(), rs_dest_src1.GetReg());
  RegStorage rs_tmp_vector_2 = Get128BitRegister(AllocTempDouble());
