...
LIR* X86Mir2Lir::LoadConstantWide(int r_dest_lo, int r_dest_hi, int64_t value) {
    ...
    if (X86_FPREG(r_dest_lo)) {
      ...
      if (value == 0) {
        ...
        // value.
        res = LoadBaseDisp(rl_method.reg, 256 /* bogus */,
                           RegStorage(RegStorage::k64BitSolo, r_dest_lo), kDouble, INVALID_SREG);
        ...
        if (val_hi != 0) {
          r_dest_hi = AllocTempDouble().GetLowReg();
    ...
    if (!pair) {
      ...
      if (r_base == r_dest) {
        if (r_dest_hi == r_index) {
          ...
          load = NewLIR5(opcode, r_dest, r_base, r_index, scale,
                         displacement + LOWORD_OFFSET);
          OpRegCopy(RegStorage(RegStorage::k32BitSolo, r_dest_hi),
                    RegStorage(RegStorage::k32BitSolo, temp));
          FreeTemp(temp);
        } else {
        ...
        if (r_dest == r_index) {
          ...
          load2 = NewLIR5(opcode, r_dest_hi, r_base, r_index, scale,
                          displacement + HIWORD_OFFSET);
          OpRegCopy(RegStorage(RegStorage::k32BitSolo, r_dest),
                    RegStorage(RegStorage::k32BitSolo, temp));
          FreeTemp(temp);
        } else {
          load = NewLIR5(opcode, r_dest, r_base, r_index, scale,
