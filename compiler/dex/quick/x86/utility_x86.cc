...
LIR* X86Mir2Lir::LoadConstantWide(int r_dest_lo, int r_dest_hi, int64_t value) {
    ...
    if (X86_FPREG(r_dest_lo)) {
      ...
      if (value == 0) {
        ...
        int displacement = SRegOffset(base_of_code->s_reg_low);
        LoadBaseDisp(rX86_SP, displacement, start_of_method_reg, kWord,
                     base_of_code->s_reg_low);

        // Load the proper value from the literal area.
        // We don't know the proper offset for the value, so pick one that will force
        // 4 byte offset.  We will fix this up in the assembler later to have the right
        // value.
        res = LoadBaseDisp(start_of_method_reg, 256 /* bogus */, r_dest_lo, kDouble, INVALID_SREG);
        res->target = data_target;
        res->flags.fixup = kFixupLoad;
        SetMemRefType(res, true, kLiteral);
        // Redo after we assign target to ensure size is correct.
        SetupResourceMasks(res);
        FreeTemp(start_of_method_reg);
      } else {
        if (val_lo == 0) {
...
void X86Mir2Lir::AnalyzeMIR() {
  ...
  if (store_method_addr_) {
    base_of_code = mir_graph_->GetNewCompilerTemp(kCompilerTempVR, false);
  } else {
    base_of_code = nullptr;
  }
}
