...
LIR* X86Mir2Lir::OpFpRegCopy(RegStorage r_dest, RegStorage r_src) {
  ...
  DCHECK_EQ(r_dest.Is64BitVector(), r_src.Is64BitVector());
  if (r_dest.Is64BitVector()) {
    opcode = kX86MovsdRR;
  } else {
    if (r_dest.Is32BitVector()) {
      if (r_src.Is32BitVector()) {
        opcode = kX86MovssRR;
      } else {  // Fpr <- Gpr
        opcode = kX86MovdxrRR;
      }
    } else {  // Gpr <- Fpr
      DCHECK(r_src.Is32BitVector()) << "Raw: 0x" << std::hex << r_src.GetRawBits();
      opcode = kX86MovdrxRR;
    }
  }
...
LIR* X86Mir2Lir::LoadBaseIndexedDisp(RegStorage r_base, RegStorage r_index, int scale,
  ...
  bool is64bit = ((size == kLong) || (size == kDouble));
  X86OpCode opcode = kX86Nop;
  switch (size) {
    case kLong:
    case kDouble:
...
LIR* X86Mir2Lir::LoadBaseDispWide(RegStorage r_base, int displacement, RegStorage r_dest,
                                  int s_reg) {
  return LoadBaseIndexedDisp(r_base, RegStorage::InvalidReg(), 0, displacement, r_dest, kLong,
                             s_reg);
}
