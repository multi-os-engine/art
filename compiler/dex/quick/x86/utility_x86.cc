...
LIR* X86Mir2Lir::LoadConstantNoClobber(RegStorage r_dest, int value) {
  ...
  if (value == 0) {
    ...
    // 64-bit immidiate is not supported by LIR structure
  ...
  if (r_dest != r_src1 && r_dest != r_src2) {
    if (op == kOpAdd) {  // lea special case, except can't encode rbp as base
      if (r_src1 == r_src2) {
        ...
        return NewLIR5(is64Bit ? kX86Lea64RA : kX86Lea32RA, r_dest.GetReg(), r_src1.GetReg() /* base */,
                       r_src2.GetReg() /* index */, 0 /* scale */, 0 /* disp */);
      } else {
        return NewLIR5(is64Bit ? kX86Lea64RA : kX86Lea32RA, r_dest.GetReg(), r_src2.GetReg() /* base */,
...
LIR* X86Mir2Lir::OpRegRegImm(OpKind op, RegStorage r_dest, RegStorage r_src, int value) {
  ...
  if (r_dest != r_src) {
    if (false && op == kOpLsl && value >= 0 && value <= 3) {  // lea shift special case
      ...
      return NewLIR5(Gen64Bit() ? kX86Lea64RA : kX86Lea32RA, r_dest.GetReg(), r_src.GetReg() /* base */,
...
RegLocation X86Mir2Lir::ForceTempWide(RegLocation loc) {
  if (!Gen64Bit()) {
    return Mir2Lir::ForceTempWide(loc);
  }
  DCHECK(loc.wide);
