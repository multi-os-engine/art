...
size_t X86Mir2Lir::ComputeSize(const X86EncodingMap* entry, int base, int displacement, int reg_r, int reg_x, bool has_sib) {
  size_t size = 0;
  ...
  if ((NeedsRex(base) || NeedsRex(reg_r) || NeedsRex(reg_x)) && entry->skeleton.prefix1 != REX_W && entry->skeleton.prefix2 != REX_W) {
    ++size;  // REX_R
  }
  ++size;  // opcode
...
int X86Mir2Lir::GetInsnSize(LIR* lir) {
  ...
  switch (entry->kind) {
    ...
    case kArray:  // lir operands - 0: base, 1: index, 2: scale, 3: disp
      return ComputeSize(entry, lir->operands[0], lir->operands[3], NO_REG, lir->operands[1], true);
    ...
    case kArrayReg:  // lir operands - 0: base, 1: index, 2: scale, 3: disp, 4: reg
      return ComputeSize(entry, lir->operands[0], lir->operands[3], lir->operands[4], lir->operands[1], true);
    ...
    case kRegArray:   // lir operands - 0: reg, 1: base, 2: index, 3: scale, 4: disp
      return ComputeSize(entry, lir->operands[1], lir->operands[4], lir->operands[0], lir->operands[2], true);
    ...
    case kRegImm: {  // lir operands - 0: reg, 1: immediate
      ...
      if (entry->skeleton.ax_opcode == 0) {
        ...
        LOG(INFO) << "!!! kRegImm" << size;
      }
    }
    case kMemImm:  // lir operands - 0: base, 1: disp, 2: immediate
      return ComputeSize(entry, lir->operands[0], lir->operands[1], NO_REG, lir->operands[0], false);
    ...
    case kMovRegImm:  // lir operands - 0: reg, 1: immediate
      return (entry->skeleton.prefix1 != 0 || NeedsRex(lir->operands[0])?1:0) + 1 + entry->skeleton.immediate_bytes;
    ...
    case kPcRel:
      if (entry->opcode == kX86PcRelLoadRA) {
        ...
        return ComputeSize(entry, lir->operands[1], 0x12345678, lir->operands[0], lir->operands[2], true);
      } else {
    ...
    case kMacro:  // lir operands - 0: reg
      ...
      return 5 /* call opcode + 4 byte displacement */ + 1 /* pop reg */ +
          ComputeSize(&X86Mir2Lir::EncodingMap[kX86Sub32RI], 0, 0, lir->operands[0], NO_REG, false) -
