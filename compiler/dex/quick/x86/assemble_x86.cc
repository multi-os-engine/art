...
#undef ENCODING_MAP
  ...
  { kX86Cmov16RRC, kRegRegCond, IS_BINARY_OP | REG_DEF0_USE1 | USES_CCODES, {0, 0, 0x0F, 0x40, 0, 0, 0, 0}, "Cmovcc16RR", "!2c !0r,!1r"},
  { kX86Cmov32RRC, kRegRegCond, IS_BINARY_OP | REG_DEF0_USE1 | USES_CCODES, {0, 0, 0x0F, 0x40, 0, 0, 0, 0}, "Cmovcc32RR", "!2c !0r,!1r"},
...
void X86Mir2Lir::EmitRegRegCond(const X86EncodingMap* entry, uint8_t reg1, uint8_t reg2, uint8_t condition) {
  ...
  // In order to determine where to encode the condition, check if there is an extra opcode.
  if (entry->skeleton.extra_opcode1 != 0) {
    // Encode condition as part of the extra opcode.
    code_buffer_.push_back(entry->skeleton.opcode);
    code_buffer_.push_back(entry->skeleton.extra_opcode1 | condition);
  } else {
    // Encode condition as part of the opcode.
    code_buffer_.push_back(entry->skeleton.opcode | condition);
  }

  // Not expecting to have to encode immediate, another extra opcode,
  // nor anything special for ModR/M since we have two registers.
  DCHECK_EQ(0, entry->skeleton.extra_opcode2);
  DCHECK_EQ(0, entry->skeleton.immediate_bytes);
  DCHECK_EQ(0, entry->skeleton.modrm_opcode);
