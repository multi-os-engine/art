...
class X86Mir2Lir : public Mir2Lir {
 ...
 public:
  ...
  RegStorage TargetReg(SpecialTargetRegister symbolic_reg, WideKind is_wide) OVERRIDE {
    if (is_wide == kWide64) {
      if (cu_->target64) {
        return As64BitReg(TargetReg32(symbolic_reg));
      } else {
        // x86: construct a pair.
        DCHECK((kArg0 <= symbolic_reg && symbolic_reg < kArg7) ||
               (kFArg0 <= symbolic_reg && symbolic_reg < kFArg7));
  ...
  RegStorage TargetRefReg(SpecialTargetRegister symbolic_reg) OVERRIDE {
    return TargetReg(symbolic_reg, cu_->target64 ? kWide64 : kWide32);
  }
