...
void X86Mir2Lir::GenLongToFP(RegLocation rl_dest, RegLocation rl_src, bool is_double) {
  ...
  if (rl_src.location == kLocPhysReg) {
    ...
    if (reg_info != nullptr && reg_info->IsTemp()) {
      ...
      StoreBaseDisp(TargetRefReg(kSp), src_v_reg_offset, rl_src.reg, k64, kNotVolatile);
    }
  }

  // Push the source virtual register onto the x87 stack.
  LIR *fild64 = NewLIR2NoDest(kX86Fild64M, TargetRefReg(kSp).GetReg(),
  ...
  LIR *fstp = NewLIR2NoDest(opcode, TargetRefReg(kSp).GetReg(), displacement);
  ...
  if (rl_result.location == kLocPhysReg) {
    ...
    rl_result = EvalLoc(rl_dest, kFPReg, true);
    if (is_double) {
      LoadBaseDisp(TargetRefReg(kSp), dest_v_reg_offset, rl_result.reg, k64, kNotVolatile);

      StoreFinalValueWide(rl_dest, rl_result);
    } else {
      Load32Disp(TargetRefReg(kSp), dest_v_reg_offset, rl_result.reg);
...
void X86Mir2Lir::GenRemFP(RegLocation rl_dest, RegLocation rl_src1, RegLocation rl_src2, bool is_double) {
  ...
  if (rl_src1.location == kLocPhysReg) {
    ...
    if (reg_info != nullptr && reg_info->IsTemp()) {
      ...
      // It must have been register promoted if it is not a temp but is still in physical
      // register. Since we need it to be in memory to convert, we place it there now.
      StoreBaseDisp(TargetRefReg(kSp), src1_v_reg_offset, rl_src1.reg, is_double ? k64 : k32,
                    kNotVolatile);
    }
  }
  ...
  if (rl_src2.location == kLocPhysReg) {
    ...
    if (reg_info != nullptr && reg_info->IsTemp()) {
      ...
      StoreBaseDisp(TargetRefReg(kSp), src2_v_reg_offset, rl_src2.reg, is_double ? k64 : k32,
                    kNotVolatile);
    }
  }
  ...
  LIR *fld_2 = NewLIR2NoDest(fld_opcode, TargetRefReg(kSp).GetReg(),
  ...
  LIR *fld_1 = NewLIR2NoDest(fld_opcode, TargetRefReg(kSp).GetReg(),
  ...
  LIR *fst = NewLIR2NoDest(opcode, TargetRefReg(kSp).GetReg(), displacement);
...
bool X86Mir2Lir::GenInlinedAbsFloat(CallInfo* info) {
  ...
  if (v_src_reg == v_dst_reg) {
    ...
    ScopedMemRefType mem_ref_type(this, ResourceMask::kDalvikReg);
    LIR *lir = NewLIR3(kX86And32MI, TargetRefReg(kSp).GetReg(), displacement, 0x7fffffff);
...
bool X86Mir2Lir::GenInlinedAbsDouble(CallInfo* info) {
  ...
  if (rl_src.location == kLocPhysReg && rl_src.reg.IsFloat()) {
    ...
    int displacement = SRegOffset(rl_dest.s_reg_low);
    ScopedMemRefType mem_ref_type(this, ResourceMask::kDalvikReg);
    LIR *lir = NewLIR3(kX86And32MI, TargetRefReg(kSp).GetReg(), displacement  + HIWORD_OFFSET, 0x7fffffff);
