...
bool X86Mir2Lir::GenInlinedAbsDouble(CallInfo* info) {
  ...
  if (rl_src.location == kLocPhysReg && rl_src.reg.IsFloat()) {
    ...
    AnnotateDalvikRegAccess(lir, (displacement + HIWORD_OFFSET) >> 2, false /*is_load */, false /* is_64bit */);
    AnnotateDalvikRegAccess(lir, (displacement + HIWORD_OFFSET) >> 2, true /* is_load */, false /* is_64bit*/);
    return true;
  } else {
    rl_src = LoadValueWide(rl_src, kCoreReg);
    RegLocation rl_result = EvalLoc(rl_dest, kCoreReg, true);
