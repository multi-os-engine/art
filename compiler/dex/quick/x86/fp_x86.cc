...
bool X86Mir2Lir::GenInlinedAbsFloat(CallInfo* info) {
  ...
  // if augument is the same as inlined intrinsic target
...
bool X86Mir2Lir::GenInlinedAbsDouble(CallInfo* info) {
  ...
  if (rl_src.location == kLocPhysReg && rl_src.reg.IsFloat()) {
    RegStorage signMask = AllocTempDouble();
    LoadConstantWide(signMask, 0x7fffffffffffffff);
    RegLocation rl_result = EvalLoc(rl_dest, kFPReg, true);
    OpRegCopyWide(rl_result.reg, rl_src.reg);
    NewLIR2(kX86PandRR, rl_result.reg.GetReg(), signMask.GetReg());
    StoreValueWide(rl_dest, rl_result);
    return true;
  }
  rl_src = LoadValueWide(rl_src, kCoreReg);
