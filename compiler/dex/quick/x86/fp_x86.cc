...
void X86Mir2Lir::GenArithOpDouble(Instruction::Code opcode,
  ...
  if (r_dest == r_src2) {
    ...
    OpRegCopy(RegStorage(RegStorage::k64BitSolo, r_src2),
              RegStorage(RegStorage::k64BitSolo, r_dest));
  }
  OpRegCopy(RegStorage(RegStorage::k64BitSolo, r_dest),
            RegStorage(RegStorage::k64BitSolo, r_src1));
...
void X86Mir2Lir::GenLongToFP(RegLocation rl_dest, RegLocation rl_src, bool is_double) {
  ...
  if (rl_src.location == kLocPhysReg) {
    ...
    if (lo_info != nullptr && lo_info->is_temp) {
      ...
      StoreBaseDispWide(TargetReg(kSp).GetReg(), src_v_reg_offset, rl_src.reg.GetLowReg(), rl_src.reg.GetHighReg());
    }
  }
  ...
  if (rl_result.location == kLocPhysReg) {
    ...
    if (is_double) {
      ...
      LoadBaseDispWide(TargetReg(kSp).GetReg(), dest_v_reg_offset, rl_result.reg.GetLowReg(), rl_result.reg.GetHighReg(), INVALID_SREG);

      StoreValueWide(rl_dest, rl_result);
    } else {
      rl_result = EvalLoc(rl_dest, kFPReg, true);

      LoadWordDisp(TargetReg(kSp).GetReg(), dest_v_reg_offset, rl_result.reg.GetReg());
