  ...
  if (size == k64) {
    if (cu_->instruction_set == kX86_64) {
      ...
      /* Use one bswap instruction to reverse byte order first and then use 3 rounds of
         ...
         compared to generic luni implemetation which has 5 rounds of swapping bits.
         x = bswap x
    ...
    if (rl_i.reg.GetLowReg() == rl_result.reg.GetLowReg()) {
      // First REV shall clobber rl_result.reg.GetLowReg(), save the value in a temp for the second REV.
      r_i_low = AllocTemp();
