...
void X86Mir2Lir::GenLongArith(RegLocation rl_dest, RegLocation rl_src, Instruction::Code op) {
  ...
  // if left operand is in memory and right operand is in
  // register and both belong to the same dalvik register
  // then we should clobber the right one because it doesn't
  // hold valid data anymore
