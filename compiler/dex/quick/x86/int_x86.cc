...
void X86Mir2Lir::GenSelect(BasicBlock* bb, MIR* mir) {
  ...
  if (is_constant_case) {
    ...
    /*
     * 1) Case when both constants are zero:
     *     xor result_reg, result_reg
     * 2) When the true case is zero and result_reg is not same as src_reg:
     *     xor result_reg, result_reg
     *     cmp $0, src_reg
     *     mov t1, $false_case
     *     cmovnz result_reg, t1
     * 3) When the false case is zero and result_reg is not same as src_reg:
     *     xor result_reg, result_reg
     *     cmp $0, src_reg
     *     mov t1, $true_case
     *     cmovz result_reg, t1
     * 4) All other cases (we do compare first to set eflags):
     *     cmp $0, src_reg
    ...
    const bool both_const_zero_case = (true_val == 0 && false_val == 0);
    ...
    const bool catch_all_case = !(both_const_zero_case || true_zero_case || false_zero_case);

    if (both_const_zero_case || true_zero_case || false_zero_case) {
