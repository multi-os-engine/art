...
bool X86Mir2Lir::GenInlinedCas(CallInfo* info, bool is_long, bool is_object) {
  ...
  if (is_long) {
    ...
    if (!obj_in_si && !obj_in_di) {
      LoadWordDisp(TargetRefReg(kSp), SRegOffset(rl_src_obj.s_reg_low) + push_offset, rs_obj);
    ...
    if (!off_in_si && !off_in_di) {
      LoadWordDisp(TargetRefReg(kSp), SRegOffset(rl_src_offset.s_reg_low) + push_offset, rs_off);
      // Dalvik register annotation in LoadBaseIndexedDisp() used wrong offset. Fix it.
      DCHECK(!DECODE_ALIAS_INFO_WIDE(last_lir_insn_->flags.alias_info));
      int reg_id = DECODE_ALIAS_INFO_REG(last_lir_insn_->flags.alias_info) - push_offset / 4u;
      AnnotateDalvikRegAccess(last_lir_insn_, reg_id, true, false);
...
void X86Mir2Lir::GenImulMemImm(RegStorage dest, int sreg, int displacement, int val) {
  ...
  switch (val) {
    case 0:
      ...
      break;
    case 1:
      LoadBaseDisp(TargetRefReg(kSp), displacement, dest, k32, kNotVolatile);
      break;
    default:
      m = NewLIR4(IS_SIMM8(val) ? kX86Imul32RMI8 : kX86Imul32RMI, dest.GetReg(),
                  TargetRefReg(kSp).GetReg(), displacement, val);
...
void X86Mir2Lir::GenMulLong(Instruction::Code, RegLocation rl_dest, RegLocation rl_src1,
  ...
  if (rl_src2.is_const) {
    ...
    if (src1_in_reg) {
      ...
      LIR *m = NewLIR2(kX86Mul32DaM, TargetRefReg(kSp).GetReg(), displacement + LOWORD_OFFSET);
  ...
  if (src1_in_reg) {
    ...
    LoadBaseDisp(TargetRefReg(kSp), SRegOffset(rl_src1.s_reg_low) + HIWORD_OFFSET, rs_r1, k32,
                 kNotVolatile);
  }
  ...
  if (is_square) {
    ...
    if (src2_in_reg) {
      ...
      LIR *m = NewLIR3(kX86Imul32RM, rs_r1.GetReg(), TargetRefReg(kSp).GetReg(),
    ...
    if (src2_in_reg) {
      ...
      LoadBaseDisp(TargetRefReg(kSp), SRegOffset(rl_src2.s_reg_low) + HIWORD_OFFSET, rs_r0, k32,
                   kNotVolatile);
    }
    ...
    if (src1_in_reg) {
      ...
      int displacement = SRegOffset(rl_src1.s_reg_low);
      LIR *m = NewLIR3(kX86Imul32RM, rs_r0.GetReg(), TargetRefReg(kSp).GetReg(),
                       displacement + LOWORD_OFFSET);
      AnnotateDalvikRegAccess(m, (displacement + LOWORD_OFFSET) >> 2,
                              true /* is_load */, true /* is_64bit */);
    }
  ...
  if (src2_in_reg) {
    ...
    LoadBaseDisp(TargetRefReg(kSp), SRegOffset(rl_src2.s_reg_low) + LOWORD_OFFSET, rs_r0, k32,
                 kNotVolatile);
  }
...
void X86Mir2Lir::GenIntToLong(RegLocation rl_dest, RegLocation rl_src) {
  ...
  if (rl_src.location == kLocPhysReg) {
    ...
    int displacement = SRegOffset(rl_src.s_reg_low);
    ScopedMemRefType mem_ref_type(this, ResourceMask::kDalvikReg);
    LIR *m = NewLIR3(kX86MovsxdRM, rl_result.reg.GetReg(), TargetRefReg(kSp).GetReg(),
                     displacement + LOWORD_OFFSET);
    AnnotateDalvikRegAccess(m, (displacement + LOWORD_OFFSET) >> 2,
                            true /* is_load */, true /* is_64bit */);
  }
