...
static int32_t LowestSetBit(uint64_t x) {
  int bit_posn = 0;
  while ((x & 0xf) == 0) {
    bit_posn += 4;
    x >>= 4;
...
bool X86Mir2Lir::BadOverlap(RegLocation rl_src, RegLocation rl_dest) {
  DCHECK(rl_src.wide);
  DCHECK(rl_dest.wide);
...
void X86Mir2Lir::GenMulLong(RegLocation rl_dest, RegLocation rl_src1,
  ...
  if (rl_src2.is_const) {
    ...
    if (val == 0) {
      ...
      GenAddLong(rl_dest, rl_src1, rl_src1);
      return;
...
void X86Mir2Lir::GenShiftImmOpLong(Instruction::Code opcode, RegLocation rl_dest,
  ...
  if (shift_amount == 0) {
    ...
    GenAddLong(rl_dest, rl_src, rl_src);
    return;
  }
