...
void X86Mir2Lir::GenArrayGet(int opt_flags, OpSize size, RegLocation rl_array,
  ...
  if (!constant_index) {
    ...
    constant_value = mir_graph_->ConstantValue(rl_index);
    // If index is constant, just fold it into the data offset
    data_offset += constant_value << scale;
  }
  ...
  if (!(opt_flags & MIR_IGNORE_RANGE_CHECK)) {
    if (constant_index) {
      GenMemImmedCheck(kCondLs, rl_array.low_reg, len_offset,
                       constant_value, kThrowConstantArrayBounds);
    } else {
  ...
  if ((size == kLong) || (size == kDouble)) {
    if (constant_index) {
      LoadBaseIndexedDisp(rl_array.low_reg, INVALID_REG, 0, data_offset, rl_result.low_reg,
    ...
    if (constant_index) {
      LoadBaseIndexedDisp(rl_array.low_reg, INVALID_REG, 0,
...
void X86Mir2Lir::GenArrayPut(int opt_flags, OpSize size, RegLocation rl_array,
  ...
  if ((size == kSignedByte || size == kUnsignedByte) && rl_src.low_reg >= 4) {
    ...
    if (constant_index) {
      StoreBaseIndexedDisp(rl_array.low_reg, INVALID_REG, 0, data_offset, temp,
    ...
    if (constant_index) {
      StoreBaseIndexedDisp(rl_array.low_reg, INVALID_REG, 0, data_offset, rl_src.low_reg,
