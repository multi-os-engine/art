...
void X86Mir2Lir::GenDivZeroCheck(RegStorage reg_lo, RegStorage reg_hi) {
...
void X86Mir2Lir::GenArrayGet(int opt_flags, OpSize size, RegLocation rl_array,
  ...
  if ((size == kLong) || (size == kDouble)) {
    // FIXME: stop using INVALID_REG as key to non-indexed operation.
    LoadBaseIndexedDisp(rl_array.reg.GetReg(), rl_index.reg.IsInvalid() ? INVALID_REG : rl_index.reg.GetReg(), scale, data_offset, rl_result.reg.GetLowReg(),
                        rl_result.reg.GetHighReg(), size, INVALID_SREG);
    StoreValueWide(rl_dest, rl_result);
  } else {
    // FIXME: stop using INVALID_REG as key to non-indexed operation.
    LoadBaseIndexedDisp(rl_array.reg.GetReg(), rl_index.reg.IsInvalid() ? INVALID_REG : rl_index.reg.GetReg(), scale, data_offset,
...
void X86Mir2Lir::GenArrayPut(int opt_flags, OpSize size, RegLocation rl_array,
  ...
  if ((size == kSignedByte || size == kUnsignedByte) && rl_src.reg.GetReg() >= 4) {
    ...
    // FIXME: don't use INVALID_REG as a magic value.
    StoreBaseIndexedDisp(rl_array.reg.GetReg(), rl_index.reg.IsInvalid() ? INVALID_REG : rl_index.reg.GetReg(), scale, data_offset,
