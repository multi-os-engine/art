...
void X86Mir2Lir::GenCmpLong(RegLocation rl_dest, RegLocation rl_src1,
  ...
  if (Gen64Bit()) {
    ...
    OpRegReg(kOpCmp, rl_src1.reg, rl_src2.reg);
    OpCondRegReg(kOpCmov, kCondLt, rl_result.reg, temp_reg);  // result = (src1 < src2) ? -result : result
...
void X86Mir2Lir::GenLongArith(RegLocation rl_dest, RegLocation rl_src, Instruction::Code op) {
  ...
  LIR *lir = NewLIR3(x86op, r_base, displacement + LOWORD_OFFSET, Gen64Bit() ? rl_src.reg.GetReg() : rl_src.reg.GetLowReg());
  AnnotateDalvikRegAccess(lir, (displacement + LOWORD_OFFSET) >> 2,
                          true /* is_load */, true /* is64bit */);
  AnnotateDalvikRegAccess(lir, (displacement + LOWORD_OFFSET) >> 2,
...
void X86Mir2Lir::GenDivRemLong(Instruction::Code, RegLocation rl_dest, RegLocation rl_src1,
  ...
  // RHS is -1.
  // FIXME! We can set 0x8000000000000000 to r3q using only one movabs.
  NewLIR2(kX86Mov32RI, rs_r3q.GetReg(), 1);
  NewLIR2(kX86Sal64RI, rs_r3q.GetReg(), 63);
...
bool X86Mir2Lir::GenLongLongImm(RegLocation rl_dest, RegLocation rl_src1,
  ...
  if (Gen64Bit()) {
    ...
    if (rl_dest.location == kLocPhysReg && rl_src1.location == kLocPhysReg && !rl_dest.reg.IsFloat()) {
      X86OpCode x86op = GetOpcode(op, rl_dest, false, val);
