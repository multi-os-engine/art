...
static int Mips64NextSDCallInsn(CompilationUnit* cu, CallInfo* info ATTRIBUTE_UNUSED, int state,
  ...
  if (direct_code != 0 && direct_method != 0) {
    ...
    case 0: {  // Grab target method* from thread pointer
      ...
      uint32_t string_init_base_offset = Thread::QuickEntryPointOffsetWithSize(OFFSETOF_MEMBER(QuickEntryPoints, pNewEmptyString), kMips64PointerSize);
      int string_init_offset = string_init_base_offset + offset * kMips64PointerSize;
      cg->LoadRefDisp(rs_rMIPS64_SELF, string_init_offset, arg0_ref, kNotVolatile);
      break;
    }
      ...
      if (direct_code == 0) {
        ...
        cg->LoadWordDisp(arg0_ref,
                         mirror::ArtMethod::EntryPointFromQuickCompiledCodeOffset(
                             kMips64PointerSize).Int32Value(), cg->TargetPtrReg(kInvokeTgt));
      }
      break;
    default:
      return -1;
