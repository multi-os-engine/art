...
LIR* Mips64Mir2Lir::OpRegRegReg(OpKind op, RegStorage r_dest, RegStorage r_src1, RegStorage r_src2) {
  Mips64OpCode opcode = kMipsNop;
  switch (op) {
    case kOpAdd:
      opcode = kMipsAddu;
...
LIR* Mips64Mir2Lir::OpRegReg(OpKind op, RegStorage r_dest_src1, RegStorage r_src2) {
  ...
  switch (op) {
    ...
    case kOp2Byte:
      if (cu_->GetInstructionSetFeatures()->AsMips64InstructionSetFeatures()
          ->IsMips64IsaRevGreaterThanEqual2()) {
...
LIR* Mips64Mir2Lir::OpMovMemReg(RegStorage r_base, int offset, RegStorage r_src, MoveType move_type) {
  UNUSED(r_base, offset, r_src, move_type);
  UNIMPLEMENTED(FATAL);
  UNREACHABLE();
}

LIR* Mips64Mir2Lir::OpCondRegReg(OpKind op, ConditionCode cc, RegStorage r_dest, RegStorage r_src) {
...
// FIXME: don't split r_dest into 2 containers.
LIR* Mips64Mir2Lir::StoreBaseDispBody(RegStorage r_base, int displacement,
