...
RegStorage Mips64Mir2Lir::Solo64ToPair64(RegStorage reg) {
    DCHECK(reg.IsDouble());
...
ResourceMask Mips64Mir2Lir::GetRegMaskCommon(const RegStorage& reg) const {
  if (reg.IsDouble()) {
      return ResourceMask::TwoBits(reg.GetRegNum() * 2 + kMips64FPReg0);
  } else if (reg.IsSingle()) {
    return ResourceMask::Bit(reg.GetRegNum() + kMips64FPReg0);
  } else {
...
void Mips64Mir2Lir::SetupTargetResourceMasks(LIR* lir, uint64_t flags,
                                           ResourceMask* use_mask, ResourceMask* def_mask) {
  DCHECK_EQ(cu_->instruction_set, kMips64);
  ...
  if (flags & REG_DEF_HI) {
    def_mask->SetBit(kMips64RegHI);
  }

  if (flags & REG_DEF_LO) {
    def_mask->SetBit(kMips64RegLO);
  }

  if (flags & REG_USE_HI) {
    use_mask->SetBit(kMips64RegHI);
  }

  if (flags & REG_USE_LO) {
    use_mask->SetBit(kMips64RegLO);
  }
}
...
void Mips64Mir2Lir::DumpResourceMask(LIR *mips_lir, const ResourceMask& mask, const char *prefix) {
  char buf[256];
  buf[0] = 0;
...
void Mips64Mir2Lir::CompilerInitializeRegAlloc() {
  reg_pool_.reset(new (arena_) RegisterPool(this, arena_, core_regs, empty_pool /* core64 */, sp_regs,
...
RegStorage Mips64Mir2Lir::LoadHelper(QuickEntrypointEnum trampoline) {
  // NOTE: native pointer.
  LoadWordDisp(rs_rMIPS_SELF, GetThreadOffset<8>(trampoline).Int32Value(), rs_rT9);
  return rs_rT9;
}
