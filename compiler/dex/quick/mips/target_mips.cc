...
void MipsMir2Lir::SpillCoreRegs() {
  ...
  if (cu_->target64) {
    offset = 0;
    OpRegImm(kOpSub, rs_sp, num_core_spills_ * 8);
  } else {
  ...
  for (int reg = 0; mask; mask >>= 1, reg++) {
    if (mask & 0x1) {
      if (cu_->target64) {
        StoreWordDisp(rs_sp, offset, RegStorage::Solo64(reg));
        offset += 8;
      } else {
        offset -= 4;
        Store32Disp(rs_sp, offset, RegStorage::Solo32(reg));
      }
    }
  }
}
...
void MipsMir2Lir::UnSpillCoreRegs() {
  ...
  for (int reg = 0; mask; mask >>= 1, reg++) {
    if (mask & 0x1) {
      if (cu_->target64) {
        LoadWordDisp(rs_sp, offset, RegStorage::Solo64(reg));
        offset += 8;
      } else {
        offset -= 4;
        Load32Disp(rs_sp, offset, RegStorage::Solo32(reg));
      }
    }
  }
