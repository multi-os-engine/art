...
RegStorage MipsMir2Lir::InToRegStorageMipsMapper::GetNextReg(bool, bool is_wide, bool is_ref) {
  ...
  if (cur_core_reg_ < coreArgMappingToPhysicalRegSize) {
    result = ml_->TargetReg(coreArgMappingToPhysicalReg[cur_core_reg_++],
                          is_ref ? kRef : kNotWide);
    if (is_wide && cur_core_reg_ < coreArgMappingToPhysicalRegSize) {
      result = RegStorage::MakeRegPair(result,
              ml_->TargetReg(coreArgMappingToPhysicalReg[cur_core_reg_++], kNotWide));
    }
  }
  return result;
}
