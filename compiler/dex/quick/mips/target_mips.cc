...
void MipsMir2Lir::SpillCoreRegs() {
  ...
  if (cu_->target64) {
    offset = 0;
    OpRegImm(kOpSub, TargetPtrReg(kSp), num_core_spills_ * 8);
  } else {
    offset = num_core_spills_ * 4;
    OpRegImm(kOpSub, TargetPtrReg(kSp), offset);
  }
  for (int reg = 0; mask; mask >>= 1, reg++) {
    if (mask & 0x1) {
      if (cu_->target64) {
        StoreWordDisp(TargetPtrReg(kSp), offset, RegStorage::Solo64(reg));
        offset += 8;
      } else {
        offset -= 4;
        Store32Disp(TargetPtrReg(kSp), offset, RegStorage::Solo32(reg));
      }
    }
  }
}
...
void MipsMir2Lir::UnSpillCoreRegs() {
  ...
  if (cu_->target64) {
    offset -= num_core_spills_ * 8;
  }
  for (int reg = 0; mask; mask >>= 1, reg++) {
    if (mask & 0x1) {
      if (cu_->target64) {
        LoadWordDisp(TargetPtrReg(kSp), offset, RegStorage::Solo64(reg));
        offset += 8;
      } else {
        offset -= 4;
        Load32Disp(TargetPtrReg(kSp), offset, RegStorage::Solo32(reg));
      }
    }
  }
