...
static RegStorage core_regs[] = {rs_rZERO, rs_rAT, rs_rV0, rs_rV1, rs_rA0, rs_rA1, rs_rA2, rs_rA3,
                                 rs_rT0, rs_rT1, rs_rT2, rs_rT3, rs_rT4, rs_rT5, rs_rT6, rs_rT7,
                                 rs_rS0, rs_rS1, rs_rS2, rs_rS3, rs_rS4, rs_rS5, rs_rS6, rs_rS7,
                                 rs_rT8, rs_rT9, rs_rK0, rs_rK1, rs_rGP, rs_rSP, rs_rFP, rs_rRA};
static RegStorage ReservedRegs[] = {rs_rZERO, rs_rAT, rs_rS0, rs_rS1, rs_rK0, rs_rK1, rs_rGP,
                                    rs_rSP, rs_rRA};
static RegStorage core_temps[] = {rs_rV0, rs_rV1, rs_rA0, rs_rA1, rs_rA2, rs_rA3, rs_rT0, rs_rT1,
                                  rs_rT2, rs_rT3, rs_rT4, rs_rT5, rs_rT6, rs_rT7, rs_rT8};
static RegStorage FpRegs[] = {rs_rF0, rs_rF1, rs_rF2, rs_rF3, rs_rF4, rs_rF5, rs_rF6, rs_rF7,
                              rs_rF8, rs_rF9, rs_rF10, rs_rF11, rs_rF12, rs_rF13, rs_rF14, rs_rF15};
static RegStorage fp_temps[] = {rs_rF0, rs_rF1, rs_rF2, rs_rF3, rs_rF4, rs_rF5, rs_rF6, rs_rF7,
                                rs_rF8, rs_rF9, rs_rF10, rs_rF11, rs_rF12, rs_rF13, rs_rF14,
                                rs_rF15};
...
void MipsMir2Lir::CompilerInitializeRegAlloc() {
  ...
  int num_sp_regs = sizeof(FpRegs)/sizeof(*FpRegs);
  ...
  reg_pool_->num_sp_regs = num_sp_regs;
  reg_pool_->sp_regs = static_cast<RegisterInfo*>
      (arena_->Alloc(num_sp_regs * sizeof(*reg_pool_->sp_regs), kArenaAllocRegAlloc));
  CompilerInitPool(reg_pool_->core_regs, core_regs, reg_pool_->num_core_regs);
  CompilerInitPool(reg_pool_->sp_regs, FpRegs, reg_pool_->num_sp_regs);
  // Keep special registers from being allocated
  for (int i = 0; i < num_reserved; i++) {
    if (NO_SUSPEND && (ReservedRegs[i] == rs_rMIPS_SUSPEND)) {
...
RegStorage MipsMir2Lir::AllocTempDouble() {
  RegisterInfo* p = reg_pool_->sp_regs;
  int num_regs = reg_pool_->num_sp_regs;
  ...
  for (int i = 0; i < num_regs; i+=2) {
    ...
    if ((p[next].is_temp && !p[next].InUse() && !p[next].live) &&
      ...
      p[next].MarkInUse();
      p[next+1].MarkInUse();
      DCHECK_EQ((p[next].reg.GetRegNum()+1), p[next+1].reg.GetRegNum());
      DCHECK_EQ((p[next].reg.GetRegNum() & 0x1), 0);
      reg_pool_->next_fp_reg = next + 2;
      if (reg_pool_->next_fp_reg >= num_regs) {
        reg_pool_->next_fp_reg = 0;
      }
      // FIXME: should return k64BitSolo.
      return RegStorage(RegStorage::k64BitPair, p[next].reg.GetReg(), p[next+1].reg.GetReg());
    }
    next += 2;
  }
...
RegStorage MipsMir2Lir::AllocPreservedDouble(int s_reg) {
  UNIMPLEMENTED(WARNING) << "Needs promotion support (can probably reuse Arm version?";
  return RegStorage::InvalidReg();
}
