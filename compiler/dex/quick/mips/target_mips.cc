...
ResourceMask MipsMir2Lir::GetRegMaskCommon(const RegStorage& reg) const {
  return reg.IsDouble()
      ...
      ? ResourceMask::RawMask(UINT64_C(0x3) << (reg.GetRegNum() * 2 + kMipsFPReg0), 0u)
...
void MipsMir2Lir::SetupTargetResourceMasks(LIR* lir, uint64_t flags,
  ...
  if (flags & REG_DEF_SP) {
    def_mask->SetTargetSpecificBit(kMipsRegSP);
  }

  if (flags & REG_USE_SP) {
    use_mask->SetTargetSpecificBit(kMipsRegSP);
  }

  if (flags & REG_DEF_LR) {
    def_mask->SetTargetSpecificBit(kMipsRegLR);
  }

  if (flags & REG_DEF_HI) {
    def_mask->SetTargetSpecificBit(kMipsRegHI);
  }

  if (flags & REG_DEF_LO) {
    def_mask->SetTargetSpecificBit(kMipsRegLO);
  }

  if (flags & REG_USE_HI) {
    use_mask->SetTargetSpecificBit(kMipsRegHI);
  }

  if (flags & REG_USE_LO) {
    use_mask->SetTargetSpecificBit(kMipsRegLO);
  }
}
