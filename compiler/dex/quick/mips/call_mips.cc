...
void MipsMir2Lir::GenLargeSparseSwitch(MIR* mir, DexOffset table_offset, RegLocation rl_src) {
  ...
  RegStorage r_end = cu_->target64 ? AllocTempWide() : AllocTemp();
  if (size_hi) {
  ...
  RegStorage r_base = cu_->target64 ? AllocTempWide() : AllocTemp();
  NewLIR4(cu_->target64 ? kMips64Delta : kMips32Delta, r_base.GetReg(), 0, WrapPointer(base_label),
          WrapPointer(tab_rec));
  ...
  OpRegRegReg(kOpAdd, TargetPtrReg(kLr), TargetPtrReg(kLr), r_disp);
...
void MipsMir2Lir::GenLargePackedSwitch(MIR* mir, DexOffset table_offset, RegLocation rl_src) {
  ...
  if (cu_->target64) {
    r_base = AllocTempWide();
    NewLIR4(kMips64Delta, r_base.GetReg(), 0, WrapPointer(base_label), WrapPointer(tab_rec));
  } else {
    r_base = AllocTemp();
    NewLIR4(kMips32Delta, r_base.GetReg(), 0, WrapPointer(base_label), WrapPointer(tab_rec));
  }
...
void MipsMir2Lir::UnconditionallyMarkGCCard(RegStorage tgt_addr_reg) {
  ...
  if (cu_->target64) {
    reg_card_base = AllocTempWide();
    reg_card_no = AllocTempWide();
    // NOTE: native pointer.
    ...
    reg_card_base = AllocTemp();
    reg_card_no = AllocTemp();
    // NOTE: native pointer.
...
void MipsMir2Lir::GenEntrySequence(RegLocation* ArgLocs, RegLocation rl_method) {
  ...
  /*
   * On entry, rMIPS_ARG0, rMIPS_ARG1, rMIPS_ARG2 & rMIPS_ARG3 are live.
   * On Mips64, rMIPS64_ARG4, rMIPS64_ARG5, rMIPS64_ARG6 & rMIPS64_ARG7 are also live.
  ...
  RegStorage check_reg;
  RegStorage new_sp;
  InstructionSet target = (cu_->target64) ? kMips64 : kMips;
  int ptrSize = cu_->target64 ? 8 : 4;
  ...
  if (cu_->target64) {
    check_reg = AllocTempWide();
    new_sp = AllocTempWide();
  } else {
    check_reg = AllocTemp();
    new_sp = AllocTemp();
  }
  ...
  const int frame_sub = frame_size_ - spill_count * ptrSize;
  if (!skip_overflow_check) {
    class StackOverflowSlowPath : public LIRSlowPath {
     public:
      ...
      void Compile() OVERRIDE {
        ...
        // Mips32: RA is offset 0 since we push in reverse order.
        // Mips64: Load RA from the top of the frame.
        m2l_->LoadWordDisp(m2l_->TargetPtrReg(kSp), cu_->target64 ? sp_displace_ - 8 : 0,
                           m2l_->TargetPtrReg(kLr));
    ...
    OpRegRegImm(kOpSub, new_sp, TargetPtrReg(kSp), frame_sub);
    LIR* branch = OpCmpBranch(kCondUlt, new_sp, check_reg, nullptr);
    AddSlowPath(new(arena_)StackOverflowSlowPath(this, branch, spill_count * ptrSize));
    // TODO: avoid copy for small frame sizes.
    OpRegCopy(TargetPtrReg(kSp), new_sp);  // Establish stack.
  } else {
    OpRegImm(kOpSub, TargetPtrReg(kSp), frame_sub);
  }
...
void MipsMir2Lir::GenSpecialEntryForSuspend() {
  ...
  OpRegImm(kOpSub, TargetPtrReg(kSp), frame_size_);
  StoreWordDisp(TargetPtrReg(kSp), frame_size_ - (cu_->target64 ? 8 : 4), TargetPtrReg(kLr));
  StoreWordDisp(TargetPtrReg(kSp), 0, TargetPtrReg(kArg0));
}

void MipsMir2Lir::GenSpecialExitForSuspend() {
  // Pop the frame. Don't pop ArtMethod*, it's no longer needed.
  LoadWordDisp(TargetPtrReg(kSp), frame_size_ - (cu_->target64 ? 8 : 4), TargetPtrReg(kLr));
  OpRegImm(kOpAdd, TargetPtrReg(kSp), frame_size_);
}
