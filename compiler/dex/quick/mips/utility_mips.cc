...
LIR* MipsMir2Lir::LoadConstantNoClobber(RegStorage r_dest, int value) {
  ...
  if (value == 0) {
    res = NewLIR2(kMipsMove, r_dest.GetReg(), r_ZERO);
  } else if ((value > 0) && (value <= 65535)) {
    res = NewLIR3(kMipsOri, r_dest.GetReg(), r_ZERO, value);
  } else if ((value < 0) && (value >= -32768)) {
    res = NewLIR3(kMipsAddiu, r_dest.GetReg(), r_ZERO, value);
  } else {
...
LIR* MipsMir2Lir::OpReg(OpKind op, RegStorage r_dest_src) {
  ...
  return NewLIR2(opcode, r_RA, r_dest_src.GetReg());
}
...
LIR* MipsMir2Lir::OpRegReg(OpKind op, RegStorage r_dest_src1, RegStorage r_src2) {
  ...
  switch (op) {
    case kOpMov:
      ...
      break;
    case kOpMvn:
      return NewLIR3(kMipsNor, r_dest_src1.GetReg(), r_src2.GetReg(), r_ZERO);
    case kOpNeg:
      return NewLIR3(kMipsSubu, r_dest_src1.GetReg(), r_ZERO, r_src2.GetReg());
    case kOpAdd:
    case kOpAnd:
    case kOpMul:
    case kOpOr:
...
LIR* MipsMir2Lir::LoadConstantWide(int r_dest_lo, int r_dest_hi, int64_t value) {
  LIR *res;
  res = LoadConstantNoClobber(RegStorage(RegStorage::k32BitSolo, r_dest_lo), Low32Bits(value));
  LoadConstantNoClobber(RegStorage(RegStorage::k32BitSolo, r_dest_hi), High32Bits(value));
  return res;
}
...
LIR* MipsMir2Lir::LoadBaseIndexed(int r_base, int r_index, int r_dest,
  ...
  if (!scale) {
    ...
    first = OpRegRegImm(kOpLsl, RegStorage(RegStorage::k32BitSolo, t_reg),
                        RegStorage(RegStorage::k32BitSolo, r_index), scale);
 ...
 */
  ...
  if (short_form) {
    ...
    if (pair) {
      ...
      res = OpRegRegImm(kOpAdd, r_tmp, RegStorage(RegStorage::k32BitSolo, r_base), displacement);
      load = NewLIR3(opcode, r_dest, LOWORD_OFFSET, r_tmp.GetReg());
      load2 = NewLIR3(opcode, r_dest_hi, HIWORD_OFFSET, r_tmp.GetReg());
      FreeTemp(r_tmp);
    } else {
      RegStorage r_tmp = (r_base == r_dest) ? AllocTemp() :
                          RegStorage(RegStorage::k32BitSolo, r_dest);
      res = OpRegRegImm(kOpAdd, r_tmp, RegStorage(RegStorage::k32BitSolo, r_base), displacement);
      load = NewLIR3(opcode, r_dest, 0, r_tmp.GetReg());
      if (r_tmp.GetReg() != r_dest)
        FreeTemp(r_tmp);
    }
  }
...
LIR* MipsMir2Lir::StoreBaseDispBody(int r_base, int displacement,
  ...
  if (short_form) {
    ...
    RegStorage r_scratch = AllocTemp();
    res = OpRegRegImm(kOpAdd, r_scratch, RegStorage(RegStorage::k32BitSolo, r_base), displacement);
    if (!pair) {
      store =  NewLIR3(opcode, r_src, 0, r_scratch.GetReg());
    } else {
      store =  NewLIR3(opcode, r_src, LOWORD_OFFSET, r_scratch.GetReg());
      store2 = NewLIR3(opcode, r_src_hi, HIWORD_OFFSET, r_scratch.GetReg());
    }
    FreeTemp(r_scratch);
  }
