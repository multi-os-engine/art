 ...
 */
  ...
  if (cu_->target64) {
    if (short_form) {
      if (displacement & 0x7 && opcode == kMips64Ld) {
        ...
        FreeTemp(r_tmp);
      } else if (displacement & 0x7 && opcode == kMipsFldc1) {
        RegStorage r_tmp = AllocTemp();
        r_dest = Fp64ToSolo32(r_dest);
...
LIR* MipsMir2Lir::StoreBaseDispBody(RegStorage r_base, int displacement, RegStorage r_src,
  ...
  if (cu_->target64) {
    if (short_form) {
      if (displacement & 0x7 && opcode == kMips64Sd) {
        RegStorage r_tmp = AllocTemp();
        ...
        FreeTemp(r_tmp);
      } else if (displacement & 0x7 && opcode == kMipsFsdc1) {
        RegStorage r_tmp = AllocTemp();
