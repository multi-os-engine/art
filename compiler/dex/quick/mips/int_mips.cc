...
void MipsMir2Lir::OpRegCopyWide(int dest_lo, int dest_hi, int src_lo,
  ...
  if (dest_fp) {
    ...
    if (src_fp) {
      ...
      // Handle overlap
      if (src_hi == dest_lo) {
        OpRegCopy(RegStorage::Solo32(dest_hi), RegStorage::Solo32(src_hi));
        OpRegCopy(RegStorage::Solo32(dest_lo), RegStorage::Solo32(src_lo));
      } else {
        OpRegCopy(RegStorage::Solo32(dest_lo), RegStorage::Solo32(src_lo));
        OpRegCopy(RegStorage::Solo32(dest_hi), RegStorage::Solo32(src_hi));
      }
...
RegLocation MipsMir2Lir::GenDivRem(RegLocation rl_dest, RegStorage reg1, RegStorage reg2,
                                    bool is_div) {
  NewLIR4(kMipsDiv, r_HI, r_LO, reg1.GetReg(), reg2.GetReg());
  RegLocation rl_result = EvalLoc(rl_dest, kCoreReg, true);
  if (is_div) {
...
RegLocation MipsMir2Lir::GenDivRemLit(RegLocation rl_dest, RegStorage reg1, int lit,
  ...
  NewLIR4(kMipsDiv, r_HI, r_LO, reg1.GetReg(), t_reg);
  RegLocation rl_result = EvalLoc(rl_dest, kCoreReg, true);
  if (is_div) {
    NewLIR2(kMipsMflo, rl_result.reg.GetReg(), r_LO);
  } else {
...
void MipsMir2Lir::GenDivZeroCheck(RegStorage reg_lo, RegStorage reg_hi) {
  RegStorage t_reg = AllocTemp();
...
LIR* MipsMir2Lir::OpTestSuspend(LIR* target) {
  OpRegImm(kOpSub, rs_MIPS_SUSPEND, 1);
  return OpCmpImmBranch((target == NULL) ? kCondEq : kCondNe, rs_MIPS_SUSPEND, 0, target);
}
