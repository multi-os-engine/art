...
LIR* MipsMir2Lir::OpTestSuspend(LIR* target) {
  RegStorage r_suspend(RegStorage::k32BitSolo, rMIPS_SUSPEND);
  OpRegImm(kOpSub, r_suspend, 1);
  return OpCmpImmBranch((target == NULL) ? kCondEq : kCondNe, r_suspend, 0, target);
}
...
void MipsMir2Lir::GenNegLong(RegLocation rl_dest, RegLocation rl_src) {
  ...
  RegStorage t_reg = AllocTemp();
  NewLIR3(kMipsSltu, t_reg.GetReg(), r_ZERO, rl_result.reg.GetLowReg());
  OpRegRegReg(kOpSub, rl_result.reg.GetHigh(), rl_result.reg.GetHigh(), t_reg);
  FreeTemp(t_reg);
  StoreValueWide(rl_dest, rl_result);
}
...
void MipsMir2Lir::GenArrayGet(int opt_flags, OpSize size, RegLocation rl_array,
  ...
  if ((size == kLong) || (size == kDouble)) {
    ...
    LoadBaseDispWide(reg_ptr.GetReg(), 0, rl_result.reg.GetLowReg(), rl_result.reg.GetHighReg(), INVALID_SREG);
...
void MipsMir2Lir::GenArrayPut(int opt_flags, OpSize size, RegLocation rl_array,
  ...
  rl_array = LoadValue(rl_array, kCoreReg);
  rl_index = LoadValue(rl_index, kCoreReg);
  RegStorage reg_ptr(RegStorage::kInvalid);
  ...
  if ((size == kLong) || (size == kDouble)) {
    ...
    StoreBaseIndexed(reg_ptr.GetReg(), rl_index.reg.GetReg(), rl_src.reg.GetReg(),
                     scale, size);
  }
