...
LIR* Mir2Lir::CallHelper(int r_tgt, ThreadOffset helper_offset, bool safepoint_pc) {
  ...
  if (cu_->instruction_set == kX86) {
    ...
    call_inst = OpReg(kOpBlx, RegStorage(RegStorage::k32BitSolo, r_tgt));
    FreeTemp(r_tgt);
  }
  if (safepoint_pc) {
  ...
  for (int i = 0; i < cu_->num_ins; i++) {
    ...
    if (reg != INVALID_REG) {
      ...
      if ((v_map->core_location == kLocPhysReg) && !t_loc->fp) {
        OpRegCopy(RegStorage(RegStorage::k32BitSolo, v_map->core_reg),
                  RegStorage(RegStorage::k32BitSolo, reg));
        need_flush = false;
      } else if ((v_map->fp_location == kLocPhysReg) && t_loc->fp) {
        OpRegCopy(RegStorage(RegStorage::k32BitSolo, v_map->FpReg),
                  RegStorage(RegStorage::k32BitSolo, reg));
        need_flush = false;
      } else {
        need_flush = true;
...
int Mir2Lir::LoadArgRegs(CallInfo* info, int call_state,
  ...
  for (; (next_reg <= last_arg_reg) && (next_arg < info->num_arg_words); next_reg++) {
    ...
    if (rl_arg.wide && (next_reg <= TargetReg(kArg2).GetReg())) {
      ...
      LoadValueDirectFixed(rl_arg, RegStorage(RegStorage::k32BitSolo, next_reg));
    }
    ...
    while (next_use < info->num_arg_words) {
      ...
      if (rl_arg.location == kLocPhysReg) {
        ...
        if (rl_arg.wide) {
          ...
          LoadValueDirectFixed(rl_arg, RegStorage(RegStorage::k32BitSolo, low_reg));
        }
        call_state = next_call_insn(cu_, info, call_state, target_method,
                                    vtable_idx, direct_code, direct_method, type);
      }
...
bool Mir2Lir::GenInlinedCharAt(CallInfo* info) {
  ...
  if (cu_->instruction_set != kX86) {
    ...
    if (range_check) {
      ...
      intrinsic_launchpads_.Insert(launch_pad);
      if (rl_idx.is_const) {
        RegStorage invalid_reg;
        OpCmpMemImmBranch(kCondUlt, invalid_reg, rl_obj.reg, count_offset,
  ...
  if (cu_->instruction_set != kX86) {
    ...
    LoadBaseIndexedDisp(reg_ptr.GetReg(), reg_off.GetReg(), 1, data_offset, rl_result.reg.GetReg(),
...
bool Mir2Lir::GenInlinedAbsLong(CallInfo* info) {
  ...
  if (cu_->instruction_set == kThumb2) {
    ...
    RegLocation rl_dest = InlineTargetWide(info);
    RegLocation rl_result = EvalLoc(rl_dest, kCoreReg, true);
    OpRegCopyWide(rl_result.reg.GetLowReg(), rl_result.reg.GetHighReg(), rl_src.reg.GetLowReg(), rl_src.reg.GetHighReg());
    FreeTemp(rl_src.reg.GetLowReg());
    FreeTemp(rl_src.reg.GetHighReg());
    RegStorage sign_reg = AllocTemp();
    // abs(x) = y<=x>>31, (x+y)^y.
    OpRegRegImm(kOpAsr, sign_reg, rl_result.reg.GetHigh(), 31);
    OpRegReg(kOpAdd, rl_result.reg.GetLow(), sign_reg);
    OpRegReg(kOpAdc, rl_result.reg.GetHigh(), sign_reg);
    OpRegReg(kOpXor, rl_result.reg.GetLow(), sign_reg);
    OpRegReg(kOpXor, rl_result.reg.GetHigh(), sign_reg);
    StoreValueWide(rl_dest, rl_result);
    return true;
...
bool Mir2Lir::GenInlinedAbsFloat(CallInfo* info) {
  ...
  RegLocation rl_dest = InlineTarget(info);
  RegLocation rl_result = EvalLoc(rl_dest, kCoreReg, true);
  RegStorage signMask = AllocTemp();
  LoadConstant(signMask, 0x7fffffff);
  OpRegRegReg(kOpAnd, rl_result.reg, rl_src.reg, signMask);
  FreeTemp(signMask);
  StoreValue(rl_dest, rl_result);
  return true;
}
...
bool Mir2Lir::GenInlinedAbsDouble(CallInfo* info) {
  ...
  RegStorage signMask = AllocTemp();
  LoadConstant(signMask, 0x7fffffff);
  OpRegReg(kOpAnd, rl_result.reg.GetHigh(), signMask);
  FreeTemp(signMask);
  StoreValueWide(rl_dest, rl_result);
  return true;
}
...
bool Mir2Lir::GenInlinedIndexOf(CallInfo* info, bool zero_based) {
  ...
  RegStorage r_tgt(RegStorage::k32BitSolo, LoadHelper(QUICK_ENTRYPOINT_OFFSET(pIndexOf)));
...
bool Mir2Lir::GenInlinedStringCompareTo(CallInfo* info) {
  ...
  RegStorage r_tgt(RegStorage::k32BitSolo, (cu_->instruction_set != kX86) ?
...
bool Mir2Lir::GenInlinedUnsafeGet(CallInfo* info,
  ...
  if (is_long) {
    ...
    LoadBaseDispWide(rl_object.reg.GetReg(), 0, rl_result.reg.GetLowReg(), rl_result.reg.GetHighReg(), INVALID_SREG);
    StoreValueWide(rl_dest, rl_result);
  } else {
...
bool Mir2Lir::GenInlinedUnsafePut(CallInfo* info, bool is_long,
  ...
  if (is_long) {
    ...
    OpRegReg(kOpAdd, rl_object.reg, rl_offset.reg);
    StoreBaseDispWide(rl_object.reg.GetReg(), 0, rl_value.reg.GetLowReg(), rl_value.reg.GetHighReg());
  } else {
