...
bool Mir2Lir::GenInlinedGet(CallInfo* info) {
  ...
  Load32Disp(TargetReg(kArg1), mirror::ReferenceClass::SlowPathEnabledOffset().Int32Value(),
      reg_slow_path);
  RegStorage reg_mask = AllocTemp();
  Load32Disp(TargetReg(kArg1), mirror::ReferenceClass::DisableIntrinsicOffset().Int32Value(),
      reg_mask);
