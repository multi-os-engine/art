  ...
  if (pcrLabel) {
    if (Runtime::Current()->ExplicitNullChecks()) {
      ...
      *pcrLabel = nullptr;
      ...
      GenImplicitNullCheck(TargetReg(kArg1), info->opt_flags);
    }
  }
  return call_state;
}
...
bool Mir2Lir::GenInlinedCharAt(CallInfo* info) {
  ...
  if (cu_->instruction_set != kX86 && cu_->instruction_set != kX86_64) {
    ...
    if (range_check) {
      ...
      if (rl_idx.is_const) {
        int check_value = mir_graph_->ConstantValue(rl_idx.orig_sreg);
        NewLIR3(IS_SIMM8(check_value) ?  kX86Cmp32MI8 : kX86Cmp32MI,
            rl_obj.reg.GetReg(), count_offset,
            check_value);
        MarkPossibleNullPointerException(0);
        range_check_branch = OpCondBranch(kCondUlt, nullptr);
     } else {
