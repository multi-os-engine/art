    ...
    if (reg.Valid()) {
      ...
      if (t_loc->location == kLocPhysReg) {
        ...
        if (t_loc->ref) {
          ...
          LoadBaseDisp(TargetPtrReg(kSp), offset, t_loc->reg, t_loc->wide ? k64 : k32, kNotVolatile);
        }
      }
    }
  }
 ...
 * argument ("this") into kArg1 here rather than the standard GeDalvikArgs.
 */
 ...
 * Please note that you should first flush promoted registers first if you copy.
 * If implementation does copying it may skip several first VRs but must copy
 ...
 * @param count the number of rest arguments we can handle.
 * @return the number of arguments which we did not handled. Unhandled arguments
...
int Mir2Lir::GenDalvikArgs(CallInfo* info, int call_state,
  ...
  const int first_after_last_mapped_in = in_to_reg_storage_mapping.GetMaxMappedIn() + 1;
  int regs_left_to_pass_via_stack = info->num_arg_words - first_after_last_mapped_in;

  // If it is a range case we can try to copy rest VRs (not mapped to physical registers)
  // using more optimial algorithm.
  if (info->is_range && regs_left_to_pass_via_stack > 1) {
    // It is possible that the last mapped reg is 32 bit while arg is 64-bit.
    // We'd like to handle this case while we're handling mapped regs, so we should skip it here.
    DCHECK_LT(first_after_last_mapped_in, info->num_arg_words);
    int adjust = info->args[first_after_last_mapped_in].high_word ? 1 : 0;
    regs_left_to_pass_via_stack = GenDalvikArgsBulkCopy(info,
                                                        first_after_last_mapped_in + adjust,
                                                        regs_left_to_pass_via_stack - adjust);
  }

  // Now handle rest VRs mapped to stack if they are.
  if (in_to_reg_storage_mapping.IsThereStackMapped()) {
    ...
    for (int i = start_index;
         i < first_after_last_mapped_in + regs_left_to_pass_via_stack; i++) {
  ...
  for (int i = start_index; i < first_after_last_mapped_in; i++) {
    RegLocation rl_arg = info->args[i];
    rl_arg = UpdateRawLoc(rl_arg);
    RegStorage reg = in_to_reg_storage_mapping.Get(i);
    if (reg.Valid()) {
      if (rl_arg.wide) {
        ...
        if (!reg.Is64Bit()) {
          ...
          if (rl_arg.location == kLocPhysReg) {
            ...
            LoadBaseDisp(TargetPtrReg(kSp), SRegOffset(rl_arg.s_reg_low + 1), reg, k32, kNotVolatile);
