...
RegLocation Mir2Lir::InlineTarget(CallInfo* info) {
  ...
  if (info->result.location == kLocInvalid) {
    ...
    res = info->result;
    // TODO: Update CallInfo for string init to StringFactory so return value is correct.
...
bool Mir2Lir::GenInlinedStringFactoryNewStringFromString(CallInfo* info) {
  ...
  CallRuntimeHelperRegLocation(kQuickAllocStringFromString, rl_arg0, true);
  RegLocation rl_return = GetReturn(kRefReg);
  RegLocation rl_dest = InlineTarget(info);
  StoreValue(rl_dest, rl_return);
  return true;
...
void Mir2Lir::GenInvokeNoInline(CallInfo* info) {
  ...
  if (PrettyMethod(method_info.GetTargetMethod().dex_method_index, *method_info.GetTargetMethod().dex_file).find("java.lang.String.<init>") != std::string::npos) {
    info->string_init = true;
  }
