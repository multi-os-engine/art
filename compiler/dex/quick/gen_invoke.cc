...
void Mir2Lir::FlushIns(RegLocation* ArgLocs, RegLocation rl_method) {
  ...
  // If 64-bit targets, explicitly flush 8 bytes of Method* to workaround an issue that rl_method occupies only 4 bytes, as rl_method is not a wide register.  Or if Method* has been promoted, explicitly flush.
  // TODO.  After we can use rl_method as a wide register, we will drop cu_->target64.
