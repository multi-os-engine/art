  ...
  for (int i = 0; i < cu_->num_ins; i++) {
    ...
    if (!reg.IsInvalid()) {
      ...
      if ((v_map->core_location == kLocPhysReg) && !t_loc->fp) {
        OpRegCopy(RegStorage(RegStorage::k32BitSolo, v_map->core_reg), reg);
        need_flush = false;
      } else if ((v_map->fp_location == kLocPhysReg) && t_loc->fp) {
        OpRegCopy(RegStorage(RegStorage::k32BitSolo, v_map->FpReg), reg);
        need_flush = false;
      } else {
        need_flush = true;
      }
      ...
      if (v_map->core_location == kLocPhysReg) {
        LoadWordDisp(TargetReg(kSp), SRegOffset(start_vreg + i),
                     RegStorage(RegStorage::k32BitSolo, v_map->core_reg));
      }
      if (v_map->fp_location == kLocPhysReg) {
        LoadWordDisp(TargetReg(kSp), SRegOffset(start_vreg + i),
                     RegStorage(RegStorage::k32BitSolo, v_map->FpReg));
      }
    }
  }
}
    ...
    while (next_use < info->num_arg_words) {
      ...
      if (rl_arg.location == kLocPhysReg) {
        ...
        if (rl_arg.wide) {
          ...
          LoadValueDirectWideFixed(rl_arg, low_reg.GetReg(), high_reg.GetReg());
        } else {
      ...
      if (rl_arg.wide) {
        StoreBaseDispWide(TargetReg(kSp).GetReg(), outs_offset, low_reg.GetReg(), high_reg.GetReg());
        next_use += 2;
      } else {
