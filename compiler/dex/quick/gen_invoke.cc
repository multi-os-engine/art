...
void Mir2Lir::GenInvoke(CallInfo* info) {
  if (PrettyMethod(info->index, *mir_graph_->GetCurrentDexCompilationUnit()->GetDexFile()) == "void java.lang.String.<init>(java.lang.String)") {
    LOG(INFO) << " GenInvoke @ " << std::hex << info->offset << std::dec << " of type " << info->type << " on idx " << info->index << " (" << PrettyMethod(info->index, *mir_graph_->GetCurrentDexCompilationUnit()->GetDexFile()) << ") in " << PrettyMethod(cu_->method_idx, *cu_->dex_file);
  }
