  ...
  switch (state) {
    ...
    case 2:
      if (Runtime::Current()->IsEmbeddedImtAndVTableEnabled()) {
        ...
        cg->LoadRefDisp(cg->TargetRefReg(kArg0), mirror::Class::VTableOffset().Int32Value(),
    ...
    case 3:
      if (Runtime::Current()->IsEmbeddedImtAndVTableEnabled()) {
        int32_t offset = mirror::Class::EmbeddedImTableOffset().Uint32Value() +
            (method_idx % ClassLinker::kImtSize) * sizeof(mirror::Class::ImTableEntry);
        // Load target method to kArg0.
        cg->LoadRefDisp(cg->TargetRefReg(kArg0), offset,
                        cg->TargetRefReg(kArg0),
                        kNotVolatile);
        ...
        cg->LoadRefDisp(cg->TargetRefReg(kArg0), mirror::Class::ImTableOffset().Int32Value(),
                        cg->TargetRefReg(kInvokeTgt),
                        kNotVolatile);
      }
      break;
