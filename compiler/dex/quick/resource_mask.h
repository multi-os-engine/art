...
class ResourceMask {
 ...
 public:
  ...
  void SetResourceBit(ResourceBit bit) {
    DCHECK_GE(bit, kLowestCommonResource);
    DCHECK_LE(bit, kHighestCommonResource);
    COMPILE_ASSERT(kLowestCommonResource >= 64 && kHighestCommonResource < 128, using_masks_1);
    masks_[1] |= UINT64_C(1) << (bit & 63u);
  }

  void SetTargetSpecificBit(size_t bit) {
    DCHECK_LE(bit, kHighestCommonResource);
 ...
 private:
  ...
  friend class UseOrDefMaskCache;
};

constexpr ResourceMask kEncodeNone = ResourceMask::RawMask(0u, 0u);
constexpr ResourceMask kEncodeAll = ResourceMask::RawMask(~UINT64_C(0), ~UINT64_C(0));
...
constexpr ResourceMask kEncodeLiteralOrDalvikReg = kEncodeLiteral.Union(kEncodeDalvikReg);
constexpr ResourceMask kEncodeMem = kEncodeLiteralOrDalvikReg.Union(kEncodeHeapRef).Union(
    ResourceMask::Bit(ResourceMask::kMustNotAlias));
