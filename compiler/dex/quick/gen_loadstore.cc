  ...
  if (rl_dest.fp) {
    ...
    if (promotion_map_[pmap_index].fp_location == kLocPhysReg) {
      ...
      if (promotion_map_[pmap_index].core_location == kLocPhysReg) {
        ...
        StoreBaseDisp(TargetRefReg(kSp), SRegOffset(rl_dest.s_reg_low), temp_reg, k32, kNotVolatile);
      }
      if (!zero_reg.Valid()) {
        FreeTemp(temp_reg);
      }
...
void Mir2Lir::LoadValueDirect(RegLocation rl_src, RegStorage r_dest) {
  ...
  if (rl_src.location == kLocPhysReg) {
    ...
    ScopedMemRefType mem_ref_type(this, ResourceMask::kDalvikReg);
    if (rl_src.ref) {
      LoadRefDisp(TargetRefReg(kSp), SRegOffset(rl_src.s_reg_low), r_dest, kNotVolatile);
    } else {
      Load32Disp(TargetRefReg(kSp), SRegOffset(rl_src.s_reg_low), r_dest);
    }
  }
}
...
void Mir2Lir::LoadValueDirectWide(RegLocation rl_src, RegStorage r_dest) {
  ...
  if (rl_src.location == kLocPhysReg) {
    ...
    DCHECK((rl_src.location == kLocDalvikFrame) ||
           (rl_src.location == kLocCompilerTemp));
    ScopedMemRefType mem_ref_type(this, ResourceMask::kDalvikReg);
    LoadBaseDisp(TargetRefReg(kSp), SRegOffset(rl_src.s_reg_low), r_dest, k64, kNotVolatile);
  }
}
...
void Mir2Lir::StoreValue(RegLocation rl_dest, RegLocation rl_src) {
  ...
  if (IsDirty(rl_dest.reg) && LiveOut(rl_dest.s_reg_low)) {
    ...
    ScopedMemRefType mem_ref_type(this, ResourceMask::kDalvikReg);
    if (rl_dest.ref) {
      StoreRefDisp(TargetRefReg(kSp), SRegOffset(rl_dest.s_reg_low), rl_dest.reg, kNotVolatile);
    } else {
      Store32Disp(TargetRefReg(kSp), SRegOffset(rl_dest.s_reg_low), rl_dest.reg);
    }
    MarkClean(rl_dest);
...
void Mir2Lir::StoreValueWide(RegLocation rl_dest, RegLocation rl_src) {
  ...
  if (IsDirty(rl_dest.reg) && (LiveOut(rl_dest.s_reg_low) ||
    ...
    ScopedMemRefType mem_ref_type(this, ResourceMask::kDalvikReg);
    StoreBaseDisp(TargetRefReg(kSp), SRegOffset(rl_dest.s_reg_low), rl_dest.reg, k64, kNotVolatile);
    MarkClean(rl_dest);
    def_end = last_lir_insn_;
