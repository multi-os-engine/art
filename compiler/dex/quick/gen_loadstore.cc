...
LIR* Mir2Lir::LoadWordDisp(int r_base, int displacement, int r_dest) {
...
LIR* Mir2Lir::StoreWordDisp(int r_base, int displacement, int r_src) {
...
LIR* Mir2Lir::StoreWordDisp(RegStorage r_base, int displacement, RegStorage r_src) {
  return StoreWordDisp(r_base.GetReg(), displacement, r_src.GetReg());
}
...
void Mir2Lir::LoadValueDirectWide(RegLocation rl_src, int reg_lo,
  ...
  if (rl_src.location == kLocPhysReg) {
    OpRegCopyWide(reg_lo, reg_hi, rl_src.reg.GetLowReg(), rl_src.reg.GetHighReg());
  } else if (IsInexpensiveConstant(rl_src)) {
    LoadConstantWide(reg_lo, reg_hi, mir_graph_->ConstantValueWide(rl_src));
  } else {
    DCHECK((rl_src.location == kLocDalvikFrame) ||
           (rl_src.location == kLocCompilerTemp));
    LoadBaseDispWide(TargetReg(kSp).GetReg(), SRegOffset(rl_src.s_reg_low),
...
RegLocation Mir2Lir::LoadValueWide(RegLocation rl_src, RegisterClass op_kind) {
  ...
  rl_src = EvalLoc(rl_src, op_kind, false);
  if (IsInexpensiveConstant(rl_src) || rl_src.location != kLocPhysReg) {
    LoadValueDirectWide(rl_src, rl_src.reg.GetLowReg(), rl_src.reg.GetHighReg());
    rl_src.location = kLocPhysReg;
...
void Mir2Lir::StoreValueWide(RegLocation rl_dest, RegLocation rl_src) {
  ...
  if (rl_src.location == kLocPhysReg) {
    if (IsLive(rl_src.reg) ||
      ...
      rl_dest = EvalLoc(rl_dest, kAnyReg, false);
      OpRegCopyWide(rl_dest.reg.GetLowReg(), rl_dest.reg.GetHighReg(),
                    rl_src.reg.GetLowReg(), rl_src.reg.GetHighReg());
    } else {
      // Just re-assign the registers.  Dest gets Src's regs
      rl_dest.reg = rl_src.reg;
      Clobber(rl_src.reg);
    ...
    // Load Src either into promoted Dest or temps allocated for Dest
    rl_dest = EvalLoc(rl_dest, kAnyReg, false);
    LoadValueDirectWide(rl_src, rl_dest.reg.GetLowReg(), rl_dest.reg.GetHighReg());
  }

  // Dest is now live and dirty (until/if we flush it to home location)
  MarkLive(rl_dest.reg.GetLowReg(), rl_dest.s_reg_low);
  ...
  if (IsDirty(rl_dest.reg) && (oat_live_out(rl_dest.s_reg_low) ||
    ...
    StoreBaseDispWide(TargetReg(kSp).GetReg(), SRegOffset(rl_dest.s_reg_low),
                      rl_dest.reg.GetLowReg(), rl_dest.reg.GetHighReg());
    MarkClean(rl_dest);
    def_end = last_lir_insn_;
...
void Mir2Lir::StoreFinalValueWide(RegLocation rl_dest, RegLocation rl_src) {
  ...
  if (rl_dest.location == kLocPhysReg) {
    OpRegCopyWide(rl_dest.reg.GetLowReg(), rl_dest.reg.GetHighReg(), rl_src.reg.GetLowReg(), rl_src.reg.GetHighReg());
  } else {
    // Just re-assign the registers.  Dest gets Src's regs.
    rl_dest.location = kLocPhysReg;
    rl_dest.reg = rl_src.reg;
...
RegLocation Mir2Lir::ForceTempWide(RegLocation loc) {
  ...
  if (IsTemp(loc.reg.GetLowReg())) {
    ...
    RegStorage temp_low = AllocTemp();
    // TODO: need conversion utility here.
    OpRegCopy(temp_low, RegStorage(RegStorage::k32BitSolo, loc.reg.GetLowReg()));
  ...
  if (IsTemp(loc.reg.GetHighReg())) {
    ...
    // TODO: need conversion utility here.
    OpRegCopy(temp_high, RegStorage(RegStorage::k32BitSolo, loc.reg.GetHighReg()));
