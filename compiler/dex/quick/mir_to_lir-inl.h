...
inline void Mir2Lir::SetupResourceMasks(LIR* lir, bool leave_mem_ref) {
  ...
  if (!leave_mem_ref && (flags & (IS_LOAD | IS_STORE))) {
    ...
    SetMemRefType(lir, flags & IS_LOAD, ResourceMask::kHeapRef);
  }
  ...
  ResourceMask def_mask, use_mask;
  ...
  if (flags & SETS_CCODES) {
    def_mask.SetResourceBit(ResourceMask::kCCode);
  }

  if (flags & USES_CCODES) {
    use_mask.SetResourceBit(ResourceMask::kCCode);
  }
