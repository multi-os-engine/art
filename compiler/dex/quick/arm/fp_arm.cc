...
void ArmMir2Lir::GenConversion(Instruction::Code opcode, RegLocation rl_dest, RegLocation rl_src) {
  ...
  switch (opcode) {
    ...
    case Instruction::LONG_TO_DOUBLE: {
      ...
      RegStorage src_reg_as_single = RegStorage::FloatSolo32(rl_src.reg.GetRegNum() << 1);
      rl_result = EvalLoc(rl_dest, kFPReg, true);
      RegStorage tmp1 = AllocTempDouble();
      RegStorage tmp2 = AllocTempDouble();

      NewLIR2(kThumb2VcvtF64S32, tmp1.GetReg(), src_reg_as_single.GetReg() + 1);
      NewLIR2(kThumb2VcvtF64U32, rl_result.reg.GetReg(), src_reg_as_single.GetReg());
    ...
    case Instruction::LONG_TO_FLOAT: {
      ...
      // Long to double.
      NewLIR2(kThumb2VcvtF64S32, high_val.GetReg(), src_reg_as_single.GetReg() + 1);
      NewLIR2(kThumb2VcvtF64U32, low_val.GetReg(), src_reg_as_single.GetReg());
