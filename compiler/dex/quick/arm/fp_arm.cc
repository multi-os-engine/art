...
void ArmMir2Lir::GenConversion(Instruction::Code opcode,
  ...
  switch (opcode) {
    ...
    case Instruction::LONG_TO_DOUBLE: {
      ...
      int tmp1 = AllocTempDouble().GetLowReg();
      int tmp2 = AllocTempDouble().GetLowReg();

      NewLIR2(kThumb2VcvtF64S32, tmp1 | ARM_FP_DOUBLE, (src_reg & ~ARM_FP_DOUBLE) + 1);
      NewLIR2(kThumb2VcvtF64U32, S2d(rl_result.reg.GetLowReg(), rl_result.reg.GetHighReg()), (src_reg & ~ARM_FP_DOUBLE));
      LoadConstantWide(tmp2, tmp2 + 1, 0x41f0000000000000LL);
      NewLIR3(kThumb2VmlaF64, S2d(rl_result.reg.GetLowReg(), rl_result.reg.GetHighReg()), tmp1 | ARM_FP_DOUBLE,
              tmp2 | ARM_FP_DOUBLE);
      FreeTemp(tmp1);
      FreeTemp(tmp2);
      StoreValueWide(rl_dest, rl_result);
