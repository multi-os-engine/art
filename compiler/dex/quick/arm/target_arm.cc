...
RegStorage ArmMir2Lir::AllocTempDouble() {
  ...
  for (int i = 0; i < num_regs; i+=2) {
    ...
    if ((p[next].is_temp && !p[next].InUse() && !p[next].live) &&
      ...
      int dp_reg = (p[next].reg.GetRegNum() >> 1) | RegStorage::kFloatingPoint;
...
RegStorage ArmMir2Lir::AllocPreservedDouble(int s_reg) {
  ...
  if (promotion_map_[p_map_idx+1].fp_location == kLocPhysReg) {
    ...
    // FIXME: rework.
    RegisterInfo* p = GetRegInfo(RegStorage(RegStorage::k32BitSolo, RegStorage::kFloatingPoint | (high_reg-1)));
    if (p->InUse() || p->IsTemp()) {
      // Already allocated or not preserved - fail.
      return res;  // Invalid.
    }
    // OK - good to go.
    int dp_reg = (p->GetReg().GetRegNum() >> 1) | RegStorage::kFloatingPoint;
