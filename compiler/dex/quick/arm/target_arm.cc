...
#endif
    default: res_reg = CTargetReg(reg);
  }
  return res_reg;
}
...
#endif
  const int fpArgMappingToPhysicalRegSize = arraysize(fpArgMappingToPhysicalReg);
  ...
  if (is_double_or_float) {
    if (is_wide) {
      ...
      if (cur_fp_double_reg_ < fpArgMappingToPhysicalRegSize) {
        // TODO:
#if TODO_ENABLE_THIS_WHEN_FLUSHINS_SUPPORT_64BIT_REGISTER
...
RegStorage ArmMir2Lir::InToRegStorageMapping::Get(int in_position) {
...
#else
  // TODO: 1. Implement FlushIns for long, double argument registers.
  // TODO: 2. Consolidate all targets.
  Mir2Lir::FlushIns(ArgLocs, rl_method);
#endif
}
...
#else
  return GenDalvikArgsRange(info, call_state, pcrLabel, next_call_insn, target_method,
                            vtable_idx, direct_code, direct_method, type, skip_this);
#endif
}
