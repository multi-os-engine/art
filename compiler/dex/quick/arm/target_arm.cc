...
// TODO: move to RegStorage and elminate target variants.
bool ArmMir2Lir::SameRegType(int reg1, int reg2) {
  return (RegStorage::IsDouble(reg1) == RegStorage::IsDouble(reg2)) &&
         (RegStorage::IsSingle(reg1) == RegStorage::IsSingle(reg2));
}
...
uint64_t ArmMir2Lir::GetRegMaskCommon(int reg) {
  uint64_t seed;
  int shift;
  int reg_id = reg & RegStorage::kRegNumMask;
  /* Each double register is equal to a pair of single-precision FP registers */
  if (RegStorage::IsDouble(reg)) {
    seed = 0x3;
    reg_id = reg_id << 1;
  } else {
    seed = 1;
  ...
  shift = RegStorage::IsFloat(reg) ? kArmFPReg0 : 0;
...
RegStorage ArmMir2Lir::AllocTempDouble() {
  ...
  for (int i = 0; i < num_regs; i+=2) {
    ...
    if ((p[next].is_temp && !p[next].InUse() && !p[next].live) &&
      ...
      int dp_reg = ((p[next].reg & RegStorage::kRegNumMask) >> 1) | RegStorage::kFloatingPoint;
...
RegStorage ArmMir2Lir::AllocPreservedDouble(int s_reg) {
  ...
  if (promotion_map_[p_map_idx+1].fp_location == kLocPhysReg) {
    ...
    // OK - good to go.
    int dp_reg = ((p->reg & RegStorage::kRegNumMask) >> 1) | RegStorage::kFloatingPoint;
    ...
    for (int i = 0; i < reg_pool_->num_fp_regs; i++) {
      if (!FPRegs[i].is_temp && !FPRegs[i].InUse() &&
        ...
        int dp_reg = ((FPRegs[i].reg & RegStorage::kRegNumMask) >> 1) | RegStorage::kFloatingPoint;
        res = RegStorage(RegStorage::k64BitSolo, dp_reg);
        FPRegs[i].MarkInUse();
  ...
  if (res.Valid()) {
    int low_single = ((res.GetReg() & RegStorage::kRegNumMask) << 1) | RegStorage::kFloatingPoint;
