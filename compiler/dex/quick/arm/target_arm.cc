...
constexpr ResourceMask ArmMir2Lir::GetRegMaskArm(RegStorage reg) {
  return reg.IsDouble()
      ...
      ? ResourceMask::RawMask(UINT64_C(0x3) << (reg.GetRegNum() * 2 + kArmFPReg0), 0u)
...
void ArmMir2Lir::SetupTargetResourceMasks(LIR* lir, uint64_t flags,
  ...
  if ((flags & (REG_DEF_SP | REG_USE_SP | REG_DEF_LIST0 | REG_DEF_LIST1 |
    ...
    if (flags & REG_DEF_SP) {
      def_mask->SetTargetSpecificBit(kArmRegSP);
    }

    if (flags & REG_USE_SP) {
      use_mask->SetTargetSpecificBit(kArmRegSP);
    }
    ...
    if (flags & REG_USE_PC) {
      use_mask->SetTargetSpecificBit(kArmRegPC);
    }
    ...
    if (opcode == kThumbPush || opcode == kThumbPop) {
      ...
      if ((opcode == kThumbPush) && (use_mask->Intersects(r8Mask))) {
        ...
        use_mask->SetTargetSpecificBit(kArmRegLR);
      } else if ((opcode == kThumbPop) && (def_mask->Intersects(r8Mask))) {
        def_mask->ClearBits(r8Mask);
        def_mask->SetTargetSpecificBit(kArmRegPC);;
      }
    }
    if (flags & REG_DEF_LR) {
      def_mask->SetTargetSpecificBit(kArmRegLR);
    }
  }
}
