...
static RegStorage core_regs[] = {rs_r0, rs_r1, rs_r2, rs_r3, rs_rARM_SUSPEND, rs_r5, rs_r6, rs_r7,
                                 rs_r8, rs_rARM_SELF, rs_r10, rs_r11, rs_r12, rs_rARM_SP,
                                 rs_rARM_LR, rs_rARM_PC};
static RegStorage ReservedRegs[] = {rs_rARM_SUSPEND, rs_rARM_SELF, rs_rARM_SP, rs_rARM_LR,
                                    rs_rARM_PC};
static RegStorage FpRegs[] = {rs_fr0, rs_fr1, rs_fr2, rs_fr3, rs_fr4, rs_fr5, rs_fr6, rs_fr7,
                              rs_fr8, rs_fr9, rs_fr10, rs_fr11, rs_fr12, rs_fr13, rs_fr14, rs_fr15,
                              rs_fr16, rs_fr17, rs_fr18, rs_fr19, rs_fr20, rs_fr21, rs_fr22,
                              rs_fr23, rs_fr24, rs_fr25, rs_fr26, rs_fr27, rs_fr28, rs_fr29,
                              rs_fr30, rs_fr31, rs_dr0, rs_dr1, rs_dr2, rs_dr3, rs_dr4, rs_dr5,
                              rs_dr6, rs_dr7, rs_dr8, rs_dr9, rs_dr10, rs_dr11, rs_dr12, rs_dr13,
                              rs_dr14, rs_dr15};
static RegStorage FpDoubles[] = {rs_dr0, rs_dr1, rs_dr2, rs_dr3, rs_dr4, rs_dr5, rs_dr6, rs_dr7,
                                 rs_dr8, rs_dr9, rs_dr10, rs_dr11, rs_dr12, rs_dr13, rs_dr14,
                                 rs_dr15};
static RegStorage core_temps[] = {rs_r0, rs_r1, rs_r2, rs_r3, rs_r12};
static RegStorage fp_temps[] = {rs_fr0, rs_fr1, rs_fr2, rs_fr3, rs_fr4, rs_fr5, rs_fr6, rs_fr7,
                                rs_fr8, rs_fr9, rs_fr10, rs_fr11, rs_fr12, rs_fr13, rs_fr14,
                                rs_fr15};
...
RegStorage ArmMir2Lir::TargetReg(SpecialTargetRegister reg) {
  ...
  switch (reg) {
    ...
    case kArg0: res_reg = rs_rARM_ARG0; break;
    case kArg1: res_reg = rs_rARM_ARG1; break;
    case kArg2: res_reg = rs_rARM_ARG2; break;
    case kArg3: res_reg = rs_rARM_ARG3; break;
    case kFArg0: res_reg = rs_rARM_FARG0; break;
    case kFArg1: res_reg = rs_rARM_FARG1; break;
    case kFArg2: res_reg = rs_rARM_FARG2; break;
    case kFArg3: res_reg = rs_rARM_FARG3; break;
    case kRet0: res_reg = rs_rARM_RET0; break;
    case kRet1: res_reg = rs_rARM_RET1; break;
    case kInvokeTgt: res_reg = rs_rARM_INVOKE_TGT; break;
    case kHiddenArg: res_reg = rs_r12; break;
    case kHiddenFpArg: res_reg = RegStorage::InvalidReg(); break;
    case kCount: res_reg = rs_rARM_COUNT; break;
  }
  return res_reg;
}
...
uint64_t ArmMir2Lir::GetRegMaskCommon(int reg) {
  ...
  int shift = ARM_FPREG(reg) ? kArmFPReg0 : 0;
  if (ARM_DOUBLEREG(reg)) {
    // TODO: replace this with use of alias in RegisterInfo.
    // Convert to pair of singles.
    seed = 0x3;   // Pair of bit positions.
    reg_id = reg_id << 1;
  } else {
    seed = 0x1;  // Single bit position.
  }
  shift += reg_id;
  return (seed << shift);
}
...
void ArmMir2Lir::CompilerInitializeRegAlloc() {
  ...
  for (int i = 0; i < num_reserved; i++) {
    if (no_suspend && (ReservedRegs[i] == rs_rARM_SUSPEND)) {
...
RegStorage ArmMir2Lir::AllocTempDouble() {
  ...
  for (int i = 0; i < num_regs; i+=2) {
    if (next >= num_regs)
      next = 0;
    if ((p[next].is_temp && !p[next].InUse() && !p[next].live) &&
      (p[next+1].is_temp && !p[next+1].InUse() && !p[next+1].live)) {
      Clobber(p[next].reg);
  ...
  for (int i = 0; i < num_regs; i+=2) {
    if (next >= num_regs)
      next = 0;
    if (p[next].is_temp && !p[next].InUse() && p[next+1].is_temp && !p[next+1].InUse()) {
      Clobber(p[next].reg);
      Clobber(p[next+1].reg);
      p[next].MarkInUse();
      p[next+1].MarkInUse();
...
RegStorage ArmMir2Lir::AllocPreservedDouble(int s_reg) {
  ...
  if (promotion_map_[p_map_idx+1].fp_location == kLocPhysReg) {
    ...
    for (int i = 0; i < reg_pool_->num_fp_regs; i++) {
      if (!FPRegs[i].is_temp && !FPRegs[i].InUse() &&
        ((FPRegs[i].reg & 0x1) == 0x0) &&
        !FPRegs[i+1].is_temp && !FPRegs[i+1].InUse() &&
