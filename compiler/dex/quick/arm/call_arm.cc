...
static int ArmNextSDCallInsn(CompilationUnit* cu, CallInfo* info,
  ...
  if (direct_code != 0 && direct_method != 0) {
    ...
    case 0: {  // Grab target method* from thread pointer
      ...
      uint32_t string_init_base_offset = Thread::QuickEntryPointOffsetWithSize(OFFSETOF_MEMBER(QuickEntryPoints, pNewEmptyString), kArmPointerSize);
      int string_init_offset = string_init_base_offset + offset * kArmPointerSize;
      cg->LoadRefDisp(rs_rARM_SELF, string_init_offset, arg0_ref, kNotVolatile);
      break;
    }
