...
void ArmMir2Lir::GenEntrySequence(RegLocation* ArgLocs, RegLocation rl_method) {
  ...
  if (core_spill_mask_ != 0u) {
    ...
    cfi_.RelOffsets(DwarfCoreReg(0), 0, core_spill_mask_, kArmPointerSize);
  }
  ...
  if (num_fp_spills_ != 0u) {
    ...
    cfi_.RelOffsets(DwarfFpReg(0), 0, fp_spill_mask_, kArmPointerSize);
  }
...
void ArmMir2Lir::GenExitSequence() {
  ...
  if (num_fp_spills_) {
    ...
    cfi_.AdjustCFAOffset(-num_fp_spills_ * kArmPointerSize);
    cfi_.Restores(DwarfFpReg(0), fp_spill_mask_);
  }
  ...
  if (core_spill_mask_ != 0u) {
    ...
    if (!unspill_LR_to_PC) {
      cfi_.AdjustCFAOffset(-num_core_spills_ * kArmPointerSize);
      cfi_.Restores(DwarfCoreReg(0), core_spill_mask_);
      DCHECK_EQ(cfi_.current_cfa_offset(), 0);  // empty stack.
    }
  }
  if (!unspill_LR_to_PC) {
...
void ArmMir2Lir::GenSpecialEntryForSuspend() {
  ...
  cfi_.RelOffsets(DwarfCoreReg(0), 4, core_spill_mask_, kArmPointerSize);
}
