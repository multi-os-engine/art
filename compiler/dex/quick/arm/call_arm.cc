...
void ArmMir2Lir::GenEntrySequence(RegLocation* ArgLocs, RegLocation rl_method) {
  ...
  if (core_spill_mask_ != 0u) {
    ...
    cfi_.RelOffsets(DwarfCoreReg(0), 0, core_spill_mask_, kRegSize);
  }
  ...
  if (num_fp_spills_ > 0) {
    ...
    cfi_.RelOffsets(DwarfFpReg(0), 0, fp_spill_mask_, kRegSize);
  }
...
void ArmMir2Lir::GenSpecialEntryForSuspend() {
  ...
  cfi_.RelOffsets(DwarfCoreReg(0), 8, core_spill_mask_, kArmPointerSize);
}
