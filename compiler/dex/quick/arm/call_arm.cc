...
void ArmMir2Lir::GenMonitorEnter(int opt_flags, RegLocation rl_src) {
  FlushAllRegs();
  LoadValueDirectFixed(rl_src, RegStorage(RegStorage::k32BitSolo, r0));  // Get obj
  ...
  if (kArchVariantHasGoodBranchPredictor) {
    ...
    if ((opt_flags & MIR_IGNORE_NULL_CHECK) && !(cu_->disable_opt & (1 << kNullCheckElimination))) {
      ...
      null_check_branch = OpCmpImmBranch(kCondEq, RegStorage(RegStorage::k32BitSolo, r0), 0, NULL);
    }
    ...
    LIR* not_unlocked_branch = OpCmpImmBranch(kCondNe, RegStorage(RegStorage::k32BitSolo, r1),
                                              0, NULL);
    NewLIR4(kThumb2Strex, r1, r2, r0, mirror::Object::MonitorOffset().Int32Value() >> 2);
    LIR* lock_success_branch = OpCmpImmBranch(kCondEq, RegStorage(RegStorage::k32BitSolo, r1),
                                              0, NULL);
    ...
    GenNullCheck(rl_src.s_reg_low, RegStorage(RegStorage::k32BitSolo, r0), opt_flags);
    LoadWordDisp(rARM_SELF, Thread::ThinLockIdOffset().Int32Value(), r2);
    NewLIR3(kThumb2Ldrex, r1, r0, mirror::Object::MonitorOffset().Int32Value() >> 2);
...
void ArmMir2Lir::GenMonitorExit(int opt_flags, RegLocation rl_src) {
  ...
  if (kArchVariantHasGoodBranchPredictor) {
    ...
    LIR* slow_unlock_branch = OpCmpBranch(kCondNe, RegStorage(RegStorage::k32BitSolo, r1),
                                          RegStorage(RegStorage::k32BitSolo, r2), NULL);
...
void ArmMir2Lir::GenEntrySequence(RegLocation* ArgLocs, RegLocation rl_method) {
  ...
  if (!skip_overflow_check) {
    OpRegRegImm(kOpSub, rs_rARM_LR, rs_rARM_SP, frame_size_ - (spill_count * 4));
    GenRegRegCheck(kCondUlt, RegStorage(RegStorage::k32BitSolo, rARM_LR),
                   RegStorage(RegStorage::k32BitSolo, r12), kThrowStackOverflow);
    OpRegCopy(RegStorage(RegStorage::k32BitSolo, rARM_SP), RegStorage(RegStorage::k32BitSolo,
                         rARM_LR));     // Establish stack
  } else {
    OpRegImm(kOpSub, rs_rARM_SP, frame_size_ - (spill_count * 4));
  }
