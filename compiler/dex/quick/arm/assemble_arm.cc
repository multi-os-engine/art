...
extern "C" pthread_mutex_t vmarko_stat_mutex;
...
pthread_mutex_t vmarko_stat_mutex = PTHREAD_MUTEX_INITIALIZER;
uint64_t vmarko_sum_time = 0u;
uint32_t vmarko_count = 0u;
uint32_t vmarko_count_simple = 0u;
uint32_t vmarko_hist_unsigned[33] = {
...
void ArmMir2Lir::AssembleLIR() {
  ...
  pthread_mutex_lock(&vmarko_stat_mutex);
  struct {
    void Unsigned(uint32_t value) {
      ++vmarko_hist_unsigned[value ? 32 - CLZ(value) : 0];
    }
  } stat;
  uint64_t diff_time = end_time - start_time;
  stat.Unsigned(static_cast<uint32_t>(diff_time));
  vmarko_count += 1;
  vmarko_sum_time += diff_time;
  if (assembler_retries == 0) {
    vmarko_count_simple += 1u;
  }
  pthread_mutex_unlock(&vmarko_stat_mutex);
}
