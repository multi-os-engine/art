...
void ArmMir2Lir::OpRegCopyWide(RegStorage r_dest, RegStorage r_src) {
  ...
  if (dest_fp) {
    if (src_fp) {
      ...
      OpRegCopy(RegStorage(RegStorage::k64BitSolo, S2d(r_dest.GetLowReg(), r_dest.GetHighReg())),
                RegStorage(RegStorage::k64BitSolo, S2d(r_src.GetLowReg(), r_src.GetHighReg())));
    } else {
