...
LIR* ArmMir2Lir::OpCmpImmBranch(ConditionCode cond, RegStorage reg, int check_value, LIR* target) {
  ...
  if (!skip && (ARM_LOWREG(reg.GetRegNum())) && (check_value == 0) &&
...
LIR* ArmMir2Lir::OpRegCopyNoInsert(RegStorage r_dest, RegStorage r_src) {
  ...
  if (ARM_LOWREG(r_dest.GetRegNum()) && ARM_LOWREG(r_src.GetRegNum()))
    opcode = kThumbMovRR;
  else if (!ARM_LOWREG(r_dest.GetRegNum()) && !ARM_LOWREG(r_src.GetRegNum()))
     opcode = kThumbMovRR_H2H;
  else if (ARM_LOWREG(r_dest.GetRegNum()))
...
bool ArmMir2Lir::GenInlinedCas(CallInfo* info, bool is_long, bool is_object) {
  ...
  if (is_long) {
    ...
    if (ARM_LOWREG(r_tmp.GetRegNum()) && ARM_LOWREG(r_tmp_high.GetRegNum())) {
