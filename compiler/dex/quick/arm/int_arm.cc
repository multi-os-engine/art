...
void ArmMir2Lir::GenFusedLongCmpImmBranch(BasicBlock* bb, RegLocation rl_src1,
  ...
  rl_src1 = LoadValueWide(rl_src1, kCoreReg);
  // TODO: support 64-bit regs; also utilty to split pair into 2 solos.
  RegStorage low_reg(RegStorage::k32BitSolo, rl_src1.reg.GetLowReg());
  RegStorage high_reg(RegStorage::k32BitSolo, rl_src1.reg.GetHighReg());
...
RegLocation ArmMir2Lir::GenDivRemLit(RegLocation rl_dest, int reg1, int lit,
  ...
  rl_result = GenDivRem(rl_result, reg1, lit_temp.GetReg(), is_div);
  FreeTemp(lit_temp);
...
RegLocation ArmMir2Lir::GenDivRem(RegLocation rl_dest, int reg1, int reg2,
  ...
  if (is_div) {
    ...
    OpRegRegReg(kOpDiv, rl_result.reg, RegStorage(RegStorage::k32BitSolo, reg1),
                RegStorage(RegStorage::k32BitSolo, reg2));
  } else {
    ...
    OpRegRegReg(kOpDiv, temp, RegStorage(RegStorage::k32BitSolo, reg1),
                RegStorage(RegStorage::k32BitSolo, reg2));
    OpRegReg(kOpMul, temp, RegStorage(RegStorage::k32BitSolo, reg2));
    OpRegRegReg(kOpSub, rl_result.reg, RegStorage(RegStorage::k32BitSolo, reg1), temp);
    FreeTemp(temp);
  }
...
bool ArmMir2Lir::GenInlinedPeek(CallInfo* info, OpSize size) {
  ...
  if (size == kLong) {
    ...
    if (rl_address.reg.GetReg() != rl_result.reg.GetLowReg()) {
      LoadBaseDisp(rl_address.reg.GetReg(), 0, rl_result.reg.GetLowReg(), kWord, INVALID_SREG);
...
bool ArmMir2Lir::GenInlinedPoke(CallInfo* info, OpSize size) {
  ...
  if (size == kLong) {
    ...
    StoreBaseDisp(rl_address.reg.GetReg(), 0, rl_value.reg.GetLowReg(), kWord);
...
bool ArmMir2Lir::GenInlinedCas(CallInfo* info, bool is_long, bool is_object) {
  ...
  // TODO: create a regloc narrowing utility that takes care of both of these things.
  ...
  if (is_long) {
    ...
    if (!expected_is_good_reg && !new_value_is_good_reg) {
      ...
      if (expected_is_core_reg) {
        FlushRegWide(rl_src_expected.reg.GetLowReg(), rl_src_expected.reg.GetHighReg());
      ...
      if (new_value_is_core_reg) {
        FlushRegWide(rl_src_new_value.reg.GetLowReg(), rl_src_new_value.reg.GetHighReg());
  ...
  if (is_long) {
    ...
    if (!load_early) {
      LoadValueDirectWide(rl_src_expected, rl_expected.reg.GetLowReg(), rl_expected.reg.GetHighReg());
    }
    ...
    if (!load_early) {
      LoadValueDirectWide(rl_src_new_value, rl_new_value.reg.GetLowReg(), rl_new_value.reg.GetHighReg());
    }
  ...
  if (!load_early) {
    FreeTemp(rl_expected.reg.GetLowReg());  // Now unneeded.
...
void ArmMir2Lir::GenMulLong(Instruction::Code opcode, RegLocation rl_dest,
    ...
    if ((rl_src1.s_reg_low == rl_src2.s_reg_low) ||
        (!IsTemp(rl_src1.reg.GetLowReg()) && !IsTemp(rl_src1.reg.GetHighReg())) ||
        (!IsTemp(rl_src2.reg.GetLowReg()) && !IsTemp(rl_src2.reg.GetHighReg()))) {
      special_case = false;
    }
    // Tuning: if rl_dest has been promoted and is *not* either operand, could use directly.
    int res_lo = AllocTemp().GetReg();
    int res_hi;
    if (rl_src1.reg.GetLowReg() == rl_src2.reg.GetLowReg()) {
      res_hi = AllocTemp().GetReg();
      NewLIR3(kThumb2MulRRR, tmp1, rl_src1.reg.GetLowReg(), rl_src1.reg.GetHighReg());
      NewLIR4(kThumb2Umull, res_lo, res_hi, rl_src1.reg.GetLowReg(), rl_src1.reg.GetLowReg());
      OpRegRegRegShift(kOpAdd, res_hi, res_hi, tmp1, EncodeShift(kArmLsl, 1));
    } else {
      // In the special case, all temps are now allocated
      NewLIR3(kThumb2MulRRR, tmp1, rl_src2.reg.GetLowReg(), rl_src1.reg.GetHighReg());
      if (special_case) {
      ...
      NewLIR4(kThumb2Umull, res_lo, res_hi, rl_src2.reg.GetLowReg(), rl_src1.reg.GetLowReg());
      NewLIR4(kThumb2Mla, tmp1, rl_src1.reg.GetLowReg(), rl_src2.reg.GetHighReg(), tmp1);
      NewLIR4(kThumb2AddRRR, res_hi, tmp1, res_hi, 0);
      if (special_case) {
        FreeTemp(rl_src1.reg.GetLowReg());
        Clobber(rl_src1.reg.GetLowReg());
...
void ArmMir2Lir::GenArrayGet(int opt_flags, OpSize size, RegLocation rl_array,
  ...
  RegStorage reg_len(RegStorage::kInvalid);
  ...
  if (rl_dest.wide || rl_dest.fp || constant_index) {
    ...
    if (rl_dest.wide) {
      LoadBaseDispWide(reg_ptr, data_offset, rl_result.reg.GetLowReg(), rl_result.reg.GetHighReg(), INVALID_SREG);
      if (!constant_index) {
        FreeTemp(reg_ptr);
      }
      StoreValueWide(rl_dest, rl_result);
    ...
    LoadBaseIndexed(reg_ptr.GetReg(), rl_index.reg.GetReg(), rl_result.reg.GetReg(), scale, size);
    FreeTemp(reg_ptr);
    StoreValue(rl_dest, rl_result);
  }
}
...
void ArmMir2Lir::GenArrayPut(int opt_flags, OpSize size, RegLocation rl_array,
  ...
  if (rl_src.wide || rl_src.fp || constant_index) {
    ...
    if (rl_src.wide) {
      StoreBaseDispWide(reg_ptr.GetReg(), data_offset, rl_src.reg.GetLowReg(), rl_src.reg.GetHighReg());
    } else {
    ...
    StoreBaseIndexed(reg_ptr.GetReg(), rl_index.reg.GetReg(), rl_src.reg.GetReg(), scale, size);
  }
  ...
  switch (opcode) {
    ...
    case Instruction::SHL_LONG_2ADDR:
      if (shift_amount == 1) {
        ...
        // FIXME: this is awful.  Need utilties to break up pairs into solos.
        OpRegCopy(RegStorage(RegStorage::k32BitSolo, rl_result.reg.GetHighReg()), rl_src.reg);
