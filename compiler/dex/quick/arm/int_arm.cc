...
bool ArmMir2Lir::GenInlinedCas(CallInfo* info, bool is_long, bool is_object) {
  ...
  DCHECK(!reg_pool_->core_regs_.Get(rARM_LR)->IsTemp());
  MarkTemp(rs_rARM_LR);
  FreeTemp(rs_rARM_LR);
  LockTemp(rs_rARM_LR);
  bool load_early = true;
  if (is_long) {
    RegStorage expected_reg = is_long ? rl_src_expected.reg.GetLow() : rl_src_expected.reg;
    RegStorage new_val_reg = is_long ? rl_src_new_value.reg.GetLow() : rl_src_new_value.reg;
