...
LIR* ArmMir2Lir::LoadConstantWide(int r_dest_lo, int r_dest_hi, int64_t value) {
  ...
  if (ARM_FPREG(r_dest_lo)) {
    ...
    if ((InexpensiveConstantInt(val_lo) && (InexpensiveConstantInt(val_hi)))) {
      res = LoadConstantNoClobber(RegStorage(RegStorage::k32BitSolo, r_dest_lo), val_lo);
      LoadConstantNoClobber(RegStorage(RegStorage::k32BitSolo, r_dest_hi), val_hi);
    }
  }
  if (res == NULL) {
...
LIR* ArmMir2Lir::LoadBaseIndexed(int r_base, int r_index, int r_dest,
                                 int scale, OpSize size) {
  bool all_low_regs = ARM_LOWREG(r_base) && ARM_LOWREG(r_index) && ARM_LOWREG(r_dest);
  LIR* load;
  ArmOpcode opcode = kThumbBkpt;
  ...
  switch (size) {
    ...
    case kSingle:
      ...
      if (scale) {
        NewLIR4(kThumb2AddRRR, reg_ptr, r_base, r_index,
                EncodeShift(kArmLsl, scale));
      } else {
        OpRegRegReg(kOpAdd, RegStorage(RegStorage::k32BitSolo, reg_ptr),
                    RegStorage(RegStorage::k32BitSolo, r_base),
                    RegStorage(RegStorage::k32BitSolo, r_index));
      }
  ...
  if (thumb_form)
    load = NewLIR3(opcode, r_dest, r_base, r_index);
  else
    load = NewLIR4(opcode, r_dest, r_base, r_index, scale);

  return load;
}

LIR* ArmMir2Lir::StoreBaseIndexed(int r_base, int r_index, int r_src,
                                  int scale, OpSize size) {
  bool all_low_regs = ARM_LOWREG(r_base) && ARM_LOWREG(r_index) && ARM_LOWREG(r_src);
  LIR* store = NULL;
  ArmOpcode opcode = kThumbBkpt;
  bool thumb_form = (all_low_regs && (scale == 0));
  int reg_ptr;
  ...
  if (thumb_form)
    store = NewLIR3(opcode, r_src, r_base, r_index);
  else
    store = NewLIR4(opcode, r_src, r_base, r_index, scale);
...
LIR* ArmMir2Lir::LoadBaseDispBody(RegStorage r_base, int displacement, RegStorage r_dest,
  ...
  bool all_low_regs = (ARM_LOWREG(r_base.GetReg()) && ARM_LOWREG(r_dest.GetReg()));
    ...
    case kLong:
      ...
      if (ARM_FPREG(r_dest.GetReg())) {
        if (ARM_SINGLEREG(r_dest.GetReg())) {
          DCHECK(ARM_FPREG(r_dest_hi.GetReg()));
        // FIXME - cleanup.
          r_dest = RegStorage(RegStorage::k64BitSolo, S2d(r_dest.GetReg(), r_dest_hi.GetReg()));
        }
        opcode = kThumb2Vldrd;
        ...
        if (displacement <= 1020) {
          load = NewLIR4(kThumb2LdrdI8, r_dest.GetReg(), r_dest_hi.GetReg(), r_base.GetReg(),
                         displacement >> 2);
        } else {
          load = LoadBaseDispBody(r_base, displacement, r_dest,
                                 RegStorage(RegStorage::kInvalid), kWord, s_reg);
          LoadBaseDispBody(r_base, displacement + 4, r_dest_hi,
                           RegStorage(RegStorage::kInvalid), kWord, INVALID_SREG);
        }
...
LIR* ArmMir2Lir::LoadBaseDisp(RegStorage r_base, int displacement, RegStorage r_dest,
  ...
  return LoadBaseDispBody(r_base, displacement, r_dest, RegStorage(RegStorage::kInvalid), size,
                          s_reg);
}

// FIXME: this should probably just go away - or perhaps DCHECK for 64 bits and continue.
LIR* ArmMir2Lir::LoadBaseDispWide(int r_base, int displacement, int r_dest_lo,
                                  int r_dest_hi, int s_reg) {
  return LoadBaseDispBody(RegStorage(RegStorage::k32BitSolo, r_base), displacement, RegStorage(RegStorage::k32BitSolo, r_dest_lo),
                          RegStorage(RegStorage::k32BitSolo, r_dest_hi), kLong, s_reg);
}


LIR* ArmMir2Lir::StoreBaseDispBody(RegStorage r_base, int displacement,
                                   RegStorage r_src, RegStorage r_src_hi, OpSize size) {
  LIR* store = NULL;
  ArmOpcode opcode = kThumbBkpt;
  bool short_form = false;
  bool thumb2Form = (displacement < 4092 && displacement >= 0);
  bool all_low_regs = (ARM_LOWREG(r_base.GetReg()) && ARM_LOWREG(r_src.GetReg()));
  int encoded_disp = displacement;
  bool is64bit = false;
  bool already_generated = false;
  switch (size) {
    ...
    case kDouble:
      ...
      if (!ARM_FPREG(r_src.GetReg())) {
        if (displacement <= 1020) {
          store = NewLIR4(kThumb2StrdI8, r_src.GetReg(), r_src_hi.GetReg(), r_base.GetReg(),
                          displacement >> 2);
        } else {
          store = StoreBaseDispBody(r_base, displacement, r_src, RegStorage(RegStorage::kInvalid),
                                    kWord);
          StoreBaseDispBody(r_base, displacement + 4, r_src_hi, RegStorage(RegStorage::kInvalid),
                            kWord);
        }
        already_generated = true;
      } else {
        if (ARM_SINGLEREG(r_src.GetReg())) {
          DCHECK(ARM_FPREG(r_src_hi.GetReg()));
          r_src = RegStorage(RegStorage::k64BitSolo, S2d(r_src.GetReg(), r_src_hi.GetReg()));
        }
        opcode = kThumb2Vstrd;
        if (displacement <= 1020) {
          short_form = true;
  ...
  if (!already_generated) {
    if (short_form) {
      ...
      store = StoreBaseIndexed(r_base.GetReg(), r_scratch.GetReg(), r_src.GetReg(), 0, size);
      FreeTemp(r_scratch);
    }
  }
...
LIR* ArmMir2Lir::StoreBaseDisp(RegStorage r_base, int displacement, RegStorage r_src,
                               OpSize size) {
  return StoreBaseDispBody(r_base, displacement, r_src, RegStorage(RegStorage::kInvalid), size);
}

LIR* ArmMir2Lir::StoreBaseDispWide(int r_base, int displacement,
                                   int r_src_lo, int r_src_hi) {
  return StoreBaseDispBody(RegStorage(RegStorage::k32BitSolo, r_base), displacement,
                           RegStorage(RegStorage::k32BitSolo, r_src_lo),
                           RegStorage(RegStorage::k32BitSolo, r_src_hi), kLong);
}
...
LIR* ArmMir2Lir::StoreBaseIndexedDisp(int r_base, int r_index, int scale,
...
LIR* ArmMir2Lir::LoadBaseIndexedDisp(int r_base, int r_index, int scale,
