...
LIR* ArmMir2Lir::LoadConstantWide(int r_dest_lo, int r_dest_hi, int64_t value) {
  ...
  if (ARM_FPREG(r_dest_lo)) {
    ...
    if ((InexpensiveConstantInt(val_lo) && (InexpensiveConstantInt(val_hi)))) {
      res = LoadConstantNoClobber(RegStorage::Solo32(r_dest_lo), val_lo);
      LoadConstantNoClobber(RegStorage::Solo32(r_dest_hi), val_hi);
    }
  }
  if (res == NULL) {
    ...
    case kLong:
      ...
      if (ARM_FPREG(r_dest.GetReg())) {
        ...
        if (displacement <= 1020) {
          ...
          load = LoadBaseDispBody(r_base, displacement, r_dest, RegStorage::InvalidReg(), kWord,
                                  s_reg);
          LoadBaseDispBody(r_base, displacement + 4, r_dest_hi, RegStorage::InvalidReg(), kWord,
                           INVALID_SREG);
        }
...
LIR* ArmMir2Lir::LoadBaseDisp(RegStorage r_base, int displacement, RegStorage r_dest,
  ...
  return LoadBaseDispBody(r_base, displacement, r_dest, RegStorage::InvalidReg(), size, s_reg);
}
    ...
    case kDouble:
      ...
      if (!ARM_FPREG(r_src.GetReg())) {
        if (displacement <= 1020) {
          ...
          store = StoreBaseDispBody(r_base, displacement, r_src, RegStorage::InvalidReg(), kWord);
          StoreBaseDispBody(r_base, displacement + 4, r_src_hi, RegStorage::InvalidReg(), kWord);
        }
        already_generated = true;
      } else {
