    ...
    case kLong:
      if (ARM_FPREG(dest_low_reg)) {
        ...
        if (r_dest.IsPair()) {
          ...
          r_dest = RegStorage(RegStorage::k64BitSolo, S2d(r_dest.GetLowReg(), r_dest.GetHighReg()));
        }
        opcode = kThumb2Vldrd;
    ...
    case kDouble:
      if (!ARM_FPREG(src_low_reg)) {
        ...
        if (r_src.IsPair()) {
          ...
          r_src = RegStorage(RegStorage::k64BitSolo, S2d(r_src.GetLowReg(), r_src.GetHighReg()));
        }
        opcode = kThumb2Vstrd;
        if (displacement <= 1020) {
          short_form = true;
