...
class GlobalValueNumbering {
 ...
 private:
  ...
  // Look up a value in the global value map.
  uint16_t LookupValue(uint16_t op, uint16_t operand1, uint16_t operand2, uint16_t modifier) {
    ...
    ValueMap::iterator it = global_value_map_.find(key);
    if (it != global_value_map_.end()) {
      res = it->second;
    } else {
      res = NewValueName();
      global_value_map_.Put(key, res);
    }
    return res;
  };
  ...
  bool HasValue(uint16_t op, uint16_t operand1, uint16_t operand2, uint16_t modifier,
    ...
    // except that it doesn't add an enrtry to the global value map if it's not there.
    uint64_t key = BuildKey(op, operand1, operand2, modifier);
