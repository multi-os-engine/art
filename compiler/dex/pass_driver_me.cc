...
namespace art {
  bool PassDriverME::RunPass(const Pass* pass, bool time_split) {
    ...
    const std::string& print_pass_list = pass_manager_->GetOptions().GetPrintPassList();
  ...
  void PassDriverME::FillOverriddenPassSettings(
      const char* pass_name, SafeMap<const std::string, int>& settings_to_fill) {
    const std::string& settings = pass_manager_->GetOptions().GetOverriddenPassOptions();
