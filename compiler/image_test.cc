...
void ImageTest::TestWriteRead(ImageHeader::StorageMode storage_mode) {
  // Test does not work with optimizing.
  ...
  if (storage_mode == ImageHeader::kStorageModeStore) {
...
TEST_F(ImageTest, WriteReadStore) {
  TestWriteRead(ImageHeader::kStorageModeStore);
}
