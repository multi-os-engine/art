...
inline static constexpr Elf32_Word NextOffset(const Elf32_Shdr& cur, const Elf32_Shdr& prev) {
  return RoundUp(prev.sh_size + prev.sh_offset, cur.sh_addralign);
}

static inline uint8_t MakeStInfo(uint8_t binding, uint8_t type) {
...
bool ElfWriterQuick::ElfBuilder::Write() {
  ...
  std::vector<uint8_t> hash = dynsym_builder_.GenerateHashContents();

  if (debug_) {
    LOG(INFO) << ".hash size (bytes)=" << hash.size() << std::hex << " " << hash.size();
  }
  ...
  hash_builder_.section_.sh_size = hash.size();
  ...
  // This will add the 2 values we cannot know until now time
...
bool ElfWriterQuick::ElfBuilder::WriteOutFile(std::vector<ElfFilePiece>& pieces) {
...
std::vector<Elf32_Dyn> ElfWriterQuick::ElfDynamicBuilder::GetDynamics(Elf32_Sword strsz,
  ...
  for (auto it = dynamics_.begin(); it != dynamics_.end(); ++it) {
    if (it->section_) {
      // We are adding an address relative to a section.
      ret.push_back({it->tag_, {it->off_ + static_cast<Elf32_Sword>(it->section_->section_.sh_addr)}});
    } else {
...
inline void ElfWriterQuick::ElfBuilder::AssignSectionStr(
    ElfSectionBuilder* builder, std::string& strtab) {
  builder->section_.sh_name = strtab.size();
  strtab += builder->name_;
  strtab += '\0';
  if (debug_) {
    LOG(INFO) << "adding section name \"" << &strtab[builder->section_.sh_name] << "\" "
...
std::vector<uint8_t> ElfWriterQuick::ElfSymtabBuilder::GenerateHashContents() {
  ...
  PushWord(&hash, nbuckets);     // Set buckets.
  PushWord(&hash, chain_size);   // Set nchain (i.e. length of the symtab including null).
  uint32_t bucket_offset = hash.size();
  uint32_t chain_offset = bucket_offset + nbuckets*sizeof(Elf32_Word);
  hash.resize(hash.size() + nbuckets*sizeof(Elf32_Word) + chain_size*sizeof(Elf32_Word), 0);

  Elf32_Word* buckets = reinterpret_cast<Elf32_Word*>(hash.data() + bucket_offset);
  Elf32_Word* chain   = reinterpret_cast<Elf32_Word*>(hash.data() + chain_offset);
