...
class DebugInfoTask : public Task {
 public:
  ...
  void Run(Thread*) {
    result_ = dwarf::MakeMiniDebugInfo(isa_, method_infos_);
  }
...
class ElfWriterQuick FINAL : public ElfWriter {
 public:
  ...
  void Start() OVERRIDE;
  void PrepareDebugInfo(const ArrayRef<const dwarf::MethodDebugInfo>& method_infos) OVERRIDE;
...
void ElfWriterQuick<ElfTypes>::PrepareDebugInfo(
  ...
  if (compiler_options_->GetGenerateMiniDebugInfo()) {
    ...
    debug_info_task_ = std::unique_ptr<DebugInfoTask>(
        new DebugInfoTask(builder_->GetIsa(), method_infos));
