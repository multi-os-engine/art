...
bool ElfWriterQuick<ElfTypes>::Write(
  ...
  auto write_rodata = [oat_writer](File* f)->bool {
    std::unique_ptr<BufferedOutputStream> output_stream(
        new BufferedOutputStream(new FileOutputStream(f)));
    return oat_writer->WriteRodata(output_stream.get());
  };
  auto write_text = [oat_writer](File* f)->bool {
    std::unique_ptr<BufferedOutputStream> output_stream(
        new BufferedOutputStream(new FileOutputStream(f)));
    return oat_writer->WriteCode(output_stream.get());
  };
  std::unique_ptr<ElfBuilder<ElfTypes>> builder(new ElfBuilder<ElfTypes>(
      isa, rodata_size, write_rodata, text_size, write_text, bss_size));
...
static void WriteDebugSymbols(ElfBuilder<ElfTypes>* builder, OatWriter* oat_writer) {
  ...
  typename ElfBuilder<ElfTypes>::SymtabSection* symtab = builder->GetSymtab();
  for (auto it = method_info.begin(); it != method_info.end(); ++it) {
