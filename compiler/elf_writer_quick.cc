...
bool ElfWriterQuick::Write(OatWriter* oat_writer,
  ...
  if (hasLineInfo || hasCFI) {
    ...
    ElfRawSectionBuilder rel_debug(".rel.debug",       SHT_PROGBITS, 0, nullptr, 0, 1, 5);

    FillInCFIInformation(oat_writer, debug_info.GetBuffer(),
                         debug_abbrev.GetBuffer(), debug_str.GetBuffer(),
                         hasLineInfo ? debug_line.GetBuffer() : nullptr,
                         rel_debug.GetBuffer(), builder->text_builder_.section_.sh_addr);
    ...
    if (hasCFI) {
      ElfRawSectionBuilder eh_frame(".eh_frame",  SHT_PROGBITS, SHF_ALLOC, nullptr, 0, 4, 0);
      eh_frame.SetBuffer(*cfi);
      ElfFile::FixupEHFrame(builder->text_builder_.section_.sh_addr,
                            &eh_frame.GetBuffer()->front(),
                            eh_frame.GetBuffer()->size());
    ...
    if (rel_debug.GetBuffer()->size() > 0) {
      builder->RegisterRawSection(rel_debug);
    }
  }
...
static void PushRelAddr(byte info_index, std::vector<uint8_t>& section,
               uint32_t rel_value, std::vector<uint8_t>& dbg_rel_table) {
  PushByte(dbg_rel_table, info_index);
  PushWord(dbg_rel_table, section.size());
  PushWord(section, rel_value);
}
...
class LineTableGenerator : public Leb128Encoder {
 public:
  ...
  void SetAddr(uintptr_t addr) {
    ...
    PushRelAddr(ElfFile::DEBUG_SECTION_INDEX::LINE, data_, addr, rel_table_);
  }
...
void ElfWriterQuick::FillInCFIInformation(OatWriter* oat_writer,
                                          ...
                                          std::vector<uint8_t>* dbg_rel_table,
  ...
  PushRelAddr(ElfFile::DEBUG_SECTION_INDEX::INFO, *dbg_info, 0, *dbg_rel_table);
  PushRelAddr(ElfFile::DEBUG_SECTION_INDEX::INFO, *dbg_info, 0, *dbg_rel_table);

  if (dbg_line == nullptr) {
    for(unsigned i = 0; i < method_info.size(); ++i) {
      ...
      PushRelAddr(ElfFile::DEBUG_SECTION_INDEX::INFO, *dbg_info,
                  dbg.low_pc_ + text_section_offset, *dbg_rel_table);
      PushRelAddr(ElfFile::DEBUG_SECTION_INDEX::INFO, *dbg_info,
                  dbg.high_pc_ + text_section_offset, *dbg_rel_table);
    }
  } else {
    ...
    typedef std::unordered_map<const char*, int> files_t;
    files_t files;
    for(unsigned i = 0; i < method_info.size(); ++i) {
      const OatWriter::DebugInfo &dbg = method_info[i];
      const char* file_name = dbg.src_file_name_ == nullptr ? "null" : dbg.src_file_name_;
      files_t::const_iterator found = files.find(file_name);
      if (found == files.end()) {
        unsigned file_index = 1 + files.size();
    ...
    // one long program for all methods
    LineTableGenerator line_table_generator(LINE_BASE, LINE_RANGE, OPCODE_BASE,
                                            *dbg_line, 0, 1, *dbg_rel_table);

    SrcMap pc2java_map;
    for(unsigned i = 0; i < method_info.size(); ++i) {
      const OatWriter::DebugInfo &dbg = method_info[i];
      const char* file_name = dbg.src_file_name_ == nullptr ? "null" : dbg.src_file_name_;
      unsigned file_index = files[file_name];
      DCHECK(file_index != 0) << file_name;
      ...
      pc2java_map.clear();
      getLineInfoForJava(dbg.dbgstream_, dbg.pc2dex_map_, pc2java_map, dbg.low_pc_);
