...
static void WriteDebugSymbols(const CompilerDriver* compiler_driver,
  ...
  for (auto it = method_info.begin(); it != method_info.end(); ++it) {
    ...
    // Conforming to aaelf, add $t to indicate start of a sequence of thumb2
    // instructions, so that disassembler tools can correctly disassemble.
    if (compiler_driver->GetInstructionSet() == kThumb2) {
      symtab->AddSymbol("$t", &builder->text_builder_, it->low_pc_ & ~1, true,
