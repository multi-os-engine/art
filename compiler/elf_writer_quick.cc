...
#include "dwarf/elf_writer_debug.h"
...
std::unique_ptr<ElfWriter> CreateElfWriterQuick(InstructionSet instruction_set,
                                                ...
                                                File* elf_file) {
  if (kProduce64BitELFFiles && Is64BitInstructionSet(instruction_set)) {
    return MakeUnique<ElfWriterQuick64>(instruction_set, compiler_options, elf_file);
  } else {
    return MakeUnique<ElfWriterQuick32>(instruction_set, compiler_options, elf_file);
  }
}

template <typename ElfTypes>
static void WriteDebugSymbols(ElfBuilder<ElfTypes>* builder,
                              const ArrayRef<const CompiledMethodDebugInfo>& debug_infos);
...
void ElfWriterQuick<ElfTypes>::EndRoData(OutputStream* rodata) {
  CHECK(builder_->GetRoData() == rodata);
...
void ElfWriterQuick<ElfTypes>::WriteDebugInfo(
  ...
  if (compiler_options_->GetGenerateDebugInfo()) {
    if (!debug_infos.empty()) {
      ...
      WriteDebugSymbols(builder_.get(), debug_infos);
      // Generate CFI (stack unwinding information).
      dwarf::WriteCFISection(builder_.get(), debug_infos, kCFIFormat);
      // Write DWARF .debug_* sections.
      dwarf::WriteDebugSections(builder_.get(), debug_infos);
    }
  }
}
...
static void WriteDebugSymbols(ElfBuilder<ElfTypes>* builder,
  ...
  for (const CompiledMethodDebugInfo& info : debug_infos) {
    if (info.deduped_) {
