...
bool ElfWriterQuick::Write(OatWriter& oat_writer,
  ...
  bool generateDebugInformation = oat_writer.GetCFIInfo().size() > 0;
  ...
  uint32_t shdbg_frm_size = oat_writer.GetCFIInfo().size();
  ...
  if (generateDebugInformation) {
    ...
    section_headers[SH_DBG_INFO].sh_type      = llvm::ELF::SHT_PROGBITS;
    ...
    section_headers[SH_DBG_ABRV].sh_type      = llvm::ELF::SHT_PROGBITS;
    ...
    section_headers[SH_DBG_FRM].sh_type      = llvm::ELF::SHT_PROGBITS;
    ...
    section_headers[SH_DBG_STR].sh_type      = llvm::ELF::SHT_PROGBITS;
  ...
  if (generateDebugInformation) {
    ...
    if (!elf_file_->WriteFully(&oat_writer.GetCFIInfo()[0], shdbg_frm_size)) {
...
void ElfWriterQuick::FillInCFIInformation(OatWriter& oat_writer,
  ...
  dbg_abbrev.push_back(llvm::dwarf::DW_TAG_compile_unit);

  // There are children (the methods).
  dbg_abbrev.push_back(llvm::dwarf::DW_CHILDREN_yes);

  // DW_LANG_Java DW_FORM_data1.
  dbg_abbrev.push_back(llvm::dwarf::DW_AT_language);
  dbg_abbrev.push_back(llvm::dwarf::DW_FORM_data1);

  // DW_AT_low_pc DW_FORM_addr.
  dbg_abbrev.push_back(llvm::dwarf::DW_AT_low_pc);
  dbg_abbrev.push_back(llvm::dwarf::DW_FORM_addr);

  // DW_AT_high_pc DW_FORM_addr.
  dbg_abbrev.push_back(llvm::dwarf::DW_AT_high_pc);
  dbg_abbrev.push_back(llvm::dwarf::DW_FORM_addr);
  ...
  dbg_abbrev.push_back(llvm::dwarf::DW_TAG_subprogram);

  // There are no children.
  dbg_abbrev.push_back(llvm::dwarf::DW_CHILDREN_no);

  // Name of the method.
  dbg_abbrev.push_back(llvm::dwarf::DW_AT_name);
  dbg_abbrev.push_back(llvm::dwarf::DW_FORM_strp);
  ...
  dbg_info.push_back(llvm::dwarf::DW_LANG_Java);
