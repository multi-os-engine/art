...
std::vector<uint8_t>* ConstructCFAFrameX86(bool is_x86_64) {
...
std::vector<uint8_t>* ConstructCFAFrame(InstructionSet isa) {
  switch (isa) {
    case kX86:
      return ConstructCFAFrameX86(false);
    case kX86_64:
      return ConstructCFAFrameX86(true);
...
void ElfWriterQuick::WriteDebugSymbols(ElfBuilder& builder, OatWriter* oat_writer) {
  std::unique_ptr<std::vector<uint8_t>> cfi_info(
      ConstructCFAFrame(compiler_driver_->GetInstructionSet()));
  ...
  if (cfi_info.get() != nullptr) {
    ...
    eh_frame.SetBuffer(*cfi_info.get());
