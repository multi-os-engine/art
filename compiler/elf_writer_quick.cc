  ...
  switch (isa) {
    ...
    case kX86_64: {
      ...
      for (int reg = 0; reg < 16; reg++) {
        if (reg < 12 && reg != 3 && reg != 5) {  // except EBX and EBP.
  ...
  if (dbg_line != nullptr) {
    ...
    for (size_t i = 0; i < method_infos.size(); i++) {
      ...
      if (method_info.src_file_name_ != nullptr) {
        ...
        if (file_name_slash == file_name.npos &&  // Just filename.
            class_name.front() == 'L' &&  // Type descriptor for a class.
            class_name_slash != class_name.npos) {  // Has package name.
          std::string package_name = class_name.substr(1, class_name_slash - 1);
          if ((directory_index = directories_map[package_name]) == 0) {
            directory_index = 1 + directories.size();
            directories_map[package_name] = directory_index;
        ...
        // Add file entry.
        if ((file_index = files_map[full_path]) == 0) {
          file_index = 1 + files.size();
          files_map[full_path] = file_index;
...
static void WriteDebugSymbols(const CompilerDriver* compiler_driver,
  ...
  for (auto it = method_info.begin(); it != method_info.end(); ++it) {
    ...
    int thumb_offest = it->compiled_method_->CodeDelta();
    symtab->AddSymbol(it->method_name_, &builder->GetTextBuilder(),
                      it->low_pc_ + thumb_offest, true,
    ...
    if (unwind_opcodes != nullptr && unwind_opcodes->size() > 0) {
