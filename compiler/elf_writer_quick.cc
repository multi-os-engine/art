...
bool WriteCIE(dwarf::DebugFrameWriter* cfi, InstructionSet isa) {
  // TODO: fp registers
  switch (isa) {
    case kArm:
    case kThumb2: {
      ...
      for (int reg = 0; reg < 32; reg++) {
        ...
        if (16 <= reg && reg <= 31) {  // s16-s31 used for promotion
    ...
    case kArm64: {
      ...
      for (int reg = 0; reg < 30; reg++) {
        if ((8 <= reg && reg <= 15) ||  // Caller save registers
            (20 <= reg && reg <= 29)) {  // Callee save registers (promotion targets)
          opcodes.SameValue(0, reg);
        } else {
          opcodes.Undefined(0, reg);
        }
      ...
      for (int reg = 0; reg < 32; reg++) {
        int dwarf_fp_base = 64;
        if (8 <= reg && reg <= 15) {  // bottom 64-bits preserved across C calls
          opcodes.SameValue(0, dwarf_fp_base + reg);
        } else {
          opcodes.Undefined(0, dwarf_fp_base + reg);
        }
...
static void WriteDebugSymbols(const CompilerDriver* compiler_driver,
  ...
  bool hasCFI = WriteCIE(&cfi, compiler_driver->GetInstructionSet());
