...
class DebugInfoEntryWriter FINAL : private Writer<Vector> {
 ...
 public:
  ...
  uint32_t StartTag(Tag tag) {
    ...
    depth_++;
    inside_entry_ = true;
    return dchecked_integral_cast<uint32_t>(abbrev_code_offset_ +
                                            kCompilationUnitHeaderSize);
  }
  ...
  void WriteBlock(Attribute attrib, const void* ptr, int num_bytes) {
    AddAbbrevAttribute(attrib, DW_FORM_block);
    this->PushUleb128(num_bytes);
    this->PushData(ptr, num_bytes);
  }

  void WriteExprLoc(Attribute attrib, const void* ptr, int num_bytes) {
  ...
  // NB: New in DWARF4. Previous versions use DW_FORM_data4 or DW_FORM_data8.
  ...
  void WriteStrp(Attribute attrib, uint32_t address) {
