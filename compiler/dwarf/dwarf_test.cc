...
TEST_F(DwarfTest, DebugFrame) {
  ...
  opcodes.RelOffsets(Reg(2), 8, 1 | (1 << 3), 4);  // push R2 and R5
  DW_CHECK_NEXT("DW_CFA_offset: r2 (edx) at cfa-100");
  DW_CHECK_NEXT("DW_CFA_offset: r5 (ebp) at cfa-96");
  opcodes.Restores(Reg(2), 1 | (1 << 3));  // pop R2 and R5
...
// Try to use both the long and short encoding, and check that they are equivalent.
TEST_F(DwarfTest, DebugLineSpecialOpcodes) {
  const bool is64bit = false;
  const int code_factor_bits = 1;
  uint64_t pc = 0x01000000;
  int line = 1;
  DebugLineOpCodeWriter<> opcodes_long(is64bit, code_factor_bits);
  opcodes_long.SetAddress(pc);
  DebugLineOpCodeWriter<> opcodes_short(is64bit, code_factor_bits);
  opcodes_short.SetAddress(pc);
  for (int addr_delta = 0; addr_delta < 64; addr_delta += 2) {
    for (int line_delta = 16; line_delta >= -16; --line_delta) {
      pc += addr_delta;
      line += line_delta;
      opcodes_long.AdvancePC(pc);
      opcodes_long.AdvanceLine(line);
      opcodes_long.AddRow();
      ASSERT_EQ(opcodes_long.current_address(), pc);
      ASSERT_EQ(opcodes_long.current_line(), line);
      opcodes_short.AddRow(pc, line);
      ASSERT_EQ(opcodes_short.current_address(), pc);
      ASSERT_EQ(opcodes_short.current_line(), line);
    }
  }
  ...
  DebugLineWriter<> debug_line_long(&debug_line_data_);
  debug_line_long.WriteTable(directories, files, opcodes_long);
  auto lines_long = Objdump(is64bit, "-WL");
  debug_line_data_.clear();
  lines_long.erase(lines_long.begin());  // temp filename.

  DebugLineWriter<> debug_line_short(&debug_line_data_);
  debug_line_short.WriteTable(directories, files, opcodes_short);
  auto lines_short = Objdump(is64bit, "-WL");
  debug_line_data_.clear();
  lines_short.erase(lines_short.begin());  // temp filename.

  CheckObjdumpOutput(lines_long, lines_short);
}
