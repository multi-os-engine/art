...
void WriteCFISection(ElfBuilder<ElfTypes>* builder,
                     const ArrayRef<const CompiledMethodDebugInfo>& method_infos,
                     CFIFormat format) {
                       ...
                       : builder->GetEhFrame());
  {
    ...
    buffer.clear();
    for (const CompiledMethodDebugInfo& mi : method_infos) {
...
struct CompilationUnit {
  std::vector<const CompiledMethodDebugInfo*> methods_;
...
class DebugInfoWriter {
  ...
  class CompilationUnitWriter {
   public:
    ...
    void WriteRegLocation(const CompiledMethodDebugInfo* method_info, uint16_t vreg,
...
class DebugLineWriter {
 ...
 public:
  ...
  size_t WriteCompilationUnit(CompilationUnit& compilation_unit) {
    ...
    for (const CompiledMethodDebugInfo* mi : compilation_unit.methods_) {
