 ...
 public:
  ...
  std::vector<std::string> Objdump(const char* args) {
    ...
    if (!debug_info_data_.empty()) {
      builder.WriteSection(".debug_info", &debug_info_data_, nullptr);
    }
    if (!debug_abbrev_data_.empty()) {
      builder.WriteSection(".debug_abbrev", &debug_abbrev_data_, nullptr);
    }
    if (!debug_str_data_.empty()) {
      builder.WriteSection(".debug_str", &debug_str_data_, nullptr);
    }
    if (!debug_line_data_.empty()) {
      builder.WriteSection(".debug_line", &debug_line_data_, nullptr);
    }
    if (!debug_frame_data_.empty()) {
      builder.WriteSection(".debug_frame", &debug_frame_data_, nullptr);
    }
    EXPECT_TRUE(builder.End());
