...
class DwarfTest : public CommonRuntimeTest {
 public:
  ...
  // Compare two objdumps.
  static void CheckObjdumpOutput(const std::vector<std::string>& expected_lines,
                                 const std::vector<std::string>& actual_lines) {
    for (size_t i = 0; i < expected_lines.size() || i < actual_lines.size(); i++) {
      auto expected_line = i < expected_lines.size() ? expected_lines[i].data() : "";
      auto actual_line = i < actual_lines.size() ? actual_lines[i].data() : "";
      EXPECT_STREQ(expected_line, actual_line);
    }
  }
