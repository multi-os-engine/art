...
namespace art {
namespace dwarf {
  class DedupVector : public std::vector<uint8_t> {
   public:
    ...
    size_t Insert(const uint8_t* ptr, size_t num_bytes) {
      ...
      for (auto it = range.first; it != range.second; ++it) {
        ...
        if (offset + num_bytes <= this->size() &&
            memcmp(this->data() + offset, ptr, num_bytes) == 0) {
          return offset;
        }
      }
      // Append the data at the end of the vector.
      const size_t new_offset = this->size();
      hash_to_offset_.emplace(hash, new_offset);
      this->insert(this->end(), ptr, ptr + num_bytes);
      return new_offset;
    }
