...
class PatchOat {
 public:
  ...
  static bool Patch(const File* oat_in, const std::string& art_location,
                    off_t delta, File* oat_out, File* art_out, TimingLogger& timings);
 ...
 private:
  ...
  PatchOat(ElfFile* oat_file, off_t delta, bool check, TimingLogger& timings)
      : oat_file_(oat_file), delta_(delta), check_(check), timings_(timings) {}
  PatchOat(ElfFile* oat_file, MemMap* image, gc::accounting::ContinuousSpaceBitmap* bitmap,
           MemMap* heap, off_t delta, bool check, TimingLogger& timings)
      : oat_file_(oat_file), image_(image), bitmap_(bitmap), heap_(heap),
        delta_(delta), check_(check), timings_(timings) {}
  ~PatchOat() {}
