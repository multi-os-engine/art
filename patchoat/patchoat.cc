...
bool PatchOat::Patch(const std::string& image_location, off_t delta,
  ...
  std::unique_ptr<MemMap> image(MemMap::MapFile(image_len, PROT_READ | PROT_WRITE, MAP_PRIVATE,
                                                input_image->Fd(), 0, input_image->GetPath().c_str(),
                                                &error_msg));
...
static void Usage(const char *fmt, ...) {
  ...
  UsageError("");
  UsageError("  --input-image-location=<file.art>: Specifies the 'location' of the image file to be");
  ...
  UsageError("");
  UsageError("  --output-image-file=<file.art>: Specifies the exact file to write the patched image");
  UsageError("      file to.");
  UsageError("");
  ...
  UsageError("");
  UsageError("  --patched-image-location=<file.art>: Use the same patch delta as was used to patch");
