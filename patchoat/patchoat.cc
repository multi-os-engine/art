...
static ImageInfo GetImageInfo(const std::string& image_location, InstructionSet isa) {
  ImageAssistant ia(image_location, isa);
  ImageInfo info = ia.GetSystemImageInfo();
  if (!info.IsValid()) {
    return ia.GetCacheImageInfo();
  } else {
    return info;
  }
}
...
static int patchoat(int argc, char **argv) {
  ...
  if (!patched_image_location.empty()) {
    ...
    ImageAssistant ia(patched_image_location, isa);
    ImageInfo info = ia.GetSelectedImageInfo();
    if (!info.IsValid()) {
      Usage("Unable to determine image file for location %s", patched_image_location.c_str());
    } else if (info.GetImageState() != ImageState::kImageUsable) {
