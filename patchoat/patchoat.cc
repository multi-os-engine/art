...
// encoded in its oat header value encoded in its oat header
template <typename ElfFileImpl>
static bool ReadOatBaseDelta(const ElfFileImpl* oat_file, off_t* delta, std::string* error_msg) {
  auto rodata_sec = oat_file->FindSectionByName(".rodata");
  if (rodata_sec == nullptr) {
    LOG(ERROR) << "Unable to find .rodata section to get current patch delta";
    return false;
  }
...
static bool ReadOatBaseDelta(const ElfFile* elf_file, off_t* delta, std::string* error_msg) {
  CHECK(delta != nullptr);
...
bool PatchOat::Patch(File* input_oat, const std::string& image_location, off_t delta, File*
                     output_oat, File* output_image, InstructionSet isa, TimingLogger* timings, bool
                     output_oat_opened_from_fd, bool new_oat_out) {
  CHECK(Runtime::Current() == nullptr);
  CHECK(output_image != nullptr);
  CHECK_GE(output_image->Fd(), 0);
  ...
  PatchOat p(isa, elf.release(), image.release(), ispc->GetLiveBitmap(), ispc->GetMemMap(), delta,
             timings);
  t.NewTiming("Patching files");
...
bool PatchOat::Patch(File* input_oat, off_t delta, bool match_delta, File* output_oat,
                     TimingLogger* timings, bool output_oat_opened_from_fd, bool new_oat_out) {
  CHECK(input_oat != nullptr);
  CHECK(output_oat != nullptr);
  CHECK_GE(input_oat->Fd(), 0);
  CHECK_GE(output_oat->Fd(), 0);
...
static int patchoat(int argc, char **argv) {
  ...
  if (have_image_files && match_delta) {
  ...
  if (have_image_files && have_oat_files) {
    ...
    ret = PatchOat::Patch(input_oat.get(), input_image_location, base_delta, output_oat.get(),
                          output_image.get(), isa, &timings,
