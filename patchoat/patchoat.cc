...
bool PatchOat::Patch(const std::string& art_location, off_t delta,
                     File* output_art, InstructionSet isa,
  ...
  std::string image_filename(GetSystemImageFilename(art_location.c_str(), isa));
...
bool PatchOat::Patch(const File* input_oat, const std::string& art_location, off_t delta,
                     File* output_oat, File* output_art, InstructionSet isa,
                     TimingLogger& timings) {
  std::string err;
  CHECK(Runtime::Current() == nullptr);
  CHECK(output_art != nullptr);
  ...
  if (isa == kNone) {
    ...
    isa_name = ElfISAToRuntimeString(elf_hdr.e_machine);
    isa = GetInstructionSetFromString(isa_name);
  } else {
    isa_name = GetInstructionSetString(isa);
  }
  std::string image_filename(GetSystemImageFilename(art_location.c_str(), isa));
  std::unique_ptr<File> input_art(OS::OpenFileForReading(image_filename.c_str()));
  if (input_art.get() == nullptr) {
...
static void Usage(const char *fmt, ...) {
  ...
  UsageError("      patched. If --instruction-set is not given it will use the instruction set");
  ...
  UsageError("");
  UsageError("  --output-art-file=<file.art>: Specifies the exact file to write the patched art");
  UsageError("      file to.");
  UsageError("");
  UsageError("  --output-art-fd=<file-descriptor>: Specifies the file-descriptor to write the");
  UsageError("      the patched art file to.");
  UsageError("");
  UsageError("  --output-art-location=<file.art>: Specifies the 'location' to write the patched");
  UsageError("      art file to. If used one must also specify the --instruction-set");
  UsageError("");
  ...
  UsageError("");
  UsageError("  --patched-art-location=<file.art>: Use the same patch delta as was used to patch");
  UsageError("      the given image location. If used one must also specify the --instruction-set");
  UsageError("");
...
int patchoat(int argc, char **argv) {
  ...
  std::string output_art_location;
  int output_art_fd = -1;
  bool have_output_art = false;
  ...
  std::string patched_art_location;
  ...
  for (int i = 0; i < argc; i++) {
    ...
    if (option.starts_with("--instruction-set=")) {
    ...
    } else if (option.starts_with("--output-art-location=")) {
      if (have_output_art) {
        Usage("Only one of --output-art-file, --output-art-location and --output-art-fd may "
              "be used.");
      }
      have_output_art = true;
      output_art_location= option.substr(strlen("--output-art-location=")).data();
      ...
      if (have_output_art) {
        Usage("Only one of --output-art-file, --output-art-location and --output-art-fd "
              "may be used.");
      }
      have_output_art = true;
    ...
    } else if (option.starts_with("--patched-art-location=")) {
      patched_art_location = option.substr(strlen("--patched-art-location=")).data();
  ...
  if ((!input_art_location.empty()) != have_output_art) {
  ...
  bool have_art_files = have_output_art;
  bool have_oat_files = have_output_oat;

  if (!have_oat_files && !have_art_files) {
    Usage("Must be patching either an oat or an art file or both.");
  }

  if (!have_oat_files && !isa_set) {
    Usage("Must include ISA if patching an art file without an oat file.");
  }
  ...
  if (!output_art_location.empty()) {
    if (!isa_set) {
      Usage("specifying a location requires specifying an instruction set");
    }
    output_art_filename = GetSystemImageFilename(output_art_location.c_str(), isa);
    if (debug) {
      LOG(INFO) << "Using output-art-file " << output_art_filename;
    }
  }
  if (!patched_art_location.empty()) {
    if (!isa_set) {
      Usage("specifying a location requires specifying an instruction set");
    }
    patched_art_filename = GetSystemImageFilename(patched_art_location.c_str(), isa);
    if (debug) {
      LOG(INFO) << "Using patched-art-file " << patched_art_filename;
    }
  }
  ...
  bool ret;
  if (have_art_files && have_oat_files) {
    TimingLogger::ScopedTiming pt("patch image and oat", &timings);
    ret = PatchOat::Patch(input_oat.get(), input_art_location, base_delta,
                          output_oat.get(), output_art.get(), isa, timings);
    ...
    CHECK(have_art_files);
    ret = PatchOat::Patch(input_art_location, base_delta, output_art.get(), isa, timings);
  }
  cleanup(ret);
