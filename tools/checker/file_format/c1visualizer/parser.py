...
def __parseC1Line(line, lineNo, state):
  ...
  if state.currentState == C1ParserState.StartingCfgBlock:
    ...
    if re.match("name\s+\"[^\"]+\"", line):
      ...
      Logger.fail("Expected output group name", state.fileName, lineNo)
    ...
    if re.match("method\s+\"[^\"]*\"", line):
      ...
      if not methodName:
        Logger.fail("Empty method name in output", state.fileName, lineNo)
    ...
    if line == "begin_cfg":
      ...
      if state.lastMethodName is None:
        Logger.fail("Expected method header", state.fileName, lineNo)
      ...
      Logger.fail("C1visualizer line not inside a group", state.fileName, lineNo)

def ParseC1visualizerStream(fileName, stream):
  c1File = C1visualizerFile(fileName)
  state = C1ParserState(fileName)
  fnProcessLine = lambda line, lineNo: __parseC1Line(line, lineNo, state)
  for pass_name, pass_lines, startLineNo in SplitStream(stream, fnProcessLine):
    C1visualizerPass(c1File, pass_name, pass_lines, startLineNo + 1)
  return c1File
