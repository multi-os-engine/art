...
from file_format.checker.parser import CheckerParser
...
class CheckerParser_PrefixTest(unittest.TestCase):
  ...
  def tryParse(self, string):
    ...
    checkFile = CheckerParser().parse("<test-file>", "CHECK", io.StringIO(checkerText))
    self.assertEqual(len(checkFile.cases), 1)
    testCase = checkFile.cases[0]
...
class CheckerParser_RegexExpressionTest(unittest.TestCase):
  ...
  def parseAssertion(self, string, variant=""):
    ...
    checkerFile = CheckerParser().parse("<test-file>", "CHECK", io.StringIO(checkerText))
    self.assertEqual(len(checkerFile.cases), 1)
    testCase = checkerFile.cases[0]
...
class CheckerParser_FileLayoutTest(unittest.TestCase):
  ...
  def assertParsesTo(self, checkerText, expectedData):
    ...
    actualFile = CheckerParser().parse("<test_file>", "CHECK", io.StringIO(ToUnicode(checkerText)))
