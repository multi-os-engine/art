...
def ParseCheckerStream(fileName, prefix, stream):
  ...
  for case_name, case_lines, startLineNo in SplitStream(stream, fnProcessLine, fnLineOutsideChunk):
    test_case = TestCase(checkerFile, case_name, startLineNo)
    for case_line in case_lines:
      ParseCheckerAssertion(test_case, case_line[0], case_line[1], case_line[2])
  return checkerFile
