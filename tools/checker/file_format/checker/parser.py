...
def __extractLine(prefix, line, arch = None, dbg = False):
  ...
  dbg_specifier = r"-DEBUGGABLE" if dbg else r""
...
def __processLine(line, lineNo, prefix, fileName):
  ...
  for dbg in [True, False]:
    for arch in [None] + archs_list:
      startLine = __extractLine(prefix + "-START", line, arch, dbg)
      if startLine is not None:
        return None, startLine, (arch, dbg)
