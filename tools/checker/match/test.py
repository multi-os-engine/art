...
from file_format.c1visualizer.parser import C1visualizerParser
...
class MatchLines_Test(unittest.TestCase):
  ...
  def createTestAssertion(self, checkerString):
    ...
    return CheckerParser().parseAssertion(testCase, checkerString, TestAssertion.Variant.InOrder, 0)
  ...
  def matches(self, checkerString, c1String):
    ...
    c1String = \
      ...
      """
        end_cfg
      """
    checkerFile = CheckerParser().parse(
        "<checker-file>", "CHECK", io.StringIO(ToUnicode(checkerString)))
    c1File = C1visualizerParser().parse("<c1-file>", io.StringIO(ToUnicode(c1String)))
    try:
