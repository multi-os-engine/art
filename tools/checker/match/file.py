...
def __matchIndependentChecks(checkLines, outputLines, startLineNo, varState):
  ...
  for checkLine in checkLines:
    ...
    if varState is None:
      Logger.testFailed("Could not match check line \"" + checkLine.orig_text + "\" " +
                        "starting from output line " + str(startLineNo),
                        checkLine.getFileName(), checkLine.lineNo)
...
def __matchNotLines(checkLines, outputLines, startLineNo, varState):
  for checkLine in checkLines:
    ...
    if matchVarState is not None:
      Logger.testFailed("CHECK-NOT line \"" + checkLine.orig_text + "\" matches output line " + \
                        str(matchLineNo), checkLine.getFileName(), checkLine.lineNo)
...
def __matchGroups(checkGroup, outputGroup):
  assert isinstance(checkGroup, TestCase)
  assert isinstance(outputGroup, C1visualizerPass)
...
def MatchFiles(checkerFile, c1File):
  assert isinstance(checkerFile, CheckerFile)
  assert isinstance(c1File, C1visualizerFile)

  for testCase in checkerFile.cases:
    ...
    if c1Pass is None:
      Logger.fail("Test case \"" + testCase.name + "\" not found in the C1visualizer output",
                  testCase.getFileName(), testCase.lineNo)
