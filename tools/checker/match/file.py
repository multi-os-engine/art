...
MatchInfo = namedtuple("MatchInfo", ["start", "end", "variables"])
...
def findMatchingLine(assertion, c1Pass, start, variables, skipLines=[]):
  """ Finds the first line in c1Pass which matches `assertion`.

  Scan starts on line `start` and skips lines in `skipLines`.
  ...
  """
  for i in range(start, len(c1Pass.body)):
    if i in skipLines: continue
    newVariables = MatchLines(assertion, c1Pass.body[i], variables)
    if newVariables is not None:
      return MatchInfo(i, i, newVariables)
  raise MatchFailedException(assertion, start)

def matchDagGroup(assertions, c1Pass, start, variables):
  """ Attempts to find matching c1Pass lines for a group of DAG assertions.

  Assertions are matched in the list order and variable values propagated. Scan
  always starts on line `start` and each line can only match one assertion.
  ...
  for assertion in assertions:
    ...
    match = findMatchingLine(assertion, c1Pass, start, variables, matchedLines)
    variables = match.variables
    assert match.start == match.end
    assert match.start not in matchedLines
    matchedLines.append(match.start)
  return MatchInfo(min(matchedLines), max(matchedLines), variables)

def testNotGroup(assertions, c1Pass, start, end, variables):
  ...
  for assertion in assertions:
    ...
    for i in range(start, end):
      if MatchLines(assertion, c1Pass.body[i], variables) is not None:
...
def MatchTestCase(testCase, c1Pass):
  ...
  for assertionGroup in assertionGroups:
    if assertionGroup is None:
      ...
      length = len(c1Pass.body)
      match = MatchInfo(length, length, None)
      ...
      match = findMatchingLine(assertionGroup[0], c1Pass, matchFrom, variables)
    else:
      ...
      match = matchDagGroup(assertionGroup, c1Pass, matchFrom, variables)
    ...
    if pendingNotAssertions:
      ...
      testNotGroup(pendingNotAssertions, c1Pass, matchFrom, match.start, variables)
      pendingNotAssertions = None

    # Update state.
    assert matchFrom <= match.end
    matchFrom = match.end + 1
    variables = match.variables
  ...
  for testCase in checkerFile.testCases:
    ...
    try:
      ...
      if e.assertion.variant == TestAssertion.Variant.Not:
        Logger.testFailed("NOT assertion matched line {}".format(e.lineNo),
                          e.assertion.fileName, e.assertion.lineNo)
      else:
        Logger.testFailed("Assertion could not be matched starting from line {}".format(e.lineNo),
                          e.assertion.fileName, e.assertion.lineNo)
