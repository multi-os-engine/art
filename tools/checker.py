  ...
  def __parse(self, line):
    ...
    while line:
      ...
      if self.__isMatchAtStart(matchWhitespace):
        # A whitespace in the check line create a new separator of line parts.
  ...
  def match(self, outputLine, initialVarState):
    ...
    for part in self.lineParts:
      ...
      while True:
        ...
        if (match is None) or (not afterSeparator and not self.__isMatchAtStart(match)):
          return None
        matchEnd = matchStart + match.end()
        matchStart += match.start()
        if not afterSeparator or self.__isSeparated(outputLine, matchStart):
          break
        else:
          matchStart += 1
