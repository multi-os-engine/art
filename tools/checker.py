...
class CheckGroup(CommonEqualityMixin):
  ...
  # List pop which doesn't change the original list object.
  def __pop(self, list):
...
class CheckFile(FileSplitMixin):
  ...
  def match(self, outputFile, printInfo=False):
    for checkGroup in self.groups:
      # TODO: Currently does not handle multiple occurences of the same group
...
class OutputFile(FileSplitMixin):
  """Representation of the output generated by the test and split into groups
     ...
     marks the beginning of a method. It is parsed for the method's name but
...
def ParseArguments():
  ...
  parser.add_argument("test_file", help="the source of the test also containing checks")
