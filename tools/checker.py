...
# This script compares two files: a check file and an output file. Check file is typically
# the source file of a test which contains the expected output in its comments. The "check-lines"
# begin with the 'CHECK' prefix followed by a pattern that is searched for in the output file. They
# are matched in the order of appearance in the check file.
#
# Check patterns are treated as plain text rather than regular expressions but they are whitespace
# agnostic.
#
# Regex patterns can be inserted enclosed in '{{' and '}}' brackets. If these need to be used
# inside the body of the regex, they need to be separated with round brackets, e.g. '{{(fo{2})}}'
# will match 'foo'.
#
# Regex patterns can be named and referenced later. A new variable is defined with '[[name:regex]]'
...
#   // CHECK: return [[REG]]
#
#   Provided the output file is a smali dump, these checks will match the output if it contains
#   an 'add' instruction whose destination register matches the register referenced by a 'return'
#   instruction further in the file.
...
class CheckLine:
  ...
  # Returns a RegexPart object containing the given plain text but escaped for use as a regular
  # expression.
  def _parseText(self, text):
    return Regex(RegexVariant.Text, None, re.escape(text))

  # Returns a RegexPart object that holds a regex pattern specified in the check line.
  def _parsePattern(self, pattern):
    return Regex(RegexVariant.Pattern, None, pattern[2:len(pattern)-2])

  # Returns a RegexPart object that hold a variable use (either a reference or a definition).
  ...
  # This method parses the content of a check line stripped of the initial CHECK keyword. It starts
  # from the beginning and looks for the nearest whitespace/pattern/variable in the text. These are
  # parsed in a suitable manner and stretches of plain text between are escaped. These individual
  ...
  def _parseCheckLine(self, line):
    ...
    while line:
      ...
      if self._isMatchAtStart(matchWhitespace):
        ...
        firstMatch = self._firstMatchStart([ matchWhitespace, matchPattern, matchVariable ], line)
  ...
  # Variable references are substituted according to their value in the current environment.
  # This method raises an exception if a variable name is not defined in the environment.
  def _generatePattern(self, regexPart, env):
    if regexPart.variant == RegexVariant.VarRef:
      try:
        return re.escape(env[regexPart.name])
      except KeyError:
        raise Exception("ERROR: Variable '" + regexPart.name + "' undefined on check line " +
                        str(self.lineNo))
    else:
      return regexPart.pattern

  # Attempts to match the check line against a line from the output file in the given environment.
  # It returns the new environment when successful and None otherwise.
  # The algorithm will first search for the first element on its regex part list and then attempt
  # to match the rest of the list immediately following the first match. If this is unsuccessful
  # the algorithm repeats, attempting to find another match for the first element.
  # Because the lines are not being matched as a whole but rather by parts, a variable can be
  # referenced on the same line, immediately after it has been defined.
...
class CheckFile:
  ...
  # Attempts to parse a check line. The regex searches for a comment symbol followed by the
  # specified check keyword and a colon at the very beginning of the line. Whitespaces are ignored.
  ...
  # Reads from the output file and attempts to match the check lines in their given order.
