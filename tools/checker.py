...
# compiler. "Check lines" are assertions placed in comments of the Java file.
...
#  - In-order: Must match an output line which appears in the output group later
#              than lines matched against any preceeding checks.
#              These lines are marked with CHECK.
#  - No-order: Must match an output line which appears in the output group later
#              than lines matched against any preceeding in-order checks.
#              In other words, the order of output lines does not matter between
#              consecutive no-order checks.
#              These lines are marked with CHECK-DAG.
#  - Not:      Must not match any output line which appear in the output group
...
#              These lines are marked with CHECK-NOT.
#
    ...
    while line:
      ...
      if self.__isMatchAtStart(matchWhitespace):
        ...
        if self.variant == CheckLine.Variant.Not and elem.variant == CheckElement.Variant.VarDef:
          raise Exception("NOT check lines cannot define variables " +
...
class CheckGroup(CommonEqualityMixin):
  ...
  # other's match location, i.e. either consecutive NoOrder lines or a single
  ...
  def __nextIndependentChecks(self, checkLines):
    ...
    if head.variant == CheckLine.Variant.InOrder:
      ...
      assert head.variant == CheckLine.Variant.NoOrder
      independentChecks, checkLines = self.__splitByVariant(checkLines, CheckLine.Variant.NoOrder)
  ...
  def __matchNotLines(self, checkLines, outputLines, varState):
    for checkLine in checkLines:
      ...
      if varState is not None:
        raise Exception("NOT line " + str(checkLine) + " matches output")
...
class CheckFile(FileSplitMixin):
  ...
  def _processLine(self, line, lineNo):
    ...
    if dagLine is not None:
      return (dagLine, CheckLine.Variant.NoOrder), None
