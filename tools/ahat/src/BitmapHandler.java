...
class BitmapHandler implements HttpHandler {
  ...
  public void handle(HttpExchange exchange) throws IOException {
    try {
      ...
      BufferedImage bitmap = InstanceUtils.asBitmap(inst);
      if (bitmap == null) {
        noBitmapFound(exchange);
        return;
      }
   ...
   */
  private static void noBitmapFound(HttpExchange exchange) throws IOException {
    exchange.getResponseHeaders().add("Content-Type", "text/html");
    exchange.sendResponseHeaders(404, 0);
    PrintStream ps = new PrintStream(exchange.getResponseBody());
    ps.println("<!DOCTYPE html>");
    ps.println("<html>");
    ps.println("<head><title>ahat</title></head>");
    ps.println("<body>");
    ps.println("<p>No bitmap found for the given request.</p>");
    ps.println("</body>");
    ps.println("</html>");
    ps.close();
  }
