...
class FieldReader {
  ...
  static public Object readField(Object inst, String fieldName)
    ...
    if (count > 1) {
      throw new FieldReaderException(
          "Multiple fields found with name " + fieldName + " in " + inst);
    }
    return val;
  }
  ...
  static public void verifyIsClassInstance(Object obj, String className)
    ...
    if (cls == null) {
      throw new FieldReaderException(
          "Expected class " + className + ", but no class object was found");
    }

    if (!className.equals(cls.getClassName())) {
      throw new FieldReaderException(
          "Expected class " + className + ", but got " + cls.getClassName());
    }
  }
   ...
   */
  static public BufferedImage readBitmap(Instance inst) {
    int width = 0;
    int height = 0;
    byte[] buffer = null;
    try {
      verifyIsClassInstance(inst, "android.graphics.Bitmap");
      width = readIntField(inst, "mWidth");
      height = readIntField(inst, "mHeight");
      buffer = readByteArrayField(inst, "mBuffer");
