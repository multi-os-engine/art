...
class NativeAllocationsHandler implements AhatHandler {
  ...
  public void handle(Doc doc, Query query) throws IOException {
    List<AhatSnapshot.NativeAllocation> allocs = mSnapshot.getNativeAllocations();

    doc.title("Registered Native Allocations");

    doc.section("Overview");
    long totalSize = 0;
    for (AhatSnapshot.NativeAllocation alloc : allocs) {
    ...
    if (allocs.isEmpty()) {
      ...
      doc.table(
          ...
          new Column("Referent"));
      Comparator<AhatSnapshot.NativeAllocation> compare
        = new Sort.WithPriority<AhatSnapshot.NativeAllocation>(
            new Sort.NativeAllocationByHeapName(),
            new Sort.NativeAllocationBySize());
      Collections.sort(allocs, compare);
      SubsetSelector<AhatSnapshot.NativeAllocation> selector
        = new SubsetSelector(query, ALLOCATIONS_ID, allocs);
      for (AhatSnapshot.NativeAllocation alloc : selector.selected()) {
        doc.row(
      ...
      List<AhatSnapshot.NativeAllocation> remaining = selector.remaining();
      if (!remaining.isEmpty()) {
        long total = 0;
        for (AhatSnapshot.NativeAllocation alloc : remaining) {
          total += alloc.size;
        }
