...
class TestCheckLine_Parse(unittest.TestCase):
  ...
  def __parsesPattern(self, string, pattern):
    ...
    self.assertEqual(checker.RegexVariant.Pattern, line.lineParts[0].variant)
    self.assertEqual(pattern, line.lineParts[0].pattern)

  def __parsesVarRef(self, string, name):
    line = self.__tryParse(string)
    self.assertEqual(1, len(line.lineParts))
    self.assertEqual(checker.RegexVariant.VarRef, line.lineParts[0].variant)
    self.assertEqual(name, line.lineParts[0].name)

  def __parsesVarDef(self, string, name, body):
    line = self.__tryParse(string)
    self.assertEqual(1, len(line.lineParts))
    self.assertEqual(checker.RegexVariant.VarDef, line.lineParts[0].variant)
    self.assertEqual(name, line.lineParts[0].name)
  ...
  def test_Empty(self):
    self.__doesNotParse("{{}}", checker.RegexVariant.Pattern)
    self.__doesNotParse("[[]]", checker.RegexVariant.VarRef)
    self.__doesNotParse("[[:]]", checker.RegexVariant.VarDef)

  def test_InvalidVarName(self):
    self.__doesNotParse("[[0ABC]]", checker.RegexVariant.VarRef)
    self.__doesNotParse("[[AB=C]]", checker.RegexVariant.VarRef)
    self.__doesNotParse("[[ABC=]]", checker.RegexVariant.VarRef)
    self.__doesNotParse("[[0ABC:abc]]", checker.RegexVariant.VarDef)
    self.__doesNotParse("[[AB=C:abc]]", checker.RegexVariant.VarDef)
    self.__doesNotParse("[[ABC=:abc]]", checker.RegexVariant.VarDef)
