...
public class Device {
  ...
  private String getHostCoreImagePath(boolean addArch) {
    if (addArch) {
  ...
  private void setup() {
    ...
    if (Options.executeOnHost) {
      File coreImage = new File(getHostCoreImagePath(true));
      if (!coreImage.exists()) {
  ...
  public String getHostExecutionFlags() {
    return String.format("-Xnorelocate -Ximage:%s", getHostCoreImagePath(false));
  }
