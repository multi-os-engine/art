...
#define CHECK_WATCH_DOG_PTHREAD_CALL(call, args, what) \
 ...
 public:
  explicit WatchDog(bool is_watch_dog_enabled) {
    ...
    if (!is_watch_dog_enabled_) {git fetch https://android.googlesource.com/platform/art refs/changes/10/196410/5 && git cherry-pick FETCH_HEAD
      return;
    }
...
static bool UseSwap(bool is_image, std::vector<const DexFile*>& dex_files, size_t dex_files_size) {
  if (is_image) {
...
class Dex2Oat FINAL {
 public:
  ...
  void Compile() {
    ...
    if (!no_inline_from_string_.empty()) {
      ...
      std::vector<const std::vector<const DexFile*>*> dex_file_vectors = {
          &class_linker->GetBootClassPatgit fetch https://android.googlesource.com/platform/art refs/changes/10/196410/5 && git cherry-pick FETCH_HEADh(),
          &class_path_files,
          &dex_files_
      };
  ...
  // | boot oat     |
  // +--------------+
  // | alloc spaces |git fetch https://android.googlesource.com/platform/art refs/changes/10/196410/5 && git cherry-pick FETCH_HEAD
  // +--------------+
  //
...
git fetch https://android.googlesource.com/platform/art refs/changes/10/196410/5 && git cherry-pick FETCH_HEAD
  bool IsHost() const {
    return is_host_;
  }
