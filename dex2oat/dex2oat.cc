...
class Dex2Oat FINAL {
 public:
  ...
  static void ParseUintOption(const char* const raw_option,
    ...
    // Ensure `raw_option` starts with `option_prefix`.
    DCHECK_EQ(strstr(raw_option, option_prefix.c_str()), raw_option);
  ...
  void ParseBase(const char* const raw_option) {
    const char* const option_prefix = "--base=";
    // Ensure `raw_option` starts with `option_prefix`.
    DCHECK_EQ(strstr(raw_option, option_prefix), raw_option);
    const char* image_base_str = raw_option + strlen(option_prefix);
    char* end;
    image_base_ = strtoul(image_base_str, &end, 16);
    if (end == image_base_str || *end != '\0') {
      Usage("Failed to parse hexadecimal value for option %s", raw_option);
    }
  }

  void ParseInstructionSet(const char* const raw_option) {
    const char* const option_prefix = "--instruction-set=";
    // Ensure `raw_option` starts with `option_prefix`.
    DCHECK_EQ(strstr(raw_option, option_prefix), raw_option);
    StringPiece instruction_set_str = raw_option + strlen(option_prefix);
  ...
  void ParseInstructionSetVariant(const char* const raw_option, ParserOptions* parser_options) {
    const char* const option_prefix = "--instruction-set-variant=";
    // Ensure `raw_option` starts with `option_prefix`.
    DCHECK_EQ(strstr(raw_option, option_prefix), raw_option);
    StringPiece str = raw_option + strlen(option_prefix);
  ...
  void ParseInstructionSetFeatures(const char* const raw_option, ParserOptions* parser_options) {
    const char* const option_prefix = "--instruction-set-features=";
    // Ensure `raw_option` starts with `option_prefix`.
    DCHECK_EQ(strstr(raw_option, option_prefix), raw_option);
    StringPiece str = raw_option + strlen(option_prefix);
    if (instruction_set_features_.get() == nullptr) {
    ...
    if (instruction_set_features_.get() == nullptr) {
      Usage("Error parsing '%s': %s", raw_option, parser_options->error_msg.c_str());
    }
  }

  void ParseCompilerBackend(const char* const raw_option, ParserOptions* parser_options) {
    const char* const option_prefix = "--compiler-backend=";
    // Ensure `raw_option` starts with `option_prefix`.
    DCHECK_EQ(strstr(raw_option, option_prefix), raw_option);
    parser_options->requested_specific_compiler = true;
    StringPiece backend_str = raw_option + strlen(option_prefix);
  ...
  void ParseDisablePasses(const char* const raw_option, ParserOptions* parser_options) {
    const char* const option_prefix = "--disable-passes=";
    // Ensure `raw_option` starts with `option_prefix`.
    DCHECK_EQ(strstr(raw_option, option_prefix), raw_option);
    const std::string disable_passes = raw_option + strlen(option_prefix);
  ...
  void ParsePrintPasses(const char* const raw_option, ParserOptions* parser_options) {
    const char* const option_prefix = "--print-passes=";
    // Ensure `raw_option` starts with `option_prefix`.
    DCHECK_EQ(strstr(raw_option, option_prefix), raw_option);
    const std::string print_passes = raw_option + strlen(option_prefix);
  ...
  void ParseDumpCfgPasses(const char* const raw_option, ParserOptions* parser_options) {
    const char* const option_prefix = "--dump-cfg-passes=";
    // Ensure `raw_option` starts with `option_prefix`.
    DCHECK_EQ(strstr(raw_option, option_prefix), raw_option);
    const std::string dump_passes_string = raw_option + strlen(option_prefix);
  ...
  void ParsePassOptions(const char* const raw_option, ParserOptions* parser_options) {
    const char* const option_prefix = "--pass-options=";
    // Ensure `raw_option` starts with `option_prefix`.
    DCHECK_EQ(strstr(raw_option, option_prefix), raw_option);
    const std::string pass_options = raw_option + strlen(option_prefix);
  ...
  void ParseDumpInitFailures(const char* const raw_option) {
    const char* const option_prefix = "--dump-init-failures=";
    // Ensure `raw_option` starts with `option_prefix`.
    DCHECK_EQ(strstr(raw_option, option_prefix), raw_option);
    std::string file_name = raw_option + strlen(option_prefix);
