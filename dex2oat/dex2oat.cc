...
class Dex2Oat {
 public:
  ...
  const CompilerDriver* CreateOatFile(const std::string& boot_image_option,
    ...
    SafeMap<std::string, std::string> key_value_store_fallback;
    if (image_file_location.length() > 0) {
      if (key_value_store == nullptr) {
        key_value_store = &key_value_store_fallback;
      }
...
static int dex2oat(int argc, char** argv) {
  ...
  if (!has_explicit_checks_options &&
      ImplicitCheckOptions::CheckForCompiling(kRuntimeISA, instruction_set, &explicit_null_checks,
  ...
  key_value_store.Put("dex2oat-cmdline", oss.str());
  oss.str("");  // Reset.
  oss << kRuntimeISA;
  key_value_store.Put("dex2oat-host", oss.str());
