...
class Dex2Oat FINAL {
 public:
  ...
  ~Dex2Oat() {
    // Free up opened dex files and log completion time.
    // These must be done before deleting the runtime.
  ...
  bool Setup() {
    ...
    if (boot_image_option_.empty()) {
      ...
      runtime_options.push_back(std::make_pair(boot_class_path.c_str(), nullptr));
      std::string boot_class_path_locations = "-Xbootclasspathlocations:";
      boot_class_path_locations += Join(dex_locations_, ':');
      runtime_options.push_back(std::make_pair(boot_class_path_locations.c_str(), nullptr));
    } else {
    ...
    if (boot_image_option_.empty()) {
      ...
      if (dex_filenames_.empty()) {
        ...
        dex_files_ = DexFileUniquePtrsGet(opened_dex_files_);
        ATRACE_END();
      } else {
  ...
  void Compile() {
    ...
    if (!boot_image_option_.empty()) {
      ...
      OpenClassPathFiles(runtime_->GetClassPathString(), dex_files_,
          class_path_files_);
      ScopedObjectAccess soa(self);
      DexFilePtrs class_path_files(dex_files_);
 ...
 private:
  ...
  static void OpenClassPathFiles(const std::string& class_path,
                                 ...
                                 DexFileUniquePtrs& opened_dex_files) {
  ...
  DexFileUniquePtrs class_path_files_;
