...
class Dex2Oat FINAL {
 public:
  explicit Dex2Oat(TimingLogger* timings) :
      ...
      run_passes_zip_filename_(nullptr),
      run_passes_filename_(nullptr),
      app_image_(false),
      boot_image_(false),
      multi_image_(false),
      is_host_(false),
  ...
  void ParseArgs(int argc, char** argv) {
    ...
    for (int i = 0; i < argc; i++) {
      ...
      if (option.starts_with("--dex-file=")) {
        ...
        run_passes_filename_ = option.substr(strlen("--run-passes=")).data();
      } else if (option.starts_with("--run-passes-zip=")) {
        run_passes_zip_filename_ = option.substr(strlen("--run-passes-zip=")).data();
  ...
  bool Setup() {
    ...
    if (!PrepareImageClasses() || !PrepareCompiledClasses() || !PrepareCompiledMethods() || !PrepareRunPasses()) {
      return false;
    }
 ...
 private:
  ...
  bool PrepareCompiledMethods() {
    ...
    if (compiled_methods_filename_ != nullptr) {
      ...
      if (compiled_methods_zip_filename_ != nullptr) {
        compiled_methods_.reset(ReadUnorderedCommentedInputFromZip(
          compiled_methods_zip_filename_,
          compiled_methods_filename_,
          nullptr,            // No post-processing.
          &error_msg));
      } else {
        compiled_methods_.reset(ReadUnorderedCommentedInputFromFile(
          compiled_methods_filename_,
          nullptr));          // No post-processing.
      }
  ...
  bool PrepareRunPasses() {
    // If --run-passes was specified, read the passes to run from the given file(s).
    if (run_passes_filename_ != nullptr) {
      std::string error_msg;
      if (run_passes_zip_filename_ != nullptr) {
        run_passes_.reset(ReadOrderedCommentedInputFromZip(
          run_passes_zip_filename_,
          run_passes_filename_,
          nullptr,           // No post-processing.
          &error_msg));
      } else {
        run_passes_.reset(ReadOrderedCommentedInputFromFile(
          run_passes_filename_,
          nullptr));         // No post-processing.
      }
      if (run_passes_.get() == nullptr) {
        LOG(ERROR) << "Failed to create list of compiled methods from '"
            << run_passes_filename_ << "': " << error_msg;
        return false;
      }
    } else {
      run_passes_.reset(nullptr);  // By default run everything.
    }
    return true;
  }
  ...
  static std::unordered_set<std::string>* ReadImageClassesFromFile(
    ...
    return ReadUnorderedCommentedInputFromFile(image_classes_filename, &process);
  }
  ...
  static std::unordered_set<std::string>* ReadImageClassesFromZip(
        ...
        std::string* error_msg) {
    std::function<std::string(const char*)> process = DotToDescriptor;
    return ReadUnorderedCommentedInputFromZip(zip_filename,
                                              image_classes_filename,
                                              &process,
                                              error_msg);
  }

  static std::vector<std::string>* ReadOrderedCommentedInputFromZip(
      const char* zip_filename,
      const char* input_filename,
      std::function<std::string(const char*)>* process,
      std::string* error_msg) {
    return ReadCommentedInputFromZip<std::vector<std::string>>(zip_filename,
                                                               input_filename,
                                                               process,
                                                               error_msg);
  }

  static std::vector<std::string>* ReadOrderedCommentedInputFromFile(
      const char* input_filename, std::function<std::string(const char*)>* process) {
    return ReadCommentedInputFromFile<std::vector<std::string>>(input_filename, process);
  }

  static std::unordered_set<std::string>* ReadUnorderedCommentedInputFromZip(
      const char* zip_filename,
      const char* input_filename,
      std::function<std::string(const char*)>* process,
      std::string* error_msg) {
  ...
  static std::unordered_set<std::string>* ReadUnorderedCommentedInputFromFile(
      const char* input_filename, std::function<std::string(const char*)>* process) {
    return ReadCommentedInputFromFile<std::unordered_set<std::string>>(input_filename, process);
  }
  ...
  const char* run_passes_zip_filename_;
  const char* run_passes_filename_;
  ...
  std::unique_ptr<std::vector<std::string>> run_passes_;
  bool app_image_;
  bool boot_image_;
  bool multi_image_;
  bool is_host_;
