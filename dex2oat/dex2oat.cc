...
class Dex2Oat FINAL {
 public:
  ...
  void ProcessProfiles() {
    if (IsProfileGuidedCompilation()) {
      profile_compilation_info_.reset(
          ProfileAssistant::ProcessProfiles(profile_files_, reference_profile_files_));
    }
  }
  ...
  bool ShouldCompile() const {
    ...
    // information.
    return !IsProfileGuidedCompilation() || (profile_compilation_info_ != nullptr);
  }
...
static int dex2oat(int argc, char** argv) {
  ...
  // Process profile information and asses if we need to do a profile guided compilation.
  // This operation involves I/O.
  dex2oat.ProcessProfiles();
  if (!dex2oat.ShouldCompile()) {
