...
class Dex2Oat {
 public:
  ...
  void Compile(const std::string& boot_option_image,
  ...
  bool CreateOatFile(const std::vector<const DexFile*>& dex_files,
    ...
    if (!driver_->IsImage() && driver_->GetCompilerOptions().GetIncludePatchInformation()) {
      t2.NewTiming("Patching ELF");
      std::string error_msg;
      if (!PatchOatCode(driver_.get(), oat_file, oat_location, &error_msg)) {
...
static int dex2oat(int argc, char** argv) {
  ...
  // TODO: This description is obsolete! Update!
  //
