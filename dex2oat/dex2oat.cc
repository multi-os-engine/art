...
class Dex2Oat FINAL {
 public:
  ...
  void Compile(CumulativeLogger* compiler_phases_timings,
  ...
  bool CreateOatFile(SafeMap<std::string, std::string>* key_value_store) {
    CHECK(key_value_store != nullptr);
    ...
    OatWriter oat_writer(dex_files_, image_file_location_oat_checksum,
                         image_file_location_oat_data_begin,
                         image_patch_delta,
                         driver_.get(),
                         image_writer_.get(),
    ...
    if (!driver_->WriteElf(android_root_, is_host_, dex_files_, &oat_writer, oat_file_.get())) {
 ...
 private:
  ...
  bool CreateImageFile()
    ...
    if (!image_writer_->Write(image_filename_, oat_filename_, oat_location_)) {
    ...
    std::unique_ptr<File> oat_file(OS::OpenFileReadWrite(oat_filename_.c_str()));
    if (oat_file.get() == nullptr) {
      PLOG(ERROR) << "Failed to open ELF file: " << oat_filename_;
      return false;
    }
...
static int dex2oat(int argc, char** argv) {
  ...
  // Insert some compiler things.
  {
    ...
    key_value_store->Put(OatHeader::kPicKey,
                         dex2oat.GetCompilerOptions()->GetCompilePic() ? "true" : "false");
  }

  // Let the compiler do its job.
  dex2oat.Compile(&compiler_phases_timings, key_value_store.get());

  // Write out the oat file.
  if (!dex2oat.CreateOatFile(key_value_store.get())) {
    return EXIT_FAILURE;
  }
