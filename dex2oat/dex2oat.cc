...
static void Usage(const char* fmt, ...) {
  ...
  UsageError("");
  UsageError("  --generate-dex-map: enable dex code map generation");
  UsageError("  --no-generate-dex-map: disable dex code map generation");
  UsageError("");
...
static int dex2oat(int argc, char** argv) {
  ...
  for (int i = 0; i < argc; i++) {
    ...
    } else if (option == "--generate-dex-map") {
      generate_dex_map = true;
    } else if (option == "--no-generate-dex-map") {
