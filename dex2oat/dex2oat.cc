...
static void Usage(const char* fmt, ...) {
...
#endif
  UsageError("");
  UsageError("  --huge-method-max=<method-instruction-count>: threshhold size for a huge");
  UsageError("   method.");
  ...
  UsageError("      Default: %d", CompilerOptions::kDefaultHugeMethodThreshold);
  UsageError("");
  UsageError("  --large-method-max=<method-instruction-count>: threshhold size for a large");
  UsageError("   method.");
  ...
  UsageError("");
  UsageError("  --small-method-max=<method-instruction-count>: threshhold size for a small");
  UsageError("   method.");
  ...
  UsageError("");
  UsageError("  --tiny-method-max=<method-instruction-count>: threshhold size for a tiny");
  UsageError("   method.");
  ...
  UsageError("");
  UsageError("  --num-dex-methods=<method-count>: threshhold size for a small dex file");
...
class Dex2Oat {
 public:
  static bool Create(Dex2Oat** p_dex2oat,
    ...
    UniquePtr<Dex2Oat> dex2oat(new Dex2Oat(compiler_options, compiler_backend, instruction_set,
 ...
 private:
  ...
  explicit Dex2Oat(const CompilerOptions& compiler_options,
      ...
      : compiler_options_(compiler_options),
        ...
        verification_results_(new VerificationResults(compiler_options)),
  ...
  bool CreateRuntime(Runtime::Options& runtime_options, InstructionSet instruction_set)
      SHARED_TRYLOCK_FUNCTION(true, Locks::mutator_lock_) {
...
static int dex2oat(int argc, char** argv) {
  ...
  int huge_method_threshold = -1;
  int large_method_threshold = -1;
  int small_method_threshold = -1;
  int tiny_method_threshold = -1;
  int num_dex_methods_threshold = -1;
  ...
  if (strcmp(compiler_filter_string, "interpret-only") == 0) {
    compiler_options.compiler_filter_ = CompilerOptions::kInterpretOnly;
  } else if (strcmp(compiler_filter_string, "space") == 0) {
    compiler_options.compiler_filter_ = CompilerOptions::kSpace;
  } else if (strcmp(compiler_filter_string, "balanced") == 0) {
    compiler_options.compiler_filter_ = CompilerOptions::kBalanced;
  } else if (strcmp(compiler_filter_string, "speed") == 0) {
    compiler_options.compiler_filter_ = CompilerOptions::kSpeed;
  } else if (strcmp(compiler_filter_string, "everything") == 0) {
    compiler_options.compiler_filter_ = CompilerOptions::kEverything;
  } else {
    Usage("Unknown --compiler-filter value %s", compiler_filter_string);
  }

  if (huge_method_threshold != -1) {
    compiler_options.huge_method_threshold_ = huge_method_threshold;
  }
  if (large_method_threshold != -1) {
    compiler_options.large_method_threshold_ = large_method_threshold;
  }
  if (small_method_threshold != -1) {
    compiler_options.small_method_threshold_ = small_method_threshold;
  }
  if (tiny_method_threshold != -1) {
    compiler_options.tiny_method_threshold_ = tiny_method_threshold;
  }
  if (num_dex_methods_threshold != -1) {
    compiler_options.num_dex_methods_threshold_ = num_dex_methods_threshold;
  }
  ...
  Dex2Oat* p_dex2oat;
  if (!Dex2Oat::Create(&p_dex2oat, runtime_options, compiler_options, compiler_backend,
                       instruction_set, instruction_set_features, thread_count)) {
   ...
   */
  if (!image && (compiler_options.compiler_filter_ != CompilerOptions::kInterpretOnly)) {
    ...
    if (num_methods <= compiler_options.num_dex_methods_threshold_) {
      compiler_options.compiler_filter_ = CompilerOptions::kSpeed;
