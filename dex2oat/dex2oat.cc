 ...
 public:
  ...
  bool Setup() {
    ...
    // Now that we have finalized key_value_store_, start writing the oat file.
    {
      ...
      if (!oat_writer_->WriteDexFiles(rodata_,
                                      oat_file_.get(),
                                      instruction_set_,
    ...
    // TODO: Grab dex files from OatWriter instead of opening them again.
    // For boot image, pass them to the Runtime::Create() through a new option.
    {
    ...
    // TODO: When we use the dex files opened by OatWriter, we don't need to do this.
 ...
 private:
  ...
  bool AddDexFileSources() {
    ...
    if (boot_image_filename_.empty() && dex_filenames_.empty()) {
      // TODO: When we use the dex files opened by OatWriter, we don't need to dup(zip_fd_).
      ScopedFd zip_fd(dup(zip_fd_));
      if (zip_fd.get() == -1) {
        PLOG(ERROR) << "Failed to dup(zip_fd_).";
        return false;
      }
      if (!oat_writer_->AddZippedDexFilesSource(std::move(zip_fd), zip_location_.c_str())) {
        return false;
      }
    } else {
