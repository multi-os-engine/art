...
class Dex2Oat FINAL {
 public:
  ...
  void ParseZipFd(const StringPiece& option) {
    DCHECK(option.starts_with("--zip-fd="));
  ...
  void ParseOatFd(const StringPiece& option) {
    DCHECK(option.starts_with("--oat-fd="));
  ...
  void ParseSwapFd(const StringPiece& option) {
    DCHECK(option.starts_with("--swap-fd="));
