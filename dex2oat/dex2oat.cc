  ...
  bool ReadPassesFile() {
    ...
    if (passes_to_run_filename_ != nullptr) {
      ...
      if (passes_to_run_.get() == nullptr) {
        LOG(ERROR) << "Failed to create list of run passes from '" << passes_to_run_filename_;
        return false;
      }
    } else {
      passes_to_run_.reset(nullptr);  // By default run everything.
    }
    return true;
  }
