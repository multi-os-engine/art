...
public class Main {
    ...
    private static void testUnloadClass(Constructor constructor) throws Exception {
        WeakReference<Class> klass = new WeakReference<Class>(setUpUnloadClass(constructor));
        // No strong references to class loader, should get unloaded.
        Runtime.getRuntime().gc();
        WeakReference<Class> klass2 = new WeakReference<Class>(setUpUnloadClass(constructor));
        Runtime.getRuntime().gc();
    ...
    private static Object testNoUnloadHelper(WeakReference<ClassLoader> weak_loader)
            throws Exception {
        ClassLoader loader = weak_loader.get();
        if (loader == null) {
            return null;
        }
    ...
    private static void testNoUnloadInstance(Constructor constructor) throws Exception {
        ...
        Runtime.getRuntime().gc();
        // If loaded was unloded too early due to races, just pass the test.
        boolean isNull = loader.get() == null && o != null;
