...
//        }
    ...
    private boolean displayJank = false;
    
    public void setDisplayJank(boolean value)
    {
    	displayJank = value;
    }
...
//        setProjection(projection_); is called in onSurfaceChanged()
        ...
        if (ccConfig.CC_DIRECTOR_FAST_FPS) {
            if (FPSLabel_==null) {
                ...
                FPSLabel_ = CCLabelAtlas.label("00.0", "fps.png", 18, 24, '.');
            ...
            if(RockLabel_ == null)
            {            	
            	RockLabel_ = CCLabelAtlas.label(",", "fps4.png", 18, 24, '.');
            	RockLabel_.setPosition(50, 20);
            }*/
...
//        }
    ...
    public void drawCCScene (GL10 gl) {
        ...
        //update time should be set after tick();
...
//    }
    ...
    private void showFPS(GL10 gl) {
        ...
        if (FAST_FPS_DISPLAY) {
            ...
            {            	
				...
				transitionTime+=dt;
				if(transitionTime>maxTransition)
				{
					totalFrames++;
					totalTime += dt;
					totalUpdateTime += updateTime;
					updateTime = 0;
					//Jank means dt2 - dt1 > vsync_time/2
	            	if(lastDt>0 && dt-lastDt>1.0f/120)
	            	{
	            		janks++;
	            	}
				}								
				...
				if (accumDt_ > ccConfig.CC_DIRECTOR_FPS_INTERVAL) {
					// Log.d("cocos2d",
					// "frames_ "+frames_+" accumDt "+accumDt_+" totalFrames "+totalFrames+" totalTime "+totalTime);
					...
					fpsBuilder.append(",AFPS=");
					
					if(transitionTime>maxTransition)
					{
						averageFrameRate = totalFrames / totalTime;
						fpsInt = (int) averageFrameRate;
						fpsFract = (int) ((averageFrameRate - fpsInt) * 10);
					...
					fpsBuilder.append(",AUPS=");
					if(transitionTime>maxTransition && totalFrames>0)
					{						
						float averageUpdateTime = totalUpdateTime/totalFrames*1000;
						fpsInt = (int) averageUpdateTime;
						fpsFract = (int) ((averageUpdateTime - fpsInt) * 10);
						fpsBuilder.append(fpsInt);
						fpsBuilder.append('.');
						fpsBuilder.append(fpsFract);
					...
					if(displayJank)
					{
						fpsBuilder.append(",JPS=");
						if(totalTime>0)
						{
						float jps = janks/(float)totalTime;
						
						//fpsBuilder.append(jank, 4, false, true);
						
						fpsInt = (int) jps;
						int fpsFract1 = (int) ((jps - fpsInt) * 10);
						int fpsFract2 = (int) ((jps - fpsInt-fpsFract1/10.f) * 100);
						int fpsFract3 = (int) ((jps - fpsInt-fpsFract1/10.f-fpsFract2/100.f) * 1000);
						
						fpsBuilder.append(fpsInt);
						fpsBuilder.append('.');
						fpsBuilder.append(fpsFract1);
						fpsBuilder.append(fpsFract2);
						fpsBuilder.append(fpsFract3);
						}else
						{
						fpsBuilder.append(",,,,");
						}
					...
					fpsInt = (int) frameRate_;
					fpsFract = (int) ((frameRate_ - fpsInt) * 10);
					...
					fpsBuilder.append(";TIME=");
					fpsInt = (int) totalTime;
					fpsFract = (int) ((totalTime - fpsInt) * 10);
					fpsBuilder.append(fpsInt);
					fpsBuilder.append('.');
					fpsBuilder.append(fpsFract);
					
					/*
					fpsBuilder.append(";OBJ=");
					fpsBuilder.append(objNumber);
					*/
					
					fpsBuilder.append(";SNOW=");
					fpsBuilder.append(nSnows);
					
					if(RockLabel_==null)
					{
					    fpsBuilder.append(";ROCK=");
					    fpsBuilder.append(nRocks);
					}					

					// averageFPSLabel_.setString(fpsBuilder);
					FPSLabel_.setString(fpsBuilder);				
					
					if(RockLabel_!=null)
					{
					int nspace = fpsBuilder.length();
					fpsBuilder.reset();
					...
					fpsBuilder.append("ROCK=");
					fpsBuilder.append(nRocks);
					RockLabel_.setString(fpsBuilder);
					}
				}
            }
            if(RockLabel_!=null)
	...
	public void setnRocks(int nRocks) {
		this.nRocks = nRocks;
	}
	...
	public void setnSnows(int nSnows) {
		this.nSnows = nSnows;
	}
	
	public int getnSnows()
	{
		return nSnows;
	}	
	...
	{
		if(totalTime>0)
			return janks/totalTime;
		else
			return 0;
	}
	
	public void setmaxTransition(float t)
	{
		maxTransition = t; 
	}
	...
	public double getAUTPF() {
		// TODO Auto-generated method stub
		if(totalFrames>0)
		{
			return totalUpdateTime/totalFrames;
		}else
			return 0;
	}

	public double getAAPS() {
		// TODO Auto-generated method stub
		if(totalFrames>0)
		{
			return (nRocks+nSnows)/getAUTPF();
		}else
			return 0;
	}
}
