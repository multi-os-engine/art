...
class TestClass3 {
  ...
  TestClass3() {
    this.i = 5;
    System.out.print("Don't inline".substring(0, 0));
  }
}
...
public class Main {
  ...
  /// CHECK-START: int Main.test19() load_store_elimination (before)
  /// CHECK: InstanceFieldSet
  /// CHECK: InvokeVirtual
  /// CHECK: InstanceFieldGet

  /// CHECK-START: int Main.test19() load_store_elimination (after)
  /// CHECK: InstanceFieldSet
  /// CHECK: InvokeVirtual
  /// CHECK-NOT: InstanceFieldGet

  // Final field's value shouldn't be killed by invocation.
  static int test19() {
    TestClass3 obj = new TestClass3(4);
    System.out.print(obj.toString().substring(0, 0));
    return obj.i;
  }

  /// CHECK-START: int Main.test20() load_store_elimination (before)
  /// CHECK: NewInstance
  /// CHECK: InvokeStaticOrDirect
  /// CHECK: InstanceFieldGet

  /// CHECK-START: int Main.test20() load_store_elimination (after)
  /// CHECK: NewInstance
  /// CHECK: InvokeStaticOrDirect
  /// CHECK: InstanceFieldGet

  // Final field's value should be killed by invocation if it's not
  // set yet.
  static int test20() {
  ...
  /// CHECK-START: float Main.test21(float[], float[]) load_store_elimination (before)
  /// CHECK: <<IntTypeValue:i\d+>> ArrayGet
  /// CHECK: ArraySet
  /// CHECK: <<FloatTypeValue:f\d+>> ArrayGet

  /// CHECK-START: float Main.test21(float[], float[]) load_store_elimination (after)
  /// CHECK: <<IntTypeValue:i\d+>> ArrayGet
  /// CHECK: ArraySet
  /// CHECK: <<FloatTypeValue:f\d+>> ArrayGet

  // I/F, J/D aliasing should keep the load/store.
  static float test21(float[] fa1, float[] fa2) {
    fa1[0] = fa2[0];
    return fa1[0];
  }
  ...
  public static void main(String[] args) {
    ...
    assertIntEquals(test19(), 4);
    assertIntEquals(test20(), 5);
    float[] fa1 = new float[1];
    float[] fa2 = new float[1];
    fa1[0] = 0.8f;
    fa2[0] = 1.8f;
    assertFloatEquals(test21(fa1, fa2), 1.8f);
  }
}
