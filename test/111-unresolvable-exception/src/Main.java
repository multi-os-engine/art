...
// Use JAXB, as it is part of the Java API and available for compilation, but not available
// on Android.
...
public class Main {
    ...
    private static void check(boolean b) {
      try {
        if (b) {          // Need this to not be dead code, but also not be invoked.
          throwsJaxb();   // JAXBException is checked, so we need something potentially throwing it.
        }
        throw new RuntimeException();  // Trigger exception handling.
      } catch (JAXBException e) {      // This handler will have an unresolvable class.
    ...
    private static native void throwsJaxb() throws JAXBException;
}
