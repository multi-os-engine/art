...
.PHONY: $(3)
$(3): $$(ART_OAT_TEST_$(1)_DEX) $(ART_TEST_OUT)/oat-test-dex-$(1).jar test-art-target-sync
	$(hide) mkdir -p $(ART_HOST_TEST_DIR)/android-data-$$@
	$(hide) echo Running: $$@
	$(hide) adb shell touch $(ART_TEST_DIR)/$(TARGET_$(2)ARCH)/$$@-$(LOCAL_PID)
	$(hide) adb shell rm $(ART_TEST_DIR)/$(TARGET_$(2)ARCH)/$$@-$(LOCAL_PID)
	$(hide) ulimit 2048; $$(call ART_TEST_SKIP,$$@) && \
	  adb shell sh -c "/system/bin/dalvikvm$($(2)ART_PHONY_TEST_TARGET_SUFFIX) \
	    ...
	    -Djava.library.path=$(ART_TEST_DIR)/$(TARGET_$(2)ARCH) $(1) \
	      && touch $(ART_TEST_DIR)/$(TARGET_$(2)ARCH)/$$@-$(LOCAL_PID)" \
	        > $(ART_TEST_DIR)/android-data-$$@/output.txt 2>&1 && \
	  (adb pull $(ART_TEST_DIR)/$(TARGET_$(2)ARCH)/$$@-$(LOCAL_PID) $(ART_HOST_TEST_DIR)/android-data-$$@ \
...
.PHONY: $(3)
$(3): $$(ART_OAT_TEST_$(1)_DEX) $(ART_TEST_HOST_DEPENDENCIES)
