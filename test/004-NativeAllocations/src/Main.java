...
public class Main {
    ...
    private static void testAffectsTotalMemory() throws Exception {
        ...
        if (Math.abs(change - expected) > 0.2 * expected) {
            System.out.println(String.format(
                  "Expected totalMemory increased by around %,d, but found an increase of %,d",
                  expected, change));
        }
        for (int i = 0; i < 4; i++) {
    ...
    private static void testLeadsToOutOfMemory() throws Exception {
        System.gc();
        long start = Runtime.getRuntime().totalMemory();
        int size = (int)Math.min((maxMem - start) / 32, Integer.MAX_VALUE / 4);
    ...
    private static void testNoStealingMemory() throws Exception {
        // At the very least, we shouldn't be able to register a native free
        // greater than the amount of native memory registered as allocated.
        // Here we assume we have not registered more than Integer.MAX_VALUE
        // bytes or more than 20% of the total memory with native allocations.
        long start = Runtime.getRuntime().totalMemory();
        int size = Integer.MAX_VALUE;
        try {
          register_native_free.invoke(runtime, size);
          System.out.println("Expected exception, but none thrown.");
        } catch (Exception ignored) {
        }
        long change = Runtime.getRuntime().totalMemory() - start;
        if (Math.abs(change) > 0.2 * start) {
            System.out.println(String.format(
                  "Expected no change in total memory, but found a change of %,d",
                  change));
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        register_native_allocation = vm_runtime.getDeclaredMethod(
            "registerNativeAllocation", Integer.TYPE);
        register_native_free = vm_runtime.getDeclaredMethod("registerNativeFree", Integer.TYPE);
        maxMem = Runtime.getRuntime().maxMemory();

        testAffectsTotalMemory();
        testLeadsToOutOfMemory();
        testNoStealingMemory();
