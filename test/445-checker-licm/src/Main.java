...
public class Main {
  ...
  public static int divAndIntrinsic(int[] array) {
    ...
    for (int i = 0; i < array.length; i++) {
      ...
      // As a result, the division can be moved out of the loop.
  ...
  public static int invariantBoundIntrinsic(int x) {
    int result = 0;
    // The intrinsic call used as loop bound is invariant,
    // and can be moved out of the loop header.
  ...
  public static int invariantBodyIntrinsic(int x, int y) {
    int result = 0;
    for (int i = 0; i < 10; i++) {
      // The intrinsic call used inside the loop body is invariant,
      // and can be moved completely out of the loop.
      result += Math.max(x, y);
    }
    return result;
  }
  ...
  public static void main(String[] args) {
    ...
    assertEquals(0, invariantBodyIntrinsic(2, 3));
  }
}
