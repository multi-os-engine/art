...
# Allow us to import the utils.
sys.path.append(str(Path(BUILD_TOP)/"art"/"test"/"utils"/"python"))

from utils import get_copyright
...
class InstanceTest:
  """A method that runs tests for a particular concrete type"""
...
def flatten_interfaces(dat, c):
  """Iterate over all the interfaces 'c' implements"""
  def get_ifaces(cl):
...
def main(argv):
  ...
  if not smali_dir.exists() or not smali_dir.is_dir():
    ...
    sys.exit(1)
  print(make_main_class(parse_xml((smali_dir / "classes.xml").open().read())).dump(),
        file=(smali_dir/"Main.smali").open('w'))
