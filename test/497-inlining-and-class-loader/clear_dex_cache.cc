...
extern "C" JNIEXPORT jobject JNICALL Java_Main_cloneResolvedMethods(JNIEnv* env,
  ...
  for (size_t i = 0; i != num_methods; ++i) {
    ArtMethod* method = mirror::DexCache::GetElementPtrSize(methods, i, kPointerSizeCur);
    pointer_array->SetElementPtrSize(i, method, kPointerSizeCur);
  }
  return array;
}
...
extern "C" JNIEXPORT void JNICALL Java_Main_restoreResolvedMethods(
  ...
  for (size_t i = 0; i != num_methods; ++i) {
    ArtMethod* method = old->GetElementPtrSize<ArtMethod*>(i, kPointerSizeCur);
    mirror::DexCache::SetElementPtrSize(methods, i, method, kPointerSizeCur);
  }
}
