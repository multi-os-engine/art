...
extern "C" JNIEXPORT jobject JNICALL Java_Main_cloneResolvedMethods(JNIEnv*, jclass, jclass cls) {
  ...
  return soa.Vm()->AddGlobalRef(soa.Self(), soa.Decode<mirror::Class*>(cls)->GetDexCache()->GetResolvedMethods()->Clone(soa.Self()));
}

extern "C" JNIEXPORT void JNICALL Java_Main_restoreResolvedMethods(JNIEnv*, jclass, jclass cls, jobject old_cache) {
  ScopedObjectAccess soa(Thread::Current());
