...
static void* testReflectFieldGetOnAttachedNativeThread(void*) {
  assert(jvm != NULL);
  ...
  jstring field_name = env->NewStringUTF("testReflectFieldGetOnAttachedNativeThreadField");
...
// http://b/15539150
extern "C" JNIEXPORT void JNICALL Java_JniTest_testReflectFieldGetOnAttachedNativeThreadNative(
    JNIEnv*, jclass) {
  pthread_t pthread;
  int pthread_create_result = pthread_create(&pthread,
                                             NULL,
                                             testReflectFieldGetOnAttachedNativeThread,
                                             NULL);
  assert(pthread_create_result == 0);
  int pthread_join_result = pthread_join(pthread, NULL);
  assert(pthread_join_result == 0);
