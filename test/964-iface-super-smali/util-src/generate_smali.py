...
TestData = namedtuple("TestData", ['classes', 'interfaces'])
Clazz = namedtuple("Clazz", ["name","methods","super_class","implements"])
IFace = namedtuple("IFace", ["name","methods","super_class","implements"])
...
def ParseXML(xml):
  ...
  for iface in root.find("interfaces"):
    ...
    implements = list(map(lambda a: a.text, iface.find("implements")))
    methods = list(map(lambda a: a.text, iface.find("methods")))
    ifaces[name] = IFace(name=name, super_class=iface.attrib['super'], methods=methods, implements=implements)
  for clazz in root.find('classes'):
    name = clazz.attrib['name']
    implements = list(map(lambda a: a.text, clazz.find("implements")))
    methods = list(map(lambda a: a.text, clazz.find("methods")))
    classes[name] = Clazz(name=name, super_class=clazz.attrib['super'], methods=methods, implements=implements)
...
def main(argv):
  ...
  if not smali_dir.exists() or not smali_dir.is_dir():
    ...
    sys.exit(1)
  print(MakeMainClass(ParseXML((smali_dir / "classes.xml").open().read())).dump(), file=(smali_dir/"Main.smali").open('w'))
