...
.PHONY: test-art-target-oat-$(1)$($(2)ART_MK_SUFFIX)
test-art-target-oat-$(1)$($(2)ART_MK_SUFFIX): $($(2)ART_TEST_OUT)/oat-test-dex-$(1).jar test-art-target-sync
	adb shell touch $($(2)ART_TEST_DIR)/test-art-target-oat-$(1)
	adb shell rm $($(2)ART_TEST_DIR)/test-art-target-oat-$(1)
	adb shell sh -c "/system/bin/dalvikvm $(DALVIKVM_FLAGS) -XXlib:libartd.so -Ximage:$($(2)ART_TEST_DIR)/core.art -classpath $($(2)ART_TEST_DIR)/oat-test-dex-$(1).jar -Djava.library.path=$($(2)ART_TEST_DIR) $(1) && touch $($(2)ART_TEST_DIR)/test-art-target-oat-$(1)"
	$(hide) (adb pull $($(2)ART_TEST_DIR)/test-art-target-oat-$(1) /tmp/ && echo test-art-target-oat-$(1)$($(2)ART_MK_SUFFIX) PASSED) || (echo test-art-target-oat-$(1)$($(2)ART_MK_SUFFIX) FAILED && exit 1)
...
define declare-test-art-oat-targets
  ifdef TARGET_2ND_ARCH
    ...
    ifneq ($(ART_MK_SUFFIX),)
      test-art-target-oat-$(1): test-art-target-oat-$(1)$($(2)ART_MK_SUFFIX)
    endif
  endif
