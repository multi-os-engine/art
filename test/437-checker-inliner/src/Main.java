...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    if (returnParameter(42) != 42) {
      throw new Error();
    }

    if (returnWideParameter(0x100000001L) != 0x100000001L) {
      throw new Error();
    }

    if (returnReferenceParameter(Main.class) != Main.class) {
      throw new Error();
    }

    returnVoid();
    returnVoidWithOneParameter(32);

    if (returnAdd(42, 1) != 43) {
      throw new Error();
    }

    if (returnSub(42, 1) != 41) {
      throw new Error();
    }
