...
extern "C" JNIEXPORT jboolean JNICALL Java_Main_checkAppImageContains(JNIEnv*, jclass, jclass c) {
  ScopedObjectAccess soa(Thread::Current());
  uintptr_t klass_ptr = reinterpret_cast<uintptr_t>(soa.Decode<mirror::Class*>(c));
  for (auto* space : Runtime::Current()->GetHeap()->GetContinuousSpaces()) {
    if (space->IsImageSpace()) {
      auto* image_space = space->AsImageSpace();
      const auto& image_header = image_space->GetImageHeader();
      if (image_header.IsAppImage()) {
        if (reinterpret_cast<uintptr_t>(image_space->Begin()) <= klass_ptr &&
            reinterpret_cast<uintptr_t>(image_space->GetImageEnd()) > klass_ptr) {
          return JNI_TRUE;
        }
      }
    }
