...
public class Main {
  ...
  static final int base1 = 1;
  static final int base2 = 0x70000000;
  static void narrow(int array[], int offset) {
    ...
    int biased_offset1 = offset + base1;
    if (biased_offset1 < array.length) {
      array[biased_offset1] = 1;
    }

    int biased_offset2 = offset + base2;
    if (biased_offset2 < array.length) {
      // biased_offset2 may overflow and be negative so we can't
