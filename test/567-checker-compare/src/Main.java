...
public class Main {
  ...
  /// CHECK-START: void Main.testReplaceInputWithItself(int) intrinsics_recognition (after)
  ...
  /// CHECK-START: void Main.testReplaceInputWithItself(int) instruction_simplifier (after)
  /// CHECK-DAG:     <<ArgX:i\d+>>   ParameterValue
  /// CHECK-DAG:     <<Zero:i\d+>>   IntConstant 0
  /// CHECK-DAG:                     GreaterThanOrEqual [<<ArgX>>,<<Zero>>]

  public static void testReplaceInputWithItself(int x) {
    ...
    // test that it is allowed to replace the second input of the GreaterThanOrEqual,
    // i.e. <<Zero>>, with the same instruction.
