...
public class Main {
  ...
  /// CHECK-START: int Main.round32(float) instruction_simplifier (before)
  /// CHECK-DAG: <<Result:i\d+>> InvokeStaticOrDirect intrinsic:FloatFloatToIntBits
  /// CHECK-DAG: Return [<<Result>>]
  //
  /// CHECK-START: int Main.round32(float) instruction_simplifier (after)
  ...
  /// CHECK-START: long Main.round64(double) instruction_simplifier (before)
  /// CHECK-DAG: <<Result:j\d+>> InvokeStaticOrDirect intrinsic:DoubleDoubleToLongBits
  /// CHECK-DAG: Return [<<Result>>]
  //
  /// CHECK-START: long Main.round64(double) instruction_simplifier (after)
  // Note: The ArtMethod* (typed as int or long) is optional after sharpening.
