...
extern "C" JNIEXPORT jboolean JNICALL Java_Main_throwDeoptException(JNIEnv*, jclass) {
  Thread* self = Thread::Current();
  ScopedObjectAccess soa(self);
  // Before deoptimizing to interpreter, we must push the deoptimization context.
  JValue return_value;
  return_value.SetJ(0);
  self->PushDeoptimizationContext(return_value, false, /* from_code */ false, nullptr);
