...
class JniCallDefaultMethodsTest {
 public:
  explicit JniCallDefaultMethodsTest(JNIEnv* env)
      : env_(env), concrete_class_((jclass)env_->NewGlobalRef(env_->FindClass("ConcreteClass"))) {
    assert(!env_->ExceptionCheck());
 ...
 private:
  void TestCalls(const char* declaring_class, std::vector<const char*> methods) {
    ...
    jobject obj = env_->NewGlobalRef(env_->NewObject(concrete_class_, new_method));
    assert(!env_->ExceptionCheck());
    assert(obj != nullptr);
    jclass decl_class = (jclass) env_->NewGlobalRef(env_->FindClass(declaring_class));
    assert(!env_->ExceptionCheck());
    assert(decl_class != nullptr);
    for (const char* method : methods) {
      // printf("finding method %s->%s", declaring_class, method);
    ...
    env_->DeleteGlobalRef(decl_class);
