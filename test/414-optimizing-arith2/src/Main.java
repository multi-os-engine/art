...
public class Main {
  ...
  private static void negLong() {
    expectEquals(-1L, $opt$Neg(1L));
    expectEquals(1L, $opt$Neg(-1L));
    expectEquals(0L, $opt$Neg(0L));
    expectEquals(51L, $opt$Neg(-51L));
    expectEquals(-51L, $opt$Neg(51L));
    expectEquals(9223372036854775807L, $opt$Neg(-9223372036854775807L));  // (2^63 - 1)
    expectEquals(-9223372036854775807L, $opt$Neg(9223372036854775807L));  // -(2^63 - 1)
    // See comment in negInt().
    expectEquals(-9223372036854775808L, $opt$Neg(-9223372036854775808L)); // -(2^63)
  }
