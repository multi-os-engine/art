...
"""
Generate Smali test files for test 973.
"""
...
MAX_IFACE_DEPTH = 2
# The number of dex files to create
NUM_DEX_FILES = 3
...
"""
  ...
  def get_dex_file(self):
    """The dex file to put this class in."""
    # Always put in dex file 1 since this is the main class
    return 1
...
class InterfaceCallResponse(Enum):
  ...
  def get_output_format(self):
    """Gets the format string for an interface call finishing in each of the various states."""
    dexs = " Dexfiles {my_dex_file} -> {called_dex_file}\n"
    if self == InterfaceCallResponse.NoError:
      return "No exception thrown for {iface_name}.super.call() on {tree}." + dexs
    elif self == InterfaceCallResponse.AbstractMethodError:
      return "AbstractMethodError thrown for {iface_name}.super.call() on {tree}." + dexs
    elif self == InterfaceCallResponse.NoSuchMethodError:
      return "NoSuchMethodError thrown for {iface_name}.super.call() on {tree}." + dexs
    else:
      return "IncompatibleClassChangeError thrown for {iface_name}.super.call() on {tree}." + dexs
...
"""
  SUPER_CALL_TEMPLATE = """
#     try {{
#       System.out.println("Calling {iface_name}.super.call() on {tree}. Dexfiles {my_dex_file} -> {called_dex_file}");
#       {iface_name}.super.call();
#       System.out.println("No exception thrown for {iface_name}.super.call() on {tree}. Dexfiles {my_dex_file} -> {called_dex_file}");
#     }} catch (AbstractMethodError ame) {{
#       System.out.println("AbstractMethodError thrown for {iface_name}.super.call() on {tree}. Dexfiles {my_dex_file} -> {called_dex_file}");
#     }} catch (NoSuchMethodError nsme) {{
#       System.out.println("NoSuchMethodError thrown for {iface_name}.super.call() on {tree}. Dexfiles {my_dex_file} -> {called_dex_file}");
#     }} catch (IncompatibleClassChangeError icce) {{
#       System.out.println("IncompatibleClassChangeError thrown for {iface_name}.super.call() on {tree}. Dexfiles {my_dex_file} -> {called_dex_file}");
#     }} catch (Throwable t) {{
#       System.out.println("Unknown error thrown for {iface_name}.super.call() on {tree}. Dexfiles {my_dex_file} -> {called_dex_file}");
#       throw t;
#     }}
    :call_{class_name}_{iface_name}_try_start
      const-string v1, "Calling {iface_name}.super.call() on {tree}. Dexfiles {my_dex_file} -> {called_dex_file}"
      invoke-virtual {{v0, v1}}, Ljava/io/PrintStream;->println(Ljava/lang/Object;)V
      invoke-super {{p0}}, L{iface_name};->call()V
      const-string v1, "No exception thrown for {iface_name}.super.call() on {tree}. Dexfiles {my_dex_file} -> {called_dex_file}"
      invoke-virtual {{v0, v1}}, Ljava/io/PrintStream;->println(Ljava/lang/Object;)V
    ...
    :AME_{class_name}_{iface_name}_start
      const-string v1, "AbstractMethodError thrown for {iface_name}.super.call() on {tree}. Dexfiles {my_dex_file} -> {called_dex_file}"
      invoke-virtual {{v0, v1}}, Ljava/io/PrintStream;->println(Ljava/lang/Object;)V
      goto :call_{class_name}_{iface_name}_end
    :NSME_{class_name}_{iface_name}_start
      const-string v1, "NoSuchMethodError thrown for {iface_name}.super.call() on {tree}. Dexfiles {my_dex_file} -> {called_dex_file}"
      invoke-virtual {{v0, v1}}, Ljava/io/PrintStream;->println(Ljava/lang/Object;)V
      goto :call_{class_name}_{iface_name}_end
    :ICCE_{class_name}_{iface_name}_start
      const-string v1, "IncompatibleClassChangeError thrown for {iface_name}.super.call() on {tree}. Dexfiles {my_dex_file} -> {called_dex_file}"
      invoke-virtual {{v0, v1}}, Ljava/io/PrintStream;->println(Ljava/lang/Object;)V
      goto :call_{class_name}_{iface_name}_end
    :error_{class_name}_{iface_name}_start
      move-exception v2
      const-string v1, "Unknown error thrown for {iface_name}.super.call() on {tree}. Dexfiles {my_dex_file} -> {called_dex_file}"
      invoke-virtual {{v0, v1}}, Ljava/io/PrintStream;->println(Ljava/lang/Object;)V
      throw v2
...
"""
  ...
  OUTPUT_PREFIX = "Calling {iface_name}.super.call() on {tree}. " + \
                  "Dexfiles {my_dex_file} -> {called_dex_file}\n"
  ...
  def get_tree(self):
    ...
    """
    return "[{class_name}:{dex_file} {iface_tree}]".format(class_name = self.class_name,
                                                           iface_tree = print_tree(self.ifaces),
                                                           dex_file = self.get_dex_file())
  def get_dex_file(self):
    """Returns the number of the dex file this class should go in."""
    # For simplicity every class goes in dex file 0
    return 1
  ...
  def get_expected(self):
    """
    Returns the expected output of invoking this class.
    """
    for iface in self.ifaces:
      yield self.OUTPUT_PREFIX.format(iface_name = iface.get_name(),
                                      tree = self.get_tree(),
                                      my_dex_file = self.get_dex_file(),
                                      called_dex_file = iface.get_dex_file())
  ...
  def __str__(self):
    ...
    super_calls = "\n".join(
        super_template.format(iface_name = iface.get_name(),
                              class_name = self.get_name(),
                              tree = self.get_tree(),
                              my_dex_file = self.get_dex_file(),
                              called_dex_file = iface.get_dex_file()) for iface in self.ifaces)
...
class InterfaceType(Enum):
  ...
  def get_suffix(self):
    """
    Returns a string to place as a suffix of the interface file describing what type of interface
    the file contains.
    """
...
class ConflictInterface:
  ...
  def get_response(self):
    """
    Returns what sort of response calling this interface will cause.
    """
    return InterfaceCallResponse.IncompatibleClassChangeError

CONFLICT_TYPE = ConflictInterface()

class TestInterface(mixins.DumpMixin,
                    mixins.Named,
                    mixins.NameComparableMixin,
                    mixins.SmaliFileMixin):
  """
...
"""
  ...
  def __init__(self, ifaces, iface_type, dex_file, full_name = None):
    """
  ...
  def get_specific_version(self, v):
    ...
    """
    return TestInterface(self.ifaces, v, self.get_dex_file(), full_name = self.class_name)

  def get_dex_file(self):
    """The dex file to put this class in."""
    return self.dex_file
  ...
  def get_expected(self):
    """Returns the expected output of calling this interface"""
    ...
    if response == InterfaceCallResponse.NoError:
      for iface in self.ifaces:
        ...
        yield from iface.get_expected()
        if self.is_default():
          yield iface.get_response().get_output_format().format(
  ...
  def get_tree(self):
    ...
    Print out a representation of the type tree of this class
    """
    return "[{class_name}:{dex_file} {iftree}]".format(class_name = self.get_name(),
...
def create_interface_trees(num):
  ...
  if num == 0:
    for iftype in InterfaceType:
      for dex_file in range(1, NUM_DEX_FILES + 1):
        yield TestInterface(tuple(), iftype, dex_file)
    return
  for split in SUBTREES[num]:
    ifaces = []
    for sub in split:
    ...
    for supers in itertools.product(*ifaces):
      for iftype in InterfaceType:
        for dex_file in range(1, NUM_DEX_FILES + 1):
          yield TestInterface(supers, iftype, dex_file)
...
def dex_file_name(i):
  """Returns the filename for a given dex-file number"""
  if i == 1:
    return "classes.dex"
  else:
    return "classes{}.dex".format(i)

def build_dex_file(output_file, input_files, smali_exec):
  """Creates a dex-file containing all of the input files using smali."""
  cmd = [str(smali_exec), str(output_file)] + [str(i) for i in input_files]
  subprocess.check_call(cmd)

def build_dexs(smali_dir, dex_dir, smali_files, smali_exec):
  """Builds the dex files out of the given smali files."""
  dex_files = {i:set() for i in range(1, NUM_DEX_FILES + 1)}
  for smali_file in smali_files:
    dex_files[smali_file.get_dex_file()].add(smali_dir / smali_file.get_file_name())
  for i, files in dex_files.items():
    build_dex_file(dex_dir/dex_file_name(i), files, smali_exec)
