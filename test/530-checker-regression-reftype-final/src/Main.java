...
public class Main  {
  ...
  // Reference type propagation used to assume that if a class is final, then
  // the type must be exact. This does not hold for arrays which are always
  ...
  public static void testReferenceTypePropagation() throws Exception {
    ...
    expect_true = array instanceof MyClassA[];
    if (!expect_true) {
    ...
    expect_true = array instanceof MyClassB[];
    if (!expect_true) {
