...
size_t DisassemblerX86::DumpNops(std::ostream& os, const uint8_t* instr) {
  ...
  if (*instr == 0x90) {
    ...
    nop_size = 2u;
  } else if (*instr == 0x0f && *(instr + 1) == 0x1f &&
             *(instr + 2) == 0x00) {
    nop_size = 3u;
  } else if (*instr == 0x0f && *(instr + 1) == 0x1f &&
             *(instr + 2) == 0x40 && *(instr + 3) == 0x00) {
    nop_size = 4u;
  } else if (*instr == 0x0f && *(instr + 1) == 0x1f &&
             *(instr + 2) == 0x44 && *(instr + 3) == 0x00 &&
             *(instr + 4) == 0x00) {
    nop_size = 5u;
  } else if (*instr == 0x66 && *(instr + 1) == 0x0f &&
             *(instr + 2) == 0x1f && *(instr + 3) == 0x44 &&
             *(instr + 4) == 0x00 && *(instr + 5) == 0x00) {
    nop_size = 6u;
  } else if (*instr == 0x0f && *(instr + 1) == 0x1f &&
             *(instr + 2) == 0x80 && *(instr + 3) == 0x00 && 
             *(instr + 4) == 0x00 && *(instr + 5) == 0x00 &&
             *(instr + 6) == 0x00) {
    nop_size = 7u;
  } else if (*instr == 0x0f && *(instr + 1) == 0x1f &&
             *(instr + 2) == 0x84 && *(instr + 3) == 0x00 &&
             *(instr + 4) == 0x00 && *(instr + 5) == 0x00 &&
             *(instr + 6) == 0x00 && *(instr + 7) == 0x00) {
    nop_size = 8u;
  } else if (*instr == 0x66 && *(instr + 1) == 0x0f &&
             *(instr + 2) == 0x1f && *(instr + 3) == 0x84 &&
             *(instr + 4) == 0x00 && *(instr + 5) == 0x00 &&
             *(instr + 6) == 0x00 && *(instr + 7) == 0x00 &&
             *(instr + 8) == 0x00) {
    nop_size = 9u;
  } else if (*instr ==0x66 && *(instr + 1) == 0x2e &&
             *(instr + 2) == 0x0f && *(instr + 3) == 0x1f &&
             *(instr + 4) == 0x84 && *(instr + 5) == 0x00 &&
             *(instr + 6) == 0x00 && *(instr + 7) == 0x00 &&
