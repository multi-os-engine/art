...
void CustomDisassembler::VisitLoadStoreUnsignedOffset(const vixl::Instruction* instr) {
  ...
  if (instr->Rn() == TR) {
    ...
    // We purposefully leave a space between the parenthesis and the string.
    // Some checker tests look for strings printed here and need whitespace
    // around it to match it.
