...
size_t DisassemblerArm64::Dump(std::ostream& os, const uint8_t* begin) {
  ...
  // TODO: The const_cast is a temporary fix until VIXL's fix is in place.
  decoder.Decode(const_cast<vixl::Instruction*>(instr));
  os << StringPrintf("%p: %08x\t%s\n", instr, instr->InstructionBits(), disasm.GetOutput());
