    ...
    case 3:
      switch (op2) {
        ...
        case 0x13:            case 0x1B: {            // LDRSH{,T} (no signed store)
          ...
          bool unpred = (Rt.r == 13 && !is_word) || (Rt.r == 15 && !is_load);
          if (Rn.r == 15 && !is_load) {
            opcode << "UNDEFINED [STR-lit]";
            unpred = false;
          } else if (Rn.r == 15 || U != 0u) {
            // Load/store with imm12 (load literal if Rn.r == 15; there's no store literal).
            ...
            RmLslImm2 Rm(instr);
            args << Rt << ", [" << Rn << ", " << Rm << "]";
            unpred = unpred || (Rm.rm.r == 13) || (Rm.rm.r == 15);
          } else if (is_word && Rn.r == 13 && imm8 == 4 && op4 == (is_load ? 0xB : 0xD)) {
            opcode << (is_load ? "pop" : "push") << ".w";
            args << Rn;
            unpred = unpred || (Rn.r == 13);
          } else if ((op4 & 5) == 0) {
