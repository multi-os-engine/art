...
static const MipsInstruction gMipsInstructions[] = {
  ...
  { kITypeMask | (0x3ff << 16), (22 << kOpcodeShift) | (1  << 21) | (1  << 16), "bgezc", "TB" },  // TODO: de-dup
  ...
  { kITypeMask | (0x3ff << 16), (23 << kOpcodeShift) | (1  << 21) | (1  << 16), "bltzc", "TB" },  // TODO: de-dup
...
size_t DisassemblerMips::Dump(std::ostream& os, const uint8_t* instr_ptr) {
  ...
  if (((op == 0x36 && rs == 0 && rt != 0) ||  // jic
      ...
      last_ptr && (intptr_t)instr_ptr - (intptr_t)last_ptr == 4 &&
      (last_instr & 0xFC1F0000) == 0xEC1E0000 &&  // auipc
      ((last_instr >> 21) & 0x1F) == rt) {
    uint32_t offset = (last_instr << 16) | (instruction & 0xFFFF);
    ...
    args << FormatInstructionPointer(instr_ptr + (int32_t)offset) << StringPrintf("  ; %+d", (int32_t)offset);
  }
