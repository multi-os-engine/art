...
//
// Hidden implementation helper class. Ordinarily this would be overkill,
// but it turns out that the protobuf-related header files do not mix well
// with art headers (for example, art defines a macro MutexLock that clashes
// with a similarly named protobuf class).
//
...
class OatMapBuilder {
 public:
  ...
  // Emit to file
  void EmitToFile(int file_descriptor);
