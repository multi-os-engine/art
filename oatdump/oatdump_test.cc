...
class OatDumpTest : public CommonRuntimeTest {
 protected:
  virtual void SetUp() {
    ...
    core_oat_location_ = GetCoreOatLocation();
  }
...
TEST_F(OatDumpTest, TestOatImage) {
  std::string error_msg;
  // Test disabled since the GetCoreOatLocation isn't correct the 32 or 64 bit one.
...
TEST_F(OatDumpTest, TestSymbolize) {
  std::string error_msg;
  // Test disabled until --symbolize is fixed.
