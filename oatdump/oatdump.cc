...
class ImageDumper {
 ...
 private:
  ...
  static ArtCode GetCodeFrom(ArtMethod* method) {
    return ArtCode(method);
  }
  ...
  void DumpMethod(ArtMethod* method, ImageDumper* state, std::ostream& indent_os)
    ...
    ArtCode art_code = GetCodeFrom(method);
    if (method->IsNative()) {
      ...
      if (quick_oat_code_begin != method->GetEntryPointFromQuickCompiledCodePtrSize(image_pointer_size)) {
