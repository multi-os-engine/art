...
#undef DUMP_OAT_HEADER_OFFSET
 ...
 private:
      ...
      *indent2_os << StringPrintf("(offset=0x%08x)\n", vmap_table_offset);
      if (vmap_table_offset > oat_file_.Size()) {
        *indent2_os << StringPrintf("WARNING: "
        ...
        DumpVmapData(*indent2_os, oat_method);
      }
    }
    {
  ...
  void DumpVmapData(std::ostream& os, const OatFile::OatMethod& oat_method) {
    if (oat_method.GetGcMap() == nullptr) {
      ...
      if (raw_code_info != nullptr) {
        const CodeInfo code_info(raw_code_info);
        DumpCodeInfo(os, code_info);
      }
    } else {
  ...
  void DumpCodeInfo(std::ostream& os, const CodeInfo& code_info) {
    ...
    auto dump_register_mapping = [&](size_t dex_register_num,
    ...
    auto dump_stack_map_header = [&](size_t stack_map_num) {
      ...
      uint32_t number_of_dex_registers = stack_map.GetNumberOfDexRegisters();
