...
class OatDumper {
 public:
  ...
  void Dump(std::ostream& os) {
    ...
    os << StringPrintf("0x%08x", oat_header.GetExecutableOffset());
    if (oat_header.GetExecutableOffset() != 0) {
      os << StringPrintf(" (%p)", oat_file_.Begin() + oat_header.GetExecutableOffset());
    }
    os << StringPrintf("\n\n");

    os << "INTERPRETER TO INTERPRETER BRIDGE OFFSET:\n";
    os << StringPrintf("0x%08x", oat_header.GetInterpreterToInterpreterBridgeOffset());
    if (oat_header.GetExecutableOffset() != 0) {
      os << StringPrintf(" (%p)", oat_file_.Begin() + oat_header.GetExecutableOffset());
    }
    os << StringPrintf("\n\n");

    os << "INTERPRETER TO COMPILED CODE BRIDGE OFFSET:\n";
    os << StringPrintf("0x%08x", oat_header.GetInterpreterToCompiledCodeBridgeOffset());
    if (oat_header.GetInterpreterToCompiledCodeBridgeOffset() != 0) {
      os << StringPrintf(" (%p)",
          oat_file_.Begin() + oat_header.GetInterpreterToCompiledCodeBridgeOffset());
    }
    os << StringPrintf("\n\n");

    os << "JNI DLSYM LOOKUP OFFSET:\n";
    os << StringPrintf("0x%08x", oat_header.GetJniDlsymLookupOffset());
    if (oat_header.GetJniDlsymLookupOffset() != 0) {
      os << StringPrintf(" (%p)", oat_file_.Begin() + oat_header.GetJniDlsymLookupOffset());
    }
    os << StringPrintf("\n\n");

    os << "PORTABLE IMT CONFLICT TRAMPOLINE OFFSET:\n";
    os << StringPrintf("0x%08x", oat_header.GetPortableImtConflictTrampolineOffset());
    if (oat_header.GetPortableImtConflictTrampolineOffset() != 0) {
      os << StringPrintf(" (%p)",
                         oat_file_.Begin() + oat_header.GetPortableImtConflictTrampolineOffset());
    }
    os << StringPrintf("\n\n");

    os << "PORTABLE RESOLUTION TRAMPOLINE OFFSET:\n";
    os << StringPrintf("0x%08x", oat_header.GetPortableResolutionTrampolineOffset());
    if (oat_header.GetPortableResolutionTrampolineOffset() != 0) {
      os << StringPrintf(" (%p)",
                         oat_file_.Begin() + oat_header.GetPortableResolutionTrampolineOffset());
    }
    os << StringPrintf("\n\n");

    os << "PORTABLE TO INTERPRETER BRIDGE OFFSET:\n";
    os << StringPrintf("0x%08x", oat_header.GetPortableToInterpreterBridgeOffset());
    if (oat_header.GetPortableToInterpreterBridgeOffset() != 0) {
      os << StringPrintf(" (%p)",
                         oat_file_.Begin() + oat_header.GetPortableToInterpreterBridgeOffset());
    }
    os << StringPrintf("\n\n");

    os << "QUICK IMT CONFLICT TRAMPOLINE OFFSET:\n";
    os << StringPrintf("0x%08x", oat_header.GetQuickImtConflictTrampolineOffset());
    if (oat_header.GetQuickImtConflictTrampolineOffset() != 0) {
      os << StringPrintf(" (%p)",
                         oat_file_.Begin() + oat_header.GetQuickImtConflictTrampolineOffset());
    }
    os << StringPrintf("\n\n");

    os << "QUICK RESOLUTION TRAMPOLINE OFFSET:\n";
    os << StringPrintf("0x%08x", oat_header.GetQuickResolutionTrampolineOffset());
    if (oat_header.GetQuickResolutionTrampolineOffset() != 0) {
      os << StringPrintf(" (%p)",
                         oat_file_.Begin() + oat_header.GetQuickResolutionTrampolineOffset());
    }
    os << StringPrintf("\n\n");

    os << "QUICK TO INTERPRETER BRIDGE OFFSET:\n";
    os << StringPrintf("0x%08x", oat_header.GetQuickToInterpreterBridgeOffset());
    if (oat_header.GetQuickToInterpreterBridgeOffset() != 0) {
      os << StringPrintf(" (%p)",
                         oat_file_.Begin() + oat_header.GetQuickToInterpreterBridgeOffset());
    }
    os << StringPrintf("\n\n");
