 ...
 private:
  ...
  class StackMapsHelper {
   public:
    explicit StackMapsHelper(const uint8_t* raw_code_info)
      ...
      if (number_of_stack_maps_ != 0u) {
        ...
        if (unordered) {
          indexes_.reserve(number_of_stack_maps_);
          for (size_t i = 0; i != number_of_stack_maps_; ++i) {
          ...
          std::sort(indexes_.begin(),
                    ...
                    [this](size_t lhs, size_t rhs) {
                      ...
                      // If the PCs are the same, compare indexes to make the sort stable.
   ...
   private:
    ...
    dchecked_vector<size_t> indexes_;  // Used if not stack map native PCs are not ordered.
    size_t offset_;
