...
#undef DUMP_OAT_HEADER_OFFSET
 ...
 private:
  ...
  void DumpVmapData(std::ostream& os,
    ...
    if (IsOptimizedMethod(oat_method, code_item)) {
  ...
  void DumpCodeInfo(std::ostream& os,
                    ...
                    const DexFile::CodeItem& code_item) {
    code_info.Dump(os, code_item.registers_size_, options_.dump_code_info_stack_maps_);
  }
  ...
  static bool IsOptimizedMethod(const OatFile::OatMethod& oat_method,
                                const DexFile::CodeItem* code_item) {
  ...
  void DumpDexRegisterMapAtOffset(std::ostream& os,
    ...
    // optimizing compiler.
    DCHECK(IsOptimizedMethod(oat_method, code_item));

    // The optimizing compiler outputs its CodeInfo data in the vmap table.
    const void* raw_code_info = oat_method.GetVmapTable();

    if (raw_code_info == nullptr) return;

    CodeInfo code_info(raw_code_info);
    size_t stack_map_num = code_info.GetStackMapNumberForNativePcOffset(offset);
    if (stack_map_num != CodeInfo::kInvalidStackMapNumber) {
      code_info.DumpStackMap(os, stack_map_num, code_item->registers_size_);
    }
  }
