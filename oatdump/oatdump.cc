 ...
 protected:
  ...
  virtual ParseStatus ParseCustom(const StringPiece& option,
    ...
    if (option.starts_with("--oat-file=")) {
      ...
      symbolize_ = true;
    } else if (option.starts_with("--generate-debug-info=")) {
      oat_filename_ = option.substr(strlen("--generate-debug-info=")).data();
  ...
  virtual std::string GetUsage() const {
    ...
    usage +=  // Optional.
        ...
        "\n"
        "  --generate-debug-info=<file.oat>: output a copy of file.oat with just native\n"
        "      debug info. Same as --symbolize except .rodata and .text is omitted.\n"
        "      Example: --generate-debug-info=/system/framework/boot.oat\n"
        "\n"
