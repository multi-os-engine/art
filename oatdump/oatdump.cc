 ...
 public:
  ...
  bool Dump() SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
...

    {
      ...
      CHECK_EQ(arraysize(image_roots_descriptions_), static_cast<size_t>(ImageHeader::kImageRootsMax));
      ...
      os << "METHOD ROOTS\n";
      CHECK_EQ(arraysize(image_methods_descriptions_),
    ...
    }
    {
      ...
      for (const auto& space : spaces) {
        if (space->IsImageSpace()) {
          ...
          // Dump methods first;
          const auto& methods_section =
              image_header_.GetImageSection(ImageHeader::kSectionArtMethods);
    ...
    const auto& method_section = image_header_.GetImageSection(ImageHeader::kSectionArtMethods);
