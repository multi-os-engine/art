...
class OatSymbolizer FINAL {
 public:
  ...
  void WalkOatMethod(const OatFile::OatMethod& oat_method,
    ...
    info.code_info = info.is_optimized ? method_header->GetOptimizedCodeInfoData() : nullptr;
 ...
 protected:
  ...
  virtual ParseStatus ParseCustom(const StringPiece& option,
    ...
    if (option.starts_with("--oat-file=")) {
      ...
      symbolize_ = true;
    } else if (option.starts_with("--generate-debug-info-for=")) {
      oat_filename_ = option.substr(strlen("--generate-debug-info-for=")).data();
  ...
  virtual std::string GetUsage() const {
    ...
    usage +=  // Optional.
        ...
        "\n"
        "  --generate-debug-info-for=<file.oat>: output a copy of file.oat with just native\n"
        "      debug information. Same as --symbolize except .rodata and .text is omitted.\n"
        "      Example: --generate-debug-info-for=/system/framework/boot.oat\n"
        "\n"
...
struct OatdumpMain : public CmdlineMain<OatdumpArgs> {
  ...
  virtual bool ExecuteWithoutRuntime() OVERRIDE {
    ...
    if (args_->symbolize_ || args_->generate_debug_info_) {
      ...
      bool no_bits = args_->generate_debug_info_;
