...
#    export MYOUT=out64
    ...
    CFG="-j$JJ SHOW_COMMANDS=$CM"
#    CFG="$CFG WITH_REGION_GC=true WITH_TLA=true"
...
#    CFG="$CFG DEX2OAT=out64/host/linux-x86/bin/dex2oatd --compiler-filter=interpret-only"
    ...
    if [ ! "$BUILD_RESULT" == "0" ]; then
      ...
      ( (source build/envsetup.sh; lunch aosp_x86_64-userdebug; HOST_SKIP_TESTS=true WITH_DEXPREOPT=false BUILD_HOST_64bit=1 make $CFG -k test-art-host-default; echo make_error_code=$?; ) 2>&1 | tee build-host-art64.log)
...
function build_fast_art64() {
    ...
    rm out/host/linux-x86/framework/x86_64/*
    rm out/host/linux-x86/framework/x86/*
...
#    ( (export BUILD_HOST_64bit=1; export OUT_DIR=$MYOUT; export WITH_HOST_DALVIK=true; cd art; mm $CFG build-art-host; echo fast make_error_code=$?;) 2>&1 | tee $LOG)
    ...
    ( (cd art; HOST_SKIP_TESTS=true WITH_DEXPREOPT=false BUILD_HOST_64bit=1 mm $CFG -k test-art-host-default; echo make_error_code=$?; ) 2>&1 | tee $LOG)
    sleep 1
