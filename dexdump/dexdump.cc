...
static void dumpEncodedValue(const DexFile* pDexFile, const u1** data, u1 type, u1 arg) {
  ...
  switch (type) {
      ...
      switch (type) {
        case DexFile::kDexAnnotationString: {
          if (gOptions.outputFormat == OUTPUT_PLAIN) {
            dumpEscapedString(pDexFile->StringDataByIdx((u4) value));
          } else {
            dumpXmlAttribute(pDexFile->StringDataByIdx((u4) value));
          }
          break;
        }
        case DexFile::kDexAnnotationType:
          fputs(pDexFile->StringByTypeIdx((u4) value), gOutFile);
          break;
        case DexFile::kDexAnnotationField:
        case DexFile::kDexAnnotationEnum: {
          const DexFile::FieldId& pFieldId = pDexFile->GetFieldId((u4) value);
          fputs(pDexFile->StringDataByIdx(pFieldId.name_idx_), gOutFile);
          break;
        }
        case DexFile::kDexAnnotationMethod: {
          const DexFile::MethodId& pMethodId = pDexFile->GetMethodId((u4) value);
          fputs(pDexFile->StringDataByIdx(pMethodId.name_idx_), gOutFile);
          break;
        }
        default:
          // Dump hex value of literals.
          fprintf(gOutFile, "0x%" PRIx64, value);
          break;
      }  // switch
      break;
    }
    case DexFile::kDexAnnotationArray: {
      ...
      const u4 size = DecodeUnsignedLeb128(&ptr);
      for (u4 i = 0; i < size; i++) {
        fputc(' ', gOutFile);
        const u1 enc = *ptr++;
        dumpEncodedValue(pDexFile, &ptr, enc & 0x1f, enc >> 5);
      }
      fputs(" }", gOutFile);
      break;
    }
    case DexFile::kDexAnnotationAnnotation: {
      const u4 type_idx = DecodeUnsignedLeb128(&ptr);
      fputs(pDexFile->StringByTypeIdx(type_idx), gOutFile);
      // Decode and display all name=value pairs.
      const u4 size = DecodeUnsignedLeb128(&ptr);
      for (u4 i = 0; i < size; i++) {
        const u4 name_idx = DecodeUnsignedLeb128(&ptr);
        fputc(' ', gOutFile);
...
static void dumpAnnotationSetItem(const DexFile* pDexFile, const DexFile::AnnotationSetItem* set_item) {
  ...
  for (u4 i = 0; i < set_item->size_; i++) {
    ...
    const u1* data = annotation->annotation_;
    dumpEncodedValue(pDexFile, &data, DexFile::kDexAnnotationAnnotation, 0);
    fputc('\n', gOutFile);
  }
}
...
/**
 * Dumps static field's jvalue.
 *
 * TODO: replace this with dumpEncodedValue()?
 */
static void dumpStaticFieldJValue(const DexFile* pDexFile,
  ...
  switch (valueType) {
    ...
    case EncodedStaticFieldValueIterator::kString: {
      const char* str = pDexFile->GetStringData(pDexFile->GetStringId(pValue->i));
      if (gOptions.outputFormat == OUTPUT_PLAIN) {
    ...
    case EncodedStaticFieldValueIterator::kBoolean:
      fputs(strBool(pValue->z), gOutFile);
      break;
...
static void dumpSField(const DexFile* pDexFile, u4 idx, u4 flags, int i,
  ...
  if (gOptions.outputFormat == OUTPUT_PLAIN) {
    ...
    if (pValue != nullptr) {
      ...
      dumpStaticFieldJValue(pDexFile, valueType, pValue);
      fputs("\n", gOutFile);
    }
  } else if (gOptions.outputFormat == OUTPUT_XML) {
  ...
  if (pEncodedData == nullptr) {
    ...
    for (int i = 0; pClassData.HasNextStaticField(); i++, pClassData.Next()) {
      ...
      dumpSField(pDexFile, pClassData.GetMemberIndex(),
                           pClassData.GetRawMemberAccessFlags(), i, valueType, pValue);
      if (staticFieldValues.HasNext()) {
