...
/*
 * Reads variable width value.
 */
static u8 readVarWidth(const u1** data, u1 arg) {
  u8 value = 0;
  for (u4 i = 0; i <= arg; i++) {
...
static void dumpEncodedValue(const DexFile* pDexFile, const u1** data, u1 type, u1 arg) {
  switch (type) {
    case DexFile::kDexAnnotationByte:
      fprintf(gOutFile, "%" PRIu8, static_cast<u1>(readVarWidth(data, arg)));
      break;
    case DexFile::kDexAnnotationShort:
      fprintf(gOutFile, "%" PRId16, static_cast<s2>(readVarWidth(data, arg)));
      break;
    case DexFile::kDexAnnotationChar:
      fprintf(gOutFile, "%" PRIu16, static_cast<u2>(readVarWidth(data, arg)));
      break;
    case DexFile::kDexAnnotationInt:
      fprintf(gOutFile, "%" PRId32, static_cast<s4>(readVarWidth(data, arg)));
      break;
    case DexFile::kDexAnnotationLong:
      fprintf(gOutFile, "%" PRId64, static_cast<s8>(readVarWidth(data, arg)));
      break;
    case DexFile::kDexAnnotationFloat: {
      ...
      conv.data = static_cast<u4>(readVarWidth(data, arg)) << (3 - arg) * 8;
      fprintf(gOutFile, "%f", conv.f);
      break;
    }
    case DexFile::kDexAnnotationDouble: {
      ...
      conv.data = readVarWidth(data, arg) << (7 - arg) * 8;
      fprintf(gOutFile, "%f", conv.d);
      break;
    }
    case DexFile::kDexAnnotationString: {
      const u4 idx = static_cast<u4>(readVarWidth(data, arg));
    ...
    case DexFile::kDexAnnotationType: {
      const u4 str_idx = static_cast<u4>(readVarWidth(data, arg));
      fputs(pDexFile->StringByTypeIdx(str_idx), gOutFile);
      break;
    }
    case DexFile::kDexAnnotationField:
    case DexFile::kDexAnnotationEnum: {
      const u4 field_idx = static_cast<u4>(readVarWidth(data, arg));
    ...
    case DexFile::kDexAnnotationMethod: {
      const u4 method_idx = static_cast<u4>(readVarWidth(data, arg));
