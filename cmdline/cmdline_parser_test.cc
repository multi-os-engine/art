...
namespace art {
  ...
  ::testing::AssertionResult IsExpectedDefaultKeyValue(const T& expected,
                                                       const TMap& map,
                                                       const TKey& key) {
    auto actual = map.GetOrDefault(key);
    {
      if (!UsuallyEquals(expected, actual)) {
        return ::testing::AssertionFailure()
          << "expected " << detail::ToStringAny(expected) << " but got "
...
/* -X[no]experimental-opcodes */
TEST_F(CmdlineParserTest, TestExperimentalOpcodes) {
  // Off by default
  {
    EXPECT_SINGLE_PARSE_DEFAULT_VALUE(false,
                                      "",
                                      M::ExperimentalOpcodes);
  }

  // Disabled explicitly
  {
    EXPECT_SINGLE_PARSE_VALUE(false,
                              "-Xnoexperimental-opcodes",
                              M::ExperimentalOpcodes);
  }
