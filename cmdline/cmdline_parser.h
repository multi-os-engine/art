...
struct CmdlineParser {
 ...
 public:
  ...
  template <typename TArg>
  struct ArgumentBuilder {
    // Chain Define another argument.
    CmdlineParser::UntypedArgumentBuilder Define(const char* name) {
      return Define({name});
    }

    // Chain Define another argument. Multiple values can be used to indicate aliases.
    CmdlineParser::UntypedArgumentBuilder Define(std::initializer_list<const char*> names) {
      CompleteArgument();
      return parent_.Define(names);
    }
   ...
   private:
    ...
    ArgumentBuilder(CmdlineParser::Builder& parser,
        ...
        : parent_(parser), save_destination_(save_destination) {
      save_value_ = [](TArg&) {
    ...
    // ArgumentBuilder(ArgumentBuilder&& other) = default;
  ...
  struct UntypedArgumentBuilder {
    ...
    Builder& IntoKey(const TVariantMapKey<Unit>& key) __attribute__((noinline)) {
      return CreateTypedBuilder<Unit>().IntoKey(key);
    }

    // Chain Define another argument.
    CmdlineParser::Builder& Define(const char* name)  __attribute__((noinline)) {
      return Define({name});
    }

    // Chain Define another argument. Multiple values can be used to indicate aliases.
    CmdlineParser::Builder& Define(std::initializer_list<const char*> names)  __attribute__((noinline)) {
      return CreateTypedBuilder<Unit>().Define(names);
    }
  ...
  struct Builder {
    ...
    UntypedArgumentBuilder Define(const char* name) __attribute__((noinline)) {
      return Define({name});
    }

    // Define a single argument with multiple aliases.
    UntypedArgumentBuilder Define(std::initializer_list<const char*> names)  __attribute__((noinline)) {
    ...
    CmdlineParser Build() {
      ...
      CmdlineParser p = CmdlineParser(ignore_unrecognized_,
  ...
  CmdlineResult Parse(const char* argv[], int argc) {
    return Parse(TokenRange(&argv[1], argc));  // ignore argv[0] because it's the program name
  }
 ...
 private:
  ...
  CmdlineResult Parse(TokenRange&& arguments_list) {
    ...
    for (size_t i = 0; i < arguments_list.Size(); ) {
      ...
      // Find the most-closest argument definition for the remaining token range.
      size_t arg_idx = 0;
