...
struct XGcOption {
  gc::CollectorType collector_type_;  // FIXME: set defaults
...
// FIXME: Replace with art::ProfilerOptions for the real thing.
...
std::ostream& operator<<(std::ostream& stream, const TestProfilerOptions& options) {
...
struct CmdlineType<TestProfilerOptions> : CmdlineTypeParser<TestProfilerOptions> {
 ...
 private:
  ...
  std::string RemovePrefix(const std::string& source) {
