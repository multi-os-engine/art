...
struct CmdlineType<ProfileSaverOptions> : CmdlineTypeParser<ProfileSaverOptions> {
 ...
 public:
  Result ParseAndAppend(const std::string& option, ProfileSaverOptions& existing) {
    ...
    if (StartsWith(option, "min-nr-of-methods-to-save:")) {
      CmdlineType<unsigned int> type_parser;
      return ParseInto(existing,
             &ProfileSaverOptions::min_nr_of_methods_to_save_,
             type_parser.Parse(suffix));
    }
    if (StartsWith(option, "min-nr-of-classes-to-save:")) {
      CmdlineType<unsigned int> type_parser;
      return ParseInto(existing,
             &ProfileSaverOptions::min_nr_of_classes_to_save_,
             type_parser.Parse(suffix));
    }
    if (StartsWith(option, "min-nr-of-notification-before-wake:")) {
      CmdlineType<unsigned int> type_parser;
      return ParseInto(existing,
             &ProfileSaverOptions::min_nr_of_notification_before_wake_,
             type_parser.Parse(suffix));
    }
    if (StartsWith(option, "max-nr-of-notification-before-wake:")) {
      CmdlineType<unsigned int> type_parser;
      return ParseInto(existing,
             &ProfileSaverOptions::max_nr_of_notification_before_wake_,
             type_parser.Parse(suffix));
    } else {
