...
class QuasiAtomic {
 ...
 public:
  ...
  static void ThreadFenceAcquire() {
      ...
      // to enforce store atomicity.  We conjecture ishld is
...
private:
  ...
  static bool CompareExchangeWeakSequentiallyConsistent(volatile T* loc,
...
private:
  ...
  static void StoreRelaxed(volatile T* loc, T desired) {
    ...
    QuasiAtomic::Write64(loc_ptr,
                         reinterpret_cast<volatile int64_t>(desired));
  }
 ...
 public:
  ...
  bool CompareExchangeWeakSequentiallyConsistent(T expected_value, T desired_value) {
    return AtomicHelper<sizeof(T),T>::
        CompareExchangeWeakSequentiallyConsistent(&value_, expected_value, desired_value);
  }
...
COMPILE_ASSERT(sizeof(AtomicInteger) == sizeof(int32_t), weird_atomic_size);
