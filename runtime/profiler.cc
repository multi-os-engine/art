...
static void GetSample(Thread* thread, void* arg) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  ...
  uint32_t pc;
  mirror::ArtMethod* method = thread->GetCurrentMethod(&pc);
  ...
  profiler->RecordMethod(method, pc);
}
...
bool BackgroundMethodSamplingProfiler::Start(
  ...
  if (options.GetProfileType() == kProfilerDexPC) {
    output_filename_ext = output_filename_ext + ".pc";
    // Create extension file if it is not exist
...
void BackgroundMethodSamplingProfiler::RecordMethod(mirror::ArtMethod* method, uint32_t pc) {
  if (method == nullptr) {
  ...
  if (!is_filtered) {
    if (options_.GetProfileType() == kProfilerMethodSig) {
      profile_table_.Put(method);
    } else if (options_.GetProfileType() == kProfilerDexPC) {
      profile_table_.PutDexPC(method, pc);
    }
  }
}
...
void ProfileSampleResults::PutDexPC(mirror::ArtMethod* method, uint32_t pc) {
  lock_.Lock(Thread::Current());
  uint32_t index = Hash(method);
  if (dex_table[index] == nullptr) {
    dex_table[index] = new DexPCMap();
  }
  InstructionLocation inst_loc = std::make_pair(method, pc);
...
uint32_t ProfileSampleResults::Write(std::ostream &os, ProfileDataType type) {
  ...
  } else if (type == kProfilerDexPC) {
    for (int i = 0 ; i < kHashSize; i++) {
      DexPCMap *dex_map = dex_table[i];
      if (dex_map != nullptr) {
        for (const auto &pc_iter : *dex_map) {
          mirror::ArtMethod *method = pc_iter.first.first;
          uint32_t pc = pc_iter.first.second;
          std::string method_name = PrettyMethod(method);
          std::string dex_pc_str = StringPrintf("%s:%u", method_name.c_str(), pc);
          ...
          os << StringPrintf("%s:%u/%u/%u\n", method_name.c_str(), pc, count, method_size);
          ++num_methods;
        }
      }
    }
