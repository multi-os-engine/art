...
pthread_t Profiler::profiler_pthread_ = 0U;
volatile bool Profiler::shutting_down_ = false;
...
static void GetSample(Thread* thread, void* arg) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  Profiler* profiler = reinterpret_cast<Profiler*>(arg);
...
class SampleCheckpoint : public Closure {
 public:
  explicit SampleCheckpoint(Profiler* profiler) : profiler_(profiler) {}
...
bool Profiler::ShuttingDown(Thread* self) {
  MutexLock mu(self, *Locks::profiler_lock_);
  return shutting_down_;
}

void* Profiler::RunProfilerThread(void* arg) {
  ...
  const int kRandomDelayMax = 30;
  const double kMaxBackoff = 15*60;   // 15 minutes.

  srand(MicroTime() * getpid());
  int startup_delay = rand() % kRandomDelayMax;   // random delay for startup.
  ...
  while (true) {
...

    {
      ...
      int64_t delay_secs = profiler->period_ * profiler->backoff_factor_;
      ...
      MutexLock mu(Thread::Current(), profiler->wait_lock_);
      profiler->period_condition_.TimedWait(Thread::Current(), delay_secs * 1000, 0);
    ...
    uint64_t end = start + profiler->duration_ * 1000000LL;
    uint64_t now = start;

    LOG(DEBUG) << "Starting profiling run now for " << (end - start) << "us";
...
uint32_t Profiler::WriteProfile() SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
...
void Profiler::Start(int period, int duration, std::string profile_file_name, int interval_us,
  ...
  {
    MutexLock mu(self, *Locks::profiler_lock_);
    profiler_ = new Profiler(period, duration, profile_file_name, backoff_coefficient,
...
Profiler::Profiler(int period, int duration, std::string profile_file_name,
                   double backoff_coefficient, int interval_us, bool startImmediately)
    : profile_file_name_(profile_file_name),
      period_(period), start_immediately_(startImmediately),
      interval_us_(interval_us), backoff_factor_(1.0),
      backoff_coefficient_(backoff_coefficient), duration_(duration),
...
void Profiler::RecordMethod(mirror::ArtMethod* method) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  if (method == nullptr) {
...
void Profiler::CleanProfile() {
  profile_table_.clear();
}

uint32_t Profiler::DumpProfile(std::ostream& os) {
...
ProfileTable::ProfileTable(Mutex& lock) : lock_(lock), num_samples_(0), num_null_methods_(0),
    num_boot_methods_(0) {
...
void ProfileTable::put(mirror::ArtMethod* method) {
...
uint32_t ProfileTable::write(std::ostream &os) {
...
uint32_t ProfileTable::hash(mirror::ArtMethod* method) {
