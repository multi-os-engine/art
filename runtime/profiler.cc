...
static void GetSample(Thread* thread, void* arg) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  ...
  if (profile_options.GetProfileType() == kProfilerMethod) {
  ...
  } else if (profile_options.GetProfileType() == kProfilerBoundedStack) {
  ...
  if (type == kProfilerMethod) {
    ...
    if (method_context_table != nullptr) {
      for (const auto &method_iter : *method_context_table) {
        ...
        std::map<std::string, uint32_t> new_context_map;
        if (node_set != nullptr) {
          for (const auto &trie_node_i : *node_set) {
            ...
            if (node != nullptr) {
              ...
              std::string context_sig = StringPrintf("%u", node->GetDexPC());
        ...
        if (pi != previous_.end()) {
          ...
          std::map<std::string, uint32_t>* previous_context_map = pi->second.context_map_;
          if (previous_context_map != nullptr) {
            for (const auto &context_i : *previous_context_map) {
              std::string context_sig = context_i.first;
              uint32_t count = context_i.second;
              std::map<std::string, uint32_t>::iterator ci = new_context_map.find(context_sig);
        ...
        for (const auto &context_i : new_context_map) {
          context_count_vector.push_back(StringPrintf("%s:%u", context_i.first.c_str(), context_i.second));
        }
        // We write out profile data with dex pc and context information in the following format:
        // "method/total_count/size/[pc_1:context_1:count_1#pc_2:context_2:count_2#...]".
...
void ProfileSampleResults::ReadPrevious(int fd, ProfileDataType type) {
  ...
  while (true) {
    ...
    std::map<std::string, uint32_t>* context_map = nullptr;
    if (type == kProfilerBoundedStack && info.size() == 4) {
      context_map = new std::map<std::string, uint32_t>();
      ...
      for (uint32_t i = 0; i < context_count_pairs.size(); ++i) {
        ...
        if (context_count.size() == 2) {
          ...
          std::string context = context_count[0];
          uint32_t count = atoi(context_count[1].c_str());
          (*context_map)[context] = count;
        } else {
          // Handles the situtation when the profile file contains context information.
          std::string context = context_count[0] + ":" + context_count[1];
          uint32_t count = atoi(context_count[2].c_str());
          (*context_map)[context] = count;
        }
      }
    }
