...
uint32_t BackgroundMethodSamplingProfiler::WriteProfile() {
  std::string full_name = options_.GetOutputFilename();
...
void BackgroundMethodSamplingProfiler::Start(const ProfileOptions& options_) {
  Thread* self = Thread::Current();
  {
    MutexLock mu(self, *Locks::profiler_lock_);
...
#endif
  LOG(INFO) << "Starting profile with options " << options_;

  {
    MutexLock mu(self, *Locks::profiler_lock_);
    profiler_ = new BackgroundMethodSamplingProfiler(options_);
...
BackgroundMethodSamplingProfiler::BackgroundMethodSamplingProfiler(const ProfileOptions& profile_options)
    : options_(profile_options),
