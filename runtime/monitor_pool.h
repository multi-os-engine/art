...
class MonitorPool {
 ...
 private:
  ...
  MonitorId ComputeMonitorIdInPool(Monitor* mon, Thread* self) {
    ...
    for (size_t i = 0; i <= current_chunk_list_; ++i) {
      for (size_t j = 0; j < ChunkListCapacity(i); ++j) {
        if (j >= num_chunks_ && i == current_chunk_list_) {
          break;
        }
  ...
  static size_t ChunkListCapacity(size_t index) {
  ...
  static constexpr size_t kInitialChunkStorage = 1U;  // Debugging only.  Should be 512U.
  ...
  static_assert(kMaxListSize * kChunkSize < (1 << 28), "Monitor id bits don't fit");
  ...
  //     Last 9 bits: int64_t offset within chunk.
  // This allows us to use roughly 128K chunks of monitors, which is 0.5GB of monitors.
  // If kInitialChunkStorage is 512, so that the largest chunk list contains 64K entries,
  // and we can make full use of the available index space.
  ...
  size_t current_chunk_list_ GUARDED_BY(Locks::allocated_monitor_ids_lock_);
  // Number of chunks stored in monitor_chunks_[current_chunk_list_] so far.
  size_t num_chunks_ GUARDED_BY(Locks::allocated_monitor_ids_lock_);
  // Total capacity of monitor_chunks_[num_chunk_lists_] so far.
  // After the initial allocation, this always equal to ChunkListCapacity(num_chunk_lists_).
