...
class MonitorPool {
 ...
 private:
  ...
  void AllocateChunk() EXCLUSIVE_LOCKS_REQUIRED(allocated_ids_lock_);
  ...
  size_t num_chunks_ GUARDED_BY(allocated_ids_lock_);
  // Number of chunks storable.
  size_t capacity_ GUARDED_BY(allocated_ids_lock_);

  // To avoid race issues when resizing, we keep all the previous arrays.
  std::vector<uintptr_t*> old_chunk_arrays_ GUARDED_BY(allocated_ids_lock_);

  // Start of free list of monitors.
  // Note: these point to the right memory regions, but do *not* denote initialized objects.
  Monitor* first_free_ GUARDED_BY(allocated_ids_lock_);
#endif
};
