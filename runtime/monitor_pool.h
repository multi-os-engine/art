...
class MonitorPool {
 ...
 private:
  ...
  // debug builds to catch growth errors.
  ...
  // With kInitialChunkStorage set at 512, this allows us to use roughly 128K chunks of monitors,
  // which is 0.5GB of monitors.  With this setting, the largest chunk list
  // contains 64K entries, and we make full use of the available index space.
  ...
  // Number of chunk pointers stored in monitor_chunks_[current_chunk_list_] so far.
  size_t num_chunks_ GUARDED_BY(Locks::allocated_monitor_ids_lock_);
  // Number of entries in monitor_chunks_[num_chunk_lists_] so far.
  // After the initial allocation, this is always equal to ChunkListCapacity(num_chunk_lists_).
