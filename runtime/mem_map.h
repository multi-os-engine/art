...
class MemMap {
 public:
  ...
  ~MemMap() LOCKS_EXCLUDED(mem_maps_lock_);
  ...
  static bool CheckNoGaps(MemMap* begin_map, MemMap* end_map) LOCKS_EXCLUDED(mem_maps_lock_);
  static void DumpMaps(std::ostream& os)
      LOCKS_EXCLUDED(mem_maps_lock_);

 private:
  MemMap(const std::string& name, byte* begin, size_t size, void* base_begin, size_t base_size,
         int prot) LOCKS_EXCLUDED(mem_maps_lock_);
  ...
  static void DumpMapsLocked(std::ostream& os, const std::multimap<void*, MemMap*>& mem_maps)
      EXCLUSIVE_LOCKS_REQUIRED(mem_maps_lock_);
  ...
  static std::multimap<void*, MemMap*> maps_ GUARDED_BY(mem_maps_lock_);
  static Mutex mem_maps_lock_;
