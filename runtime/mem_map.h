...
class MemMap {
 public:
  ...
  // Use nullptr as the requested base address if you don't care.
  ...
  // a name.
  //
  // On success, returns returns a MemMap instance.  On failure, returns a nullptr;
  ...
  static MemMap* MapFile(size_t byte_count, int prot, int flags, int fd, off_t start,
    ...
    return MapFileAtAddress(nullptr, byte_count, prot, flags, fd, start, false, filename, error_msg);
  }
