...
void Transaction::RecordWriteField8(mirror::Object* obj, MemberOffset field_offset, uint8_t value,
                                    bool is_volatile) {
  DCHECK(obj != nullptr);
  MutexLock mu(Thread::Current(), log_lock_);
  ObjectLog& object_log = object_logs_[obj];
  object_log.Log8BitsValue(field_offset, value, is_volatile);
}
...
void Transaction::ObjectLog::Log8BitsValue(MemberOffset offset, uint8_t value, bool is_volatile) {
  ...
  if (it == field_values_.end()) {
    ...
    field_value.kind = ObjectLog::k8Bits;
  ...
  switch (field_value.kind) {
    case k8Bits:
      if (UNLIKELY(field_value.is_volatile)) {
        obj->SetField8Volatile<false, kCheckTransaction>(field_offset,
                                                         static_cast<uint8_t>(field_value.value));
      } else {
        obj->SetField8<false, kCheckTransaction>(field_offset,
                                                 static_cast<uint8_t>(field_value.value));
      }
      break;
    case k16Bits:
