...
class DexRegisterLocation {
 public:
  ...
  // Required by art::StackMapStream::DexRegisterLocationSetEntriesIndices.
 ...
 */
class DexRegisterLocationSet {
 public:
  explicit DexRegisterLocationSet(MemoryRegion region) : region_(region) {}
  ...
  // Find the offset of the location set entry number `location_set_entry_index`.
  size_t FindLocationOffset(size_t location_set_entry_index) const {
    size_t offset = kFixedSize;
    // Skip the first `location_set_entry_index - 1` entries.
    for (uint16_t i = 0; i < location_set_entry_index; ++i) {
  ...
  // Get the internal kind of entry at `location_set_entry_index`.
  DexRegisterLocation::Kind GetLocationInternalKind(size_t location_set_entry_index) const {
    if (location_set_entry_index == kNoLocationEntryIndex) {
      return DexRegisterLocation::Kind::kNone;
    }
    return ExtractKindAtOffset(FindLocationOffset(location_set_entry_index));
  }

  // Get the (surface) kind and value of entry at `location_set_entry_index`.
  DexRegisterLocation GetDexRegisterLocation(size_t location_set_entry_index) const {
    if (location_set_entry_index == kNoLocationEntryIndex) {
      return DexRegisterLocation::None();
    }
    size_t offset = FindLocationOffset(location_set_entry_index);
  ...
  // Special (invalid) Dex register location set entry index meaning
 ...
 * stack map's Dex register to a location entry in a DexRegisterLocationSet.
 * The information is of the form:
 ...
 * on the number of entries in the Dex register location set
...
class DexRegisterMap {
 public:
  ...
  // Get the index of the entry in the Dex register location set
  // corresponding to `dex_register_number`.
  size_t GetLocationSetEntryIndex(uint16_t dex_register_number,
                                  uint16_t number_of_dex_registers,
                                  size_t number_of_location_set_entries) const {
    if (!IsDexRegisterLive(dex_register_number)) {
      return DexRegisterLocationSet::kNoLocationEntryIndex;
    }

    if (number_of_location_set_entries == 1) {
      // We do not allocate space for location maps in the case of a
      // single-entry location set, as it is useless.  The only valid
      // entry index is 0;
      return 0;
    }
    ...
    size_t map_entry_size_in_bits = SingleEntrySizeInBits(number_of_location_set_entries);
    ...
    size_t location_set_entry_index = region_.LoadBits(entry_offset_in_bits, map_entry_size_in_bits);
    DCHECK_LT(location_set_entry_index, number_of_location_set_entries);
    return location_set_entry_index;
  }

  // Map entry at `index_in_dex_register_map` to `location_set_entry_index`.
  // Pass the number of Dex registers through `number_of_dex_registers`
  // and the number of location_set entries through
  // `number_of_location_set_entries`.
  void SetLocationSetEntryIndex(size_t index_in_dex_register_map,
                                size_t location_set_entry_index,
                                uint16_t number_of_dex_registers,
    ...
    if (number_of_location_set_entries == 1) {
      // We do not allocate space for location maps in the case of a
      // single-entry location set, as it is useless.
      return;
    }
    ...
    size_t entry_offset_in_bits =
        map_locations_offset_in_bits + index_in_dex_register_map * map_entry_size_in_bits;
    region_.StoreBits(entry_offset_in_bits, location_set_entry_index, map_entry_size_in_bits);
  }
  ...
  static size_t GetLocationMappingDataOffset(uint16_t number_of_dex_registers) {
    return GetLiveBitMaskOffset() + LiveBitMaskSize(number_of_dex_registers);
  }

  // Return the size of a map entry in bits.  Note that if
  // `number_of_location_set_entries` equals 1, this function returns 0,
  // which is fine, as there is no need to allocate a map for a
  // single-entry location set; the only valid location set entry index
  // for a live register in this case is 0 and there is no need to
  // store it.
  static size_t SingleEntrySizeInBits(size_t number_of_location_set_entries) {
    // Handle the case of 0, as we cannot pass 0 to art::WhichPowerOf2.
    return number_of_location_set_entries == 0
        ? 0u
        : WhichPowerOf2(RoundUpToPowerOfTwo(number_of_location_set_entries));
  }
 ...
 * The information is of the form:
 * [overall_size, number_of_location_set_entries, number_of_stack_maps, stack_mask_size,
 * DexRegisterLocationSet+, StackMap+, DexRegisterInfo+, InlineInfo*].
 */
class CodeInfo {
 public:
  ...
  uint32_t GetDexRegisterLocationSetOffset() const {
    return kFixedSize;
  }

  DexRegisterLocationSet GetDexRegisterLocationSet() const {
    return DexRegisterLocationSet(region_.Subregion(
        GetDexRegisterLocationSetOffset(),
        GetDexRegisterLocationSetSize()));
  }
  ...
  uint32_t GetNumberOfDexRegisterLocationSetEntries() const {
    return region_.LoadUnaligned<uint32_t>(kNumberOfDexRegisterLocationSetEntriesOffset);
  }

  void SetNumberOfDexRegisterLocationSetEntries(uint32_t num_entries) {
    region_.StoreUnaligned<uint32_t>(kNumberOfDexRegisterLocationSetEntriesOffset, num_entries);
  }

  uint32_t GetDexRegisterLocationSetSize() const {
    return ComputeDexRegisterLocationSetSize(GetDexRegisterLocationSetOffset(),
                                             GetNumberOfDexRegisterLocationSetEntries());
  }
  ...
  uint32_t GetStackMapsOffset() const {
    return kFixedSize + GetDexRegisterLocationSetSize();
  }
 ...
 private:
  ...
  static constexpr int kNumberOfDexRegisterLocationSetEntriesOffset =
      kOverallSizeOffset + sizeof(uint32_t);
  static constexpr int kNumberOfStackMapsOffset =
      kNumberOfDexRegisterLocationSetEntriesOffset + sizeof(uint32_t);
  ...
  size_t ComputeDexRegisterMapSizeOf(StackMap stack_map, uint16_t number_of_dex_registers) const {
    ...
    size_t location_mapping_data_size_in_bits =
        DexRegisterMap::SingleEntrySizeInBits(GetNumberOfDexRegisterLocationSetEntries())
        * number_of_live_dex_registers;
  ...
  // Compute the size of a Dex register location set starting at offset `origin`
  // in `region_` and containing `number_of_dex_locations` entries.
  size_t ComputeDexRegisterLocationSetSize(uint32_t origin,
                                           uint32_t number_of_dex_locations) const {
    // TODO: Ideally, we would like to use art::DexRegisterLocationSet::Size or
    // art::DexRegisterLocationSet::FindLocationOffset, but the
    // DexRegisterLocationSet is not yet built.  Try to factor common code.
    size_t offset = origin + DexRegisterLocationSet::kFixedSize;

    // Skip the first `number_of_dex_locations - 1` entries.
    for (uint16_t i = 0; i < number_of_dex_locations; ++i) {
      // Read the first next byte and inspect its first 3 bits to decide
      // whether it is a short or a large location.
      DexRegisterLocationSet::ShortLocation first_byte =
          region_.LoadUnaligned<DexRegisterLocationSet::ShortLocation>(offset);
      DexRegisterLocation::Kind kind =
          DexRegisterLocationSet::ExtractKindFromShortLocation(first_byte);
      if (DexRegisterLocation::IsShortLocationKind(kind)) {
        // Short location.  Skip the current byte.
        offset += DexRegisterLocationSet::SingleShortEntrySize();
      } else {
        // Large location.  Skip the 5 next bytes.
        offset += DexRegisterLocationSet::SingleLargeEntrySize();
      }
    }
