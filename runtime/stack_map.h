...
class DexRegisterCompressedMap {
 public:
  ...
  // TODO: Have a unique LocationKind enum (instead of
  // DexRegisterCompressedMap::LocationKind and
  // DexRegisterMap::LocationKind).
  ...
  static const char* PrettyDescriptor(LocationKind kind) {
    switch (kind) {
      ...
      case kConstant:
        return "as constant";
      case kInStackLargeOffset:
        return "in stack (large offset)";
      case kConstantBigValue:
  ...
  // TODO: Move this out of DexRegisterCompressedMap and use it
  // instead of DexRegisterEntry in
  ...
  DexRegisterLocation GetLocationKindAndValue(uint16_t register_index) const {
    ...
    if (IsShortLocationKind(kind)) {
      ...
      return { kind, region_.Load<int32_t>(offset + sizeof(LocationKind)) };
    }
  }

  // Can the entry (`kind`, `value`) be turned into a compressed short
  // location?
