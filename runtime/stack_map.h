...
class DexRegisterLocation {
 public:
  ...
  // Required by art::StackMapStream::DexRegisterDictionaryEntriesIndices.
...
class DexRegisterDictionary {
 public:
  ...
  // Get the internal kind of entry at `dictionary_entry_index`.
  DexRegisterLocation::Kind GetLocationInternalKind(size_t dictionary_entry_index) const {
    if (dictionary_entry_index == NoLocationEntryIndex()) {
      return DexRegisterLocation::Kind::kNone;
    }
    return ExtractKindAtOffset(FindLocationOffset(dictionary_entry_index));
  }

  // Get the (surface) kind and value of entry at `dictionary_entry_index`.
  DexRegisterLocation GetDexRegisterLocation(size_t dictionary_entry_index) const {
    if (dictionary_entry_index == NoLocationEntryIndex()) {
  ...
  static constexpr size_t NoLocationEntryIndex() {
    return -1;
  }
 ...
 public:
  ...
  size_t GetDictionaryEntryIndex(uint16_t dex_register_number,
    ...
    if (!IsDexRegisterLive(dex_register_number)) {
      return DexRegisterDictionary::NoLocationEntryIndex();
    }
