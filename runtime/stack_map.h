...
class StackMap {
 public:
  ...
  uint32_t GetNativePc() const {
    return region_.Load<uint32_t>(kNativePcOffset);
  }

  void SetNativePc(uint32_t native_pc) {
    return region_.Store<uint32_t>(kNativePcOffset, native_pc);
  }
 ...
 private:
  ...
  static constexpr int kDexRegisterMapOffsetOffset = kNativePcOffset + sizeof(uint32_t);
...
class CodeInfo {
 public:
  ...
  StackMap GetStackMapForNativePc(uint32_t native_pc) {
