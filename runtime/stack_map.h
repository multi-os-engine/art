...
class StackMapEncoding {
 ...
 private:
  ...
  static constexpr size_t kInvalidNumberOfBytes = static_cast<size_t>(-1);
...
class StackMap {
 public:
  StackMap() {}
  StackMap(MemoryRegion region) : region_(region) {}
  ...
  uint32_t GetNativePcOffset(const StackMapEncoding& encoding) const {
    return LoadAt(encoding.NumberOfBytesForNativePc(),
