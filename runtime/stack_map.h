...
class StackMap {
 public:
  ...
  uint32_t GetDexPc(const EncodingInfo& enc) const;

  void SetDexPc(const EncodingInfo& enc, uint32_t dex_pc);

  uint32_t GetNativePcOffset(const EncodingInfo& enc) const;

  void SetNativePcOffset(const EncodingInfo& enc, uint32_t native_pc_offset);

  uint32_t GetDexRegisterMapOffset(const EncodingInfo& enc) const;

  void SetDexRegisterMapOffset(const EncodingInfo& enc, uint32_t offset);

  uint32_t GetInlineDescriptorOffset(const EncodingInfo& enc) const;

  void SetInlineDescriptorOffset(const EncodingInfo& enc, uint32_t offset);

  uint32_t GetRegisterMask(const EncodingInfo& enc) const;

  void SetRegisterMask(const EncodingInfo& enc, uint32_t mask);

  MemoryRegion GetStackMask(const EncodingInfo& enc) const;

  void SetStackMask(const EncodingInfo& enc, const BitVector& sp_map) {
    MemoryRegion region = GetStackMask(enc);
    sp_map.CopyTo(region.start(), region.size());
  }

  bool HasDexRegisterMap(const EncodingInfo& enc) const {
    return GetDexRegisterMapOffset(enc) != kNoDexRegisterMap;
  }

  bool HasInlineInfo(const EncodingInfo& enc) const {
    return GetInlineDescriptorOffset(enc) != kNoInlineInfo;
  }
...
class EncodingInfo {
 public:
  ...
  EncodingInfo(size_t stack_mask_size,
      ...
      : stack_mask_size_(stack_mask_size),
  ...
  static EncodingInfo ComputeFromSizes(size_t stack_mask_size,
                                       size_t inline_info_size,
                                       size_t dex_register_map_size,
                                       size_t dex_pc_max,
                                       size_t native_pc_max,
  ...
  size_t StackMapSize() const {
    return StackMap::ComputeStackMapSizeInternal(stack_mask_size_,
                                                 bytes_for_inline_info_,
                                                 bytes_for_dex_register_map_,
                                                 bytes_for_dex_pc_,
                                                 bytes_for_native_pc_,
                                                 bytes_for_register_mask_);
  }

  size_t GetStackMaskSize() const { return stack_mask_size_; }
  ...
  size_t ComputeStackMapStackMaskOffset() const {
    return ComputeStackMapRegisterMaskOffset() + (NumberOfBytesForRegisterMask() * sizeof(uint8_t));
  }
  ...
  size_t ComputeStackMapNativePcOffset() const {
    return ComputeStackMapDexPcOffset() + (NumberOfBytesForDexPc() * sizeof(uint8_t));
  }

  size_t ComputeStackMapDexRegisterMapOffset() const {
    return ComputeStackMapNativePcOffset() + (NumberOfBytesForNativePc() * sizeof(uint8_t));
  }
...
class CodeInfo {
 public:
  ...
  EncodingInfo ExtractEncoding() const {
    return EncodingInfo(GetStackMaskSize(),
  ...
  void SetEncoding(const EncodingInfo& enc) {
    region_.StoreUnaligned<uint32_t>(kStackMaskSizeOffset, enc.GetStackMaskSize());
    region_.StoreBit(kHasInlineInfoBitOffset, enc.NumberOfBytesForInlineInfo() != 0);
    SetEncodingAt(kInlineInfoBitOffset, enc.NumberOfBytesForInlineInfo());
    SetEncodingAt(kDexRegisterMapBitOffset, enc.NumberOfBytesForDexRegisterMap());
    SetEncodingAt(kDexPcBitOffset, enc.NumberOfBytesForDexPc());
    SetEncodingAt(kNativePcBitOffset, enc.NumberOfBytesForNativePc());
    SetEncodingAt(kRegisterMaskBitOffset, enc.NumberOfBytesForRegisterMask());
  }
  ...
  DexRegisterLocationCatalog GetDexRegisterLocationCatalog(const EncodingInfo& encoding) const {
  ...
  StackMap GetStackMapAt(size_t i, const EncodingInfo& encoding) const {
    size_t stack_map_size = encoding.StackMapSize();
  ...
  uint32_t GetDexRegisterLocationCatalogSize(const EncodingInfo& encoding) const {
  ...
  size_t GetStackMapsSize(const EncodingInfo& encoding) const {
    return encoding.StackMapSize() * GetNumberOfStackMaps();
  }

  uint32_t GetDexRegisterLocationCatalogOffset(const EncodingInfo& encoding) const {
    return GetStackMapsOffset() + GetStackMapsSize(encoding);
  }

  size_t GetDexRegisterMapsOffset(const EncodingInfo& encoding) const {
  ...
  InlineInfo GetInlineInfoOf(StackMap stack_map, const EncodingInfo& encoding) const {
  ...
  StackMap GetStackMapForDexPc(uint32_t dex_pc, const EncodingInfo& encoding) const {
 ...
 private:
  ...
  MemoryRegion GetStackMaps(const EncodingInfo& encoding) const {
