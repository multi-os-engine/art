...
class DexRegisterMap {
 public:
  ...
  void SetRegisterInfo(size_t offset, DexRegisterLocation::Kind kind, int32_t value) {
    if (DexRegisterLocation::IsShortLocationKind(kind)) {
      ...
      region_.Store<ShortLocation>(offset, CompressLocation(kind, value));
    } else {
  ...
  DexRegisterLocation GetLocationKindAndValue(uint16_t register_index) const {
    ...
    ShortLocation first_byte = region_.Load<ShortLocation>(offset);
    ...
    if (DexRegisterLocation::IsShortLocationKind(kind)) {
      ...
      int32_t value = region_.Load<int32_t>(offset + sizeof(DexRegisterLocation::Kind));
      if (kind == DexRegisterLocation::Kind::kInStackLargeOffset) {
  ...
  int32_t GetConstant(uint16_t register_index) const {
    ...
    DCHECK(location.kind == DexRegisterLocation::Kind::kConstant
           || location.kind == DexRegisterLocation::Kind::kConstantLargeValue);
    return location.value;
  }
  ...
  static size_t EntrySize(const DexRegisterLocation& location) {
    if (IsCompressibleLocation(location)) {
      return DexRegisterMap::SingleShortEntrySize();
    } else {
      return DexRegisterMap::SingleLargeEntrySize();
    }
  }
 ...
 private:
  ...
  static ShortLocation CompressLocation(DexRegisterLocation::Kind kind, int32_t value) {
  ...
  size_t ComputeDexRegisterMapSize(uint32_t origin, uint32_t number_of_dex_registers) const {
    ...
    for (uint16_t i = 0; i < number_of_dex_registers; ++i) {
      ...
      DexRegisterMap::ShortLocation first_byte =
          region_.Load<DexRegisterMap::ShortLocation>(offset);
