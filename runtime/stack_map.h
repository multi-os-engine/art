...
class CodeInfo {
 public:
  ...
  struct Header {
    uint8_t header_size;
    CompressedUint32 non_header_size;
    StackMapEncoding stack_map_encoding;
    CompressedUint32 number_of_stack_maps;
    CompressedUint32 stack_map_size_in_bytes;
  ...
  explicit CodeInfo(const void* data) {
    ...
    const size_t size = header->header_size + header->non_header_size.Get();
  ...
  StackMapEncoding ExtractEncoding() const {
    return GetHeader()->stack_map_encoding;
  }

  bool HasInlineInfo() const {
    return GetHeader()->stack_map_encoding.InlineInfoBitSize() > 0;
  }
  ...
  StackMap GetStackMapAt(size_t i, const StackMapEncoding& encoding) const {
    size_t stack_map_size = GetHeader()->stack_map_size_in_bytes.Get();
  ...
  size_t GetStackMapsSize(const StackMapEncoding&) const {
    return GetHeader()->stack_map_size_in_bytes.Get() * GetNumberOfStackMaps();
  }
