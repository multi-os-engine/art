...
class CodeInfo {
 public:
  ...
  void SetEncoding(size_t inline_info_size,
    ...
    if (inline_info_size != 0) {
      ...
      SetEncodingAt(kInlineInfoBitOffset, EncodingSizeInBytes(dex_register_map_size + inline_info_size + 1));
    } else {
  ...
  void SetEncodingAt(size_t offset, size_t number_of_bytes) {
    region_.StoreBit(offset, (number_of_bytes & 1));
    region_.StoreBit(offset + 1, (number_of_bytes & 2));
    region_.StoreBit(offset + 2, (number_of_bytes & 4));
  }
  ...
  size_t NumberOfBytesForEncoding(size_t offset) const {
    return region_.LoadBit(offset)
        + (region_.LoadBit(offset + 1) << 1)
        + (region_.LoadBit(offset + 2) << 2);
  }

  size_t NumberOfBytesForInlineInfo() const {
    return NumberOfBytesForEncoding(kInlineInfoBitOffset);
  }

  size_t NumberOfBytesForDexRegisterMap() const {
    return NumberOfBytesForEncoding(kDexRegisterMapBitOffset);
  }

  size_t NumberOfBytesForRegisterMask() const {
    return NumberOfBytesForEncoding(kRegisterMaskBitOffset);
  }

  size_t NumberOfBytesForNativePc() const {
    return NumberOfBytesForEncoding(kNativePcBitOffset);
  }

  size_t NumberOfBytesForDexPc() const {
    return NumberOfBytesForEncoding(kDexPcBitOffset);
  }
 ...
 private:
  ...
  static constexpr int kNumberOfStackMapsOffset = kEncodingInfoOffset + sizeof(uint16_t);
