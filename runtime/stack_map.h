...
class CodeInfo {
 public:
  ...
  void SetEncoding(size_t inline_info_size,
    ...
    if (inline_info_size != 0) {
      region_.StoreBit(kHasInlineInfoBit, 1);
      region_.StoreBit(kHasSmallInlineInfoBit, IsUint<kBitsForSmallEncoding>(
          // + 1 to also encode kNoInlineInfo.
          inline_info_size + dex_register_map_size + 1));
    } else {
      region_.StoreBit(kHasInlineInfoBit, 0);
      region_.StoreBit(kHasSmallInlineInfoBit, 0);
    }
    region_.StoreBit(kHasSmallDexRegisterMapBit,
                     // + 1 to also encode kNoDexRegisterMap.
                     IsUint<kBitsForSmallEncoding>(dex_register_map_size + 1));
    region_.StoreBit(kHasSmallDexPcBit, IsUint<kBitsForSmallEncoding>(dex_pc_max));
    region_.StoreBit(kHasSmallNativePcBit, IsUint<kBitsForSmallEncoding>(native_pc_max));
  }

  bool HasInlineInfo() const {
    return region_.LoadBit(kHasInlineInfoBit);
  }

  bool HasSmallInlineInfo() const {
    return region_.LoadBit(kHasSmallInlineInfoBit);
  }

  bool HasSmallDexRegisterMap() const {
    return region_.LoadBit(kHasSmallDexRegisterMapBit);
  }

  bool HasSmallNativePc() const {
    return region_.LoadBit(kHasSmallNativePcBit);
  }

  bool HasSmallDexPc() const {
    return region_.LoadBit(kHasSmallDexPcBit);
  }
 ...
 private:
  ...
  static constexpr int kHasInlineInfoBit = (kEncodingInfoOffset * kBitsPerByte);
  static constexpr int kHasSmallInlineInfoBit = kHasInlineInfoBit + 1;
  static constexpr int kHasSmallDexRegisterMapBit = kHasSmallInlineInfoBit + 1;
  static constexpr int kHasSmallDexPcBit = kHasSmallDexRegisterMapBit + 1;
  static constexpr int kHasSmallNativePcBit = kHasSmallDexPcBit + 1;
