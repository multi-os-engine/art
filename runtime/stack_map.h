...
class StackMap {
 public:
  ...
  uint32_t GetDexPc(const StackMapEncoding& enc) const {
    return LoadAt(enc.NumberOfBytesForDexPc(), enc.ComputeStackMapDexPcOffset());
  }

  void SetDexPc(const StackMapEncoding& enc, uint32_t dex_pc) {
    StoreAt(enc.NumberOfBytesForDexPc(), enc.ComputeStackMapDexPcOffset(), dex_pc);
  }

  uint32_t GetNativePcOffset(const StackMapEncoding& enc) const {
    return LoadAt(enc.NumberOfBytesForNativePc(), enc.ComputeStackMapNativePcOffset());
  }

  void SetNativePcOffset(const StackMapEncoding& enc, uint32_t native_pc_offset) {
    StoreAt(enc.NumberOfBytesForNativePc(), enc.ComputeStackMapNativePcOffset(), native_pc_offset);
  }

  uint32_t GetDexRegisterMapOffset(const StackMapEncoding& enc) const {
    return LoadAt(enc.NumberOfBytesForDexRegisterMap(),
                  enc.ComputeStackMapDexRegisterMapOffset(),
                  /* check_max */ true);
  }

  void SetDexRegisterMapOffset(const StackMapEncoding& enc, uint32_t offset) {
    StoreAt(enc.NumberOfBytesForDexRegisterMap(),
            enc.ComputeStackMapDexRegisterMapOffset(),
            offset);
  }
  ...
  uint32_t GetInlineDescriptorOffset(const StackMapEncoding& enc) const {
    ...
    return LoadAt(enc.NumberOfBytesForInlineInfo(),
  ...
  void SetInlineDescriptorOffset(const StackMapEncoding& enc, uint32_t offset) {
    ...
    StoreAt(enc.NumberOfBytesForInlineInfo(), enc.ComputeStackMapInlineInfoOffset(), offset);
  }

  uint32_t GetRegisterMask(const StackMapEncoding& enc) const {
    return LoadAt(enc.NumberOfBytesForRegisterMask(), enc.ComputeStackMapRegisterMaskOffset());
  }

  void SetRegisterMask(const StackMapEncoding& enc, uint32_t mask) {
    StoreAt(enc.NumberOfBytesForRegisterMask(), enc.ComputeStackMapRegisterMaskOffset(), mask);
  }

  MemoryRegion GetStackMask(const StackMapEncoding& enc) const {
    return region_.Subregion(enc.ComputeStackMapStackMaskOffset(),
                             enc.NumberOfBytesForStackMask());
  }
 ...
 private:
  ...
  ALWAYS_INLINE  uint32_t LoadAt(size_t number_of_bytes,
                                 size_t offset,
                                 bool check_max = false) const {
  ...
  ALWAYS_INLINE void StoreAt(size_t number_of_bytes, size_t offset, uint32_t value) const {
    if (number_of_bytes == 0u) {
      DCHECK_EQ(value, 0u);
    } else if (number_of_bytes == 1u) {
