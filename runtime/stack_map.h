 ...
 * DexRegisterLocations are either on 1 or 5 bytes (see art::DexRegisterLocation::Kind).
 */
class DexRegisterMap {
 public:
  ...
  static size_t LiveBitMaskSize(size_t number_of_dex_registers) {
  ...
  size_t FindLocationOffset(uint16_t dex_register_index, uint16_t number_of_dex_registers) const {
    ...
    size_t offset = kFixedSize + LiveBitMaskSize(number_of_dex_registers);
...
class CodeInfo {
 ...
 private:
  ...
  size_t ComputeDexRegisterMapSize(uint32_t origin, uint32_t number_of_dex_registers) const {
    ...
    offset += DexRegisterMap::LiveBitMaskSize(number_of_dex_registers);
