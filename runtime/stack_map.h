...
class DexRegisterCompressedMap {
 public:
  ...
  std::pair<LocationKind, int32_t> GetLocationKindAndValue(uint16_t register_index) {
    size_t entry = FindEntry(register_index);
    // Read the first byte and inspect its first 3 bits to get the location.
    CompressedShortLocation first_byte =
        region_.Load<CompressedShortLocation>(entry);
  ...
  // itself (nor in the corresponding stack map).
  size_t Size(size_t number_of_dex_registers) const {
    // The size of the map is equal to the first past-the-end entry.
    return FindEntry(number_of_dex_registers);
  }
...
class DexRegisterTable {
 public:
  ...
  // This method requires the number of dex registers to be passed as
  // argument, as this value is not stored in the Dex register table
  // itself (nor in the corresponding stack map).
  size_t Size(size_t number_of_dex_registers) const {
    return kFixedSize + number_of_dex_registers * SingleEntrySize();
  }
