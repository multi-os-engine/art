...
class StackMap {
 public:
  ...
  uint32_t GetDexPc(const StackMapEncoding& enc) const;

  void SetDexPc(const StackMapEncoding& enc, uint32_t dex_pc);

  uint32_t GetNativePcOffset(const StackMapEncoding& enc) const;

  void SetNativePcOffset(const StackMapEncoding& enc, uint32_t native_pc_offset);

  uint32_t GetDexRegisterMapOffset(const StackMapEncoding& enc) const;

  void SetDexRegisterMapOffset(const StackMapEncoding& enc, uint32_t offset);

  uint32_t GetInlineDescriptorOffset(const StackMapEncoding& enc) const;

  void SetInlineDescriptorOffset(const StackMapEncoding& enc, uint32_t offset);

  uint32_t GetRegisterMask(const StackMapEncoding& enc) const;

  void SetRegisterMask(const StackMapEncoding& enc, uint32_t mask);

  MemoryRegion GetStackMask(const StackMapEncoding& enc) const;

  void SetStackMask(const StackMapEncoding& enc, const BitVector& sp_map) {
    MemoryRegion region = GetStackMask(enc);
    sp_map.CopyTo(region.start(), region.size());
  }

  bool HasDexRegisterMap(const StackMapEncoding& enc) const {
    return GetDexRegisterMapOffset(enc) != kNoDexRegisterMap;
  }

  bool HasInlineInfo(const StackMapEncoding& enc) const {
...
class StackMapEncoding {
 public:
  ...
  static StackMapEncoding ComputeFromSizes(size_t stack_mask_size,
                                       size_t inline_info_size,
                                       size_t dex_register_map_size,
                                       size_t dex_pc_max,
                                       size_t native_pc_max,
...
class CodeInfo {
 public:
  ...
  StackMapEncoding ExtractEncoding() const {
    return StackMapEncoding(GetStackMaskSize(),
  ...
  void SetEncoding(const StackMapEncoding& enc) {
 ...
 private:
  ...
  static constexpr int kStackMapEncodingOffset = kOverallSizeOffset + sizeof(uint32_t);
  static constexpr int kNumberOfDexRegisterLocationCatalogEntriesOffset =
      kStackMapEncodingOffset + sizeof(uint16_t);
  ...
  static constexpr int kHasInlineInfoBitOffset = (kStackMapEncodingOffset * kBitsPerByte);
