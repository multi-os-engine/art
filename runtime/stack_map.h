...
class DexRegisterMap {
 public:
  ...
  static bool IsValidKind(DexRegisterLocation::Kind kind) {
    return DexRegisterLocation::IsShortLocationKind(kind);
  }

  DexRegisterLocation::Kind GetLocationKind(uint16_t register_index) const {
    DexRegisterLocation::Kind kind = region_.Load<DexRegisterLocation::Kind>(
        kFixedSize + register_index * SingleEntrySize());
    // Ensure we do not use kinds specific to DexRegisterCompressedMap.
    DCHECK(IsValidKind(kind)) << DexRegisterLocation::PrettyDescriptor(kind);
    return kind;
  }

  void SetRegisterInfo(uint16_t register_index, DexRegisterLocation::Kind kind, int32_t value) {
    // Ensure we do not use kinds specific to DexRegisterCompressedMap.
    DCHECK(IsValidKind(kind)) << DexRegisterLocation::PrettyDescriptor(kind);
  ...
  int32_t GetValue(uint16_t register_index) const {
    return region_.Load<int32_t>(
        kFixedSize + sizeof(DexRegisterLocation::Kind) + register_index * SingleEntrySize());
  }
