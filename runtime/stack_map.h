...
class InlineInfo {
 ...
 private:
  static const int kDepthOffset = 0;
  static const int kFixedSize = kDepthOffset + sizeof(uint8_t);

  static const uint32_t kNoInlineInfo = -1;
...
class StackMap {
 ...
 private:
  static const int kDexPcOffset = 0;
  static const int kNativePcOffset = kDexPcOffset + sizeof(uint32_t);
  static const int kDexRegisterMapOffsetOffset = kNativePcOffset + sizeof(T);
  static const int kInlineDescriptorOffsetOffset = kDexRegisterMapOffsetOffset + sizeof(uint32_t);
  static const int kRegisterMaskOffset = kInlineDescriptorOffsetOffset + sizeof(uint32_t);
  static const int kFixedSize = kRegisterMaskOffset + sizeof(uint32_t);
  static const int kStackMaskOffset = kFixedSize;
...
class CodeInfo {
 ...
 private:
  static const int kNumberOfStackMapsOffset = 0;
  static const int kStackMaskSizeOffset = kNumberOfStackMapsOffset + sizeof(uint32_t);
  static const int kFixedSize = kStackMaskSizeOffset + sizeof(uint32_t);
