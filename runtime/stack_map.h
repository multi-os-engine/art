...
class InlineInfo {
 public:
  InlineInfo(InlineInfoEncoding encoding, MemoryRegion region, size_t offset = 0)
  ...
  void SetDexRegisterMapOffsetAtDepth(uint32_t depth, uint32_t offset) {
 ...
 private:
  ...
  InlineInfoEncoding encoding_;  // Hold by value since InlineInfo can outlive CodeInfo.
  MemoryRegion region_;
...
struct CodeInfoHeader {
  ...
  void Encode(Vector* dest) const {
    ...
    dest->insert(dest->end(), stack_map_ptr, stack_map_ptr + sizeof(stack_map_encoding));
    if (stack_map_encoding.GetInlineInfoEncoding().BitSize() > 0) {
      const uint8_t* inline_info_ptr = reinterpret_cast<const uint8_t*>(&inline_info_encoding);
      dest->insert(dest->end(), inline_info_ptr, inline_info_ptr + sizeof(inline_info_encoding));
    }
  }
};
