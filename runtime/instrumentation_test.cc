...
class InstrumentationTest : public CommonRuntimeTest {
 public:
  ...
  static const std::string clientOneKey;
  static const std::string clientTwoKey;
  ...
  void UndeoptimizeMethod(Thread* self, Handle<mirror::ArtMethod> method,
                          const std::string& key, bool disable_deoptimization)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    Runtime* runtime = Runtime::Current();
    instrumentation::Instrumentation* instrumentation = runtime->GetInstrumentation();
    self->TransitionFromRunnableToSuspended(kSuspended);
  ...
  void DeoptimizeEverything(Thread* self, const std::string& key, bool enable_deoptimization)
        SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    Runtime* runtime = Runtime::Current();
    instrumentation::Instrumentation* instrumentation = runtime->GetInstrumentation();
    self->TransitionFromRunnableToSuspended(kSuspended);
  ...
  void UndeoptimizeEverything(Thread* self, const std::string& key, bool disable_deoptimization)
        SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    Runtime* runtime = Runtime::Current();
    instrumentation::Instrumentation* instrumentation = runtime->GetInstrumentation();
    self->TransitionFromRunnableToSuspended(kSuspended);
  ...
  void EnableMethodTracing(Thread* self, const std::string& key, bool needs_interpreter)
        SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    Runtime* runtime = Runtime::Current();
    instrumentation::Instrumentation* instrumentation = runtime->GetInstrumentation();
    self->TransitionFromRunnableToSuspended(kSuspended);
  ...
  void DisableMethodTracing(Thread* self, const std::string& key)
        SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    Runtime* runtime = Runtime::Current();
    instrumentation::Instrumentation* instrumentation = runtime->GetInstrumentation();
    self->TransitionFromRunnableToSuspended(kSuspended);
...
const std::string InstrumentationTest::clientOneKey("TestClient1");
const std::string InstrumentationTest::clientTwoKey("TestClient2");
...
TEST_F(InstrumentationTest, DeoptimizeDirectMethod) {
  ...
  const std::string instrumentation_key("DeoptimizeDirectMethod");
...
TEST_F(InstrumentationTest, FullDeoptimization) {
  ...
  const std::string instrumentation_key("FullDeoptimization");
...
//  EXPECT_TRUE(class_linker->IsQuickToInterpreterBridge(deoptimized_quick_entrypoint));
  ...
  const std::string instrumentation_key("MixedDeoptimization");
...
TEST_F(InstrumentationTest, MethodTracing_Interpreter) {
  ...
  const std::string instrumentation_key("MethodTracing");
...
TEST_F(InstrumentationTest, ConfigureStubs_Nothing) {
  ...
  // Check no-op.
  CheckConfigureStubs(clientOneKey, Instrumentation::InstrumentationLevel::kInstrumentNothing);
  CHECK_INSTRUMENTATION(Instrumentation::InstrumentationLevel::kInstrumentNothing, 0U);
}
...
TEST_F(InstrumentationTest, ConfigureStubs_InstrumentationStubs) {
  ...
  CheckConfigureStubs(clientOneKey, Instrumentation::InstrumentationLevel::kInstrumentWithInstrumentationStubs);
...
TEST_F(InstrumentationTest, ConfigureStubs_Interpreter) {
  ...
  CheckConfigureStubs(clientOneKey, Instrumentation::InstrumentationLevel::kInstrumentWithInterpreter);
...
TEST_F(InstrumentationTest, MultiConfigureStubs_Nothing) {
  ...
  CheckConfigureStubs(clientTwoKey, Instrumentation::InstrumentationLevel::kInstrumentNothing);
  CHECK_INSTRUMENTATION(Instrumentation::InstrumentationLevel::kInstrumentNothing, 0U);
}
...
TEST_F(InstrumentationTest, MultiConfigureStubs_InstrumentationStubs) {
  ...
  CheckConfigureStubs(clientTwoKey, Instrumentation::InstrumentationLevel::kInstrumentWithInstrumentationStubs);
...
TEST_F(InstrumentationTest, MultiConfigureStubs_Interpreter) {
  ...
  CheckConfigureStubs(clientTwoKey, Instrumentation::InstrumentationLevel::kInstrumentWithInterpreter);
