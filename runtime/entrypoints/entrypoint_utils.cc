  ...
  if (LIKELY(!soa.Self()->IsExceptionPending())) {
    ...
    if (exception->IsCheckedException()) {
      ...
      ArtMethod* proxy_method = rcvr->GetClass()->FindVirtualMethodForInterface(
          interface_method->GetArtMethod(), kPointerSizeCur);
      auto virtual_methods = proxy_class->GetVirtualMethodsSlice(kPointerSizeCur);
      size_t num_virtuals = proxy_class->NumVirtualMethods();
      size_t method_size = ArtMethod::Size(kPointerSizeCur);
