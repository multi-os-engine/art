...
inline ArtMethod* GetResolvedMethod(ArtMethod* outer_method,
  ...
  if (inline_info.GetDexPcAtDepth(encoding, inlining_depth) == static_cast<uint32_t>(-1)) {
    ...
    inlined_method = cls->FindVirtualMethod("charAt", "(I)C", kPointerSizeCur);
  } else {
    ...
    switch (invoke_type) {
      ...
      case kStatic:
        inlined_method =
            klass->FindDirectMethod(klass->GetDexCache(), method_index, kPointerSizeCur);
        break;
      case kSuper:
      case kVirtual:
        inlined_method =
            klass->FindVirtualMethod(klass->GetDexCache(), method_index, kPointerSizeCur);
        break;
      default:
  ...
  outer_method->SetDexCacheResolvedMethod(method_index, inlined_method, kPointerSizeCur);
  return inlined_method;
}
...
inline ArtField* FindFieldFromCode(uint32_t field_idx,
  ...
  if (access_check) {
    ...
    ArtMethod* method = referrer->GetInterfaceMethodIfProxy(kPointerSizeCur);
...
inline ArtField* FindFieldFast(uint32_t field_idx, ArtMethod* referrer, FindFieldType type,
  ...
  ArtField* resolved_field =
      referrer->GetDeclaringClass()->GetDexCache()->GetResolvedField(field_idx, kPointerSizeCur);
  if (UNLIKELY(resolved_field == nullptr)) {
    return nullptr;
  }
...
inline ArtMethod* FindMethodFast(uint32_t method_idx, mirror::Object* this_object,
  ...
  ArtMethod* resolved_method =
      referring_class->GetDexCache()->GetResolvedMethod(method_idx, kPointerSizeCur);
  if (UNLIKELY(resolved_method == nullptr)) {
    return nullptr;
  }
  if (access_check) {
  ...
  if (type == kInterface) {  // Most common form of slow path dispatch.
    return this_object->GetClass()->FindVirtualMethodForInterface(resolved_method, kPointerSizeCur);
    ...
    if (method_reference_class == nullptr) {
      ...
      return super_class->GetVTableEntry(resolved_method->GetMethodIndex(), kPointerSizeCur);
    } else {
      return method_reference_class->FindVirtualMethodForInterfaceSuper(
          resolved_method, kPointerSizeCur);
    }
  } else {
    DCHECK(type == kVirtual);
    return this_object->GetClass()->GetVTableEntry(
        resolved_method->GetMethodIndex(), kPointerSizeCur);
  }
}
