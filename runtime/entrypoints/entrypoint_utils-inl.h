  ...
  switch (type) {
    ...
    case kSuper: {
      ...
      if (method_reference_class == nullptr) {
        // I dont't think this should ever happen.
        ThrowNoSuchMethodError(type, resolved_method->GetDeclaringClass(),
                               resolved_method->GetName(), resolved_method->GetSignature());
        return nullptr;
        ...
        // TODO Put in a real finder.
        ArtMethod* result = method_reference_class->FindVirtualMethodForInterfaceSuper(
            resolved_method, class_linker->GetImagePointerSize());
        if (result == nullptr) {
          ThrowAbstractMethodError(method_idx, referring_class->GetDexFile());
          return nullptr;
        } else {
          return result;
        }
...
inline ArtMethod* FindMethodFast(uint32_t method_idx, mirror::Object* this_object,
  ...
  if (type == kInterface) {  // Most common form of slow path dispatch.
    ...
    if (method_reference_class == nullptr) {
      ...
      // It is not an interface.
      return referring_class->GetSuperClass()->GetVTableEntry(resolved_method->GetMethodIndex(),
                                                              sizeof(void*));
    } else {
