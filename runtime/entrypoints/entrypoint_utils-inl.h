...
inline ArtMethod* FindMethodFromCode(uint32_t method_idx, mirror::Object** this_object,
  ...
  switch (type) {
    ...
    case kVirtual: {
      ...
      if (access_check &&
          (!klass->HasVTable() ||
           vtable_index >= static_cast<uint32_t>(klass->GetVTableLength(sizeof(void*))))) {
      ...
      return klass->GetVTableEntry(vtable_index, sizeof(void*));
    }
    case kSuper: {
      ...
      if (access_check) {
        ...
        if (super_class == nullptr || !super_class->HasVTable() ||
            vtable_index >= static_cast<uint32_t>(super_class->GetVTableLength(sizeof(void*)))) {
          // Behavior to agree with that of the verifier.
          ThrowNoSuchMethodError(type, resolved_method->GetDeclaringClass(),
                                 resolved_method->GetName(), resolved_method->GetSignature());
          return nullptr;  // Failure.
      ...
      return super_class->GetVTableEntry(vtable_index, sizeof(void*));
    }
    case kInterface: {
      ...
      ArtMethod* imt_method = (*this_object)->GetClass()->GetEmbeddedImTableEntry(
          imt_index, sizeof(void*));
      if (!imt_method->IsImtConflictMethod() && !imt_method->IsImtUnimplementedMethod()) {
        if (kIsDebugBuild) {
          mirror::Class* klass = (*this_object)->GetClass();
          ArtMethod* method = klass->FindVirtualMethodForInterface(resolved_method, sizeof(void*));
...
inline mirror::String* ResolveStringFromCode(ArtMethod* referrer,
                                             uint32_t string_idx) {
  ClassLinker* class_linker = Runtime::Current()->GetClassLinker();
