...
inline ArtMethod* GetResolvedMethod(ArtMethod* outer_method,
  ...
  ScopedAssertNoThreadSuspension(Thread::Current(), __FUNCTION__);
  ...
  if (inline_info.GetDexPcAtDepth(encoding, inlining_depth) == static_cast<uint32_t>(-1)) {
    ...
    if (kIsDebugBuild) {
      ...
      DCHECK(std::string(name) == "charAt");
      DCHECK(std::string(dex_file->GetMethodShorty(method_id)) == "CI")
          << std::string(dex_file->GetMethodShorty(method_id));
      DCHECK(std::string(dex_file->StringByTypeIdx(method_id.class_idx_)) == "Ljava/lang/String;")
    ...
    DCHECK_EQ(klass->GetDexCache(), caller->GetDexCache())
        << "Compiler only supports inlining calls within the same dex cache";
    switch (invoke_type) {
      case kDirect: FALLTHROUGH_INTENDED;
      case kStatic:
        inlined_method =
