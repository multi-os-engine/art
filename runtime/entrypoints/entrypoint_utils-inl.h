    ...
    case kVirtual: {
      ...
      if (access_check &&
          (!klass->ShouldHaveEmbeddedImtAndVTable() ||
           vtable_index >= static_cast<uint32_t>(klass->GetEmbeddedVTableLength()))) {
      ...
      return klass->GetEmbeddedVTableEntry(vtable_index);
    }
    case kSuper: {
      ...
      if (access_check) {
        ...
        if (super_class == nullptr || !super_class->ShouldHaveEmbeddedImtAndVTable() ||
            vtable_index >= static_cast<uint32_t>(super_class->GetEmbeddedVTableLength())) {
          // Behavior to agree with that of the verifier.
          ThrowNoSuchMethodError(type, resolved_method->GetDeclaringClass(),
                                 resolved_method->GetName(), resolved_method->GetSignature());
          return nullptr;  // Failure.
      ...
      DCHECK(super_class->ShouldHaveEmbeddedImtAndVTable());
      return super_class->GetEmbeddedVTableEntry(vtable_index);
    }
    case kInterface: {
...
static inline mirror::ArtMethod* FindMethodFast(uint32_t method_idx,
  ...
  if (type == kInterface) {  // Most common form of slow path dispatch.
    ...
    return referrer->GetDeclaringClass()->GetSuperClass()
                   ->GetEmbeddedVTableEntry(resolved_method->GetMethodIndex());
  } else {
    DCHECK(type == kVirtual);
    return this_object->GetClass()->GetEmbeddedVTableEntry(resolved_method->GetMethodIndex());
  }
}
