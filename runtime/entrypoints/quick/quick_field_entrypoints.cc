...
// TODO
extern "C" uint8_t artGet8StaticFromCode(uint32_t field_idx,
  ...
  if (LIKELY(field != NULL)) {
    return field->Get8(field->GetDeclaringClass());
  }
...
// TODO
extern "C" uint16_t artGet16StaticFromCode(uint32_t field_idx,
                                           mirror::ArtMethod* referrer,
                                           Thread* self, StackReference<mirror::ArtMethod>* sp)
    SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  LOG(FATAL) << "Not implemented";
  mirror::ArtField* field = FindFieldFast(field_idx, referrer, StaticPrimitiveRead,
                                          sizeof(int16_t));
  if (LIKELY(field != NULL)) {
    return field->Get16(field->GetDeclaringClass());
  }
  FinishCalleeSaveFrameSetup(self, sp, Runtime::kRefsOnly);
...
// TODO
extern "C" uint8_t artGet8InstanceFromCode(uint32_t field_idx, mirror::Object* obj,
...
// TODO
extern "C" uint16_t artGet16InstanceFromCode(uint32_t field_idx, mirror::Object* obj,
                                             mirror::ArtMethod* referrer, Thread* self,
                                             StackReference<mirror::ArtMethod>* sp)
    SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  LOG(FATAL) << "Not implemented";
...
// TODO
extern "C" int artSet8StaticFromCode(uint32_t field_idx, uint8_t new_value,
                                      mirror::ArtMethod* referrer, Thread* self,
                                      StackReference<mirror::ArtMethod>* sp)
    SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  LOG(FATAL) << "Not implemented";
  ...
  if (LIKELY(field != NULL)) {
    ...
    field->Set8<false>(field->GetDeclaringClass(), new_value);
    return 0;  // success
  }
  FinishCalleeSaveFrameSetup(self, sp, Runtime::kRefsOnly);
...
extern "C" int artSet16StaticFromCode(uint32_t field_idx, uint16_t new_value,
  ...
  if (LIKELY(field != NULL)) {
    // Compiled code can't use transactional mode.
    field->Set16<false>(field->GetDeclaringClass(), new_value);
    return 0;  // success
  }
  FinishCalleeSaveFrameSetup(self, sp, Runtime::kRefsOnly);
...
extern "C" int artSet8InstanceFromCode(uint32_t field_idx, mirror::Object* obj, uint8_t new_value,
  ...
  if (LIKELY(field != NULL && obj != NULL)) {
    // Compiled code can't use transactional mode.
    field->Set8<false>(obj, new_value);
    return 0;  // success
  }
  FinishCalleeSaveFrameSetup(self, sp, Runtime::kRefsOnly);
  {
...
extern "C" int artSet16InstanceFromCode(uint32_t field_idx, mirror::Object* obj, uint16_t new_value,
  ...
  if (LIKELY(field != NULL && obj != NULL)) {
    // Compiled code can't use transactional mode.
    field->Set16<false>(obj, new_value);
    return 0;  // success
  }
  FinishCalleeSaveFrameSetup(self, sp, Runtime::kRefsOnly);
  {
