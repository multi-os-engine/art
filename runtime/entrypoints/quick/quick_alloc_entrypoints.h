...
// starting it does hold the mutator lock until after it has been added to the thread list. However,
// Thread::Init is guarded by the runtime shutdown lock, so we can prevent these races by holding
// the runtime shutdown lock and the mutator lock when we update the entrypoints.
