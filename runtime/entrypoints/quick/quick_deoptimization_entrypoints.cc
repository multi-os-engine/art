...
extern "C" NO_RETURN void artDeoptimizeFromCompiledCode(Thread* self)
  ...
  // We cannot smash the caller-saves, as we need the ArtMethod in a param register that would be
  // caller-saved. This has the downside that we can't track incorrect register usage down the line.
