...
class QuickArgumentVisitor {
  ...
  static constexpr size_t kBytesPerFprSpillLocation = 8;  // FPR spill size is 8 bytes.
  static constexpr size_t kQuickCalleeSaveFrame_RefAndArgs_Fpr1Offset = 80;  // Offset of first FPR arg.
  ...
  static size_t GprIndexToGprOffset(uint32_t gpr_index) {
    switch (gpr_index) {
      case 0: return (5 * kBytesPerGprSpillLocation);
      case 1: return (0 * kBytesPerGprSpillLocation);
      case 2: return (1 * kBytesPerGprSpillLocation);
      case 3: return (6 * kBytesPerGprSpillLocation);
      case 4: return (7 * kBytesPerGprSpillLocation);
      default:
  ...
  byte* GetParamAddress() const {
    if (!kSoftFloatAbi) {
      ...
      if (UNLIKELY((type == Primitive::kPrimDouble) || (type == Primitive::kPrimFloat))) {
        if ((kNumFprArgs != 0) && (fpr_index_ < kNumFprArgs)) {
  ...
  static size_t StackArgumentStartFromShorty(bool is_static, const char* shorty,
    ...
    if (kSoftFloatAbi) {
      ...
      if (!is_static && (gprs_seen < kNumGprArgs)) {
        gprs_seen++;
        offset += kBytesPerGprSpillLocation;
      }
      for (uint32_t i = 1; i < shorty_len; ++i) {
        switch (shorty[i]) {
          ...
          case 'J':
            ...
            break;
          case 'F':
            if ((kNumFprArgs != 0) && (fprs_seen < kNumFprArgs)) {
              fprs_seen++;
              offset += kBytesPerFprSpillLocation;
            }
            break;
          case 'D':
            if ((kNumFprArgs != 0) && (fprs_seen < kNumFprArgs)) {
