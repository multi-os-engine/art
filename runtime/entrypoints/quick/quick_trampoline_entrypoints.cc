...
extern "C" uint64_t artQuickToInterpreterBridge(mirror::ArtMethod* method, Thread* self,
  ...
  if (method->IsAbstract()) {
    ...
    if (Dbg::IsForcedInterpreterNeededForUpcall(self, caller)) {
...
extern "C" const void* artQuickResolutionTrampoline(mirror::ArtMethod* called,
  ...
  if (LIKELY(!self->IsExceptionPending())) {
    ...
    // TODO: The called method is not above of the class initializer in the call stack,
    // this is a temporal workaround for that.
    ...
    // TODO: This line belongs to the workaround above.
    self->PopManagedStackFragment(fragment);

    if (LIKELY(called_class->IsInitialized())) {
      if (Dbg::IsForcedInterpreterNeededForCalling(self, called)) {
        code = GetQuickToInterpreterBridge();
      } else if (Dbg::IsForcedInstrumentationNeededForResolution(self, caller)) {
