...
extern "C" uint64_t artQuickProxyInvokeHandler(mirror::ArtMethod* proxy_method,
  ...
  DCHECK(!proxy_mh.GetMethod()->IsStatic()) << PrettyMethod(proxy_method);
  std::vector<jvalue> args;
  uint32_t shorty_len = 0;
  const char* shorty = proxy_mh.GetMethod()->GetShorty(&shorty_len);
  BuildQuickArgumentVisitor local_ref_visitor(sp, proxy_mh.GetMethod()->IsStatic(), shorty,
                                              shorty_len, &soa, &args);
