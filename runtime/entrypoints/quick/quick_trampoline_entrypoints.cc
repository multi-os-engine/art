...
extern "C" TwoWordReturn artInvokeInterfaceTrampoline(uint32_t deadbeef ATTRIBUTE_UNUSED,
  ...
  if (LIKELY(interface_method->GetDexMethodIndex() != DexFile::kDexNoIndex)) {
    ...
    uint32_t method_index = interface_method->GetDexMethodIndex();
    ArtMethod* conflict_method = imt->Get(mirror::Class::GetIMTIndex(method_index), sizeof(void*));
  ...
  if (conflict_method->IsRuntimeMethod()) {
    ...
    if (new_conflict_method != conflict_method) {
      ...
      // data is consistent.
      imt->Set(mirror::Class::GetIMTIndex(method_index),
               new_conflict_method,
               sizeof(void*));
    }
  }
