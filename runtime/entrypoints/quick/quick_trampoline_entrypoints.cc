...
extern "C" uint64_t artQuickToInterpreterBridge(ArtMethod* method, Thread* self, ArtMethod** sp)
  ...
  if (deopt_frame != nullptr) {
    ...
    // Sanity-check: are the methods as expected? We check that the last shadow frame (the bottom of
    // to call-stack) corresponds to the called method.
    ...
    // TODO: Does interpreter-to-interpreter work with that, e.g., if the caller of the deopted
    //       frame was the interpreter.
