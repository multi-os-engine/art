...
extern "C" TwoWordReturn artInvokeInterfaceTrampoline(uint32_t deadbeef ATTRIBUTE_UNUSED,
  ...
  bool is_new_entry = conflict_method->IsImtConflictMethod();

  // Create a new entry if the existing one is the shared conflict method.
  // TODO: pass the linear allocator to CreateImtConflictMethod.
  ...
  if (is_new_entry) {
    // Update the IMT if we create a new conflict method.
