...
extern "C" uint64_t artQuickToInterpreterBridge(ArtMethod* method, Thread* self, ArtMethod** sp)
  ...
  if (method->IsAbstract()) {
    ...
    auto* np_method = method->GetInterfaceMethodIfProxy(sizeof(void*));
...
extern "C" const void* artQuickResolutionTrampoline(
  ...
  *sp = called;
  ...
  // LOG(ERROR) << "TRAMPOLINE " << PrettyMethod(called) << "@" << code;
  if (PrettyMethod(called) ==
      "java.util.Properties java.lang.System.initUnchangeableSystemProperties()") {
    // LOG(ERROR) << "HERE " << sizeof(mirror::Class);
  }
  return code;
}
...
static TwoWordReturn artInvokeCommon(uint32_t method_idx, mirror::Object* this_object,
  ...
  LOG(ERROR) << "artInvokeCommon " << PrettyMethod(method) << " code=" << code;
...
#define EXPLICIT_INVOKE_COMMON_TEMPLATE_DECL(type, access_check)                                \
  ...
  TwoWordReturn artInvokeCommon<type, access_check>(                                            \
      uint32_t method_idx, mirror::Object* this_object, ArtMethod* caller_method, Thread* self, \
      ArtMethod** sp)
...
extern "C" TwoWordReturn artInvokeInterfaceTrampoline(
    ArtMethod* interface_method, mirror::Object* this_object, ArtMethod* caller_method,
    Thread* self, ArtMethod** sp) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  ScopedQuickEntrypointChecks sqec(self);
  ArtMethod* method;
