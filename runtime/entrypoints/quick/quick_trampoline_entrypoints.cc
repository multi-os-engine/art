...
#error "Unsupported architecture"
 ...
 private:
  inline void PushGpr(uintptr_t val) {
    delegate_->PushGpr(val);
  }
  inline void PushFpr4(float val) {
    delegate_->PushFpr4(val);
  }
  inline void PushFpr8(uint64_t val) {
    delegate_->PushFpr8(val);
  }
  inline void PushStack(uintptr_t val) {
    delegate_->PushStack(val);
  }
  inline uintptr_t PushSirt(mirror::Object* ref) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
...
class BuildGenericJniFrameSizeCompute FINAL {
 public:
  BuildGenericJniFrameSizeCompute() : num_sirt_references_(0), num_stack_entries_(0) {}
  ...
  uint32_t GetFirstSirtEntryOffset() {
    return 8 + sizeof(StackReference<mirror::Object>);
  }
  ...
  void ComputeLayout(bool is_static, const char* shorty, uint32_t shorty_len, void* sp,
    ...
    *table = reinterpret_cast<StackIndirectReferenceTable*>(sp8);
    // TODO: null out the padding SirtRefs here?

    // now align the call stack under the Sirt
    sp8 -= GetStackSize();
    // This aligns by 16.
    uintptr_t mask = ~0x0F;
    ...
    *start_stack = reinterpret_cast<uintptr_t*>(sp8);
    ...
    size_t fregs = BuildGenericJniFrameStateMachine<BuildGenericJniFrameSizeCompute>::kNumNativeFprArgs;
    sp8 -= fregs * sizeof(uintptr_t);
    *start_fpr = reinterpret_cast<uint32_t*>(sp8);
    size_t iregs = BuildGenericJniFrameStateMachine<BuildGenericJniFrameSizeCompute>::kNumNativeGprArgs;
  ...
  void ComputeAll(bool is_static, const char* shorty, uint32_t shorty_len)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    BuildGenericJniFrameStateMachine<BuildGenericJniFrameSizeCompute> sm(this);
...
class BuildGenericJniFrameVisitor FINAL : public QuickArgumentVisitor {
 public:
  BuildGenericJniFrameVisitor(mirror::ArtMethod** sp, bool is_static, const char* shorty,
    ...
    BuildGenericJniFrameSizeCompute fsc;
...
extern "C" ssize_t artQuickGenericJniTrampoline(Thread* self, mirror::ArtMethod** sp)
  ...
  *(sp32-1) = cookie;
  ...
  size_t window_size = visitor.GetWindowSize();
