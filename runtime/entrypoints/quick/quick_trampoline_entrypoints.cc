...
class QuickArgumentVisitor {
  ...
  static constexpr bool kQuickSoftFloatAbi = true;  // This is a hard float ABI.
  static constexpr bool kQuickDoubleRegistersAligned = false;
  static constexpr size_t kNumQuickGprArgs = 3;  // 3 arguments passed in GPRs.
  static constexpr size_t kNumQuickFprArgs = 0;  // 16 arguments passed in FPRs.
#else
  static constexpr bool kQuickSoftFloatAbi = false;  // This is a hard float ABI.
  static constexpr bool kQuickDoubleRegistersAligned = true;
  static constexpr size_t kNumQuickGprArgs = 3;  // 3 arguments passed in GPRs.
  static constexpr size_t kNumQuickFprArgs = 16;  // 16 arguments passed in FPRs.
#endif
...
#error "Unsupported architecture"
 ...
 public:
  ...
  QuickArgumentVisitor(StackReference<mirror::ArtMethod>* sp, bool is_static, const char* shorty,
    ...
    COMPILE_ASSERT(!(kQuickSoftFloatAbi && kQuickDoubleRegistersAligned), kdouble_align_unexpected);
    // For register alignment, we want to assume that counters(fpr_double_index_) are even if the
    // next register is even.
    COMPILE_ASSERT(!kQuickDoubleRegistersAligned || kNumQuickFprArgs % 2 == 0,
  ...
  byte* GetParamAddress() const {
    if (!kQuickSoftFloatAbi) {
      ...
      if (UNLIKELY((type == Primitive::kPrimDouble) || (type == Primitive::kPrimFloat))) {
        if (UNLIKELY(type == Primitive::kPrimDouble && kQuickDoubleRegistersAligned)) {
          if (fpr_double_index_ + 1 < kNumQuickFprArgs + 1) {
  ...
  void VisitArguments() SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    ...
    gpr_index_ = 0;
    fpr_index_ = 0;
    if (kQuickDoubleRegistersAligned) {
      fpr_double_index_ = 0;
    }
    stack_index_ = 0;
        ...
        case Primitive::kPrimLong:
          if (kQuickSoftFloatAbi || (cur_type_ == Primitive::kPrimLong)) {
            ...
            is_split_long_or_double_ = (GetBytesPerFprSpillLocation(kRuntimeISA) == 4) &&
                ((fpr_index_ + 1) == kNumQuickFprArgs) && !kQuickDoubleRegistersAligned;
            Visit();
            if (kBytesStackArgLocation == 4) {
              stack_index_+= 2;
            } else {
 ...
 private:
  ...
  uint32_t fpr_double_index_;  // Index into spilled FPRs for aligned double.
