...
static inline mirror::ArtMethod* FindMethodFromCode(uint32_t method_idx,
  ...
  // Don't need to save the this object unless we need to do a full resolve.
  mirror::ArtMethod* resolved_method =
      class_linker->GetResolvedMethod(self, method_idx, *referrer, type);
  if (UNLIKELY(resolved_method == nullptr)) {
    {
      StackHandleScope<2> hs(self);
      mirror::Object* dummy = nullptr;
      HandleWrapper<mirror::Object> h_this(hs.NewHandleWrapper(type == kStatic ? &dummy :
          this_object));
