...
static ArtMethod* FindVirtualMethod(mirror::Object* receiver, ArtMethod* method)
    SHARED_REQUIRES(Locks::mutator_lock_) {
  return receiver->GetClass()->FindVirtualMethodForVirtualOrInterface(method, kPointerSizeCur);
}
...
static void InvokeWithArgArray(const ScopedObjectAccessAlreadyRunnable& soa,
  ...
  if (UNLIKELY(soa.Env()->check_jni)) {
    CheckMethodArguments(soa.Vm(), method->GetInterfaceMethodIfProxy(kPointerSizeCur), args);
  }
...
JValue InvokeWithVarArgs(const ScopedObjectAccessAlreadyRunnable& soa, jobject obj, jmethodID mid,
  ...
  const char* shorty = method->GetInterfaceMethodIfProxy(kPointerSizeCur)->GetShorty(&shorty_len);
  JValue result;
...
jobject InvokeMethod(const ScopedObjectAccessAlreadyRunnable& soa, jobject javaMethod,
  ...
  if (!m->IsStatic()) {
    ...
    if (declaring_class->IsStringClass() && m->IsConstructor()) {
      ...
      m = receiver->GetClass()->FindVirtualMethodForVirtualOrInterface(m, kPointerSizeCur);
    }
  }

  // Get our arrays of arguments and their types, and check they're the same size.
  auto* objects = soa.Decode<mirror::ObjectArray<mirror::Object>*>(javaArgs);
  auto* np_method = m->GetInterfaceMethodIfProxy(kPointerSizeCur);
