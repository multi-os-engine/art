...
MonitorPool::MonitorPool()
    : current_chunk_list_(0), num_chunks_(0), capacity_(0), first_free_(nullptr) {
...
void MonitorPool::AllocateChunk() {
  ...
  if (num_chunks_ == capacity_) {
    if (capacity_ != 0U) {
      ++current_chunk_list_;
      CHECK_LT(current_chunk_list_, kMaxChunkLists);
      VLOG(monitor) << "Expanding to capacity "
          << 2 * ChunkListCapacity(current_chunk_list_) - kInitialChunkStorage;
    }  // else we're initializing
    capacity_ = ChunkListCapacity(current_chunk_list_);
    uintptr_t* new_list = new uintptr_t[capacity_]();
    DCHECK(monitor_chunks_[current_chunk_list_] == nullptr);
    monitor_chunks_[current_chunk_list_] = new_list;
    num_chunks_ = 0;
  }
  ...
  // Add the chunk.
  monitor_chunks_[current_chunk_list_][num_chunks_] = reinterpret_cast<uintptr_t>(chunk);
  num_chunks_++;
  ...
  // Eagerly compute id.
  last->monitor_id_ = OffsetToMonitorId(current_chunk_list_* (kMaxListSize * kChunkSize)
...
void MonitorPool::FreeInternal() {
  ...
  if (capacity_ == 0) return;   // Never initialized.
  for (size_t i = 0; i <= current_chunk_list_; ++i) {
    ...
    for (size_t j = 0; j < ChunkListCapacity(i); ++j) {
      if (i < current_chunk_list_ || j < num_chunks_) {
