...
std::string CommonRuntimeTest::GetOatFileName(const std::string& jar_prefix) {
  if (IsHost()) {
    const char* host_dir = getenv("ANDROID_HOST_OUT");
    CHECK(host_dir != nullptr);
    return StringPrintf("%s/framework/%s.art", host_dir, jar_prefix.c_str());
  }
  return StringPrintf("%s/framework/%s.art", GetAndroidRoot(), jar_prefix.c_str());
}
