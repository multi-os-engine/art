...
CommonRuntimeTestBase::CommonRuntimeTestBase() {}
...
void CommonRuntimeTestBase::SetUpAndroidRoot() {
  if (IsHost()) {
...
void CommonRuntimeTestBase::SetUpAndroidData(std::string& android_data) {
...
void CommonRuntimeTestBase::TearDownAndroidData(const std::string& android_data,
                                                bool fail_on_error) {
  if (fail_on_error) {
...
std::string CommonRuntimeTestBase::GetAndroidHostToolsDir() {
  return GetAndroidToolsDir("prebuilts/gcc/linux-x86/host",
                            "x86_64-linux-glibc2.15",
                            "x86_64-linux");
}

std::string CommonRuntimeTestBase::GetAndroidTargetToolsDir(InstructionSet isa) {
  switch (isa) {
    case kArm:
    case kThumb2:
...
std::string CommonRuntimeTestBase::GetCoreArtLocation() {
  return GetCoreFileLocation("art");
}

std::string CommonRuntimeTestBase::GetCoreOatLocation() {
  return GetCoreFileLocation("oat");
}

std::unique_ptr<const DexFile> CommonRuntimeTestBase::LoadExpectSingleDexFile(
    const char* location) {
...
void CommonRuntimeTestBase::SetUp() {
  SetUpAndroidRoot();
...
void CommonRuntimeTestBase::FinalizeSetup() {
...
void CommonRuntimeTestBase::ClearDirectory(const char* dirpath) {
...
void CommonRuntimeTestBase::TearDown() {
...
std::vector<std::string> CommonRuntimeTestBase::GetLibCoreDexFileNames() {
  return std::vector<std::string>({GetDexFileName("core-oj", IsHost()),
                                   GetDexFileName("core-libart", IsHost())});
}

std::string CommonRuntimeTestBase::GetTestAndroidRoot() {
  if (IsHost()) {
    const char* host_dir = getenv("ANDROID_HOST_OUT");
    CHECK(host_dir != nullptr);
    return host_dir;
...
std::string CommonRuntimeTestBase::GetTestDexFileName(const char* name) {
  CHECK(name != nullptr);
  std::string filename;
  if (IsHost()) {
...
std::vector<std::unique_ptr<const DexFile>> CommonRuntimeTestBase::OpenTestDexFiles(
    const char* name) {
...
std::unique_ptr<const DexFile> CommonRuntimeTestBase::OpenTestDexFile(const char* name) {
  std::vector<std::unique_ptr<const DexFile>> vector = OpenTestDexFiles(name);
  EXPECT_EQ(1U, vector.size());
  return std::move(vector[0]);
}

std::vector<const DexFile*> CommonRuntimeTestBase::GetDexFiles(jobject jclass_loader) {
...
const DexFile* CommonRuntimeTestBase::GetFirstDexFile(jobject jclass_loader) {
...
jobject CommonRuntimeTestBase::LoadDex(const char* dex_name) {
...
std::string CommonRuntimeTestBase::GetCoreFileLocation(const char* suffix) {
