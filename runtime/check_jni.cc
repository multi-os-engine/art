...
class ScopedCheck {
 public:
  explicit ScopedCheck(JNIEnv* env, int flags, const char* functionName)
      : function_name_(functionName), flags_(flags), indent_(0), has_method_(true) {
    CheckThread(env, flags);
  }

  // For JavaVM* functions.
  // TODO: it's not correct that this is a lock function, but making it so aids annotalysis.
  explicit ScopedCheck(bool has_method, const char* functionName)
      : function_name_(functionName), flags_(kFlag_Invocation), indent_(0),
   ...
   */
  void CheckFieldType(ScopedObjectAccess& soa, jvalue value, jfieldID fid, char prim, bool isStatic)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    StackHandleScope<1> hs(Thread::Current());
    Handle<mirror::ArtField> f(hs.NewHandle(CheckFieldID(soa, fid)));
    if (f.Get() == nullptr) {
      return;
    }
    ...
    if (!field_type->IsPrimitive()) {
      ...
      if (java_object != nullptr) {
        ...
        if (!Runtime::Current()->GetHeap()->IsValidObjectAddress(obj)) {
          ...
          if (!obj->InstanceOf(field_type)) {
            AbortF("attempt to set field %s with value of wrong type: %s",
      ...
      AbortF("attempt to set field %s with value of wrong type: %c",
    ...
    if (isStatic != f.Get()->IsStatic()) {
      if (isStatic) {
        AbortF("accessing non-static field %s as static",
               PrettyField(f.Get()).c_str());
      } else {
        AbortF("accessing static field %s as non-static",
               PrettyField(f.Get()).c_str());
      }
      return;
    }
   ...
   */
  void CheckInstanceFieldID(ScopedObjectAccess& soa, jobject java_object, jfieldID fid)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
   ...
   */
  void CheckStaticFieldID(ScopedObjectAccess& soa, jclass java_class, jfieldID fid)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  ...
  bool CheckInstantiable(ScopedObjectAccess& soa, jclass jc)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    mirror::Class* c = soa.Decode<mirror::Class*>(jc);
    if (!c->IsInstantiable()) {
      AbortF("can't make objects of type %s: %p",
             PrettyDescriptor(c).c_str(), c);
      return false;
    }
    return true;
  }
  ...
  bool CheckPrimitiveArrayType(ScopedObjectAccess& soa, jarray array, Primitive::Type type)
    ...
    return a->GetClass()->GetPrimitiveType() == type;
  }
 ...
 private:
  ...
  bool CheckNonHeapValue(char fmt, JniValueType arg) {
    if (strchr("BCDFIJSfmpEv.", fmt) == nullptr) {
      // If a format we want to process.
      switch (fmt) {
        case 'b':
          return CheckBoolean(arg.b);
        case 'Z':
  ...
  void CheckThread(JNIEnv* env, int flags) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    Thread* self = Thread::Current();
    if (self == nullptr) {
...
class CheckJNI {
 public:
  ...
  static void FatalError(JNIEnv* env, const char* msg) {
    ...
    ScopedObjectAccess soa(env);
    ScopedCheck sc(env, kFlag_ExcepOkay | kFlag_NullableUtf, __FUNCTION__);
  ...
  static jobject AllocObject(JNIEnv* env, jclass c) {
    ...
    ScopedCheck sc(env, kFlag_Default, __FUNCTION__);
    JniValueType args[2] = {{.E = env}, {.c = c}};
    if (sc.Check(soa, true, "Ec", args) && sc.CheckInstantiable(soa, c)) {
      JniValueType result;
  ...
  static jobject NewObjectV(JNIEnv* env, jclass c, jmethodID mid, va_list vargs) {
    ...
    if (sc.Check(soa, true, "Ecm.", args) && sc.CheckInstantiable(soa, c) &&
...
#undef FIELD_ACCESSORS
  ...
  static void CallVoidMethodA(JNIEnv* env, jobject obj, jmethodID mid, jvalue* vargs) {
    CallMethodA(__FUNCTION__, env, obj, nullptr, mid, vargs, Primitive::kPrimNot, kVirtual);
  }

  static void CallNonvirtualVoidMethodA(JNIEnv* env, jobject obj, jclass c, jmethodID mid,
                                        jvalue* vargs) {
    CallMethodA(__FUNCTION__, env, obj, c, mid, vargs, Primitive::kPrimNot, kDirect);
  }

  static void CallStaticVoidMethodA(JNIEnv* env, jclass c, jmethodID mid, jvalue* vargs) {
    CallMethodA(__FUNCTION__, env, c, nullptr, mid, vargs, Primitive::kPrimNot, kStatic);
  }

  static void CallVoidMethodV(JNIEnv* env, jobject obj, jmethodID mid, va_list vargs) {
    CallMethodV(__FUNCTION__, env, obj, nullptr, mid, vargs, Primitive::kPrimNot, kVirtual);
  }

  static void CallNonvirtualVoidMethodV(JNIEnv* env, jobject obj, jclass c, jmethodID mid,
                                        va_list vargs) {
    CallMethodV(__FUNCTION__, env, obj, c, mid, vargs, Primitive::kPrimNot, kDirect);
  }

  static void CallStaticVoidMethodV(JNIEnv* env, jclass c, jmethodID mid, va_list vargs) {
    CallMethodV(__FUNCTION__, env, nullptr, c, mid, vargs, Primitive::kPrimNot, kStatic);
  }
...
#undef CALL
  ...
  static void GetStringRegion(JNIEnv* env, jstring string, jsize start, jsize len, jchar* buf) {
    ...
    if (sc.Check(soa, true, "Eszzp", args)) {
  ...
  static void SetObjectArrayElement(JNIEnv* env, jobjectArray array, jsize index, jobject value) {
    ...
    if (sc.Check(soa, true, "EazL", args)) {
...
#undef PRIMITIVE_ARRAY_FUNCTIONS
 ...
 private:
  ...
  static jobject NewRef(const char* function_name, JNIEnv* env, jobject obj, IndirectRefKind kind) {
    ScopedObjectAccess soa(env);
    ScopedCheck sc(env, kFlag_Default, function_name);
    JniValueType args[2] = {{.E = env}, {.L = obj}};
    if (sc.Check(soa, true, "EL", args)) {
      JniValueType result;
      switch (kind) {
  ...
  static void DeleteRef(const char* function_name, JNIEnv* env, jobject obj, IndirectRefKind kind) {
    ScopedObjectAccess soa(env);
    ScopedCheck sc(env, kFlag_ExcepOkay, function_name);
    JniValueType args[2] = {{.E = env}, {.L = obj}};
  ...
  static const void* GetStringCharsInternal(const char* function_name, JNIEnv* env, jstring string,
    ...
    ScopedCheck sc(env, flags, function_name);
  ...
  static void GetPrimitiveArrayRegion(const char* function_name, Primitive::Type type, JNIEnv* env,
    ...
    if (sc.Check(soa, true, "Eazzp", args) && sc.CheckPrimitiveArrayType(soa, array, type)) {
      switch (type) {
        case Primitive::kPrimBoolean:
