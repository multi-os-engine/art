...
//  gLogVerbosity.verifier = true;  // TODO: don't check this in!
  ...
  for (size_t i = 0; i < options.size(); ++i) {
    ...
    } else if (StartsWith(option, "-Xbootclasspathlocations:")) {
      boot_class_path_locations_string_ = option.substr(strlen("-Xbootclasspathlocations:")).data();
  ...
  if (!boot_class_path_locations_string_.empty()) {
    ...
    if (files.size() != locations.size()) {
      Usage("Number of boot class path files does not match the number of boot class path locations given: %s vs. %s\n", boot_class_path_string_.c_str(), boot_class_path_locations_string_.c_str());
      return false;
    }
  }
