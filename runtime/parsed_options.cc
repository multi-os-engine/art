...
bool ParsedOptions::DoParse(const RuntimeOptions& options,
  ...
  if (args.GetOrDefault(M::Experimental) & ExperimentalFlags::kJvmti) {
    LOG(WARNING) << "Experimental jvmti agent support has been enabled. No guarantees are made "
                 << "the completeness, accuracy, reliability, or stability of the jvmti "
    ...
    LOG(WARNING) << "jvmti agent support has not been enabled. Enable experimental jvmti agent "
                 << " support with '-XExperimental:jvmti'. Ignored options are:";
...
void ParsedOptions::Usage(const char* fmt, ...) {
  ...
  UsageMessage(stream, "  -agentlib:library=options (Experimental feature, "
                       "requires -Xexperimental:jvmti, some features might not be supported)\n");
  ...
  UsageMessage(stream, "  -Xexperimental:jvmti "
                       "(Enable new and experimental jvmti agent support)\n");
  UsageMessage(stream, "\n");
