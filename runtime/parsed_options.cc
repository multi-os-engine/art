...
std::unique_ptr<RuntimeParser> ParsedOptions::MakeParser(bool ignore_unrecognized) {
  ...
  parser_builder->
      ...
      .Define({"-Xexperimental-opcodes", "-Xnoexperimental-opcodes"})
          .WithType<bool>()
          .WithValues({true, false})
          .IntoKey(M::ExperimentalOpcodes)
      .Ignore({
...
bool ParsedOptions::Parse(const RuntimeOptions& options, bool ignore_unrecognized,
  ...
  if (args.GetOrDefault(M::ExperimentalOpcodes)) {
    LOG(WARNING) << "Experimental opcodes have been enabled. All experimental opcodes have "
        "an unstable specification and is nearly guaranteed to change over time. "
...
void ParsedOptions::Usage(const char* fmt, ...) {
  ...
  UsageMessage(stream, "  -X[no]experimental-opcodes\n"
