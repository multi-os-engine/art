...
class ArtMethod FINAL {
 public:
  ...
  mirror::Class* GetDexCacheResolvedType(uint32_t type_idx, PointerSize ptr_size)
      SHARED_REQUIRES(Locks::mutator_lock_);
  void SetDexCacheResolvedTypes(GcRoot<mirror::Class>* new_dex_cache_types, PointerSize ptr_size)
      SHARED_REQUIRES(Locks::mutator_lock_);
  ...
  mirror::Class* GetClassFromTypeIndex(uint16_t type_idx, bool resolve, PointerSize ptr_size)
      SHARED_REQUIRES(Locks::mutator_lock_);
  ...
  const void* GetEntryPointFromQuickCompiledCode() {
    return GetEntryPointFromQuickCompiledCodePtrSize(kPointerSizeCur);
  }
  ...
  ALWAYS_INLINE void SetProfilingInfo(ProfilingInfo* info) {
    SetDataPtrSize(info, kPointerSizeCur);
  }
  ...
  static MemberOffset ProfilingInfoOffset() {
    DCHECK(IsImagePointerSize(kPointerSizeCur));
    return DataOffset(kPointerSizeCur);
  }

  void* GetEntryPointFromJni() {
    DCHECK(IsNative());
    return GetEntryPointFromJniPtrSize(kPointerSizeCur);
  }
  ...
  void SetEntryPointFromJni(const void* entrypoint) {
    DCHECK(IsNative());
    SetEntryPointFromJniPtrSize(entrypoint, kPointerSizeCur);
  }
 ...
 private:
  ...
  ALWAYS_INLINE T GetNativePointer(MemberOffset offset, PointerSize pointer_size) const {
    ...
    if (pointer_size == PointerSize::kPointerSize32) {
