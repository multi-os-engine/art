...
class MANAGED ArtMethod FINAL {
 public:
  class Iterator {
   public:
    Iterator(uintptr_t ptr, size_t pointer_size)
        : ptr_(ptr), stride_(ArtMethod::ObjectSize(pointer_size)) {
    }
    void operator++() {
      ptr_ += stride_;
    }
    ArtMethod* operator*() const {
      return reinterpret_cast<ArtMethod*>(ptr_);
    }
  ...
  FrameOffset GetHandleScopeOffset() SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    constexpr size_t handle_scope_offset = sizeof(StackReference<ArtMethod>);
  ...
  ALWAYS_INLINE bool IsRuntimeMethod() SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);
 ...
 private:
  ...
  ALWAYS_INLINE T GetEntryPoint(MemberOffset offset, size_t pointer_size) const {
    const auto uptr_this = reinterpret_cast<uintptr_t>(this);
    if (pointer_size == 4) {
      return reinterpret_cast<T>(
          *reinterpret_cast<const uint32_t*>(uptr_this + offset.Uint32Value()));
    } else {
      DCHECK_EQ(pointer_size, 8u);
      auto v = *reinterpret_cast<const uint64_t*>(uptr_this + offset.Uint32Value());
      // Check that we dont lose any bits.
      DCHECK_EQ(reinterpret_cast<uint64_t>(reinterpret_cast<T>(v)), v);
  ...
  ALWAYS_INLINE void SetEntryPoint(MemberOffset offset, T new_value, size_t pointer_size) {
    ...
    if (pointer_size == 4) {
      ...
      DCHECK_EQ(static_cast<uint32_t>(ptr), ptr);  // Check that we dont lose any bits.
      *reinterpret_cast<uint32_t*>(uptr_this + offset.Uint32Value()) = static_cast<uint32_t>(ptr);
    } else {
      DCHECK_EQ(pointer_size, 8u);
      *reinterpret_cast<uint64_t*>(uptr_this + offset.Uint32Value()) =
