...
class StackVisitor {
 ...
 public:
   ...
   *     +-------------------------------+
   *     | Compiler temp region          |  ... (reg > max_special_temp_value)
   *     |      .                        |
   *     |      .                        |
   *     | V[max_special_temp_value + 2] |
   *     | V[max_special_temp_value + 1] |
   *     +-------------------------------+
   *     | OUT[outs-1]                   |
   *     | OUT[outs-2]                   |
   *     |       .                       |
   *     | OUT[0]                        |
   *     | curMethod*                    |  ... (reg == num_total_code_regs == special_temp_value) <<== sp, 16-byte aligned
   *     +===============================+
   */
  static int GetVRegOffset(const DexFile::CodeItem* code_item,
    ...
    if (reg == temp_threshold) {
      ...
      int relative_offset = (reg - (temp_threshold + 1)) * sizeof(uint32_t);
