...
class ShadowFrame {
 public:
  ...
  ArtMethod** GetMethodAddress() SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    DCHECK(method_ != nullptr);
    return &method_;
  }
...
class PACKED(4) ManagedStack {
 public:
  ...
  StackReference<ArtMethod>* GetTopQuickFrame() const {
    return top_quick_frame_;
  }

  void SetTopQuickFrame(StackReference<ArtMethod>* top) {
 ...
 private:
  StackReference<ArtMethod>* top_quick_frame_;
  ManagedStack* link_;
...
class StackVisitor {
 ...
 public:
  ...
  uint32_t* GetVRegAddrFromQuickCode(StackReference<ArtMethod>* cur_quick_frame,
  ...
  static int GetVRegOffsetFromQuickCode(const DexFile::CodeItem* code_item,
    ...
    if (reg == temp_threshold) {
       ...
       */
      int temps_start = sizeof(StackReference<ArtMethod>) +
  ...
  static int GetOutVROffset(uint16_t out_num, InstructionSet isa) {
    ...
    return sizeof(StackReference<ArtMethod>) + (out_num * sizeof(uint32_t));
  }
  ...
  StackReference<ArtMethod>* GetCurrentQuickFrame() const {
  ...
  HandleScope* GetCurrentHandleScope() const {
    StackReference<ArtMethod>* sp = GetCurrentQuickFrame();
 ...
 private:
  ...
  StackReference<ArtMethod>* cur_quick_frame_;
