...
class QuickFrame : public ManagedFrame {
 public:
  QuickFrame(StackReference<mirror::ArtMethod>* method, uintptr_t pc, Context* context)
      : method_(method), pc_(pc), context_(context) {}
  ...
  uintptr_t GetSp() const { return reinterpret_cast<uintptr_t>(method_); }
  Context* GetContext() const { return context_; }
  StackReference<mirror::ArtMethod>* GetStackReference() const { return method_; }
  ...
  uint32_t* GetVRegAddr(const DexFile::CodeItem* code_item,
    ...
    uintptr_t vreg_addr = GetSp() + offset;
  ...
  uintptr_t* CalleeSaveAddress(int num, size_t frame_size) const
    ...
    uintptr_t save_addr = GetSp() + frame_size - ((num + 1) * kPointerSize);
 ...
 private:
  ...
  StackReference<mirror::ArtMethod>* method_;
  uintptr_t pc_;
  Context* context_;
};
...
class PACKED(4) ManagedStack {
 public:
  ManagedStack()
      : link_(NULL), top_shadow_frame_(NULL), top_quick_frame_(nullptr), top_quick_frame_pc_(0) {}
  ...
  mirror::ArtMethod* GetTopMethod() const SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    return top_shadow_frame_ != nullptr
        ...
        : QuickFrame(top_quick_frame_, 0, nullptr).GetMethod();
  }
