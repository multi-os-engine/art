...
class StackVisitor {
 ...
 public:
  ...
  bool IsInInlinedFrame() const {
    return inline_visitor_ != nullptr;
  }
 ...
 private:
  ...
  size_t cur_depth_;
  // Inlined framed visitor if the current quick frame
  // has inlined methods. Null if there is no inline frame.
  InlineFrameVisitor* inline_visitor_;
