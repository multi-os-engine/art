...
static inline void DestroyShadowFrame(ShadowFrame* frame);

using ShadowFrameAllocaUniquePtr = std::unique_ptr<ShadowFrame, decltype(&DestroyShadowFrame)>;
...
#define CreateShadowFrame(num_vregs, link, method, dex_pc) ({                                \
...
//  static inline ShadowFrameAllocaUniquePtr Create(uint32_t num_vregs, ShadowFrame* link,
//                                                  ArtMethod* method, uint32_t dex_pc) {
//    size_t frame_size = ComputeSize(num_vregs);
//    void* alloca_mem = alloca(frame_size);
//    return ShadowFrameAllocaUniquePtr(
//        Create(num_vregs, link, method, dex_pc, alloca_mem), DestroyShadowFrame);
//  }
...
void DestroyShadowFrame(ShadowFrame* frame) {
  if (frame != nullptr) {
    frame->~ShadowFrame();
  }
}
