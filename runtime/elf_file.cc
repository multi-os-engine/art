...
bool ElfFile::FixupEHFrame(off_t base_address_delta,
...
struct PACKED(1) DebugLineHeader {
  ...
  uint16_t version;
  uint32_t header_length;  // TODO 32-bit specific size
  uint8_t minimum_instruction_lenght;
  ...
  uint8_t* GetNextOpcode(uint8_t* op) const {
    // only standard and special opcodes.
  ...
  uint8_t* GetDebugLineData() const {
    const uint8_t* hdr_start =
        reinterpret_cast<const uint8_t*>(&header_length) + sizeof(header_length);
    return const_cast<uint8_t*>(hdr_start + header_length);
  }
};
