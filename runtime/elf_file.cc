...
ElfFile::ElfFile(bool is_elf64, ElfFileImpl32* elf32, ElfFileImpl64* elf64)
    : is_elf64_(is_elf64), elf32_(elf32), elf64_(elf64) {
}
...
ElfFile* ElfFile::Open(File* file, bool writable, bool program_header_only, std::string* error_msg) {
  ...
  if (header[EI_CLASS] == ELFCLASS64) {
    ElfFile *elf_file = new ElfFile(true,
        nullptr,
        ElfFileImpl64::Open(file, writable, program_header_only, error_msg));
    return elf_file;
  } else if (header[EI_CLASS] == ELFCLASS32) {
    ElfFile *elf_file = new ElfFile(false,
        ElfFileImpl32::Open(file, writable, program_header_only, error_msg),
        nullptr);
    return elf_file;
  } else {
