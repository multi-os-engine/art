...
void ElfFile::GdbJITSupport() {
  ...
  const Elf32_Shdr* info = all.FindSectionByName(".debug_info");
  const Elf32_Shdr* abbrev = all.FindSectionByName(".debug_abbrev");
  const Elf32_Shdr* frame = all.FindSectionByName(".debug_frame");
  const Elf32_Shdr* str = all.FindSectionByName(".debug_str");
  if (info == NULL || abbrev == NULL || frame == NULL || str == NULL ||
...
#endif
  const int kDwarfInfoLowPCOffset = 13;
  const int kDwarfInfoHighPCOffset = 17;
  const int kDwarfInfoFirstMethodOffset = 0x15;
  const int kDwarfInfoMethodSize = 1 + 3 * sizeof(uint32_t);
  const int kDwarfInfoMethodLowPCOffset = 1 + sizeof(uint32_t);
  const int kDwarfInfoMethodHighPCOffset = 1 + 2 * sizeof(uint32_t);
  ...
  // the old method
  if (all.GetSectionHeaderNum() == kExpectedSectionsInOATFile) {
    // TODO Some of these constants are not so constant. We really need to
    // rewrite this whole thing.
    const char* kStrtabContents = ".symtab\0.strtab\0";
    const int kStrtabLength = 16;
    const int kNumSymbols = 2;
    ...
    uint32_t needed_size = KB + info->sh_size + abbrev->sh_size + frame->sh_size
                              + str->sh_size + all.GetSectionNameStringSection().sh_size + kStrtabLength;
    ...
    for (Elf32_Word i = 1; i < kExpectedSectionsInOATFile; i++) {
      ...
      if (section_header.sh_addr == 0 && section_header.sh_type != SHT_DYNSYM) {
        ...
        memcpy(jit_elf_image_+offset, kStrtabContents, kStrtabLength);
        offset += kStrtabLength;
        section_index++;
    ...
    // TODO 8 was the offset of the debug_info section. This might be variable
    // in the future.
    const int kDebugInfoSectionOffset = 8;
    byte *p = jit_elf_image_+debug_offsets[kDebugInfoSectionOffset];
    byte *end = p + all.GetSectionHeader(kDebugInfoSectionOffset).sh_size;

    // For debug_info; patch compilation using low_pc @ offset 13, high_pc at offset 17.
    IncrementUint32(p + kDwarfInfoLowPCOffset, text_start_addr);
    IncrementUint32(p + kDwarfInfoHighPCOffset, text_start_addr);
    ...
    for (p += kDwarfInfoFirstMethodOffset; p < end; p += kDwarfInfoMethodSize) {
      IncrementUint32(p + kDwarfInfoMethodLowPCOffset, text_start_addr);
      IncrementUint32(p + kDwarfInfoMethodHighPCOffset, text_start_addr);
    }

    // Now we have to handle the debug_frame method start addresses
    // TODO 10 was the offset of the debug_frame section. This might be variable
    // in the future.
    const int kDebugFrameSectionOffset = 10;
    p = jit_elf_image_+debug_offsets[kDebugFrameSectionOffset];
    end = p + all.GetSectionHeader(kDebugFrameSectionOffset).sh_size;
    ...
    *sp = all.GetSectionHeader(0);
    ...
    for (Elf32_Word i = 1; i < kExpectedSectionsInOATFile; i++) {
      ...
      if (section_header.sh_addr == 0 && section_header.sh_type != SHT_DYNSYM) {
        ...
        *sp = section_header;
        sp->sh_offset = debug_offsets[i];
        sp->sh_size += kStrtabLength; /* sizeof ".symtab\0.strtab\0" */
        sp->sh_addr = 0;
    ...
    *sp = *text_header;
    ...
    sp->sh_size = kNumSymbols * sizeof(Elf32_Sym);
    sp->sh_link = elf_hdr.e_shnum + 1;  // Link to .strtab section.
    sp->sh_info = 0;
    sp->sh_addralign = kSymbtabAlignment;
    ...
    // Fix the dwarf data.
    {
      byte* debug_info = all.Begin() + info->sh_offset;
      byte* end = debug_info + info->sh_size;
      // For debug_info; patch compilation using low_pc @ offset 13, high_pc at offset 17.
      IncrementUint32(debug_info + kDwarfInfoLowPCOffset, text_sec->sh_addr);
      IncrementUint32(debug_info + kDwarfInfoHighPCOffset, text_sec->sh_addr);

      // Now fix the low_pc, high_pc for each method address.
      // First method starts at offset 0x15, each subsequent method is 1+3*4 bytes further.
      for (debug_info += kDwarfInfoFirstMethodOffset; debug_info < end; debug_info += kDwarfInfoMethodSize) {
        IncrementUint32(debug_info + kDwarfInfoMethodLowPCOffset, text_sec->sh_addr);
        IncrementUint32(debug_info + kDwarfInfoMethodHighPCOffset, text_sec->sh_addr);
      }

      // Now we have to handle the debug_frame method start addresses
      byte* debug_frame = all.Begin() + frame->sh_offset;
      end = debug_frame + frame->sh_size;

      // Skip past the CIE.
      debug_frame += *reinterpret_cast<uint32_t *>(debug_frame) + 4;

      // And walk the FDEs.
      for (; debug_frame < end; debug_frame += *reinterpret_cast<uint32_t *>(debug_frame) + sizeof(uint32_t)) {
        IncrementUint32(debug_frame + 2 * sizeof(uint32_t), text_sec->sh_addr);
      }
    }
