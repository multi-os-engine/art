...
class DebugAbbrev {
 public:
  ...
  static DebugAbbrev* Create(const byte* dbg_abbrev, size_t dbg_abbrev_size) {
    std::unique_ptr<DebugAbbrev> abrev(new DebugAbbrev);
    ...
    while (dbg_abbrev < last) {
      ...
      if (tag.get() == NULL) {
        ...
        abrev->tags_.insert(std::pair<uint32_t, uint32_t>(tag->index_, abrev->tag_list_.size()));
        abrev->tag_list_.push_back(std::move(tag));
      }
    }
    return abrev.release();
  }
...
class DebugInfoIterator {
 public:
  static DebugInfoIterator* Create(DebugInfoHeader* header, size_t frame_size,
                                   DebugAbbrev* abrev) {
    std::unique_ptr<DebugInfoIterator> iter(new DebugInfoIterator(header, frame_size, abrev));
    if (iter->GetCurrentTag() == NULL) {
      return NULL;
    } else {
      return iter.release();
    }
  ...
  bool next() {
    if (current_entry_ == NULL || current_tag_ == NULL) {
      return false;
    }
  ...
  byte* GetPointerToField(uint8_t dwarf_field) {
    if (current_tag_ == NULL || current_entry_ == NULL || current_entry_ >= last_entry_) {
      return NULL;
    }
...
void ElfFile::GdbJITSupport() {
  ...
  if (debug_info == NULL || debug_abbrev == NULL || debug_frame == NULL ||
      debug_str == NULL || text_sec == NULL || strtab_sec == NULL || symtab_sec == NULL) {
    return;
  }
#ifdef __LP64__
  if (true) {
  ...
  if (!FixupDebugSections(
    ...
    // Failure.
    LOG(INFO) << "Failed to load GDB data";
    return;
  }
