...
ElfFile* ElfFile::Open(File* file, bool writable, bool program_header_only, std::string* error_msg) {
  ...
  if (header[EI_CLASS] == ELFCLASS64) {
    ...
    elf_file->is_elf64_ = true;
    elf_file->elf_.elf64_.reset(ElfFileImpl<Elf64_Ehdr, Elf64_Phdr, Elf64_Shdr, Elf64_Word,
        Elf64_Sword, Elf64_Addr, Elf64_Sym, Elf64_Rel, Elf64_Rela, Elf64_Dyn>
        ::Open(file, writable, program_header_only, error_msg));
    return elf_file;
  } else if (header[EI_CLASS] == ELFCLASS32) {
    ElfFile *elf_file = new ElfFile();
    elf_file->is_elf64_ = false;
    elf_file->elf_.elf32_.reset(ElfFileImpl<Elf32_Ehdr, Elf32_Phdr, Elf32_Shdr, Elf32_Word,
        Elf32_Sword, Elf32_Addr, Elf32_Sym, Elf32_Rel, Elf32_Rela, Elf32_Dyn>
        ::Open(file, writable, program_header_only, error_msg));
    return elf_file;
  } else {
