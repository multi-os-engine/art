...
class Runtime {
 public:
  ...
  MethodFrameInfo GetCalleeSaveMethodFrameInfo(CalleeSaveType type) const {
    return callee_save_method_frame_infos_[type];
  }

  MethodFrameInfo GetRuntimeMethodFrameInfo(mirror::ArtMethod* method) const;
  MethodFrameInfo GetJniMethodFrameInfo(mirror::ArtMethod* method) const
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);
 ...
 private:
  ...
  MethodFrameInfo callee_save_method_frame_infos_[kLastCalleeSaveType];
