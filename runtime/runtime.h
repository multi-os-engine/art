...
typedef SafeMap<MethodReference, SafeMap<uint32_t, std::set<uint32_t>>, MethodReferenceComparator> MethodRefToStringInitRegMap;
...
class Runtime {
 public:
  ...
  MethodRefToStringInitRegMap* GetStringInitMap() {
    return method_ref_string_init_reg_map_.get();
  }
 ...
 private:
  ...
  std::unique_ptr<MethodRefToStringInitRegMap> method_ref_string_init_reg_map_;
