 ...
 * conversion.
 *
 * Modified UTF-8 consits of a series of bytes up to 21 bit Unicode code points as follows:
...
size_t CountModifiedUtf8Chars(const char* utf8, size_t byte_count) {
  ...
  for (const char *p = first; p < end; ++p) {
    ...
    if ((ic & 0xC0) == 0x80) {
      // it's a 10xxxxxx continuation byte just ignore it
      continue;
    }
    // must be first byte of multi-byte encoding
    len++;
    if ((ic & 0xF8) == 0xF0) {
      // start of four-byte encoding, needs an extra utf16 char
      len++;
    }
  }
  return len;
...
void ConvertModifiedUtf8ToUtf16(uint16_t* utf16_data_out, const char* utf8_data_in,
                                size_t out_chars, size_t in_bytes) {
...
void ConvertUtf16ToModifiedUtf8(char* utf8_out, const uint16_t* utf16_in,
                                size_t byte_count, size_t char_count) {
