...
class ClassLinker {
 ...
 private:
  ...
  //    is the standard case and is true when the method is not overridable by a default method or
  //    the class defines a concrete implementation of the method.
  // 2) The method must be translated to a different default method. This happens when a super class
  //    implements an interface with a default method and this class implements a subinterface of
  //    the superclass's interface. We note this with CreateTranslatedMethod.
  ...
  class MethodTranslation {
   public:
    ...
    static MethodTranslation CreateConflictingMethod() {
      return MethodTranslation(Type::kConflict, nullptr);
    }

    // This slot must become an abstract method.
    static MethodTranslation CreateAbstractMethod() {
      return MethodTranslation(Type::kAbstract, nullptr);
    }
  ...
  //          if it was found.  Otherwise *out_default_method will be set to null.
  // * False - Conflicting method implementations were found when searching for target_method. The
  //           value of *out_default_method is null.
