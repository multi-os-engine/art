...
class ExecutableOatFileManager;
class Runtime;
...
class ClassLinker {
 public:
  ...
  // Finds or create the oat file holding dex_location. Then loads and returns
 ...
 private:
  ...
  // Find an opened oat dex file that is up to date according to the given oat
  // file manager.
  const OatFile* FindOpenedOatFile(ExecutableOatFileManager& oat_file_manager)
      LOCKS_EXCLUDED(dex_lock_);
