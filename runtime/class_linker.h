...
class ClassLinker {
 ...
 private:
  ...
  // Does any setup needed to ensure that a method cannot be invoked.
  void ClassLinker::EnsureNotInvocable(ArtMethod* method) SHARED_REQUIRES(Locks::mutator_lock_);
  ...
  class MethodTranslation {
   public:
    ...
    static MethodTranslation ConflictMethod() {
      return MethodTranslation(nullptr);
    }

    // Use the given method as the current value for this vtable slot during translation
    static MethodTranslation NewMethod(ArtMethod* new_method) {
      return MethodTranslation(new_method);
    }
   ...
   private:
    explicit MethodTranslation(ArtMethod* translation) : translation_(translation) {}
