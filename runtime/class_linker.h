...
class ClassLinker {
 ...
 private:
  ...
  // Links the virtual methods for the given class. Any vtable entries that might need to be updated
  // with default methods are stored into the default_translations map. The default_translations map
  // is keyed on the vtable index that needs to be updated. We use this map because if we override a
  // default method with another default method we need to update the vtable to point to the new
  // method. Unfortunately since we copy the ArtMethod* we cannot just do a simple scan, we
  // therefore store the vtable index's that might need to be updated with the method they will turn
  // into.
  // TODO This whole default_translations thing is very dirty. There should be a better way.
  bool LinkVirtualMethods(Thread* self,
                          Handle<mirror::Class> klass,
                          /*out*/std::unordered_map<int32_t, ArtMethod*>* default_translations)
      SHARED_REQUIRES(Locks::mutator_lock_);
  ...
  // into out_default_method and returns true. Otherwise, if an error occurs (such as a
  // default_method conflict) stores an appropriate message for an IncompatibleClassChangeError into
  // icce_message and returns false. The caller should then throw the IncompatibleClassChangeError.
  // Done this way due to fact that we cannot always restore the right thread state.
