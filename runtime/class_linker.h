...
class ClassLinker {
 public:
  ...
  const ClassProfile* GetClassProfile() SHARED_REQUIRES(Locks::class_profile_lock_) {
    return class_profile_.get();
  }
 ...
 private:
  ...
  // Class profile for knowing which classes are loaded.
  std::unique_ptr<ClassProfile> class_profile_ GUARDED_BY(Locks::class_profile_lock_);
