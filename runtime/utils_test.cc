...
TEST_F(UtilsTest, JniShortName_JniLongName) {
  ...
  m = c->FindVirtualMethod("charAt", "(I)C", kPointerSizeCur);
  ASSERT_TRUE(m != nullptr);
  EXPECT_EQ("Java_java_lang_String_charAt", JniShortName(m));
  EXPECT_EQ("Java_java_lang_String_charAt__I", JniLongName(m));

  m = c->FindVirtualMethod("indexOf", "(Ljava/lang/String;I)I", kPointerSizeCur);
  ASSERT_TRUE(m != nullptr);
  ...
  m = c->FindDirectMethod("copyValueOf", "([CII)Ljava/lang/String;", kPointerSizeCur);
  ASSERT_TRUE(m != nullptr);
