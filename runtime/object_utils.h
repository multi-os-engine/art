...
class MethodHelper {
 public:
  ...
  // May cause thread suspension due to GetClassFromTypeIdx calling ResolveType.
  ...
  bool HasSameSignatureWithDifferentClassLoaders(MethodHelper* other)
    ...
    const DexFile::TypeList* types = method_->GetInterfaceMethodIfProxy()->GetParameterTypeList();
    const DexFile::TypeList* other_types =
        other->method_->GetInterfaceMethodIfProxy()->GetParameterTypeList();
    if (types == nullptr) {
  ...
  mirror::Class* GetDexCacheResolvedType(uint16_t type_idx)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    return method_->GetInterfaceMethodIfProxy()->GetDexCacheResolvedTypes()->Get(type_idx);
  }
