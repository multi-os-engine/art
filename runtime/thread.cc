...
bool Thread::IsExceptionThrownByCurrentMethod(mirror::Throwable* exception) const {
  CountStackDepthVisitor count_visitor(const_cast<Thread*>(this));
  count_visitor.WalkStack();
  // The location at which we call this method has already popped native Java frames.
  // Therefore we discard native Java frames from the top of the stack recorded
  // by the exception.
  return count_visitor.GetDepth() == exception->GetStackDepthDiscardTopNatives();
}
