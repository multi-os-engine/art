...
class MonitorExitVisitor : public RootVisitor {
 public:
  ...
  void VisitRoots(mirror::CompressedReference<mirror::Object>** roots, size_t count,
                  const RootInfo& info ATTRIBUTE_UNUSED) OVERRIDE
                      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    for (size_t i = 0; i < count; ++i) {
      Process(roots[i]->AsMirrorPtr());
    }
  }

  // NO_THREAD_SAFETY_ANALYSIS due to MonitorExit.
  void Process(mirror::Object* entered_monitor) NO_THREAD_SAFETY_ANALYSIS {
...
void Thread::HandleScopeVisitRoots(RootVisitor* visitor, uint32_t thread_id) {
  BufferedRootVisitor<256> buffered_visitor(visitor, RootInfo(kRootNativeStack, thread_id));
 ...
 public:
  ...
  void VisitRoots(mirror::CompressedReference<mirror::Object>** roots, size_t count,
                  const RootInfo& info ATTRIBUTE_UNUSED) OVERRIDE
                      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    for (size_t i = 0; i < count; ++i) {
      VerifyObject(roots[i]->AsMirrorPtr());
    }
  }
};
