...
void Thread::InstallImplicitProtection() {
  ...
  *marker = kMarker;
  ...
  // Touch the pages in the region to map them in.  Otherwise mprotect fails.
  // TODO: we can probably just get away with reading a byte from every page,
  // but since this is only done at thread spawn time it's probably not worth
  // the effort.
