...
void Thread::Destroy() {
  ...
  if (opeer_ != nullptr) {
    ...
    // this.nativePeer = 0;
    if (Runtime::Current()->IsActiveTransaction()) {
      soa.DecodeField(WellKnownClasses::java_lang_Thread_nativePeer)->SetInt<true>(opeer_, 0);
    } else {
      soa.DecodeField(WellKnownClasses::java_lang_Thread_nativePeer)->SetInt<false>(opeer_, 0);
    }
