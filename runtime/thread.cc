...
struct StackDumpVisitor : public StackVisitor {
  ...
  bool VisitFrame() SHARED_REQUIRES(Locks::mutator_lock_) {
    ...
    m = m->GetInterfaceMethodIfProxy(kPointerSizeCur);
...
jobjectArray Thread::InternalStackTraceToStackTraceElementArray(
  ...
  for (int32_t i = 0; i < depth; ++i) {
    ...
    ArtMethod* method = method_trace->GetElementPtrSize<ArtMethod*>(i, kPointerSizeCur);
    uint32_t dex_pc = method_trace->GetElementPtrSize<uint32_t>(
        i + method_trace->GetLength() / 2, kPointerSizeCur);
    int32_t line_number;
    ...
    const char* method_name = method->GetInterfaceMethodIfProxy(kPointerSizeCur)->GetName();
...
template
void Thread::DumpThreadOffset<PointerSize::kPointerSize32>(std::ostream& os, uint32_t offset);
template
void Thread::DumpThreadOffset<PointerSize::kPointerSize64>(std::ostream& os, uint32_t offset);
