...
void Thread::RemoveDebuggerShadowFrameForFrameId(size_t frame_id) {
  if (tlsPtr_.frame_id_to_shadow_frame->GetFrameId() == frame_id) {
    tlsPtr_.frame_id_to_shadow_frame = tlsPtr_.frame_id_to_shadow_frame->GetNext();
    return;
  }
  FrameIdToShadowFrame* prev = tlsPtr_.frame_id_to_shadow_frame;
  for (FrameIdToShadowFrame* record = tlsPtr_.frame_id_to_shadow_frame->GetNext();
       record != nullptr;
