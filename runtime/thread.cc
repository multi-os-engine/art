...
class ReferenceMapVisitor : public StackVisitor {
 public:
  ...
  void VisitShadowFrame(ShadowFrame* shadow_frame) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    ArtMethod** method_addr = shadow_frame->GetMethodAddress();
    visitor_(reinterpret_cast<mirror::Object**>(method_addr), 0 /*ignored*/, this);
    ArtMethod* m = *method_addr;
    DCHECK(m != nullptr);
 ...
 private:
  void VisitQuickFrame() SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    StackReference<ArtMethod>* cur_quick_frame = GetCurrentQuickFrame();
    ArtMethod* m = cur_quick_frame->AsMirrorPtr();
    ArtMethod* old_method = m;
