...
bool Thread::ModifySuspendCount(Thread* self, int delta, Barrier* suspend_barrier, bool for_debugger) {
  if (kIsDebugBuild) {
...
bool Thread::PassActiveSuspendBarriers(Thread* self) {
  ...
  VLOG(threads) << "Thread " << self << " ran " << barrier_count << " barrier pass for " << this;
...
bool Thread::RunCheckpointFunction() {
  ...
  VLOG(threads) << "Thread " << Thread::Current() << " ran " << collected_checkpoints << " checkpoints for " << this;
