...
void Thread::InstallImplicitProtection(bool is_main_stack) {
  ...
  // Map in the main stack.  This must be done by reading from the
  // current stack pointer downwards as the stack map be mapped using VM_GROWSDOWN
  ...
  if (is_main_stack) {
    madvise(pregion, stack_top - pregion - kPageSize, MADV_DONTNEED);
  }
}
