...
void Thread::InitStackHwm() {
  void* stack_base;
  size_t stack_size;
  GetThreadStack(tlsPtr_.pthread_self, &stack_base, &stack_size);
  ...
  tlsPtr_.stack_begin = reinterpret_cast<byte*>(stack_base);
  tlsPtr_.stack_size = stack_size;
...
Thread::Thread(bool daemon) {
  tlsPtr_.wait_mutex = new Mutex("a thread wait mutex");
  tlsPtr_.wait_cond = new ConditionVariable("a thread wait condition variable",
                                            *tlsPtr_.wait_mutex);
...
Thread::~Thread() {
  ...
  delete tlsPtr_.wait_cond;
  delete tlsPtr_.wait_mutex;
...
bool Thread::Interrupted() {
  MutexLock mu(Thread::Current(), *tlsPtr_.wait_mutex);
...
void Thread::Interrupt() {
  Thread* self = Thread::Current();
  MutexLock mu(self, *GetWaitMutex());
...
void Thread::NotifyLocked(Thread* self) {
  if (tlsPtr_.wait_monitor != nullptr) {
    tlsPtr_.wait_cond->Signal(self);
  }
}
