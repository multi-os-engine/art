...
TEST_F(ModUnionTableTest, MainTest) {
  ...
  for (size_t i = 0; i < static_cast<size_t>(ModUnionTableFactory::kTableTypeCount); ++i) {
    ...
    // spaces.
    ASSERT_TRUE(table->ContainsCardFor(reinterpret_cast<uintptr_t>(obj1))) << type;
    ASSERT_TRUE(table->ContainsCardFor(reinterpret_cast<uintptr_t>(obj2))) << type;
    ...
    ASSERT_FALSE(table->ContainsCardFor(reinterpret_cast<uintptr_t>(obj4))) << type;
    {
    ...
    for (auto* ptr = space->Begin(); ptr < AlignUp(space->End(), CardTable::kCardSize);
      ...
      ASSERT_TRUE(table->ContainsCardFor(reinterpret_cast<uintptr_t>(ptr))) << type;
    }
