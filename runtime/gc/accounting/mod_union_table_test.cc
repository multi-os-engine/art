 ...
 public:
  ...
  mirror::ObjectArray<mirror::Object>* AllocObjectArray(
    ...
    if (obj != nullptr) {
      ...
      DCHECK_GE(bytes_allocated, size);
    }
    return obj;
  }
  void ResetClass() {
 ...
 private:
  mirror::Class* GetObjectArrayClass(Thread* self, space::ContinuousMemMapAllocSpace* space)
    ...
    if (java_lang_object_array_ == nullptr) {
      ...
      // We want the class to be inside the same space, so lets make a copy of it.
...
TEST_F(ModUnionTableTest, MainTest) {
  ...
  for (size_t i = 0; i < static_cast<size_t>(ModUnionTableFactory::kTableTypeCount); ++i) {
    ResetClass();
    auto type = static_cast<ModUnionTableFactory::TableType>(i);
    ...
    ASSERT_TRUE(other_space.get() != nullptr) << type;
    heap->AddSpace(other_space.get());
    std::unique_ptr<ModUnionTable> table(ModUnionTableFactory::Create(type, space,
                                                                      other_space.get()));
    ASSERT_TRUE(table.get() != nullptr) << type;
    ...
    ASSERT_TRUE(other_space_ref1 != nullptr) << type;
    auto* other_space_ref2 = AllocObjectArray(self, other_space.get(), CardTable::kCardSize);
    ASSERT_TRUE(other_space_ref2 != nullptr) << type;
    ...
    ASSERT_TRUE(visited.find(other_space_ref1) != visited.end()) << type;
    ASSERT_TRUE(visited.find(other_space_ref2) != visited.end()) << type;
    ...
    // spaces.
    ASSERT_TRUE(table->ContainsCard(reinterpret_cast<uintptr_t>(obj1))) << type;
    ASSERT_TRUE(table->ContainsCard(reinterpret_cast<uintptr_t>(obj2))) << type;
    ...
    ASSERT_FALSE(table->ContainsCard(reinterpret_cast<uintptr_t>(obj3))) << type;
    ASSERT_FALSE(table->ContainsCard(reinterpret_cast<uintptr_t>(obj4))) << type;
    {
    ...
    for (auto* ptr = space->Begin(); ptr < space->End(); ptr += CardTable::kCardSize) {
      ASSERT_TRUE(table->ContainsCard(reinterpret_cast<uintptr_t>(ptr))) << type;
    }
    ...
    table->Dump(oss2);
    ASSERT_EQ(oss.str(), oss2.str()) << type;
    // Clean up so that the other iterations of the loop have a nice clean environment.
    auto* other_bitmap = other_space->GetLiveBitmap();
    other_bitmap->Clear(other_space_ref1);
    other_space->Free(self, other_space_ref1);
    other_bitmap->Clear(other_space_ref2);
    other_space->Free(self, other_space_ref2);
    auto* bitmap = space->GetLiveBitmap();
    space->Free(self, obj1);
    bitmap->Clear(obj1);
    space->Free(self, obj2);
    bitmap->Clear(obj2);
    space->Free(self, obj3);
    bitmap->Clear(obj3);
    space->Free(self, obj4);
    bitmap->Clear(obj4);
