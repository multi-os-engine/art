...
inline bool Bitmap::AtomicTestAndSet(uintptr_t bit_index) {
...
inline bool Bitmap::Test(uintptr_t bit_index) const {
  CheckValidBitIndex(bit_index);
...
inline bool Bitmap::Modify(uintptr_t bit_index) {
  CheckValidBitIndex(bit_index);
  const size_t word_index = BitIndexToWordIndex(bit_index);
  const uintptr_t word_mask = BitIndexToMask(bit_index);
  ...
  DCHECK_EQ(Test(bit_index), kSetBit);
