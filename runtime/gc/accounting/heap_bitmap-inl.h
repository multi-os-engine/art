...
inline bool HeapBitmap::Test(const mirror::Object* obj) {
  ...
  for (const auto& bitmap : large_object_bitmaps_) {
    if (bitmap->HasAddress(obj) && bitmap->Test(obj)) {
      return true;
    }
  }
  return false;
...
template<typename SlowPathVisitor>
inline bool HeapBitmap::Set(const mirror::Object* obj, const SlowPathVisitor& visitor) {
  ContinuousSpaceBitmap* bitmap = GetContinuousSpaceBitmap(obj);
  if (LIKELY(bitmap != nullptr)) {
