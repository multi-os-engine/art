...
SpaceBitmap<kAlignment>* SpaceBitmap<kAlignment>::Create(
  ...
  const uint64_t bytes_covered_per_word = kAlignment * kBitsPerWord;
  size_t bitmap_size = (RoundUp(static_cast<uint64_t>(heap_capacity), bytes_covered_per_word) /
      bytes_covered_per_word) * kWordSize;
