...
mirror::Reference* ReferenceQueue::DequeuePendingReference() {
  ...
  if (kUseBakerOrBrooksReadBarrier && heap->CurrentCollectorType() == kCollectorTypeCC &&
    ...
    CHECK(ref != nullptr);
    CHECK_EQ(ref->GetReadBarrierPointer(), collector::ConcurrentCopying::GetGrayPtr())
        << "ref=" << ref << " rb_ptr=" << ref->GetReadBarrierPointer();
    if (heap->ConcurrentCopyingCollector()->RegionSpace()->IsInToSpace(ref)) {
      // Moving objects.
      ref->SetReadBarrierPointer(collector::ConcurrentCopying::GetWhitePtr());
      CHECK_EQ(ref->GetReadBarrierPointer(), collector::ConcurrentCopying::GetWhitePtr());
    } else {
      // Non-moving objects.
      ref->SetReadBarrierPointer(collector::ConcurrentCopying::GetBlackPtr());
      CHECK_EQ(ref->GetReadBarrierPointer(), collector::ConcurrentCopying::GetBlackPtr());
    }
  }
  return ref;
}
