...
Heap::Heap(size_t initial_size, size_t growth_limit, size_t min_free, size_t max_free,
  ...
  */
  ...
  // We may use the same space the main space if we don't need to compact from the main space.
  // This is the case if we support homogeneous compaction or have a moving background collector
  // type.
  bool separate_non_moving_space = Runtime::Current()->IsZygote() ||
  ...
  main_mem_map_2.reset(MapAnonymousPreferredAddress(kMemMapSpaceName[1], nullptr,
                                                    capacity_, PROT_READ | PROT_WRITE, &error_str));
