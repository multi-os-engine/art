...
void Heap::DumpGcPerformanceInfo(std::ostream& os) {
  ...
  if (blocking_gc_count_rate_histogram_.SampleSize() > 0U) {
    os << "Histogram of blocking GC count per " << NsToMs(kGcCountRateHistogramWindowDuration) << " ms: ";
...
uint64_t Heap::GetBlockingGcCount() {
  return blocking_gc_count_;
}

uint64_t Heap::GetBlockingGcTime() {
  return blocking_gc_time_;
}

Histogram<uint64_t>* Heap::GetGcCountRateHistogram() {
  return &gc_count_rate_histogram_;
}

Histogram<uint64_t>* Heap::GetBlockingGcCountRateHistogram() {
  return &blocking_gc_count_rate_histogram_;
}
