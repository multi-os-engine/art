...
void Heap::UpdateProcessState(ProcessState process_state) {
  if (process_state_ != process_state) {
    ...
    if (process_state_ == kProcessStateJankPerceptible) {
      ...
      RequestCollectorTransition(background_collector_type_, kIsDebugBuild ? 0 : kHeapTransitionWait);
    }
  }
}
...
void Heap::DoPendingTransitionOrTrim() {
  ...
  // Transition the heap if the desired collector type is not the same as the current collector type.
