...
void Heap::TransitionCollector(CollectorType collector_type) {
  ...
  LOG(INFO) << "TransitionCollector: " << static_cast<int>(collector_type_)
...
void Heap::PreZygoteFork() {
  ...
  LOG(INFO) << "Starting PreZygoteFork";
  ...
  if (semi_space_collector_ != nullptr) {
    ...
    DisableRosAllocVerification disable_rosalloc_verif(this);
...
void Heap::PreGcRosAllocVerification() {
  if (verify_pre_gc_rosalloc_) {
    // Revoke thread local runs otherwise we may encounter freed slots
    // (garbage data) that are still marked allocated in thread local
    // runs.
...
void Heap::PostGcRosAllocVerification() {
  if (verify_post_gc_rosalloc_) {
    // Revoke thread local runs otherwise we may see freed slots
    // (garbage data) that are still marked allocated in thread local
    // runs.
    // RevokeAllThreadLocalBuffers();
    for (const auto& space : continuous_spaces_) {
