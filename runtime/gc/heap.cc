...
Heap::Heap(size_t initial_size, size_t growth_limit, size_t min_free, size_t max_free,
    ...
    : non_moving_space_(nullptr),
      ...
      count_direct_delay_OOM_(0),
      count_indirect_delay_OOM_(0),
      count_requested_Ros2Ros_compaction_(0),
      count_ignored_Ros2Ros_compaction_(0),
...
*/
  ...
  // Note: background collector is CMS in earlier AOSP code.
      ...
      if (allocator == kAllocatorTypeRosAlloc) {
        ...
        switch (result) {
          case Ros2RosCompact::kErrorIgnore:
            ...
            ptr = TryToAllocate<true, true>(self, allocator, alloc_size, bytes_allocated, usable_size);
            if (ptr != nullptr) {
              android_atomic_inc(&count_indirect_delay_OOM_);
            }
            break;
          case Ros2RosCompact::kSuccess:
            ...
            ptr = TryToAllocate<true, true>(self, allocator, alloc_size, bytes_allocated, usable_size);
            if (ptr != nullptr) {
              android_atomic_inc(&count_direct_delay_OOM_);
            }
           break;
        ...
        LOG(INFO) << "Heap Ros2Ros compaction delay an OOM,"
                  << " requested Ros2Ros compaction " << count_requested_Ros2Ros_compaction_
                  << " performed Ros2Ros compaction " << count_performed_Ros2Ros_compaction_
                  << " ignored Ros2Ros compaction " << count_ignored_Ros2Ros_compaction_
                  << " direct delayed count = " << count_direct_delay_OOM_
                  << " indirect delayed count = " << count_indirect_delay_OOM_;
      } else {
...
Ros2RosCompact::Result Heap::RequestRos2RosCompact() {
  ...
  android_atomic_inc(&count_requested_Ros2Ros_compaction_);
  ...
  if ( last_gc_type == collector::kGcRos2RosCompact ||
    ...
    android_atomic_inc(&count_ignored_Ros2Ros_compaction_);
    FinishGC(self, collector::kGcTypeNone);
  ...
  if (strcmp(main_space_->GetName(), kRosAllocSpace) == 0) {
  ...
  heap_size_before_Ros2Ros = (uint32_t)from_space->AsContinuousSpace()->End()
                              - (uint32_t)from_space->AsContinuousSpace()->Begin();
  Compact(to_space, from_space, kGcCauseRos2RosCompact);
  heap_size_after_Ros2Ros = (uint32_t)to_space->AsContinuousSpace()->End()
                              - (uint32_t)to_space->AsContinuousSpace()->Begin();
  // Clean.
