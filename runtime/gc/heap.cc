...
collector::GcType Heap::CollectGarbageInternal(collector::GcType gc_type, GcCause gc_cause,
  ...
  if (CareAboutPauseTimes()) {
    ...
    if (was_slow) {
        ...
        VLOG(heap) << Dumpable<TimingLogger>(collector->GetTimings());
    }
  }
...
void Heap::GrowForUtilization(collector::GarbageCollector* collector_ran) {
  ...
  if (gc_type != collector::kGcTypeSticky) {
    ...
    collector::GarbageCollector* non_sticky_collector = nullptr;
    for (const auto& collector : garbage_collectors_) {
      if (collector->GetCollectorType() == collector_type_ &&
          collector->GetGcType() == non_sticky_gc_type) {
        non_sticky_collector = collector;
        break;
      }
    }
    ...
    if (collector_ran->GetEstimatedLastIterationThroughput() >=
      ...
      next_gc_type_ = collector_ran->GetGcType();
    } else {
