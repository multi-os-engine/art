...
void Heap::TransitionCollector(CollectorType collector_type) {
  ...
  reference_processor_.EnqueueClearedReferences(self);
  const uint64_t duration = NanoTime() - start_time;
  GrowForUtilization(collector);
...
collector::GcType Heap::CollectGarbageInternal(collector::GcType gc_type, GcCause gc_cause,
  ...
  collector::GarbageCollector* const collector =
      FindOrCreateCollector(self, collector_type_,
                            compacting_gc ? collector::kGcTypeAny : gc_type);
  if (compacting_gc) {
...
collector::GarbageCollector* Heap::FindOrCreateCollector(
  ...
  for (const auto& collector : garbage_collectors_) {
    if (collector->GetCollectorType() == type &&
        (gc_type == collector::kGcTypeAny || collector->GetGcType() == gc_type)) {
      return collector;
    }
  }
