...
Heap::Heap(size_t initial_size, size_t growth_limit, size_t min_free, size_t max_free,
    ...
    : non_moving_space_(nullptr),
      ...
      long_pause_log_threshold_(0 * long_pause_log_threshold),
  ...
  if (kUseRosAlloc) {
    rosalloc_space_ = space::RosAllocSpace::Create(name, initial_size, growth_limit, capacity,
                                                   requested_alloc_space_begin, low_memory_mode_);
    CHECK(rosalloc_space_ != nullptr) << "Failed to create rosalloc space";
    rosalloc_space_->SetFootprintLimit(rosalloc_space_->Capacity());
    AddSpace(rosalloc_space_);
    non_moving_space_ = rosalloc_space_;
  } else {
    dlmalloc_space_ = space::DlMallocSpace::Create(name, initial_size, growth_limit, capacity,
                                                   nullptr);
    AddSpace(dlmalloc_space_);
    CHECK(dlmalloc_space_ != nullptr) << "Failed to create dlmalloc space";
    dlmalloc_space_->SetFootprintLimit(dlmalloc_space_->Capacity());
    non_moving_space_ = dlmalloc_space_;
  }
...
void Heap::UpdateProcessState(ProcessState process_state) {
  if (process_state_ != process_state) {
    ...
    if (process_state_ == kProcessStateJankPerceptible) {
      TransitionCollector(kCollectorTypeCMS);
    } else {
      TransitionCollector(kCollectorTypeSS);
    }
  } else {
...
void Heap::MarkAllocStackAsLive(accounting::ObjectStack* stack) {
  space::ContinuousSpace* space1 = rosalloc_space_;
  space::ContinuousSpace* space2 = dlmalloc_space_;
...
void Heap::RecordFree(int32_t freed_objects, int32_t freed_bytes) {
...
void Heap::TransitionCollector(CollectorType collector_type) {
  if (collector_type != collector_type_) {
    ...
    size_t before_size = num_bytes_allocated_.load();
    ThreadList* tl = Runtime::Current()->GetThreadList();
    Thread* self = Thread::Current();
    ScopedThreadStateChange tsc(self, kWaitingPerformingGc);
    Locks::mutator_lock_->AssertNotHeld(self);
    // Busy wait until we can GC.
    while (!StartGC(self)) {
      usleep(100);
    }
    tl->SuspendAll();
    switch (collector_type) {
      case kCollectorTypeSS: {
        ...
        Compact(temp_space_, rosalloc_space_);
        DCHECK(rosalloc_mem_map_.get() == nullptr);
        rosalloc_mem_map_.reset(rosalloc_space_->ReleaseMemMap());
        madvise(rosalloc_mem_map_->Begin(), rosalloc_mem_map_->Size(), MADV_DONTNEED);
        RemoveSpace(rosalloc_space_);
        break;
      }
      ...
      case kCollectorTypeCMS: {
        if (collector_type_ == kCollectorTypeSS) {
          ...
          MemMap* mem_map = rosalloc_mem_map_.release();
    ...
    int32_t after_size = num_bytes_allocated_.load();
    LOG(INFO) << "Heap transition to " << process_state_ << " took "
        << PrettyDuration(duration) << " " << before_size << "->"
        << after_size << "=" << (before_size - after_size) << " delta";
  }
}
...
void Heap::PreZygoteFork() {
  ...
  if (true) {
    dlmalloc_space_ = space::DlMallocSpace::Create("Non moving dlmalloc space", 2 * MB, 64 * MB,
                                                   64 * MB, nullptr);
    AddSpace(dlmalloc_space_);
    CHECK(dlmalloc_space_ != nullptr) << "Failed to create dlmalloc space";
    dlmalloc_space_->SetFootprintLimit(dlmalloc_space_->Capacity());
