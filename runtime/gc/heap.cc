...
Heap::Heap(size_t initial_size,
    ...
    : non_moving_space_(nullptr),
      ...
      num_native_bytes_registered_(0),
...
void Heap::RegisterNativeAllocation(JNIEnv* env, size_t bytes) {
  ...
  if (TryToAllocate<false, false>(self, kAllocatorTypeNull, bytes, &ignored)
      || AllocateInternalWithGc(self, kAllocatorTypeNull, false, bytes, &ignored)) {
...
void Heap::RegisterNativeFree(JNIEnv* env, size_t bytes) {
  ...
  do {
    expected_size = num_native_bytes_registered_.LoadRelaxed();
  ...
  } while (!num_native_bytes_registered_.CompareExchangeWeakRelaxed(expected_size,
        expected_size - bytes));
