...
// Define space name.
static const char kDlMallocSpace[2][32] = {"main dlmalloc space","main dlmalloc space 1"};
static const char kRosAllocSpace[2][32] = {"main rosalloc space","main rosalloc space 1"};
static const char kMemMapSpace[2][32] = {"main space","main space 1"};
...
Heap::Heap(size_t initial_size, size_t growth_limit, size_t min_free, size_t max_free,
    ...
    : non_moving_space_(nullptr),
      ...
      min_interval_sticky_compaction_by_oom_(min_interval_sticky_compaction_by_oom),
      last_time_sticky_compaction_by_oom_(NanoTime()),
      count_performed_sticky_compaction_(0),
      desired_sticky_compaction_(false) {
...
requested_alloc_space_begin ->     +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
                                   ...
                                   +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
                                   +-      main alloc space 1 (capacity)         +-
                                   +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
                                   +-        main alloc space (capacity)         +-
                                   +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
*/
  main_space_bk_start_pos_ = requested_alloc_space_begin + kNonMovingSpaceCapacity;
  use_sticky_compaction_ = use_sticky_compaction &&
  ...
  if (is_zygote) {
    ...
    MemMap* mem_map = MemMap::MapAnonymous(
        kMemMapSpace[0], requested_alloc_space_begin + kNonMovingSpaceCapacity + capacity, capacity,
        PROT_READ | PROT_WRITE, true, &error_str);
    if (mem_map == nullptr) {
      // Fails to reserve the backup space.
      mem_map = MemMap::MapAnonymous(
          kMemMapSpace[0], requested_alloc_space_begin + kNonMovingSpaceCapacity, capacity,
          PROT_READ | PROT_WRITE, true, &error_str);
      CHECK(mem_map != nullptr) << error_str;
      // Disable sticky compaction for background transition.
    ...
    CreateMainMallocSpace(mem_map, initial_size, growth_limit, capacity, 0);
  } else {
    std::string error_str;
    MemMap* mem_map = MemMap::MapAnonymous(kMemMapSpace[0], requested_alloc_space_begin + kNonMovingSpaceCapacity + capacity,
...
void Heap::CreateMainMallocSpace(MemMap* mem_map, size_t initial_size, size_t growth_limit,
                                 size_t capacity, size_t alloc_space_index, bool is_backup) {
  ...
  if (kUseRosAlloc) {
    ...
    malloc_space = space::RosAllocSpace::CreateFromMemMap(mem_map, kRosAllocSpace[alloc_space_index],
    ...
    malloc_space = space::DlMallocSpace::CreateFromMemMap(mem_map, kDlMallocSpace[alloc_space_index],
                                                          kDefaultStartingSize, initial_size,
...
void Heap::UpdateProcessState(ProcessState process_state) {
  if (process_state_ != process_state) {
    ...
    if (process_state_ == kProcessStateJankPerceptible) {
      ...
      // Note: take kCollectorTypeStickyCompact as parameter is a little bit confused because
      // We still use kCollectorTypeCMS after application is switched background.
      if (use_sticky_compaction_) {
        desired_sticky_compaction_ = true;
        RequestCollectorTransition(kCollectorTypeStickyCompact, kIsDebugBuild ? 0 : kCollectorTransitionWait);
      } else {
        RequestCollectorTransition(background_collector_type_, kIsDebugBuild ? 0 : kCollectorTransitionWait);
      }
    }
  }
}
...
void Heap::DoPendingTransitionOrTrim() {
  ...
  // Launch sticky compaction if it is desired.
  if (!CareAboutPauseTimes() && desired_sticky_compaction_) {
     RequestStickyCompact();
     desired_sticky_compaction_ = false;
     // No need Trim(). Sticky compaction may free more virtual and physical memory.
     return;
  }
  ...
  if (ptr == nullptr) {
    if (!use_sticky_compaction_) {
      ...
      if ((allocator == kAllocatorTypeRosAlloc || allocator == kAllocatorTypeDlMalloc) &&
          (current_time - last_time_sticky_compaction_by_oom_)/1000 > min_interval_sticky_compaction_by_oom_) {
        last_time_sticky_compaction_by_oom_ = current_time;
        StickyCompact::Result result = RequestStickyCompact();
        switch (result) {
          case StickyCompact::kErrorIgnore:
            // Some sticky compaction is already performed in the blocking time (current sticky
          ...
          case StickyCompact::kSuccess:
           ...
           break;
          case StickyCompact::kErrorMemMap:
            // Throw OOM by default.
            break;
          case StickyCompact::kErrorShuttingDown:
            // Throw OOM by default.
            break;
          default: {
            LOG(FATAL) << "Unimplemented sticky compaction result "
        ...
        LOG(INFO) << "Heap sticky compaction delay an OOM,"
                  << " requested sticky compaction " << count_requested_sticky_compaction_.LoadSequentiallyConsistent()
                  << " performed sticky compaction " << count_performed_sticky_compaction_
                  << " ignored sticky compaction " << count_ignored_sticky_compaction_.LoadSequentiallyConsistent()
...
StickyCompact::Result Heap::RequestStickyCompact() {
  double heap_size_before_compaction = 0;
  double heap_size_after_compaction = 0;
  size_t map_space_index = 0;
  size_t alloc_space_index = 0;
  Thread* self = Thread::Current();
  // Inc requested sticky compaction.
  count_requested_sticky_compaction_++;
  // Store performed sticky compaction at a new request arrival.
  self->SetPerformedStickyCompactionCount(count_performed_sticky_compaction_);
  ...
  for (;;) {
    {
      ...
      MutexLock mu(self, *gc_complete_lock_);
      // Ensure there is only one GC at a time.
      WaitForGcToCompleteLocked(kGcCauseStickyCompact, self);
      ...
      if (!copying_transition || disable_moving_gc_count_ == 0) {
        collector_type_running_ = kCollectorTypeStickyCompact;
        break;
      }
    }
    usleep(1000);
  ...
  if (Runtime::Current()->IsShuttingDown(self)) {
    ...
    FinishGC(self, collector::kGcTypeNone);
    return StickyCompact::kErrorShuttingDown;
  }

  // Check whether some sticky compaction is already performed during the blocking time.
  if (count_performed_sticky_compaction_ > self->GetPerformedStickyCompactionCount()) {
    count_ignored_sticky_compaction_++;
    FinishGC(self, collector::kGcTypeNone);
    return StickyCompact::kErrorIgnore;
  }
  // Suspend all threads.
  tl->SuspendAll();
  ...
  // Create MemMap space.
  std::string error_str;
  MemMap* mem_map = MemMap::MapAnonymous(kMemMapSpace[map_space_index], main_space_bk_start_pos_,
                                         capacity_, PROT_READ | PROT_WRITE, true, &error_str);
  if (mem_map == nullptr) {
    // in case of huge pressure on virtual memory.
    tl->ResumeAll();
    FinishGC(self, collector::kGcTypeNone);
    return StickyCompact::kErrorMemMap;
  }
  // Create a rosalloc space.
  CreateMainMallocSpace(mem_map, kDefaultInitialSize, growth_limit_, capacity_, alloc_space_index, true);
  AddSpace(main_space_bk_);
  // Launch compaction.
  space::MallocSpace* to_space = main_space_bk_;
  space::MallocSpace* from_space = main_space_;
  to_space->GetMemMap()->Protect(PROT_READ | PROT_WRITE);
  heap_size_before_compaction = from_space->AsContinuousSpace()->End()
                              - from_space->AsContinuousSpace()->Begin();
  Compact(to_space, from_space, kGcCauseStickyCompact);
  heap_size_after_compaction = to_space->AsContinuousSpace()->End()
                              - to_space->AsContinuousSpace()->Begin();
  // Clean.
  main_space_bk_start_pos_ = from_space->AsContinuousSpace()->Begin();
  ...
  main_space_bk_ = nullptr;
  if (kUseRosAlloc) {
    rosalloc_space_ = to_space->AsRosAllocSpace();
  } else {
    dlmalloc_space_ = to_space->AsDlMallocSpace();
  }
  // Update performed sticky compaction count.
  count_performed_sticky_compaction_++;
  // Print statics log and resume all threads.
  uint64_t duration = NanoTime() - start_time;
  LOG(INFO) << "Heap sticky compaction took " << PrettyDuration(duration) <<
  " compact-ratio: " << std::fixed << heap_size_after_compaction/heap_size_before_compaction;
  tl->ResumeAll();
  // Finish GC.
  reference_processor_.EnqueueClearedReferences(self);
  GrowForUtilization(semi_space_collector_);
  FinishGC(self, collector::kGcTypeFull);
  return StickyCompact::kSuccess;
}
...
void Heap::PreZygoteFork() {
  ...
  if (kCompactZygote) {
    ...
    if (reset_main_space) {
      ...
      CreateMainMallocSpace(mem_map, kDefaultInitialSize, mem_map->Size(), mem_map->Size(), 0);
...
void Heap::RequestCollectorTransition(CollectorType desired_collector_type, uint64_t delta_time) {
  Thread* self = Thread::Current();
  {
    ...
    // We do not want to change desired_collector_type_ to kCollectorTypeStickyCompact
    if (desired_collector_type != kCollectorTypeStickyCompact) {
