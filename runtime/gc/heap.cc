...
Heap::Heap(size_t initial_size, size_t growth_limit, size_t min_free, size_t max_free,
    ...
    : non_moving_space_(nullptr),
      ...
      use_homogeneous_space_compaction_(use_homogeneous_space_compaction) {
  ...
  */
  ...
  if (is_zygote) {
    ...
    if (use_homogeneous_space_compaction_) {
      main_space_1_map = MemMap::MapAnonymous(kMemMapSpaceName[1], main_space_map->End(), capacity_,
                                               PROT_READ | PROT_WRITE, true, &error_str);
      if (main_space_1_map == nullptr) {
        LOG(WARNING) << "Disabling homogeneous space compaction due to error " << error_str;
        use_homogeneous_space_compaction_ = false;
      }
    }
    ...
    if (main_space_1_map != nullptr) {
      ...
      main_space_backup_ = CreateMallocSpaceFromMemMap(main_space_1_map, initial_size, growth_limit_,
                                                   capacity_, name, true);
    }
  } else {
    std::string error_str;
    ...
    if (use_homogeneous_space_compaction_) {
      ...
      if (main_space_1_map == nullptr) {
        LOG(WARNING) << "Disabling homogeneous space compaction for non zygote due to error " << error_str;
        use_homogeneous_space_compaction_ = false;
...
void Heap::CreateMainMallocSpace(MemMap* mem_map, size_t initial_size, size_t growth_limit,
  ...
  bool can_move_objects = IsMovingGc(background_collector_type_) !=
      IsMovingGc(foreground_collector_type_) || use_homogeneous_space_compaction_;
...
void Heap::UpdateProcessState(ProcessState process_state) {
  if (process_state_ != process_state) {
    ...
    if (process_state_ == kProcessStateJankPerceptible) {
      ...
      // Note: take kCollectorTypeHomogeneousSpaceCompact as parameter is a little bit confused because
      // We still use kCollectorTypeCMS after application is switched background.
      if (use_homogeneous_space_compaction_) {
        RequestCollectorTransition(kCollectorTypeHomogeneousSpaceCompact, kIsDebugBuild ? 0 : kCollectorTransitionWait);
      } else {
  ...
  if (!large_object_allocation && total_bytes_free >= byte_count) {
    ...
    for (const auto& space : continuous_spaces_) {
      if (space->IsMallocSpace()) {
        ...
        if (kUseRosAlloc ? space->IsRosAllocSpace() : space->IsDlMallocSpace()) {
          ...
          gc::space::MallocSpace* current_malloc_space;
          if (kUseRosAlloc) {
            current_malloc_space = main_space_->AsRosAllocSpace();
          } else {
            current_malloc_space = main_space_->AsDlMallocSpace();
          }
          if (space == current_malloc_space) {
...
mirror::Object* Heap::AllocateInternalWithGc(Thread* self, AllocatorType allocator,
  ...
  if (ptr == nullptr && use_homogeneous_space_compaction_) {
