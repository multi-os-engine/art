...
class RootMatchesObjectVisitor : public RootVisitor {
 public:
  ...
  void VisitRoots(mirror::CompressedReference<mirror::Object>** roots, size_t count,
      ...
      OVERRIDE SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    for (size_t i = 0; i < count; ++i) {
      Check(roots[i]->AsMirrorPtr(), info);
    }
  }

  void Check(mirror::Object* root, const RootInfo& ri) {
    if (root == obj_) {
      LOG(INFO) << "Object " << obj_ << " is a root " << ri.ToString();
    }
  }
...
class VerifyReferenceVisitor : public RootVisitor {
 public:
  ...
  void VerifyRoot(mirror::Object* root, const RootInfo& root_info)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    if (root == nullptr) {
  ...
  void VisitRoots(mirror::CompressedReference<mirror::Object>** roots, size_t count,
      ...
      OVERRIDE SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    for (size_t i = 0; i < count; ++i) {
      VerifyRoot(roots[i]->AsMirrorPtr(), info);
    }
  }
