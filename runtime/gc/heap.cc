...
void Heap::GrowForUtilization(collector::GcType gc_type, uint64_t gc_duration) {
  ...
  if (!ignore_max_footprint_) {
    ...
    if (concurrent_gc_) {
      ...
      // right away.
      const size_t new_concurrent_start_bytes =
          std::max(max_allowed_footprint_ - remaining_bytes, bytes_allocated);
      DCHECK_LE(remaining_bytes, max_allowed_footprint_);
      DCHECK_LE(max_allowed_footprint_, growth_limit_);
      concurrent_start_bytes_ = new_concurrent_start_bytes;
    }
  }
}
