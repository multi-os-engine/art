...
void Heap::CreateThreadPool() {
  ...
  concurrent_gc_request_thread_pool_.reset(
      new ThreadPool("Concurrent GC request thread pool", 1U));
}
...
void Heap::DeleteThreadPool() {
  ...
  if (concurrent_gc_request_thread_pool_.get() != nullptr) {
    concurrent_gc_request_thread_pool_->Wait(Thread::Current(), true, false);
    concurrent_gc_request_thread_pool_.reset(nullptr);
  }
}
...
void Heap::RequestConcurrentGCAndSaveObject(Thread* self, mirror::Object** obj) {
  ...
  if (concurrent_gc_request_thread_pool_.get() != nullptr) {
    ...
    // since there may be a task which is blocked on the mutator lock. Some programs without
    // full runtimes (such as dex2oat) don't have a thread pool. In this case, ignore the GC
    // request.
    concurrent_gc_request_thread_pool_->AddTask(self, new ConcurrentGCRequestTask());
    concurrent_gc_request_thread_pool_->StartWorkers(self);
  }
}
