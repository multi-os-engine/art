...
void AllocRecordObjectMap::VisitRoots(RootVisitor* visitor) {
  ...
  // Finally, visit all of the stack frames to make sure no methods in the stack traces
  // get unloaded by class unloading.
  for (auto&& pair : entries_) {
    const AllocRecord* const record = pair.second;
    if (record != nullptr) {
