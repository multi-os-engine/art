...
class MarkCompact : public GarbageCollector {
 ...
 protected:
  ...
  void Compact() EXCLUSIVE_LOCKS_REQUIRED(Locks::mutator_lock_);
  // Calculate the forwarding address of objects marked as "live" in the objects_before_forwarding
  // bitmap.
  void CalculateObjectForwardingAddresses() EXCLUSIVE_LOCKS_REQUIRED(Locks::mutator_lock_)
      SHARED_LOCKS_REQUIRED(Locks::heap_bitmap_lock_);
  // Update the references of objects by using the forwarding addresses.
  void UpdateReferences() EXCLUSIVE_LOCKS_REQUIRED(Locks::mutator_lock_);
  ...
  static mirror::Object* IsMarkedCallback(mirror::Object* object, void* arg)
      SHARED_LOCKS_REQUIRED(Locks::heap_bitmap_lock_);
  void FowardObject(mirror::Object* obj) EXCLUSIVE_LOCKS_REQUIRED(Locks::heap_bitmap_lock_,
                                                                  Locks::mutator_lock_);
  ...
  typedef accounting::SpaceBitmap<kObjectAlignment / 2> ObjectsBeforeForwardingBitmap;
  std::unique_ptr<ObjectsBeforeForwardingBitmap> objects_before_forwarding_;
  ...
  std::vector<LockWord> lock_words_to_restore_;
