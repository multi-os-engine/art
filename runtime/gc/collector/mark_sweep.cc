...
class VerifyRootMarkedVisitor : public RootVisitor {
 public:
  ...
  void VisitRoots(mirror::CompressedReference<mirror::Object>** roots, size_t count,
                  const RootInfo& info) OVERRIDE
                      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_, Locks::heap_bitmap_lock_) {
    for (size_t i = 0; i < count; ++i) {
      CheckMarked(roots[i]->AsMirrorPtr(), info);
    }
  }

  void CheckMarked(mirror::Object* obj, const RootInfo& info)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_, Locks::heap_bitmap_lock_) {
    CHECK(collector_->IsMarked(obj)) << info.ToString();
  }
...
class VerifyRootVisitor : public RootVisitor {
 public:
  ...
  void VisitRoots(mirror::Object*** roots, size_t count, const RootInfo& info) OVERRIDE
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_, Locks::heap_bitmap_lock_) {
    for (size_t i = 0; i < count; ++i) {
      collector_->VerifyRoot(*roots[i], info);
    }
  }
  ...
  void VisitRoots(mirror::CompressedReference<mirror::Object>** roots, size_t count,
                  const RootInfo& info) OVERRIDE
                      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_, Locks::heap_bitmap_lock_) {
    for (size_t i = 0; i < count; ++i) {
      collector_->VerifyRoot(roots[i]->AsMirrorPtr(), info);
    }
  }
...
class CheckpointMarkThreadRoots : public Closure, public RootVisitor {
 public:
  ...
  void VisitRoots(mirror::Object*** roots, size_t count, const RootInfo& info ATTRIBUTE_UNUSED)
      ...
      EXCLUSIVE_LOCKS_REQUIRED(Locks::heap_bitmap_lock_) {
    for (size_t i = 0; i < count; ++i) {
      mark_sweep_->MarkObjectNonNull(*roots[i]);
    }
  }
