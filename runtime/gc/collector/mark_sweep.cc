...
void MarkSweep::MarkingPhase() {
  ...
  Thread* self = Thread::Current();
  BindBitmaps();
  // Done adding spaces, create the largest immune region we can.
...
void MarkSweep::UpdateAndMarkModUnion() {
  for (const auto& space : immune_spaces_.GetSpaces()) {
    ...
    if (mod_union_table != nullptr) {
      ...
      // No mod-union table, scan all the live bits.
...
inline mirror::Object* MarkSweep::IsMarked(mirror::Object* object) {
  if (immune_spaces_.GetImmuneRegion().ContainsObject(object)) {
    return object;
  }
