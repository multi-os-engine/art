...
class GarbageCollector {
 public:
  ...
  uint64_t GetTotalPausedTimeNs() const LOCKS_EXCLUDED(pause_histogram_lock_);
  ...
  size_t GetTotalNumberOfPauses() const LOCKS_EXCLUDED(pause_histogram_lock_);
  void DumpPerformanceInfo(std::ostream& os) const LOCKS_EXCLUDED(pause_histogram_lock_);
