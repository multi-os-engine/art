...
// Called back from Runtime::FlipThread() during a pause.
...
void ConcurrentCopying::FlipThreads() {
  TimingLogger::ScopedTiming split("FlipThreads", GetTimings());
  if (kVerboseMode) {
  ...
  size_t barrier_count = Runtime::Current()->FlipThreads(&thread_flip_visitor, &flip_callback, this);
  {
  ...
  if (kVerboseMode) {
    LOG(INFO) << "time=" << region_space_->Time();
    region_space_->DumpNonFreeRegions(LOG(INFO));
    LOG(INFO) << "GC end of FlipThreads";
  }
}
 ...
 public:
  ...
  void operator()(mirror::Object* obj) const SHARED_LOCKS_REQUIRED(Locks::mutator_lock_)
    ...
    if (kUseBakerReadBarrier) {
      obj->AtomicSetReadBarrierPointer(ConcurrentCopying::GetWhitePtr(),
                                       ConcurrentCopying::GetGrayPtr());
    }
    if (cc_bitmap->AtomicTestAndSet(obj)) {
      // Already marked. Do nothing.
    } else {
      // Newly marked. Set the gray bit and push it onto the mark stack.
      CHECK(!kUseBakerReadBarrier || obj->GetReadBarrierPointer() == ConcurrentCopying::GetGrayPtr());
...
void ConcurrentCopying::MarkingPhase() {
  ...
  }
  {
    ...
    if (image != nullptr) {
      ...
      CHECK(image_root == marked_image_root) << "An image object does not move";
  ...
  Thread* self = Thread::Current();
  {
    TimingLogger::ScopedTiming split6("ProccesMarkStack", GetTimings());
...
class ConcurrentCopyingVerifyNoFromSpaceRefsVisitor {
 public:
  ...
  void operator()(mirror::Object* ref) const
    ...
    if (kUseBakerReadBarrier) {
      if (collector_->RegionSpace()->IsInToSpace(ref)) {
        ...
        CHECK(ref->GetReadBarrierPointer() == ConcurrentCopying::GetBlackPtr() ||
              (ref->GetReadBarrierPointer() == ConcurrentCopying::GetWhitePtr() &&
  ...
  static void RootCallback(mirror::Object** root, void *arg,
  ...
  static void ObjectCallback(mirror::Object* obj, void *arg)
    ...
    if (kUseBakerReadBarrier) {
      if (collector->RegionSpace()->IsInToSpace(obj)) {
        ...
        CHECK(obj->GetReadBarrierPointer() == ConcurrentCopying::GetBlackPtr() ||
              (obj->GetReadBarrierPointer() == ConcurrentCopying::GetWhitePtr() &&
...
void ConcurrentCopying::VerifyNoFromSpaceReferences() {
  ...
  // The to-space.
  region_space_->WalkToSpace(ConcurrentCopyingVerifyNoFromSpaceRefsObjectVisitor::ObjectCallback, this);
  // Non-moving spaces.
  {
    WriterMutexLock mu(self, *Locks::heap_bitmap_lock_);
...
bool ConcurrentCopying::ProcessMarkStack() {
  ...
  while ((to_ref = PopOffMarkStack()) != nullptr) {
    ...
    if (kUseBakerReadBarrier) {
      DCHECK(to_ref->GetReadBarrierPointer() == GetGrayPtr())
    ...
    if (to_ref->GetClass<kVerifyNone, kWithoutReadBarrier>()->IsTypeOfReferenceClass() &&
      ...
      if (kUseBakerReadBarrier) {
        if (region_space_->IsInToSpace(to_ref)) {
          ...
          bool success = to_ref->AtomicSetReadBarrierPointer(GetGrayPtr(), GetWhitePtr());
          CHECK(success) << "Must succeed as we won the race.";
          CHECK(to_ref->GetReadBarrierPointer() == GetWhitePtr());
        } else {
          ...
          // later (concurrently).
          bool success = to_ref->AtomicSetReadBarrierPointer(GetGrayPtr(), GetBlackPtr());
          CHECK(success) << "Must succeed as we won the race.";
          CHECK(to_ref->GetReadBarrierPointer() == GetBlackPtr());
        }
      }
    }
...
class ConcurrentCopyingClearBlackPtrsVisitor {
 public:
  ...
  void operator()(mirror::Object* obj) const SHARED_LOCKS_REQUIRED(Locks::mutator_lock_)
    ...
    CHECK(obj->GetReadBarrierPointer() == ConcurrentCopying::GetBlackPtr()) << obj;
    obj->SetReadBarrierPointer(ConcurrentCopying::GetWhitePtr());
    CHECK(obj->GetReadBarrierPointer() == ConcurrentCopying::GetWhitePtr()) << obj;
  }
...
void ConcurrentCopying::ClearBlackPtrs() {
  ...
  if (kEnableReadBarrierInvariantChecks || kIsDebugBuild) {
...
class ConcurrentCopyingComputeUnevacFromSpaceLiveRatioVisitor {
 public:
  ...
  void operator()(mirror::Object* ref) const SHARED_LOCKS_REQUIRED(Locks::mutator_lock_)
    ...
    if (kUseBakerReadBarrier) {
      CHECK(ref->GetReadBarrierPointer() == ConcurrentCopying::GetBlackPtr()) << ref;
      // Clear the black ptr.
      ref->SetReadBarrierPointer(ConcurrentCopying::GetWhitePtr());
    }
...
mirror::Object* ConcurrentCopying::Copy(mirror::Object* from_ref) {
  ...
  mirror::Object* to_ref;
  {
    to_ref = region_space_->AllocNonvirtual<true>(region_space_alloc_size,
                                                  &region_space_bytes_allocated, nullptr);
  ...
  while (true) {
    ...
    // Set the gray ptr.
    if (kUseBakerReadBarrier) {
      to_ref->SetReadBarrierPointer(GetGrayPtr());
    }
...
mirror::Object* ConcurrentCopying::Mark(mirror::Object* from_ref) {
  ...
  if (region_space_->IsInFromSpace(from_ref)) {
    to_ref = GetFwdPtr(from_ref);
    if (kUseBakerReadBarrier) {
      DCHECK(to_ref != GetGrayPtr()) << "from_ref=" << from_ref << " to_ref=" << to_ref;
    }
    if (to_ref == nullptr) {
    ...
    // This may or may not succeed, which is ok.
    if (kUseBakerReadBarrier) {
      from_ref->AtomicSetReadBarrierPointer(GetWhitePtr(), GetGrayPtr());
    }
    ...
    if (immune_region_.ContainsObject(from_ref)) {
      ...
      if (!is_los && mark_bitmap->Test(from_ref)) {
        ...
        if (kUseBakerReadBarrier) {
          DCHECK(to_ref->GetReadBarrierPointer() == GetGrayPtr() ||
                 to_ref->GetReadBarrierPointer() == GetBlackPtr());
        }
      } else if (is_los && los_bitmap->Test(from_ref)) {
        // Already marked in LOS.
        to_ref = from_ref;
        ...
        if (IsOnAllocStack(from_ref)) {
          ...
          if (kUseBakerReadBarrier) {
            DCHECK(to_ref->GetReadBarrierPointer() == GetWhitePtr());
          }
        } else {
