...
void ConcurrentCopying::InitializePhase() {
  ...
  mark_from_read_barrier_checks_ = measure_read_barrier_slow_path_;
  rb_slow_path_ns_ = 0;
...
void ConcurrentCopying::FinishPhase() {
  ...
  if (measure_read_barrier_slow_path_) {
    ...
    rb_slow_path_time_histogram_.AdjustAndAddValue(rb_slow_path_ns_);
...
mirror::Object* ConcurrentCopying::MarkFromReadBarrierWithChecks(mirror::Object* from_ref) {
  ...
  if (measure_read_barrier_slow_path_) {
    rb_slow_path_ns_ += NanoTime() - start_time;  // Racy, may lose writes.
  }
  return ret;
}
