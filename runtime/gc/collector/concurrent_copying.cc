...
class ConcurrentCopyingComputeUnevacFromSpaceLiveRatioVisitor {
 public:
  ...
  void operator()(mirror::Object* ref) const SHARED_LOCKS_REQUIRED(Locks::mutator_lock_)
    ...
    if (kUseBakerReadBarrier) {
      DCHECK(ref->GetReadBarrierPointer() == ReadBarrier::BlackPtr()) << ref;
      // Clear the black ptr.
...
mirror::Object* ConcurrentCopying::IsMarked(mirror::Object* from_ref) {
  ...
  if (rtype == space::RegionSpace::kRegionTypeToSpace) {
    // It's already marked.
    return from_ref;
  }
  mirror::Object* to_ref;
  if (rtype == space::RegionSpace::kRegionTypeFromSpace) {
  ...
  } else if (rtype == space::RegionSpace::kRegionTypeUnevacFromSpace) {
