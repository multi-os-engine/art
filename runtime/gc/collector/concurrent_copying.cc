...
void ConcurrentCopying::ProcessMarkStackRef(mirror::Object* to_ref) {
  ...
  if (to_ref->GetClass<kVerifyNone, kWithoutReadBarrier>()->IsTypeOfReferenceClass() &&
    ...
    // Leave this Reference gray in the queue so that GetReferent() will trigger RB. We will change
    // it to black or white later in ReferenceQueue::DequeuePendingReference().
