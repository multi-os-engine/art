...
class ConcurrentCopyingImmuneSpaceObjVisitor {
 public:
  ...
  void operator()(mirror::Object* obj) const SHARED_REQUIRES(Locks::mutator_lock_)
    ...
    if (kUseBakerReadBarrier) {
      cas_success = obj->AtomicSetReadBarrierPointer(ReadBarrier::WhitePtr(),
                                                     ReadBarrier::GrayPtr());
    }
    if (cc_bitmap->AtomicTestAndSet(obj)) {
      // Already marked.
      if (kUseBakerReadBarrier && cas_success &&
          obj->GetReadBarrierPointer() == ReadBarrier::GrayPtr()) {
        collector_->PushOntoFalseGrayStack(obj);
      }
    } else {
