...
Iteration::Iteration()
    : gc_cause_(kGcCauseForAlloc), clear_soft_references_(false), duration_ns_(0),
...
void GarbageCollector::Run(GcCause gc_cause, bool clear_soft_references) {
  ...
  cumulative_timings_.AddLogger(current_iteration->timings_);
  ...
  current_iteration->duration_ns_ = end_time - start_time;
  if (Locks::mutator_lock_->IsExclusiveHeld(self)) {
    ...
    RegisterPause(current_iteration->duration_ns_);
  }
  total_time_ns_ += current_iteration->GetDurationNs();
  for (uint64_t pause_time : current_iteration->pause_times_) {
