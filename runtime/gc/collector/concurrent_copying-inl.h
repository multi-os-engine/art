...
inline mirror::Object* ConcurrentCopying::MarkFromReadBarrier(mirror::Object* from_ref) {
  ...
  if (LIKELY(!rb_mark_stack_full_ && ret->AtomicSetMarkBit(0, 1))) {
    ...
    if (!rb_mark_stack_->AtomicPushBack(ret)) {
      // Mark stack is full, set the bit back to one.
      CHECK(ret->AtomicSetMarkBit(1, 0));
      // rb_mark_stack_full_ racy, but this OK since AtomicPushBack is thread safe.
      rb_mark_stack_full_ = true;
    }
  }
  return ret;
}
