...
class ConcurrentCopying : public GarbageCollector {
 ...
 private:
  ...
  void ScanImmuneObject(mirror::Object* obj)
      SHARED_REQUIRES(Locks::mutator_lock_) REQUIRES(!mark_stack_lock_);
  mirror::Object* MarkFromReadBarrierWithChecks(mirror::Object* from_ref)
      SHARED_REQUIRES(Locks::mutator_lock_)
      REQUIRES(!mark_stack_lock_, !skipped_blocks_lock_, !immune_gray_stack_lock_);
  ...
  // mark_from_read_barrier_checks_ is true if systrace is enabled or measure_read_barrier_time_
  // is true.
  bool mark_from_read_barrier_checks_;
  uint64_t rb_slow_path_ns_;
