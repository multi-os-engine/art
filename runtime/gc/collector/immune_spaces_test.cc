...
class ImmuneSpacesTest : public CommonRuntimeTest {
 ...
 public:
  ...
  DummyImageSpace* CreateImageSpace(uint8_t* image_begin,
    ...
    std::unique_ptr<DummyOatFile> oat_file(new DummyOatFile(oat_begin, oat_begin + oat_size));
    // Create image header.
...
// Test [image1][image2][image1 oat][image2 oat][space] producing a single large immune region.
TEST_F(ImmuneSpacesTest, MultiImage) {
  ...
  constexpr size_t kSpace3Size = kPageSize * 3;
  constexpr size_t kImage1OatSize = kPageSize * 5;
  constexpr size_t kImage2OatSize = kPageSize * 8;
  constexpr size_t kMemorySize = kImage1Size + kImage2Size + kSpace3Size +
      kImage1OatSize + kImage2OatSize;
  ...
  DummySpace space3(space3_begin, space3_begin + kSpace3Size);
