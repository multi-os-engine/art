...
void SemiSpace::MarkThreadRoots() {
  ...
  for (const auto& thread : thread_list) {
    ...
    if (iter == thread_roots_stacks_->end()) {
      ...
      CHECK(tms != nullptr);
      // Insert into the thread_roots_stacks_.
      thread_roots_stacks_->insert(
          std::pair<Thread*, ThreadRootMarkStack*>(thread, tms));
...
mirror::Object* SemiSpace::MarkNonForwardedObjectParallel(mirror::Object* obj,
  ...
  if (generational_ && reinterpret_cast<uint8_t*>(obj) < last_gc_to_space_end_) {
    ...
    if (UNLIKELY(forward_address == nullptr)) {
      ...
      if (*win == true) {
        // The lock word was updated by the current thread.
...
void SemiSpace::ProcessMarkStackParallel(size_t thread_count) {
  ...
  // Experiment show it helps improve the performance of parallel copy.
