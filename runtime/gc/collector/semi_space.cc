...
void SemiSpace::UpdateAndMarkModUnion() {
  for (auto& space : heap_->GetContinuousSpaces()) {
    ...
    if (immune_region_.ContainsSpace(space)) {
      ...
      if (heap_->FindRememberedSetFromSpace(space) != nullptr) {
...
void SemiSpace::MarkReachableObjects() {
  ...
  for (auto& space : heap_->GetContinuousSpaces()) {
    ...
    if (collect_from_space_only_ && heap_->FindModUnionTableFromSpace(space) == nullptr &&
        space->GetLiveBitmap() != nullptr) {
      DCHECK(space == GetHeap()->GetNonMovingSpace() ||
...
void SemiSpace::ProcessMarkStack() {
  ...
  while (!mark_stack_->IsEmpty()) {
    ...
    if (collect_from_space_only_ && promo_dest_space->HasAddress(obj)) {
