...
void SemiSpace::MarkingPhase() {
  if (kEnableBumpPointerSpacesOnlyCollection) {
    if (whole_heap_collection_) {
      LOG(INFO) << "Whole heap collection";
    } else {
      LOG(INFO) << "Bump pointer space only collection";
    }
  }
...
void SemiSpace::Sweep(bool swap_bitmaps) {
  ...
  int32_t before_sweep_freed_objects = freed_bytes_.load();
  ...
  int32_t after_sweep_freed_objects = freed_bytes_.load();
  LOG(INFO) << "Sweep collected " << PrettySize(after_sweep_freed_objects - before_sweep_freed_objects) << " in the non-moving space(s)";
