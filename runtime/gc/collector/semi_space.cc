...
static constexpr bool kEnableSimplePromo = true;
// If true, collect the bump pointer spaces only, as opposed to the
// whole heap in some collections.
static constexpr bool kEnableBumpPointerSpacesOnlyCollection = true;
        ...
        BindLiveToMarkBitmap(to_space_);
                 ...
                 || (kEnableBumpPointerSpacesOnlyCollection &&
                     !whole_heap_collection_ && (space == GetHeap()->GetNonMovingSpace() ||
                                                 GetHeap()->IsMainFreeListSpace(space)))) {
        ImmuneSpace(space);
      }
    }
  }
    ...
    if (IsImmuneSpace(space)) {
      ...
      if (table != nullptr) {
        ...
        DCHECK(kEnableBumpPointerSpacesOnlyCollection && !whole_heap_collection_ &&
               (space == heap_->GetNonMovingSpace() || heap_->IsMainFreeListSpace(space)));
      }
    }
  }
}
...
void SemiSpace::MarkReachableObjects() {
  ...
  for (auto& space : heap_->GetContinuousSpaces()) {
    ...
    if (IsImmuneSpace(space) && heap_->FindModUnionTableFromSpace(space) == nullptr) {
      DCHECK(kEnableBumpPointerSpacesOnlyCollection && !whole_heap_collection_ &&
             (space == GetHeap()->GetNonMovingSpace() || GetHeap()->IsMainFreeListSpace(space)));
    ...
    if (from_space_->HasAddress(obj)) {
      ...
      if (forward_address == nullptr) {
        ...
        if (kEnableSimplePromo && reinterpret_cast<byte*>(obj) < last_gc_to_space_end_) {
          ...
          space::MallocSpace* promo_dest_space = GetHeap()->GetMainFreeListSpace();
        ...
        DCHECK(to_space_->HasAddress(forward_address) ||
               (kEnableSimplePromo && GetHeap()->GetMainFreeListSpace()->HasAddress(forward_address)));
      }
      ret = forward_address;
...
void SemiSpace::ProcessMarkStack(bool paused) {
  ...
  if (kEnableSimplePromo && kEnableBumpPointerSpacesOnlyCollection && !whole_heap_collection_) {
    ...
    promo_dest_space = GetHeap()->GetMainFreeListSpace();
...
inline Object* SemiSpace::GetMarkedForwardAddress(mirror::Object* obj) const
  ...
  if (from_space_->HasAddress(obj)) {
    ...
    DCHECK(forwarding_address == nullptr || to_space_->HasAddress(forwarding_address) ||
           (kEnableSimplePromo && GetHeap()->GetMainFreeListSpace()->HasAddress(forwarding_address)));
