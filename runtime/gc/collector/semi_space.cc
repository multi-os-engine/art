...
void SemiSpace::MarkingPhase() {
  ...
  if (kEnableSimplePromo) {
    ...
    // LOG(INFO) << "Marking: from-space=" << reinterpret_cast<void*>(from_space_->Begin()) << "-"
    //          << reinterpret_cast<void*>(from_space_->End());
    // LOG(INFO) << "Marking: last_gc_to_space_end_=" << reinterpret_cast<void*>(last_gc_to_space_end_);
    bytes_promoted_ = 0;
  }
...
void SemiSpace::ReclaimPhase() {
  ...
  if (kEnableSimplePromo) {
    ...
    // LOG(INFO) << "Reclaim: to-space: " << reinterpret_cast<void*>(to_space_->Begin()) << "-"
    //           << reinterpret_cast<void*>(to_space_->End()) << "-" << reinterpret_cast<void*>(to_space_->Limit());;
    // LOG(INFO) << "Reclaim: last_gc_to_space_end_: " << reinterpret_cast<void*>(last_gc_to_space_end_);
    LOG(INFO) << "bytes_promoted_=" << PrettySize(bytes_promoted_);
  }
}
    ...
    if (from_space_->HasAddress(obj)) {
      ...
      if (forward_address == nullptr) {
        ...
        if (kEnableSimplePromo && reinterpret_cast<byte*>(obj) < last_gc_to_space_end_) {
          // If it's allocated before the last GC (older), promote it to
          // the non-moving space (sort of an old generation.)
          ...
          if (forward_address == nullptr) {
            ...
            // LOG(INFO) << "Promo: " << reinterpret_cast<void*>(obj)
            //          << " -> " << reinterpret_cast<void*>(forward_address);
