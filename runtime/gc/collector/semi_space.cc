...
void SemiSpace::MarkingPhase() {
  ...
  RevokeAllThreadLocalBuffers();
  // Record freed memory.
  const uint64_t from_bytes = from_space_->GetBytesAllocated();
  const uint64_t to_bytes = bytes_moved_;
  ...
  // space.
  RecordFree(from_objects - to_objects, static_cast<int64_t>(from_bytes - to_bytes));
