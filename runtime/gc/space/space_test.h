...
void SpaceTest::AllocAndFreeTestBody(CreateSpaceFn create_space) {
  ...
  // Succeeds, fits without adjusting the footprint limit.
  size_t ptr1_bytes_allocated, ptr1_usable_size;
  SirtRef<mirror::Object> ptr1(self, space->Alloc(self, 1 * MB, &dummy, nullptr));
  EXPECT_TRUE(ptr1.get() != nullptr);
  EXPECT_LE(1U * MB, ptr1_bytes_allocated);
  EXPECT_LE(1U * MB, ptr1_usable_size);
  EXPECT_LE(ptr1_usable_size, ptr1_bytes_allocated);
...
void SpaceTest::AllocAndFreeListTestBody(CreateSpaceFn create_space) {
  ...
  for (size_t i = 0; i < arraysize(lots_of_objects); i++) {
    ...
    lots_of_objects[i] = space->AllocWithGrowth(self, 1024, &allocation_size, nullptr);
    EXPECT_TRUE(lots_of_objects[i] != nullptr);
    SirtRef<mirror::Object> obj(self, lots_of_objects[i]);
    InstallClass(obj, 1024);
    lots_of_objects[i] = obj.get();
    EXPECT_EQ(allocation_size, space->AllocationSize(lots_of_objects[i], nullptr));
  }
