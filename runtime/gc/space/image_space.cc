...
ImageSpace* ImageSpace::DoGenerateImage(const ImageAssistant& ia, const InstructionSet image_isa,
                                        std::string* error_msg) {
  ImageSpace* space;
  // We should always return cache if we are compiling.
  ImageInfo info = ia.GetCacheImageInfo();
  ...
  if (!Runtime::Current()->IsImageDex2OatEnabled()) {
    *error_msg = "Not attempting to compile image because -Xnoimage-dex2oat";
    return nullptr;
  } else if (!ia.ImageCreationAllowed(error_msg)) {
    return nullptr;
  } else if (!GenerateImage(info.GetFilename(), image_isa, error_msg)) {
    *error_msg = StringPrintf("Failed to generate image '%s': %s",
    ...
    image_lock.Init(info.GetFilename().c_str(), error_msg);
    space = ImageSpace::Init(info.GetFilename().c_str(), ia.GetLocation().c_str(), true, error_msg);
    if (space == nullptr) {
...
ImageSpace* ImageSpace::Create(const char* image_location,
                               ...
                               std::string* error_msg) {
  ImageAssistant ia(image_location, image_isa);
  ...
  ImageInfo info = ia.GetSelectedImageInfo(relocate);
  ImageState state = info.GetImageState(relocate);
  if (state == ImageState::kImageUnusable) {
    return DoGenerateImage(ia, image_isa, error_msg);
  } else {
    ImageSpace* space;
    bool relocated_version_used = false;
    const std::string* image_filename;
    if (state == ImageState::kImageOutOfDate) {
      ...
      if (!Runtime::Current()->IsImageDex2OatEnabled()) {
        ...
        success = false;
      } else if (!ia.ImageCreationAllowed(&reason)) {
        // Whether we can write to the cache.
        success = false;
      } else {
        // Try to relocate.
        success = RelocateImage(image_location, info.GetFilename().c_str(), image_isa, &reason);
      }

      if (success) {
        relocated_version_used = true;
        image_filename = &info.GetFilename();
      } else {
        std::string system_filename;
        ia.FindSystemImageFilename(&system_filename);
        *error_msg = StringPrintf("Unable to relocate image '%s' from '%s' to '%s': %s",
                                  image_location, system_filename.c_str(),
                                  info.GetFilename().c_str(), reason.c_str());
        // We failed to create files, remove any possibly garbage output.
        // Since ImageCreationAllowed was true above, we are the zygote
        // and therefore the only process expected to generate these for
        // the device.
    ...
    } else if (state == ImageState::kImageUsable) {
      image_filename = &info.GetFilename();
    ...
    }
    {
      ...
      space = ImageSpace::Init(image_filename->c_str(), image_location,
                               !(info.IsSystem() || relocated_version_used), error_msg);
    }
    ...
    if (relocated_version_used) {
      ...
      *error_msg = StringPrintf("Attempted to use relocated version of %s at %s generated from %s "
                                ...
                                image_location, info.GetFilename().c_str(), system_filename.c_str(),
                                error_msg->c_str());
      PruneDalvikCache(image_isa);
      return nullptr;
    } else if (info.IsSystem()) {
