...
ImageSpace::ImageSpace(const std::string& name, const char* original_image_file_name,
                       MemMap* mem_map,
                       accounting::ContinuousSpaceBitmap* live_bitmap)
    : MemMapSpace(name, mem_map, mem_map->Begin(), mem_map->End(), mem_map->End(),
                  kGcRetentionPolicyNeverCollect),
      original_image_file_name_(original_image_file_name) {
...
ImageSpace* ImageSpace::Create(const char* original_image_file_name,
  ...
  bool is_system = false;
  if (GetImageFileLocation(original_image_file_name, image_isa, &image_location, &is_system)) {
    ImageSpace* space = ImageSpace::Init(image_location.c_str(), original_image_file_name,
                                         !is_system, &error_msg);
    if (space != nullptr) {
      return space;
    }
...
ImageSpace* ImageSpace::Init(const char* image_file_name, const char* original_image_file_name,
  ...
  UniquePtr<ImageSpace> space(new ImageSpace(image_file_name, original_image_file_name,
