...
ImageSpace* ImageSpace::Create(const char* image_location,
  ...
  ImageInfo info = image_assistant.GetImageInfo(relocate);
  ...
  switch (state) {
    ...
    case ImageState::kImageOutOfDate: {
      ...
      // Check whether we are allowed to relocate. If compilation is disabled we assume that we
      // should not relocate either. This is mainly used for testing.
      if (!Runtime::Current()->IsImageDex2OatEnabled()) {
      ...
      if (success) {
        ...
        std::string system_filename(image_assistant.GetSystemImageFilename());
  ...
  if (space != nullptr) {
    ...
    return space;
  } else if (relocated_version_used) {
