...
static void PruneDexCache(InstructionSet isa) {
  CHECK_NE(isa, kNone) << "Cannot prune isa kNone";
  std::string cache_dir_path = GetDalvikCacheOrDie(GetInstructionSetString(isa), false);
  if (!OS::DirectoryExists(cache_dir_path.c_str())) {
    LOG(INFO) << cache_dir_path << " does not exist and cannot be pruned.";
    return;
  }
  ...
  for (struct dirent* de = readdir(cache_dir); de != nullptr; de = readdir(cache_dir)) {
    ...
    if (de->d_type == DT_DIR) {
      LOG(WARNING) << "dalvik cache directory " << cache_dir_path << " is not flat!";
      continue;
    }
    ...
    if (TEMP_FAILURE_RETRY(unlinkat(dir_fd, name, 0)) != 0) {
      // Whatever stopped us from unlinking this file will probably stop us from unlinking all the
      // others in this directory so we might as well stop here.
...
static bool GenerateImage(const std::string& image_filename, InstructionSet image_isa,
  ...
  if (Runtime::Current()->ShouldPruneDexCache()) {
