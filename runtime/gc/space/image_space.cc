...
class FixupVisitor : public ValueObject {
 public:
  ...
  // Return the relocated address of an heap object.
  template <typename T>
...
class FixupArtMethodVisitor : public FixupVisitor, public ArtMethodVisitor {
 public:
  ...
  virtual void Visit(ArtMethod* method) NO_THREAD_SAFETY_ANALYSIS {
    ...
    if (method->IsNative()) {
      const void* old_native_code = method->GetEntryPointFromJni();
      const void* new_native_code = ForwardCode(old_native_code);
      if (old_native_code != new_native_code) {
        method->SetEntryPointFromJni(new_native_code);
      }
    } else {
      DCHECK(method->GetEntryPointFromJni() == nullptr);
    }
    const void* old_code = method->GetEntryPointFromQuickCompiledCode();
...
static bool RelocateInPlace(ImageHeader& image_header,
  ...
  CHECK(boot_image_space != nullptr) << "Can not relocate app image without boot image space.";
...
ImageSpace* ImageSpace::Init(const char* image_filename,
  ...
  // Check that the file is larger or equal to the data size.
  const uint64_t image_file_size = static_cast<uint64_t>(file->GetLength());
  if (image_file_size < image_header->GetDataSize()) {
  ...
  for (uint8_t* address : addresses) {
    ...
    if (image_header->GetStorageMode() == ImageHeader::kStorageModeUncompressed) {
      ...
      map.reset(MemMap::MapAnonymous(image_location,
                                     ...
                                     error_msg));
      if (map != nullptr) {
        TimingLogger::ScopedTiming timing2("ReadImageData", &logger);
        ...
        if (!file->ReadFully(&temp_buffer[0], stored_size)) {
          *error_msg = StringPrintf("Failed to read image section of size %zu at offset %zu",
                                    stored_size,
                                    write_offset);
          return nullptr;
        }
        timing2.NewTiming("DecompressImage");
