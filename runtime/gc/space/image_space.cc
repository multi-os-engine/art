...
bool ImageSpace::FindImageFilename(const char* image_location,
  ...
  *dalvik_cache_exists = false;
  ...
  GetDalvikCache(GetInstructionSetString(image_isa), &dalvik_cache,
                 &have_android_data, dalvik_cache_exists, true);
  ...
  if (have_android_data && *dalvik_cache_exists) {
    ...
    if (!GetDalvikCacheFilename(image_location, dalvik_cache.c_str(), cache_filename)) {
      return *has_system;
    }
...
//  argv.push_back(orig_base_offset_arg);
...
ImageHeader* ImageSpace::ReadImageHeaderOrDie(const char* image_location,
  ...
  if (FindImageFilename(image_location, image_isa, &system_filename, &has_system,
    ...
    if (Runtime::Current()->ShouldRelocate()) {
      ...
      // We don't want to relocate, Just pick the system one if we have it and return.
      if (has_system) {
...
ImageSpace* ImageSpace::Create(const char* image_location,
  ...
  if (found_image) {
    ...
    if (relocate) {
      ...
      if (has_system) {
        if (has_cache && ChecksumsMatch(system_filename.c_str(), cache_filename.c_str())) {
          ...
          LOG(INFO) << "Relocating already created image at " << system_filename << " to "
                    << cache_filename;
    ...
    }
    {
      ...
      space = ImageSpace::Init(image_filename->c_str(), image_location,
                               (image_filename == &cache_filename), &error_msg);
    }
    if (space != nullptr) {
      return space;
    }
  ...
  if (relocate) {
    LOG(INFO) << "Relocating newly generated image " << cache_filename;
    if (!RelocateImage(cache_filename.c_str(), cache_filename.c_str(), image_isa, &error_msg)) {
      LOG(FATAL) << "Failed to relocate newly reated image " << cache_filename.c_str();
      return nullptr;
    }
  }
  {
    ...
    LOG(INFO) << "Using generated image file " << cache_filename << " for image location "
              << image_location;
