...
static bool RelocateImage(const ImageAssistant& image_assistant,
                          const ImageInfo& destination,
                          std::string* error_msg) {
  std::string reason;
  std::string image_location(image_assistant.GetLocation());
  std::string dest_filename(destination.GetFilename());
  InstructionSet isa = image_assistant.GetIsa();
  if (!image_assistant.ImageCreationAllowed(&reason)) {
    *error_msg = StringPrintf("Unable to relocate image '%s' from '%s' to '%s': %s",
                              image_location.c_str(),
                              image_assistant.GetSystemImageFilename().c_str(),
                              dest_filename.c_str(),
                              reason.c_str());
  ...
  if (!Runtime::Current()->IsImageDex2OatEnabled()) {
    *error_msg = StringPrintf("Unable to relocate image '%s' from '%s' to '%s' because dex2oat is "
                              "disabled",
                              image_location.c_str(),
                              image_assistant.GetSystemImageFilename().c_str(),
                              dest_filename.c_str());
...
ImageSpace* ImageSpace::GenerateAndLoadImage(const ImageAssistant& image_assistant,
  ...
  // We should always place the newly compiled image into cache since /system might be read only.
  std::string cache_filename(image_assistant.GetCacheImageFilename());
  if (!GenerateImage(cache_filename, image_isa, error_msg)) {
  ...
  ImageSpace* space = ImageSpace::Init(cache_filename.c_str(),
                                       image_assistant.GetLocation().c_str(),
                                       true,
                                       error_msg);
  if (space == nullptr) {
...
ImageSpace* ImageSpace::LoadPresentImage(const ImageAssistant& assistant,
                                         const ImageInfo& image,
                                         std::string* error_msg) {
  std::string image_filename(image.GetFilename());
  std::string image_location(assistant.GetLocation());
  VLOG(startup) << "Using image file " << image_filename.c_str() << " for image location "
                << image_location.c_str();
  // If we are in /system we can assume the image is good. We can also assume this if we are using a
  // relocated image.
  bool should_validate_oat = !(image.IsSystemImage() || image.IsRelocated());
  ImageSpace* space = ImageSpace::Init(image_filename.c_str(),
                                       image_location.c_str(),
                                       should_validate_oat,
                                       error_msg);

  if (space != nullptr) {
    // We succeeded in loading the image.
    return space;
  } else if (image.IsRelocated()) {
    // Something is wrong with the relocated copy (even though checksums match). Cleanup.  This can
    // happen if the .oat is corrupt, since the above only checks the .art checksums.
    // TODO: Check the oat file validity earlier.
    std::string system_filename(assistant.GetSystemImageFilename());
    *error_msg = StringPrintf("Attempted to use relocated version of %s at %s generated from %s "
                              "but image failed to load: %s",
                              image_location.c_str(), image_filename.c_str(),
                              system_filename.c_str(), error_msg->c_str());
    PruneDalvikCache(assistant.GetIsa());
    return nullptr;
  } else if (image.IsSystemImage()) {
    ...
    *error_msg = StringPrintf("Failed to load /system image '%s': %s",
    ...
    // We tried to load a non-relocated, non-system image. We should just log a warning and try to
    // generate a new image.
    LOG(WARNING) << *error_msg;
    return GenerateAndLoadImage(assistant, assistant.GetIsa(), error_msg);
  }
}
...
ImageSpace* ImageSpace::Create(const char* image_location,
  ...
  ImageState state = info.GetImageState();
  ...
  switch (state) {
    ...
    case ImageState::kImageUsable:
      return LoadPresentImage(image_assistant, info, error_msg);
    case ImageState::kImageOutOfDate: {
      if (RelocateImage(image_assistant, info, error_msg)) {
        return LoadPresentImage(image_assistant, info, error_msg);
      } else {
        return nullptr;
      }
...
ImageSpace* ImageSpace::Init(const char* image_filename, const char* image_location,
  ...
  // we leave Init. This lock is to prevent races with other tools that
