...
RegionSpace::RegionSpace(const std::string& name, MemMap* mem_map)
  ...
  CHECK(IsAligned<kRegionSize>(mem_map_size));
  CHECK(IsAligned<kRegionSize>(mem_map->Begin()));
...
void RegionSpace::FreeLarge(mirror::Object* large_obj, size_t bytes_allocated) {
  ...
  for (uint8_t* addr = begin_addr; addr < end_addr; addr += kRegionSize) {
    Region* reg = RefToRegion(reinterpret_cast<mirror::Object*>(addr));
    if (addr == begin_addr) {
      DCHECK(reg->IsLarge());
    } else {
  ...
  if (end_addr < Limit()) {
    ...
    Region* following_reg = RefToRegion(reinterpret_cast<mirror::Object*>(end_addr));
    DCHECK(!following_reg->IsLargeTail());
  }
}

void RegionSpace::DumpRegions(std::ostream& os) const {
  for (size_t i = 0; i < num_regions_; ++i) {
    regions_[i].Dump(os);
  }
}

void RegionSpace::DumpNonFreeRegions(std::ostream& os) const {
  for (size_t i = 0; i < num_regions_; ++i) {
...
void RegionSpace::RevokeThreadLocalBuffersLocked(Thread* thread) {
  ...
  if (tlab_start != nullptr) {
    ...
    Region* r = RefToRegion(reinterpret_cast<mirror::Object*>(tlab_start));
    DCHECK(r->IsNormal());
