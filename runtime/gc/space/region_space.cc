  ...
  for (size_t i = 0; i < num_regions_; ++i) {
    ...
    if (!r->IsFree()) {
      ...
      if (LIKELY(num_expected_large_tails == 0U)) {
        DCHECK((state == kRegionStateAllocated || state == kRegionStateLarge) &&
               type == kRegionTypeToSpace);
        ...
        if (UNLIKELY(state == kRegionStateLarge && type == kRegionTypeToSpace)) {
        ...
        DCHECK(state == kRegionStateLargeTail && type == kRegionTypeToSpace);
