...
void RegionSpace::WalkInternal(ObjectCallback* callback, void* arg) {
  // TODO: MutexLock on region_lock_ won't work due to lock order issues (the
  // classloader classes lock and the monitor lock).  Make it possible
  // for Heap::VisitObjects() to do a suspend all internally.
  // MutexLock mu(Thread::Current(), region_lock_);
  for (size_t i = 0; i < num_regions_; ++i) {
    Region* r = &regions_[i];
