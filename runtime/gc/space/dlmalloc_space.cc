...
void DlMallocSpace::Clear() {
  ...
  GetLiveBitmap()->Clear();
  GetMarkBitmap()->Clear();
  static const size_t kDefaultStartingSize = 2 * MB;
  static const size_t kDefaultInitialSize = 2 * MB;
  end_ = Begin() + kDefaultStartingSize;
  mspace_ = CreateMspace(mem_map_->Begin(), kDefaultStartingSize, kDefaultInitialSize);
}
