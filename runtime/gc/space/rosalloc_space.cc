...
RosAllocSpace* RosAllocSpace::CreateFromMemMap(MemMap* mem_map, const std::string& name,
  ...
  /*if (kIsDebugBuild && starting_size == kPageSize) {
    // On debug builds, a page is reserved for magic number things. So make sure we have at least
    // two.
    starting_size += kPageSize;
    CHECK_LT(starting_size, capacity);
  }*/
