...
mirror::Object* ReferenceProcessor::GetReferent(Thread* self, mirror::Reference* reference) {
  ...
  while (slow_path_enabled_) {
    ...
    // it to the mutator as long as the GC is not preserving references. If the GC is preserving
    // references, the mutator could take a white field and move it somewhere else in the heap
    // causing corruption since this field would get swept.
