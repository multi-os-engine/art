...
class ReferenceQueue {
 public:
  ...
  // Enqueue a reference if is not already pending enqueued. Thread safe to call from multiple
  ...
  // Enqueue a reference. Unlike EnqueuePendingReference, EnqueueReference
  // requires (CHECKs) that the reference is not pending-enqueued. Not thread
  // safe, used when mutators are paused to minimize lock overhead.
  void EnqueueReference(mirror::Reference* ref) SHARED_REQUIRES(Locks::mutator_lock_);

  // Enqueue a reference that may already be pending-enqueued.
