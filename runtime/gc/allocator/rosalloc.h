...
class RosAlloc {
 ...
 public:
  ...
  static const size_t kNumThreadLocalSizeBracketIdx = 11;
 ...
 private:
  ...
  void* AllocFromRunThreadUnsafe(Thread* self, size_t size, size_t* bytes_allocated)
      LOCKS_EXCLUDED(lock_);
  void* AllocFromCurrentRun(Thread* self, size_t idx);
  ...
  // Free a run by adding it to non_full_runs_ or freeing the pages.
  void FreeRun(Thread* self, size_t idx, Run* run);

  // Revoke the current runs which share an index with the thread local runs.
  void RevokeCurrentRuns();
