...
class RosAlloc {
 private:
  ...
  class Run {
   public:
    enum RunFlags : byte {
      kRunFlagThreadLocal = 1,  // Thread local run, used for thread local allocations.
      kRunFlagInvalid = 2,      // Invalid type, used for the dedicated revoked run.
    };
    byte magic_num_;             // The magic number used for debugging.
    byte size_bracket_idx_;      // The index of the size bracket of this run.
    RunFlags run_flags_;         // True if this run is used as a thread-local run.
    byte to_be_bulk_freed_;      // Used within BulkFree() to flag a run that's involved with a bulk free.
    uint32_t first_bitmap_idx_;  // 32 * index of the first bitmap word which may contain an available slot.
    ...
    // Returns the number of bitmap words per section.
    size_t NumberOfBitmapWords() const {
      return RoundUp(numOfSlots[size_bracket_idx_], 32) / 32;
    }
    void SetRunFlag(RunFlags flag) {
      run_flags_ = static_cast<RunFlags>(run_flags_ | Run::kRunFlagThreadLocal);
    }
    void ClearRunFlag(RunFlags flag) {
      run_flags_ = static_cast<RunFlags>(run_flags_ & ~Run::kRunFlagThreadLocal);
    }
    bool IsThreadLocal() const {
      return (run_flags_ & Run::kRunFlagThreadLocal) != 0;
    }
    bool IsInvalid() const {
      return (run_flags_ & Run::kRunFlagInvalid) != 0;
    }
 ...
 public:
  ...
  void RevokeThreadLocalRuns(Thread* thread, bool revoke_to_null);
  // Releases the thread-local runs assigned to all the threads back to the common set of runs.
  void RevokeAllThreadLocalRuns(bool revoke_to_null) LOCKS_EXCLUDED(Locks::thread_list_lock_);
