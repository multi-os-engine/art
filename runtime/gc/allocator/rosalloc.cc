...
RosAlloc::RosAlloc(void* base, size_t capacity, size_t max_capacity,
  ...
  if (!IsAligned<kPageSize>(page_release_size_threshold_)) {
    LOG(WARNING) << "Page release size threshold is " << page_release_size_threshold_;
  }
...
size_t RosAlloc::UsableSize(const void* ptr) {
  DCHECK_LE(base_, ptr) << "base is " << std::hex << reinterpret_cast<uintptr_t>(base_);
  DCHECK_LT(ptr, base_ + footprint_);
  size_t pm_idx = RoundDownToPageMapIndex(ptr);
