...
RosAlloc::Run* RosAlloc::dedicated_full_run_ = nullptr;
...
void* RosAlloc::AllocLargeObject(Thread* self, size_t size, size_t* bytes_allocated) {
  ...
  *bytes_allocated = total_bytes;
  ...
  if (kCheckZeroMemory) {
    uword* words = reinterpret_cast<uword*>(r);
    for (size_t i = 0; i < total_bytes / sizeof(uword); ++i) {
      DCHECK_EQ(words[i], 0U);
    }
  }
  return r;
}
...
RosAlloc::Run* RosAlloc::AllocRun(Thread* self, size_t idx) {
  ...
  if (LIKELY(new_run != nullptr)) {
    if (kUsePrefetchDuringAllocRun) {
      byte* begin = reinterpret_cast<byte*>(new_run + headerSizes[idx]);
      for (size_t i = 0; i < numOfSlots[idx]; ++i) {
        __builtin_prefetch(begin);
        begin += bracketSizes[idx];
      }
    }
    if (kIsDebugBuild) {
...
std::string RosAlloc::Run::Dump() {
  ...
  stream << "RosAlloc Run = " << reinterpret_cast<void*>(this)
         ...
         << " is_thread_local_=" << static_cast<int>(is_thread_local_)
...
void RosAlloc::Run::FreeSlot(void* ptr) {
  ...
  first_bitmap_idx_ = std::min(first_bitmap_idx_, static_cast<uint32_t>(vec_idx));
...
void RosAlloc::Initialize() {
  ...
  // One page is enough room since we don't have any actual data in the dedicated full run.
  dedicated_full_run_ = reinterpret_cast<Run*>(dedicated_full_run_storage_);
