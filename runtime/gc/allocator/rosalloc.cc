...
size_t RosAlloc::RevokeThreadLocalRuns(Thread* thread) {
  ...
  for (size_t idx = 0; idx < kNumThreadLocalSizeBrackets; idx++) {
    ...
    if (thread_local_run != dedicated_full_run_) {
      ...
      // The above bracket index lock guards thread local free bitmap to avoid race condition
      // with unioning bulk free bitmap to thread local free bitmap by GC thread in BulkFree.
      // When thread local run is true, and thread local free bitmap is generated by GC thread
      // in BulkFree, thus it's no need to merge bulk free bitmap after merging thread local free
      // bitmap since alloc bitmap is already updated. After thread local run is false, GC thread
      // will help merge bulk free bitmap in BulkFree.
      bool dont_care;
