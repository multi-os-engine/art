...
size_t RosAlloc::ReleasePages() {
  ...
  while (i < page_map_size_) {
    ...
    byte pm = page_map_[i];
    switch (pm) {
      // This is probalby the start of a free page run.
      case kPageMapEmpty: {
        // Free page runs can start with kPageMapReleased due to coalescing.
        // revent other threads racing in and modifying the page map.
        MutexLock mu(self, lock_);
        ...
        if (pm == kPageMapEmpty) {
          ...
          FreePageRun* fpr = reinterpret_cast<FreePageRun*>(base_ + i * kPageSize);
          // There is a race condition where another thread can coalesce the free page run with a
          // previous one before we acquire lock_. In that case free_page_runs_.find will not find
          // a run starting at fpr. In this case we just skip reclaiming the page range and go to
          // the next page.
          if (free_page_runs_.find(fpr) != free_page_runs_.end()) {
            size_t fpr_size = fpr->ByteSize(this);
            DCHECK(IsAligned<kPageSize>(fpr_size));
      ...
      case kPageMapReleased:         // Fall through since the page is already released.
        ++i;
        break;  // Skip.
      default:
        LOG(FATAL) << "Unreachable - page map type: " << pm;
