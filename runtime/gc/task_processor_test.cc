...
class WorkUntilInterruptedTask : public SelfDeletingTask {
 public:
  WorkUntilInterruptedTask(TaskProcessor* task_processor, Atomic<bool>* done_running)
...
TEST_F(TaskProcessorTest, Interrupt) {
  ...
  thread_pool.AddTask(self, new WorkUntilInterruptedTask(&task_processor, &done_running));
