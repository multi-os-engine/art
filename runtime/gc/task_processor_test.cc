...
class SelfInterruptTask : public HeapTask {
 public:
  explicit SelfInterruptTask(TaskProcessor* task_processor)
     : HeapTask(NanoTime()), task_processor_(task_processor) {
  }
  virtual void Run(Thread* self) OVERRIDE {
    task_processor_->Interrupt(self);
  }
...
class WorkUntilInterruptedThread : public SelfDeletingTask {
 public:
  WorkUntilInterruptedThread(TaskProcessor* task_processor, volatile bool* done_running)
      : task_processor_(task_processor), done_running_(done_running) {
  }
  virtual void Run(Thread* self) OVERRIDE {
    task_processor_->RunTasksUntilInterrupted(self);
    *done_running_ = true;
  }

 private:
  TaskProcessor* const task_processor_;
  volatile bool* done_running_;
};

TEST_F(TaskProcessorTest, TestInterrupt) {
  ThreadPool thread_pool("task processor test", 1U);
  Thread* const self = Thread::Current();
  TaskProcessor task_processor;
  static constexpr size_t kRecursion = 10;
  volatile bool done_running = false;
  Atomic<size_t> counter(0);
  task_processor.AddTask(self, new RecursiveTask(&task_processor, &counter, kRecursion));
  // Add a task which will wait until interrupted to teh thread pool.
  thread_pool.AddTask(self, new WorkUntilInterruptedThread(&task_processor, &done_running));
  ...
  ASSERT_FALSE(done_running);
  task_processor.Interrupt(self);
  thread_pool.Wait(self, true, false);
  // After the interrupt and wait, the WorkUntilInterruptedThread task should have terminated and
  // set done_running_ to true.
  ASSERT_TRUE(done_running);
  ...
  task_processor.AddTask(self, new SelfInterruptTask(&task_processor));
  task_processor.AddTask(self, new RecursiveTask(&task_processor, &counter, kRecursion));
  thread_pool.AddTask(self, new WorkUntilInterruptedThread(&task_processor, &done_running));
  thread_pool.StartWorkers(self);
  thread_pool.Wait(self, true, false);
