...
class Heap {
 public:
  ...
  uint64_t GetGcCount();
  uint64_t GetGcTime();
  uint64_t GetBlockingGcCount();
  uint64_t GetBlockingGcTime();
  Histogram<uint64_t>* GetGcCountRateHistogram();
  Histogram<uint64_t>* GetBlockingGcCountRateHistogram();
 ...
 private:
  ...
  volatile bool running_collection_is_blocking_ GUARDED_BY(gc_complete_lock_);
  ...
  Histogram<uint64_t> gc_count_rate_histogram_;
  // The histogram of the number of blocking GC invocations per window duration.
  Histogram<uint64_t> blocking_gc_count_rate_histogram_;
