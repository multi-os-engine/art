...
class Heap {
 ...
 private:
  ...
  // Concurrent GC request thrad pool, needs to be separate since we can't have GC threads
  // reqesting concurrent GC or else we get deadlocks.
  std::unique_ptr<ThreadPool> concurrent_gc_request_thread_pool_;
