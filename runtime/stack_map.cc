  ...
  for (; num_finished < num_locations && stack_map_index >= end; --stack_map_index) {
    ...
    if (offset != StackMap::kNoDexRegisterMap && offset != StackMap::kSameDexRegisterMap) {
      ...
      for (size_t i = 0; i < bitmap_size_in_bytes; ++i) {
        uint8_t bits = bitmap.LoadUnaligned<uint8_t>(i);
        for (uint32_t bit : LowToHighBits<uint32_t>(bits)) {
          uint32_t vreg = i * kBitsPerByte + bit;
          if (vreg < num_locations) {
            DexRegisterLocation modified_location = DexRegisterLocation::Decode(
                &dex_register_maps_region_, &decode_offset);
            if (!finished[vreg]) {
              locations[vreg] = modified_location;
              finished[vreg] = true;
              num_finished++;
            }
          }
