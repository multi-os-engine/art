...
uint32_t StackMap::GetDexPc(const CodeInfo& info) const {
  return info.HasSmallDexPc()
      ? region_.LoadUnaligned<CodeInfo::kSmallEncoding>(info.ComputeStackMapDexPcOffset())
      : region_.LoadUnaligned<CodeInfo::kLargeEncoding>(info.ComputeStackMapDexPcOffset());
}

void StackMap::SetDexPc(const CodeInfo& info, uint32_t dex_pc) {
  DCHECK(!info.HasSmallDexPc() || IsUint<CodeInfo::kBitsForSmallEncoding>(dex_pc)) << dex_pc;
  info.HasSmallDexPc()
      ? region_.StoreUnaligned<CodeInfo::kSmallEncoding>(info.ComputeStackMapDexPcOffset(), dex_pc)
      : region_.StoreUnaligned<CodeInfo::kLargeEncoding>(info.ComputeStackMapDexPcOffset(), dex_pc);
}

uint32_t StackMap::GetNativePcOffset(const CodeInfo& info) const {
  return info.HasSmallNativePc()
      ? region_.LoadUnaligned<CodeInfo::kSmallEncoding>(info.ComputeStackMapNativePcOffset())
      : region_.LoadUnaligned<CodeInfo::kLargeEncoding>(info.ComputeStackMapNativePcOffset());
}

void StackMap::SetNativePcOffset(const CodeInfo& info, uint32_t native_pc_offset) {
  DCHECK(!info.HasSmallNativePc()
         || IsUint<CodeInfo::kBitsForSmallEncoding>(native_pc_offset)) << native_pc_offset;
  info.HasSmallNativePc()
      ? region_.StoreUnaligned<CodeInfo::kSmallEncoding>(info.ComputeStackMapNativePcOffset(), native_pc_offset)
      : region_.StoreUnaligned<CodeInfo::kLargeEncoding>(info.ComputeStackMapNativePcOffset(), native_pc_offset);
}

uint32_t StackMap::GetDexRegisterMapOffset(const CodeInfo& info) const {
  if (info.HasSmallDexRegisterMap()) {
    uint8_t value = region_.LoadUnaligned<CodeInfo::kSmallEncoding>(
    ...
    return region_.LoadUnaligned<CodeInfo::kLargeEncoding>(info.ComputeStackMapDexRegisterMapOffset());
  }
}
...
void StackMap::SetDexRegisterMapOffset(const CodeInfo& info, uint32_t offset) {
  ...
  info.HasSmallDexRegisterMap()
      ? region_.StoreUnaligned<CodeInfo::kSmallEncoding>(dex_register_map_entry, offset)
      : region_.StoreUnaligned<CodeInfo::kLargeEncoding>(dex_register_map_entry, offset);
}
...
uint32_t StackMap::GetInlineDescriptorOffset(const CodeInfo& info) const {
  ...
  if (info.HasSmallInlineInfo()) {
    ...
    return region_.LoadUnaligned<CodeInfo::kLargeEncoding>(info.ComputeStackMapInlineInfoOffset());
  }
}
...
void StackMap::SetInlineDescriptorOffset(const CodeInfo& info, uint32_t offset) {
  ...
  info.HasSmallInlineInfo()
      ? region_.StoreUnaligned<CodeInfo::kSmallEncoding>(inline_entry, offset)
      : region_.StoreUnaligned<CodeInfo::kLargeEncoding>(inline_entry, offset);
}

uint32_t StackMap::GetRegisterMask(const CodeInfo& info) const {
  return region_.LoadUnaligned<CodeInfo::kLargeEncoding>(info.ComputeStackMapRegisterMaskOffset());
}

void StackMap::SetRegisterMask(const CodeInfo& info, uint32_t mask) {
  region_.StoreUnaligned<CodeInfo::kLargeEncoding>(info.ComputeStackMapRegisterMaskOffset(), mask);
}
...
size_t StackMap::ComputeStackMapSize(size_t stack_mask_size,
  ...
  return StackMap::kFixedSize
      + stack_mask_size
      + (has_inline_info ? CodeInfo::NumberOfBytesForEntry(is_small_inline_info) : 0)
      + CodeInfo::NumberOfBytesForEntry(is_small_dex_map)
      + CodeInfo::NumberOfBytesForEntry(is_small_dex_pc)
      + CodeInfo::NumberOfBytesForEntry(is_small_native_pc);
}
...
size_t StackMap::ComputeStackMapSize(size_t stack_mask_size,
  ...
  return ComputeStackMapSize(
      ...
      IsUint<CodeInfo::kBitsForSmallEncoding>(inline_info_size + dex_register_map_size + 1),
      // + 1 to also encode kNoDexRegisterMap.
      IsUint<CodeInfo::kBitsForSmallEncoding>(dex_register_map_size + 1),
      IsUint<CodeInfo::kBitsForSmallEncoding>(dex_pc_max),
      IsUint<CodeInfo::kBitsForSmallEncoding>(native_pc_max));
}
