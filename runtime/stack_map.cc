...
void StackMap::SetNativePcOffset(const CodeInfo& info, uint32_t native_pc_offset) {
  ...
  info.HasSmallNativePc()
      ? region_.StoreUnaligned<CodeInfo::kSmallEncoding>(entry, native_pc_offset)
      : region_.StoreUnaligned<CodeInfo::kLargeEncoding>(entry, native_pc_offset);
}
...
uint32_t StackMap::GetDexRegisterMapOffset(const CodeInfo& info) const {
  if (info.HasSmallDexRegisterMap()) {
    ...
    if (value == std::numeric_limits<CodeInfo::kSmallEncoding>::max()) {
      return kNoDexRegisterMap;
    } else {
      return value;
    }
  } else {
    return region_.LoadUnaligned<CodeInfo::kLargeEncoding>(
        info.ComputeStackMapDexRegisterMapOffset());
  }
}
