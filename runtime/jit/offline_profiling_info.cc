...
static constexpr const uint64_t kMinimumTimeBetweenSavesNs = 500 * 1000000;
...
void OfflineProfilingInfo::SaveMethodsInfo(const std::string& filename,
  ...
  ProfilingInfo profiling_info;
  {
    ...
    for (auto it = methods.begin(); it != methods.end(); it++) {
      AddMethodInfo(*it, profiling_info);
    }
  }

  // This doesn't need locking because we are trying to lock the file for exclusive
  // access and fail immediately if we can't.
  if (Serialize(filename, profiling_info)) {
...
void OfflineProfilingInfo::AddMethodInfo(ArtMethod* method, ProfilingInfo& info) {
