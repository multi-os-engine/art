...
static const std::string kFirstDexFileKeySubstitute = ":classes.dex";
/**
...
bool ProfileCompilationInfo::ProcessLine(const std::string& line,
  ...
  Split(line, kFieldSeparator, &parts);
  if (parts.size() < 3) {
    return false;
  }

  // If we detect the synthetic key, reset the multidex_suffix to the empty string.
  const std::string& multidex_suffix = (parts[0] == kFirstDexFileKeySubstitute) ? "" : parts[0];
  uint32_t checksum;
...
// Returns the first position of kLineSeparator in the buffer (starting from start_from),
...
static int GetLineFromBuffer(char* buffer, int n, int start_from, std::string& line) {
  ...
  if (new_line_pos != -1) {
    line.append(buffer + start_from, new_line_pos - start_from);
  } else {
    line.append(buffer + start_from, n);
  }
  return new_line_pos;
}
...
bool ProfileCompilationInfo::Load(const std::vector<const DexFile*>& dex_files) {
  ...
  while (success) {
    ...
    // Process any line which might be in the buffer, or append to the current line
    // if it's just a continuation.
    while (true) {
      current_start_pos = GetLineFromBuffer(buffer, n, current_start_pos, line);
