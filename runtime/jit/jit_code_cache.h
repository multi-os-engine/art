...
class JitCodeCache {
 ...
 private:
  ...
  // The maximum capacity this code cache can go to.
  size_t max_capacity_ GUARDED_BY(lock_);

  // The current capacity of the code cache.
  size_t current_capacity_ GUARDED_BY(lock_);

  // The current footprint of the code portion of the code cache.
  size_t code_end_ GUARDED_BY(lock_);

  // The current footprint of the data portion of the code cache.
