...

  {
    ...
    if (!vreg_map.IsValid()) {
      ...
      for (uint16_t vreg = 0; vreg < number_of_vregs; ++vreg) {
        ...
        if (location.GetKind() == DexRegisterLocation::Kind::kConstant) {
          // We skip constants because the compiled code knows how to handle them.
          continue;
        }

        DCHECK_EQ(location.GetKind(), DexRegisterLocation::Kind::kInStack);

        int32_t vreg_value = shadow_frame->GetVReg(vreg);
        int32_t slot_offset = location.GetValue();
    ...
    native_pc = stack_map.GetNativePcOffset() + osr_method->GetEntryPoint();
    VLOG(jit) << "Jumping to "
              << method_name
              << "@"
