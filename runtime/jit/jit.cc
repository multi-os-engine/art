...
JitOptions* JitOptions::CreateFromRuntimeArguments(const RuntimeArgumentMap& options) {
  ...
  if (options.Exists(RuntimeArgumentMap::JITPriorityWeight)) {
    jit_options->priority_weight_ =
        *options.Get(RuntimeArgumentMap::JITPriorityWeight);
    if (jit_options->priority_weight_ > jit_options->compile_threshold_) {
