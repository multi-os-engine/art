...
bool Jit::LoadCompiler(std::string* error_msg) {
  ...
  jit_compile_method_ =
      reinterpret_cast<const OatQuickMethodHeader* (*)(void*, ArtMethod*, Thread*, bool)>(
...
bool Jit::CompileMethod(ArtMethod* method, Thread* self, bool osr) {
  ...
  bool success = (jit_compile_method_(jit_compiler_handle_, method, self, osr) != nullptr);
...
bool Jit::CheckNeedForOnStackReplacement(Thread* thread,
                                         ArtMethod* method,
                                         uint32_t dex_pc,
  ...
  // We found a stack map, now fill the frame with dex register values form the interpreter.
  DexRegisterMap vreg_map =
      code_info.GetDexRegisterMapOf(stack_map, encoding, number_of_vregs);
  DCHECK(vreg_map.IsValid()) << PrettyMethod(method) << "@" << (dex_pc + dex_pc_offset);
