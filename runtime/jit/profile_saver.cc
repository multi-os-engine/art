...
void ProfileSaver::SetTrackedDexLocations(
      const std::vector<std::string>& dex_base_locations) {
  MutexLock mu(Thread::Current(), *Locks::profiler_lock_);
  if (instance_ != nullptr && instance_->shutting_down_) {
    instance_->tracked_dex_base_locations_.clear();
    instance_->tracked_dex_base_locations_.insert(
        dex_base_locations.begin(), dex_base_locations.end());
