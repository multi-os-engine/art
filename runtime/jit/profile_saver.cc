...
void ProfileSaver::NotifyJitActivityInternal() {
  ...
  if (jit_activity_notifications_ > options_.GetMinNrOfNotificationBeforeWake()) {
    MutexLock wait_mutex(Thread::Current(), wait_lock_);
    if ((NanoTime() - last_time_ns_saver_woke_up_) > MsToNs(options_.GetMinSavePeriodMs())) {
      WakeUpSaver();
    }
  } else if (jit_activity_notifications_ > options_.GetMaxNrOfNotificationBeforeWake()) {
...
class GetMethodsVisitor : public ClassVisitor {
 public:
  GetMethodsVisitor(std::vector<MethodReference>* methods,
...
bool ProfileSaver::ProcessProfilingInfo(uint16_t* new_methods) {
  ...
  *new_methods = 0;
  ...
  for (const auto& it : tracked_locations) {
    ...
    if (delta_number_of_methods < options_.GetMinNrOfMethodsToSave() &&
        delta_number_of_classes < options_.GetMinNrOfClassesToSave()) {
