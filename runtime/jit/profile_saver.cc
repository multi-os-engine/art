...
void ProfileSaver::Run() {
  ...
  while (true) {
    ...
    if (!ProcessProfilingInfo()) {
      // If we don't need to save is less likely that we will need to do so in
      // the future. Increase the time between saves according to the
      // kBacjoffCoef, but make it no larger than kMaxBackoffMs.
      save_period_ms =
          std::min(kMaxBackoffMs, static_cast<uint64_t>(kBackoffCoef * save_period_ms));
      VLOG(profiler) << "Increased the period to save profiling information to " << save_period_ms << " ms";
    }
  }
}
...
void ProfileSaver::Stop() {
...

  {
    ...
    if (instance_ == nullptr) {
      DCHECK(false) << "Tried to stop an unstarted.";
      return;
    }
