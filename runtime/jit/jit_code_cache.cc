...
void JitCodeCache::GarbageCollectCache(Thread* self) {
...

  {
    ...
    if (!Runtime::Current()->GetInstrumentation()->AreExitStubsInstalled() &&
      ...
      DCHECK(CheckCompiledCodeHasProfilingInfo());
    }
...
bool JitCodeCache::CheckCompiledCodeHasProfilingInfo() {
  ...
  for (const auto& it : method_code_map_) {
    if (it.second->GetProfilingInfo(sizeof(void*)) == nullptr) {
      const void* code_ptr = it.first;
      ArtMethod* method = it.second;
      const OatQuickMethodHeader* method_header = OatQuickMethodHeader::FromCodePointer(code_ptr);
      // The code must be dead.
      return method_header->GetEntryPoint() != method->GetEntryPointFromQuickCompiledCode();
    }
  }
  return true;
}
