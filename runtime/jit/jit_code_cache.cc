...
JitCodeCache* JitCodeCache::Create(size_t capacity) {
  ...
  if (map == nullptr) {
    LOG(WARNING) << "Failed to create read write execute cache: " << error_msg << " size="
        << capacity;
...
JitCodeCache::JitCodeCache(MemMap* mem_map, bool fallback_mode)
    : lock_("Jit code cache", kJitCodeCacheLock), num_methods_(0), fallback_mode_(fallback_mode) {
  VLOG(jit) << "Created jit code cache size=" << PrettySize(mem_map->Size()) << " fallback="
      << fallback_mode;
