...
JitCodeCache* JitCodeCache::Create(size_t capacity, std::string* error_msg) {
  ...
  MemMap* map = MemMap::MapAnonymous(
      "jit-code-cache", nullptr, capacity, PROT_READ | PROT_WRITE, false, false, &error_str);
  if (map == nullptr) {
...
uint8_t* JitCodeCache::CommitCode(Thread* self,
  ...
  size_t total_size = RoundUp(sizeof(OatQuickMethodHeader) + code_size, sizeof(void*));
  ...
  uint8_t* code_ptr = result + sizeof(OatQuickMethodHeader);

  std::copy(code, code + code_size, code_ptr);
  new (result) OatQuickMethodHeader(
