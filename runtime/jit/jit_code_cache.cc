...
void JitCodeCache::GarbageCollectCache(Thread* self) {
  ...
  // Do the following actions depending on the current state of the code cache.
...

  {
    ...
    bool next_collection_will_be_full =
        ((current_capacity_ >= kReservedCapacity) && last_collection_increased_code_cache_) ||
        (current_capacity_ == max_capacity_);

    // Start polling the liveness of compiled code to prepare for the next full collection.
    if (next_collection_will_be_full) {
      // Save the entry point of methods we have compiled, and update the entry
      // poinf of those methods to the interpreter. If the method is invoked, the
...
void JitCodeCache::DoCollection(Thread* self, bool collect_profiling_info) {
  {
    ...
    if (collect_profiling_info) {
      ...
      for (ProfilingInfo* info : profiling_infos_) {
        CHECK(info->GetSavedEntryPoint() == nullptr);
      }
    }
  ...
  if (collect_profiling_info) {
    MutexLock mu(self, lock_);
    // Free all profiling infos of methods not compiled or being compiled.
