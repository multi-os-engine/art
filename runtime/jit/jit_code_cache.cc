  ...
  if (collect_profiling_info) {
    ...
    auto profiling_kept_end = std::remove_if(profiling_infos_.begin(), profiling_infos_.end(),
      [this] (ProfilingInfo* info) NO_THREAD_SAFETY_ANALYSIS {
        ...
        if (ContainsPc(ptr) && info->GetMethod()->GetProfilingInfo(kPointerSizeCur) == nullptr) {
        ...
        } else if (info->GetMethod()->GetProfilingInfo(kPointerSizeCur) != info) {
...
bool JitCodeCache::CheckLiveCompiledCodeHasProfilingInfo() {
  ...
  for (const auto& it : method_code_map_) {
    ArtMethod* method = it.second;
    if (method->GetProfilingInfo(kPointerSizeCur) == nullptr) {
      const void* code_ptr = it.first;
      const OatQuickMethodHeader* method_header = OatQuickMethodHeader::FromCodePointer(code_ptr);
      if (method_header->GetEntryPoint() == method->GetEntryPointFromQuickCompiledCode()) {
...
ProfilingInfo* JitCodeCache::AddProfilingInfoInternal(Thread* self ATTRIBUTE_UNUSED,
  ...
  ProfilingInfo* info = method->GetProfilingInfo(kPointerSizeCur);
  if (info != nullptr) {
    return info;
  }
...
void JitCodeCache::InvalidateCompiledCodeFor(ArtMethod* method,
  ...
  ProfilingInfo* profiling_info = method->GetProfilingInfo(kPointerSizeCur);
