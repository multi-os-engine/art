...
JitCodeCache* JitCodeCache::Create(size_t capacity, std::string* error_msg) {
  ...
  MemMap* data_map = MemMap::MapAnonymous(
    "data-code-cache", nullptr, capacity, PROT_ALL, false, false, &error_str);
  if (data_map == nullptr) {
  ...
  MemMap* code_map = data_map->RemapAtEnd(divider, "jit-code-cache", PROT_ALL, &error_str);
  if (code_map == nullptr) {
    std::ostringstream oss;
    oss << "Failed to create read write execute cache: " << error_str << " size=" << capacity;
    *error_msg = oss.str();
...
JitCodeCache::JitCodeCache(MemMap* code_map, MemMap* data_map)
  ...
  CHECKED_MPROTECT(code_map_->Begin(), code_map_->Size(), PROT_CODE);
  CHECKED_MPROTECT(data_map_->Begin(), data_map_->Size(), PROT_DATA);
}
...
class ScopedCodeCacheWrite {
 public:
  ScopedCodeCacheWrite(MemMap* code_map) : code_map_(code_map) {
    CHECKED_MPROTECT(code_map_->Begin(), code_map_->Size(), PROT_ALL);
  }
