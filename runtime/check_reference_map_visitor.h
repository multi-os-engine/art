...
class CheckReferenceMapVisitor : public StackVisitor {
 ...
 private:
  void CheckOptimizedMethod(int* registers, int number_of_references, uint32_t native_pc_offset)
    ...
    DexRegisterMap dex_register__map =
    ...
    for (int i = 0; i < number_of_references; ++i) {
      ...
      DexRegisterLocation location = dex_register__map.GetLocationKindAndValue(reg);
      switch (location.kind) {
        ...
        case DexRegisterLocation::Kind::kInStack:
          DCHECK_EQ(location.value % kFrameSlotSize, 0u);
          CHECK(stack_mask.LoadBit(location.value / kFrameSlotSize));
          break;
