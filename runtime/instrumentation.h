...
class Instrumentation {
 public:
  ...
  void DisableDeoptimization(const std::string& key)
      EXCLUSIVE_LOCKS_REQUIRED(Locks::mutator_lock_)
      LOCKS_EXCLUDED(deoptimized_methods_lock_);
  ...
  void DeoptimizeEverything(const std::string& key)
      EXCLUSIVE_LOCKS_REQUIRED(Locks::mutator_lock_)
      LOCKS_EXCLUDED(Locks::thread_list_lock_, Locks::classlinker_classes_lock_);

  // Executes everything with compiled code (or interpreter if there is no code).
  void UndeoptimizeEverything(const std::string& key)
      EXCLUSIVE_LOCKS_REQUIRED(Locks::mutator_lock_)
      LOCKS_EXCLUDED(Locks::thread_list_lock_, Locks::classlinker_classes_lock_);
  ...
  void EnableMethodTracing(const std::string& key,
  ...
  void DisableMethodTracing(const std::string& key)
      EXCLUSIVE_LOCKS_REQUIRED(Locks::mutator_lock_)
      LOCKS_EXCLUDED(Locks::thread_list_lock_, Locks::classlinker_classes_lock_);
 ...
 private:
  ...
  // by a string key.
  typedef SafeMap<std::string, InstrumentationLevel> InstrumentationLevelTable;
