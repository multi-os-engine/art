...
class Instrumentation {
 ...
 private:
  ...
  void UpdateInterpreterHandlerTable() REQUIRES(Locks::mutator_lock_) {
     ...
     * collapsed into a single conditionally-executed ldw instruction.  Best-case
     * in the Art goto interpreter is a pair of nested if-statments (the outermost of
     * which most likely succeeds forcing the execution of the inner) followed by
     * pointer-chase.
