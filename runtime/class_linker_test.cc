...
class ClassLinkerTest : public CommonRuntimeTest {
 protected:
  ...
  void AssertMethod(ArtMethod* method) SHARED_REQUIRES(Locks::mutator_lock_) {
    ...
    EXPECT_TRUE(method->HasDexCacheResolvedMethods(kPointerSizeCur));
    EXPECT_TRUE(method->HasDexCacheResolvedTypes(kPointerSizeCur));
  ...
  void AssertClass(const std::string& descriptor, Handle<mirror::Class> klass)
    ...
    if (klass->IsInterface()) {
      ...
      for (ArtMethod& m : klass->GetDirectMethods(kPointerSizeCur)) {
    ...
    for (ArtMethod& method : klass->GetDirectMethods(kPointerSizeCur)) {
      AssertMethod(&method);
      EXPECT_TRUE(method.IsDirect());
      EXPECT_EQ(klass.Get(), method.GetDeclaringClass());
    }

    for (ArtMethod& method : klass->GetDeclaredVirtualMethods(kPointerSizeCur)) {
      AssertMethod(&method);
      EXPECT_FALSE(method.IsDirect());
      EXPECT_EQ(klass.Get(), method.GetDeclaringClass());
    }

    for (ArtMethod& method : klass->GetCopiedMethods(kPointerSizeCur)) {
      AssertMethod(&method);
      EXPECT_FALSE(method.IsDirect());
  ...
  void AssertDexFile(const DexFile& dex, mirror::ClassLoader* class_loader)
    ...
    for (size_t i = 0, num_methods = dex_cache->NumResolvedMethods(); i != num_methods; ++i) {
      EXPECT_TRUE(
          mirror::DexCache::GetElementPtrSize(resolved_methods, i, kPointerSizeCur) != nullptr)
          << dex.GetLocation() << " i=" << i;
    }
  }
...
TEST_F(ClassLinkerTest, StaticFields) {
  ...
  ArtMethod* clinit = statics->FindDirectMethod("<clinit>", "()V", kPointerSizeCur);
...
TEST_F(ClassLinkerTest, Interfaces) {
  ...
  ArtMethod* Ii = I->FindVirtualMethod("i", void_sig, kPointerSizeCur);
  ArtMethod* Jj1 = J->FindVirtualMethod("j1", void_sig, kPointerSizeCur);
  ArtMethod* Jj2 = J->FindVirtualMethod("j2", void_sig, kPointerSizeCur);
  ArtMethod* Kj1 = K->FindInterfaceMethod("j1", void_sig, kPointerSizeCur);
  ArtMethod* Kj2 = K->FindInterfaceMethod("j2", void_sig, kPointerSizeCur);
  ArtMethod* Kk = K->FindInterfaceMethod("k", void_sig, kPointerSizeCur);
  ArtMethod* Ai = A->FindVirtualMethod("i", void_sig, kPointerSizeCur);
  ArtMethod* Aj1 = A->FindVirtualMethod("j1", void_sig, kPointerSizeCur);
  ArtMethod* Aj2 = A->FindVirtualMethod("j2", void_sig, kPointerSizeCur);
  ...
  EXPECT_EQ(Kj1, Jj1);
  EXPECT_EQ(Kj2, Jj2);
  EXPECT_EQ(Ai, A->FindVirtualMethodForInterface(Ii, kPointerSizeCur));
  EXPECT_EQ(Aj1, A->FindVirtualMethodForInterface(Jj1, kPointerSizeCur));
  EXPECT_EQ(Aj2, A->FindVirtualMethodForInterface(Jj2, kPointerSizeCur));
  EXPECT_EQ(Ai, A->FindVirtualMethodForVirtualOrInterface(Ii, kPointerSizeCur));
  EXPECT_EQ(Aj1, A->FindVirtualMethodForVirtualOrInterface(Jj1, kPointerSizeCur));
  EXPECT_EQ(Aj2, A->FindVirtualMethodForVirtualOrInterface(Jj2, kPointerSizeCur));
...
TEST_F(ClassLinkerTest, ResolveVerifyAndClinit) {
  ...
  ArtMethod* clinit = klass->FindClassInitializer(kPointerSizeCur);
  ArtMethod* getS0 = klass->FindDirectMethod("getS0", "()Ljava/lang/Object;", kPointerSizeCur);
...
TEST_F(ClassLinkerTest, ValidatePredefinedClassSizes) {
  ...
  EXPECT_EQ(c->GetClassSize(), mirror::Class::ClassClassSize(kPointerSizeCur));

  c = class_linker_->FindClass(soa.Self(), "Ljava/lang/Object;", class_loader);
  ASSERT_TRUE(c != nullptr);
  EXPECT_EQ(c->GetClassSize(), mirror::Object::ClassSize(kPointerSizeCur));

  c = class_linker_->FindClass(soa.Self(), "Ljava/lang/String;", class_loader);
  ASSERT_TRUE(c != nullptr);
  EXPECT_EQ(c->GetClassSize(), mirror::String::ClassSize(kPointerSizeCur));

  c = class_linker_->FindClass(soa.Self(), "Ljava/lang/DexCache;", class_loader);
  ASSERT_TRUE(c != nullptr);
  EXPECT_EQ(c->GetClassSize(), mirror::DexCache::ClassSize(kPointerSizeCur));
}
...
static void CheckVerificationAttempted(mirror::Class* c, bool preverified)
  ...
  for (auto& m : c->GetMethods(kPointerSizeCur)) {
