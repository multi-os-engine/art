...
void ClassLinker::VerifyClass(Thread* self, Handle<mirror::Class> klass) {
  ...
  // At this point if verification then supertype is the "first" supertype that failed verification
  // (without a specific order). If verification succeeded, then supertype is either null or the
  // original superclass of klass and is verified.
  DCHECK(supertype.Get() == nullptr ||
