...
const void* ClassLinker::GetQuickOatCodeFor(ArtMethod* method) {
  CHECK(method->IsInvocable()) << PrettyMethod(method);
...
const void* ClassLinker::GetOatMethodQuickCodeFor(ArtMethod* method) {
  if (method->IsNative() || !method->IsInvocable() || method->IsProxyMethod()) {
    return nullptr;
  }
  bool found;
  OatFile::OatMethod oat_method = FindOatMethodFor(method, &found);
...
// method. Should only be called on non-invocable methods.
void ClassLinker::EnsureNotInvocable(ArtMethod* method) {
  DCHECK(!method->IsInvocable());
...
void ClassLinker::LinkCode(ArtMethod* method, const OatFile::OatClass* oat_class,
  ...
  if (!method->IsInvocable()) {
    EnsureNotInvocable(method);
    return;
  }
