...
bool ClassLinker::LinkClass(Thread* self,
  ...
  if (!LinkMethods(self, klass, interfaces, imt_data)) {
    return false;
  }
  ...
  ImTable* imt = nullptr;
  if (klass->ShouldHaveImt()) {
    if (klass->HasSuperClass() && klass->GetSuperClass()->ShouldHaveImt()) {
...
void ClassLinker::FillIMTAndConflictTables(mirror::Class* klass) {
  ...
  // We need to replace IMT of the class by a new IMT with conflict tables.
  // Here we assume that the IMT of the super class is up to date.
  ImTable* imt = nullptr;
  ImTable* super_imt = nullptr;
  if (klass->HasSuperClass() && klass->GetSuperClass()->ShouldHaveImt()) {
    super_imt = klass->GetSuperClass()->GetImt(image_pointer_size_);
  }
  if (super_imt != nullptr) {
    // Check that data from the super IMT and the imt_data are same.
    // In this case we can share the super IMT.
    // TODO: Share the conflict table if it is the same.
    bool imt_equals = true;
    for (size_t i = 0; i < ImTable::kSize && imt_equals; ++i) {
      imt_equals = super_imt->Get(i, image_pointer_size_) == imt_data[i];
    }
    if (imt_equals) {
      imt = super_imt;
    }
  ...
  if (imt == nullptr) {
    // Super IMT can't be shared. We need to create a new IMT and fill
    // it from the imt_data. But we can reuse memory of the old IMT.
    ImTable* old_imt = klass->GetImt(image_pointer_size_);
    if (super_imt == old_imt) {
      // The address of the old IMT equals to the address of the super IMT.
      // It means that the super IMT had been shared with the old IMT and
      // this memory was reused. Thus we need to allocate new memory.
      LinearAlloc* allocator = GetAllocatorForClassLoader(klass->GetClassLoader());
      imt = reinterpret_cast<ImTable*>(
          allocator->Alloc(Thread::Current(), ImTable::SizeInBytes(image_pointer_size_)));
      if (imt == nullptr) {
        return;
      }
    } else {
      // The address of the old IMT and the address of the super IMT are different.
      // It means that the super IMT hasn't not been shared.
      // The memory of the class' IMT could be reused.
      imt = old_imt;
    }
    imt->Populate(imt_data, image_pointer_size_);
  }
