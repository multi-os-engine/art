...
bool ClassLinker::LinkFields(Handle<mirror::Class> klass, bool is_static, size_t* class_size) {
  ...
  // TODO shuffle fields forward? could save up to 3 bytes for each object.
  ...
  // Shuffle fields forward and align if necessary.
  ...
  CHECK(grouped_and_sorted_fields.empty()) << grouped_and_sorted_fields.size() << " fields remaining.";

  // Subclass expects superclass to be 4 byte aligned at end.
  if (!IsAligned<4>(field_offset.Uint32Value())) {
    field_offset = MemberOffset(field_offset.Uint32Value() + (0x4 - (field_offset.Uint32Value() & 0x3)));
  }
  ...
  if (kIsDebugBuild) {
    // Make sure that all the references are grouped together.
    bool seen_ref = false;
    bool passed_refs = false;
    for (size_t i = 0; i < num_fields; i++) {
      ...
      if (is_primitive) {
        // This is always okay, we are checking for reference grouping.
        if (seen_ref) {
          passed_refs = true;  // We have reencountered primitives after seen refs.
        }
      } else {
        seen_ref = true;
        if (passed_refs) {
          LOG(FATAL) << "Encountered ungrouped ref field: " << PrettyField(field);
        }
      }
    }
  }
