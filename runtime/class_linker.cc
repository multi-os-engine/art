...
static bool ContainsOverridingMethodOf(MethodNameAndSignatureComparator& target,
  ...
  for (size_t k = ifstart + 1; k < iftable_count; k++) {
    mirror::Class* current_iface = iftable->GetInterface(k);
    ...
    for (size_t m = 0; m < num_instance_methods; m++) {
      ...
      if (UNLIKELY(chosen_iface != nullptr)) {
        ...
        if (!iface->IsAssignableFrom(chosen_iface) &&
            !ContainsOverridingMethodOf(target_name_comparator, iftable, k, iface,
                                        image_pointer_size_)) {
      ...
      } else if (!ContainsOverridingMethodOf(target_name_comparator, iftable, k, iface,
                                             image_pointer_size_)) {
