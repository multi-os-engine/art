...
void ClassLinker::VerifyClass(Thread* self, Handle<mirror::Class> klass) {
  ...
  // Verify all default super-interfaces. Don't bother if the superclass has already had a soft
  // verification failure.
  //
  // Interfaces shouldn't bother to do this recursive verification because they cannot cause
  // recursive initialization by themselves. If they are being verified due to a class
  // initialization (which would need all the default interfaces to be verified) the class code will
  // do so anyway.
