...
class ClassLinker::FindVirtualMethodHolderVisitor {
 public:
  FindVirtualMethodHolderVisitor(const ArtMethod* method, PointerSize pointer_size)
      : method_(method),
        pointer_size_(pointer_size) {}

  bool operator()(mirror::Class* klass) SHARED_REQUIRES(Locks::mutator_lock_) {
...
mirror::Class* ClassLinker::GetHoldingClassOfCopiedMethod(ArtMethod* method) {
  ...
  for (const ClassLoaderData& data : class_loaders_) {
    if (!self->IsJWeakCleared(data.weak_root)) {
      if (visitor.holder_ != nullptr) {
        break;
      }
      data.class_table->Visit(visitor);
    }
  }
