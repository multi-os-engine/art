...
ClassLinker::ClassLinker(InternTable* intern_table)
    ...
    : dex_lock_("ClassLinker dex lock", kDefaultMutexLevel),
      ...
      image_pointer_size_(kPointerSizeCur) {
...
bool ClassLinker::InitFromBootImage(std::string* error_msg) {
  ...
  if (!runtime->IsAotCompiler()) {
    ...
    if (image_pointer_size_ != kPointerSizeCur) {
...
class FixupArtMethodArrayVisitor : public ArtMethodVisitor {
 public:
  ...
  virtual void Visit(ArtMethod* method) SHARED_REQUIRES(Locks::mutator_lock_) {
    GcRoot<mirror::Class>* resolved_types = method->GetDexCacheResolvedTypes(kPointerSizeCur);
    ...
    ArtMethod** resolved_methods = method->GetDexCacheResolvedMethods(kPointerSizeCur);
...
bool ClassLinker::UpdateAppImageClassLoadersAndDexCaches(
        ...
        for (int32_t j = 0; j < static_cast<int32_t>(num_types); j++) {
          ...
          if (space->HasAddress(klass)) {
            ...
            if (VLOG_IS_ON(image)) {
              ...
              for (ArtMethod& m : klass->GetVirtualMethods(kPointerSizeCur)) {
                VLOG(image) << PrettyMethod(&m);
              }
            }
          } else {
  ...
  if (*out_forward_dex_cache_array) {
    ...
    header.VisitPackedArtMethods(&visitor, space->Begin(), kPointerSizeCur);
...
bool ClassLinker::AddImageSpace(
  ...
  if (kIsDebugBuild && app_image) {
    ...
    header.VisitPackedArtMethods(&visitor2, space->Begin(), kPointerSizeCur);
  }
...
mirror::PointerArray* ClassLinker::AllocPointerArray(Thread* self, size_t length) {
  return down_cast<mirror::PointerArray*>(image_pointer_size_ == PointerSize::kPointerSize64
