    ...
    for (size_t j = 0; j < super_vtable_length; ++j) {
      ...
      if (hash_index != hash_table.GetNotFoundIndex()) {
        ...
        if (!FindDefaultMethodImplementation(self,
          ...
          continue;
        } else if (UNLIKELY(default_method == nullptr)) {
          // We note that this vtable entry must be made abstract.
          default_translations->insert({j, ClassLinker::MethodTranslation::CreateAbstractMethod()});
        } else if (UNLIKELY(super_method->IsDefaultConflict() ||
  ...
  for (size_t k = ifstart + 1; k < iftable_count; k++) {
    ...
    for (size_t m = 0; m < num_instance_methods; m++) {
      ...
      if (UNLIKELY(target.HasSameNameAndSignature(
        ...
        // Check if the i'th interface is a supertype of this one.
  ...
  for (size_t k = iftable_count; k != 0; ) {
    ...
    for (size_t m = 0; m < num_instance_methods; m++) {
      ...
      if (UNLIKELY(chosen_iface.Get() != nullptr)) {
        ...
        // masked by the chosen interface or some other interface.
        if (!iface->IsAssignableFrom(chosen_iface.Get()) &&
            !ContainsOverridingMethodOf(self, target_name_comparator, iftable, k, iface,
                                        image_pointer_size_)) {
      ...
      } else if (!ContainsOverridingMethodOf(self, target_name_comparator, iftable, k, iface,
                                             image_pointer_size_)) {
...
static ArtMethod* FindMatchingMethod(MethodNameAndSignatureComparator& cmp,
...
bool ClassLinker::LinkInterfaceMethods(
  ...
  for (size_t i = ifcount; i != 0; ) {
    ...
    if (num_methods > 0) {
      ...
      // TODO In the presence of default methods this optimization is (somewhat) incorrect. Default
      //      methods should always look through all the superclasses because they are the last
      //      choice of an implementation. We get around this by looking at the super-classes
      //      iftable methods (copied into method_array previously) when we are looking for the
      //      implementation of a super-interface method but that is rather dirty.
      if (super_interface) {
      ...
      for (size_t j = 0; j < num_methods; ++j) {
        ...
        if (UNLIKELY(!found_impl && super_interface)) {
          ...
          // TODO This is rather dirty but it is faster then searching through the entire vtable
          //      every time.
        ...
        if (UNLIKELY(!found_impl)) {
          ...
          if (UNLIKELY(!FindDefaultMethodImplementation(self,
            ...
            if (vtable_impl != nullptr && vtable_impl->IsDefaultConflict()) {
              // We can use the method from the superclass, don't bother adding it to virtuals.
              default_conflict_method = vtable_impl;
            } else {
              // Se if we already have a conflict method for this method.
              ArtMethod* preexisting_conflict = FindMatchingMethod(interface_name_comparator,
            ...
            if (vtable_impl != nullptr &&
              ...
              ArtMethod* old = FindMatchingMethod(interface_name_comparator, default_methods);
              if (old == nullptr) {
            ...
            SetIMTRef(unimplemented_method,
                      ...
                      /*out*/imt_ptr);
            found_impl = true;
          } else if (vtable_impl != nullptr &&
                     vtable_impl->IsAbstract() &&
                     !vtable_impl->IsDefaultConflict()) {
            // We need to make this an abstract method but the version in the vtable already is so
            // don't do anything.
            method_array->SetElementPtrSize(j, vtable_impl, image_pointer_size_);
            SetIMTRef(unimplemented_method,
                      conflict_method,
                      image_pointer_size_,
                      vtable_impl,
        ...
        if (!found_impl && !super_interface) {
          ...
          ArtMethod* miranda_method = FindMatchingMethod(interface_name_comparator,
                                                         miranda_methods);
      ...
      if (translation_it != default_translations.end()) {
        if (translation_it->second.IsInConflict()) {
          ...
          for (auto& mir_method : miranda_methods) {
            if (old_method_comparator.HasSameNameAndSignature(mir_method)) {
