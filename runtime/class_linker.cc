...
class MethodNameAndSignatureComparator FINAL : public ValueObject {
 ...
 private:
  ...
  // Lazily compute name from the dex file's strings.
  const char* name_;
  // Lazily compute name length.
  uint32_t name_len_;
};
...
bool ClassLinker::LinkVirtualMethods(Thread* self, Handle<mirror::Class> klass) {
  ...
  if (klass->HasSuperClass()) {
    ...
    for (size_t i = 0; i < num_virtual_methods; ++i) {
      ...
      MethodNameAndSignatureComparator virtual_method_name_comparator(local_method);
      size_t j = 0;
    ...
    if (num_methods > 0) {
      ...
      for (size_t j = 0; j < num_methods; ++j) {
        ...
        for (k = input_array->GetLength() - 1; k >= 0; --k) {
          ...
          if (interface_name_comparator.HasSameNameAndSignature(vtable_method)) {
            ...
            if (imt_ref == runtime->GetImtUnimplementedMethod()) {
              ...
              MethodNameAndSignatureComparator imt_ref_name_comparator(imt_ref);
              if (imt_ref_name_comparator.HasSameNameAndSignature(vtable_method)) {
                out_imt->SetReference(imt_index, vtable_method);
              } else {
                out_imt->SetReference(imt_index, conflict_method);
              }
