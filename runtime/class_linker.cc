...
void ClassLinker::InitFromImageInterpretOnlyCallback(mirror::Object* obj, void* arg) {
  ...
  if (obj->IsArtMethod() && (false)) {
    ...
    if (!method->IsNative()) {
      ...
      Runtime* runtime = Runtime::Current();
      if (method != runtime->GetResolutionMethod() && method != runtime->GetImtConflictMethod()) {
...
const void* ClassLinker::GetQuickOatCodeFor(mirror::ArtMethod* method) {
  ...
  if (jit != nullptr) {
    ...
    // LOG(INFO) << entrypoint << "@" << PrettyMethod(method) << " " << reinterpret_cast<const void*>(jit->GetCodeCache()->CodeCachePtr());
    if (jit->GetCodeCache()->ContainsCodePtr(entrypoint)) {
      return entrypoint;
    }
  }
  if (method->IsNative()) {
