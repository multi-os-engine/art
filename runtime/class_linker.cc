...
bool ClassLinker::ValidateSuperClassDescriptors(Handle<mirror::Class> klass) {
  ...
  if (klass->HasSuperClass() &&
    ...
    for (int i = klass->GetSuperClass()->GetEmbeddedVTableLength() - 1; i >= 0; --i) {
      mh.ChangeMethod(klass->GetEmbeddedVTableEntry(i));
      super_mh.ChangeMethod(klass->GetSuperClass()->GetEmbeddedVTableEntry(i));
...
bool ClassLinker::LinkVirtualMethods(Thread* self, Handle<mirror::Class> klass) {
  if (klass->HasSuperClass()) {
    uint32_t max_count = klass->NumVirtualMethods() +
        klass->GetSuperClass()->GetEmbeddedVTableLength();
    size_t actual_count = klass->GetSuperClass()->GetEmbeddedVTableLength();
    ...
    for (int i=0; i<klass->GetSuperClass()->GetEmbeddedVTableLength(); i++) {
      vtable->Set<false>(i, klass->GetSuperClass()->GetEmbeddedVTableEntry(i));
    }
