...
bool ClassLinker::LinkFields(Thread* self, Handle<mirror::Class> klass, bool is_static,
  ...
  if (kIsDebugBuild) {
    // Make sure that fields are ordered by name and all reference fields are at the
    // beginning as far as alignment allows.
    MemberOffset start_ref_offset =
        !is_static
        ? klass->GetFirstReferenceInstanceFieldOffset()
        // Can't use klass->GetFirstReferenceStaticFieldOffset() yet.
        : MemberOffset(klass->ShouldHaveEmbeddedImtAndVTable()
    ...
    MemberOffset end_ref_offset(start_ref_offset.Uint32Value() +
                                num_reference_fields * sizeof(uint32_t));
    ...
    for (size_t i = 0; i < num_fields; i++) {
      ...
      if (is_primitive) {
        if (offset.Uint32Value() < end_ref_offset.Uint32Value()) {
          ...
          CHECK(!IsAligned<4>(offset.Uint32Value()));
        }
      } else {
        CHECK_EQ(current_ref_offset.Uint32Value(), offset.Uint32Value());
        current_ref_offset = MemberOffset(current_ref_offset.Uint32Value() + sizeof(uint32_t));
      }
    }
  ...
  if (super_class != nullptr) {
    ...
    if (reference_offsets != mirror::Class::kClassWalkSuper) {
      ...
      if (num_reference_fields != 0u) {
        ...
        } else if (num_reference_fields == 32) {
          // NOTE: Working around shift by 32 which is undefined behavior.
          DCHECK_EQ(start_bit, 0u);
          reference_offsets = 0xffffffffu;
        } else {
          reference_offsets |= ((1u << num_reference_fields) - 1u) << start_bit;
        }
      }
    }
  }
