...
void ClassLinker::VisitClassRoots(RootVisitor* visitor, VisitRootFlags flags) {
  ...
  if ((flags & kVisitRootFlagAllRoots) != 0) {
    for (GcRoot<mirror::Class>& root : class_table_) {
      ...
      root.Read()->VisitNativeRoots(buffered_visitor);
    }
    ...
    for (auto& root : new_class_roots_) {
      ...
      old_ref->VisitNativeRoots(buffered_visitor);
...
mirror::PointerArray* ClassLinker::AllocPointerArray(Thread* self, size_t length) {
  return down_cast<mirror::PointerArray*>(image_pointer_size_ == 8 ?
...
void ClassLinker::LoadClassMembers(Thread* self, const DexFile& dex_file,
  ...
  for (size_t i = 0; it.HasNextDirectMethod(); i++, it.Next()) {
    ArtMethod* method = klass->GetDirectMethod(i, image_pointer_size_);
  ...
  for (size_t i = 0; it.HasNextVirtualMethod(); i++, it.Next()) {
    ArtMethod* method = klass->GetVirtualMethod(i, image_pointer_size_);
    LoadMethod(self, dex_file, it, klass, method);
...
mirror::Class* ClassLinker::CreateArrayClass(Thread* self, const char* descriptor, size_t hash,
  ...
  {
    ArtMethod* imt_methods[mirror::Class::kImtSize] = {
        Runtime::Current()->GetImtUnimplementedMethod()
    };
    new_class->PopulateEmbeddedImtAndVTable(imt_methods, image_pointer_size_);
  }
...
mirror::Class* ClassLinker::CreateProxyClass(ScopedObjectAccessAlreadyRunnable& soa, jstring name,
  ...
  // Proxies have 1 direct method, the constructor
  {
    ...
    CreateProxyConstructor(klass, proxy_class, klass->GetDirectMethod(0, image_pointer_size_));
  }
  ...
  size_t i = 0;
  for (auto& virtual_method : klass->GetVirtualMethods(image_pointer_size_)) {
...
void ClassLinker::CreateProxyConstructor(Handle<mirror::Class> klass, mirror::Class* proxy_class,
                                         ArtMethod* out) {
  // Create constructor for Proxy that must initialize h
  CHECK_EQ(proxy_class->NumDirectMethods(), 16u);
  ArtMethod* proxy_constructor = proxy_class->GetDirectMethod(2, image_pointer_size_);
  // Ensure constructor is in dex cache so that we can use the dex cache to look up the overridden
  // constructor method.
  proxy_class->GetDexCache()->SetResolvedMethod(proxy_constructor->GetDexMethodIndex(),
                                                proxy_constructor, image_pointer_size_);
...
void ClassLinker::CheckProxyMethod(ArtMethod* method, ArtMethod* prototype) const {
  ...
  CHECK_EQ(method->GetInterfaceMethodIfProxy(image_pointer_size_)->GetReturnType(),
...
static bool HasSameSignatureWithDifferentClassLoaders(Thread* self, ArtMethod* method1,
                                                      ArtMethod* method2, std::string* error_msg)
    SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  {
    StackHandleScope<1> hs(self);
  ...
  if (types1 == nullptr) {
    if (types2 != nullptr && types2->Size() != 0) {
      *error_msg = StringPrintf("Type list mismatch with %s", PrettyMethod(method2, true).c_str());
      return false;
    }
    return true;
...
bool ClassLinker::ValidateSuperClassDescriptors(Handle<mirror::Class> klass) {
  ...
  if (klass->HasSuperClass() &&
    ...
    for (int i = klass->GetSuperClass()->GetVTableLength(image_pointer_size_) - 1; i >= 0; --i) {
      auto* m = klass->GetVTableEntry(i, image_pointer_size_);
      auto* super_m = klass->GetSuperClass()->GetVTableEntry(i, image_pointer_size_);
      if (m != super_m) {
        std::string error_msg;
        if (!HasSameSignatureWithDifferentClassLoaders(self, m, super_m, &error_msg)) {
          ThrowLinkageError(klass.Get(),
                            "Class %s method %s resolves differently in superclass %s: %s",
                            PrettyDescriptor(klass.Get()).c_str(), PrettyMethod(m).c_str(),
                            PrettyDescriptor(klass->GetSuperClass()).c_str(), error_msg.c_str());
          return false;
        }
      }
    }
  ...
  for (int32_t i = 0; i < klass->GetIfTableCount(); ++i) {
    if (klass->GetClassLoader() != klass->GetIfTable()->GetInterface(i)->GetClassLoader()) {
      ...
      for (uint32_t j = 0; j < num_methods; ++j) {
        auto* ift = klass->GetIfTable();
        auto* m = ift->GetMethodArray(i)->GetElementPtrSize<ArtMethod*>(j, image_pointer_size_);
        auto* super_m = ift->GetInterface(i)->GetVirtualMethod(j, image_pointer_size_);
        if (m != super_m) {
          std::string error_msg;
          if (!HasSameSignatureWithDifferentClassLoaders(self, m, super_m, &error_msg)) {
            ThrowLinkageError(klass.Get(),
...
bool ClassLinker::LinkClass(Thread* self, const char* descriptor, Handle<mirror::Class> klass,
  ...
  ArtMethod* imt[mirror::Class::kImtSize] = { Runtime::Current()->GetImtUnimplementedMethod() };
  ...
  if (!klass->IsTemp() || (!init_done_ && klass->GetClassSize() == class_size)) {
    ...
    *new_class = klass->CopyOf(self, class_size, imt, image_pointer_size_);
...
bool ClassLinker::LinkInterfaceMethods(Thread* self, Handle<mirror::Class> klass,
  ...
  std::vector<std::unique_ptr<ArtMethod>> miranda_methods;
  ...
  for (size_t i = 0; i < ifcount; ++i) {
    ...
    if (num_methods > 0) {
      ...
      Handle<mirror::PointerArray> method_array;

      ArtMethod* input_virtual_methods = nullptr;
      Handle<mirror::PointerArray> input_vtable_array;
      ...
      if (super_interface) {
        ...
        method_array = hs2.NewHandle(down_cast<mirror::PointerArray*>(
        ...
        method_array = hs2.NewHandle(AllocPointerArray(self, num_methods));
      ...
      for (size_t j = 0; j < num_methods; ++j) {
        ...
        for (k = input_array_length - 1; k >= 0; --k) {
          ArtMethod* vtable_method = input_virtual_methods != nullptr ?
              input_virtual_methods + ArtMethod::ObjectSize(image_pointer_size_) * k :
          ...
          if (interface_name_comparator.HasSameNameAndSignature(
            ...
            if (imt_ref == unimplemented_method) {
              ...
              // If we are not a conflict anstd::vectord we have the same signature and name as the imt entry,
        ...
        if (k < 0 && !super_interface) {
          ...
          for (auto& mir_method : miranda_methods) {
            if (interface_name_comparator.HasSameNameAndSignature(mir_method.get())) {
              miranda_method = mir_method.get();
              break;
            }
          }
          if (miranda_method == nullptr) {
            // Point the interface table at a phantom slot.
            miranda_methods.push_back(
                MakeUnique<ArtMethod>(*interface_method, image_pointer_size_));
          }
  ...
  if (!miranda_methods.size()) {
    ...
    StrideIterator<ArtMethod> out(
        reinterpret_cast<uintptr_t>(virtuals), ArtMethod::ObjectSize(image_pointer_size_));
    ...
    std::unordered_map<ArtMethod*, ArtMethod*> move_table;
    for (auto& mir_method : miranda_methods) {
      ArtMethod* out_method = &*out;
      out->CopyFrom(mir_method.get(), image_pointer_size_);
      ...
      move_table.insert(std::make_pair(mir_method.get(), out_method));
      ++out;
      ++old_vtable_count;
    }
    CHECK_EQ(old_vtable_count, new_vtable_count);
    // TODO: Fix up pointers in the interface table.
