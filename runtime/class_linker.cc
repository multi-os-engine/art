...
void ClassLinker::InitFromCompiler(const std::vector<const DexFile*>& boot_class_path) {
  ...
  // Setup Reference.
  Handle<mirror::Class> java_lang_ref_Reference(hs.NewHandle(AllocClass(self, java_lang_Class.Get(),
  ...
  // Flag denotes class is Reference, and not its subclass
  Reference_class->SetAccessFlags(
      Reference_class->GetAccessFlags() |
          kAccClassIsReferenceClass);
...
void ClassLinker::FinishInit(Thread* self) {
  ...
  down_cast<mirror::ReferenceClass*>(mirror::Reference::GetJavaLangRefReference())->Init();
