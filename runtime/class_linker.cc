...
void ClassLinker::VerifyClass(Thread* self, Handle<mirror::Class> klass) {
  ...
  if (NullOrVerified(supertype) && !klass->IsInterface()) {
    ...
    for (int32_t i = 0; i < iftable_count; i++) {
      supertype.Assign(klass->GetIfTable()->GetInterface(i));
      // We only care if we have default interfaces and can skip if we are already verified...
      if (LIKELY(!supertype->HasDefaultMethods() || supertype->IsVerified())) {
        continue;
      } else if (UNLIKELY(!AttemptSupertypeVerification(self, klass, supertype))) {
        return;
      } else if (UNLIKELY(!NullOrVerified(supertype))) {
        // We failed to verify the supertype. Stop checking and clean up.
        break;
      }
    }
  }
...
ArtMethod* ClassLinker::ResolveMethod(const DexFile& dex_file,
  ...
  switch (type) {
    ...
    case kSuper: {
      // TODO fill in.
      resolved = klass->FindSuperMethod(dex_cache.Get(), method_idx, image_pointer_size_);
      break;
    }
    case kVirtual:
  ...
  if (resolved == nullptr) {
    ...
    switch (type) {
      ...
      case kSuper: {
        // TODO fill in.
        resolved = klass->FindSuperMethod(name, signature, image_pointer_size_);
        break;
      }
      case kVirtual:
