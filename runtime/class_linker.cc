...
inline void ClassLinker::AddGap(uint32_t gap_start, uint32_t gap_end, FieldGaps* gaps) {
  DCHECK(gaps != nullptr);
...
template<int n>
void ClassLinker::ShuffleForward(const size_t num_fields, size_t* current_field_idx,
  ...
  while (!grouped_and_sorted_fields->empty()) {
    ...
    if (!IsAligned<n>(field_offset->Uint32Value())) {
      ...
      *field_offset = MemberOffset(RoundUp(field_offset->Uint32Value(), n));
      AddGap(old_offset.Uint32Value(), field_offset->Uint32Value(), gaps);
    }
    ...
    if (!gaps->empty() && gaps->top().size >= n) {
      ...
      if (gap.size > n) {
        AddGap(gap.start_offset + n, gap.start_offset + gap.size, gaps);
      }
    } else {
