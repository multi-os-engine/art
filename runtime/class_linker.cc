...
const OatFile* ClassLinker::FindOatFileContainingDexFileFromDexLocation(
  ...
  std::unique_ptr<const OatFile> oat_file(OpenOatFileFromDexLocation(dex_location_str, isa,
                                                                     &already_opened, &error_msg));
  if (oat_file.get() == nullptr) {
    error_msgs->push_back(StringPrintf("Failed to open oat file from dex location '%s': %s",
                                       dex_location, error_msg.c_str()));
    return nullptr;
...
const OatFile* ClassLinker::OpenOatFileFromDexLocation(const std::string& dex_location,
  ...
  GetDalvikCache(GetInstructionSetString(kRuntimeISA), &dalvik_cache,
                 &have_android_data, &have_dalvik_cache, false);
  ...
  // We know that niether the odex nor the cache'd version is already in use, if it even exists.
  //
  ...
  // 6) Close the odex version to free up its address space.
  ...
  *already_opened = false;
  ...
  bool odex_checksum_verified = false;
  {
    ...
    if (odex_oat_file.get() != nullptr && CheckOatFile(odex_oat_file.get(), isa,
                                                       ...
                                                       &odex_error_msg)) {
      *error_msg = odex_error_msg;
      return odex_oat_file.release();
    } else if (odex_checksum_verified) {
      // We can just relocate
      odex_patch_delta = GetRequiredDelta(odex_oat_file.get(), isa);
  ...
  if (have_dalvik_cache) {
    ...
    if (cache_oat_file.get() != nullptr && CheckOatFile(cache_oat_file.get(), isa,
      ...
      *error_msg = cache_error_msg;
      ...
      // We can just relocate
      cache_patch_delta = GetRequiredDelta(cache_oat_file.get(), isa);
  ...
  if (runtime->CanRelocate()) {
    // Run relocation
    if (odex_checksum_verified && IsAligned<kPageSize>(odex_patch_delta)) {
      ret = PatchAndRetrieveOat(odex_filename, cache_filename, odex_patch_delta, isa, error_msg);
    } else if (cache_checksum_verified && IsAligned<kPageSize>(cache_patch_delta)) {
      CHECK(have_dalvik_cache);
      ret = PatchAndRetrieveOat(cache_filename, cache_filename, cache_patch_delta, isa, error_msg);
    }
  }
  if (ret == nullptr && have_dalvik_cache && OS::FileExists(cache_filename.c_str())) {
    ...
    if (TEMP_FAILURE_RETRY(unlink(cache_filename.c_str())) != 0) {
      PLOG(FATAL) << "Failed to remove obsolete oat file from " << cache_filename;
    }
  }
  if (ret == nullptr) {
    std::string relocation_msg;
    if (runtime->CanRelocate()) {
      relocation_msg = StringPrintf(" and relocation failed (error %s)", error_msg->c_str());
    }
...
const OatFile* ClassLinker::PatchAndRetrieveOat(const std::string& input_oat,
  ...
  std::string base_offset_delta_arg("--base-offset-delta=");
  StringAppendF(&base_offset_delta_arg, "%d", delta);

  std::vector<std::string> argv;
  argv.push_back(patchoat);
  argv.push_back(input_oat_filename_arg);
  argv.push_back(output_oat_filename_arg);
  argv.push_back(base_offset_delta_arg);
  ...
  if (success) {
    ...
    if (output.get() != nullptr && CheckOatFile(output.get(), isa, &checksum_verified, error_msg)) {
      ...
      *error_msg = StringPrintf("Patching of oat file '%s' with delta of %d succeeded "
                                "but output file '%s' failed verifcation: %s",
                                input_oat.c_str(), delta, output_oat.c_str(), error_msg->c_str());
    } else {
      *error_msg = StringPrintf("Patching of oat file '%s' with delta of %d succeeded "
    ...
    *error_msg = StringPrintf("Patching of oat file '%s' with delta of %d to '%s' "
                              "failed: %s", input_oat.c_str(), delta, output_oat.c_str(),
                              error_msg->c_str());
  }
  return nullptr;
}
...
int32_t ClassLinker::GetRequiredDelta(const OatFile* oat_file, InstructionSet isa) {
  Runtime* runtime = Runtime::Current();
  CHECK(IsSameIsa(runtime->GetInstructionSet(), isa)) << "Expected: " << isa << " Recieved: "
                                                      << runtime->GetInstructionSet();
  const ImageHeader& image_header = runtime->GetHeap()->GetImageSpace()->GetImageHeader();
  const OatHeader& oat_header = oat_file->GetOatHeader();
  return image_header.GetPatchDelta() - oat_header.GetImagePatchDelta();
}
...
bool ClassLinker::CheckOatFile(const OatFile* oat_file, InstructionSet isa,
  ...
  CHECK(IsSameIsa(runtime->GetInstructionSet(), isa)) << "expected: " << isa << " recieved: "
                                                      << runtime->GetInstructionSet();
  const ImageHeader& image_header = runtime->GetHeap()->GetImageSpace()->GetImageHeader();
  uint32_t real_image_checksum = image_header.GetOatChecksum();
  void* real_image_oat_offset = reinterpret_cast<void*>(image_header.GetOatDataBegin());
  int32_t real_patch_delta = image_header.GetPatchDelta();
