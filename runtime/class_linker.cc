...
bool ClassLinker::InitializeClass(Thread* self, Handle<mirror::Class> klass,
  ...
  if (!klass->IsInterface()) {
    ...
    for (size_t i = 0; i < num_direct_interfaces; i++) {
      ...
      // with default methods are initialized. Normal interface initialization will not affect
      // super-interfaces.
...
bool ClassLinker::InitializeDefaultInterfaceRecursive(Thread* self,
  ...
  // interface since we can still avoid the traversal.
  if (result) {
...
bool ClassLinker::LinkVirtualMethods(
  ...
  if (klass->IsInterface()) {
    ...
    if (super_class->ShouldHaveEmbeddedImtAndVTable()) {
      ...
      // might give us new default methods). If not then we can skip the rest since the class cannot
      // override any of the super-class's methods.
...
bool ClassLinker::FindDefaultMethodImplementation(Thread* self,
  ...
  *out_default_method = nullptr;
  ...
  for (size_t k = iftable_count - 1; true; k--) {
...
static size_t FillIfTable(mirror::IfTable* iftable,
  ...
  for (mirror::Class* interface : to_process) {
    ...
    if (classes_in_iftable.find(interface) == classes_in_iftable.end()) {
      ...
      for (int32_t j = 0; j < ifcount; j++) {
        ...
        if (classes_in_iftable.find(super_interface) == classes_in_iftable.end()) {
...
bool ClassLinker::LinkInterfaceMethods(
  ...
  if (!miranda_methods.empty() || !default_methods.empty()) {
    ...
    for (ArtMethod* mir_method : miranda_methods) {
      ...
      DCHECK(new_method.GetAccessFlags() & kAccAbstract) << "Miranda method should be abstract!";
    ...
    for (ArtMethod* def_method : default_methods) {
      ...
      out->SetAccessFlags(new_method.GetAccessFlags() & ~kAccPreverified);
    ...
    for (size_t i = 0; i < old_vtable_count; ++i) {
      ArtMethod* m = vtable->GetElementPtrSize<ArtMethod*>(i, image_pointer_size_);
      // Try and find what we need to change this method to.
      std::unordered_map<int32_t, ArtMethod*>::const_iterator translation =
          default_translations.find(i);
