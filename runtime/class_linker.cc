...
const OatFile* ClassLinker::FindOpenedOatFile(ExecutableOatFileManager& oat_file_manager) {
  ReaderMutexLock mu(Thread::Current(), dex_lock_);
  for (const OatFile* oat_file : oat_files_) {
    CHECK(oat_file != nullptr);
    if (oat_file_manager.GivenOatFileIsUpToDate(*oat_file)) {
      return oat_file;
    }
  }
  return nullptr;
...
std::vector<std::unique_ptr<const DexFile>> ClassLinker::OpenDexFilesFromOat(
  ...
  ExecutableOatFileManager oat_file_manager(dex_location, oat_location);

  // Lock the target oat location to avoid races generating and loading the
  // oat file.
  std::string error_msg;
  if (!oat_file_manager.Lock(&error_msg)) {
    // Don't worry too much if this fails. If it does fail, it's unlikely we
    // can generate an oat file anyway.
    VLOG(class_linker) << "OatFileManager::Lock: " << error_msg;
  }

  // Check if we already have an up-to-date oat file open.
  source_oat_file = FindOpenedOatFile(oat_file_manager);

  // If we didn't have an up-to-date oat file open, try to load one from disk.
  if (source_oat_file == nullptr) {
    // Update the oat file on disk if we can. This may fail, but that's okay.
    // Best effort is all that matters here.
    if (!oat_file_manager.MakeUpToDate(&error_msg)) {
  ...
  if (source_oat_file != nullptr) {
    dex_files = oat_file_manager.LoadDexFiles(*source_oat_file, dex_location);
    if (dex_files.empty()) {
  ...
  // Fall back to interpreted running if we couldn't load any dex_files.
  if (dex_files.empty()) {
