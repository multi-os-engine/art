  ...
  if (preverified || verifier_failure != verifier::MethodVerifier::kHardFailure) {
    ...
    if (verifier_failure == verifier::MethodVerifier::kNoFailure) {
      ...
      if (Runtime::Current()->IsAotCompiler()) {
        ...
        mirror::Class::SetStatus(klass, mirror::Class::kStatusVerified, self);
        // As this is a fake verified status, make sure the methods are _not_ marked skip_access_checks
        // later.
...
mirror::Class* ClassLinker::CreateProxyClass(ScopedObjectAccessAlreadyRunnable& soa,
  ...
  // Set the class access flags incl. VerificationAttempted, so we do not try to set the flag on the methods.
...
bool ClassLinker::LinkInterfaceMethods(
  ...
  if (has_new_virtuals) {
    ...
    for (ArtMethod* def_method : default_methods) {
      ...
      // Clear the skip_access_checks flag if it is present. Since this class hasn't been verified yet it
      // shouldn't have methods that are skip_access_checks.
    ...
    for (ArtMethod* conf_method : default_conflict_methods) {
      ...
      // this as a default, non-abstract method, since thats what it is. Also clear the skip_access_checks
