...
void SanityCheckImageObjectsCallback(mirror::Object* obj, void* arg ATTRIBUTE_UNUSED)
    SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  DCHECK(obj != nullptr);
  if (obj->GetClass() == nullptr) {
    LOG(ERROR) << "Null class " << obj;
    return;
  }
  if (obj->IsClass()) {
...
void ClassLinker::InitFromImage() {
  ...
  if (kIsDebugBuild) {
    heap->VisitObjects(SanityCheckImageObjectsCallback, nullptr);
  }
...
void ClassLinker::VisitClassRoots(RootVisitor* visitor, VisitRootFlags flags) {
  ...
  if ((flags & kVisitRootFlagAllRoots) != 0) {
    for (GcRoot<mirror::Class>& root : class_table_) {
      ...
      // TODO: Only visit fields if the class moved?
    ...
    for (auto& root : new_class_roots_) {
      ...
      new_ref->VisitFieldRoots(buffered_visitor);
...
void ClassLinker::LoadClassMembers(Thread* self, const DexFile& dex_file,
  ...
  CHECK_EQ(klass->NumStaticFields(), num_sfields);
  CHECK_EQ(klass->NumInstanceFields(), num_ifields);
...
mirror::Class* ClassLinker::CreateProxyClass(ScopedObjectAccessAlreadyRunnable& soa, jstring name,
  ...
  ArtField* sfields = AllocArtFieldArray(self, 2);
  klass->SetSFields(sfields);
