...
bool ClassLinker::LinkInterfaceMethods(Thread* self, Handle<mirror::Class> klass,
  ...
  for (size_t i = 0; i < ifcount; ++i) {
    ...
    if (num_methods > 0) {
      ...
      uintptr_t input_virtual_methods = 0;
      ...
      if (super_interface) {
        ...
        // whole vtable.
        input_virtual_methods = reinterpret_cast<uintptr_t>(klass->GetVirtualMethodsPtr());
      ...
      for (size_t j = 0; j < num_methods; ++j) {
        ...
        for (k = input_array_length - 1; k >= 0; --k) {
          ArtMethod* vtable_method = input_virtual_methods != 0u ?
              reinterpret_cast<ArtMethod*>(input_virtual_methods + method_size * k) :
        ...
        if (k < 0 && !super_interface) {
          ...
          if (miranda_method == nullptr) {
            ...
            miranda_method = miranda_methods.back().get();
          }
  ...
  if (!miranda_methods.empty()) {
    ...
    klass->SetVirtualMethodsPtr(virtuals);
    klass->SetNumVirtualMethods(new_method_count);
...
ArtMethod* ClassLinker::ResolveMethod(const DexFile& dex_file, uint32_t method_idx,
  ...
  switch (type) {
    ...
    case kStatic:
      ...
      DCHECK(resolved->GetDeclaringClassUnchecked() != nullptr);
      break;
    case kInterface:
...
ArtMethod* ClassLinker::CreateRuntimeMethod() {
  ...
  method->SetDeclaringClass(nullptr);
