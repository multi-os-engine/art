...
static bool ContainsOverridingMethodOf(Thread* self,
  ...
  DCHECK_LE(ifstart, iftable->Count());
  ...
  for (size_t k = ifstart + 1; k < iftable_count; k++) {
    ...
    for (size_t m = 0; m < num_instance_methods; m++) {
      ...
      if (UNLIKELY(target.HasSameNameAndSignature(
        ...
        DCHECK(iface.Get() != current_iface.Get())
            << "Started before one of the interfaces being checked";
      ...
      if (UNLIKELY(chosen_iface.Get() != nullptr)) {
        ...
        if (!iface->IsAssignableFrom(chosen_iface.Get()) &&
            !ContainsOverridingMethodOf(self, target_name_comparator, iftable, k, iface,
                                        image_pointer_size_)) {
          if (kIsDebugBuild || UNLIKELY(VLOG_IS_ON(class_linker))) {
        ...
        if (UNLIKELY(!found_impl)) {
          ...
          if (UNLIKELY(!FindDefaultMethodImplementation(self,
            ...
            SetIMTRef(unimplemented_method,
                      ...
                      /*out*/imt_ptr);
            found_impl = true;
          } else if (vtable_impl != nullptr && vtable_impl->IsAbstract()) {
            // We need to make this an abstract method but it is already there so we do nothing.
