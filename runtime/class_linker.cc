...
bool ClassLinker::ValidateSuperClassDescriptors(SirtRef<mirror::Class>& klass) {
...
bool ClassLinker::IsSameMethodSignatureInDifferentClassContexts(const mirror::ArtMethod* method,
  ...
  for (DexFileParameterIterator it(dex_file, proto_id); it.HasNext(); it.Next()) {
    ...
    if (descriptor[0] == 'L' || descriptor[0] == '[') {
      ...
      if (!IsSameDescriptorInDifferentClassContexts(descriptor, class_loader1, class_loader2)) {
        return false;
      }
    }
  }
...
bool ClassLinker::IsSameDescriptorInDifferentClassContexts(const char* descriptor,
  ...
  CHECK(class_loader1.get() != nullptr);
  CHECK(class_loader2.get() != nullptr);
  Thread* self = Thread::Current();
  SirtRef<mirror::Class> found1(self, FindClass(descriptor, class_loader1));
