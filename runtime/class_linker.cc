    ...
    *out_forward_dex_cache_array = num_dex_caches_with_bss_arrays != 0;
    ...
    for (size_t i = 0; i < num_dex_caches; i++) {
      ...
      // If the oat file expects the dex cache arrays to be in the BSS, then allocate there and
        ...
        const size_t num_strings = mirror::DEX_CACHE_SIZE;
        ...
        // std::copy_n.
        // This part is not atomic. However, we won't be reading anything from the new
        if (num_strings != 0u) {
          std::atomic<uint64_t>* const image_resolved_strings = dex_cache->GetStrings();
          std::atomic<uint64_t>* const strings =
              reinterpret_cast<std::atomic<uint64_t>*>(raw_arrays + layout.StringsOffset());
          for (size_t j = 0; kIsDebugBuild && j < num_strings; ++j) {
            DCHECK_EQ(strings[j].load(std::memory_order_relaxed), 0u);
            strings[j].store(image_resolved_strings[j].load(std::memory_order_relaxed), std::memory_order_relaxed);
          }
...
class UpdateClassLoaderAndResolvedStringsVisitor {
 public:
  ...
  bool operator()(mirror::Class* klass) const SHARED_REQUIRES(Locks::mutator_lock_) {
    if (forward_strings_) {
      std::atomic<uint64_t>* strings = klass->GetDexCacheStrings();
      if (strings != nullptr) {
        DCHECK(
        ...
        std::atomic<uint64_t>* new_strings = klass->GetDexCache()->GetStrings();
...
mirror::DexCache* ClassLinker::AllocDexCache(Thread* self,
  ...
  std::atomic<uint64_t>* strings = (dex_file.NumStringIds() == 0u) ? nullptr :
      reinterpret_cast<std::atomic<uint64_t>*>(raw_arrays + layout.StringsOffset());
  ...
  if (kIsDebugBuild) {
    ...
    for (size_t i = 0; i < mirror::DEX_CACHE_SIZE; ++i) {
      CHECK_EQ(strings[i].load(std::memory_order_relaxed), 0u);
    }
