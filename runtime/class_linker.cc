...
// Determine if the given iface has any subinterface in the given list that overrides the method
// specified by 'target'.
//
// Arguments
...
bool ClassLinker::LinkInterfaceMethods(
  ...
  if (!miranda_methods.empty() || !default_methods.empty() || !default_conflict_methods.empty()) {
    ...
    for (ArtMethod* conf_method : default_conflict_methods) {
      ...
      // preverified.
      constexpr uint32_t set_flags = kAccDefault | kAccDefaultConflict;
      constexpr uint32_t mask_flags = ~(kAccAbstract | kAccPreverified);
      new_method.SetAccessFlags((new_method.GetAccessFlags() | set_flags) & mask_flags);
