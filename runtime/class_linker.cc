  ...
  if (!klass->IsInterface()) {
    ...
    if (UNLIKELY(num_direct_interfaces > 0)) {
      ...
      MutableHandle<mirror::Class> handle_scope_iface(
          hs_iface.NewHandle(static_cast<mirror::Class*>(nullptr)));
...
bool ClassLinker::InitializeDefaultInterfaceRecursive(Thread* self,
  ...
  if (num_direct_ifaces > 0) {
    StackHandleScope<1> hs(self);
    MutableHandle<mirror::Class> handle_super_iface(
        hs.NewHandle(static_cast<mirror::Class*>(nullptr)));
