...
static void CheckProxyMethod(Handle<mirror::ArtMethod> method, Handle<mirror::ArtMethod> prototype)
  ...
  CHECK_EQ(MethodHelper(method).GetReturnType(), MethodHelper(prototype).GetReturnType());
}
  ...
  if (klass->HasSuperClass() &&
    ...
    for (int i = klass->GetSuperClass()->GetVTable()->GetLength() - 1; i >= 0; --i) {
      ...
      if (mh.GetMethod() != super_mh.GetMethod()) {
        if (!mh.HasSameSignatureWithDifferentClassLoaders(&super_mh)) {
...
bool ClassLinker::LinkVirtualMethods(Thread* self, Handle<mirror::Class> klass) {
  if (klass->HasSuperClass()) {
    ...
    MethodHelper super_mh(hs.NewHandle<mirror::ArtMethod>(nullptr));
    for (size_t i = 0; i < klass->NumVirtualMethods(); ++i) {
      local_mh.ChangeMethod(klass->GetVirtualMethodDuringLinking(i));
      size_t j = 0;
      for (; j < actual_count; ++j) {
        super_mh.ChangeMethod(vtable->Get(j));
        if (local_mh.HasSameNameAndSignature(&super_mh)) {
          if (klass->CanAccessMember(super_mh.GetMethod()->GetDeclaringClass(),
                                     super_mh.GetMethod()->GetAccessFlags())) {
            if (super_mh.GetMethod()->IsFinal()) {
              ThrowLinkageError(klass.Get(), "Method %s overrides final method in class %s",
                                PrettyMethod(local_mh.GetMethod()).c_str(),
                                super_mh.GetMethod()->GetDeclaringClassDescriptor());
              return false;
            }
            vtable->Set<false>(j, local_mh.GetMethod());
            local_mh.GetMethod()->SetMethodIndex(j);
            break;
          } else {
            LOG(WARNING) << "Before Android 4.1, method " << PrettyMethod(local_mh.GetMethod())
                         << " would have incorrectly overridden the package-private method in "
                         << PrettyDescriptor(super_mh.GetMethod()->GetDeclaringClassDescriptor());
          }
        }
      }
      if (j == actual_count) {
        // Not overriding, append.
        vtable->Set<false>(actual_count, local_mh.GetMethod());
        local_mh.GetMethod()->SetMethodIndex(actual_count);
        actual_count += 1;
      }
    }
      ...
      for (size_t j = 0; j < num_methods; ++j) {
        ...
        for (k = vtable->GetLength() - 1; k >= 0; --k) {
          ...
          if (interface_mh.HasSameNameAndSignature(&vtable_mh)) {
            if (!vtable_mh.GetMethod()->IsAbstract() && !vtable_mh.GetMethod()->IsPublic()) {
              ThrowIllegalAccessError(
                  klass.Get(),
                  "Method '%s' implementing interface method '%s' is not public",
                  PrettyMethod(vtable_mh.GetMethod()).c_str(),
                  PrettyMethod(interface_mh.GetMethod()).c_str());
              return false;
            }
            method_array->Set<false>(j, vtable_mh.GetMethod());
            // Place method in imt if entry is empty, place conflict otherwise.
            uint32_t imt_index = interface_mh.GetMethod()->GetDexMethodIndex() % kImtSize;
            if (imtable->Get(imt_index) == NULL) {
              imtable->Set<false>(imt_index, vtable_mh.GetMethod());
        ...
        if (k < 0) {
          ...
          if (miranda_method.Get() == NULL) {
            ...
            miranda_method.Assign(down_cast<mirror::ArtMethod*>(
                interface_mh.GetMethod()->Clone(self)));
