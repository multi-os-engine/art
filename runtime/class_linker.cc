  ...
  bool operator()(mirror::ArtField* field1, mirror::ArtField* field2)
    ...
    if (type1 != type2) {
      ...
      bool is32bit1 = is_primitive1 && !is64bit1 &&
          (type1 != Primitive::kPrimBoolean && type1 != Primitive::kPrimShort && type1 != Primitive::kPrimChar);
      bool is32bit2 = is_primitive2 && !is64bit2 &&
          (type2 != Primitive::kPrimBoolean && type2 != Primitive::kPrimShort && type2 != Primitive::kPrimChar);
...
bool ClassLinker::LinkFields(Handle<mirror::Class> klass, bool is_static, size_t* class_size) {
  ...
  if (current_field != num_fields && !IsAligned<8>(field_offset.Uint32Value())) {
    for (size_t i = 0; i < grouped_and_sorted_fields.size(); i++) {
      ...
      if ((type == Primitive::kPrimBoolean || type == Primitive::kPrimShort || type == Primitive::kPrimChar) && i < grouped_and_sorted_fields.size()) {
        field = grouped_and_sorted_fields[i];
        CHECK(field->GetTypeAsPrimitiveType() == Primitive::kPrimBoolean || field->GetTypeAsPrimitiveType() == Primitive::kPrimShort || Primitive::kPrimChar);
  ...
  while (!grouped_and_sorted_fields.empty()) {
    ...
    field_offset = MemberOffset(field_offset.Uint32Value() +
                                ((type == Primitive::kPrimLong || type == Primitive::kPrimDouble)
                                 ? sizeof(uint64_t)
                                 : ((type == Primitive::kPrimBoolean || type == Primitive::kPrimShort || type == Primitive::kPrimChar)? sizeof(uint16_t) : sizeof(uint32_t))));
    current_field++;
  }
