 ...
 public:
  ...
  virtual void Visit(ArtMethod* method) NO_THREAD_SAFETY_ANALYSIS {
    ...
    if (resolved_types != nullptr) {
      DCHECK(header_.GetImageSection(ImageHeader::kSectionDexCacheArrays).Contains(
...
bool ClassLinker::AddImageSpace(
  ...
  if (class_table_section.Size() > 0u && !app_image) {
  ...
  if (app_image) {
    ...
    for (int32_t i = 0, num_dex_caches = dex_caches->GetLength(); i < num_dex_caches; i++) {
      ...
      }
      {
        ...
        RegisterDexFileLocked(*dex_file, hs2.NewHandle(dex_cache));
      }
      GcRoot<mirror::Class>* const types = dex_cache->GetResolvedTypes();
      if (!added_classes || kIsDebugBuild) {
        for (int32_t j = 0; j < static_cast<int32_t>(num_types); j++) {
          ...
          if (klass != nullptr) {
            ...
            if (!added_classes) {
              // TODO: If there are multiple dex caches, there may be the same class multiple times
            ...
            DCHECK_EQ(table->Lookup(klass), klass);
