...
static void SanityCheckArtMethodPointerArray(
    ArtMethod** arr, size_t size, size_t pointer_size,
    gc::space::ImageSpace* space) SHARED_REQUIRES(Locks::mutator_lock_) {
  ...
  for (size_t j = 0; j < size; ++j) {
    auto* method = mirror::DexCache::GetElementPtrSize(arr, j, pointer_size);
    // expected_class == null means we are a dex cache.
    if (method != nullptr) {
...
void ClassLinker::VisitDexCacheRoots(RootVisitor* visitor,
                                     GcRoot<mirror::DexCache>* dex_cache_root) {
  dex_cache_root->VisitRoot(visitor, RootInfo(kRootVMInternal));

  mirror::DexCache* dex_cache = dex_cache_root->Read();
  GcRoot<mirror::String>* strings = dex_cache->GetStrings();
  for (size_t i = 0, num_strings = dex_cache->NumStrings(); i != num_strings; ++i) {
    if (strings[i].Read() != nullptr) {
      strings[i].VisitRoot(visitor, RootInfo(kRootVMInternal));
    }
  }
  GcRoot<mirror::String>* resolved_types = dex_cache->GetStrings();
  for (size_t i = 0, num_resolved_types = dex_cache->NumStrings(); i != num_resolved_types; ++i) {
    if (resolved_types[i].Read() != nullptr) {
      resolved_types[i].VisitRoot(visitor, RootInfo(kRootVMInternal));
    }
  }
}
  ...
  Thread* const self = Thread::Current();
  {
    ...
    if ((flags & kVisitRootFlagAllRoots) != 0) {
      for (GcRoot<mirror::DexCache>& dex_cache : dex_caches_) {
        VisitDexCacheRoots(visitor, &dex_cache);
      }
    } else if ((flags & kVisitRootFlagNewRoots) != 0) {
      for (size_t index : new_dex_cache_roots_) {
        VisitDexCacheRoots(visitor, &dex_caches_[index]);
      }
    }
    if ((flags & kVisitRootFlagClearRootLog) != 0) {
...
mirror::DexCache* ClassLinker::AllocDexCache(Thread* self, const DexFile& dex_file) {
  ...
  if (dex_file.NumStringIds() != 0u || dex_file.NumTypeIds() != 0u ||
    ...
    if (sizeof(void*) > 4u && image_pointer_size_ == 4u) {
