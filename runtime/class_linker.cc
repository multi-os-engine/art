...
bool ClassLinker::CheckOatFile(const OatFile* oat_file, InstructionSet isa,
  ...
  *checksum_verified = oat_image_checksum == real_image_checksum;
  if (!*checksum_verified) {
    StringAppendF(&compound_msg, " Oat Image Checksum Incorrect (expected 0x%x, recieved 0x%x)",
  ...
  if (!offset_verified) {
    StringAppendF(&compound_msg, " Oat Image oat offset incorrect (expected 0x%p, recieved 0x%p)",
  ...
  if (!patch_delta_verified) {
    StringAppendF(&compound_msg, " Oat image patch delta incorrect (expected 0x%x, recieved 0x%x)",
