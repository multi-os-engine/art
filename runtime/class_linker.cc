...
void ClassLinker::MoveImageClassesToClassTable() {
  ...
  std::string temp;
  ClassTable* const class_table = InsertClassTableForClassLoader(nullptr /* use bootclasspath */);
  for (int32_t i = 0; i < dex_caches->GetLength(); i++) {
...
mirror::Class* ClassLinker::CreateLambdaProxyClass(ScopedObjectAccessAlreadyRunnable& soa,
  ...
  if (UNLIKELY(existing != nullptr)) {
    ...
    // FIXME: should probably do *existing = true or some such, since the args could've changed (in theory).
    return existing;
  }
  ...
  // Set up the Constructor method.
  {
    ...
    LengthPrefixedArray<ArtMethod>* directs = AllocArtMethodArray(self, allocator, kNumDirectMethods);
    // Currently AllocArtMethodArray cannot return null, but the OOM logic is left there in case we
    // want to throw OOM in the future.
    if (UNLIKELY(directs == nullptr)) {
      self->AssertPendingOOMException();
...
void ClassLinker::CreateLambdaProxyMethod(Handle<mirror::Class> klass,
  ...
  // XX: Don't think the set entry point part is safe if we are called during AOT.
  // CHECK(!Runtime::Current()->IsAotCompiler());  // FIXME: gtest thinks its AOT....
