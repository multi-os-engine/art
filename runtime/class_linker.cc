  ...
  if (klass->IsInterface()) {
    ...
    for (size_t j = 0; j < super_vtable_length; ++j) {
      ...
      if (hash_index != hash_table.GetNotFoundIndex()) {
        ...
        } else if (UNLIKELY(super_method->IsDefaultConflicting() ||
...
// Determine if the given iface has any subinterface that overrides the method specified by 'target'
//
// Arguments
  ...
  *out_default_method = nullptr;
  ...
  for (size_t k = iftable_count; k != 0; ) {
    ...
    for (size_t m = 0; m < num_instance_methods; m++) {
      ...
      } else if (!ContainsOverridingMethodOf(self,
                                             target_name_comparator,
                                             iftable,
                                             k,
                                             iface,
      ...
      if (translation_it != default_translations.end()) {
        if (translation_it->second.IsInConflict()) {
          ...
          if (miranda_method != nullptr) {
            ...
            // We actually didn't need to update this method. Go onto the next one.
            continue;
          }
        } else {
