...
static void SanityCheckArtMethodPointerArray(mirror::PointerArray* arr,
                                             mirror::Class* expected_class, size_t pointer_size)
    SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  CHECK(arr != nullptr);
  ...
  if (obj->IsClass()) {
    ...
    if (klass->ShouldHaveEmbeddedImtAndVTable()) {
      for (size_t i = 0; i < mirror::Class::kImtSize; ++i) {
        SanityCheckArtMethod(klass->GetEmbeddedImTableEntry(i, pointer_size), nullptr);
      }
...
mirror::Class* ClassLinker::CreateProxyClass(ScopedObjectAccessAlreadyRunnable& soa, jstring name,
  ...
  // Proxies have 1 direct method, the constructor
  {
    ...
    CreateProxyConstructor(klass, proxy_class,
                           klass->GetDirectMethodUnchecked(0, image_pointer_size_));
  }
