...
static std::string GetMultiDexClassesDexName(size_t number, const char* dex_location) {
  if (number == 0) {
    ...
    return StringPrintf("%s:classes%zu.dex", dex_location, number + 1);
  }
}
...
static bool LoadMultiDexFilesFromOatFile(const OatFile* oat_file, const char* dex_location,
  ...
  for (size_t i = 0; success; ++i) {
    ...
    const OatFile::OatDexFile* oat_dex_file = oat_file->GetOatDexFile(next_name);
...
bool ClassLinker::OpenMultiDexFilesFromOat(const char* dex_location, const char* oat_location,
                                           std::vector<std::string>* error_msgs,
                                           std::vector<const DexFile*>* dex_files) {
  ...
  const OatFile* open_oat_file = FindOpenedOatFile(oat_location, dex_location,
  ...
  if (open_oat_file == nullptr) {
    if (oat_location != nullptr) {
      ...
      open_oat_file = FindOatFileInOatLocationForDexFile(dex_location, dex_location_checksum,
      ...
      open_oat_file = FindOatFileContainingDexFileFromDexLocation(dex_location,
                                                                  dex_location_checksum_pointer,
  ...
  bool success = LoadMultiDexFilesFromOatFile(open_oat_file, dex_location, false, error_msgs,
                                              dex_files);
  if (success) {
    if (needs_registering) {
      // We opened the oat file, so we must register it.
      RegisterOatFile(open_oat_file);
    }
    return true;
  }
  ...
  if (scoped_flock.GetFile() == nullptr) {
    std::string error_msg;
    if (!scoped_flock.Init(oat_location, &error_msg)) {
      error_msgs->push_back(error_msg);
      return false;
  ...
  // Create the oat file.
  open_oat_file = CreateOatFileForDexLocation(dex_location, scoped_flock.GetFile()->Fd(),
  ...
  success = LoadMultiDexFilesFromOatFile(open_oat_file, dex_location, true, error_msgs, dex_files);
  if (success) {
    RegisterOatFile(open_oat_file);
    return true;
  } else {
    delete open_oat_file;
    return false;
  }
}
