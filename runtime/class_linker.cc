...
void ClassLinker::LoadClassMembers(Thread* self, const DexFile& dex_file,
  ...
  // Ensure that the card is marked in so that remembered sets pick up native roots.
...
bool ClassLinker::LinkClass(Thread* self, const char* descriptor, Handle<mirror::Class> klass,
  ...
  if (!klass->IsTemp() || (!init_done_ && klass->GetClassSize() == class_size)) {
    ...
    // Set 0 to null since we don't want the GC to ever see two classes with the same native
    // arrays. If this occurs, it causes bugs in remembered sets since the GC may not see any
    // references to the from space and clean the card. Though there was references to the from
    // space which got marked by the first class.
