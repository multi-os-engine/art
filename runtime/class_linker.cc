...
bool ClassLinker::InitFromImage(std::string* error_msg) {
  ...
  if (class_tables_added != 0) {
    // If an image space section it should still have a non-empty class section with 0 classes in
    // the hash set.
    CHECK_EQ(spaces.size(), class_tables_added)
        << "Expected non-empty class section for each for each image space.";
    dex_cache_boot_image_class_lookup_required_ = false;
  }
