...
mirror::Class* ClassLinker::FindClass(Thread* self, const char* descriptor,
  ...
  if (descriptor[0] == '[') {
    ...
    if (FindClassInPathClassLoader(soa, self, descriptor, hash, class_loader, &cp_klass)) {
      ...
      // TODO: We handle the boot cp loader in FindClassInPathClassLoader. Try to unify this and
      //       the branch above. The current issue is to throw the right exception.
