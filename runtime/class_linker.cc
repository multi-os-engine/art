...
OatFile::OatClass ClassLinker::FindOatClass(const DexFile& dex_file,
  ...
  Runtime* const runtime = Runtime::Current();
  if (runtime->UseJit() && runtime->IsNativeDebuggingEnabled()) {
    // If we are using JIT and native debugging is enabled then we don't want to use any precompiled
    // code from the oat files as they don't contain native debug info.
    *found = false;
