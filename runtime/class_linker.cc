...
void ClassLinker::VisitClasses(ClassVisitor* visitor) {
  ...
  ReaderMutexLock mu(Thread::Current(), *Locks::classlinker_classes_lock_);
  // Not safe to have thread suspension when we are holdin ga lock.
  ScopedAssertNoThreadSuspension nts(Thread::Current(), __FUNCTION__);
  for (auto& pair : classes_) {
    ClassTable* class_table = pair.second;
...
class GetClassesInToVector : public ClassVisitor {
 public:
  virtual bool Visit(mirror::Class* klass) OVERRIDE {
...
class GetClassInToObjectArray : public ClassVisitor {
 public:
  explicit GetClassInToObjectArray(mirror::ObjectArray<mirror::Class>* arr)
      : arr_(arr), index_(0) {}
  virtual bool Visit(mirror::Class* klass) OVERRIDE SHARED_REQUIRES(Locks::mutator_lock_) {
    ++index_;
  ...
  if (!kMovingClasses) {
    ...
    while (true) {
      size_t class_table_size;
      {
        ...
        // Add 100 in case new classes get loaded when we are fillin in the object array.
