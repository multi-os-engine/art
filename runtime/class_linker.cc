...
static bool ContainsOverridingMethodOf(Thread* self,
  ...
  for (size_t k = ifstart + 1; k < iftable_count; k++) {
    ...
    for (size_t m = 0; m < num_instance_methods; m++) {
      ...
      // Skip methods added to deal with invoke-super
      if (UNLIKELY(current_method->GetDeclaringClass() != current_iface.Get())) {
        break;
      }
  ...
  *out_default_method = nullptr;
  ...
  for (size_t k = iftable_count; k != 0; ) {
    ...
    for (size_t m = 0; m < num_instance_methods; m++) {
      ...
      if (current_method->GetDeclaringClass() != iface.Get()) {
        // TODO Could we use this information?
        // This is a method from a super-interface. Ignore it.
        continue;
      }
...
bool ClassLinker::LinkInterfaceMethods(
    ...
    if (num_methods > 0) {
      ...
      for (size_t j = 0; j < num_methods; ++j) {
        ...
        for (int32_t k = input_array_length - 1; k >= 0; --k) {
          ...
          DCHECK(!is_interface || vtable_method->GetDeclaringClass() == klass.Get())
              << "Interface should only have its own methods in its virtuals prior to linking!";
  ...
  // TODO don't extend virtuals of interface
  if (has_new_virtuals) {
