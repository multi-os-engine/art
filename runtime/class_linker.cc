...
bool ClassLinker::OpenDexFilesFromOat(const char* dex_location,
    const char* oat_location, std::vector<const DexFile*>* dex_files) {
  // TODO: Should we check whether we have an open oat file first?
  // Wouldn't it be bad if the same oat file was loaded multiple times?
  ExecutableOatFileManager oat_file_manager(dex_location, oat_location);

  // Update the oat file if we can. This may fail, but that's okay.
  oat_file_manager.MakeUpToDate();

  // Load whatever dex files we can.
  std::unique_ptr<OatFile> oat_file;
  bool success = oat_file_manager.LoadDexFiles(dex_files, &oat_file);
