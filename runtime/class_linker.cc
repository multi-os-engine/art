...
bool ClassLinker::AddImageSpace(
  ...
  for (int32_t i = 0; i < dex_caches->GetLength(); i++) {
    ...
    // If non qualified, qualify it. TODO: Only store qualified paths.
  ...
  if (app_image) {
    ...
    for (mirror::ClassLoader* image_class_loader : image_class_loaders) {
      ...
      if (!FlattenPathClassLoader(image_class_loader, &image_dex_file_names)) {
        LOG(ERROR) << "Failed to flatten image dex path list hierarchy";
      }
      std::list<mirror::String*> loader_dex_file_names;
      if (!FlattenPathClassLoader(class_loader.Get(), &loader_dex_file_names)) {
        LOG(ERROR) << "Failed to flatten loader dex path list hierarchy";
      }
      ...
      // Ignore the number of image dex files since we are adding those to the main class
      // loader anyways.
    ...
    for (int32_t i = 0, num_dex_caches = dex_caches->GetLength(); i < num_dex_caches; i++) {
      ...
      for (int32_t j = 0; j < static_cast<int32_t>(num_types); j++) {
        ...
        if (klass != nullptr) {
          ...
          // this occurs.
          // TODO: If there are multiple dex caches, there may be the same class multiple times in
          // different dex caches. Check for this since inserting will add duplicates otherwise.
          if (num_dex_caches > 1) {
  ...
  VLOG(image) << "Adding image space took " << PrettyDuration(NanoTime() - start_time);
  return true;
}
