...
bool ClassLinker::SetupInterfaceLookupTable(Thread* self, Handle<mirror::Class> klass,
  ...
  // ordering since the original is a Depth first traversal.
  {
    ...
    for (size_t i = 0; i < num_interfaces; i++) {
      ...
      to_process.push_back(interface);
    }
    while (!to_process.empty()) {
      ...
      if (!duplicate && new_interfaces.end() == std::find(new_interfaces.begin(),
                                                          new_interfaces.end(),
                                                          interface)) {
        // TODO Not sure if below is right. Having a should make a stress test involving class
        // inheritance.
        // Add this non-duplicate interface.
        new_interfaces.push_front(interface);
        ...
        for (int32_t j = interface->GetIfTableCount() - 1; j >= 0; j--) {
          ...
          if (!super_duplicate) {
            ...
            auto existing_interface = std::find(new_interfaces.begin() + 1,
                                                new_interfaces.end(),
                                                super_interface);
            if (existing_interface == new_interfaces.end()) {
              new_interfaces.push_front(super_interface);
            } else {
              // We need to try to add this existing interface again since it should be before
              // 'interface' but is after it. We therefore remove it from the list and have it be
              // processed again. This might also cause other super-interfaces of 'super_interface'
              // to be reordered as well. We don't push it to new_interfaces so that we can continue
              // to use the duplicate check above.
              new_interfaces.erase(existing_interface, existing_interface + 1);
    ...
    for (auto new_iface = new_interfaces.begin();
         new_iface != new_interfaces.end();
         ++new_iface, ++idx) {
