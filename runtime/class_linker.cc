...
LengthPrefixedArray<ArtField>* ClassLinker::AllocArtFieldArray(Thread* self, size_t length) {
  constexpr size_t field_size = sizeof(ArtField);
  auto* ret = LengthPrefixedArray<ArtField>::Create(
      Runtime::Current()->GetLinearAlloc()->Alloc(
          self, LengthPrefixedArray<ArtField>::ComputeSize(length, field_size)), length);
  CHECK(ret != nullptr);
  std::uninitialized_fill_n(&ret->At(0, field_size), length, ArtField());
  return ret;
}
...
LengthPrefixedArray<ArtMethod>* ClassLinker::AllocArtMethodArray(Thread* self, size_t length) {
  ...
  auto* ret = LengthPrefixedArray<ArtMethod>::Create(
      Runtime::Current()->GetLinearAlloc()->Alloc(
          self, LengthPrefixedArray<ArtMethod>::ComputeSize(length, method_size)), length);
  CHECK(ret != nullptr);
                                   ...
                                   const OatFile::OatClass* oat_class) {
  {
    ...
    for (; it.HasNextStaticField(); it.Next()) {
      ...
      if (num_sfields == 0 || LIKELY(field_idx > last_field_idx)) {
        ...
        LoadField(it, klass, &sfields->At(num_sfields, sizeof(ArtField)));
        ++num_sfields;
    ...
    LengthPrefixedArray<ArtField>* ifields =
    ...
    for (; it.HasNextInstanceField(); it.Next()) {
      ...
      if (num_ifields == 0 || LIKELY(field_idx > last_field_idx)) {
        ...
        LoadField(it, klass, &ifields->At(num_ifields, sizeof(ArtField)));
        ++num_ifields;
        last_field_idx = field_idx;
      }
    }
    ...
    // Load methods.
    klass->SetDirectMethodsPtr(it.NumDirectMethods() != 0 ?
        AllocArtMethodArray(self, it.NumDirectMethods()) : nullptr);
    klass->SetVirtualMethodsPtr(it.NumVirtualMethods() != 0 ?
        AllocArtMethodArray(self, it.NumVirtualMethods()) : nullptr);
    size_t class_def_method_index = 0;
...
mirror::Class* ClassLinker::CreateProxyClass(ScopedObjectAccessAlreadyRunnable& soa, jstring name,
  ...
  ArtField& interfaces_sfield = sfields->At(0, sizeof(ArtField));
  interfaces_sfield.SetDexFieldIndex(0);
  interfaces_sfield.SetDeclaringClass(klass.Get());
  interfaces_sfield.SetAccessFlags(kAccStatic | kAccPublic | kAccFinal);

  // 2. Create a static field 'throws' that holds exceptions thrown by our methods.
  ArtField& throws_sfield = sfields->At(1, sizeof(ArtField));
...
bool ClassLinker::LinkInterfaceMethods(Thread* self, Handle<mirror::Class> klass,
  ...
  if (!miranda_methods.empty()) {
    ...
    auto* virtuals = LengthPrefixedArray<ArtMethod>::Create(runtime->GetLinearAlloc()->Realloc(
...
bool ClassLinker::LinkFields(Thread* self, Handle<mirror::Class> klass, bool is_static,
  ...
  for (size_t i = 0; i < num_fields; i++) {
    grouped_and_sorted_fields.push_back(&fields->At(i, sizeof(ArtField)));
  }
  ...
  if (!is_static && klass->DescriptorEquals("Ljava/lang/ref/Reference;")) {
    ...
    CHECK_STREQ(fields->At(num_fields - 1, sizeof(ArtField)).GetName(), "referent")
        << PrettyClass(klass.Get());
    --num_reference_fields;
  }
  ...
  if (kIsDebugBuild) {
    ...
    for (size_t i = 0; i < num_fields; i++) {
      ArtField* field = &fields->At(i, sizeof(ArtField));
      ...
      if (i != 0) {
        ArtField* const prev_field = &fields->At(i - 1, sizeof(ArtField));
