...
// A helper class to abstract different array-like types from each other.
template<typename T>
class ArrayLikeProxy {
 public:
  ArrayLikeProxy() : type_(kNone), pointer_array_(NullHandle<mirror::PointerArray>()), slice_() {}

  ArrayLikeProxy(Handle<mirror::PointerArray> array, size_t pointer_size)
      : type_(kPointerArray), pointer_array_(array), slice_(), pointer_size_(pointer_size) {}

  explicit ArrayLikeProxy(ArraySlice<T> slice)
      : type_(kSlice),
        pointer_array_(NullHandle<mirror::PointerArray>()),
        slice_(slice),
        pointer_size_(0) {}
  ...
  T* At(int32_t index)
    ...
    } else if (type_ == kPointerArray) {
      return pointer_array_->GetElementPtrSize<T*>(index, pointer_size_);
    } else {
      LOG(FATAL) << "Array used without assignment";
      return nullptr;
    }
  }

  int32_t Size() SHARED_REQUIRES(Locks::mutator_lock_) {
    if (type_ == kSlice) {
      return slice_.size();
    } else if (type_ == kPointerArray) {
      return pointer_array_->GetLength();
    } else {
      return 0;
    }
  }
 ...
 private:
  ...
  Type type_;
  Handle<mirror::PointerArray> pointer_array_;
  ArraySlice<T> slice_;
...
static void SanityCheckVTable(Handle<mirror::Class> klass, uint32_t pointer_size)
  ...
  auto* superclass = (klass->HasSuperClass()) ? klass->GetSuperClass() : nullptr;
  ...
  for (int32_t i = 0; i < check_vtable->GetLength(); ++i) {
    ...
    auto virtuals = klass->GetVirtualMethodsSliceUnchecked(pointer_size);
...
bool ClassLinker::LinkInterfaceMethods(
  ...
  for (size_t i = ifcount; i != 0; ) {
    ...
    if (num_methods > 0) {
      ...
      // whole vtable. For a new interface, however, we need the whole vtable in case a new
      ...
      ArrayLikeProxy<ArtMethod> input_methods = (super_interface || is_interface) ?
          ArrayLikeProxy<ArtMethod>(klass->GetDeclaredVirtualMethodsSlice(image_pointer_size_)) :
          ArrayLikeProxy<ArtMethod>(vtable, image_pointer_size_);
      int32_t input_array_length = input_methods.Size();
      ...
      for (size_t j = 0; j < num_methods; ++j) {
        ...
        for (int32_t k = input_array_length - 1; k >= 0; --k) {
          ArtMethod* vtable_method = input_methods.At(k);
  ...
  if (is_interface) {
    ...
    // since we already have them in the iftable.
