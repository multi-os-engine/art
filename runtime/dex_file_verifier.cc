...
bool DexFileVerifier::CheckSizeLimit(uint32_t size, uint32_t limit, const char* label) {
  if (size > limit) {
    ...
    LOG(FATAL) << "Deliberate crash for " << dex_file_->GetLocation() << failure_reason_;
    return false;
  }
  return true;
}
  ...
  if (previous_item_ != nullptr) {
    ...
    if (UNLIKELY(prev->return_type_idx_ > item->return_type_idx_)) {
      ...
      while (curr_it.HasNext() && prev_it.HasNext()) {
        ...
        CHECK_NE(prev_idx, DexFile::kDexNoIndex16) << dex_file_->GetLocation();
        CHECK_NE(curr_idx, DexFile::kDexNoIndex16) << dex_file_->GetLocation();
