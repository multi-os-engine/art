...
static std::string ComputeMonitorDescription(Thread* self,
  ...
  if ((o->GetLockWord(false).GetState() == LockWord::kThinLocked) &&
    ...
    return StringPrintf("<@addr=0x%" PRIxPTR "> (a %s)", reinterpret_cast<intptr_t>(o),
...
static void RemoveMonitors(Thread* self,
  ...
  std::vector<jobject> to_remove;
  for (std::pair<uintptr_t, jobject>& pair : *locked_objects) {
    if (frame == pair.first) {
      to_remove.push_back(pair.second);
    }
  }
  for (jobject jobj : to_remove) {
    locked_objects->erase(std::find(locked_objects->begin(),
                                    locked_objects->end(),
                                    std::make_pair(frame, jobj)));
  ...
  if (it != locked_objects_.end()) {
    ...
    for (std::pair<uintptr_t, jobject>& pair : locked_objects_) {
      if (self->DecodeJObject(pair.second) == mirror_obj) {
        ...
        vm->JniAbortF("<JNI MonitorExit>", "Unlocking monitor that wasn't locked here: %s",
                      monitor_descr.c_str());
        will_abort = true;
        break;
      }
...
void JNIEnvExt::CheckMonitorsReleasedImpl() {
  uintptr_t current_frame = GetJavaCallFrame(self);
  // Linear scan, quite expensive, but only done in check-jni. We can optimize by iterating
  // backwards as stacks always grow down on the architectures we support and the entries in
  // the vector should be ordered - but then this is CheckJNI...
  bool will_abort = false;
  for (std::pair<uintptr_t, jobject>& pair : locked_objects_) {
    if (pair.first == current_frame) {
      std::string monitor_descr = ComputeMonitorDescription(self, pair.second);
      vm->JniAbortF("<JNI End>", "Still holding a locked object on JNI end: %s",
                    monitor_descr.c_str());
      will_abort = true;
      break;
    }
