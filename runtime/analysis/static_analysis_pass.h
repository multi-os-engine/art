...
class StaticAnalysisPass : public Pass {
 public:
   ...
   */
  virtual uint32_t PerformAnalysis(StaticAnalysisMethodCumulativeStats* stats, mirror::ArtMethod* method, const DexFile* dex_file) const {
    UNUSED(stats);
    UNUSED(method);
    UNUSED(dex_file);
    return kMethodNone;
  ...
  bool Worker(const PassDataHolder* data) const {
    ...
    *(static_analysis_pass_data_holder->static_analysis_method_info) |= PerformAnalysis(GetStats(), static_analysis_pass_data_holder->method, static_analysis_pass_data_holder->dex_file);
    return true;
  }
