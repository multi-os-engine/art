...
class StaticAnalyzer {
  public:
    ...
    /**
     * Query function.
     * These functions provide the ability to perform queries on the profile info bitmap stored.
     */
    /**
     ...
     *  it will be represented as the corresponding bitmap which is in profiling_info.h.
     *  Otherwise, false.
     * @param method The ArtMethod* used as the key to search the method profile info table for the
     *  method's corresponding profile info bit map.
     * @param method_size_bitmap The bitmap corresponding to a particular size that we are searching for within
     *  the entry in the method profile info table.
     */
    bool IsMethodSizeIn(mirror::ArtMethod* method, uint32_t method_size_bitmap) const;
    // TODO Add more query functions

    typedef SafeMap<mirror::ArtMethod*, uint32_t*> MethodProfileInfoTable;
    // All method references with method info.
    mutable Mutex methods_profile_info_lock_ DEFAULT_MUTEX_ACQUIRED_AFTER;
    MethodProfileInfoTable methods_profile_info_ GUARDED_BY(methods_profile_info_lock_);

    /**
     * @brief Public call that analyzes the passed in ArtMethod* and analyzes it over various passes
     *  Each pass returns a bitmap containing profiled info. Once the complete bitmap is assembled representing
     *  all the profiled information collected, it is stored in a map "methods_profile_info_".
  ...
  private:
    ...
    /**
     * @brief Searches for and returns a bitmap with profiled info for a particular ArtMethod*.
     * @param method The ArtMethod* used as the key to search the method profile info table for the
     *  particular bit map.
     */
    uint32_t* GetMethodProfileInfo(mirror::ArtMethod* method) const
    LOCKS_EXCLUDED(methods_profile_info_lock_);
