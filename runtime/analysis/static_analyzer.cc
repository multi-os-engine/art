...
StaticAnalyzer::StaticAnalyzer()
  : methods_profile_info_lock_("methods profile info lock") {
  CreatePasses();
}
StaticAnalyzer::~StaticAnalyzer() {
  Thread* self = Thread::Current();
  {
    MutexLock mu(self, methods_profile_info_lock_);
    STLDeleteValues(&methods_profile_info_);
  }
}
bool StaticAnalyzer::IsMethodSizeIn(mirror::ArtMethod* method, uint32_t method_size_bitmap) const {
  if (method != nullptr) {
    uint32_t* method_profile_info = GetMethodProfileInfo(method);
    if (method_profile_info != nullptr) {
      return (*method_profile_info & kMethodSizeMask) == method_size_bitmap;
    }
  }
  return false;
}
...
void StaticAnalyzer::AnalyzeMethod(mirror::ArtMethod* method,
  ...
  if (GetMethodProfileInfo(method) != nullptr) {
    return;
  }
  uint32_t method_profile_info = 0x0;
  // Loop through different passes and bitwise OR the returned profiling info with with the method_profile_info variable.
  for (StaticAnalysisPass* cur_pass : pass_list_) {
    method_profile_info |= cur_pass->PerformAnalysis(method, dex_file);
  }
  // If after performing the different analysis, no important information was found, do not store anything.
  if (method_profile_info == kMethodNone) {
    return;
  }
  {
    MutexLock mu(Thread::Current(), methods_profile_info_lock_);
    methods_profile_info_.Put(method, new uint32_t(method_profile_info));
  }
}
...
uint32_t* StaticAnalyzer::GetMethodProfileInfo(mirror::ArtMethod* method) const {
  MutexLock mu(Thread::Current(), methods_profile_info_lock_);
  MethodProfileInfoTable::const_iterator it = methods_profile_info_.find(method);
  if (it == methods_profile_info_.end()) {
    return nullptr;
  }
