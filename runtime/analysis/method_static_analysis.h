...
#ifndef ART_RUNTIME_ANALYSIS_METHOD_STATIC_ANALYSIS_H_
#define ART_RUNTIME_ANALYSIS_METHOD_STATIC_ANALYSIS_H_
 ...
 *
 * Method Level:
 *  Returns a method profile info bitmap to indicate the following:
 *    Nothing.
 */
class MethodLogisticsAnalysis : public StaticAnalysisPass {
 public:
  MethodLogisticsAnalysis() :
      ...
      num_of_methods_(0),
      num_of_native_methods_(0),
      num_of_abstract_methods_(0),
      num_of_analyzable_methods_(0) {
  }
  /**
   ...
   */
  uint32_t PerformAnalysis(mirror::ArtMethod* method, const DexFile& dex_file) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    UNUSED(dex_file);
    if (method != nullptr) {
      ++num_of_methods_;
      if (method->IsNative()) {
        ++num_of_native_methods_;
      } else if (method->IsAbstract()) {
        ++num_of_abstract_methods_;
      } else {
        ++num_of_analyzable_methods_;
      }
    }
    return kMethodNone;
  }
   ...
   */
  void DumpPassAnalysis(std::stringstream& string_stream) {
    string_stream << num_of_methods_ <<" methods total. "
        << num_of_native_methods_ << " native methods found. "
        << num_of_abstract_methods_ << " abstract methods found. "
        << num_of_analyzable_methods_ << " analyzable methods found. ";
  }
...
class MethodMiscLogisticsAnalysis : public StaticAnalysisPass {
 public:
  MethodMiscLogisticsAnalysis() :
      ...
      num_of_methods_that_are_constructors_(0) {
  }
  /**
   * @fn PerformAnalysis(mirror::ArtMethod* method, const DexFile& dex_file)
  ...
  uint32_t PerformAnalysis(mirror::ArtMethod* method, const DexFile& dex_file) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    ...
    if (method != nullptr) {
      if (method->IsConstructor()) {
        ++num_of_methods_that_are_constructors_;
      }
    }
    return kMethodNone;
  }
   ...
   */
  void DumpPassAnalysis(std::stringstream& string_stream) {
    string_stream << num_of_methods_that_are_constructors_ << " constructor methods. ";
  }
 ...
 * Looks at various aspects of method sizes (for more information, refer to profiling_info.h).
 * APK Level:
 *  Keeps a cumulative count for the following:
 ...
 *
 * Method Level:
 *  Returns a method profile info bitmap to indicate ONE of the following:
 *    Is Sub Tiny Method?
 *    Is Tiny Method?
 *    Is Sub Small Method?
 *    Is Small Method?
...
class MethodSizeAnalysis : public StaticAnalysisPass {
 public:
    MethodSizeAnalysis() :
      ...
      size_of_all_methods_(0),
      num_of_sub_tiny_methods_(0),
      num_of_tiny_methods_(0),
      num_of_sub_small_methods_(0),
      num_of_small_methods_(0),
      num_of_medium_methods_(0),
      num_of_large_methods_(0),
      num_of_too_large_methods_(0) {
    }
    /**
     * @fn PerformAnalysis(mirror::ArtMethod* method, const DexFile& dex_file)
    ...
    uint32_t PerformAnalysis(mirror::ArtMethod* method, const DexFile& dex_file) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
      ...
      uint32_t method_profile_info = kMethodNone;
      if (code_item != nullptr) {
        const uint32_t insns_size = code_item->insns_size_in_code_units_;
        size_of_all_methods_.FetchAndAdd(insns_size);  // Atomic equivalent to sizeOfAllMethods += insns_size;
        // Categorize the methods
        if (insns_size <= kSubTinyMethodLimit) {
          method_profile_info |= kMethodSizeSubTiny;
          ++num_of_sub_tiny_methods_;
        } else if (insns_size <= kTinyMethodLimit) {
          method_profile_info |= kMethodSizeTiny;
          ++num_of_tiny_methods_;
        } else if (insns_size <= kSubSmallMethodLimit) {
          method_profile_info |= kMethodSizeSubSmall;
          ++num_of_sub_small_methods_;
        } else if (insns_size <= kSmallMethodLimit) {
          method_profile_info |= kMethodSizeSmall;
          ++num_of_small_methods_;
        } else if (insns_size <= kMediumMethodLimit) {
          method_profile_info |= kMethodSizeMedium;
          ++num_of_medium_methods_;
        } else if (insns_size < kLargeMethodLimit) {
          method_profile_info |= kMethodSizeLarge;
          ++num_of_large_methods_;
        } else {
          method_profile_info |= kMethodSizeTooLarge;
          ++num_of_too_large_methods_;
        }
      }
      return method_profile_info;
    }
    /**
     * @fn DumpPassAnalysis(std::stringstream& string_stream)
     ...
     */
    void DumpPassAnalysis(std::stringstream& string_stream) {
      string_stream << size_of_all_methods_ << " size of Methods in Dex File. "
          << num_of_sub_tiny_methods_ << " number of sub tiny methods. "
          << num_of_tiny_methods_ << " number of tiny methods. "
          << num_of_sub_small_methods_ << " number of sub small methods. "
          << num_of_small_methods_ << " number of small methods. "
          << num_of_medium_methods_ << " number of medium methods. "
          << num_of_large_methods_ << " number of large methods. "
          << num_of_too_large_methods_ << " number of too large methods. ";
    }
 ...
 *
 * Method Level:
 *  Returns a method profile info bitmap with the following (for more information, refer to profiling_info.h):
 *    % of Getters.
 *    % of Setters.
 *    % of Invokes.
...
class MethodOpcodeAnalysis : public StaticAnalysisPass {
 public:
  MethodOpcodeAnalysis() :
      ...
      num_of_constant_assigns_(0),
      num__of_method_invokes_(0),
      num_of_unconditional_jumps_(0),
      num_of_conditional_jumps_(0),
      num_of_fp_instructions_(0),
      num_of_methods_with_try_catch_(0),
      num_of_throw_instructions_(0),
      num_of_math_instructions_(0),
      num_of_data_movement_setters_instructions_(0),
      num_of_data_movement_getters_instructions_(0) {
  }
  /**
   * @fn PerformAnalysis(mirror::ArtMethod* method, const DexFile& dex_file)
   * @brief Performs the method size logistics analysis of the method to accumulate the number of
    ...
    if (method != nullptr) {
      ...
      if (code_item != nullptr) {
        ...
        if (code_item->tries_size_ > 0) {
          method_profile_info |= kMethodContainsTryCatch;
          ++num_of_methods_with_try_catch_;
        }
         ...
         */
        method_profile_info |= GetProfileInfoBitValue(num_of_data_movement_getters, num_of_instructions,
        ...
        /**
         * Profile Info: Setters.
         * Determine the ratio of Setters within the method.
         */
        method_profile_info |= GetProfileInfoBitValue(num_of_data_movement_setters, num_of_instructions,
        ...
        /**
         * Profile Info: Arithmetic Instructions.
         * Determine the ratio of Arithmetic Instructions within the method.
         */
        method_profile_info |= GetProfileInfoBitValue(num_of_math, num_of_instructions,
  ...
  void DumpPassAnalysis(std::stringstream& string_stream) {
    string_stream
        << num_of_constant_assigns_ << " number of constant assigns. "
        << num_of_unconditional_jumps_ << " number of unconditional jumps. "
        << num_of_conditional_jumps_ << " number of conditional jumps. "
        << num__of_method_invokes_ << " number of method invokes. "
        << num_of_fp_instructions_ << " number of Floating Point Instructions. "
        << num_of_methods_with_try_catch_ << " number of methods with try catch. "
        << num_of_throw_instructions_ << " number of throw instructions. "
        << num_of_math_instructions_ << " number of math instructions. "
        << num_of_data_movement_getters_instructions_ << " number of getters. "
        << num_of_data_movement_setters_instructions_ << " number of setters. ";
  }
  ...
  Atomic<uint32_t> num__of_method_invokes_;
 ...
 private:
   ...
   */
  bool IsConstantOperation(const Instruction* insn) {
    switch (insn->Opcode()) {
   ...
   * @param insn The instruction that is being checked for its type.
   */
  bool IsInvokeOperation(const Instruction* insn) {
   ...
   * @param insn The instruction that is being checked for its type.
   */
  bool IsUnconditionalJumpOperation(const Instruction* insn) {
   ...
   * @param insn The instruction that is being checked for its type.
   */
  bool IsConditionalJumpOperation(const Instruction* insn) {
   ...
   * @param insn The instruction that is being checked for its type.
   */
  bool IsFloatingPointOperation(const Instruction* insn) {
    switch (insn->Opcode()) {
   ...
   * @param insn The instruction that is being checked for its type.
   */
  bool IsThrowOperation(const Instruction* insn) {
    return insn->IsThrow();
  }
  /**
   ...
   * @param insn The instruction that is being checked for its type.
   */
  bool IsMathOperation(const Instruction* insn) {
    switch (insn->Opcode()) {
   ...
   * @param insn The instruction that is being checked for its type.
   */
  bool IsSetterOperation(const Instruction* insn) {
    switch (insn->Opcode()) {
   ...
   * @param insn The instruction that is being checked for its type.
   * */
  bool IsGetterOperation(const Instruction* insn) {
    switch (insn->Opcode()) {
...
#endif  // ART_RUNTIME_ANALYSIS_METHOD_STATIC_ANALYSIS_H_
