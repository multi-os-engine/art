...
class InternTable {
 ...
 private:
  ...
  mirror::String* Insert(mirror::String* s, bool is_strong)
      LOCKS_EXCLUDED(intern_table_lock_)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);
  ...
  mirror::String* InsertStrong(mirror::String* s, uint32_t hash_code)
      EXCLUSIVE_LOCKS_REQUIRED(intern_table_lock_);
  ...
  mirror::String* Insert(Table& table, mirror::String* s, uint32_t hash_code)
      EXCLUSIVE_LOCKS_REQUIRED(intern_table_lock_);
  void RemoveWeak(const mirror::String* s, uint32_t hash_code)
      EXCLUSIVE_LOCKS_REQUIRED(intern_table_lock_);
  ...
  mirror::String* InsertWeakFromTransaction(mirror::String* s, uint32_t hash_code)
      LOCKS_EXCLUDED(intern_table_lock_);
  void RemoveStrongFromTransaction(const mirror::String* s, uint32_t hash_code)
      LOCKS_EXCLUDED(intern_table_lock_);
  void RemoveWeakFromTransaction(const mirror::String* s, uint32_t hash_code)
      LOCKS_EXCLUDED(intern_table_lock_);
