...
typedef std::vector<const DexFile*> DexFilePtrs;
typedef std::unique_ptr<const DexFile> DexFileUniquePtr;
typedef std::vector<DexFileUniquePtr> DexFileUniquePtrs;
...
class DexFile {
 public:
  ...
  static DexFileUniquePtr Open(const uint8_t* base, size_t size,
 ...
 private:
  // Opens a .dex file
  static DexFileUniquePtr OpenFile(int fd, const char* location, bool verify, std::string* error_msg);
  ...
  // return.
  static DexFileUniquePtr Open(const ZipArchive& zip_archive, const char* entry_name,
                               const std::string& location, std::string* error_msg,
                               ZipOpenErrorCode* error_code);

  // Opens a .dex file at the given address backed by a MemMap
  static DexFileUniquePtr OpenMemory(const std::string& location,
                                     uint32_t location_checksum,
                                     MemMap* mem_map,
                                     std::string* error_msg);

  // Opens a .dex file at the given address, optionally backed by a MemMap
  static DexFileUniquePtr OpenMemory(const uint8_t* dex_file,
                                     size_t size,
                                     const std::string& location,
                                     uint32_t location_checksum,
                                     MemMap* mem_map,
