...
struct ShortyFieldType : ValueObject {
  ...
  inline ShortyFieldType(decltype(kByte) c) : value_(c) {
  }
  ...
  // that would either point to the next shorty type, or to nullptr if there are
  // no more types.
  //
  // DCHECKs that each of the nested types is a valid shorty field type.
  static const char* ParseFromFieldTypeDescriptor(const char* field_type_descriptor,
    ...
    switch (shorter_type) {
      case kArray:
        ...
        break;
      case kLambda:
        while (*field_type_descriptor != '\0' && *field_type_descriptor != ';') {
          ++field_type_descriptor;
        }
        safe_type = kLambda;
        DCHECK(type_set);
...
#define IS_VALID_TYPE_SPECIALIZATION(type, name) \
  ...
  IS_VALID_TYPE_SPECIALIZATION(uint8_t, PrimitiveNarrow);
  IS_VALID_TYPE_SPECIALIZATION(uint16_t, PrimitiveNarrow);
