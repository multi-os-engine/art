...
/*
 * GC support TODOs:
 * (Although there's some code for storing objects, it is DCHECKED false because it is incomplete).
 *
...
void ClosureBuilder::CaptureVariablePrimitive(T value) {
  ...
  ShortyFieldTypeTraits::MaxType value_storage;
...
void ClosureBuilder::CaptureVariableObject(mirror::Object* object) {
  ...
  if (kIsDebugBuild) {
    if (kUseReadBarrier) {
      DCHECK(false) << "can't yet safely capture objects with read barrier";
      UNREACHABLE();
    }
  }
}
...
Closure* ClosureBuilder::CreateInPlace(void* memory, ArtLambdaMethod* target_method) const {
  ...
  CHECK_EQ(target_method->GetNumberCapturedVariables(), values_.size())
  ...
  if (UNLIKELY(is_dynamic_size_)) {
    ...
    written_size =
        WriteValues(target_method,
                    ...
                    header_size,
                    GetSize() - header_size /* variables size */);
  } else {
...
size_t ClosureBuilder::WriteValues(ArtLambdaMethod* target_method,
  ...
  size_t i = 0;
  uint8_t* variables_offset = variables;
  size_t remaining_size = variables_size;
  while (shorty_types != nullptr && *shorty_types != '\0') {
    ShortyFieldType shorty{*shorty_types};  // NOLINT [readability/braces] [4]

    size_t var_size = shorty.GetStaticSize();
    if (LIKELY(shorty.IsStaticSize())) {
      // TODO: needs more work to support concurrent GC, e.g. read barriers
      if (kUseReadBarrier == false) {
        DCHECK(shorty.IsObject() == false) << "can't yet safely write objects with read barrier";
      } else {
        DCHECK(shorty.IsObject() == false) << "writing objects not yet supported, no GC support";
      }
      ...
      memcpy(variables_offset, &values_[i], var_size);
    } else {
      ...
      nested_closure->CopyTo(variables_offset, remaining_size);
  ...
  DCHECK_EQ(i, target_method->GetNumberCapturedVariables());
  return total_size;
}
