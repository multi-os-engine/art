...
// Inline representation of a closure.
// The closure itself is logically immutable, although in practices any object references
// it (recursively) contains can be moved and updated.
struct PACKED(sizeof(ArtLambdaMethod*)) Closure {
  ...
  size_t GetNumberCapturedVariables() const;

  // Returns a type descriptor string that represents each captured variable.
  // e.g. "LObject;ZB" would mean a capture tuple of (Object, boolean, byte)
 ...
 private:
  ...
  struct VariableInfo {
    ...
    static constexpr size_t kUpToIndexDontCare = 0;  // Use when kIndex is not set.
  };
  ...
  // Cast the variable at 'captured_variables_[offset]' into T, returning its address.
