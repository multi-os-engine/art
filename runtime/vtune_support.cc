...
static bool IsCoreOat(OatFile &oat_file) {
  return EndsWith(oat_file.GetLocation(), "@boot.oat") ||
         EndsWith(oat_file.GetLocation(), "/boot.oat");
}
...
void SendOatFileToVTune(OatFile &oat_file) {
  ...
  if (!process_listed && !core_expected) {
    return;
  }

  bool rejected = (!process_listed && core_expected && !IsCoreOat(oat_file)) ||
                                    (!core_expected && IsCoreOat(oat_file));

  LOG(DEBUG) << "VTUNE: package=" << vtune_package
             << "; process_listed=" << process_listed
             << "; core_expected=" << core_expected
             << "; IsCoreOat=" << IsCoreOat(oat_file)
             << "; oat_file=" << oat_file.GetLocation()
             << (rejected ? "; rejected" : "");
