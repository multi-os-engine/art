...
bool Monitor::Deflate(Thread* self, mirror::Object* obj) {
  ...
  if (lw.GetState() == LockWord::kFatLocked) {
    ...
    if (owner != nullptr) {
      ...
      LockWord new_lw = LockWord::FromHashCode(monitor->GetHashCode(), lw.GCBits());
      // Assume no concurrent read barrier state changes as mutators are suspended.
      obj->SetLockWord(new_lw, false);
      ...
      LockWord new_lw = LockWord::FromDefault(lw.GCBits());
      // Assume no concurrent read barrier state changes as mutators are suspended.
      obj->SetLockWord(new_lw, false);
...
mirror::Object* Monitor::MonitorEnter(Thread* self, mirror::Object* obj, bool trylock) {
  ...
  while (true) {
    ...
    switch (lock_word.GetState()) {
      case LockWord::kUnlocked: {
        LockWord thin_locked(LockWord::FromThinLockId(thread_id, 0, lock_word.GCBits()));
      ...
      case LockWord::kThinLocked: {
        ...
        if (owner_thread_id != thread_id) {
          ...
          if (lock_word.ThinLockCount() != 0) {
            uint32_t new_count = lock_word.ThinLockCount() - 1;
            new_lw = LockWord::FromThinLockId(thread_id,
                                              new_count,
                                              lock_word.GCBits());
          } else {
            new_lw = LockWord::FromDefault(lock_word.GCBits());
          }
          if (!kUseReadBarrier) {
