...
class OatFileAssistant {
 ...
 private:
  ...
  // Indicates whether or not we should try to make the dex file up to date
  // (i.e. compiling or relocating). This is true only if we are given a
  // not-null oat location in the constructor, which is the signal that the
  // caller wants the dexfile to be optimized.
  ...
  // for a compiled file (.oat or .odex) will be searched. The paths are:
