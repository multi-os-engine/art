...
inline size_t DexCacheArraysLayout::TypesSize(size_t num_elements) const {
  // App image patching relies on having enough room for a pointer in the types array. See
  // FixupArtMethodArrayVisitor
  return RoundUp(ArraySize(sizeof(GcRoot<mirror::Class>), num_elements), pointer_size_);
}
...
inline size_t DexCacheArraysLayout::MethodsSize(size_t num_elements) const {
  // App image patching relies on having enough room for a pointer in the methods array.
  return RoundUp(ArraySize(pointer_size_, num_elements), pointer_size_);
}
