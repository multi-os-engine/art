...
template <typename T>
static PointerSize GcRootAsPointerSize() {
  if (sizeof(GcRoot<T>) == 4) {
    return PointerSize::kPointerSize32;
  } else if (sizeof(GcRoot<T>) == 8) {
    return PointerSize::kPointerSize64;
  } else {
    LOG(FATAL) << "Unsupported GcRoot size";
    UNREACHABLE();
  }
}
