...
bool OatFile::Setup(std::string* error_msg) {
  ...
  for (size_t i = 0; i < dex_file_count; i++) {
    ...
    std::string base_location = DexFile::GetBaseLocation(dex_file_location.c_str());
    if (last_base_location != base_location) {
      ...
      if (path != nullptr && path.get() != base_location) {
        ...
        last_path.clear();  // Empty means that location and canonical location are the same.
      }
    }
  ...
  if (primary_it != oat_dex_files_.end()) {
    ...
    if (secondary_lb != secondary_oat_dex_files_.end() && key == secondary_lb->first) {
      ...
      if (dex_canonical_location != dex_location) {
        DCHECK_NE(dex_canonical_location, dex_location);
