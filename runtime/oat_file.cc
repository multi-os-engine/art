...
template <typename T>
ALWAYS_INLINE bool OatFile::ReadDexFileData(/*inout*/const uint8_t** oat, /*out*/T* value) {
  DCHECK_LE(*oat, End());
  if (UNLIKELY(static_cast<size_t>(End() - *oat) < sizeof(T))) {
    return false;
  }
...
bool OatFile::Setup(const char* abs_dex_location, std::string* error_msg) {
  ...
  for (size_t i = 0; i < dex_file_count; i++) {
    ...
    if (UNLIKELY(!ReadDexFileData(&oat, &dex_file_location_size))) {
    ...
    if (UNLIKELY(!ReadDexFileData(&oat, &dex_file_checksum))) {
    ...
    if (UNLIKELY(!ReadDexFileData(&oat, &dex_file_offset))) {
