...
OatFile* OatFile::Open(const std::string& filename,
  ...
  if (ret.get() != nullptr) {
    ...
    if (implicit_checks_value == nullptr) {
      // TODO: Always regenerate when missing?
      return ret.release();
    } else {
      ...
      if (ImplicitCheckOptions::Parse(implicit_checks_value, &explicit_null_checks,
        ...
        if (runtime != nullptr &&  // Unstarted runtime, e.g., oatdump. Trust it's not going to be
                                   // executed.
            (runtime->ExplicitNullChecks() != explicit_null_checks ||
