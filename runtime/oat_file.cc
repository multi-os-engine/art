...
OatFile::OatFile(const std::string& location)
    : location_(location), begin_(NULL), end_(NULL), dlopen_handle_(NULL),
      secondary_oat_dex_files_lock_("OatFile secondary lookup lock", kOatFileSecondaryLookupLock) {
  CHECK(!location_.empty());
}

OatFile::~OatFile() {
  STLDeleteValues(&oat_dex_files_by_canonical_location_);
...
bool OatFile::Setup(std::string* error_msg) {
  ...
  for (size_t i = 0; i < GetOatHeader().GetDexFileCount(); i++) {
    ...
    // Create the OatDexFile and add it to the owning map indexed by the canonical location.
    OatDexFile* oat_dex_file = new OatDexFile(this,
                                              dex_file_location,
                                              dex_file_checksum,
                                              dex_file_pointer,
                                              methods_offsets_pointer);
    StringPiece canonical_location_key(oat_dex_file->GetDexCanonicalLocation());
    oat_dex_files_by_canonical_location_.Put(canonical_location_key, oat_dex_file);

    // Now add the OatDexFile to the non-owning map indexed by the dex_file_location.
...
const OatFile::OatDexFile* OatFile::GetOatDexFile(const char* dex_location,
  ...
  if (primary_it != oat_dex_files_.end()) {
    ...
    MutexLock mu(Thread::Current(), secondary_oat_dex_files_lock_);
...
OatFile::OatDexFile::OatDexFile(const OatFile* oat_file,
    ...
    : oat_file_(oat_file),
      ...
      dex_canonical_location_(DexFile::GetDexCanonicalLocation(dex_file_location.c_str())),
