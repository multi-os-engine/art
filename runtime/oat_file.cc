...
//
// These are used in OpenInternal to try all our loaders.
//
...
OatFileBase* OatFileBase::OpenOatFile(const std::string& elf_filename,
                                      ...
                                      const char* abs_dex_location,
                                      std::string* error_msg) {
  std::unique_ptr<OatFileBase> ret(new T(location, executable));
...
class DlOpenOatFile FINAL : public OatFileBase {
 public:
  DlOpenOatFile(const std::string& filename, bool executable)
      : OatFileBase(filename, executable), dlopen_handle_(nullptr) {
  }
...
bool DlOpenOatFile::Load(const std::string& elf_filename,
  ...
  if (!kIsTargetBuild) {
    ...
    if (manager == nullptr) {  // Manager may be null if we are running without a runtime.
      *error_msg = "OatFileManager is null.";
      return false;
    }
  ...
  CHECK(dlopen_handle_ != nullptr || !success);
  if (reserved_location && !success) {
...
#else
  {
    ...
    extinfo.flags = ANDROID_DLEXT_FORCE_LOAD |            // Force-load, don't reuse handle
                    ...
                    ANDROID_DLEXT_FORCE_FIXED_VADDR |     // Take a non-zero vaddr as absolute
                                                          //   (non-pic boot image).
                    ANDROID_DLEXT_LOAD_AT_FIXED_ADDRESS;  // Use the requested addr if vaddr = 0
                                                          //   (pic boot image).
...
/*
enum class OpenType {  // private
  kInternal,
  kDlOpen
};

template <OpenType open>
static std::unique_ptr<OatFile> OpenInternal(const std::string& filename,
                                             const std::string& location,
                                             uint8_t* requested_base,
                                             uint8_t* oat_file_begin,
                                             bool executable,
  ...
  if (open == OpenType::kDlOpen) {
    if (!kUseDlopen) {
      *error_msg = "Dlopen disabled.";
    ...
    if (executable == false) {
      ...
      //       !executable is a sign that we may want to patch), which may not be allowed for
      //       various reasons.
      *error_msg = "Cannot open non-executable oat file with dlopen.";
      return std::unique_ptr<OatFile>();
    }
    // dlopen always returns the same library if it is already opened on the host. For this reason
    // we only use dlopen if we are the target or we do not already have the dex file opened. Having
    ...
    if (success) {
      ...
      std::unique_ptr<OatFile> ret(OpenDlopen(filename,
                                              location,
                                              requested_base,
                                              oat_file_begin,
                                              abs_dex_location,
      ...
      if (ret == nullptr && kPrintDlOpenErrorMessage) {
        LOG(ERROR) << "Failed to dlopen: " << *error_msg;
      }
      return ret;
    }
    ...
    std::unique_ptr<OatFile> ret(OpenElfFile(file.get(),
                                             location,
                                             requested_base,
                                             oat_file_begin,
                                             false,
