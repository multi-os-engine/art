...
bool OatFile::Setup(std::string* error_msg) {
  ...
  for (size_t i = 0; i < dex_file_count; i++) {
    ...
    // Calculate canonical location. Avoid unnecessary realpath() calls for multi-dex oat files.
    const char* base_location = DexFile::GetBaseLocation(dex_file_location.c_str(), &temp);
    if (last_base_location != base_location) {
      last_base_location = base_location;
      // TODO: Remove LOG before submitting.
      LOG(INFO) << "VMarko: OatFile::Setup() calling realpath() for " << base_location;
      UniqueCPtr<const char[]> path(realpath(base_location, nullptr));
      if (path != nullptr && strcmp(path.get(), base_location) != 0) {
        last_path = path.get();
      } else {
        last_path.clear();  // Empty means that plain and canonical locations are the same.
      }
    }
    std::string canonical_location;
    if (!last_path.empty()) {
      canonical_location = last_path + dex_file_location.substr(last_base_location.size());
      DCHECK(canonical_location.size() == last_path.size() ||
             canonical_location[last_path.size()] == DexFile::kMultiDexSeparator);
    }
    ...
    // Add the plain and canonical (if different) location to the oat_dex_files_ table.
    StringPiece key(oat_dex_file->GetDexFileLocation());
    oat_dex_files_.Put(key, oat_dex_file);
    if (!last_path.empty()) {
      DCHECK_NE(oat_dex_file->GetCanonicalDexFileLocation(), oat_dex_file->GetDexFileLocation());
...
const OatFile::OatDexFile* OatFile::GetOatDexFile(DexFile::LocationHelper* dex_location,
  ...
  StringPiece key(dex_location->GetDexLocation());
  ...
  if (primary_it != oat_dex_files_.end()) {
    ...
    if (secondary_lb != secondary_oat_dex_files_.end() && key == secondary_lb->first) {
      ...
      const char* dex_canonical_location = dex_location->GetCanonicalDexLocation();
      if (dex_canonical_location != dex_location->GetDexLocation()) {
        DCHECK_STRNE(dex_canonical_location, dex_location->GetDexLocation());
  ...
  if (warn_if_not_found) {
    ...
    LOG(WARNING) << "Failed to find OatDexFile for DexFile " << dex_location->GetDexLocation()
                 << " ( canonical path " << dex_location->GetCanonicalDexLocation() << ")"
    ...
    if (kIsDebugBuild) {
      for (Table::const_iterator it = oat_dex_files_.begin(); it != oat_dex_files_.end(); ++it) {
        LOG(WARNING) << "OatFile " << GetLocation()
                     ...
                     << " (canonical path " << it->second->GetCanonicalDexFileLocation() << ")"
