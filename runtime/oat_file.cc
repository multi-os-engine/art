...
OatFile::OatFile(const std::string& location)
    : location_(location), begin_(NULL), end_(NULL), dlopen_handle_(NULL),
      has_oat_files_with_different_canonical_location_(false),
...
const OatFile::OatDexFile* OatFile::GetOatDexFile(const char* dex_location,
  ...
  if (it != oat_dex_files_.end()) {
    ...
    // We can stop if dex_canonical_location == dex_location and we do not have dex files that
    // have a canonical location that's different from their embedded one.
    if (has_oat_files_with_different_canonical_location_ ||
        dex_canonical_location != dex_location) {
