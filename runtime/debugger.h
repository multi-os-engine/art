...
class Dbg {
 public:
  ...
  // the interpreter. We pass the catch handler frame so we can ignore frames that
  // will be unwound by the exception.
  ...
  static bool IsForcedInterpreterNeededForException(Thread* thread,
    ...
    return IsForcedInterpreterNeededForExceptionImpl(thread, catch_handler_frame);
  }
