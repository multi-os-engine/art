...
struct DebugInvokeReq {
  ...
  ~DebugInvokeReq() {
    if (arg_values != nullptr) {
      // Delete the argument array allocated with new uint64_t[] in function RequestInvoke.
      delete[] arg_values;
    }
  ...
  // Reply
  JDWP::ExpandBuf* const reply;       // the reply containing the result of the invoke. We take ownership
                                // of the reply so we must delete it upon destruction.
...
class Dbg {
 public:
  ...
  // invocation completes. This is required to allow the JDWP thread to process incoming commands
  // from the debugger while the invocation is still in progress in the event thread, especially
  // if this one is suspended by a debug event occurring in another thread.
