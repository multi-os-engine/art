...
DexCacheProfileData::DexCacheProfileData(const DexFile& dex_file)
    : DexCacheProfileData(dex_file.GetHeader().checksum_, dex_file.NumClassDefs()) {}

DexCacheProfileData::DexCacheProfileData(uint32_t checksum, size_t num_class_defs)
    : checksum_(checksum),
...
std::unordered_set<std::string> DexCacheProfileData::GetClassDescriptors(
  ...
  DCHECK_EQ(dex_file.GetHeader().checksum_, DexFileChecksum());
...
void ClassProfile::Collect() {
  ...
  for (const ClassLinker::DexCacheData& data : class_linker->GetDexCachesData()) {
    ...
    if (found == dex_caches_.end()) {
      dex_cache_data = new DexCacheProfileData(*dex_file);
