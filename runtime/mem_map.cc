...
MemMap* MemMap::MapAnonymous(const char* name, byte* expected, size_t byte_count, int prot,
  ...
  if (low_4gb && ((size_t)expected + page_aligned_byte_count < (size_t)expected ||
                  (size_t)expected + page_aligned_byte_count > 0xffffffff)) {
    *error_msg = StringPrintf("The requested address space (%zd, %zd) cannot fit in low_4gb",
                               (size_t)expected, (size_t)expected + page_aligned_byte_count);
    return nullptr;
  }
