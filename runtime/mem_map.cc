...
static bool CheckOverlapping(uintptr_t expected,
                             uintptr_t limit,
                             std::string* error_msg) {
  std::unique_ptr<BacktraceMap> map(new BacktraceMap());
  if (!map->Build()) {
  ...
  *error_msg = StringPrintf("Requested region 0x%08" PRIxPTR "-0x%08" PRIxPTR " does not overlap "
                            "any existing map:\n%s\n",
                            expected, limit, maps.c_str());
  return false;
}

static bool CheckNonOverlapping(uintptr_t expected,
                                uintptr_t limit,
                                std::string* error_msg) {
  std::unique_ptr<BacktraceMap> map(new BacktraceMap());
  if (!map->Build()) {
...
static bool CheckMapRequest(byte* expected_ptr, void* actual_ptr, size_t byte_count,
                            bool reuse, std::string* error_msg) {
...
MemMap* MemMap::MapAnonymous(const char* name, byte* expected_ptr, size_t byte_count, int prot,
  ...
  if (!CheckMapRequest(expected_ptr, actual, page_aligned_byte_count, false, error_msg)) {
    return nullptr;
  }
...
MemMap* MemMap::MapFileAtAddress(byte* expected_ptr, size_t byte_count, int prot, int flags, int fd,
  ...
  std::ostringstream check_map_request_error_msg;
  if (!CheckMapRequest(expected_ptr, actual, page_aligned_byte_count, reuse, error_msg)) {
    return nullptr;
  }
