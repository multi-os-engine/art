...
MemMap* MemMap::MapFileAtAddress(uint8_t* expected_ptr, size_t byte_count, int prot, int flags,
  ...
  if (redzone_size != 0) {
    // __asan_poison_memory_region(actual, kPageSize);
    ...
    MAKE_MEM_NOACCESS(actual, real_start - actual);
    MAKE_MEM_NOACCESS(real_end, mapping_end - real_end);
    page_aligned_byte_count -= redzone_size;
  }
...
MemMap::~MemMap() {
  ...
  if (redzone_size_ != 0)
    MAKE_MEM_UNDEFINED((char*)base_begin_ + base_size_ - redzone_size_,
                       redzone_size_);
#endif

  if (!reuse_) {
#ifdef ADDRESS_SANITIZER
    MAKE_MEM_UNDEFINED(base_begin_, base_size_);
#endif
...
#ifdef ADDRESS_SANITIZER
  MAKE_MEM_UNDEFINED(tail_base_begin, tail_base_size);
#endif
...
#ifdef ADDRESS_SANITIZER
  MAKE_MEM_UNDEFINED(reinterpret_cast<void*>(
                         reinterpret_cast<uintptr_t>(BaseBegin()) + new_size),
                     base_size_ - new_size);
#endif
