...
MemMap::next_mem_pos = kPageSize * 2;   // first page to check for low-mem extent
#endif
...
MemMap* MemMap::MapAnonymous(const char* name, byte* expected, size_t byte_count, int prot,
  ...
  if (low_4gb && expected == nullptr) {
    ...
    for (uintptr_t ptr = next_mem_pos; ptr < 4 * GB; ptr += kPageSize) {
      ...
      next_mem_pos = tail_ptr;  // update early, as we break out when we found and mapped a region
