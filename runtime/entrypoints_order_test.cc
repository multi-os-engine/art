...
class EntrypointsOrderTest : public CommonRuntimeTest {
 protected:
  void CheckThreadOffsets() {
    ...
    EXPECT_OFFSET_DIFFP(Thread, tlsPtr_, deoptimization_return_value_stack,
                        deoptimization_exception_to_restore, sizeof(void*));
    EXPECT_OFFSET_DIFFP(Thread, tlsPtr_, deoptimization_exception_to_restore, name, sizeof(void*));
