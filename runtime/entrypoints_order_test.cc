...
class EntrypointsOrderTest : public CommonRuntimeTest {
 protected:
  void CheckThreadOffsets() {
    ...
    EXPECT_OFFSET_DIFFP(Thread, tlsPtr_, thread_local_mark_stack, return_pc_mapping_stack, sizeof(void*));
    EXPECT_OFFSET_DIFF(Thread, tlsPtr_.return_pc_mapping_stack, Thread, wait_mutex_, sizeof(void*),
                       thread_tlsptr_end);
  }
