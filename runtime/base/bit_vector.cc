...
void BitVector::Dump(std::ostream& os, const char *prefix, bool print_indices) {
  std::string buffer;
  DumpHelper(buffer, prefix, print_indices);
  os << buffer << std::endl;
}

void BitVector::DumpDot(FILE* file, const char* prefix, bool print_indices, bool last_entry) {
  std::string buffer;
  DumpHelper(buffer, prefix, print_indices);
...
void BitVector::DumpHelper(std::string& buffer, const char* prefix, bool print_indices) {
  // Initialize it.
  if (prefix != nullptr) {
    buffer = prefix;
  } else {
