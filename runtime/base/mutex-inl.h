...
inline void ReaderWriterMutex::SharedUnlock(Thread* self) {
  ...
  do {
    ...
    if (LIKELY(cur_state > 0)) {
      // Reduce state by 1.
      done = state_.CompareExchangeWeakRelease(cur_state, cur_state - 1);
      if (done && (cur_state - 1) == 0) {  // Weak CAS may fail spuriously.
        if (num_pending_writers_.LoadSequentiallyConsistent() > 0 ||
            num_pending_readers_.LoadSequentiallyConsistent() > 0) {
