  ...
  if (!recursive_ || recursion_count_ == 0) {
    ...
    do {
      ...
      if (LIKELY(cur_state == 1)) {
        ...
        exclusive_owner_ = 0;
        // Change state to 0 with release memory ordering.
        done =  state_.CompareExchangeWeakRelease(cur_state, 0 /* new state */);
        if (LIKELY(done)) {  // Spurious fail?
          // Wake a contender
          if (UNLIKELY(num_contenders_.LoadSequentiallyConsistent() > 0)) {
...
void ReaderWriterMutex::ExclusiveUnlock(Thread* self) {
  ...
  do {
    ...
    if (LIKELY(cur_state == -1)) {
      ...
      done =  state_.CompareExchangeWeakRelease(-1 /* cur_state*/, 0 /* new state */);
      if (LIKELY(done)) {  // Weak CAS may fail spuriously.
        // Wake any waiters.
        if (UNLIKELY(num_pending_readers_.LoadSequentiallyConsistent() > 0 ||
                     num_pending_writers_.LoadSequentiallyConsistent() > 0)) {
          futex(state_.Address(), FUTEX_WAKE, -1, NULL, NULL, 0);
        }
      }
    } else {
