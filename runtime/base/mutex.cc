  ...
  if (!recursive_ || recursion_count_ == 0) {
    ...
    do {
      ...
      if (LIKELY(cur_state == 1)) {
        ...
        // Note, the relaxed loads below musn't reorder before the Store.
        // TODO: the ordering here is non-trivial as state is split across 3 fields, fix by placing
        // a status bit into the state on contention.
        state_.StoreSequentiallyConsistent(0);
        // Wake a contender.
...
#if ART_USE_FUTEXES
    , state_(0), num_pending_readers_(0), num_pending_writers_(0), last_read_locker_(nullptr),
#endif
    exclusive_owner_(0) {
#if !ART_USE_FUTEXES
...
void ReaderWriterMutex::ExclusiveLock(Thread* self) {
  ...
  do {
    ...
    if (LIKELY(cur_state == 0)) {
      // Change state from 0 to 0xFFFF and impose load/store ordering appropriate for lock acquisition.
      done =  state_.CompareExchangeWeakAcquire(0 /* cur_state*/, 0xFFFF /* new state */);
    } else {
      // Failed to acquire, hang up.
      ScopedContentionRecorder scr(this, SafeGetTid(self), GetExclusiveOwnerTid());
...
void ReaderWriterMutex::ExclusiveUnlock(Thread* self) {
  ...
  do {
    rw_mutex_futex_state_type cur_state = state_.LoadRelaxed();
    if (LIKELY(cur_state == 0xFFFF)) {
      // We're no longer the owner.
      exclusive_owner_ = 0;
      // Change state from 0xFFFF to 0 and impose load/store ordering appropriate for lock release.
      // Note, the relaxed loads below musn't reorder before the CompareExchange.
      // TODO: the ordering here is non-trivial as state is split across 3 fields, fix by placing
      // a status bit into the state on contention.
      done =  state_.CompareExchangeWeakSequentiallyConsistent(0xFFFF /* cur_state*/, 0 /* new state */);
  ...
  do {
    ...
    if (cur_state != 0xFFFF) {
      ...
      if (((last_read_locker_ != self) &&
           (cur_state & ~(UINT64_C(0xFFFF)) != 0x0101010101010000)
           ) || (self == nullptr)) {
