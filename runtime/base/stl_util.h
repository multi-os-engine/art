...
size_t IndexOfExistingElement(const Container& container, const T& value) {
...
template <typename Container, typename T>
void RemoveExistingElement(Container& container, const T& value) {
  auto it = std::find(container.begin(), container.end(), value);
  DCHECK(it != container.end());  // Must exist.
  container.erase(it);
}
...
template <typename Container, typename T>
void ReplaceExistingElement(Container& container, const T& old_value, const T& new_value) {
