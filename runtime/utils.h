...
// Iterator pair adapter, for helping with for each.
template<typename Iterator>
class IteratorPair {
 public:
  IteratorPair(const IteratorPair&) = default;
  IteratorPair(IteratorPair&&) = default;
  IteratorPair& operator=(const IteratorPair&) = default;
  IteratorPair& operator=(IteratorPair&&) = default;
  IteratorPair(Iterator begin_it, Iterator end_it) : begin_(begin_it), end_(end_it) { }
...
template<typename Iterator>
static inline IteratorPair<Iterator> MakeIteratorPair(
    const Iterator& begin_it, const Iterator& end_it) {
  return IteratorPair<Iterator>(begin_it, end_it);
}

static inline bool ValidPointerSize(size_t pointer_size) {
