...
static void Charsets_asciiBytesToChars(JNIEnv* env, jclass, jbyteArray javaBytes, jint offset,
...
static void Charsets_isoLatin1BytesToChars(JNIEnv* env, jclass, jbyteArray javaBytes, jint offset,
                                           jint length, jcharArray javaChars) {
  ScopedByteArrayRO bytes(env, javaBytes);
  if (bytes.get() == nullptr) {
    return;
...
static jbyteArray Charsets_toAsciiBytes(JNIEnv* env, jclass, jstring java_string, jint offset,
                                        jint length) {
    return charsToBytes(env, java_string, offset, length, 0x7f);
}

static jbyteArray Charsets_toIsoLatin1Bytes(JNIEnv* env, jclass, jstring java_string, jint offset,
                                            jint length) {
    return charsToBytes(env, java_string, offset, length, 0xff);
}

static jbyteArray Charsets_toUtf8Bytes(JNIEnv* env, jclass, jstring java_string, jint offset,
                                       jint length) {
  ScopedObjectAccess soa(env);
  StackHandleScope<1> hs(soa.Self());
  Handle<mirror::String> string(hs.NewHandle(soa.Decode<mirror::String*>(java_string)));
