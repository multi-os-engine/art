 ...
 */
static jbyteArray charsToBytes(JNIEnv* env, jstring java_string, jint offset, jint length, jchar maxValidChar) {
  ScopedObjectAccess soa(env);
  SirtRef<mirror::String> string(soa.Self(), soa.Decode<mirror::String*>(java_string));
  if (string.get() == NULL) {
    return NULL;
  }
...
static jbyteArray Charsets_toAsciiBytes(JNIEnv* env, jclass, jstring java_string, jint offset, jint length) {
    return charsToBytes(env, java_string, offset, length, 0x7f);
}

static jbyteArray Charsets_toIsoLatin1Bytes(JNIEnv* env, jclass, jstring java_string, jint offset, jint length) {
    return charsToBytes(env, java_string, offset, length, 0xff);
}

static jbyteArray Charsets_toUtf8Bytes(JNIEnv* env, jclass, jstring java_string, jint offset, jint length) {
  ScopedObjectAccess soa(env);
  SirtRef<mirror::String> string(soa.Self(), soa.Decode<mirror::String*>(java_string));
  if (string.get() == NULL) {
    return NULL;
...
static JNINativeMethod gMethods[] = {
  ...
  NATIVE_METHOD(Charsets, toAsciiBytes, "!(Ljava/lang/String;II)[B"),
  NATIVE_METHOD(Charsets, toIsoLatin1Bytes, "!(Ljava/lang/String;II)[B"),
  NATIVE_METHOD(Charsets, toUtf8Bytes, "!(Ljava/lang/String;II)[B"),
};

void register_java_nio_charset_Charsets(JNIEnv* env) {
  REGISTER_NATIVE_METHODS("java/nio/charset/Charsets");
}
