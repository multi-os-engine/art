...
static jobjectArray Class_getDeclaredFieldsUnchecked(JNIEnv* env, jobject javaThis,
  ...
  for (int32_t i = 0; i < num_ifields; ++i) {
    ...
    if (publicOnly == JNI_FALSE || art_field->IsPublic()) {
      auto* field = mirror::Field::CreateFromArtField(soa.Self(), art_field);
      if (field == nullptr) {
        return nullptr;
      }
...
static jobject Class_getDeclaredFieldInternal(JNIEnv* env, jobject javaThis, jstring name) {
  ScopedFastNativeObjectAccess soa(env);
  auto* c = DecodeClass(soa, javaThis);
  auto* name_string = soa.Decode<mirror::String*>(name);
  auto* instance_fields = c->GetIFields();
  if (instance_fields != nullptr) {
    auto* art_field = FindFieldByName(soa.Self(), name_string, instance_fields);
    if (art_field != nullptr) {
      return soa.AddLocalReference<jobject>(mirror::Field::CreateFromArtField(soa.Self(),
                                                                              art_field));
    }
  }
  auto* static_fields = c->GetSFields();
  if (static_fields != nullptr) {
    auto* art_field = FindFieldByName(soa.Self(), name_string, static_fields);
    if (art_field != nullptr) {
      return soa.AddLocalReference<jobject>(mirror::Field::CreateFromArtField(soa.Self(),
                                                                              art_field));
    }
