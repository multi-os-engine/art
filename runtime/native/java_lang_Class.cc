...
static jobject Class_getDeclaredAnnotation(JNIEnv* env, jobject javaThis, jclass annotationType) {
  ScopedFastNativeObjectAccess soa(env);
  mirror::Class* klass = DecodeClass(soa, javaThis);
  if (klass->IsProxyClass()) {
    return nullptr;
  }
  mirror::Class* annotation_class = soa.Decode<mirror::Class*>(annotationType);
  return soa.AddLocalReference<jobject>(klass->GetDexFile().GetAnnotationForClass(klass, annotation_class));
}
...
static jboolean Class_isDeclaredAnnotationPresent(JNIEnv* env, jobject javaThis, jclass annotationType) {
  ScopedFastNativeObjectAccess soa(env);
  mirror::Class* klass = DecodeClass(soa, javaThis);
  if (klass->IsProxyClass()) {
    return false;
