...
static inline bool MethodMatchesConstructor(mirror::ArtMethod* m, jboolean publicOnly)
    SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  DCHECK(m != nullptr);
  ...
  if (direct_methods != nullptr) {
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      if (MethodMatchesConstructor(method, publicOnly)) {
        ...
        hconstructors->SetWithoutChecks<false>(i, constructor);
      }
    }
...
static jobject Class_getDeclaredMethodInternal(JNIEnv* env, jobject javaThis,
  ...
  return soa.AddLocalReference<jobject>(mirror::Method::CreateFromArtMethod(soa.Self(), result));
}
