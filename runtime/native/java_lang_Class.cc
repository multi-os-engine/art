...
static mirror::ObjectArray<mirror::Field>* GetDeclaredFields(
  ...
  for (ArtField& field : ifields) {
    if (!public_only || field.IsPublic()) {
      auto* reflect_field = mirror::Field::CreateFromArtField<kPointerSizeCur>(self,
                                                                               &field,
                                                                               force_resolve);
...
ALWAYS_INLINE static inline mirror::Field* GetDeclaredField(
  ...
  if (art_field != nullptr) {
    return mirror::Field::CreateFromArtField<kPointerSizeCur>(self, art_field, true);
  }
...
static jobject Class_getDeclaredConstructorInternal(
  ...
  mirror::Constructor* result = mirror::Class::GetDeclaredConstructorInternal<kPointerSizeCur,
                                                                              false>(
      soa.Self(),
...
static jobjectArray Class_getDeclaredConstructorsInternal(
  ...
  for (auto& m : h_klass->GetDirectMethods(kPointerSizeCur)) {
  ...
  for (auto& m : h_klass->GetDirectMethods(kPointerSizeCur)) {
    if (MethodMatchesConstructor(&m, publicOnly != JNI_FALSE)) {
      DCHECK_EQ(Runtime::Current()->GetClassLinker()->GetImagePointerSize(), kPointerSizeCur);
      DCHECK(!Runtime::Current()->IsActiveTransaction());
      auto* constructor = mirror::Constructor::CreateFromArtMethod<kPointerSizeCur, false>(
          soa.Self(), &m);
...
static jobject Class_getDeclaredMethodInternal(JNIEnv* env, jobject javaThis,
  ...
  DCHECK_EQ(Runtime::Current()->GetClassLinker()->GetImagePointerSize(), kPointerSizeCur);
  DCHECK(!Runtime::Current()->IsActiveTransaction());
  mirror::Method* result = mirror::Class::GetDeclaredMethodInternal<kPointerSizeCur, false>(
      soa.Self(),
      DecodeClass(soa, javaThis),
...
static jobjectArray Class_getDeclaredMethodsUnchecked(JNIEnv* env, jobject javaThis,
  ...
  for (auto& m : klass->GetDeclaredMethods(kPointerSizeCur)) {
  ...
  for (auto& m : klass->GetDeclaredMethods(kPointerSizeCur)) {
    ...
    if ((publicOnly == JNI_FALSE || (modifiers & kAccPublic) != 0) &&
      ...
      DCHECK_EQ(Runtime::Current()->GetClassLinker()->GetImagePointerSize(), kPointerSizeCur);
      DCHECK(!Runtime::Current()->IsActiveTransaction());
      auto* method = mirror::Method::CreateFromArtMethod<kPointerSizeCur, false>(soa.Self(), &m);
      if (method == nullptr) {
        soa.Self()->AssertPendingException();
        return nullptr;
      }
