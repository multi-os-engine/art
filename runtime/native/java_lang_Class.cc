...
static ALWAYS_INLINE inline bool MethodMatchesConstructor(mirror::ArtMethod* m, jboolean publicOnly)
    SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  DCHECK(m != nullptr);
  DCHECK(publicOnly == JNI_FALSE || publicOnly == JNI_TRUE) << publicOnly;
  return (publicOnly == JNI_FALSE || m->IsPublic()) && !m->IsStatic() && m->IsConstructor();
}

// TODO: Unstarted runtime cutouts for these?
static jobjectArray Class_getDeclaredConstructorsInternal(
  ...
  auto count = direct_methods != nullptr ? direct_methods->GetLength() : 0u;
  // Two pass approach for speed.
  for (size_t i = 0; i < count; ++i) {
    constructor_count += MethodMatchesConstructor(direct_methods->GetWithoutChecks(i), publicOnly);
  }
  auto hconstructors = hs.NewHandle(mirror::ObjectArray<mirror::Constructor>::Alloc(
      soa.Self(), mirror::Constructor::ArrayClass(), constructor_count));
  if (UNLIKELY(hconstructors.Get() == nullptr)) {
    soa.Self()->AssertPendingException();
    return nullptr;
  }
  constructor_count = 0;
  for (size_t i = 0; i < count; ++i) {
    auto* method = direct_methods->GetWithoutChecks(i);
    if (MethodMatchesConstructor(method, publicOnly)) {
      auto* constructor = mirror::Constructor::CreateFromArtMethod(soa.Self(), method);
      if (UNLIKELY(constructor == nullptr)) {
        soa.Self()->AssertPendingException();
        return nullptr;
      }
      hconstructors->SetWithoutChecks<false>(constructor_count++, constructor);
    }
  }
  return soa.AddLocalReference<jobjectArray>(hconstructors.Get());
}
