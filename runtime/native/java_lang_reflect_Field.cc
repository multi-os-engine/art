...
static jobjectArray Field_getSignatureAnnotation(JNIEnv* env, jobject javaField) {
  ScopedFastNativeObjectAccess soa(env);
  ArtField* field = soa.Decode<mirror::Field*>(javaField)->GetArtField();
  return soa.AddLocalReference<jobjectArray>(field->GetDexFile()->GetSignatureAnnotationForField(field));
}

static jboolean Field_isAnnotationPresentNative(JNIEnv* env, jobject javaField, jclass annotationType) {
  ScopedFastNativeObjectAccess soa(env);
  ArtField* field = soa.Decode<mirror::Field*>(javaField)->GetArtField();
  mirror::Class* klass = soa.Decode<mirror::Class*>(annotationType);
...
static JNINativeMethod gMethods[] = {
  ...
  NATIVE_METHOD(Field, getAnnotationNative, "!(Ljava/lang/Class;)Ljava/lang/annotation/Annotation;"),
