...
static void CopyProfileFile(const char* oldfile, const char* newfile) {
  ...
  if (src.get() == -1) {
    LOG(ERROR) << "Failed to open profile file " << oldfile << ": " << strerror(errno)
  ...
  if (fstat(src.get(), &stat_src) == -1) {
    LOG(ERROR) << "Failed to get stats for profile file  " << oldfile << ": " << strerror(errno)
      << ". My uid:gid is " << getuid() << ":" << getgid();
    return;
  }
  ...
  if (dst.get()  == -1) {
    LOG(ERROR) << "Failed to create/write prev profile file " << newfile << ": " << strerror(errno)
      << ".  My uid:gid is " << getuid() << ":" << getgid();
    return;
  }

  if (sendfile(dst.get(), src.get(), 0, stat_src.st_size) == -1) {
    LOG(ERROR) << "Failed to copy profile file " << oldfile << " to " << newfile << ": " << strerror(errno)
      << ". My uid:gid is " << getuid() << ":" << getgid();
  }
}
