...
static jlong DexFile_openDexFileNative(JNIEnv* env, jclass, jstring javaSourceName, jstring javaOutputName, jint) {
  ...
  std::unique_ptr<DexFileUniquePtrs> dex_files(new std::vector<std::unique_ptr<const DexFile>>());
...
static DexFileUniquePtrs* toDexFiles(jlong dex_file_address, JNIEnv* env) {
  DexFileUniquePtrs* dex_files = reinterpret_cast<DexFileUniquePtrs*>(
...
static void DexFile_closeDexFile(JNIEnv* env, jclass, jlong cookie) {
  std::unique_ptr<DexFileUniquePtrs> dex_files(toDexFiles(cookie, env));
...
static jclass DexFile_defineClassNative(JNIEnv* env, jclass, jstring javaName, jobject javaLoader,
                                        jlong cookie) {
  DexFileUniquePtrs* dex_files = toDexFiles(cookie, env);
  if (dex_files == NULL) {
