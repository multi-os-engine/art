...
static bool ConvertJavaArrayToDexFiles(
  ...
  oat_file = reinterpret_cast<const OatFile*>(static_cast<uintptr_t>(*(long_data)));
...
static jlongArray ConvertDexFilesToJavaArray(JNIEnv* env,
  ...
  jlong* tmp = long_data;
  *tmp = reinterpret_cast<uintptr_t>(oat_file);
  ++tmp;
  for (std::unique_ptr<const DexFile>& dex_file : vec) {
...
static jboolean DexFile_closeDexFile(JNIEnv* env, jclass, jobject cookie) {
  ...
  int32_t i = 1;  // Oat file is at index 0.
