...
static jbyte kDexoptUnneeded = 0;
static jbyte kPatchoatNeeded = 1;
static jbyte kDexoptNeeded = 2;
    ...
    if (oat_dex_file != nullptr) {
      ...
      if (!DexFile::GetChecksum(filename, &location_checksum, &error_msg)) {
        ...
        if (ClassLinker::VerifyOatChecksums(oat_file.get(), target_instruction_set, &error_msg)) {
          ...
          return kDexoptUnneeded;
        } else if (should_relocate_if_possible &&
                    ClassLinker::IsOatFilePatchable(oat_file.get(), target_instruction_set)) {
          if (kVerboseLogging) {
            LOG(INFO) << "DexFile_isDexOptNeeded file " << oat_filename
...
static jbyte IsDexOptNeededInternal(JNIEnv* env, const char* filename,
  ...
  const bool kVerboseLogging = true;  // Spammy logging.
  ...
  if (require_system_version && system_decision == kPatchoatNeeded
                             && dalvik_cache_decision == kDexoptUnneeded) {
...
static jboolean DexFile_isDexOptNeeded(JNIEnv* env, jclass, jstring javaFilename) {
  ...
  ScopedUtfChars filename(env, javaFilename);
  return kDexoptUnneeded != IsDexOptNeededInternal(env, filename.c_str(), nullptr /* pkgname */,
