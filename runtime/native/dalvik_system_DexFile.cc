...
static jlong DexFile_openDexFileNative(JNIEnv* env, jclass, jstring javaSourceName, jstring javaOutputName, jint) {
  ...
  std::vector<const DexFile*>* dex_files = new std::vector<const DexFile*>();
  std::vector<std::string> error_msgs;

  bool success = linker->OpenMultiDexFilesFromOat(sourceName.c_str(), outputName.c_str(),
                                                  &error_msgs, dex_files);

  if (success) {
    return static_cast<jlong>(reinterpret_cast<uintptr_t>(dex_files));
  } else {
...
static void DexFile_closeDexFile(JNIEnv* env, jclass, jlong cookie) {
  ...
  dex_files = toDexFiles(cookie, env);
...
static jobjectArray DexFile_getClassNameList(JNIEnv* env, jclass, jlong cookie) {
  ...
  if (dex_files != nullptr) {
    ...
    if (result != nullptr) {
      ...
      jsize i = 0;
      for (; it != it_end; it++, ++i) {
        ScopedLocalRef<jstring> jdescriptor(env, env->NewStringUTF(*it));
