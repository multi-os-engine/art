...
static jbyte IsDexOptNeededInternal(JNIEnv* env, const char* filename,
  ...
  // Always treat elements of the current bootclasspath as up-to-date.
  // We do this even if the target ISA is different from the current ISA.
  // We don't expect anything on the boot class path could be updated anyway.
  // TODO [ruhler]: This check seems odd to me. Do we gain anything from it?
  // I'm leaving it in to be consistent with how the code was before the
  // OatFileManager class was introduced.
  if (oat_file_manager.IsInCurrentBootClassPath()) {
    return kUpToDate;
  }
  ...
  switch (status) {
    ...
    case OatFileManager::kOutOfDate: return kDexoptNeeded;
  }
  UNREACHABLE();
}
