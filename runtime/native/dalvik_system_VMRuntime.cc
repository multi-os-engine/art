...
static void VMRuntime_registerAppInfo(JNIEnv* env,
  ...
  // TODO(calin): For now, use the application directory for the profile file.
  // This should be moved to the application code_cache directory. However, that
  // might not exist (if this is an isolated process the framework will not create
  // the directory) and thus extra checks are needed.
