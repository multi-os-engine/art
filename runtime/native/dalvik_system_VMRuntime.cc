...
static void PreloadDexCachesResolveField(Handle<mirror::DexCache> dex_cache, uint32_t field_idx,
                                         bool is_static)
    SHARED_REQUIRES(Locks::mutator_lock_) {
  ArtField* field = dex_cache->GetResolvedField(field_idx, kPointerSizeCur);
  if (field != nullptr) {
    return;
  }
  const DexFile* dex_file = dex_cache->GetDexFile();
  ...
  dex_cache->SetResolvedField(field_idx, field, kPointerSizeCur);
}
...
static void PreloadDexCachesResolveMethod(Handle<mirror::DexCache> dex_cache, uint32_t method_idx,
    ...
    SHARED_REQUIRES(Locks::mutator_lock_) {
  ArtMethod* method = dex_cache->GetResolvedMethod(method_idx, kPointerSizeCur);
  if (method != nullptr) {
    return;
  }
  const DexFile* dex_file = dex_cache->GetDexFile();
  ...
  switch (invoke_type) {
    case kDirect:
    case kStatic:
      method = klass->FindDirectMethod(dex_cache.Get(), method_idx, kPointerSizeCur);
      break;
    case kInterface:
      method = klass->FindInterfaceMethod(dex_cache.Get(), method_idx, kPointerSizeCur);
      break;
    case kSuper:
    case kVirtual:
      method = klass->FindVirtualMethod(dex_cache.Get(), method_idx, kPointerSizeCur);
      break;
    default:
  ...
  dex_cache->SetResolvedMethod(method_idx, method, kPointerSizeCur);
}
...
static void PreloadDexCachesStatsFilled(DexCacheStats* filled)
  ...
  for (const DexFile* dex_file : class_linker->GetBootClassPath()) {
    ...
    for (size_t j = 0; j < dex_cache->NumResolvedMethods(); j++) {
      ArtMethod* method = dex_cache->GetResolvedMethod(j, kPointerSizeCur);
      if (method != nullptr) {
        filled->num_methods++;
      }
    }
