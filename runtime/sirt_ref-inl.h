...
template<class T> SirtRef<T>::SirtRef(Thread* self, T* object) : self_(self), sirt_(object) {
  VerifyObject(object);
  self_->PushSirt(&sirt_);
}

template<class T> SirtRef<T>::~SirtRef() {
  StackIndirectReferenceTable* top_sirt = self_->PopSirt();
  DCHECK_EQ(top_sirt, &sirt_);
}

template<class T> T& SirtRef<T>::operator*() const {
  return *get();
}
...
template<class T> T* SirtRef<T>::get() const {
  return down_cast<T*>(sirt_.GetReference(0));
}

template<class T> T* SirtRef<T>::reset(T* object) {
  VerifyObject(object);
  T* old_ref = get();
