...
void OatFileManager::RegisterOatFile(std::unique_ptr<const OatFile>&& oat_file) {
...
OatFile& OatFileManager::RegisterImageOatFile(gc::space::ImageSpace* space) {
  VLOG(startup) << __PRETTY_FUNCTION__ << " entering";
  OatFile* oat_file = space->ReleaseOatFile();
  RegisterOatFile(std::unique_ptr<const OatFile>(oat_file));
...
static void FreeDexFilesInHeap(/*inout*/std::priority_queue<DexFileAndClassPair>* heap) {
  while (!heap->empty()) {
...
std::vector<std::unique_ptr<const DexFile>> OatFileManager::OpenDexFilesFromOat(
  ...
  if (oat_file != nullptr) {
    ...
    if (accept_oat_file) {
      ...
      RegisterOatFile(std::unique_ptr<const OatFile>(source_oat_file));
    }
  }
