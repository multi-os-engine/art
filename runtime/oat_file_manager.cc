...
//
// TODO: Is it possible the oat and odex files could overlap, meaning we can't
// have both simultaneously loaded executable?
...
bool OatFileManager::OdexFileIsRelocated() {
  ...
  CHECK(odex_file_.get() != nullptr)
    << "If odex_file_ is null, we should have had OdexFileIsOutOfDate";
...
bool OatFileManager::OatFileIsRelocated() {
  ...
  CHECK(oat_file_.get() != nullptr)
    << "If oat_file_ is null, we should have had OatFileIsOutOfDate";
...
void OatFileManager::LoadOdexFile() {
  CHECK(!oat_file_released_) << "LoadOdexFile called after oat file released.";
...
void OatFileManager::LoadOatFile() {
  CHECK(!oat_file_released_) << "LoadOatFile called after oat file released.";
...
bool OatFileManager::GivenOatFileIsRelocated(const OatFile& file) {
  // TODO: Should we cache these results? If we don't, we can easily end up
  // generating duplicate log entries, which may be confusing to log readers.
  if (file.IsPic()) {
    return true;
...
bool ExecutableOatFileManager::LoadDexFiles(
  ...
  std::unique_ptr<OatFile> best_oat_file = GetBestOatFile();
  if (best_oat_file.get() != nullptr) {
    LoadDexFilesFromGivenOatFile(dex_location_, *best_oat_file, dex_files);
    *oat_file = std::move(best_oat_file);
    return true;
  }
