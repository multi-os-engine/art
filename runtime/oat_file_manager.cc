...
void OatFileManager::RegisterOatFile(const OatFile* oat_file) {
  ...
  if (kIsDebugBuild) {
    for (const std::unique_ptr<const OatFile>& existing : oat_files_) {
      CHECK_NE(oat_file, existing.get()) << oat_file->GetLocation();
  ...
  oat_files_.push_back(std::unique_ptr<const OatFile>(oat_file));
}
...
std::vector<std::unique_ptr<const DexFile>> OatFileManager::OpenDexFilesFromOat(
  ...
  bool already_loaded = false;
  {
    ...
    for (const std::unique_ptr<const OatFile>& oat_file : oat_files_) {
      ...
      if (oat_file_assistant.GivenOatFileIsUpToDate(*oat_file.get())) {
        already_loaded = true;
        break;
      }
    }
