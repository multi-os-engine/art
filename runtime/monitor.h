...
class Monitor {
 public:
  ...
  bool IsLocked() SHARED_REQUIRES(Locks::mutator_lock_);
...
class MonitorList {
 public:
  ...
  void Add(Monitor* m) SHARED_REQUIRES(Locks::mutator_lock_);
  ...
  size_t DeflateMonitors() REQUIRES(!monitor_list_lock_)
