...
#define SET_STATIC_PRIMITIVE_FIELD(fn, value) \
  ...
  static jint RegisterNativeMethods(JNIEnv* env, jclass java_class, const JNINativeMethod* methods,
    ...
    for (jint i = 0; i < method_count; ++i) {
      ...
      // a response to a thread suspension requeset which is typically
      // used for a GC root scanning, etc. If a fast JNI call takes a
      // long time, it could cause causing longer thread suspension
      // latency and GC pauses.
      //
      // Thus, fast JNI should be used with care. It should probably
      // be used for a JNI call that should take a short amount of
      // time (eg. no long-running loop) and has no chance of blocking
      // (eg. no locks, I/O, etc.)
      //
