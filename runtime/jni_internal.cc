...
static mirror::Class* EnsureInitialized(Thread* self, mirror::Class* klass)
  ...
  Handle<mirror::Class> sirt_klass(self, klass);
...
static jfieldID FindFieldID(const ScopedObjectAccess& soa, jclass jni_class, const char* name,
    ...
    SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  Handle<mirror::Class> c(soa.Self(), EnsureInitialized(soa.Self(),
  ...
  if (sig[1] != '\0') {
    Handle<mirror::ClassLoader> class_loader(soa.Self(), c->GetClassLoader());
  ...
  if (field_type == nullptr) {
    ...
    Handle<mirror::Throwable> cause(soa.Self(), soa.Self()->GetException(&throw_location));
 ...
 public:
  ...
  static jclass FindClass(JNIEnv* env, const char* name) {
    ...
    if (runtime->IsStarted()) {
      Handle<mirror::ClassLoader> class_loader(soa.Self(), GetClassLoader(soa));
  ...
  static void ExceptionDescribe(JNIEnv* env) {
    ...
    Handle<mirror::Object> old_throw_this_object(soa.Self(), nullptr);
    Handle<mirror::ArtMethod> old_throw_method(soa.Self(), nullptr);
    Handle<mirror::Throwable> old_exception(soa.Self(), nullptr);
...
bool JavaVMExt::LoadNativeLibrary(const std::string& path,
  ...
  if (sym == nullptr) {
    ...
    Handle<mirror::ClassLoader> old_class_loader(self, self->GetClassLoaderOverride());
