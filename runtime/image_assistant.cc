...
ImageState ImageInfo::GetImageState(bool relocation_needed) const {
  if (is_system_) {
    ...
    if (!IsImageValid()) {
      // We cannot use this image. The header fails verification or the file does not exist.
      return ImageState::kImageUnusable;
    } else if (cache_state == ImageState::kImageUsable) {
      ...
      CHECK((relocation_needed && cache_state == ImageState::kImageOutOfDate) ||
    ...
    if (got_system_header && !got_cache_header) {
      // We got a system header but not a cache. If we can relocate we should return kImageOutOfDate
      // to show we need relocation.
      return relocation_needed ? ImageState::kImageOutOfDate : ImageState::kImageUnusable;
    } else if (!got_system_header && !got_cache_header) {
      // We cannot find anything. Recompilation is the only option.
      return ImageState::kImageUnusable;
    } else if (!got_system_header && got_cache_header) {
      // No system image to check and the cache header is present, so we are good.
      return ImageState::kImageUsable;
    } else if (system_header.GetOatChecksum() != cache_header.GetOatChecksum()) {
      // We have a checksum mis-match. Return kImageOutOfDate if we can Relocate otherwise we cannot
      // use this image.
      return relocation_needed ? ImageState::kImageOutOfDate : ImageState::kImageUnusable;
    } else {
      // We have a system and a cache image and the checksum's match, so this is usable.
      return ImageState::kImageUsable;
    }
  }
}
