...
std::unique_ptr<File> ImageInfo::OpenImage(std::string* error_msg) const {
  std::unique_ptr<File> ret(OpenImageUnchecked());
  if (ret == nullptr) {
    *error_msg = "File could not be opened or found.";
    return std::unique_ptr<File>(nullptr);
  }
  ImageHeader header;
  const bool success = ret->ReadFully(&header, sizeof(ImageHeader));
  if (!success) {
    *error_msg = "Unable to read image header from file";
    return std::unique_ptr<File>(nullptr);
  } else if (!header.IsValid()) {
    *error_msg = "Header is invalid";
    return std::unique_ptr<File>(nullptr);
  } else {
    return ret;
  }
...
std::unique_ptr<File> ImageInfo::OpenImageUnchecked() const {
...
bool ImageInfo::GetImageHeader(ImageHeader* header) const {
  ...
  std::unique_ptr<File> image_file(OpenImageUnchecked());
  if (image_file == nullptr) {
    return false;
  }
  const bool success = image_file->ReadFully(header, sizeof(ImageHeader));
  return success && header->IsValid();
}
...
std::string ImageAssistant::GetImageFilename() const {
  return GetImageInfo().GetFilename();
}
std::string ImageAssistant::GetSystemImageFilename() const {
  return GetSystemImageInfo().GetFilename();
}
std::string ImageAssistant::GetCacheImageFilename() const {
  return GetCacheImageInfo().GetFilename();
}
...
// Returns the filename of the image corresponding to requested image_location, or the filename
// where a new image should be written if one doesn't exist. Looks for a generated image in the
// specified location and then in the dalvik-cache.
//
// Returns true if an image was found, false otherwise.
static bool FindImageFilename(const std::string& location,
                              InstructionSet isa,
  ...
  *system_filename = ::art::GetSystemImageFilename(location.c_str(), isa);
...
ImageInfo ImageAssistant::GetSpecialImageInfo(bool is_system) const {
  std::string cache_filename, system_filename;
  bool has_system = false;
  bool has_cache = false;
  bool dalvik_cache_exists = false;
  bool is_global_cache = false;
  FindImageFilename(location_,
                    isa_,
                    &system_filename,
                    &has_system,
                    &cache_filename,
                    ...
                    &has_cache,
                    &is_global_cache);
  return ImageInfo(system_filename, cache_filename, is_system);
}

ImageInfo ImageAssistant::GetCacheImageInfo() const {
  return GetSpecialImageInfo(false);
}

ImageInfo ImageAssistant::GetSystemImageInfo() const {
  return GetSpecialImageInfo(true);
}
