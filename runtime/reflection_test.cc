...
class ReflectionTest : public CommonCompilerTest {
 protected:
  ...
  void ReflectionTestMakeExecutable(ArtMethod** method,
    ...
    *method = is_static ? c->FindDirectMethod(method_name, method_signature, kPointerSizeCur)
                        : c->FindVirtualMethod(method_name, method_signature, kPointerSizeCur);
...
TEST_F(ReflectionTest, StaticMainMethod) {
  ...
  ArtMethod* method = klass->FindDirectMethod("main", "([Ljava/lang/String;)V", kPointerSizeCur);
