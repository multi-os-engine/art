...
class ExceptionTest : public CommonRuntimeTest {
 protected:
  virtual void SetUp() {
    ...
    uint32_t vmap_table_offset = code_size;
    uint32_t mapping_table_offset = vmap_table_offset + fake_vmap_table_data.size();
    ...
    const uint8_t* code_ptr = &fake_header_code_and_maps_[sizeof(method_header)];
