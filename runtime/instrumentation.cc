...
void Instrumentation::ConfigureStubs(const std::string& key,
  ...
  // TODO this can be computed so we don't really need a field.
  ...
  if (kVerboseInstrumentation) {
    LOG(INFO) << "ConfigureStubs: current instrumentation level is " << current_level;
    LOG(INFO) << "ConfigureStubs: client '" << key << "' requests level " << desired_level;
    LOG(INFO) << "ConfigureStubs: new instrumentation level is " << requested_level;
  }
