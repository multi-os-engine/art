...
void Instrumentation::DisableDeoptimization(const std::string& key) {
...
void Instrumentation::DeoptimizeEverything(const std::string& key) {
  CHECK(deoptimization_enabled_);
  ConfigureStubs(key, InstrumentationLevel::kInstrumentWithInterpreter);
}

void Instrumentation::UndeoptimizeEverything(const std::string& key) {
  CHECK(interpreter_stubs_installed_);
  CHECK(deoptimization_enabled_);
  ConfigureStubs(key, InstrumentationLevel::kInstrumentNothing);
}

void Instrumentation::EnableMethodTracing(const std::string& key, bool needs_interpreter) {
...
void Instrumentation::DisableMethodTracing(const std::string& key) {
  ConfigureStubs(key, InstrumentationLevel::kInstrumentNothing);
}
