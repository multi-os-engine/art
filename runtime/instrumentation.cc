...
static void MaybeAddListenerTo(Instrumentation::InstrumentationEvent event,
                               uint32_t events,
...
static void MaybeRemoveListenerFrom(Instrumentation::InstrumentationEvent event,
                                    uint32_t events,
                                    std::list<InstrumentationListener*>& list,
                                    InstrumentationListener* listener,
                                    bool* has_listener)
...
void Instrumentation::RemoveListener(InstrumentationListener* listener, uint32_t events) {
  Locks::mutator_lock_->AssertExclusiveHeld(Thread::Current());
  MaybeRemoveListenerFrom(kMethodEntered,
                          events,
                          method_entry_listeners_,
                          listener,
                          &have_method_entry_listeners_);
  MaybeRemoveListenerFrom(kMethodExited,
                          events,
                          method_exit_listeners_,
                          listener,
                          &have_method_exit_listeners_);
  MaybeRemoveListenerFrom(kMethodUnwind,
                          events,
                          method_unwind_listeners_,
                          listener,
                          &have_method_unwind_listeners_);
  MaybeRemoveListenerFrom(kBackwardBranch,
                          events,
                          backward_branch_listeners_,
                          listener,
                          &have_backward_branch_listeners_);
  MaybeRemoveListenerFrom(kInvokeVirtualOrInterface,
                          events,
                          invoke_virtual_or_interface_listeners_,
                          listener,
                          &have_invoke_virtual_or_interface_listeners_);
  MaybeRemoveListenerFrom(kDexPcMoved,
                          events,
                          dex_pc_listeners_,
                          listener,
                          &have_dex_pc_listeners_);
  MaybeRemoveListenerFrom(kFieldRead,
                          events,
                          field_read_listeners_,
                          listener,
                          &have_field_read_listeners_);
  MaybeRemoveListenerFrom(kFieldWritten,
                          events,
                          field_write_listeners_,
                          listener,
                          &have_field_write_listeners_);
  MaybeRemoveListenerFrom(kExceptionCaught,
                          events,
                          exception_caught_listeners_,
                          listener,
                          &have_exception_caught_listeners_);
