...
static constexpr int STACK_SIZE = 16*1024;    // 16K signal stack.

void Thread::SetUpAlternateSignalStack() {
  // Create and set an alternate signal stack.
  stack_t ss;
  ss.ss_sp = new uint8_t[STACK_SIZE];
...
void Thread::TearDownAlternateSignalStack() {
  ...
  ss.ss_size = STACK_SIZE;  // Avoid ENOMEM failure with Mac OS' buggy libc.
  SigAltStack(&ss, NULL);
