...
class PACKED(4) HandleScope {
 public:
  ...
  static size_t LinkOffset(PointerSize pointer_size ATTRIBUTE_UNUSED) {
    return 0;
  }

  // Offset of length within handle scope, used by generated code.
  static size_t NumberOfReferencesOffset(PointerSize pointer_size) {
    return static_cast<size_t>(pointer_size);
  }

  // Offset of link within handle scope, used by generated code.
  static size_t ReferencesOffset(PointerSize pointer_size) {
    return static_cast<size_t>(pointer_size) + sizeof(number_of_references_);
  }
 ...
 protected:
  ...
  ALWAYS_INLINE StackReference<mirror::Object>* GetReferences() const {
    uintptr_t address = reinterpret_cast<uintptr_t>(this) + ReferencesOffset(kPointerSizeCur);
