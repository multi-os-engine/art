...
class OatFile {
 public:
  ...
  class OatDexFile {
   public:
    ...
    const std::string& GetCanonicalDexFileLocation() const {
      if (canonical_dex_file_location_.empty()) {
        return dex_file_location_;
      }
  ...
  const OatDexFile* GetOatDexFile(DexFile::LocationHelper* dex_location,
 ...
 private:
  ...
  // Map each plain and canonical (if different) dex file location retrieved from the
