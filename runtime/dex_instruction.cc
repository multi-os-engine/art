        ...
        case CONST_STRING:
          if (file != nullptr) {
            ...
            if (string_idx < file->NumStringIds()) {
              ...
              os << StringPrintf("const-string v%d, <<invalid-string-idx-%d>> // string@%d", VRegA_21c(), string_idx, string_idx);
            }
            break;
          }
      ...
      if (Opcode() == CONST_STRING_JUMBO) {
        ...
        if (file != nullptr) {
          if (string_idx < file->NumStringIds()) {
            ...
            os << StringPrintf("%s v%d, <<invalid-string-idx-%d>> // string@%d", opcode,
                               VRegA_31c(), string_idx, string_idx);
          }
        } else {
