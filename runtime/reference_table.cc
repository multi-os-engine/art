...
struct ObjectComparator {
  bool operator()(GcRoot<mirror::Object> root1, GcRoot<mirror::Object> root2) const
    ...
    CHECK(!runtime->IsClearedJniWeakGlobal(obj1));
    CHECK(!runtime->IsClearedJniWeakGlobal(obj2));
    // Sort by class...
...
void ReferenceTable::Dump(std::ostream& os, Table& entries) {
  ...
  size_t equiv = 0;
  size_t identical = 0;
  mirror::Object* prev = sorted_entries.front().Read<kWithoutReadBarrier>();
  for (size_t idx = 1; idx < sorted_entries.size(); idx++) {
