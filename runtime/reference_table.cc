...
void ReferenceTable::Dump(std::ostream& os, Table& entries) {
  ...
  struct GcRootComparator {
    bool operator()(GcRoot<mirror::Object> root1, GcRoot<mirror::Object> root2) const
      ...
      // change during the sorting process.
  ...
  for (int idx = count - 1; idx >= first; --idx) {
    ...
    // Call GetClass before IsClearedJniWeakGlobal so that all of the object classes are guaranteed
    // to be forwarded.
    mirror::Object* klass = ref->GetClass();
