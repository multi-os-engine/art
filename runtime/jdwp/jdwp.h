...
struct JdwpState {
  ...
  // thread.
  // Respectively calls SetWaitForJdwpToken and ClearWaitForJdwpToken.
  void AcquireJdwpTokenForCommand() LOCKS_EXCLUDED(jdwp_token_lock_) {
    CHECK_EQ(Thread::Current(), GetDebugThread()) << "Must be the debugger thread";
  ...
  void AcquireJdwpTokenForEvent(ObjectId threadId) LOCKS_EXCLUDED(jdwp_token_lock_) {
    CHECK_NE(Thread::Current(), GetDebugThread()) << "Must not be the debugger thread";
    CHECK_NE(debug_thread_id_, threadId) << "Must not be the debugger thread id";
