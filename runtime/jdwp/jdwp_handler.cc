...
static JdwpError FinishInvoke(JdwpState*, Request* request, ExpandBuf* pReply,
  ...
  if (err == ERR_NONE) {
    if (is_constructor) {
      ...
      // unless we threw, in which case we return nullptr.
      resultTag = JT_OBJECT;
...
size_t JdwpState::ProcessRequest(Request* request, ExpandBuf* pReply) {
  ...
  for (i = 0; i < arraysize(gHandlers); ++i) {
    if (gHandlers[i].cmdSet == request->GetCommandSet() && gHandlers[i].cmd == request->GetCommand() && gHandlers[i].func != nullptr) {
