...
void JdwpState::PostLocationEvent(const EventLocation* pLoc, mirror::Object* thisPtr,
  ...
  std::vector<JdwpEvent*> match_list;
  {
    ...
    if ((eventFlags & Dbg::kBreakpoint) != 0) {
      FindMatchingEvents(EK_BREAKPOINT, basket, &match_list);
    }
    if ((eventFlags & Dbg::kSingleStep) != 0) {
      FindMatchingEvents(EK_SINGLE_STEP, basket, &match_list);
    }
    if ((eventFlags & Dbg::kMethodEntry) != 0) {
      FindMatchingEvents(EK_METHOD_ENTRY, basket, &match_list);
    }
    if ((eventFlags & Dbg::kMethodExit) != 0) {
      FindMatchingEvents(EK_METHOD_EXIT, basket, &match_list);
      FindMatchingEvents(EK_METHOD_EXIT_WITH_RETURN_VALUE, basket, &match_list);
    }
  }
  if (match_list.empty()) {
    // No matching event.
...
void JdwpState::PostFieldEvent(const EventLocation* pLoc, mirror::ArtField* field,
  ...
  std::vector<JdwpEvent*> match_list;
  {
    ...
    match_list.reserve(event_list_size_);
    if (is_modification) {
      FindMatchingEvents(EK_FIELD_MODIFICATION, basket, &match_list);
    } else {
      FindMatchingEvents(EK_FIELD_ACCESS, basket, &match_list);
    }
  }
  if (match_list.empty()) {
    // No matching event.
...
void JdwpState::PostThreadChange(Thread* thread, bool start) {
  ...
  std::vector<JdwpEvent*> match_list;
  {
    ...
    match_list.reserve(event_list_size_);
    if (start) {
      FindMatchingEvents(EK_THREAD_START, basket, &match_list);
    } else {
      FindMatchingEvents(EK_THREAD_DEATH, basket, &match_list);
    }
  }
  if (match_list.empty()) {
    // No matching event.
...
void JdwpState::PostException(const EventLocation* pThrowLoc, mirror::Throwable* exception_object,
  ...
  std::vector<JdwpEvent*> match_list;
  {
    MutexLock mu(Thread::Current(), event_list_lock_);
    match_list.reserve(event_list_size_);
    FindMatchingEvents(EK_EXCEPTION, basket, &match_list);
  }
  if (match_list.empty()) {
    // No matching event.
    return;
...
void JdwpState::PostClassPrepare(mirror::Class* klass) {
  ...
  std::vector<JdwpEvent*> match_list;
  {
    MutexLock mu(Thread::Current(), event_list_lock_);
    match_list.reserve(event_list_size_);
    FindMatchingEvents(EK_CLASS_PREPARE, basket, &match_list);
  }
  if (match_list.empty()) {
    // No matching event.
    return;
