...
inline uint16_t GetTrailingUtf16Char(uint32_t pseudo_pair) {
  return static_cast<uint16_t>(pseudo_pair >> 16);
}

inline uint16_t GetLeadingUtf16Char(uint32_t pseudo_pair) {
  return static_cast<uint16_t>(pseudo_pair & 0x0000FFFF);
}
...
inline uint32_t GetUtf16FromUtf8(const char** utf8_data_in) {
  ...
  const uint32_t utf32_code_point = ((one & 0x0f) << 18) | ((two & 0x3f) << 12)
      | ((three & 0x3f) << 6) | (four & 0x3f);

  // Step one: subtract 0x100000 from the code point to get a 20 bit
  // number in the range [0, 0x0FFFFF].
  const uint32_t utf16_basis = utf32_code_point - 0x100000;

  uint32_t surrogate_pair = 0;

  // Step two: Write out the high (leading) surrogate to the bottom 16 bits
  // of the of the 32 bit type.
  surrogate_pair |= ((utf16_basis >> 10) + 0xd800) & 0xffff;

  // Step three : Write out the low (trailing) surrogate to the top 16 bits.
  surrogate_pair |= ((utf16_basis & 0x03ff) + 0xdc00) << 16;
...
inline int CompareModifiedUtf8ToModifiedUtf8AsUtf16CodePointValues(const char* utf8_1,
  ...
  do {
    ...
    // TODO: Benchmark this change. The older implementation "directly" compared
    // byte values but that code was convoluted and does not appear to be backed up by
    // benchmark results.
