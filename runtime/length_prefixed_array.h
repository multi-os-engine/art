...
class LengthPrefixedArray {
 public:
  explicit LengthPrefixedArray(uint64_t length = 0) : length_(length) {}
  ...
  static size_t OffsetOfElement(size_t index, size_t element_size = sizeof(T)) {
    return sizeof(LengthPrefixedArray<T>) + index * element_size;
  }

  // TODO: Use cleaner API with std allocators.
  static LengthPrefixedArray<T>* Create(void* memory, size_t length) {
    auto* ret = reinterpret_cast<LengthPrefixedArray<T>*>(memory);
    ret->length_ = length;
    return ret;
  }
