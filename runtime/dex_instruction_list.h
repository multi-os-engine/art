...
#define DEX_INSTRUCTION_LIST(V) \
  ...
  /*  XX: Not 100% sure about these verify flags, may need to change them.  */ \
  V(0xF3, INVOKE_LAMBDA, "invoke-lambda", k25x, false, kNone, kContinue | kThrow | kInvoke, kVerifyRegC) \
  ...
  /*  XX: Not 100% sure about these verify flags, may need to change them.  */ \
  /*  XX: true/false column unused? No compiler errors when putting in garbage.  */ \
  V(0xF6, CREATE_LAMBDA, "create-lambda", k21c, false_BLARGGHH, kMethodRef, kContinue | kThrow, kVerifyRegA | kVerifyRegBMethod) \
