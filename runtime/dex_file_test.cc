...
TEST_F(DexFileTest, Open) {
  ...
  DexFileUniquePtr dex(OpenTestDexFile("Nested"));
...
static DexFileUniquePtr OpenDexFileBase64(const char* base64,
                                          const char* location) {
  // decode base64
  CHECK(base64 != NULL);
  size_t length;
...
TEST_F(DexFileTest, GetLocationChecksum) {
  ScopedObjectAccess soa(Thread::Current());
  DexFileUniquePtr raw(OpenTestDexFile("Main"));
...
TEST_F(DexFileTest, ClassDefs) {
  ScopedObjectAccess soa(Thread::Current());
  DexFileUniquePtr raw(OpenTestDexFile("Nested"));
...
TEST_F(DexFileTest, GetMethodSignature) {
  ScopedObjectAccess soa(Thread::Current());
  DexFileUniquePtr raw(OpenTestDexFile("GetMethodSignature"));
  ASSERT_TRUE(raw.get() != nullptr);
