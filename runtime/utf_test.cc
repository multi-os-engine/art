...
TEST_F(UtfTest, CountAndConvertUtf8Bytes_UnpairedSurrogate) {
  ...
  const std::map<std::vector<uint16_t>, std::vector<uint8_t>> prefixes{  // NOLINT(whitespace/braces)
      {{ 'h' }, { 'h' }},
      {{ 0 }, { 0xc0, 0x80 }},
      {{ 0x81 }, { 0xc2, 0x81 }},
      {{ 0x801 }, { 0xe0, 0xa0, 0x81 }},
  };
  const std::map<std::vector<uint16_t>, std::vector<uint8_t>> suffixes{  // NOLINT(whitespace/braces)
      {{ 'e' }, { 'e' }},
      {{ 0 }, { 0xc0, 0x80 }},
      {{ 0x7ff }, { 0xdf, 0xbf }},
      {{ 0xffff }, { 0xef, 0xbf, 0xbf }},
  };
  const std::map<std::vector<uint16_t>, std::vector<uint8_t>> tests{  // NOLINT(whitespace/braces)
