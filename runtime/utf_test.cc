...
TEST_F(UtfTest, CountAndConvertUtf8Bytes) {
  ...
  AssertConversion({
      ...
      0x010e, 0x0000,  // Two byte encodings
...
TEST_F(UtfTest, CountAndConvertUtf8Bytes_UnpairedSurrogate) {
  AssertConversion({ 'h', 'e', 0xd801 }, { 'h', 'e' });
  // 'e' will not be emmitted in the output.
  AssertConversion({ 'h', 0xd801, 'e' }, { 'h' });
  AssertConversion({ 'h', 0xd801, 0xd801, 'e' }, { 'h', 'e' });
}
