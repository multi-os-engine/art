...
OatFileAssistant::OatFileAssistant(const char* dex_location,
  ...
  allow_oat_file_update_ = (oat_location != nullptr) ||
      Runtime::Current()->GetForceOatUpdateAtLoadTime();
...
bool OatFileAssistant::MakeUpToDate(std::string* error_msg) {
  DCHECK(allow_oat_file_update_)
      << "Requested MakeUpToDate without providing an explicit oat location.";
