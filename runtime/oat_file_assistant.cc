...
bool OatFileAssistant::GenerateOatFile(std::string* error_msg) {
  ...
  std::string dex_location(dex_location_);
  Runtime* runtime = Runtime::Current();
  if (!runtime->IsDex2OatEnabled()) {
    *error_msg = "Generation of oat file for dex location " + dex_location
      + " not attempted because dex2oat is disabled.";
    return false;
  }
  ...
  if (!OS::FileExists(dex_location_)) {
    *error_msg = "Dex location " + dex_location + " does not exists.";
    return false;
  }
