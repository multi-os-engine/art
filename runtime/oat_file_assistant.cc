...
OatFileAssistant::OatFileAssistant(const char* dex_location,
    ...
    : OatFileAssistant(dex_location, nullptr, isa, nullptr, false) { }

OatFileAssistant::OatFileAssistant(const char* dex_location,
                                   const char* oat_location,
                                   const InstructionSet isa)
    : OatFileAssistant(dex_location, oat_location, isa, nullptr, false) { }

OatFileAssistant::OatFileAssistant(const char* dex_location,
                                   const InstructionSet isa,
                                   const char* package_name)
    : OatFileAssistant(dex_location, nullptr, isa, package_name, false) { }
...
OatFileAssistant::OatFileAssistant(const char* dex_location,
  ...
  CHECK(!for_execution || isa == kRuntimeISA)
    << "OatFileAssistant for_execution with wrong ISA";
...
const std::string* OatFileAssistant::OdexFileName() {
  if (!cached_odex_file_name_attempted_) {
    ...
    if (!cached_odex_file_name_found_) {
      ...
      LOG(WARNING) << "Failed to determine odex file name for "
        << " dex location: " << dex_location_ << ": " << error_msg;
    }
  }
...
bool OatFileAssistant::OdexFileNeedsRelocation() {
  return !OdexFileIsOutOfDate() && !OdexFileIsUpToDate();
}
...
bool OatFileAssistant::OatFileNeedsRelocation() {
  return !OatFileIsOutOfDate() && !OatFileIsUpToDate();
}
...
bool OatFileAssistant::GivenOatFileNeedsRelocation(const OatFile& file) {
  ...
  // is more work than we need to do. If performance becomes a concern, and
  // this method is actually called, this should be fixed.
  return !GivenOatFileIsOutOfDate(file) && !GivenOatFileIsUpToDate(file);
}
...
bool OatFileAssistant::DexFilenameToOdexFilename(const std::string& location,
  ...
  if (pos == std::string::npos) {
    *error_msg = "Dex location has no directory.";
    return false;
  }
  ...
  pos = file.rfind('.');
  if (pos == std::string::npos) {
    *error_msg = "Dex location has no extension.";
    return false;
  }
...
ExecutableOatFileAssistant::ExecutableOatFileAssistant(const char* dex_location)
  : OatFileAssistant(dex_location, nullptr, kRuntimeISA, nullptr, true) { }

ExecutableOatFileAssistant::ExecutableOatFileAssistant(const char* dex_location,
                                                   const char* oat_location)
  : OatFileAssistant(dex_location, oat_location, kRuntimeISA, nullptr, true) { }

std::unique_ptr<OatFile> ExecutableOatFileAssistant::GetBestOatFile() {
  if (OatFileIsUpToDate()) {
...
std::vector<std::unique_ptr<const DexFile>> ExecutableOatFileAssistant::LoadDexFiles(const OatFile& oat_file, const char* dex_location) {
