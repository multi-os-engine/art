...
class Arm64InstructionSetFeatures FINAL : public InstructionSetFeatures {
 public:
  ...
  static std::unique_ptr<const Arm64InstructionSetFeatures> FromVariant(const std::string& variant,
                                                                        std::string* error_msg);

  // Parse a bitmap and create an InstructionSetFeatures.
  static std::unique_ptr<const Arm64InstructionSetFeatures> FromBitmap(uint32_t bitmap);

  // Turn C pre-processor #defines into the equivalent instruction set features.
  static std::unique_ptr<const Arm64InstructionSetFeatures> FromCppDefines();

  // Process /proc/cpuinfo and use kRuntimeISA to produce InstructionSetFeatures.
  static std::unique_ptr<const Arm64InstructionSetFeatures> FromCpuInfo();

  // Process the auxiliary vector AT_HWCAP entry and use kRuntimeISA to produce
  // InstructionSetFeatures.
  static std::unique_ptr<const Arm64InstructionSetFeatures> FromHwcap();

  // Use assembly tests of the current runtime (ie kRuntimeISA) to determine the
  // InstructionSetFeatures. This works around kernel bugs in AT_HWCAP and /proc/cpuinfo.
  static std::unique_ptr<const Arm64InstructionSetFeatures> FromAssembly();
