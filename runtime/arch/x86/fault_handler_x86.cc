...
bool SuspensionHandler::Action(int sig, siginfo_t* info, void* context) {
  ...
  uint8_t* ptr = pc - 2 - sizeof(checkinst1);
  bool found = false;
  while (ptr > limit) {
...
bool StackOverflowHandler::Action(int sig, siginfo_t* info, void* context) {
  ...
  // Push the return address onto the stack, replacing the current return address
  // on the stack.
  uint32_t retaddr = reinterpret_cast<uint32_t>(pc + instr_size);
  uint32_t* curr_sp = reinterpret_cast<uint32_t*>(prevsp);
  *curr_sp = retaddr;
  // ESP remains the same.

  // Tell the stack overflow code where the new stack pointer should be.
  uc->uc_mcontext.gregs[REG_EAX] = sp;
