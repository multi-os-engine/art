...
void Thread::InitCpu() {
  ...
  //
  // We also share this between all active runtimes. This is both for simplicity (one well-known
  // slot) as well as to avoid the three-slot limitation. Downside is that we cannot free the slot
  // when it is known that a runtime ends.
