...
#ifndef __APPLE__
// On Linux, we use a GDT entry to store our segment descriptor data. This entry must be shared,
// as the kernel only guarantees three TLS entries. For simplicity (and locality), use this local
// global, which practically becomes readonly after the first (startup) thread of the runtime has
// been initialized (during Runtime::Start()).
//
// We also share this between all active runtimes. This is both for simplicity (one well-known slot)
// as well as to avoid the three-slot limitation. Downside is that we cannot free the slot when it
// is known that a runtime ends.
...
void Thread::InitCpu() {
  ...
  int rc = syscall(SYS_set_thread_area, &gdt_entry);
  if (rc != -1) {
...
void Thread::CleanupCpu() {
  ...
  // syscall(SYS_set_thread_area, &gdt_entry);
  UNUSED(selector);
#endif
}
