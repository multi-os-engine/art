...
extern "C" void art_quick_foreign_code_call_back(void *pc, void *sp, uint32_t frame_size,
                                                 intptr_t* regs, void *callback);
...
class MyRunner : public MY_PARENT_CLASS {
 private:
  ...
  ArtMethod* method_;    // Method which is currently being executed.
  uint32_t frame_size_;  // Size of the frame for the A64 method.
  void* sp_at_entry_;    // Stack pointer at entry of A64 method.

 public:
  MyRunner(Decoder* decoder, Thread *self, intptr_t *x86_regs, uintptr_t* sp_at_entry,
           uint32_t frame_size, FILE* stream = stdout) : MY_PARENT_CLASS(decoder, stream) {
    saved_br_reg_ = -1;
    saved_br_lr_ = INT64_C(-1);
    self_ = self;
    x86_regs_ = x86_regs;
    frame_size_ = frame_size;
    method_ = reinterpret_cast<ArtMethod*>(x86_regs_[0]);  // Get method pointer from %eax.
    sp_at_entry_ = sp_at_entry;                            // Set the stack pointer.
  }

  void GetArm64ArgsFromX86Regs() {
    // TODO(Arm64): implement this properly!
 ...
 void GetArm64RetValFromX86Regs() {
   GetArm64ArgsFromX86Regs();
 }

  void PutArm64RetValIntoX86Regs(ArtMethod* method)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    char return_shorty = MethodHelper(method).GetShorty()[0];
    if (return_shorty == 'J') {
      // Long return value. Place x0 in %eax, %ecx.
      uint64_t retval = xreg(0);
      x86_regs_[0] = static_cast<uint32_t>(retval);
      x86_regs_[1] = static_cast<uint32_t>(retval >> 32);
    } else {
      // All other cases: place w0 into %eax.
      x86_regs_[0] = wreg(0);
    }
  }

  void PutArm64ArgsIntoX86Regs() {
    for (unsigned int i = 0; i < arraysize(a64_from_x86_); i++) {
      if (a64_from_x86_[i] != 31) {
  ...
  void Run() {
    while (true) {
      ...
      PutArm64ArgsIntoX86Regs();

      art_quick_foreign_code_call_back(reinterpret_cast<void*>(return_pc),
                                       sp_at_entry_, frame_size_, x86_regs_, fn);
      GetArm64RetValFromX86Regs();
  ...
  void RunFrom(Instruction* first)
    ...
    set_wreg(31, (uintptr_t)sp_at_entry_, Reg31IsStackPointer);
    GetArm64ArgsFromX86Regs();
    set_pc(first);
    Run();
    PutArm64RetValIntoX86Regs(method_);
  }
};
...
artArm64CodeCall(void* a64Code, intptr_t* x86_regs, uintptr_t* sp_at_entry)
  ...
  Decoder decoder;
  MyRunner runner(&decoder, Thread::Current(), x86_regs, sp_at_entry, frame_size);
  runner.RunFrom(reinterpret_cast<Instruction*>(a64Code));
}
