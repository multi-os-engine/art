...
void FaultManager::GetMethodAndReturnPCAndSP(siginfo_t* siginfo, void* context,
  ...
  *out_sp = static_cast<uintptr_t>(uc->uc_mcontext.gregs[REG_RSP]);
  ...
  // get the method from the top of the stack.  However it's in RAX.
  ...
  if (overflow_addr == fault_addr) {
    *out_method = reinterpret_cast<mirror::ArtMethod*>(uc->uc_mcontext.gregs[REG_RDI]);
  } else {
    // The method is at the top of the stack.
    *out_method = 
        reinterpret_cast<StackReference<mirror::ArtMethod>*>(*out_sp)->AsMirrorPtr();

  }

  uint8_t* pc = reinterpret_cast<uint8_t*>(uc->uc_mcontext.gregs[REG_RIP]);
...
bool NullPointerHandler::Action(int sig, siginfo_t* info, void* context) {
  struct ucontext *uc = reinterpret_cast<struct ucontext*>(context);
  uint8_t* pc = reinterpret_cast<uint8_t*>(uc->uc_mcontext.gregs[REG_RIP]);
  uint8_t* sp = reinterpret_cast<uint8_t*>(uc->uc_mcontext.gregs[REG_RSP]);
  ...
  uint64_t retaddr = reinterpret_cast<uint64_t>(pc + instr_size);
  uint64_t* next_sp = reinterpret_cast<uint64_t*>(sp - 8);
  *next_sp = retaddr;
  uc->uc_mcontext.gregs[REG_RSP] = reinterpret_cast<uint64_t>(next_sp);
...
bool SuspensionHandler::Action(int sig, siginfo_t* info, void* context) {
  ...
  if (found) {
    ...
    // Push the return address onto the stack.
    uint64_t retaddr = reinterpret_cast<uint64_t>(pc + 2);
    uint64_t* next_sp = reinterpret_cast<uint64_t*>(sp - 8);
    *next_sp = retaddr;
    uc->uc_mcontext.gregs[REG_RSP] = reinterpret_cast<uint64_t>(next_sp);

    uc->uc_mcontext.gregs[REG_RIP] = reinterpret_cast<uintptr_t>(art_quick_test_suspend);
...
// address for the previous method is on the stack at RSP.
...
bool StackOverflowHandler::Action(int sig, siginfo_t* info, void* context) {
  ...
  uc->uc_mcontext.gregs[REG_RAX] = pregion;

  // Now arrange for the signal handler to return to art_quick_throw_stack_overflow_from_signal.
  uc->uc_mcontext.gregs[REG_RIP] = reinterpret_cast<uintptr_t>(
