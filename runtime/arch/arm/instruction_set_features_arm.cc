...
std::unique_ptr<const ArmInstructionSetFeatures> ArmInstructionSetFeatures::FromVariant(
  ...
  if (has_div == false && has_lpae == false) {
    ...
    if (FindVariantInArray(unsupported_arm_variants, arraysize(unsupported_arm_variants),
      ...
      *error_msg = StringPrintf("Attempt to use unsupported ARM variant: %s", variant.c_str());
      return std::unique_ptr<const ArmInstructionSetFeatures>();
    }
  ...
  return std::unique_ptr<const ArmInstructionSetFeatures>(
      new ArmInstructionSetFeatures(smp, has_div, has_lpae));
}

std::unique_ptr<const ArmInstructionSetFeatures> ArmInstructionSetFeatures::FromBitmap(uint32_t bitmap) {
...
std::unique_ptr<const ArmInstructionSetFeatures> ArmInstructionSetFeatures::FromCppDefines() {
  const bool smp = true;
...
std::unique_ptr<const ArmInstructionSetFeatures> ArmInstructionSetFeatures::FromCpuInfo() {
...
std::unique_ptr<const ArmInstructionSetFeatures> ArmInstructionSetFeatures::FromHwcap() {
...
std::unique_ptr<const ArmInstructionSetFeatures> ArmInstructionSetFeatures::FromAssembly() {
  const bool smp = true;
