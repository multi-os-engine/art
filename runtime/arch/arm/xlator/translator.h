...
class ARMTranslator : public Translator {
 ...
 private:
  ...
  inline bool IsVectorLoadStore(uint32_t inst) {
     uint32_t hi = inst >> 24;
     if (hi != 0xed) {
         return false;
     }
     uint32_t lo = (inst >> 8) & 0xf;
     return lo == 0xa || lo == 0xb;
  }

  inline bool IsStrHalf(uint32_t inst) {
    if (((inst >> 25) & 0b111) == 0 && ((inst >> 22) & 1) == 1 &&
      ((inst >> 20) & 1) == 0 && ((inst >> 4) & 0b1111) == 0b1011) {
        return true;
    }
    return false;
  }
