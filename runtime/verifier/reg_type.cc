...
//  if (unresolved_types_.GetSizeOf() > 16U) {
//    // Random magic number, 16 = 16B. The assumption here is that a regular set is at least as
//    // expensive, even if it only stores a small amount of entries. An alternative we may choose
//    // is a vector as backing storage, but then union-ing will be more expensive.
//    LOG(WARNING) << "UnresolvedMergedType with large unresolved part: " << Dump();
//  }
}
