...
class VerifierMetadataTest : public CommonRuntimeTest {
 protected:
  ...
  bool HasUnresolvedFields(const std::vector<std::string>& expected_fields ATTRIBUTE_UNUSED) {
    return false;
  }
...
TEST_F(VerifierMetadataTest, StaticField_UnresolvedField) {
  ...
  ASSERT_TRUE(HasUnresolvedFields({ "Ljava/util/TimeZone;->foo:I" }));
}
...
TEST_F(VerifierMetadataTest, InvokeStatic_Resolved_DeclaredInSuperclass1) {
  ...
  ASSERT_TRUE(HasExtendsRelations({ "Ljavax/net/ssl/SSLSocket; extends Ljava/net/Socket;" }));
  ASSERT_TRUE(HasResolvedMethods({
      "Ljava/net/Socket;->setSocketImplFactory(Ljava/net/SocketImplFactory;)V" }));
}
