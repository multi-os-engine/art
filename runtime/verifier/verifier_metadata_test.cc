...
TEST_F(VerifierMetadataTest, InstanceField) {
  ScopedObjectAccess soa(Thread::Current());
  ASSERT_TRUE(VerifyMethod("InstanceField_Resolved", &soa, &code_, &metadata_));
  ASSERT_TRUE(HasResolvedClasses({ "Ljava/io/InvalidClassException;", "Ljava/lang/String;" }));
}

TEST_F(VerifierMetadataTest, InstanceField_Unesolved) {
  ScopedObjectAccess soa(Thread::Current());
  ASSERT_TRUE(VerifyMethod("InstanceField_Unesolved", &soa, &code_, &metadata_));
  ASSERT_TRUE(HasResolvedClasses({ "Ljava/lang/String;" }));
  ASSERT_TRUE(HasUnresolvedClasses({ "LUnresolvedClass;", "LUnresolvedSubclass;" }));
}

TEST_F(VerifierMetadataTest, StaticField_Resolved_DefinedInReferrer) {
  ScopedObjectAccess soa(Thread::Current());
  ASSERT_TRUE(VerifyMethod("StaticField_Resolved_DefinedInReferrer", &soa, &code_, &metadata_));
...
TEST_F(VerifierMetadataTest, StaticField_Resolved_DefinedInSuperclass1) {
  ScopedObjectAccess soa(Thread::Current());
  ASSERT_TRUE(VerifyMethod("StaticField_Resolved_DefinedInSuperclass1", &soa, &code_, &metadata_));
...
TEST_F(VerifierMetadataTest, StaticField_Resolved_DefinedInSuperclass2) {
  ScopedObjectAccess soa(Thread::Current());
  ASSERT_TRUE(VerifyMethod("StaticField_Resolved_DefinedInSuperclass2", &soa, &code_, &metadata_));
  ASSERT_TRUE(HasResolvedClasses({ "Ljava/util/SimpleTimeZone;", "Ljava/util/TimeZone;" }));
  ASSERT_TRUE(HasExtendsRelations({ "Ljava/util/SimpleTimeZone; extends Ljava/util/TimeZone;" }));
  ASSERT_TRUE(HasResolvedFields({ "Ljava/util/TimeZone;->SHORT:I" }));
}

TEST_F(VerifierMetadataTest, StaticField_Resolved_DefinedInInterface1) {
  ScopedObjectAccess soa(Thread::Current());
  ASSERT_TRUE(VerifyMethod("StaticField_Resolved_DefinedInInterface1", &soa, &code_, &metadata_));
...
TEST_F(VerifierMetadataTest, StaticField_Resolved_DefinedInInterface2) {
  ScopedObjectAccess soa(Thread::Current());
  ASSERT_TRUE(VerifyMethod("StaticField_Resolved_DefinedInInterface2", &soa, &code_, &metadata_));
  ASSERT_TRUE(HasResolvedClasses({
      "Ljava/lang/String;",
      "Ljavax/xml/transform/Result;",
      "Ljavax/xml/transform/dom/DOMResult;" }));
...
TEST_F(VerifierMetadataTest, StaticField_Resolved_DefinedInInterface3) {
  ScopedObjectAccess soa(Thread::Current());
  ASSERT_TRUE(VerifyMethod("StaticField_Resolved_DefinedInInterface3", &soa, &code_, &metadata_));
  ASSERT_TRUE(HasResolvedClasses({ "Ljava/lang/String;", "Ljavax/xml/transform/Result;" }));
  ASSERT_TRUE(HasResolvedFields({
      "Ljavax/xml/transform/Result;->PI_ENABLE_OUTPUT_ESCAPING:Ljava/lang/String;" }));
}

TEST_F(VerifierMetadataTest, StaticField_Resolved_DefinedInInterface4) {
  ScopedObjectAccess soa(Thread::Current());
  ASSERT_TRUE(VerifyMethod("StaticField_Resolved_DefinedInInterface4", &soa, &code_, &metadata_));
...
TEST_F(VerifierMetadataTest, StaticField_Unresolved) {
  ScopedObjectAccess soa(Thread::Current());
  ASSERT_TRUE(VerifyMethod("StaticField_Unresolved", &soa, &code_, &metadata_));
  ASSERT_TRUE(HasUnresolvedClasses({ "LUnresolvedClass;" }));
  // TODO: unresolved field
}

TEST_F(VerifierMetadataTest, InvokeVirtual) {
  ScopedObjectAccess soa(Thread::Current());

  ASSERT_TRUE(VerifyMethod("Opcode_INVOKE_VIRTUAL_Resolved", &soa, &code_, &metadata_));
  ASSERT_TRUE(HasResolvedClasses({
  ...
  ASSERT_TRUE(VerifyMethod("Opcode_INVOKE_VIRTUAL_Unresolved", &soa, &code_, &metadata_));
  ASSERT_TRUE(HasResolvedClasses({ "Ljava/lang/String;" }));
...
TEST_F(VerifierMetadataTest, MoveException) {
  ScopedObjectAccess soa(Thread::Current());

  ASSERT_TRUE(VerifyMethod("Opcode_MOVE_EXCEPTION_Resolved", &soa, &code_, &metadata_));
  ASSERT_TRUE(HasResolvedClasses({
      "Ljava/lang/ArithmeticException;",
      "Ljava/lang/Throwable;" }));

  ASSERT_TRUE(VerifyMethod("Opcode_MOVE_EXCEPTION_Unresolved", &soa, &code_, &metadata_));
