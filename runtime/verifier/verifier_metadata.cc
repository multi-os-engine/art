...
inline static mirror::Class* GetFrontier(mirror::Class* klass,
                                         /* out */ std::set<mirror::Class*>* interfaces = nullptr)
    SHARED_REQUIRES(Locks::mutator_lock_) {
      ...
      if (num_direct_ifaces > 0u) {
        ...
        for (uint32_t i = 0u; i < num_direct_ifaces; ++i) {
          ...
          if (IsInBootClassPath(direct_iface)) {
            // Direct interface in boot classpath. Add it to `interfaces` and
            // do not scan further.
            interfaces->insert(direct_iface);
          } else {
            ...
            GetFrontier(direct_iface, interfaces);
          }
        }
      }
    }
...
void VerifierMetadata::AddAssignable(mirror::Class* destination,
  ...
  if (destination != source) {
    ...
    std::string temp;
    assignables_.emplace(
        AssignabilityTestTuple(destination->GetDescriptor(&temp),
...
void VerifierMetadata::RecordAssignabilityTest(mirror::Class* destination,
  ...
  std::set<mirror::Class*> ifaces;
  source = GetFrontier(source, &ifaces);
  ...
  if (ifaces.empty()) {
    ...
    AddAssignable(destination, source, /* is_assignable */ false);
    for (mirror::Class* iface : ifaces) {
      AddAssignable(destination, iface, /* is_assignable */ false);
    }
  } else {
    ...
    current_assignable = destination->IsAssignableFrom(source);
    found_assignable |= current_assignable;
    AddAssignable(destination, source, current_assignable);

    for (mirror::Class* iface : ifaces) {
      current_assignable = destination->IsAssignableFrom(iface);
      found_assignable |= current_assignable;
      AddAssignable(destination, iface, current_assignable);
    }
...
void VerifierMetadata::RecordFieldResolution(uint32_t dex_field_idx,
  ...
  if (IsInBootClassPath(klass_ref)) {
    ...
    std::set<mirror::Class*> ifaces;
    mirror::Class* superklass_ref = GetFrontier(klass_ref, &ifaces);

    if (field != nullptr && AreEqualOrParentAndChild(klass_decl, superklass_ref)) {
      AddField(superklass_ref, dex_field_idx, is_static, field);
    } else {
      AddField(superklass_ref, dex_field_idx, is_static, nullptr);
    }

    for (mirror::Class* iface : ifaces) {
      if (field != nullptr && AreEqualOrParentAndChild(klass_decl, iface)) {
        AddField(iface, dex_field_idx, is_static, field);
      } else {
        AddField(iface, dex_field_idx, is_static, nullptr);
      }
    }
  } else {
    ...
    AddField(GetFrontier(klass_ref), dex_field_idx, is_static, field);
  }
}
...
void VerifierMetadata::RecordMethodResolution(uint32_t dex_method_idx,
  ...
  if (IsInBootClassPath(klass_ref)) {
  ...
  } else if (resolution_type == kInterfaceMethodResolution) {
    // Interface resolution scans the referring class (not in boot classpath)
    // and all implemented interfaces. Note that `klass_ref` does not generally
    // need to be an interface for resolution to succeed, but
    DCHECK(klass_decl->IsInterface());
    ...
    DCHECK(method == nullptr || klass_ref->IsSubClass(klass_decl));
    AddMethod(GetFrontier(klass_ref), dex_method_idx, resolution_type, method);
  }
}
