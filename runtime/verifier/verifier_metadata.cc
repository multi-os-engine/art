...
VerifierMetadata::UnresolvedField::UnresolvedField(mirror::Class* klass_ref,
                                                   const std::string& name,
                                                   const std::string& type)
    : VerifierMetadata::UnresolvedFieldBase(klass_ref, name, type) {}
    ...
    if (child->IsSubClass(parent)) {
      ...
      while (!IsInBootClassPath(child)) {
        ...
        if (super != nullptr && super->Implements(parent)) {
          ...
          // THIS MAY BE BROKEN!
          // Do we need to go up all of them? Should we first look at direct
          // interfaces and then at superclasses?
...
void VerifierMetadata::AddUnresolvedField(mirror::Class* klass,
                                          const std::string& name,
                                          const std::string& type) {
  DCHECK(klass != nullptr && klass->IsResolved());
  ...
  AddResolvedClass(klass);
  unresolved_fields_.insert(UnresolvedField(klass, name, type));
}
  ...
  for (mirror::Class* klass : resolved_classes_) {
    ...
    for (ArtField* field : resolved_fields_) {
      if (field->GetDeclaringClass() == klass) {
        result << "  resolved field " << field->GetName() << " {\n";
        result << "    type = " << field->GetTypeDescriptor() << "\n";
        result << "    access_flags = " << PrettyJavaAccessFlags(field->GetAccessFlags()) << "\n";
        result << "  }\n";
      }
    }
    for (auto&& field : unresolved_fields_) {
      if (field.GetDeclaringClass() == klass) {
        result << "  unresolved field " << field.GetName() << " {\n";
