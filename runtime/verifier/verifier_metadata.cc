...
inline static uint32_t GetAccessFlags(T* element) SHARED_REQUIRES(Locks::mutator_lock_) {
  ...
  DCHECK_NE(access_flags, VerifierMetadata::kUnresolvedAccessFlags);
  return access_flags;
}
...
void VerifierMetadata::RecordClassResolution(uint16_t dex_type_idx, mirror::Class* klass) {
  ...
  uint32_t access_flags = kUnresolvedAccessFlags;
  if (klass != nullptr) {
...
void VerifierMetadata::RecordFieldResolution(uint32_t dex_field_idx, ArtField* field) {
  ...
  uint32_t access_flags = kUnresolvedAccessFlags;
  std::string declaring_klass = kUnresolvedDeclaringClass;
  if (field != nullptr) {
    std::string tmp;
    access_flags = GetAccessFlags(field);
    declaring_klass = field->GetDeclaringClass()->GetDescriptor(&tmp);
  }
...
void VerifierMetadata::RecordMethodResolution(uint32_t dex_method_idx,
  ...
  if (method != nullptr) {
    ...
    declaring_klass = method->GetDeclaringClass()->GetDescriptor(&tmp);
  }

  {
    MutexLock mu(Thread::Current(), lock_);
    methods_.emplace(MethodResolutionTuple(dex_method_idx,
                                           resolution_type,
                                           access_flags,
                                           declaring_klass));
  }
...
bool VerifierMetadata::Verify(Handle<mirror::ClassLoader> class_loader,
  ...
  for (auto&& entry : GetClasses()) {
    ...
    if (entry.IsResolved()) {
      if (klass == nullptr || entry.GetAccessFlags() != klass->GetAccessFlags()) {
        return false;
      }
    } else {
      if (klass != nullptr) {
  ...
  for (auto&& entry : GetFields()) {
    ...
    if (entry.IsResolved()) {
      if (field == nullptr ||
          entry.GetAccessFlags() != field->GetAccessFlags() ||
          entry.GetDeclaringClass() != field->GetDeclaringClass()->GetDescriptor(&tmp)) {
        return false;
      }
    } else {
      if (field != nullptr) {
  ...
  for (auto&& entry : GetMethods()) {
    ...
    if (entry.IsResolved()) {
      if (method == nullptr ||
          entry.GetAccessFlags() != method->GetAccessFlags() ||
          entry.GetDeclaringClass() != method->GetDeclaringClass()->GetDescriptor(&tmp)) {
        return false;
      }
    } else {
      if (method != nullptr) {
