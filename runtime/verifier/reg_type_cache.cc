...
const RegType& RegTypeCache::FromUnresolvedMerge(const RegType& left, const RegType& right) {
  ...
  for (size_t i = primitive_count_; i < entries_.size(); i++) {
    ...
    if (cur_entry->IsUnresolvedMergedReference()) {
      ...
      // Currently a bit-vector is copied (we don't have move constructors), and we set the vector
      // to no longer be allocatable, when a new UnresolvedMergedType is created. Thus the vectors
      // wont be "Equals." SameBitsSet isn't that much more expensive, anyways.
