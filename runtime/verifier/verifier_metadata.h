...
class VerifierMetadata {
 public:
  ...
  struct ClassResolutionTuple : public std::tuple<uint32_t, uint32_t> {
    ...
    bool IsResolved() const { return GetAccessFlags() != kUnresolvedAccessFlags; }
  };

  struct FieldResolutionTuple : public std::tuple<uint32_t, uint32_t, std::string> {
    using Base = std::tuple<uint32_t, uint32_t, std::string>;
    using Base::Base;
    ...
    bool IsResolved() const { return GetAccessFlags() != kUnresolvedAccessFlags; }
    uint32_t GetAccessFlags() const { return std::get<1>(*this); }
    const std::string& GetDeclaringClass() const { return std::get<2>(*this); }
  };

  struct MethodResolutionTuple
      : public std::tuple<uint32_t, MethodResolutionType, uint32_t, std::string> {
    using Base = std::tuple<uint32_t, MethodResolutionType, uint32_t, std::string>;
    using Base::Base;
    ...
    bool IsResolved() const { return GetAccessFlags() != kUnresolvedAccessFlags; }
    uint32_t GetAccessFlags() const { return std::get<2>(*this); }
    const std::string& GetDeclaringClass() const { return std::get<3>(*this); }
  };
  ...
  static constexpr uint32_t kUnresolvedAccessFlags = static_cast<uint32_t>(-1);
  static constexpr const char* kUnresolvedDeclaringClass = "";
