...
bool MethodVerifier::Verify() {
  ...
  if (code_item_ == nullptr) {
    // Only native or abstract methods may not have coce.
    ...
    if ((method_access_flags_ & kAccAbstract) != 0) {
      ...
      if ((method_access_flags_ & kForbidden) != 0) {
        Fail(VERIFY_ERROR_BAD_CLASS_HARD)
            << "method can't be abstract and private/static/final/native/strict/synchronizednative";
        return false;
      }
    }
    if ((class_def_->GetJavaAccessFlags() & kAccInterface) != 0) {
      ...
      // In addition to the above, interface methods must not be protected or synchronized.
      static constexpr uint32_t kForbidden = kAccProtected | kAccSynchronized;
      if ((method_access_flags_ & kForbidden) != 0) {
        Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "interface methods can't be protected or synchronized";
        return false;
      }
    }
