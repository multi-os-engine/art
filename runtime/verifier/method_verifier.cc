...
mirror::ArtMethod* MethodVerifier::VerifyInvocationArgs(const Instruction* inst,
  ...
  if (is_super) {
    ...
    if (res_method->GetMethodIndex() >= super_klass->GetEmbeddedVTableLength()) {
...
mirror::ArtMethod* MethodVerifier::GetQuickInvokedMethod(const Instruction* inst,
  ...
  CHECK(dispatch_class->ShouldHaveEmbeddedImtAndVTable()) << PrettyDescriptor(dispatch_class);
  uint16_t vtable_index = is_range ? inst->VRegB_3rc() : inst->VRegB_35c();
  CHECK_LT(static_cast<int32_t>(vtable_index), dispatch_class->GetEmbeddedVTableLength())
      << PrettyDescriptor(klass);
  mirror::ArtMethod* res_method = dispatch_class->GetEmbeddedVTableEntry(vtable_index);
