...
bool MethodVerifier::Verify() {
  ...
  insn_flags_ = arena_.AllocArray<InstructionFlags>(code_item_->insns_size_in_code_units_);
  DCHECK(insn_flags_ != nullptr);
  std::uninitialized_fill_n(insn_flags_, code_item_->insns_size_in_code_units_, InstructionFlags());
...
bool MethodVerifier::CodeFlowVerifyInstruction(uint32_t* start_guess) {
  ...
  // branch target.
  DestroyOnlyUniquePtr<RegisterLine> branch_line;
  DestroyOnlyUniquePtr<RegisterLine> fallthrough_line;
...
bool MethodVerifier::UpdateRegisters(uint32_t next_insn, RegisterLine* merge_line,
  ...
  if (!insn_flags_[next_insn].IsVisitedOrChanged()) {
    ...
    DestroyOnlyUniquePtr<RegisterLine> copy;
    if (gDebugVerify) {
