...
ArtMethod* MethodVerifier::VerifyInvocationArgs(
  ...
  if (method_type == METHOD_SUPER) {
    ...
    if (reference_class->IsInterface()) {
      ...
      // does.
      // TODO We might want to be more intelligent if we don't have a good class.
      if (!GetDeclaringClass().HasClass()) {
        Fail(VERIFY_ERROR_BAD_CLASS_SOFT) << "Unable to resolve the full class of 'this' used in an"
