...
mirror::ArtMethod* MethodVerifier::GetQuickInvokedMethod(const Instruction* inst,
  ...
  CHECK(inst->Opcode() == Instruction::INVOKE_VIRTUAL_QUICK ||
  ...
  /*
  CHECK_NE(res_method->GetInvokeType(), kInterface) <<
      PrettyClass(klass) << " " << PrettyClass(dispatch_class) << " " << PrettyMethod(res_method);
      */
  return res_method;
}
...
mirror::ArtField* MethodVerifier::GetQuickFieldAccess(const Instruction* inst,
  ...
  mirror::ArtField* f = mirror::ArtField::FindInstanceFieldWithOffset(object_type.GetClass(),
                                                                      field_offset);
  CHECK_EQ(f->GetOffset().Uint32Value(), field_offset);
  if (f == nullptr) {
