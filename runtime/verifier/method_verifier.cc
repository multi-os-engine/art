...
bool MethodVerifier::CodeFlowVerifyInstruction(uint32_t* start_guess) {
  ...
  if (IsConstructor()) {
    size_t loc = work_line_->GetUninitializedThisLoc(this);
    if (loc != kInvalidVreg) {
      track_uninitialized_this = true;
      uninitialized_this_loc = loc;
    }
  }
  ...
  if ((opcode_flags & Instruction::kContinue) != 0) {
    ...
    if (insn_flags_[next_insn_idx].IsReturn()) {
      ...
      if ((opcode == Instruction::RETURN_VOID) || (opcode == Instruction::RETURN_VOID_BARRIER)) {
        SafelyMarkAllRegistersAsConflicts(work_line_.get());
      } else {
...
bool MethodVerifier::UpdateRegisters(uint32_t next_insn, RegisterLine* merge_line,
  ...
  if (!insn_flags_[next_insn].IsVisitedOrChanged()) {
    ...
    if (!insn_flags_[next_insn].IsReturn()) {
      ...
      Instruction::Code opcode = ret_inst->Opcode();
      if ((opcode == Instruction::RETURN_VOID) || (opcode == Instruction::RETURN_VOID_BARRIER)) {
        SafelyMarkAllRegistersAsConflicts(target_line);
      } else {
        target_line->CopyFromLine(merge_line);
        if (opcode == Instruction::RETURN_WIDE) {
...
void MethodVerifier::SafelyMarkAllRegistersAsConflicts(RegisterLine* reg_line) {
  if (IsConstructor()) {
    // Before we mark all regs as conflicts, check that we don't have an uninitialized this.
    reg_line->CheckConstructorReturn(this);
  }
  reg_line->MarkAllRegistersAsConflicts(this);
}
