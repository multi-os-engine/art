...
ArtMethod* MethodVerifier::ResolveMethodAndCheckAccess(
  ...
  if (res_method == nullptr) {
    ...
    } else if (method_type == METHOD_INTERFACE || (is_super && klass->IsInterface())) {
  ...
  if (klass->IsInterface() && !is_super && method_type != METHOD_INTERFACE) {
...
ArtMethod* MethodVerifier::VerifyInvocationArgs(
  ...
  if (is_super) {
    ...
    if (res_method->GetDeclaringClass()->IsInterface()) {
      ...
      Fail(VERIFY_ERROR_FORCE_INTERPRETER) << "Currently we only allow invoke-super in interpreter "
