  ...
  for ( ; it->HasNext(); it->Next()) {
    ...
    if (reg_type.IsIntegralTypes()) {
      ...
      if (!work_line_->VerifyRegisterType(this, get_reg, reg_type)) {
        ...
        // Check that registers are consecutive. Invokes are the only instructions not specifying
        // register pairs by the first component, but require them nonetheless.
        // Only check when there's an actual register in the parameters. If there's none, this will
        // fail below.
