...
ArtMethod* MethodVerifier::VerifyInvocationArgs(
  ...
  if (is_super) {
    ...
    auto* cl = Runtime::Current()->GetClassLinker();
    if (res_method->GetMethodIndex() >= super_klass->GetVTableLength(cl->GetImagePointerSize())) {
...
ArtMethod* MethodVerifier::GetQuickInvokedMethod(const Instruction* inst,
  ...
  auto* cl = Runtime::Current()->GetClassLinker();
  auto pointer_size = cl->GetImagePointerSize();
  if (static_cast<int32_t>(vtable_index) >= dispatch_class->GetVTableLength(pointer_size)) {
