...
bool MethodVerifier::CheckSwitchTargets(uint32_t cur_offset) {
  ...
  const int32_t keys_offset = 2;
  if (switch_count > 1) {
    if (is_packed_switch) {
      /* for a packed switch, verify that keys don't overflow int32 */
      int32_t first_key = switch_insns[keys_offset] | (switch_insns[keys_offset + 1] << 16);
      int32_t last_key = first_key + (switch_count - 1);
      if (last_key <= first_key) {
        Fail(VERIFY_ERROR_BAD_CLASS_HARD) << "invalid packed switch: first key=" << first_key
                                          << ", last key=" << last_key;
        return false;
      }
    } else {
