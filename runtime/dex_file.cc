...
bool DexFile::GetChecksum(const char* filename, uint32_t* checksum, std::string* error_msg) {
  ...
  if (IsMultiDexLocation(filename)) {
    std::pair<std::unique_ptr<char>, const char*> pair = SplitMultiDexLocation(filename);
...
bool DexFile::OpenZip(int fd, const std::string& location, std::string* error_msg,
  ...
  return DexFile::OpenAll(*zip_archive, location, error_msg, dex_files);
}
...
bool DexFile::IsMultiDexLocation(const std::string& location) {
  return location.find(':') != std::string::npos;
}

std::pair<std::unique_ptr<char>, const char*> DexFile::SplitMultiDexLocation(
    const char* location) {
...
std::string DexFile::GetMultiDexBaseFilename(const std::string& location) {
