...
void DexFile::DecodeDebugLocalInfo(const CodeItem* code_item, bool is_static, uint32_t method_idx,
                                   DexDebugNewLocalCb local_cb, void* context) const {
  DCHECK(code_item != nullptr);
  DCHECK(local_cb != nullptr);
  const uint8_t* stream = GetDebugInfoStream(code_item);  // May return nullptr.
  std::unique_ptr<LocalInfo[]> local_in_reg(new LocalInfo[code_item->registers_size_]);

  const uint16_t first_arg_reg = code_item->registers_size_ - code_item->ins_size_;
  uint16_t arg_reg = first_arg_reg;
  if (!is_static) {
    ...
    local_in_reg[arg_reg].end_address_ = code_item->insns_size_in_code_units_;
    local_in_reg[arg_reg].reg_ = arg_reg;
    local_in_reg[arg_reg].is_arg_ = true;
    local_in_reg[arg_reg].is_this_ = true;
    local_in_reg[arg_reg].is_live_ = true;
    local_cb(context, local_in_reg[arg_reg]);
    arg_reg++;
  }
  ...
  for (; it.HasNext(); it.Next()) {
    ...
    uint32_t name_idx = (stream != nullptr ? DecodeUnsignedLeb128P1(&stream) : kDexNoIndex);
    ...
    local_in_reg[arg_reg].reg_ = arg_reg;
    local_in_reg[arg_reg].is_arg_ = true;
    local_in_reg[arg_reg].is_this_ = false;
    local_in_reg[arg_reg].is_live_ = true;
    local_cb(context, local_in_reg[arg_reg]);
    switch (*descriptor) {
      case 'D':
  ...
  if (stream == nullptr) {
    // We can generate parameters without the stream, but not local variables.
    return;
  }
  ...
  for (;;)  {
    ...
    switch (opcode) {
      case DBG_END_SEQUENCE:
        // Emit local variables which are still alive at the end of the method.
        for (uint16_t reg = 0; reg < first_arg_reg; reg++) {
      ...
      case DBG_START_LOCAL_EXTENDED: {
        ...
        local_in_reg[reg].is_arg_ = false;
        local_in_reg[reg].is_this_ = false;
      ...
      case DBG_END_LOCAL: {
        ...
        DCHECK(local_in_reg[reg].is_live_);
        local_in_reg[reg].end_address_ = address;
        local_cb(context, local_in_reg[reg]);
...
void DexFile::DecodeDebugPositionInfo(const CodeItem* code_item, DexDebugNewPositionCb position_cb,
                                      void* context) const {
  DCHECK(code_item != nullptr);
