...
static inline JValue Execute(
    ...
    ShadowFrame& shadow_frame, JValue result_register,
    bool force_interpreter = false) SHARED_REQUIRES(Locks::mutator_lock_) {
  DCHECK(!shadow_frame.GetMethod()->IsAbstract());
  DCHECK(!shadow_frame.GetMethod()->IsNative());
  if (LIKELY(shadow_frame.GetDexPC() == 0 && !force_interpreter)) {
    // Entering the method, but not via deoptimization, or when JIT has been disabled by client.
    if (kIsDebugBuild) {
...
void EnterInterpreterFromInvoke(Thread* self, ArtMethod* method, Object* receiver,
  ...
  if (LIKELY(!method->IsNative())) {
    JValue r = Execute(self, code_item, *shadow_frame, JValue(), force_interpreter);
    if (result != nullptr) {
      *result = r;
    }
  } else {
