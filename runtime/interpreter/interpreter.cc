...
void EnterInterpreterFromDeoptimize(Thread* self,
  ...
  while (shadow_frame != nullptr) {
    ...
    if (UNLIKELY(self->IsExceptionPending())) {
      ...
      // For an invoke, use the dex pc of the next instruction. We only do this if we do not
      // come from compiled code.
    ...
    // Following deoptimizations of shadow frames are implicit so set from_code to false.
    from_code = false;
    first = false;
  }
