...
static bool IsStringInit(const Instruction* instr,
                         ArtMethod* caller,
                         ArtMethod* callee)
    SHARED_REQUIRES(Locks::mutator_lock_) {
  if (instr->Opcode() == Instruction::INVOKE_DIRECT ||
    ...
    if (callee == nullptr) {
      // Don't know the callee. Lookup and find out.
      ...
      // Compare callee method's class name and method name against string init.
    ...
    if (callee->GetDeclaringClass()->IsStringClass() && callee->IsConstructor()) {
      return true;
    }
  }
  return false;
...
void EnterInterpreterFromDeoptimize(Thread* self,
  ...
  while (shadow_frame != nullptr) {
    ...
    if (UNLIKELY(self->IsExceptionPending())) {
      ...
      if (instr->IsInvoke()) {
        if (IsStringInit(instr, shadow_frame->GetMethod(), callee_method)) {
          ...
          // Calling string constructor in the original dex code doesn't need a result value.
          value.SetL(nullptr);
        }
