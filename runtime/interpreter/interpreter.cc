...
static void UnstartedRuntimeJni(Thread* self, ArtMethod* method,
  ...
  CHECK(Runtime::Current()->IsActiveTransaction()) << "Calling native method " << PrettyMethod(method)
      << " in unstarted runtime should only happen in a transaction";
  ...
  if (name == "java.lang.ClassLoader dalvik.system.VMStack.getCallingClassLoader()") {
    ...
    obj->SetFieldObjectTransactional(MemberOffset((static_cast<uint64_t>(args[2]) << 32) | args[1]),
                                     newValue, false);
  } else {
