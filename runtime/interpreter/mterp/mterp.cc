...
extern "C" int MterpSetUpHotnessCountdown(ArtMethod* method, ShadowFrame* shadow_frame)
  ...
  DCHECK_GT(countdown_value, std::numeric_limits<int16_t>::min());
  countdown_value = std::min(countdown_value,
                             static_cast<int32_t>(std::numeric_limits<int16_t>::max()));
  shadow_frame->SetCachedHotnessCounter(countdown_value);
...
extern "C" int16_t MterpAddHotnessBatch(ArtMethod* method,
  ...
  if (jit != nullptr) {
    ...
    jit::JitInstrumentationCache* instrumentation_cache = jit->GetInstrumentationCache();
    if (instrumentation_cache != nullptr) {
      instrumentation_cache->AddSamples(self, method, count);
    }
  }
