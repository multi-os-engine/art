...
extern "C" bool MterpCheckCast(uint32_t index, Object** obj_p, art::ArtMethod* method,
                               Thread* self)
    SHARED_REQUIRES(Locks::mutator_lock_) {
...
extern "C" bool MterpInstanceOf(uint32_t index, Object** obj_p, art::ArtMethod* method,
                                Thread* self)
    SHARED_REQUIRES(Locks::mutator_lock_) {
  Class* c = ResolveVerifyAndClinit(index, method, self, false, false);
  if (UNLIKELY(c == nullptr)) {
