...
static inline String* ResolveString(Thread* self, ShadowFrame& shadow_frame, uint32_t string_idx)
  ...
  mirror::Class* declaring_class = method->GetDeclaringClass();
  // MethodVerifier refuses methods with string_idx out of bounds.
  DCHECK_LT(string_idx % declaring_class->GetDexCache()->NumStrings(), declaring_class->GetDexFile().NumStringIds());
  uint64_t s = declaring_class->GetDexCacheStrings()[string_idx % declaring_class->GetDexCache()->NumStrings()].load(std::memory_order_relaxed);
  uint32_t index = (s & 0xFFFFFFFF00000000ULL) >> 32;
  mirror::String* string_ptr = (mirror::String*)(s & 0xFFFFFFFF);
  if (UNLIKELY(string_ptr == nullptr || index != string_idx)) {
    StackHandleScope<1> hs(self);
    Handle<mirror::DexCache> dex_cache(hs.NewHandle(declaring_class->GetDexCache()));
    string_ptr = Runtime::Current()->GetClassLinker()->ResolveString(*method->GetDexFile(), string_idx,
                                                            dex_cache);
  }
  return string_ptr;
}
