...
static inline void WriteLambdaClosureIntoVReg(ShadowFrame& shadow_frame,
...
static inline bool DoCreateLambda(Thread* self, ShadowFrame& shadow_frame,
  ...
  WriteLambdaClosureIntoVReg(shadow_frame, *called_method, vregA);
  return true;
}

// Read out the 'ArtMethod*' stored inside of vreg and vreg+1
//
// Validates that the art method points to a valid lambda function, otherwise throws
// an exception and returns null.
// (Exceptions are thrown by creating a new exception and then being put in the thread TLS)
static inline ArtMethod* ReadsLambdaClosureFromVRegOrThrow(ShadowFrame& shadow_frame,
                                                           uint32_t vreg)
    SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
...
static inline bool DoInvokeLambda(Thread* self, ShadowFrame& shadow_frame, const Instruction* inst,
  ...
  ArtMethod* const called_method = ReadsLambdaClosureFromVRegOrThrow(shadow_frame, vC);
...
static inline bool DoBoxLambda(Thread* self, ShadowFrame& shadow_frame, const Instruction* inst,
  ...
  ArtMethod* const closure_method = ReadsLambdaClosureFromVRegOrThrow(shadow_frame,
                                                                      vreg_source_closure);
...
static inline bool DoUnboxLambda(Thread* self ATTRIBUTE_UNUSED,
  ...
  WriteLambdaClosureIntoVReg(shadow_frame, *unboxed_closure, vreg_target_closure);
  return true;
}
