...
static inline bool DoBoxLambda(Thread* self, ShadowFrame& shadow_frame, const Instruction* inst,
  ...
  // XX: Do we want create-lambda to work for static methods outside of the declaring class?
  ...
  {
    // Giant hack to "guess" the interface corresponding to a box-lambda.
    ...
    const char* method_shorty =
        target_method->GetDexFile()->GetMethodShorty(target_method->GetMethodIndex());

    if (strcmp(method_shorty, "(L)I") == 0) {
      class_name = "Ljava/lang/Comparable;";
    } else if (strcmp(method_shorty, "()V") == 0) {
      class_name = "Ljava/lang/Runnable;";
    } else if (strcmp(method_shorty, "()L") == 0) {
      class_name = "Ljava/lang/Iterable;";
    } else */
#endif
    {
      ...
      if (string_reference == nullptr) {
        LOG(FATAL) << "box-lambda needs the type name stored in string vA (target), but it was null";
        UNREACHABLE();
      }

      if (!string_reference->IsString()) {
        LOG(FATAL) << "box-lambda needs the type name stored in string vA (target)";
        UNREACHABLE();
      }
