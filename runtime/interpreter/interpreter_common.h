...
static inline bool DoFieldPut(Thread* self, const ShadowFrame& shadow_frame,
  ...
  // TODO: use transactional or non-transactional versions.
  CHECK_EQ(transaction_active, Runtime::Current()->IsActiveTransaction());
  uint32_t vregA = is_static ? inst->VRegA_21c(inst_data) : inst->VRegA_22c(inst_data);
  switch (field_type) {
    case Primitive::kPrimBoolean:
      if (transaction_active) {
        f->SetBooleanTransactional(obj, shadow_frame.GetVReg(vregA));
      } else {
        f->SetBooleanNonTransactional(obj, shadow_frame.GetVReg(vregA));
      }
      break;
    case Primitive::kPrimByte:
      if (transaction_active) {
        f->SetByteTransactional(obj, shadow_frame.GetVReg(vregA));
      } else {
        f->SetByteNonTransactional(obj, shadow_frame.GetVReg(vregA));
      }
      break;
    case Primitive::kPrimChar:
      if (transaction_active) {
        f->SetCharTransactional(obj, shadow_frame.GetVReg(vregA));
      } else {
        f->SetCharNonTransactional(obj, shadow_frame.GetVReg(vregA));
      }
      break;
    case Primitive::kPrimShort:
      if (transaction_active) {
        f->SetShortTransactional(obj, shadow_frame.GetVReg(vregA));
      } else {
        f->SetShortNonTransactional(obj, shadow_frame.GetVReg(vregA));
      }
      break;
    case Primitive::kPrimInt:
      if (transaction_active) {
        f->SetIntTransactional(obj, shadow_frame.GetVReg(vregA));
      } else {
        f->SetIntNonTransactional(obj, shadow_frame.GetVReg(vregA));
      }
      break;
    case Primitive::kPrimLong:
      if (transaction_active) {
        f->SetLongTransactional(obj, shadow_frame.GetVRegLong(vregA));
      } else {
        f->SetLongNonTransactional(obj, shadow_frame.GetVRegLong(vregA));
      }
      break;
    case Primitive::kPrimNot: {
      ...
      if (transaction_active) {
        f->SetObjTransactional(obj, reg);
      } else {
        f->SetObjNonTransactional(obj, reg);
      }
      break;
    }
    default:
...
static inline bool DoIPutQuick(const ShadowFrame& shadow_frame, const Instruction* inst, uint16_t inst_data) {
  ...
  switch (field_type) {
    case Primitive::kPrimInt:
      if (transaction_active) {
        obj->SetField32Transactional(field_offset, shadow_frame.GetVReg(vregA), is_volatile);
      } else {
        obj->SetField32NonTransactional(field_offset, shadow_frame.GetVReg(vregA), is_volatile);
      }
      break;
    case Primitive::kPrimLong:
      if (transaction_active) {
        obj->SetField64Transactional(field_offset, shadow_frame.GetVRegLong(vregA), is_volatile);
      } else {
        obj->SetField64NonTransactional(field_offset, shadow_frame.GetVRegLong(vregA), is_volatile);
      }
      break;
    case Primitive::kPrimNot:
      if (transaction_active) {
        obj->SetFieldObjectTransactional(field_offset, shadow_frame.GetVRegReference(vregA), is_volatile);
      } else {
        obj->SetFieldObjectNonTransactional(field_offset, shadow_frame.GetVRegReference(vregA), is_volatile);
      }
      break;
    default:
      LOG(FATAL) << "Unreachable: " << field_type;
