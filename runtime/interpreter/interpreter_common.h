...
static inline bool ValidateLambdaTarget(ArtMethod* called_method)
    SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  bool success = false;
...
static inline bool DoCreateLambda(Thread* self, ShadowFrame& shadow_frame,
  ...
  static_assert(sizeof(uint64_t) >= sizeof(uintptr_t), "Unpossible");
...
static inline bool DoInvokeLambda(Thread* self, ShadowFrame& shadow_frame, const Instruction* inst,
   ...
   *
   * - vC is always the closure register (both vC and vC + 1 will be used to store the closure).
  ...
  // We also should only need to use 1 register instead of 2.
