...
// Forward declaration for ResolveString
...
//
// The closure must be big allocated big enough to hold the data, and should not be
...
static inline bool DoCaptureVariable(Thread* self,
  ...
  String* type_string = ResolveString(self, shadow_frame, string_idx);
  if (UNLIKELY((type_string = ResolveString(self, shadow_frame, string_idx)) == nullptr)) {
    CHECK(self->IsExceptionPending());
    return false;
  }

  uint16_t type_first_letter = type_string->CharAt(0);
