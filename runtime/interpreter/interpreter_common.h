...
static inline bool DoFieldGet(Thread* self, ShadowFrame& shadow_frame,
  ...
  if (kEnableFieldWatchpointReporting) {
    ...
    if (instrumentation->HasFieldReadListeners()) {
      instrumentation->FieldReadEvent(self, obj, shadow_frame.GetMethod(), shadow_frame.GetDexPC(), f);
    }
  }
...
static inline bool DoFieldPut(Thread* self, const ShadowFrame& shadow_frame,
  ...
  if (kEnableFieldWatchpointReporting) {
    ...
    if (instrumentation->HasFieldReadListeners()) {
      ...
      instrumentation->FieldWriteEvent(self, obj, shadow_frame.GetMethod(), shadow_frame.GetDexPC(),
                                       f, field_value);
    }
  }
