 ...
 public:
  ...
  bool VisitFrame() OVERRIDE SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    ...
    if (dex_pc != DexFile::kDexNoIndex) {
      ...
      uint32_t found_dex_pc = method->FindCatchBlock(to_find, dex_pc, &clear_exception_);
...
static inline uint32_t FindNextInstructionFollowingException(Thread* self,
  ...
  if (!self->IsExceptionReportedToInstrumentation() && instrumentation->HasExceptionCaughListeners()) {
