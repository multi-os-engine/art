...
bool DoCall(ArtMethod* called_method, Thread* self, ShadowFrame& shadow_frame,
  ...
  if (string_init && !self->IsExceptionPending()) {
    ...
    if (it == method_to_string_init_map->end()) {
      string_init_map = verifier::MethodVerifier::FindStringInitMap(method);
      if (method_to_string_init_map->find(method_ref) != method_to_string_init_map->end()) {
        method_to_string_init_map->Put(method_ref, string_init_map);
      }
    } else {
      string_init_map = it->second;
    }
    if (string_init_map.size() != 0) {
