...
static inline bool DoCallCommon(ArtMethod* called_method,
  ...
  if (LIKELY(Runtime::Current()->IsStarted())) {
    ...
    if (Runtime::Current()->GetInstrumentation()->IsForcedInterpretOnly()
        ...
        || (new_shadow_frame->GetMethod()->GetEntryPointFromQuickCompiledCode() == GetQuickToInterpreterBridge())
        || UNLIKELY(Dbg::IsForcedInterpreterNeededForCalling(self, new_shadow_frame->GetMethod()))) {
