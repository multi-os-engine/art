...
bool DoCallCommon(ArtMethod* called_method,
  ...
  if (LIKELY(code_item != nullptr)) {
    ...
    DCHECK_EQ(string_init ? num_inputs - 1 : num_inputs, code_item->ins_size_);
  } else {
  ...
  if (do_assignability_check) {
    ...
    if (is_range) {
      ...
      DCHECK_NE(first_dest_reg + arg_index, (size_t)-1);
