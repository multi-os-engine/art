...
void UnstartedRuntime::UnstartedClassGetDeclaredField(
  ...
  if (runtime->IsActiveTransaction()) {
    if (pointer_size == PointerSize::kPointerSize64) {
      field = mirror::Field::CreateFromArtField<PointerSize::kPointerSize64, true>(
          self, found, true);
    } else {
      field = mirror::Field::CreateFromArtField<PointerSize::kPointerSize32, true>(
          self, found, true);
    }
  } else {
    if (pointer_size == PointerSize::kPointerSize64) {
      field = mirror::Field::CreateFromArtField<PointerSize::kPointerSize64, false>(
          self, found, true);
    } else {
      field = mirror::Field::CreateFromArtField<PointerSize::kPointerSize32, false>(
          self, found, true);
    }
  }
  result->SetL(field);
...
void UnstartedRuntime::UnstartedClassGetDeclaredMethod(
  ...
  if (transaction) {
    if (pointer_size == PointerSize::kPointerSize64) {
      method = mirror::Class::GetDeclaredMethodInternal<PointerSize::kPointerSize64, true>(
          self, klass, name, args);
    } else {
      method = mirror::Class::GetDeclaredMethodInternal<PointerSize::kPointerSize32, true>(
          self, klass, name, args);
    }
  } else {
    if (pointer_size == PointerSize::kPointerSize64) {
      method = mirror::Class::GetDeclaredMethodInternal<PointerSize::kPointerSize64, false>(
          self, klass, name, args);
    } else {
      method = mirror::Class::GetDeclaredMethodInternal<PointerSize::kPointerSize32, false>(
          self, klass, name, args);
    }
  }
  result->SetL(method);
...
void UnstartedRuntime::UnstartedClassGetDeclaredConstructor(
  ...
  if (transaction) {
    if (pointer_size == PointerSize::kPointerSize64) {
      constructor = mirror::Class::GetDeclaredConstructorInternal<PointerSize::kPointerSize64,
                                                                  true>(self, klass, args);
    } else {
      constructor = mirror::Class::GetDeclaredConstructorInternal<PointerSize::kPointerSize32,
                                                                  true>(self, klass, args);
    }
  } else {
    if (pointer_size == PointerSize::kPointerSize64) {
