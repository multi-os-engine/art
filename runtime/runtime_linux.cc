...
struct UContext {
  ...
  void Dump(std::ostream& os) {
    ...
    DumpRegister32(os, "cs",  (context.gregs[REG_CSGSFS])&0x0FFFFFFFF);
    DumpRegister32(os, "gs",  (context.gregs[REG_CSGSFS]>>16)&0x0FFFFFFFF);
    DumpRegister32(os, "fs",  (context.gregs[REG_CSGSFS]>>32)&0x0FFFFFFFF);
    os << '\n';
#else
  ...
  void DumpRegister64(std::ostream& os, const char* name, uint64_t value) {
    os << StringPrintf(" %6s: 0x%016lx", name, value);
  }
