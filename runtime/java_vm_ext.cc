...
class LibraryNamespaces {
 public:
  ...
  android_namespace_t* GetOrCreate(JNIEnv* env, Runtime* runtime,
    ...
    if (!initialized_ && !InitPublicNamespace(runtime, libraryPath.c_str())) {
      return nullptr;
    }
 ...
 private:
  bool InitPublicNamespace(const Runtime* runtime, const char* library_path) {
    ...
    initialized_ = android_init_namespaces(public_libraries, library_path);
...
void* JavaVMExt::OpenNativeLibrary(JNIEnv* env, const char* path,
                                   jobject class_loader, jstring library_path) {
#ifdef __ANDROID__
  if (runtime_->GetTargetSdkVersion() == 0 || class_loader == nullptr) {
