...
JavaVMExt::JavaVMExt(Runtime* runtime, ParsedOptions* options)
    : runtime_(runtime),
      ...
      libraries_lock_("JNI shared libraries map lock", kLoadLibraryLock),
...
void JavaVMExt::DumpForSigQuit(std::ostream& os) {
  ...
  {
    MutexLock mu(self, libraries_lock_);
...
bool JavaVMExt::LoadNativeLibrary(JNIEnv* env, const std::string& path, jobject class_loader,
  ...
  if (library != nullptr) {
    if (env->IsSameObject(library->GetClassLoader(), class_loader) == JNI_TRUE) {
  ...
  bool created_library = false;
  {
    ...
    if (library == nullptr) {  // We won race to get libraries_lock
      library = new SharedLibrary(env, self, path, handle, class_loader);
      libraries_->Put(path, library);
  ...
  if (sym == nullptr) {
    ...
    self->SetClassLoaderOverride(old_class_loader.release());
