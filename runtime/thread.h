 ...
 public:
  ...
  uint8_t* GetUnsafeStackEndForInterpreter(bool implicit_overflow_check) const {
    if (implicit_overflow_check) {
      // The interpreter needs the extra overflow bytes that stack_end does
      // not include.
      return tlsPtr_.unsafe_stack_end + GetStackOverflowReservedBytes(kRuntimeISA);
    } else {
  ...
  bool IsBelowStackEnd() const {
    bool below =
        UNLIKELY(__builtin_frame_address(0) < GetStackEnd());
#if __has_feature(safe_stack)
    below |= UNLIKELY(__builtin___get_unsafe_stack_ptr() < GetUnsafeStackEnd());
#endif
    return below;
  }

  bool IsBelowStackEndForInterpreter(bool implicit_overflow_check) const {
    bool below = UNLIKELY(__builtin_frame_address(0) <
                          GetStackEndForInterpreter(implicit_overflow_check));
#if __has_feature(safe_stack)
    below |= UNLIKELY(__builtin___get_unsafe_stack_ptr() <
  ...
  void ResetDefaultStackEnd() {
    ...
    if (tlsPtr_.unsafe_stack_begin)
      tlsPtr_.unsafe_stack_end = tlsPtr_.unsafe_stack_begin +
