...
class Thread {
 public:
  ...
  void SetMonitorEnterObject(mirror::Object*);

  // Implements java.lang.Thread.interrupted.
  bool Interrupted() LOCKS_EXCLUDED(tlsPtr_.wait_mutex);
  // Implements java.lang.Thread.isInterrupted.
  bool IsInterrupted() LOCKS_EXCLUDED(tlsPtr_.wait_mutex);
  bool IsInterruptedLocked() EXCLUSIVE_LOCKS_REQUIRED(tlsPtr_.wait_mutex);
  void Interrupt() LOCKS_EXCLUDED(tlsPtr_.wait_mutex);
  void SetInterruptedLocked(bool i) EXCLUSIVE_LOCKS_REQUIRED(tlsPtr_.wait_mutex);
  void Notify() LOCKS_EXCLUDED(tlsPtr_.wait_mutex);

 private:
  void NotifyLocked(Thread* self) EXCLUSIVE_LOCKS_REQUIRED(tlsPtr_.wait_mutex);

 public:
  Mutex* GetWaitMutex() const LOCK_RETURNED(tlsPtr_.wait_mutex) {
    return tlsPtr_.wait_mutex;
  }

  ConditionVariable* GetWaitConditionVariable() const;

  Monitor* GetWaitMonitor() const EXCLUSIVE_LOCKS_REQUIRED(tlsPtr_.wait_mutex);
  void SetWaitMonitor(Monitor* mon) EXCLUSIVE_LOCKS_REQUIRED(tlsPtr_.wait_mutex);
 ...
 private:
  ...
  struct PACKED(4)  tls_32bit_sized_values {
    ...
    tls_32bit_sized_values() :
      suspend_count(0), debug_suspend_count(0), thin_lock_thread_id(0), tid(0), interrupted(false),
      daemon(false), throwing_OutOfMemoryError(false), no_thread_suspension(0),
    ...
    bool32_t interrupted GUARDED_BY(tlsPtr_.wait_mutex);
  ...
  struct PACKED(4) tls_ptr_sized_values {
    ...
    Mutex* wait_mutex DEFAULT_MUTEX_ACQUIRED_AFTER;

    // Condition variable waited upon during a wait.
    ConditionVariable* wait_cond GUARDED_BY(wait_mutex);
    // Pointer to the monitor lock we're currently waiting on or NULL if not waiting.
    Monitor* wait_monitor GUARDED_BY(wait_mutex);
