...
class Thread {
 ...
 private:
  ...
  static ThreadOffset<pointer_size> ThreadOffsetFromTlsPtr(size_t tls_ptr_offset) {
    ...
    size_t scale;
    size_t shrink;
    if (pointer_size == kPointerSizeCur) {
      scale = 1;
      shrink = 1;
    } else if (pointer_size > kPointerSizeCur) {
      scale = static_cast<size_t>(pointer_size) / static_cast<size_t>(kPointerSizeCur);
      shrink = 1;
    } else {
      DCHECK_GT(kPointerSizeCur, pointer_size);
      scale = 1;
      shrink = static_cast<size_t>(kPointerSizeCur) / static_cast<size_t>(pointer_size);
    }
 ...
 public:
  static uint32_t QuickEntryPointOffsetWithSize(size_t quick_entrypoint_offset,
    ...
    if (pointer_size == PointerSize::kPointerSize32) {
      return QuickEntryPointOffset<PointerSize::kPointerSize32>(quick_entrypoint_offset).
          Uint32Value();
    } else {
      return QuickEntryPointOffset<PointerSize::kPointerSize64>(quick_entrypoint_offset).
          Uint32Value();
    }
  }
