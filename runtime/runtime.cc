...
mirror::ArtMethod* Runtime::CreateCalleeSaveMethod(CalleeSaveType type) {
  ...
  method->SetFrameSizeInBytes(callee_save_method_frame_infos_[type].FrameSizeInBytes());
  method->SetCoreSpillMask(callee_save_method_frame_infos_[type].CoreSpillMask());
  method->SetFpSpillMask(callee_save_method_frame_infos_[type].FpSpillMask());
  return method.get();
}
...
MethodFrameInfo Runtime::GetRuntimeMethodFrameInfo(mirror::ArtMethod* method) const {
...
MethodFrameInfo Runtime::GetJniMethodFrameInfo(mirror::ArtMethod* method) const {
  const char* shorty = MethodHelper(method).GetShorty();
  if ((instruction_set_ == kThumb2) || (instruction_set_ == kArm)) {
    return arm::ArmJniMethodFrameInfo(method->IsStatic(), shorty);
  } else if (instruction_set_ == kMips) {
    return mips::MipsJniMethodFrameInfo(method->IsStatic(), shorty);
  } else if (instruction_set_ == kX86) {
    return x86::X86JniMethodFrameInfo(method->IsStatic(), shorty);
  } else if (instruction_set_ == kX86_64) {
    return x86_64::X86_64JniMethodFrameInfo(method->IsStatic(), shorty);
  } else if (instruction_set_ == kArm64) {
    return arm64::Arm64JniMethodFrameInfo(method->IsStatic(), shorty);
  } else {
    UNIMPLEMENTED(FATAL) << instruction_set_;
    return MethodFrameInfo(0u, 0u, 0u);
  }
}
