...
Runtime::~Runtime() {
  ...
  if (jit_.get() != nullptr) {
    ...
    // Similar, stop the profile saver thread before deleting the thread list.
...
bool Runtime::Start() {
  ...
  if (profiler_options_.IsEnabled() && !profile_output_filename_.empty()) {
    ...
    if (fd >= 0) {
      close(fd);
      StartProfileSaver(profile_output_filename_);
...
void Runtime::StartProfileSaver(const std::string& profile_output_filename) {
  if (jit_.get() != nullptr) {
