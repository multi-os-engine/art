...
InstructionSet Runtime::GetQuickCodeISA() {
  ...
  // On simulator mode, image ISA should align with simulator.
  if (NeedsSimulator()) {
    return runtime->GetSimulateISA();
  }

  // Otherwise, image ISA should align with runtime.
  if (runtime->GetInstructionSet() == kNone) {
    return kRuntimeISA;
  }
...
void Runtime::SetSimulateISA(InstructionSet instruction_set) {
  DCHECK(GetInstructionSet() != kNone) << "Error: set simulator ISA before runtime ISA.";
...
void Runtime::AddCurrentRuntimeFeaturesAsDex2OatArguments(std::vector<std::string>* argv)
  ...
  // currently being executed. In simulator mode, the dex2oat instructioni set should match
  // that being simulated.
  std::string instruction_set("--instruction-set=");
  std::string feature_string("--instruction-set-features=");
  if (simulate_isa_ == kNone) {
    instruction_set += GetInstructionSetString(kRuntimeISA);
    feature_string += InstructionSetFeatures::FromCppDefines()->GetFeatureString();
  } else {
    instruction_set += GetInstructionSetString(simulate_isa_);
    std::string error_msg;
    feature_string += InstructionSetFeatures::FromVariant(
        simulate_isa_, "default", &error_msg)->GetFeatureString();
  }
