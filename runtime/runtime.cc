...
static size_t OpenDexFiles(const std::vector<std::string>& dex_filenames,
  ...
  if (image_location.size() != 0 && OpenDexFilesFromImage(dex_filenames,
                                                          image_location,
                                                          dex_files,
                                                          &failure_count)) {
...
bool Runtime::Init(const RuntimeOptions& raw_options, bool ignore_unrecognized) {
  ...
  const std::string& image_filename = options->image_;
  ...
  if (GetHeap()->HasImageSpace()) {
    ...
    OpenDexFiles(dex_filenames, image_filename, boot_class_path);
