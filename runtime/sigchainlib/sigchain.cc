...
static struct sigaction user_sigactions[_NSIG];

// Flags for claimed signal chains.  A claimed signal chain will prevent a user's
// call to sigaction from being passed to the kernel.
static bool claimed_signals[_NSIG];
...
void ClaimSignalChain(int signal, struct sigaction* oldaction) {
  ...
  user_sigactions[signal] = *oldaction;
  claimed_signals[signal] = true;
}
...
void UnclaimSignalChain(int signal) {
  ...
  claimed_signals[signal] = false;
  sigaction(signal, &user_sigactions[signal], NULL);        // Restore old action.
}
...
void InvokeUserSignalHandler(int sig, siginfo_t* info, void* context) {
  ...
  if (!claimed_signals[sig]) {
    abort();
  }

  struct sigaction* action = &user_sigactions[sig];
...
int sigaction(int signal, const struct sigaction* new_action, struct sigaction* old_action) {
  ...
  if (signal > 0 && signal < _NSIG && claimed_signals[signal]) {
     if (old_action != NULL) {
  ...
  if (bionic_new_set != NULL) {
    ...
    if (how == SIG_BLOCK) {
      ...
      for (int i = 0 ; i < _NSIG; ++i) {
        if (claimed_signals[i] && sigismember(&tmpset, i)) {
            sigdelset(&tmpset, i);
        }
      }
    }
