...
inline mirror::Class* ArtMethod::GetDeclaringClass() {
  ...
  if (this != Runtime::Current()->GetResolutionMethod()) {
...
inline uint32_t ArtMethod::GetDexMethodIndex() {
  auto* declaring_class = GetDeclaringClassUnchecked();
  if (declaring_class != nullptr) {
    DCHECK(declaring_class->IsLoaded() || declaring_class->IsErroneous());
  } else {
    // Runtime methods are allowed to have no declaring class.
    DCHECK_EQ(dex_method_index_, DexFile::kDexNoIndex);
  }
