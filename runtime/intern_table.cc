...
void InternTable::EnsureNewWeakInternsDisallowed() {
  ...
  MutexLock mu(Thread::Current(), *Locks::intern_table_lock_);
  CHECK_EQ(weak_root_state_.LoadSequentiallyConsistent(), gc::kWeakRootStateNoReadsOrWrites);
}
...
mirror::String* InternTable::Insert(mirror::String* s, bool is_strong, bool holding_locks) {
  ...
  while (true) {
    ...
    if (weak_root_state_.LoadSequentiallyConsistent() != gc::kWeakRootStateNoReadsOrWrites) {
      break;
    }
    CHECK(!holding_locks);
    {
      ...
      while (weak_root_state_.LoadSequentiallyConsistent() == gc::kWeakRootStateNoReadsOrWrites) {
        if (kDebugLocking) {
          CHECK_EQ(1u, self->NumberOfHeldMutexes()) << "may only safely hold the mutator lock";
        }
  ...
  DCHECK_NE(weak_root_state_.LoadSequentiallyConsistent(), gc::kWeakRootStateMarkNewRoots)
      << "Unsupported";
...
void InternTable::ChangeWeakRootStateLocked(gc::WeakRootState new_state) {
  weak_root_state_.StoreSequentiallyConsistent(new_state);
