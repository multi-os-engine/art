...
ThreadList::~ThreadList() {
  ...
  if (!WaitForAllThreadsToFinish()) {
    Runtime::Current()->GetJavaVM()->NotifyShutdownFailure(
        "Unable to ensure all runtime threads have finished executing.");
  }
}
...
bool ThreadList::WaitForAllThreadsToFinish() {
  ...
  if (!last_try) {
    LOG(WARNING) << "Some threads failed to finish after being canceled and waiting a total of "
