...
size_t ThreadList::FlipThreadRoots(Closure* thread_flip_visitor,
  ...
  std::vector<Thread*> other_threads;
  {
    ...
    for (const auto& thread : list_) {
      ...
      // Resume early the threads that were runnable but are suspended just for this thread flip
      // (kSuspended), waiting for the thread flip to end at the JNI critical section entry
      // (kWaitingForGcThreadFlip), or about to transition from non-runnable (eg. kNative at the SOA
      // entry in a JNI function) to runnable (inside Thread::TransitionFromSuspendedToRunnable).
      ThreadState state = thread->GetState();
      if (state == kSuspended ||
          state == kWaitingForGcThreadFlip ||
