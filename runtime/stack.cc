...
bool StackVisitor::GetVRegPairFromDebuggerShadowFrame(uint16_t vreg, VRegKind kind_lo,
                                                      VRegKind kind_hi, uint64_t* val) const {
  uint32_t low_32bits;
  uint32_t high_32bits;
  bool success = GetVRegFromDebuggerShadowFrame(vreg, kind_lo, outof(low_32bits));
  success &= GetVRegFromDebuggerShadowFrame(vreg + 1, kind_hi, outof(high_32bits));
  if (success) {
...
bool StackVisitor::SetVRegFromDebugger(ArtMethod* m, uint16_t vreg, uint32_t new_value,
                                       VRegKind kind) {
  DCHECK(cur_shadow_frame_ == nullptr);
  const DexFile::CodeItem* code_item = m->GetCodeItem();
  if (code_item == nullptr) {
  ...
  ShadowFrame* shadow_frame = thread_->FindOrCreateDebuggerShadowFrame(
      GetFrameId(), num_regs, m, GetDexPc());
  ...
  thread_->GetUpdatedVRegFlags(GetFrameId())[vreg] = true;
  return true;
}
...
bool StackVisitor::SetVRegPairFromDebugger(ArtMethod* m, uint16_t vreg, uint64_t new_value,
                                           VRegKind kind_lo, VRegKind kind_hi) {
  if (kind_lo == kLongLoVReg) {
    DCHECK_EQ(kind_hi, kLongHiVReg);
  } else if (kind_lo == kDoubleLoVReg) {
