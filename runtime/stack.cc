...
mirror::ArtMethod* StackVisitor::GetMethod() const SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  if (cur_shadow_frame_ != nullptr) {
    ...
    if (IsInInlinedFrame()) {
      ...
      return outer_method->GetDexCacheResolvedMethod(
          inline_info.GetMethodIndexAtDepth(depth_in_stack_map));
    } else {
...
uint32_t StackVisitor::GetDexPc(bool abort_on_failure) const {
  if (cur_shadow_frame_ != nullptr) {
    ...
    if (IsInInlinedFrame()) {
      ...
      InlineInfo inline_info = code_info.GetInlineInfoOf(stack_map);
      size_t depth_in_stack_map = current_inlining_depth_ - 1;
      return inline_info.GetDexPcAtDepth(depth_in_stack_map);
    } else {
