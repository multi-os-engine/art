...
InlineInfo StackVisitor::GetCurrentInlineInfo() const {
  OatQuickMethodHeader* method_header = GetCurrentOatQuickMethodHeader();
      ...
      while (method != nullptr) {
        ...
        if (kDebugStackWalk) {
          LOG(INFO) << PrettyMethod(method) << "@" << method << " size=" << frame_size
              << " optimized=" << (cur_oat_quick_method_header_ != nullptr && cur_oat_quick_method_header_->IsOptimized())
