...
mirror::Object* QuickFrame::GetThisObject() const {
  ...
  if (m->IsStatic()) {
    ...
    HandleScope* hs = reinterpret_cast<HandleScope*>(
        GetSp() + m->GetHandleScopeOffset().SizeValue());
...
size_t QuickFrame::GetNativePcOffset() const {
  return GetMethod()->NativeQuickPcOffset(pc_);
}
...
uintptr_t QuickFrame::GetReturnPc() const {
  uintptr_t pc_addr = GetSp() + GetMethod()->GetReturnPcOffset().SizeValue();
...
void QuickFrame::SanityCheckFrame() const {
  ...
  size_t return_pc_offset = GetMethod()->GetReturnPcOffset().SizeValue();
...
class QuickFrameIterator {
 public:
  ...
  void Advance() SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    ...
    if (UNLIKELY(exit_stubs_installed_
          && (reinterpret_cast<uintptr_t>(GetQuickInstrumentationExitPc()) == new_frame.GetPc()))) {
