...
mirror::Object* QuickFrame::GetThisObject() const {
  ...
  if (m->IsStatic()) {
    ...
    HandleScope* hs = reinterpret_cast<HandleScope*>(
        reinterpret_cast<char*>(sp_) + m->GetHandleScopeOffsetInBytes());
...
uintptr_t QuickFrame::GetReturnPc() const {
  uintptr_t pc_addr = sp_ + GetMethod()->GetReturnPcOffsetInBytes();
...
QuickFrame QuickFrame::GetCaller() const {
  ...
  uintptr_t next_frame = sp_ + frame_size;
  return QuickFrame(next_frame, return_pc, context_);
}

mirror::Object* QuickFrame::GetJniThis() const {
  // Skip Method*; handle scope comes next;
  return reinterpret_cast<HandleScope*>(sp_ + sizeof(StackReference<mirror::ArtMethod>))->GetReference(0);
}
...
void StackVisitor::WalkStack(bool include_transitions) {
  ...
  for (const ManagedStack* current_fragment = thread_->GetManagedStack();
    ...
    uintptr_t top_sp = current_fragment->GetTopCompiledFrameSp();
    if (top_sp != 0) {
      // Handle Quick stack frames.
      DCHECK(shadow_frame == nullptr);
      QuickFrameIterator it(top_sp, exit_stubs_installed, &instrumentation_stack_depth, this);
      DCHECK(!it.Done());
      do {
