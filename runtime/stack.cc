...
bool StackVisitor::GetVRegPair(mirror::ArtMethod* m, uint16_t vreg, VRegKind kind_lo,
  ...
  if (cur_quick_frame_ != nullptr) {
    ...
    if (vmap_table.IsInContext(vreg, kind_lo, &vmap_offset_lo) &&
        vmap_table.IsInContext(vreg, kind_hi, &vmap_offset_hi)) {
      ...
      bool target64 = Is64BitInstructionSet(kRuntimeISA);
      if (is_float) {
        success = GetFPR(reg_lo, &ptr_val_lo);
        success &= GetFPR(reg_hi, &ptr_val_hi);
      } else {
        success = GetGPR(reg_lo, &ptr_val_lo);
        success &= GetGPR(reg_hi, &ptr_val_hi);
      }
      if (success && target64) {
      ...
      *val = (static_cast<uint64_t>(ptr_val_hi) << 32) | static_cast<uint32_t>(ptr_val_lo);
      ...
      const DexFile::CodeItem* code_item = m->GetCodeItem();
      DCHECK(code_item != NULL) << PrettyMethod(m);  // Can't be NULL or how would we compile
                                                     // its instructions?
  ...
  if (cur_quick_frame_ != nullptr) {
    ...
    if (vmap_table.IsInContext(vreg, kind_lo, &vmap_offset_lo) &&
      ...
      if (target64) {
        ...
        bool success;
        if (is_float) {
          success = GetFPR(reg_lo, &old_reg_val_lo);
          success &= GetFPR(reg_hi, &old_reg_val_hi);
        } else {
          success = GetGPR(reg_lo, &old_reg_val_lo);
          success &= GetGPR(reg_hi, &old_reg_val_hi);
        }
        if (!success) {
          return false;
        }
      ...
      bool success;
      if (is_float) {
        success = SetFPR(reg_lo, new_value_lo);
        success &= SetFPR(reg_hi, new_value_hi);
      } else {
        success = SetGPR(reg_lo, new_value_lo);
        success &= SetGPR(reg_hi, new_value_hi);
      }
      return success;
    } else {
      const DexFile::CodeItem* code_item = m->GetCodeItem();
      ...
      int offset = GetVRegOffset(code_item, frame_info.CoreSpillMask(), frame_info.FpSpillMask(),
                                 frame_info.FrameSizeInBytes(), vreg, kRuntimeISA);
      byte* vreg_addr = reinterpret_cast<byte*>(GetCurrentQuickFrame()) + offset;
      *reinterpret_cast<uint64_t*>(vreg_addr) = new_value;
      return true;
    }
  } else {
