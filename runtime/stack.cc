...
QuickMethodFrameInfo StackVisitor::GetCurrentQuickFrameInfo() const {
  ...
  QuickMethodFrameInfo callee_info = runtime->GetCalleeSaveMethodFrameInfo(Runtime::kSaveRefsAndArgs);
      ...
      while (method != nullptr) {
        ...
        if (UNLIKELY(exit_stubs_installed)) {
          ...
          if (reinterpret_cast<uintptr_t>(GetQuickInstrumentationExitPc()) == return_pc) {
            ...
            if (GetMethod() == Runtime::Current()->GetCalleeSaveMethod(Runtime::kSaveAllCalleeSaves)) {
              // Skip runtime save all callee frames which are used to deliver exceptions.
            } else if (instrumentation_frame.interpreter_entry_) {
              ArtMethod* callee = Runtime::Current()->GetCalleeSaveMethod(Runtime::kSaveRefsAndArgs);
