...
static void DeleteDexFileAndMemory(DexFile* in) {
  if (in != nullptr) {
    delete in->Begin();
    delete in;
  }
}
using DexFileUniquePtr = std::unique_ptr<DexFile, decltype(&DeleteDexFileAndMemory)>;
...
class DexFileVerifierTest : public CommonRuntimeTest {
 protected:
  void VerifyModification(const char* dex_file_base64_content,
    ...
    DexFileUniquePtr dex_file(WrapAsDexFile(dex_file_base64_content), DeleteDexFileAndMemory);
    f(dex_file.get());
 ...
 private:
  DexFile* WrapAsDexFile(const char* dex_file_content_in_base_64) {
    // Decode base64.
    size_t length;
