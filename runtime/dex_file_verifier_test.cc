...
TEST_F(DexFileVerifierTest, ProtoOrdering) {
  ...
  VerifyModification(
      ...
      [](DexFile* dex_file) {
        ...
        if (data == nullptr) {
          for (size_t i = 0, num = dex_file->NumMethodIds(); i != num; ++i) {
            LOG(ERROR) << "MethodId #" << i << ": "
                << dex_file->GetStringData(dex_file->GetStringId(dex_file->GetMethodId(i).name_idx_));
          }
        }
        CHECK(data != nullptr);
        ...
        // Now, swap the proto parameters and shorties.
