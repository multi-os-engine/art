...
void ArtMethod::Invoke(Thread* self, uint32_t* args, uint32_t args_size, JValue* result,
  ...
  if (UNLIKELY(!runtime->IsStarted() || Dbg::IsForcedInterpreterNeededForCalling(self, this))) {
    ...
    LOG(ERROR) << "ENTERING " << PrettyMethod(this) << "@" << GetEntryPointFromQuickCompiledCode();
    if (PrettyMethod(this) == "java.util.Properties java.lang.System.initUnchangeableSystemProperties()") {
      LOG(ERROR) << "HERE";
    }
  ...
  LOG(ERROR) << "EXITING " << PrettyMethod(this) << "@" << this;
