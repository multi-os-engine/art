...
void ArtMethod::Invoke(Thread* self, uint32_t* args, uint32_t args_size, JValue* result,
  ...
  if (kIsDebugBuild) {
    ...
    CHECK_STREQ(GetInterfaceMethodIfProxy(kPointerSizeCur)->GetShorty(), shorty);
  }
  ...
  if (UNLIKELY(!runtime->IsStarted() || Dbg::IsForcedInterpreterNeededForCalling(self, this))) {
    ...
    DCHECK_EQ(runtime->GetClassLinker()->GetImagePointerSize(), kPointerSizeCur);
...
bool ArtMethod::IsImagePointerSize(PointerSize pointer_size) {
  ...
  static_assert(
      (sizeof(void*) != 4) ||
      (offsetof(ArtMethod, ptr_sized_fields_) == PtrSizedFieldsOffset(PointerSize::kPointerSize32)),
      "Unexpected 32-bit class layout.");
  static_assert(
      (sizeof(void*) != 8) ||
      (offsetof(ArtMethod, ptr_sized_fields_) == PtrSizedFieldsOffset(PointerSize::kPointerSize64)),
