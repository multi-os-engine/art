...
uintptr_t ArtMethod::ToNativeQuickPc(const uint32_t dex_pc, bool abort_on_failure) {
  ...
  if (IsOptimized(sizeof(void*))) {
    ...
    // Assume the caller needs the mapping for for a catch block handler...
    StackMap stack_map = code_info.GetCatchStackMapForDexPc(dex_pc, encoding);
    if (!stack_map.IsValid()) {
      // ... otherwise search through safepoint stack maps.
      stack_map = code_info.GetSafepointStackMapForDexPc(dex_pc, encoding);
    }
    if (stack_map.IsValid()) {
