...
inline mirror::String* ClassLinker::ResolveString(uint32_t string_idx, ArtMethod* referrer) {
  ...
  uint64_t index_string = declaring_class->GetDexCacheStrings()
      [string_idx % mirror::DexCache::kDexCacheStringCacheSize].load(std::memory_order_relaxed);
  uint32_t index = (index_string & 0xFFFFFFFF00000000ULL) >> 32;
  mirror::String* string = reinterpret_cast<mirror::String*>(index_string & 0xFFFFFFFF);
  if (UNLIKELY(string == nullptr || index != string_idx)) {
