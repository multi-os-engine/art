...
// Expect: The status is kDexoptNeeded.
TEST_F(OatFileAssistantTest, DexNoOat) {
  ...
  EXPECT_EQ(OatFileAssistant::kDexoptNeeded, oat_file_assistant.GetDexOptStatus());
...
// Expect: Status is kDexoptNeeded. Loading should fail, but not crash.
...
// Expect: The status is kUpToDate.
TEST_F(OatFileAssistantTest, OatUpToDate) {
  ...
  EXPECT_EQ(OatFileAssistant::kUpToDate, oat_file_assistant.GetDexOptStatus());
  EXPECT_FALSE(oat_file_assistant.IsInBootClassPath());
  EXPECT_FALSE(oat_file_assistant.OdexFileExists());
  EXPECT_TRUE(oat_file_assistant.OdexFileIsOutOfDate());
  EXPECT_FALSE(oat_file_assistant.OdexFileIsUpToDate());
...
// Expect: The status is kUpToDate and we load all dex files.
...
TEST_F(OatFileAssistantTest, DexOdexNoOat) {
  ...
  EXPECT_EQ(OatFileAssistant::kPatchoatNeeded, oat_file_assistant.GetDexOptStatus());
...
TEST_F(OatFileAssistantTest, SelfRelocation) {
  ...
  std::string oat_location = GetISADir() + "/SelfRelocation.oat";
  ...
  EXPECT_EQ(OatFileAssistant::kSelfPatchoatNeeded, oat_file_assistant.GetDexOptStatus());
...
// Expect: The status is kUpToDate, because PIC needs no relocation.
