...
class OatFileAssistantTest : public CommonRuntimeTest {
 public:
  ...
  void GenerateVerifyAtRuntimeOdexForTest(const std::string& dex_location,
                                          const std::string& odex_location) {
    std::vector<std::string> args;
    args.push_back("--dex-file=" + dex_location);
    args.push_back("--oat-file=" + odex_location);
    ...
    std::unique_ptr<OatFile> odex_file(OatFile::Open(
        ...
        false, dex_location.c_str(), &error_msg));
    ASSERT_TRUE(odex_file.get() != nullptr) << error_msg;
    EXPECT_TRUE(odex_file->IsVerifyAtRuntime());
...
// Case: We have a DEX file and a verify-at-runtime ODEX file out of date
//       relative to the DEX file.
// Expect: The status is kDex2OatNeeded.
TEST_F(OatFileAssistantTest, VerifyAtRuntimeOdexOutOfDate) {
  std::string dex_location = GetScratchDir() + "/VerifyAtRuntimeOdexOutOfDate.jar";
  std::string odex_location = GetOdexDir() + "/VerifyAtRuntimeOdexOutOfDate.odex";

  // We create a dex, generate an oat for it, then overwrite the dex with a
  // different dex to make the oat out of date.
  Copy(GetDexSrc1(), dex_location);
  GenerateVerifyAtRuntimeOdexForTest(dex_location.c_str(), odex_location.c_str());
  Copy(GetDexSrc2(), dex_location);
...
TEST_F(OatFileAssistantTest, DexVerifyAtRuntimeOdexNoOat) {
  ...
  // Create the dex and odex files
  Copy(GetDexSrc1(), dex_location);
  GenerateVerifyAtRuntimeOdexForTest(dex_location, odex_location);
