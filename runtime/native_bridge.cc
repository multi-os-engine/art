...
void NativeBridge::SetNativeLibraryString(std::string& native_bridge_string) {
  native_bridge_string_ = native_bridge_string;
...
bool NativeBridge::Initialize() {
  ...
  // TODO: Acquire-release should allow a fast-path without grabbing a lock after we once
  //       initialized.
  if (initialized_) {
  ...
  if (!native_bridge_string_.empty()) {
    libnb_path = native_bridge_string_.c_str();
  } else {
  ...
  if (handle != nullptr) {
    ...
    if (callbacks_ == nullptr) {
      ...
      if (callbacks_->initialize(&NativeBridgeArtItf)) {
        available_ = true;
      } else {
        callbacks_ = nullptr;
        dlclose(handle);
...
std::string NativeBridge::native_bridge_string_ = "";
