...
//
// This class knows about all the loaded oat files and provides utility functions.
class OatFileManager {
 public:
  OatFileManager() {}
  ~OatFileManager();

  void RegisterOatFile(const OatFile* oat_file) REQUIRES(!Locks::oat_file_manager_lock_);

  const OatFile* FindOpenedOatFileFromOatLocation(const std::string& oat_location) const
      REQUIRES(!Locks::oat_file_manager_lock_);

  // Returns if we have a non pic oat file.
  bool HaveNonPicOatFile() const REQUIRES(!Locks::oat_file_manager_lock_);
  ...
  bool HasCollisions(const OatFile* oat_file, std::string* error_msg) const
      REQUIRES(!Locks::oat_file_manager_lock_);
  ...
  std::vector<std::unique_ptr<const DexFile>> OpenDexFilesFromOat(
      ...
      std::vector<std::string>* error_msgs) REQUIRES(!Locks::oat_file_manager_lock_);

 private:
  std::vector<const OatFile*> oat_files_ GUARDED_BY(Locks::oat_file_manager_lock_);
