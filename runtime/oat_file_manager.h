...
class OatFileManager {
 public:
  ...
  // Returns true if the dex file for the underlying dex location exists.
  // This will be false if the dex file has been stripped to save space when
  // it has been pre-compiled to an odex file.
  bool DexFileExists();
 ...
 private:
  ...
  //  foo_load_succeeded_ is true if we have successfully loaded foo
  //  foo_ is the loaded thing if foo_load_succeeded_ is true.
  ...
  //
  // Note: foo_load_succeeded_ implies foo_load_attempted_.
  //
  ...
  bool GivenOatFileIsOutOfDate(std::unique_ptr<OatFile>& file);

  // Return true if the given oat file is relocated.
  // file must not be NULL.
  // Assumes the image info has already been loaded.
  bool GivenOatFileIsRelocated(std::unique_ptr<OatFile>& file);
...
class ExecutableOatFileManager : public OatFileManager {
 public:
  ...
  // The oat_file will be set to the oat file loaded if any. There may be no
  // oat file if we had to fall back to the original unquickened dex file.
  // Returns true on success.
  //
 ...
 private:
  ...
  // because access to the loaded oat file has been taken away from the
  // OatFileManager object.
  std::unique_ptr<OatFile> SelectBestOatFile();
