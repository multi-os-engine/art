...
class NullStream FINAL : public HprofStream {
 public:
  virtual bool AllowWriting() const OVERRIDE {
    return false;
  }
...
class Hprof {
 public:
  ...
  void ProcessHeapStreaming(size_t data_len, uint32_t chunk_type)
    ...
    CHECK_EQ(body_bytes + header_bytes, data_len);
  ...
  void Dump() EXCLUSIVE_LOCKS_REQUIRED(Locks::mutator_lock_)
    ...
    size_t body_bytes = 0;
    {
      ...
      // Need to do body first.
      body_bytes = ProcessBody();
      header_bytes = ProcessHeader();
      LOG(INFO) << body_bytes << " " << header_bytes;
      if (direct_to_ddms_ && kDirectStream) {
        ...
        const size_t new_header_bytes = ProcessHeader();
        stream->SetData(&body_data);
        const size_t new_body_bytes = ProcessBody();
        CHECK_EQ(header_data.size(), new_header_bytes);
        CHECK_EQ(body_data.size(), new_body_bytes);
        CHECK_EQ(new_header_bytes, header_bytes);
        CHECK_EQ(new_body_bytes, body_bytes);
      }
    }
 ...
 private:
  ...
  size_t StartNewHeapDumpSegment() {
