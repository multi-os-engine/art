...
class EndianOutput {
 public:
  ...
  void EndRecord() {
    ...
    if (started_) {
      UpdateU4(sizeof(uint8_t) + sizeof(uint32_t), length_);
    }
...
class EndianOutputBuffered : public EndianOutput {
 ...
 protected:
  ...
  void HandleU4List(const uint32_t* values, size_t count) OVERRIDE {
    DCHECK_EQ(length_, buffer_.size());
    EnsureSpace(sizeof(uint32_t) * count);
    for (size_t i = 0; i < count; ++i) {
  ...
  void HandleU8List(const uint64_t* values, size_t count) OVERRIDE {
    DCHECK_EQ(length_, buffer_.size());
    EnsureSpace(sizeof(uint64_t) * count);
    for (size_t i = 0; i < count; ++i) {
  ...
  void EnsureSpace(size_t bytes) {
    if (length_ + bytes > buffer_.capacity()) {
      const size_t new_buffer_size = std::max(buffer_.capacity() * 2, length_ + bytes);
      buffer_.reserve(new_buffer_size);
    }
  }
...
class Hprof {
  ...
  void Dump()
    ...
    if (okay) {
      ...
      LOG(INFO) << "hprof: heap dump completed ("
          << PrettySize(RoundUp(current_output_->SumLength(), 1024))
 ...
 private:
  ...
  void ProcessHeap(bool header_first) EXCLUSIVE_LOCKS_REQUIRED(Locks::mutator_lock_)
  ...
  void ProcessBody() EXCLUSIVE_LOCKS_REQUIRED(Locks::mutator_lock_)
    ...
    current_output_->StartNewRecord(HPROF_TAG_HEAP_DUMP_SEGMENT, kHprofTime);
    runtime->VisitRoots(RootVisitor, this);
    runtime->GetHeap()->VisitObjects(VisitObjectCallback, this);
    current_output_->StartNewRecord(HPROF_TAG_HEAP_DUMP_END, kHprofTime);
    current_output_->EndRecord();
  }

  void ProcessHeader() EXCLUSIVE_LOCKS_REQUIRED(Locks::mutator_lock_) {
    // Write the header.
    WriteFixedHeader(current_output_);
    ...
    WriteStringTable(current_output_);
    WriteClassTable(current_output_);
    WriteStackTraces(current_output_);
    current_output_->EndRecord();
  }
  ...
  void WriteClassTable(EndianOutput* output) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    ...
    for (mirror::Class* c : classes_) {
      ...
      output->AddU4(nextSerialNumber++);
      output->AddObjectId(c);
      output->AddU4(kHprofNullStackTrace);
      output->AddStringId(LookupClassNameId(c));
    }
  }
  ...
  void WriteStringTable(EndianOutput* output) {
    for (const std::pair<std::string, HprofStringId>& p : strings_) {
      ...
      output->AddU4(id);
      output->AddUtf8String(string.c_str());
    }
  }
  ...
  void WriteFixedHeader(EndianOutput* output) {
    ...
    output->AddU1List(reinterpret_cast<const uint8_t*>(magic), sizeof(magic));
    ...
    output->AddU4(static_cast<uint32_t>(nowMs >> 32));
    // U4: low word of the 64-bit time.
    output->AddU4(static_cast<uint32_t>(nowMs & 0xFFFFFFFF));
  }
  ...
  void WriteStackTraces(EndianOutput* output) {
    ...
    output->AddU4(kHprofNullStackTrace);
    output->AddU4(kHprofNullThread);
    output->AddU4(0);    // no frames
  }
...
int Hprof::MarkRootObject(const mirror::Object* obj, jobject jniObj) {
  ...
  if (objects_in_segment_ >= kMaxObjectsPerSegment ||
      current_output_->Length() >= kMaxBytesPerSegment) {
  ...
  case HPROF_ROOT_VM_INTERNAL:
    current_output_->AddU1(heapTag);
    current_output_->AddObjectId(obj);
    break;
  ...
  case HPROF_ROOT_JNI_GLOBAL:
    current_output_->AddU1(heapTag);
    current_output_->AddObjectId(obj);
    current_output_->AddJniGlobalRefId(jniObj);
    break;
  ...
  case HPROF_ROOT_JAVA_FRAME:
    current_output_->AddU1(heapTag);
    current_output_->AddObjectId(obj);
    current_output_->AddU4(gc_thread_serial_number_);
    current_output_->AddU4((uint32_t)-1);
    break;
  ...
  case HPROF_ROOT_THREAD_OBJECT:
    ...
    current_output_->AddObjectId(obj);
    current_output_->AddU4(gc_thread_serial_number_);
    current_output_->AddU4((uint32_t)-1);    // xxx
    break;
...
int Hprof::DumpHeapObject(mirror::Object* obj) {
  ...
  if (heap_type != current_heap_) {
    ...
    current_output_->AddU1(HPROF_HEAP_DUMP_INFO);
    current_output_->AddU4(static_cast<uint32_t>(heap_type));   // uint32_t: heap type
    switch (heap_type) {
    case HPROF_HEAP_APP:
    ...
    current_output_->AddStringId(nameId);
  ...
  if (c == NULL) {
    ...
    if (obj->IsClass()) {
      ...
      if (sFieldCount != 0) {
        ...
        current_output_->AddU1(HPROF_PRIMITIVE_ARRAY_DUMP);
        current_output_->AddClassStaticsId(thisClass);
        current_output_->AddU4(StackTraceSerialNumber(obj));
        current_output_->AddU4(byteLength);
        current_output_->AddU1(hprof_basic_byte);
        for (int i = 0; i < byteLength; ++i) {
          current_output_->AddU1(0);
        }
      }

      current_output_->AddU1(HPROF_CLASS_DUMP);
      current_output_->AddClassId(LookupClassId(thisClass));
      current_output_->AddU4(StackTraceSerialNumber(thisClass));
      current_output_->AddClassId(LookupClassId(thisClass->GetSuperClass()));
      current_output_->AddObjectId(thisClass->GetClassLoader());
      current_output_->AddObjectId(nullptr);    // no signer
      current_output_->AddObjectId(nullptr);    // no prot domain
      current_output_->AddObjectId(nullptr);    // reserved
      current_output_->AddObjectId(nullptr);    // reserved
      if (thisClass->IsClassClass()) {
        // ClassObjects have their static fields appended, so aren't all the same size.
        // But they're at least this size.
        current_output_->AddU4(sizeof(mirror::Class));  // instance size
      } else if (thisClass->IsArrayClass() || thisClass->IsPrimitive()) {
        current_output_->AddU4(0);
      } else {
        current_output_->AddU4(thisClass->GetObjectSize());  // instance size
      }

      current_output_->AddU2(0);  // empty const pool

      // Static fields
      if (sFieldCount == 0) {
        current_output_->AddU2((uint16_t)0);
      } else {
        current_output_->AddU2((uint16_t)(sFieldCount+1));
        current_output_->AddStringId(LookupStringId(STATIC_OVERHEAD_NAME));
        current_output_->AddU1(hprof_basic_object);
        current_output_->AddClassStaticsId(thisClass);
        ...
        for (size_t i = 0; i < sFieldCount; ++i) {
          ...
          current_output_->AddStringId(LookupStringId(f->GetName()));
          current_output_->AddU1(t);
          if (size == 1) {
            current_output_->AddU1(static_cast<uint8_t>(f->Get32(thisClass)));
          } else if (size == 2) {
            current_output_->AddU2(static_cast<uint16_t>(f->Get32(thisClass)));
          } else if (size == 4) {
            current_output_->AddU4(f->Get32(thisClass));
          } else if (size == 8) {
            current_output_->AddU8(f->Get64(thisClass));
          } else {
            CHECK(false);
          }
        }
      ...
      current_output_->AddU2((uint16_t)iFieldCount);
      ...
      if (obj->IsObjectArray()) {
        ...
        current_output_->AddU1(HPROF_OBJECT_ARRAY_DUMP);

        current_output_->AddObjectId(obj);
        current_output_->AddU4(StackTraceSerialNumber(obj));
        current_output_->AddU4(length);
        current_output_->AddClassId(LookupClassId(c));

        // Dump the elements, which are always objects or NULL.
        current_output_->AddIdList(aobj->AsObjectArray<mirror::Object>());
      } else {
        size_t size;
        ...
        if (size == 1) {
          current_output_->AddU1List((const uint8_t*)aobj->GetRawData(sizeof(uint8_t), 0), length);
        } else if (size == 2) {
          current_output_->AddU2List((const uint16_t*)aobj->GetRawData(sizeof(uint16_t), 0),
                                     length);
        } else if (size == 4) {
          current_output_->AddU4List((const uint32_t*)aobj->GetRawData(sizeof(uint32_t), 0),
                                     length);
        } else if (size == 8) {
          current_output_->AddU8List((const uint64_t*)aobj->GetRawData(sizeof(uint64_t), 0),
                                     length);
        }
      }
    } else {
      // obj is an instance object.
      current_output_->AddU1(HPROF_INSTANCE_DUMP);
      current_output_->AddObjectId(obj);
      current_output_->AddU4(StackTraceSerialNumber(obj));
      current_output_->AddClassId(LookupClassId(c));
      ...
      size_t size_patch_offset = current_output_->Length();
      current_output_->AddU4(0x77777777);
      ...
      while (!sclass->IsObjectClass()) {
        ...
        for (int i = 0; i < ifieldCount; ++i) {
          ...
          if (size == 1) {
            current_output_->AddU1(f->Get32(obj));
          } else if (size == 2) {
            current_output_->AddU2(f->Get32(obj));
          } else if (size == 4) {
            current_output_->AddU4(f->Get32(obj));
          } else {
            CHECK_EQ(size, 8U);
            current_output_->AddU8(f->Get64(obj));
          }
        }
      ...
      current_output_->UpdateU4(size_patch_offset,
                                current_output_->Length() - (size_patch_offset + 4));
    }
  }
