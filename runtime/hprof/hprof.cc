 ...
 private:
  ...
  HprofStackTraceSerialNumber LookupStackTraceSerialNumber(const mirror::Object* obj)
    ...
    if (r == allocation_records_.end()) {
      ...
      gc::AllocRecordStackTrace* trace = r->second;
  ...
  void PopulateAllocationTrackingTraces()
    ...
    for (auto it = records->Begin(), end = records->End(); it != end; ++it) {
      ...
      gc::AllocRecordStackTrace* trace = it->second->GetStackTrace();
  ...
  std::unordered_map<gc::AllocRecordStackTrace*, HprofStackTraceSerialNumber> traces_;
  std::unordered_map<gc::AllocRecordStackTraceElement*, HprofStackFrameId> frames_;
  std::unordered_map<const mirror::Object*, gc::AllocRecordStackTrace*> allocation_records_;
