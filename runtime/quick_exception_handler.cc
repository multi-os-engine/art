...
void QuickExceptionHandler::FindCatch(const ThrowLocation& throw_location,
  ...
  } else if (handler_method_ != nullptr && handler_dex_pc_ != DexFile::kDexNoIndex) {
    // We're catching this exception so we finish reporting it. We do it here to avid doing it
