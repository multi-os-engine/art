...
void QuickExceptionHandler::DeoptimizeSingleFrameArchDependentFixup() {
  // Architecture-dependent work. This is to get the LR (or corresponding value) right.
  ...
  // On arm, the return pc is stored in the LR register. Oddly (and only because Quick did it that
  // way), we mark the PC register as being spilled. That means the context contains in PC the
  // value we want in LR, so copy it over.
  // TODO: Conditionally include the arm register headers to get away from the raw numbers.
  context_->SetGPR(14, context_->GetGPR(15));
#endif
}
