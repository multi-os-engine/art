 ...
 public:
  ...
  bool VisitFrame() OVERRIDE SHARED_REQUIRES(Locks::mutator_lock_) {
    ...
    if (method == nullptr || single_frame_done_) {
      ...
      return true;
    } else if (!single_frame_deopt_ && !Runtime::Current()->IsDeoptimizeable(GetCurrentQuickFramePc())) {
      // We hit some code that's not deoptimizeable. However, Single-frame deoptimization triggered
      // from compiled code is always allowed since HDeoptimize always guarantees saved environment.
      FinishStackWalk();
      return false;  // End stack walk.
    } else {
