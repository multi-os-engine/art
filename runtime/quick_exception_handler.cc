...
static VRegKind DexRegisterLocationKindToVRegKind(DexRegisterLocation::Kind kind) {
...
void QuickExceptionHandler::SetCatchEnvironmentForOptimizedHandler(StackVisitor* stack_visitor) {
  ...
  DCHECK(handler_method_ != nullptr);
  DCHECK(handler_method_->IsOptimized(sizeof(void*)));
  ...
  StackMap throw_stack_map = code_info.GetSafepointStackMapForNativePcOffset(
      stack_visitor->GetNativePcOffset(), encoding);
  ...
  for (uint16_t vreg = 0; vreg < number_of_vregs; ++vreg) {
    ...
    uint32_t vreg_value;
    VRegKind vreg_kind = DexRegisterLocationKindToVRegKind(
        throw_vreg_map.GetLocationKind(vreg, number_of_vregs, code_info, encoding));
