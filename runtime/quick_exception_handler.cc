...
class CatchPhiStackVisitor FINAL : public StackVisitor {
 public:
  CatchPhiStackVisitor(Thread* self, Context* context, QuickExceptionHandler* exception_handler)
      ...
      : StackVisitor(self, context, StackVisitor::StackWalkKind::kIncludeInlinedFrames),
        exception_handler_(exception_handler),
        num_vregs_(exception_handler_->GetHandlerMethod()->GetCodeItem()->registers_size_),
 ...
 private:
  DexRegisterMap FindThrowVRegMap() SHARED_REQUIRES(Locks::mutator_lock_) {
    ...
    StackMap stack_map = code_info_.GetStackMapForNativePcOffset(GetNativePcOffset(), stack_map_encoding_);
    DCHECK(stack_map.IsValid());
    return code_info_.GetDexRegisterMapOf(stack_map, stack_map_encoding_, num_vregs_);
  }
  ...
  bool HasCatchPhiForVReg(size_t vreg, const DexRegisterMap& catch_vreg_map) {
    DexRegisterLocation::Kind location_kind =
        catch_vreg_map.GetLocationKind(vreg, num_vregs_, code_info_, stack_map_encoding_);
  ...
  DexRegisterLocation::Kind GetVRegLocationKind(uint16_t vreg, const DexRegisterMap& vreg_map) {
    return vreg_map.GetLocationKind(vreg, num_vregs_, code_info_, stack_map_encoding_);
  }
  ...
  void SetCatchPhiSlots() SHARED_REQUIRES(Locks::mutator_lock_) {
    ...
    for (uint16_t vreg = 0; vreg < num_vregs_; ++vreg) {
      if (HasCatchPhiForVReg(vreg, catch_vreg_map)) {
        ...
        int32_t slot_offset =
            catch_vreg_map.GetStackOffsetInBytes(vreg, num_vregs_, code_info_, stack_map_encoding_);
...
void QuickExceptionHandler::SetCatchPhiValues() {
  ...
  if (*GetHandlerQuickFrame() == nullptr) {
    // This is an upcall exception and will be redelivered by the runtime.
    return;
  }
