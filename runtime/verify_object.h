...
enum VerifyObjectMode {
  ...
  kVerifyObjectModeFast,  // Sanity heap accesses quickly.
  kVerifyObjectModeAll  // Sanity heap accesses thoroughly.
};

// Verify self when we are doing an operation.
static constexpr size_t kVerifyThis = 0x1;
// Verify reads from objects.
static constexpr size_t kVerifyReads = 0x2;
// Verify writes to objects.
static constexpr size_t kVerifyWrites = 0x4;
static constexpr size_t kVerifyAll = kVerifyThis | kVerifyReads | kVerifyWrites;

static constexpr bool kVerifyStack = kIsDebugBuild;
static constexpr size_t kDefaultVerifyFlags = 0;
