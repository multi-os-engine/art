...
TEST_F(ObjectTest, AllocObjectArray) {
  ...
  EXPECT_EQ(aioobe, soa.Self()->GetException(nullptr)->GetClass());
...
TEST_F(ObjectTest, CreateMultiArray) {
  ...
  EXPECT_TRUE(soa.Self()->IsExceptionPending());
  EXPECT_EQ(PrettyDescriptor(soa.Self()->GetException(nullptr)->GetClass()),
...
TEST_F(ObjectTest, FindStaticField) {
  ...
  EXPECT_TRUE(mirror::Class::FindStaticField(soa.Self(), c, "CASE_INSENSITIVE_ORDER", "I") == nullptr);

  // Wrong name.
  EXPECT_TRUE(c->FindDeclaredStaticField("cASE_INSENSITIVE_ORDER", "Ljava/util/Comparator;") == nullptr);
  EXPECT_TRUE(
  ...
  EXPECT_TRUE(f1.Get() != nullptr);
  EXPECT_TRUE(f2.Get() != nullptr);
