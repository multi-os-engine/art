...
TEST_F(ObjectTest, Constants) {
  ...
  EXPECT_EQ(ART_METHOD_QUICK_CODE_OFFSET_32,
            ArtMethod::EntryPointFromQuickCompiledCodeOffset(PointerSize::kPointerSize32).
                Int32Value());
  EXPECT_EQ(ART_METHOD_QUICK_CODE_OFFSET_64,
            ArtMethod::EntryPointFromQuickCompiledCodeOffset(PointerSize::kPointerSize64).
                Int32Value());
}
...
TEST_F(ObjectTest, CheckAndAllocArrayFromCode) {
  ...
  ArtMethod* sort = java_util_Arrays->FindDirectMethod("sort", "([I)V", kPointerSizeCur);
...
TEST_F(ObjectTest, DescriptorCompare) {
  ...
  ArtMethod* m1_1 = klass1->GetVirtualMethod(0, kPointerSizeCur);
  EXPECT_STREQ(m1_1->GetName(), "m1");
  ArtMethod* m2_1 = klass1->GetVirtualMethod(1, kPointerSizeCur);
  EXPECT_STREQ(m2_1->GetName(), "m2");
  ArtMethod* m3_1 = klass1->GetVirtualMethod(2, kPointerSizeCur);
  EXPECT_STREQ(m3_1->GetName(), "m3");
  ArtMethod* m4_1 = klass1->GetVirtualMethod(3, kPointerSizeCur);
  EXPECT_STREQ(m4_1->GetName(), "m4");

  ArtMethod* m1_2 = klass2->GetVirtualMethod(0, kPointerSizeCur);
  EXPECT_STREQ(m1_2->GetName(), "m1");
  ArtMethod* m2_2 = klass2->GetVirtualMethod(1, kPointerSizeCur);
  EXPECT_STREQ(m2_2->GetName(), "m2");
  ArtMethod* m3_2 = klass2->GetVirtualMethod(2, kPointerSizeCur);
  EXPECT_STREQ(m3_2->GetName(), "m3");
  ArtMethod* m4_2 = klass2->GetVirtualMethod(3, kPointerSizeCur);
