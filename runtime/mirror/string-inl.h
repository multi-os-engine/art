...
class SetStringCountAndBytesVisitor {
 public:
  explicit SetStringCountAndBytesVisitor(int32_t count, uint8_t* src, int32_t high_byte)
...
class SetStringCountAndValueVisitor {
 public:
  explicit SetStringCountAndValueVisitor(int32_t count, uint16_t* src) : count_(count), src_(src) {
  }
...
inline uint16_t String::CharAt(int32_t index) {
  ...
  if (count_ < 200) {
    ...
    buffer[count_] = 0;
    LOG(INFO) << StringPrintf("Calling CharAt - '%s' index:%d count:%d", buffer, index, count_);
  } else {
    LOG(INFO) << StringPrintf("Calling CharAt - index:%d count:%d", index, count_);
  }
  */
