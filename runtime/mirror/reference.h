...
class MANAGED Reference : public Object {
 public:
  ...
  // Volatile read/write is not necessary here since the java pending next is only accessed from
  // the java threads for cleared references. And since cleared references have a null referent, we
  // never end up looking at their pending next from the GC.
