...
class PointerArray : public Array {
 public:
  ...
  T GetElementPtrSize(uint32_t idx, size_t ptr_size)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    if (ptr_size == 8) {
      return reinterpret_cast<T>(static_cast<uintptr_t>(AsLongArray()->GetWithoutChecks(idx)));
    }
    DCHECK_EQ(ptr_size, 4u);
    return reinterpret_cast<T>(static_cast<uintptr_t>(AsIntArray()->GetWithoutChecks(idx)));
  }
  ...
  void SetElementPtrSize(uint32_t idx, T element, size_t ptr_size)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    if (ptr_size == 8) {
      AsLongArray()->SetWithoutChecks<false>(idx, reinterpret_cast<uint64_t>(element));
    } else {
      DCHECK_EQ(ptr_size, 4u);
      DCHECK_LE(reinterpret_cast<uintptr_t>(element), 0xFFFFFFFF);
      AsIntArray()->SetWithoutChecks<false>(idx, static_cast<uint32_t>(reinterpret_cast<uintptr_t>(element)));
    }
  }
};
