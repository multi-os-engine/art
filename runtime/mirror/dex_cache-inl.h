...
inline Class* DexCache::GetResolvedType(uint32_t type_idx) {
  // TODO: Make sure the bounds check can never trigger and replace it with a DCHECK_LT(...).
  if (UNLIKELY(type_idx >= NumResolvedTypes())) {
    ThrowArrayIndexOutOfBoundsException(type_idx, NumResolvedTypes());
    return nullptr;
  }
...
inline ArtField* DexCache::GetResolvedField(uint32_t field_idx, size_t ptr_size) {
  ...
  auto* field = GetElementPtrSize(GetResolvedFields(), field_idx, ptr_size);
...
inline ArtMethod* DexCache::GetResolvedMethod(uint32_t method_idx, size_t ptr_size) {
  ...
  auto* method = GetElementPtrSize<ArtMethod*>(GetResolvedMethods(), method_idx, ptr_size);
