...
class MANAGED ArtMethod : public Object {
 public:
  ...
  void SetAccessFlags(uint32_t new_access_flags) {
    ...
    SetField32NonTransactional(OFFSET_OF_OBJECT_MEMBER(ArtMethod, access_flags_), new_access_flags,
                               false);
  }
  ...
  void SetMethodIndex(uint16_t new_method_index) {
    // Not called within a transaction.
    SetField32NonTransactional(OFFSET_OF_OBJECT_MEMBER(ArtMethod, method_index_), new_method_index,
                               false);
  }
  ...
  void SetCodeItemOffset(uint32_t new_code_off) {
    // Not called within a transaction.
    SetField32NonTransactional(OFFSET_OF_OBJECT_MEMBER(ArtMethod, code_item_offset_), new_code_off,
                               false);
  }
  ...
  void SetDexMethodIndex(uint32_t new_idx) {
    // Not called within a transaction.
    SetField32NonTransactional(OFFSET_OF_OBJECT_MEMBER(ArtMethod, method_dex_index_), new_idx,
                               false);
  }
  ...
  void SetEntryPointFromInterpreter(EntryPointFromInterpreter* entry_point_from_interpreter) {
    SetFieldPtrNonTransactional<EntryPointFromInterpreter*>(
  ...
  void SetEntryPointFromCompiledCode(const void* entry_point_from_compiled_code) {
    SetFieldPtrNonTransactional<const void*>(
        OFFSET_OF_OBJECT_MEMBER(ArtMethod, entry_point_from_compiled_code_),
        entry_point_from_compiled_code, false);
  }
  ...
  void SetMappingTable(const uint8_t* mapping_table) {
    SetFieldPtrNonTransactional<const uint8_t*>(OFFSET_OF_OBJECT_MEMBER(ArtMethod, mapping_table_),
        mapping_table, false);
  }
  ...
  void SetVmapTable(const uint8_t* vmap_table) {
    SetFieldPtrNonTransactional<const uint8_t*>(OFFSET_OF_OBJECT_MEMBER(ArtMethod, vmap_table_),
        vmap_table, false);
  }
  ...
  void SetFrameSizeInBytes(size_t new_frame_size_in_bytes) {
    DCHECK_EQ(sizeof(size_t), sizeof(uint32_t));
    // Not called within a transaction.
    SetField32NonTransactional(OFFSET_OF_OBJECT_MEMBER(ArtMethod, frame_size_in_bytes_),
  ...
  void SetCoreSpillMask(uint32_t core_spill_mask) {
    ...
    // Not called within a transaction.
    SetField32NonTransactional(OFFSET_OF_OBJECT_MEMBER(ArtMethod, core_spill_mask_), core_spill_mask,
                               false);
  }
  ...
  void SetFpSpillMask(uint32_t fp_spill_mask) {
    // Computed during compilation.
    // Not called within a transaction.
    SetField32NonTransactional(OFFSET_OF_OBJECT_MEMBER(ArtMethod, fp_spill_mask_), fp_spill_mask,
                               false);
  }
