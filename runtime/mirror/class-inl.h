...
inline MemberOffset Class::EmbeddedImTableEntryOffset(uint32_t i, size_t pointer_size) {
  return MemberOffset(EmbeddedImTableOffset().Uint32Value() + i * ImTableEntrySize(pointer_size));
}

inline ArtMethod* Class::GetEmbeddedImTableEntry(uint32_t i, size_t pointer_size) {
  DCHECK(ShouldHaveEmbeddedImtAndVTable());
  DCHECK_LT(i, static_cast<uint32_t>(GetEmbeddedVTableLength()));
...
inline uint32_t Class::ComputeClassSize(bool has_embedded_tables,
  ...
  if (has_embedded_tables) {
    ...
    size = RoundUp(size + sizeof(int32_t) /* vtable len */, 8) + embedded_imt_size +
...
inline MemberOffset Class::EmbeddedImTableOffset() {
  // Round up since we want the embedded imt and vtable to be 8 byte aligned in case 64 bits.
  // Add 32 bits for embedded vtable length.
  return MemberOffset(RoundUp(EmbeddedVTableLengthOffset().Uint32Value() + sizeof(uint32_t), 8u));
}

inline MemberOffset Class::EmbeddedVTableOffset(size_t pointer_size) {
  // TODO: Alignment probably broken on 64 bits?
  return MemberOffset(
      EmbeddedImTableOffset().Uint32Value() + kImtSize * ImTableEntrySize(pointer_size));
}
