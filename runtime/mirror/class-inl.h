...
inline ImTable* Class::GetImt(size_t pointer_size) {
  return GetFieldPtrWithSize<ImTable*>(MemberOffset(ImtPtrOffset()), pointer_size);
}

inline void Class::SetImt(ImTable* imt, size_t pointer_size) {
  return SetFieldPtrWithSize<false>(MemberOffset(ImtPtrOffset()), imt, pointer_size);
}
...
inline MemberOffset Class::EmbeddedVTableOffset(size_t pointer_size) {
  CheckPointerSize(pointer_size);
  return MemberOffset(
      RoundUp(ImtPtrOffset().Uint32Value() + pointer_size, pointer_size));
}
