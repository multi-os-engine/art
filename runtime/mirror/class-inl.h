...
inline void Class::SetIFields(ArtField* new_ifields) {
  if (new_ifields != nullptr) {
    DCHECK(GetFieldPtr<ArtField*>(OFFSET_OF_OBJECT_MEMBER(Class, ifields_)) == nullptr);
  }
...
void mirror::Class::VisitFieldRoots(Visitor& visitor) {
  ArtField* const sfields = GetSFields();
  ...
  ArtField* const ifields = GetIFields();
