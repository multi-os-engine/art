...
void mirror::Class::VisitNativeRoots(Visitor& visitor, size_t image_pointer_size) {
  ...
  for (auto& m : GetDirectMethods(image_pointer_size)) {
    m.VisitRoots(visitor);
  }
  for (auto& m : GetVirtualMethods(image_pointer_size)) {
    m.VisitRoots(visitor);
  }
}
