...
inline IterationRange<StrideIterator<ArtMethod>> Class::GetDirectMethods(size_t pointer_size) {
  CheckPointerSize(pointer_size);
  return MakeIterationRangeFromArraySlice(GetDirectMethodsSliceUnchecked(pointer_size));
}

inline IterationRange<StrideIterator<ArtMethod>> Class::GetDeclaredMethods(
      size_t pointer_size) {
  CheckPointerSize(pointer_size);
  return MakeIterationRangeFromArraySlice(GetDeclaredMethodsSliceUnchecked(pointer_size));
}

inline IterationRange<StrideIterator<ArtMethod>> Class::GetDeclaredVirtualMethods(
      size_t pointer_size) {
  CheckPointerSize(pointer_size);
  return MakeIterationRangeFromArraySlice(GetDeclaredVirtualMethodsSliceUnchecked(pointer_size));
}

inline IterationRange<StrideIterator<ArtMethod>> Class::GetVirtualMethods(size_t pointer_size) {
  CheckPointerSize(pointer_size);
  return MakeIterationRangeFromArraySlice(GetVirtualMethodsSliceUnchecked(pointer_size));
}

inline IterationRange<StrideIterator<ArtMethod>> Class::GetCopiedMethods(size_t pointer_size) {
  CheckPointerSize(pointer_size);
  return MakeIterationRangeFromArraySlice(GetCopiedMethodsSliceUnchecked(pointer_size));
}
