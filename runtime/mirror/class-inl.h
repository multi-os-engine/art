...
inline ArtMethod* Class::GetDirectMethodsPtr() {
  ...
  return GetDirectMethodsUnchecked();
}

inline ArtMethod* Class::GetDirectMethodsUnchecked() {
...
inline void Class::SetDirectMethodsPtr(ArtMethod* new_direct_methods) {
  DCHECK(GetDirectMethodsUnchecked() == nullptr);
...
inline ArtMethod* Class::GetVirtualMethodUnchecked(size_t i, size_t pointer_size) {
  auto* methods = GetVirtualMethodsPtr();
  DCHECK(methods != nullptr);
  return reinterpret_cast<ArtMethod*>(reinterpret_cast<uintptr_t>(methods) +
      ArtMethod::ObjectSize(pointer_size) * i);
}
...
inline ArtMethod* Class::GetEmbeddedImTableEntry(uint32_t i, size_t pointer_size) {
  const auto offset = EmbeddedImTableOffset().Uint32Value() + i * ImTableEntrySize(pointer_size);
...
inline void Class::SetEmbeddedImTableEntry(uint32_t i, ArtMethod* method, size_t pointer_size) {
  const auto offset = EmbeddedImTableOffset().Uint32Value() + i * ImTableEntrySize(pointer_size);
  SetFieldPtrWithSize<false>(MemberOffset(offset), method, pointer_size);
}
...
inline int32_t Class::GetVTableLength(size_t pointer_size) {
  if (ShouldHaveEmbeddedImtAndVTable()) {
    return GetEmbeddedVTableLength(pointer_size);
  }
...
inline int32_t Class::GetEmbeddedVTableLength(size_t pointer_size) {
  return GetField32(EmbeddedVTableLengthOffset(pointer_size));
}

inline void Class::SetEmbeddedVTableLength(int32_t len, size_t pointer_size) {
  SetField32<false>(EmbeddedVTableLengthOffset(pointer_size), len);
}

inline ArtMethod* Class::GetEmbeddedVTableEntry(uint32_t i, size_t pointer_size) {
  const auto offset = EmbeddedVTableOffset(pointer_size).Uint32Value() +
      i * VTableEntrySize(pointer_size);
  return GetFieldPtrWithSize<ArtMethod*>(MemberOffset(offset), pointer_size);
}
...
inline void Class::SetEmbeddedVTableEntry(uint32_t i, ArtMethod* method, size_t pointer_size) {
  const auto offset = EmbeddedVTableOffset(pointer_size).Uint32Value() +
      i * VTableEntrySize(pointer_size);
  SetFieldPtrWithSize<false>(MemberOffset(offset), method, pointer_size);
  CHECK_EQ(method, GetVirtualMethodDuringLinking(i, pointer_size));
}
...
inline MemberOffset Class::GetFirstReferenceStaticFieldOffset(size_t pointer_size) {
  ...
  if (ShouldHaveEmbeddedImtAndVTable()) {
    ...
    base = mirror::Class::ComputeClassSize(true, GetEmbeddedVTableLength(pointer_size),
...
inline uint32_t Class::ComputeClassSize(bool has_embedded_tables,
  ...
  if (has_embedded_tables) {
    uint32_t embedded_imt_size = kImtSize * ImTableEntrySize(pointer_size);
    uint32_t embedded_vtable_size = num_vtable_entries * VTableEntrySize(pointer_size);
...
void mirror::Class::VisitNativeRoots(Visitor& visitor) {
...
inline IteratorPair<StrideIterator<ArtMethod>> Class::GetDirectMethods(size_t pointer_size) {
  return MakeIteratorPair(DirectMethodsBegin(pointer_size), DirectMethodsEnd(pointer_size));
}
...
inline IteratorPair<StrideIterator<ArtMethod>> Class::GetVirtualMethods(size_t pointer_size) {
  return MakeIteratorPair(VirtualMethodsBegin(pointer_size), VirtualMethodsEnd(pointer_size));
}
