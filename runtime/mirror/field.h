...
class MANAGED Field : public AccessibleObject {
 public:
  ...
  static mirror::Field* CreateFromArtField(mirror::ArtField* field)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);

 private:
  HeapReference<mirror::Class> declaring_class_;
  HeapReference<mirror::String> name_;
  ...
  void SetDeclaringClass(mirror::Class* c) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    SetFieldObject<false>(OFFSET_OF_OBJECT_MEMBER(Field, declaring_class_), c);
  }

  void SetName(mirror::String* name) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);

  void SetType(mirror::Class* type) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    SetFieldObject<false>(OFFSET_OF_OBJECT_MEMBER(Field, type_), type);
  }

  void SetAccessFlags(uint32_t flags) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    SetField32<false>(OFFSET_OF_OBJECT_MEMBER(Field, access_flags_), flags);
  }

  void SetDexFieldIndex(uint32_t idx) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    SetField32<false>(OFFSET_OF_OBJECT_MEMBER(Field, dex_field_index_), idx);
  }

  void SetOffset(uint32_t offset) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    SetField32<false>(OFFSET_OF_OBJECT_MEMBER(Field, offset_), offset);
  }
