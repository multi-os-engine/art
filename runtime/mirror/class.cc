...
void Class::PopulateEmbeddedImt(ArtMethod** methods, size_t pointer_size) {
...
class CopyClassVisitor {
 public:
  CopyClassVisitor(Thread* self, Handle<mirror::Class>* orig, size_t new_length,
                   size_t copy_bytes, ArtMethod** imt,
                   size_t pointer_size)
  ...
  void operator()(mirror::Object* obj, size_t usable_size ATTRIBUTE_UNUSED) const
    ...
    if (imt_ != nullptr) {
      ArtMethod** imt_address = Class::ComputeEmbeddedImtAddr(h_new_class_obj.Get(), pointer_size_);
      h_new_class_obj->SetEmbeddedImtPtr(imt_address, pointer_size_);
...
Class* Class::CopyOf(Thread* self, int32_t new_length,
                     ArtMethod** imt, size_t pointer_size) {
