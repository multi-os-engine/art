...
class ReadBarrierOnNativeRootsVisitor {
 public:
  ...
  void VisitRoot(mirror::CompressedReference<mirror::Object>* root) const
      SHARED_REQUIRES(Locks::mutator_lock_) {
    mirror::CompressedReference<mirror::Object> old_ref = *root;
    mirror::Object* ref = ReadBarrier::BarrierForRoot(&old_ref);
    // Update the field atomically. This may fail if mutator updates before us, but it's ok.
    if (old_ref.AsMirrorPtr() != ref) {
      auto new_ref = mirror::CompressedReference<mirror::Object>::FromMirrorPtr(ref);
      // Update the field atomically. This may fail if mutator updates before us, but it's ok.
      auto* atomic_root =
