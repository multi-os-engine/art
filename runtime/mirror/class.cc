...
void Class::PopulateEmbeddedImtAndVTable(ArtMethod** methods, size_t pointer_size) {
  ...
  SetEmbeddedVTableLength(table_length, pointer_size);
...
class CopyClassVisitor {
 public:
  explicit CopyClassVisitor(Thread* self, Handle<mirror::Class>* orig, size_t new_length,
                            size_t copy_bytes, ArtMethod** imt, size_t pointer_size)
 ...
 private:
  ...
  ArtMethod** const imt_;
  const size_t pointer_size_;
  DISALLOW_COPY_AND_ASSIGN(CopyClassVisitor);
};

Class* Class::CopyOf(Thread* self, int32_t new_length, ArtMethod** imt, size_t pointer_size) {
