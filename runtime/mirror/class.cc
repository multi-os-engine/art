...
void Class::PopulateEmbeddedImtAndVTable(ArtMethod* (&methods)[kImtSize], size_t pointer_size) {
...
class CopyClassVisitor {
 public:
  explicit CopyClassVisitor(Thread* self, Handle<mirror::Class>* orig, size_t new_length,
                            size_t copy_bytes, ArtMethod* (&imt)[mirror::Class::kImtSize],
                            size_t pointer_size)
 ...
 private:
  ...
  ArtMethod* (&imt_)[mirror::Class::kImtSize];
  const size_t pointer_size_;
  DISALLOW_COPY_AND_ASSIGN(CopyClassVisitor);
};

Class* Class::CopyOf(Thread* self, int32_t new_length, ArtMethod* (&imt)[mirror::Class::kImtSize],
                     size_t pointer_size) {
