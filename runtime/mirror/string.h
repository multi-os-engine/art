...
class MANAGED String : public Object {
 public:
  ...
  uint16_t* GetValue() SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    byte* raw_addr = reinterpret_cast<byte*>(this) + ValueOffset().Int32Value();
    return reinterpret_cast<uint16_t*>(raw_addr);
  }
  ...
  static String* AllocFromStrings(Thread* self, SirtRef<String>& string, SirtRef<String>& string2)
  ...
  static String* AllocFromByteArray(Thread* self, int32_t byte_length, SirtRef<ByteArray>& array,
                                    int32_t offset, int32_t high_byte = 0)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);
  ...
  void GetChars(int32_t start, int32_t end, SirtRef<CharArray>& array, int32_t index)
      SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);
