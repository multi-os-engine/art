...
JDWP::JdwpError Dbg::PrepareInvokeMethod(uint32_t request_id, JDWP::ObjectId thread_id,
                                         ...
                                         uint32_t options) {
  Thread* const self = Thread::Current();
  DCHECK_EQ(self, GetDebugThread()) << "This must be called by the JDWP thread";
...
void Dbg::ExecuteMethod(DebugInvokeReq* pReq) {
  Thread* const self = Thread::Current();
  DCHECK_NE(self, GetDebugThread()) << "This must be called by the event thread";
...
void Dbg::ExecuteMethodWithoutPendingException(ScopedObjectAccess& soa, DebugInvokeReq* pReq) {
  ...
  if (is_constructor) {
    ...
    result_value = (exceptionObjectId == 0) ? receiverObjectId : 0;
  }
  ...
  VLOG(jdwp) << "  --> returned " << result_tag
             << StringPrintf(" %#" PRIx64 " (except=%#" PRIx64 ")", result_value, exceptionObjectId);
...
void Dbg::FinishInvokeMethod(DebugInvokeReq* pReq) {
  DCHECK_NE(Thread::Current(), GetDebugThread()) << "This must be called by the event thread";
  ...
  VLOG(jdwp) << "REPLY INVOKE #0x" << std::hex << pReq->request_id << " " << result
             << " (length=" << std::dec << replyDataLength << ")";
