...
JDWP::JdwpError Dbg::InvokeMethod(JDWP::ObjectId thread_id, JDWP::ObjectId object_id,
  ...
  Thread* self = Thread::Current();
  {
    ...
    req.reset(new DebugInvokeReq(receiver, thread, c, m, options, arg_values, arg_count));
    if (req.get() == nullptr) {
...
void Dbg::ExecuteMethod(DebugInvokeReq* pReq) {
  ...
  JValue result = InvokeWithJValues(soa, pReq->receiver, soa.EncodeMethod(m.Get()),
  ...
  if (pReq->exception != 0) {
    ...
    /*
     * Register an ObjectId for the result object now to be GC safe.
     */
