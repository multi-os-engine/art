...
std::string Dbg::GetMethodName(JDWP::MethodId method_id) {
  ...
  return m->GetInterfaceMethodIfProxy(kPointerSizeCur)->GetName();
}
...
JDWP::JdwpError Dbg::OutputDeclaredMethods(JDWP::RefTypeId class_id, bool with_generic,
  ...
  for (ArtMethod& m : c->GetMethods(ptr_size)) {
    ...
    expandBufAddUtf8String(pReply, m.GetInterfaceMethodIfProxy(kPointerSizeCur)->GetName());
    expandBufAddUtf8String(pReply,
                           m.GetInterfaceMethodIfProxy(kPointerSizeCur)->GetSignature().ToString());
    if (with_generic) {
...
void Dbg::VisitRoots(RootVisitor* visitor) {
  ...
  for (Breakpoint& breakpoint : gBreakpoints) {
    breakpoint.Method()->VisitRoots(root_visitor, kPointerSizeCur);
  }
}
