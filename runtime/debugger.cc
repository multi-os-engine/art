...
class DebugInstrumentationListener FINAL : public instrumentation::InstrumentationListener {
 public:
  ...
  void MethodUnwind(Thread* thread, mirror::Object* this_object, ArtMethod* method,
                    uint32_t dex_pc)
      OVERRIDE SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  ...
  void FieldRead(Thread* thread, mirror::Object* this_object, ArtMethod* method,
...
JDWP::JdwpError Dbg::OutputDeclaredMethods(JDWP::RefTypeId class_id, bool with_generic,
  ...
  for (size_t i = 0; i < direct_method_count + virtual_method_count; ++i) {
    ArtMethod* m = (i < direct_method_count) ? c->GetDirectMethod(i) : c->GetVirtualMethod(i - direct_method_count);
...
class CatchLocationFinder : public StackVisitor {
 public:
  CatchLocationFinder(Thread* self, const Handle<mirror::Throwable>& exception, Context* context)
    ...
    : StackVisitor(self, context),
      ...
      catch_method_(handle_scope_.NewHandle<ArtMethod>(nullptr)),
      throw_method_(handle_scope_.NewHandle<ArtMethod>(nullptr)),
  ...
  bool VisitFrame() OVERRIDE SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
    ...
    if (dex_pc != DexFile::kDexNoIndex) {
      ...
      Handle<ArtMethod> h_method(hs.NewHandle(method));
      bool unused_clear_exception;
      found_dex_pc = ArtMethod::FindCatchBlock(
 ...
 private:
  ...
  MutableHandle<ArtMethod> catch_method_;
  MutableHandle<ArtMethod> throw_method_;
...
static bool IsMethodPossiblyInlined(Thread* self, ArtMethod* m)
  ...
  Handle<ArtMethod> method(hs.NewHandle(m));
...
JDWP::JdwpError Dbg::InvokeMethod(JDWP::ObjectId thread_id, JDWP::ObjectId object_id,
  ...
  Thread* self = Thread::Current();
  {
...

    {
      ...
      HandleWrapper<ArtMethod> h_m(hs.NewHandleWrapper(&m));
...
void Dbg::ExecuteMethod(DebugInvokeReq* pReq) {
  ...
  MutableHandle<ArtMethod> m(hs.NewHandle(pReq->method.Read()));
  if ((pReq->options & JDWP::INVOKE_NONVIRTUAL) == 0 && pReq->receiver.Read() != nullptr) {
    ArtMethod* actual_method = pReq->klass.Read()->FindVirtualMethodForVirtualOrInterface(m.Get());
