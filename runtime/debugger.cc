...
std::vector<DeoptimizeRequest> Dbg::deoptimization_requests_;
...
// Process requests while all mutator threads are suspended.
void Dbg::ProcessDeoptimizationRequests() {
  Locks::mutator_lock_->AssertExclusiveHeld(Thread::Current());
  MutexLock mu(Thread::Current(), *deoptimization_lock_);
  instrumentation::Instrumentation* instrumentation = Runtime::Current()->GetInstrumentation();
  for (const DeoptimizeRequest& request : deoptimization_requests_) {
    switch (request.kind) {
      case DeoptimizeRequest::kNothing:
        LOG(WARNING) << "Processing empty deoptimization request: ignore it.";
        break;
      case DeoptimizeRequest::kFullDeoptimization:
        VLOG(jdwp) << "Deoptimize the world";
        instrumentation->DeoptimizeEverything();
        break;
      case DeoptimizeRequest::kFullUndeoptimization:
        VLOG(jdwp) << "Undeoptimize the world";
        instrumentation->UndeoptimizeEverything();
        break;
      case DeoptimizeRequest::kSelectiveDeoptimization:
        VLOG(jdwp) << "Deoptimize method " << PrettyMethod(request.method);
        instrumentation->Deoptimize(request.method);
        break;
      case DeoptimizeRequest::kSelectiveUndeoptimization:
...
void Dbg::PushRequest(const DeoptimizeRequest* req) {
  DCHECK(req != nullptr);
  if (req->kind == DeoptimizeRequest::kNothing) {
    // Nothing to do.
    return;
  }
  MutexLock mu(Thread::Current(), *deoptimization_lock_);
...
void Dbg::WatchLocation(const JDWP::JdwpLocation* location, DeoptimizeRequest* req) {
  ...
  if (need_deoptimization) {
    req->kind = DeoptimizeRequest::kSelectiveDeoptimization;
    req->method = m;
  }
}

void Dbg::UnwatchLocation(const JDWP::JdwpLocation* location, DeoptimizeRequest* req) {
  ...
  if (can_undeoptimize) {
    ...
    req->kind = DeoptimizeRequest::kSelectiveUndeoptimization;
    req->method = m;
  }
}
