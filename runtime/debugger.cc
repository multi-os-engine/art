    ...
    if (!instrumentation->InterpretOnly()) {
      if (!m->IsNative() && !m->IsProxyMethod()) {
        ...
        if (ssc->is_active) {
          ...
          if (ssc->step_depth != JDWP::SD_OUT && m->IsStatic() &&
        ...
        // point that has been replaced by the deoptimization, but instead by directy
    ...
    if (IsDebuggerActive()) {
      ...
      if (!instrumentation->InterpretOnly()) {
        if (!m->IsNative() && !m->IsProxyMethod()) {
          ...
          // stack deoptimization by calling IsForcedInterpreterNeededForCalling.
          return instrumentation->IsDeoptimized(m);
        }
      }
    }
