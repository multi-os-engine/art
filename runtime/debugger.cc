  ...
  struct SetLocalVisitor : public StackVisitor {
    ...
    bool VisitFrame() NO_THREAD_SAFETY_ANALYSIS {
      ...
      switch (tag_) {
        ...
        case JDWP::JT_DOUBLE: {
          CHECK_EQ(width_, 8U);
          bool success = SetVReg(m, reg, static_cast<uint32_t>(value_), kDoubleLoVReg);
          success &= SetVReg(m, reg + 1, static_cast<uint32_t>(value_ >> 32), kDoubleHiVReg);
          if (!success) {
            error_ = kFailureErrorCode;
          }
          break;
        ...
        case JDWP::JT_LONG: {
          CHECK_EQ(width_, 8U);
          bool success = SetVReg(m, reg, static_cast<uint32_t>(value_), kLongLoVReg);
          success &= SetVReg(m, reg + 1, static_cast<uint32_t>(value_ >> 32), kLongHiVReg);
          if (!success) {
            error_ = kFailureErrorCode;
          }
          break;
