...
class Breakpoint : public ValueObject {
 public:
  ...
  // same. IE they are either the same method or they are copied from the same method.
...
static DeoptimizationRequest::Kind GetRequiredDeoptimizationKind(Thread* self,
  ...
  if (first_breakpoint == nullptr) {
    ...
    // inlined or default, we deoptimize everything; otherwise we deoptimize only this method.
