...
void Dbg::DumpRecentAllocations() {
  ...
  for (auto it = records->RBegin(), end = records->REnd();
    ...
    LOG(INFO) << StringPrintf(" Thread %-2d %6zd bytes ", record->GetTid(), record->ByteCount())
              << PrettyClass(it->first->GetClass());

    for (size_t stack_frame = 0, depth = record->GetDepth(); stack_frame < depth; ++stack_frame) {
      gc::AllocRecordStackTraceElement* stack_element = record->StackElement(stack_frame);
...
jbyteArray Dbg::GetRecentAllocations() {
  ...
  std::vector<uint8_t> bytes;
  {
    ...
    if (records == nullptr) {
      ...
      records = new gc::AllocRecordObjectMap();
    }
    //
    ...
    for (auto it = records->RBegin(), end = records->REnd();
      ...
      gc::AllocRecord* record = it->second;
      std::string temp;
      class_names.Add(it->first->GetClass()->GetDescriptor(&temp));
    ...
    for (auto it = records->RBegin(), end = records->REnd();
      ...
      size_t allocated_object_class_name_index =
          class_names.IndexOf(it->first->GetClass()->GetDescriptor(&temp));
    ...
    if (Runtime::Current()->GetHeap()->GetAllocationRecords() != records) {
      CHECK(Runtime::Current()->GetHeap()->GetAllocationRecords() == nullptr);
