...
ReferenceCounter Dbg::full_deoptimization_event_count_;
size_t Dbg::delayed_full_undeoptimization_count_ = 0;

// Instrumentation event reference counters.
ReferenceCounter Dbg::dex_pc_change_event_ref_count_;
ReferenceCounter Dbg::method_enter_event_ref_count_;
ReferenceCounter Dbg::method_exit_event_ref_count_;
ReferenceCounter Dbg::field_read_event_ref_count_;
ReferenceCounter Dbg::field_write_event_ref_count_;
ReferenceCounter Dbg::exception_catch_event_ref_count_;
...
void Dbg::GoActive() {
...

  {
    ...
    CHECK_EQ(full_deoptimization_event_count_.GetCounter(), 0U);
    CHECK_EQ(delayed_full_undeoptimization_count_, 0U);
    CHECK_EQ(dex_pc_change_event_ref_count_.GetCounter(), 0U);
    CHECK_EQ(method_enter_event_ref_count_.GetCounter(), 0U);
    CHECK_EQ(method_exit_event_ref_count_.GetCounter(), 0U);
    CHECK_EQ(field_read_event_ref_count_.GetCounter(), 0U);
    CHECK_EQ(field_write_event_ref_count_.GetCounter(), 0U);
    CHECK_EQ(exception_catch_event_ref_count_.GetCounter(), 0U);
  }
  ...
  if (gDebuggerActive) {
    {
      ...
      full_deoptimization_event_count_.Reset();
...
ReferenceCounter* Dbg::GetReferenceCounterForEvent(uint32_t instrumentation_event) {
...
void Dbg::DelayFullUndeoptimization() {
  ...
  DCHECK_LE(delayed_full_undeoptimization_count_, full_deoptimization_event_count_.GetCounter());
}
...
void Dbg::RequestDeoptimizationLocked(const DeoptimizationRequest& req) {
  switch (req.kind) {
    case DeoptimizationRequest::kRegisterForEvent: {
      ...
      ReferenceCounter* counter = GetReferenceCounterForEvent(req.instrumentation_event);
    ...
    case DeoptimizationRequest::kFullDeoptimization: {
      ...
      if (full_deoptimization_event_count_.Increment()) {
    ...
    case DeoptimizationRequest::kFullUndeoptimization: {
      DCHECK(req.method == nullptr);
      if (full_deoptimization_event_count_.Decrement()) {
        VLOG(jdwp) << "Queue request #" << deoptimization_requests_.size()
