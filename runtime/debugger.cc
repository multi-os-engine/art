...
jbyteArray Dbg::GetRecentAllocations() {
  ...
  std::vector<uint8_t> bytes;
  {
    ...
    for (auto it = records->RBegin(), end = records->REnd();
      ...
      const gc::AllocRecord* record = it->second;
      std::string temp;
      mirror::Class* klass = record->GetClass();
      const char* class_name = UNLIKELY(klass == nullptr) ? "null" : klass->GetDescriptor(&temp);
    ...
    for (auto it = records->RBegin(), end = records->REnd();
      ...
      mirror::Class* klass = record->GetClass();
      const char* class_name = UNLIKELY(klass == nullptr) ? "null" : klass->GetDescriptor(&temp);
      size_t allocated_object_class_name_index = class_names.IndexOf(class_name);
