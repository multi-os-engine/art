...
struct DeoptimizationRequest {
  ...
  DeoptimizationRequest(bool enable, mirror::ArtMethod* method)
    : enable_(enable), method_(method) {}
};
static std::vector<DeoptimizationRequest> gDeoptimizationRequests GUARDED_BY(Locks::deoptimization_lock_);
...
static void ProcessDeoptimizationRequests()
  ...
  for (const DeoptimizationRequest& request : gDeoptimizationRequests) {
    mirror::ArtMethod* const method = request.method_;
    if (method != nullptr) {
...
void Dbg::EnableFullDeoptimization() {
  ...
  gDeoptimizationRequests.push_back(DeoptimizationRequest(true, nullptr));
}
...
void Dbg::DisableFullDeoptimization() {
  ...
  gDeoptimizationRequests.push_back(DeoptimizationRequest(false, nullptr));
}
...
void Dbg::WatchLocation(const JDWP::JdwpLocation* location) {
  ...
  if (need_deoptimization) {
    ...
    MutexLock mu(Thread::Current(), *Locks::deoptimization_lock_);
    gDeoptimizationRequests.push_back(DeoptimizationRequest(true, m));
...
void Dbg::UnwatchLocation(const JDWP::JdwpLocation* location) {
  ...
  if (can_undeoptimize) {
    ...
    MutexLock mu(Thread::Current(), *Locks::deoptimization_lock_);
    gDeoptimizationRequests.push_back(DeoptimizationRequest(false, m));
