...
inline void Instruction::GetVarArgs25x(uint32_t arg[kMaxVarArgRegs]) const {
  DCHECK_EQ(FormatOf(Opcode()), k25x);
   ...
   *   op vC, {vD, vE, vF, vG}
   *
   *  and vB is the register count (0-4) which denotes how far from vD to vG to read.
   *
  ...
  switch (count) {
    ...
    case 3:
      arg[3] = (regList >> 12) & 0x0f;  // vF
      FALLTHROUGH_INTENDED;
    case 2:
      arg[2] = (regList >> 8) & 0x0f;  // vE
      FALLTHROUGH_INTENDED;
    case 1:
      arg[1] = (regList >> 4) & 0x0f;  // vD
      FALLTHROUGH_INTENDED;
    default:  // case 0
      arg[0] = VRegC_25x();  // vC
      break;
