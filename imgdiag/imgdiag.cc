...
class ImgDiagDumper {
 ...
 private:
  ...
  bool DumpImageDiffMap(pid_t image_diff_pid, const backtrace_map_t& boot_map)
    ...
    struct ClassData {
      int dirty_object_count;

      // Track only the byte-per-byte dirtiness (in bytes)
      int dirty_object_byte_count;

      // Track the object-by-object dirtiness (in bytes)
      int dirty_object_size_in_bytes;

      int clean_object_count;

      std::string descriptor;

      int false_dirty_byte_count;
      int false_dirty_object_count;
  ...
  static std::vector<std::pair<V, K>> SortByValueDesc(const std::map<K, D> map,
                                                      std::function<V(const D&)> f) {
    // Store value->key so that we can use the default sort from pair which
    // sorts by value first and then key
    std::vector<std::pair<V, K>> value_key_vector;

    for (const auto& kv_pair : map) {
      value_key_vector.push_back(std::make_pair(f(kv_pair.second), kv_pair.first));
    }
