...
class ImgDiagDumper {
 ...
 private:
  ...
  bool DumpImageDiffMap(pid_t image_diff_pid, const backtrace_map_t& boot_map)
    ...
    if (!map_file->ReadFully(&remote_contents[0], boot_map_size, boot_map.start)) {
  ...
  static bool GetPageFrameNumber(File* page_map_file,
    ...
    if (!page_map_file->ReadFully(&page_map_entry, kPageMapEntrySize,
  ...
  static int IsPageDirty(File* page_map_file,
    ...
    if (!kpage_flags_file->ReadFully(&kpage_flags_entry,
                                     kPageFlagsEntrySize,
    ...
    if (!kpage_count_file->ReadFully(page_count /*out*/,
                                     kPageCountEntrySize,
